 ...
 *
 * <p>使用 remi-common-cache 本地缓存 JWT 解析结果，避免每个请求重复执行
...
public class CachedJwtValidator {
    ...
    private static final String INVALIDATION_CHANNEL = "remi:gateway:jwt-cache:invalidate";
     ...
     * {@link com.remisoft.common.cache.api.CacheProtectionGuard#getWithProtection} 执行解析。
     *
     * <h3>防击穿（P0-2 增强）</h3>
     ...
     * </ol>
     *
     * <p>调用时机：在 {@link com.remisoft.gateway.filter.AuthGlobalFilter} 中，
     * 当检测到 Token 在黑名单中时调用。
     *
    ...
    /**
     * Token 脱敏（P0-5：复用 remi-common-safe 的 SensitiveUtil 统一脱敏策略）
     *
     * @param jwt JWT Token
     * @return 脱敏后的字符串
     */
