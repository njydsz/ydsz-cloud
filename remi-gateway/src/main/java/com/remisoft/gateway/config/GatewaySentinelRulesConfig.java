 ...
 * <p>规则存储在 Nacos 配置：
 * <ul>
 *   <li>{@code remi-gateway-sentinel-degrade.json} — 熔断降级规则</li>
 *   <li>{@code remi-gateway-sentinel-system.json} — 系统保护规则</li>
 * </ul>
...
public class GatewaySentinelRulesConfig {
     ...
     */
    @Value("${remi.gateway.sentinel.nacos-datasource-enabled:false}")
    private boolean nacosDatasourceEnabled;

    /**
     * Nacos 中 Sentinel 规则的 DataId（熔断降级）
     */
    @Value("${remi.gateway.sentinel.degrade-rule-data-id:remi-gateway-sentinel-degrade.json}")
    private String degradeRuleDataId;

    /**
     * Nacos 中 Sentinel 规则的 DataId（系统保护）
     */
    @Value("${remi.gateway.sentinel.system-rule-data-id:remi-gateway-sentinel-system.json}")
    private String systemRuleDataId;

    /**
     * Nacos 中 Sentinel 规则的 Group
     */
    @Value("${remi.gateway.sentinel.rule-group:${spring.cloud.nacos.config.group:DEFAULT_GROUP}}")
    ...
    public void init() {
        ...
        if (nacosDatasourceEnabled) {
            ...
            log.info("[SentinelRules] Nacos 数据源未启用 (remi.gateway.sentinel.nacos-datasource-enabled=false)，仅使用代码内默认规则");
        }
    }
    ...
    private void registerDefaultDegradeRules() {
        ...
        exceptionRatioRule.setResource("remi-userinfo");
        ...
        slowCallRule.setResource("remi-system");
    ...
    private void registerNacosDataSources() {
        ...
        String namespace = System.getProperty(PropertyKeyConst.NAMESPACE,
                System.getenv().getOrDefault("NACOS_NAMESPACE", "remi"));
