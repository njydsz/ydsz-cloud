...
public class GatewayCorsConfig {
     ...
     * 注册响应式 CORS 过滤器
     *
     * <p>当 {@code remi.gateway.cors.enabled=false} 时跳过（保留原 OPTIONS 放行逻辑）。
     *
     * @param corsProperties CORS 配置属性
     * @return CorsWebFilter Bean
     */
    @Bean
    @ConditionalOnProperty(prefix = "remi.gateway.cors", name = "enabled", havingValue = "true", matchIfMissing = true)
