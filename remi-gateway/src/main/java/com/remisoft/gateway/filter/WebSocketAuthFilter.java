 ...
 *   <li>P0-4: Origin 校验（防 CSRF / 跨域 WebSocket 劫持）</li>
 *   <li>提取 Token → 校验 → 注入 X-User-* 内部头</li>
 *   <li>P0-1: 注入 X-Internal-Sig + X-Internal-Ts + X-Internal-Nonce 签名头
 *       （与 AuthGlobalFilter 一致，下游可统一校验）</li>
 *   <li>校验失败返回 401（在握手阶段拒绝，不建立连接）</li>
 * </ol>
 *
 * <h3>P0-1 安全加固</h3>
 * <p>历史版本仅注入用户信息头，未注入签名头，导致 WebSocket 请求可被下游伪造。
 * 本版本复用 {@link InternalHeaderSigner} 生成 HMAC 签名，与 AuthGlobalFilter 共用密钥与算法。
 *
 * <h3>P0-4 Origin 校验</h3>
 * <p>WebSocket 不受 SOP / CORS 约束，攻击者可通过构造恶意页面发起 WebSocket 跨域连接
 * 携带受害者 Cookie/Token。本版本校验 Origin 是否在允许列表中，仅放行可信域。
 *
 * <h3>执行顺序</h3>
...
public class WebSocketAuthFilter implements GlobalFilter, Ordered {
    ...
    /**
     * P0-1: 内部头签名密钥（与 {@link AuthGlobalFilter} 共用配置项）。
     *
     ...
     *
     * <p>逗号分隔，支持通配符（如 {@code https://*.remi.example.com}）。
     * 配置为空时表示不启用 Origin 校验（仅限 dev 环境，prod 必须配置）。
     */
    @Value("${remi.gateway.websocket.allowed-origins:}")
