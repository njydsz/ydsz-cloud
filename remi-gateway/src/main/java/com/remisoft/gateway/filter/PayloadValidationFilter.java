...
public class PayloadValidationFilter implements GlobalFilter, Ordered {
    ...
    @Value("${remi.gateway.payload-validation.enabled:true}")
    private boolean enabled;

    @Value("${remi.gateway.payload-validation.max-body-size-mb:10}")
    private int maxBodySizeMb;

    @Value("${remi.gateway.payload-validation.max-json-depth:50}")
    private int maxJsonDepth;

    @Value("${remi.gateway.payload-validation.strict-content-type:true}")
