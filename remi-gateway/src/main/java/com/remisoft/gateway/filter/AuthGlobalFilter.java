...
import com.remisoft.gateway.config.CachedJwtValidator;
import com.remisoft.gateway.config.GatewayConstants;
import com.remisoft.gateway.config.InternalHeaderSigner;
import com.remisoft.gateway.config.PathGuard;
 ...
 * <p>历史版本中，{@code internalSignSecret} 复用 {@code remi.jwt.secret}，导致一旦 JWT 密钥泄漏，
 * 攻击者可伪造内部头绕过下游服务的身份信任。本版本引入独立的
 * {@code remi.gateway.internal-sign-secret} 配置项，与 JWT 密钥隔离。
 *
 * <h3>P0-6: nonce 防重放</h3>
 * <p>历史版本仅有时间戳防重放（60 秒窗口），攻击者可在窗口内重放同一签名。
 * 本版本为每个请求生成唯一 nonce（UUID），纳入 HMAC 签名 payload 后透传给下游。
 * 下游服务调用 {@link com.remisoft.common.safe.crypto.NonceCache#verifyAndConsume(String)}
...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
     ...
     *
     * <p>历史版本硬编码 {@code TOKEN_BLACKLIST_PREFIX = "remi:token:blacklist:"} 并直接使用
     ...
     *
     * <p>配置项：{@code remi.gateway.internal-sign-secret}
     * 环境变量：{@code REMI_GATEWAY_INTERNAL_SIGN_SECRET}
     *
     * <p>历史兼容：如未配置，回退到 {@code remi.jwt.secret}（启动时记录 WARN 日志提醒运维分离密钥）。
     * 这种回退仅作为过渡，后续版本将强制要求独立配置。
     */
    @Value("${remi.gateway.internal-sign-secret:${remi.jwt.secret:}}")
    ...
    private void validateSecret() {
        if (internalSignSecret == null || internalSignSecret.isEmpty()) {
            log.error("[AuthFilter] 内部头签名密钥未配置 (remi.gateway.internal-sign-secret)，"
