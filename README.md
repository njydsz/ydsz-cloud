...
| 维度 | 数字 | 说明 |
|---|---|---|
| 后端微服务 | **15 个** | gateway / common / auth / user / config / file / audit / message / notification / workflow / scheduler / project / execution / agent / **literule** + 父 pom |
| 后端 Java 源文件 | **870+** | 业务代码 + DTO/VO/Mapper/Test |
| 后端测试 | **229 测试类 / 1500+ 用例** | `mvn test` BUILD SUCCESS 跨 15 模块 |
| 前端页面 | **57 个** | 业务页面 + 设计器 + 监控中心 |
| 前端测试 | **54 文件 / 470+ 用例** | vitest 单元 + 组件 + Playwright 4 E2E |
| Controller 数 | **80+** | execution 33 + project 8 + user 15 + workflow 3 + 其余 21+ |
| 业务 Service | **130+** | execution 30 + project 8 + user 29 + agent 12 + workflow 8 + 其余 40+ |
...
| 公共 | ydsz-pmis-common | — | 统一响应 / AOP / 注解 / Feign / 敏感数据 / JobHandler / Sentry / I18n / 权限码 / 混沌 |
| 认证 | ydsz-pmis-auth | **9001** | 登录 / Token / 2FA / 登录审计 / 二次认证 |
| 用户与资源 | ydsz-pmis-user | **9002** | 用户 / 角色 / 部门 / 考勤 / **资源池** / **Bench** / 任务调度 |
| 通知 | ydsz-pmis-notification | **9003** | 站内消息 / 邮件 / 短信 / 推送 / 工作流事件 |
| 工作流 | ydsz-pmis-workflow | **9004** | 自研 `pmis_flow_*` 引擎 + BPMN 2.0 解析 + 模板 + 模拟 |
| 项目 | ydsz-pmis-project | **9005** | 商机 / 立项 / 合同 / 补充 / 变更 / 模板 / 风险 |
| 执行 + 财务 + 报表 | ydsz-pmis-execution | **9006** | WBS / 工时 / 成本 / EVM / 双费率 / **开票/回款/信用** / **驾驶舱/高级报表** / BFF 聚合 / 全文搜索 |
| AI Agent | ydsz-pmis-agent | **9007** | 5 Agent + 4 编排 + 5 LLM Provider |
| 配置中心 | ydsz-pmis-config | **9008** | 枚举 / 字典 / 系统配置 / 特性开关 / 混沌配置 |
| 文件 | ydsz-pmis-file | **9009** | MinIO / OSS 上传下载 / 病毒扫描 / 分片 |
| 审计 | ydsz-pmis-audit | **9010** | 操作 / 登录 / 导出 / 敏感 4 类审计 + Diff |
| 消息模板 | ydsz-pmis-message | **9011** | 邮件 / 短信 / 站内 `${var}` 模板 |
| 调度 | ydsz-pmis-scheduler | **9012** | XXL-JOB 调度 + JobHandler 注册 |
| 轻量规则引擎 | ydsz-pmis-literule | — | 表达式驱动 + 规则链 + 阈值注入 + dry-run（批次 21 引入） |

> **架构决策（2026-07-01 修订）**: 原规划 11 微服务，落地时合并 finance/resource/report 到 execution/user/execution（过度拆分导致运维成本与跨服务调用复杂度反增），最终交付 14 模块；批次 21 抽取 literule 作为独立的轻量规则引擎模块，**最终 15 模块**。

### 4.3 模块依赖拓扑

```text
gateway → auth / user / project / execution / agent / notification / file / config / workflow
execution → project / user(Feign) / message / common / scheduler(JobHandler) / literule
project   → user(Feign) / workflow(Feign) / common
agent     → common / project(Feign) / execution(Feign) / literule
audit     → common / user(Feign)
message   → common
workflow  → common / notification(Feign)
...
# 3. 启动后端 (按依赖顺序)
mvn -pl ydsz-pmis-common,ydsz-pmis-literule,ydsz-pmis-user,ydsz-pmis-auth \
...
```text
ydsz-pmis/
├── ydsz-pmis-backend/          # 后端 15 模块聚合工程
│   ├── ydsz-pmis-gateway/      # 9000 API 网关
│   ├── ydsz-pmis-common/       # 公共组件 (80+ 测试类)
│   ├── ydsz-pmis-auth/         # 9001 认证 / 2FA / 审计
│   ├── ydsz-pmis-user/         # 9002 用户/资源/Bench
│   ├── ydsz-pmis-config/       # 9008 枚举/字典/特性开关/混沌
│   ├── ydsz-pmis-file/         # 9009 文件 / 病毒扫描 / 分片
│   ├── ydsz-pmis-audit/        # 9010 4 类审计 / Diff
...
│   ├── ydsz-pmis-execution/    # 9006 执行/财务/报表 (30 Service)
│   ├── ydsz-pmis-agent/        # 9007 AI Agent
│   └── ydsz-pmis-literule/     # --  轻量规则引擎
...
| 维度 | 指标 / 工具 | 阈值 / 现状 |
|---|---|---|
| 后端单测 | JUnit 5 + Mockito + AssertJ | 15 模块 100% 通过 |
