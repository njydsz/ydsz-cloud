...
-- 自动更新 updated_at（原 MySQL ON UPDATE CURRENT_TIMESTAMP）
CREATE OR REPLACE FUNCTION fn_ydsz_job_tenant_quota_set_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at := CURRENT_TIMESTAMP;
    RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_ydsz_job_tenant_quota_updated_at ON ydsz_job_tenant_quota;
CREATE TRIGGER trg_ydsz_job_tenant_quota_updated_at
BEFORE UPDATE ON ydsz_job_tenant_quota
FOR EACH ROW
EXECUTE FUNCTION fn_ydsz_job_tenant_quota_set_updated_at();
