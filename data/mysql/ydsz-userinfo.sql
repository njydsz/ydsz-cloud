...
CREATE TABLE IF NOT EXISTS ydsz_acct_user (
    ...
    dept_id         VARCHAR(32)     DEFAULT NULL COMMENT '所属部门 ID（关联 ydsz_org_department.id，支持 dept: 审批人展开）',
    leader_id       VARCHAR(32)     DEFAULT NULL COMMENT '直属上级用户 ID（关联 ydsz_acct_user.id，支持 leader: 审批人展开）',
...
CREATE TABLE IF NOT EXISTS ydsz_acct_login_history (
    id              VARCHAR(32)     PRIMARY KEY COMMENT '主键 ID（Snowflake）',
    user_id         VARCHAR(32)     DEFAULT NULL COMMENT '用户 ID（关联 ydsz_acct_user.id）',
