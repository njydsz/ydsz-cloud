...
CREATE TABLE ydsz_agt_definition (
    ...
    model_config             CLOB                     DEFAULT NULL CONSTRAINT ck_ydsz_agt_definition_model_config CHECK (model_config IS JSON),
    tool_names               CLOB                     DEFAULT NULL CONSTRAINT ck_ydsz_agt_definition_tool_names CHECK (tool_names IS JSON),
...
CREATE INDEX idx_ydsz_agt_trace_trace_conversation ON ydsz_agt_trace (conversation_id);
CREATE INDEX idx_ydsz_agt_trace_trace_agent ON ydsz_agt_trace (agent_id);
...
CREATE TABLE ydsz_agt_trace_step (
    ...
    input_json               CLOB                     DEFAULT NULL CONSTRAINT ck_ydsz_agt_trace_step_input_json CHECK (input_json IS JSON),
...
CREATE OR REPLACE TRIGGER trg_ydsz_agt_prompt_template_updated_at
...
-- 自动更新 updated_at（原 MySQL ON UPDATE CURRENT_TIMESTAMP）
CREATE OR REPLACE TRIGGER trg_ydsz_agt_definition_updated_at
...
-- 自动更新 updated_at（原 MySQL ON UPDATE CURRENT_TIMESTAMP）
CREATE OR REPLACE TRIGGER trg_ydsz_agt_token_usage_updated_at
