 ...
 * 系统配置版本管理 Mapper
 *
 * <p>对应数据表 <code>ydsz_config_version</code>。
 *
 * <p>配置变更（save / updateById / removeById）生成新版本，支持回滚、对比、审计。
 *
 ...
 * @see com.njydsz.system.server.service.ConfigVersionService 配置版本 Service
...
public interface ConfigVersionMapper extends BaseMapper<ConfigVersion> {
   ...
   */
  @Select(
      "SELECT * FROM ydsz_config_version WHERE resource_key = #{resourceKey} AND deleted = 0 "
          + "ORDER BY effective_date DESC")
  List<ConfigVersion> listByResourceKey(@Param("resourceKey") String resourceKey);
   ...
   */
  @Select(
      "SELECT * FROM ydsz_config_version WHERE resource_key = #{resourceKey} "
          + "AND version = #{version} AND deleted = 0 LIMIT 1")
  ConfigVersion selectByKeyAndVersion(
