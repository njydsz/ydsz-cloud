 ...
 * <p><b>配置项总览：</b>
 * <ul>
 *   <li>{@code ydsz.system.health-enabled}：是否启用系统模块健康检查（默认 true）</li>
 *   <li>{@code ydsz.system.config.cache-ttl-minutes}：配置缓存 TTL（默认 5）</li>
 *   <li>{@code ydsz.system.dict.cache-ttl-minutes}：字典缓存 TTL（默认 10）</li>
 *   <li>{@code ydsz.system.variable.cache-ttl-minutes}：变量缓存 TTL（默认 5）</li>
 *   <li>{@code ydsz.system.app.bcrypt-strength}：应用密钥 BCrypt 加密强度（默认 10）</li>
 *   <li>{@code ydsz.system.internal-api-ip-whitelist}：内部 API IP 白名单（空=不限制）</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
