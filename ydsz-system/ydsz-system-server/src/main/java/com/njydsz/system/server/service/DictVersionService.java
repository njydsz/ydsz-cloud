 ...
 *
 * <ul>
 *   <li><b>版本记录</b>：{@link #createVersion} — 字典项增删改时自动调用，记录变更前的全量快照
 *   <li><b>历史查询</b>：{@link #listByTypeCode} — 管理后台「字典版本管理」数据源
 *   <li><b>回滚支持</b>：快照数据可一键回滚到任意历史版本
 * </ul>
 *
 * <p><b>版本生成策略：</b>版本号默认 {@code "v" + System.currentTimeMillis()}， 满足「按时间排序」需求；如需语义化版本号（如 {@code
 * v1.2.0}），调用方可在 {@link #createVersion} 传入自定义版本字符串。
 *
 * <p><b>快照机制：</b>{@code snapshotJson} 保存变更前全量字典项 JSON 字符串， 大小一般 < 1MB；超过时需在调用方自行压缩。
 *
...
public interface DictVersionService {
   ...
   * 创建版本快照
   *
   * <p>由 {@link DictItemService} 在写操作（{@code save / updateById / removeById}）成功后异步调用。 {@code
   * snapshotJson} 一般为变更前的字典项列表 JSON 字符串（{@code null} 表示空快照）。
   *
   * @param typeCode 字典类型编码
   ...
   *   <li>物理删除当前字典项（按 typeCode）
   *   <li>从 {@code snapshotJson} 反序列化并批量插入历史字典项
   *   <li>创建新版本记录（标记回滚来源，保持完整审计链）
   *   <li>失效该 typeCode 下所有缓存
   * </ol>
   *
   * <p><b>审计设计：</b>回滚操作创建一个<b>新版本</b>而非覆盖历史， 新版本 {@code changeLog} 标注「回滚自 {sourceVersion}」，
   * 旧版本永远不变（不可变记录原则）。
   *
   * <p><b>快照兼容性：</b>若 {@code snapshotJson} 为空或解析失败， 将清空当前字典项但不再重建（行为等同「重置为空字典」）。
   *
   * @param typeCode 字典类型编码
