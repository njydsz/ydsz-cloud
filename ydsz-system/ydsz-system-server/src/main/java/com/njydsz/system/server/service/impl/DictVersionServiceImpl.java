 ...
 * 字典版本 Service 实现
 *
 * <p>对 {@link DictVersionService} 接口的完整实现，是「字典中心」版本管理子系统的核心业务逻辑层。 维护字典变更历史快照（schema + 全量字典项
 * JSON），对标大厂「配置中心 / 字典中心」版本管理能力， 支持版本回滚、变更审计、配置复盘等场景。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li><b>版本查询</b>：{@link #listByTypeCode} — 按 {@code typeCode} 查询某字典类型的所有历史版本， 按 {@code
 *       effective_date} 倒序返回（最新版本在前）
 *   <li><b>版本创建</b>：{@link #createVersion} — 由 {@link DictItemServiceImpl} 在写操作成功后调用，
 *       记录变更前的<b>全量字典项 JSON 快照</b>，写入 {@code ydsz_dict_version} 表
 *   <li><b>版本回滚</b>：未来扩展 — 通过 {@code snapshotJson} 重建字典项（{@code restoreVersion}），
 *       当前仅保留快照数据，回滚由管理后台基于快照二次开发
 * </ul>
 *
 * <p><b>事务边界：</b>
 *
 ...
 *   <li>{@link #createVersion} 通常由 {@link DictItemServiceImpl#save} / {@link
 *       DictItemServiceImpl#updateById} / {@link DictItemServiceImpl#removeById} 触发，
 *       <b>必须在同一事务边界内</b>调用，确保字典项变更与版本快照原子性
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>不可变记录</b>：版本记录<b>仅新增</b>，不修改 / 不删除（保留完整审计链）
 *   <li><b>版本号生成</b>：调用方传入（典型格式 {@code "v" + System.currentTimeMillis()}）， 内部不做格式校验
 *   <li><b>快照大小</b>：典型字典快照 < 1MB，无需压缩； 极端字典（如行政区划 70w+ 项）由调用方在写入前自行压缩
 *   <li><b>软删除</b>：{@code ydsz_dict_version} 表采用 <b>逻辑删除</b>（{@code deleted} 字段）， 与物理删除不同
 * </ul>
 *
 * <p><b>典型使用：</b>
 *
...
public class DictVersionServiceImpl implements DictVersionService {
   ...
   *
   * <ul>
   *   <li>索引：{@code (type_code, deleted, effective_date DESC)}
   *   <li>单字典类型历史版本一般 < 100 条，单次查询 < 10ms
   *   <li>若某字典频繁变更（> 1000 版本），建议按时间区间分页查询（{@code listByTypeCode} 未来扩展）
   * </ul>
   *
   ...
   * 创建字典版本快照
   *
   * <p>由 {@link DictItemServiceImpl} 在字典项变更（增 / 删 / 改）后调用， 记录变更前的<b>全量字典项 JSON 快照</b>，用于版本回滚和变更审计。
   *
   * <p><b>关键设计：</b>
   *
   * <ul>
   *   <li><b>事务一致性</b>：调用方需在<b>字典项变更事务</b>内调用本方法， 通过 Spring 事务传播保证原子性（{@code PROPAGATION_REQUIRED}）
   *   <li><b>快照时机</b>：必须在<b>变更前</b>查询并快照原字典项， 而非变更后（否则快照反映的是变更后的状态，无法回滚）
   *   <li><b>版本号语义</b>：{@code version} 字段由调用方决定格式（典型：{@code "v" + 时间戳}）， 本方法不做格式校验
   *   <li><b>生效时间</b>：{@code effectiveDate} 自动取当前时间
   * </ul>
   *
  ...
  public String createVersion(
      String typeCode, String version, String changeLog, String snapshotJson) {
   ...
   * 回滚字典到指定版本
   *
   * <p>事务边界内执行「查询快照 → 物理删除 → 批量插入 → 创建新版本 → 失效缓存」全链路。 若中间步骤失败，整个事务回滚，字典数据保持原状。
   *
  ...
  public String rollbackTo(String typeCode, String targetVersion, String operatorId) {
    ...
    if (StringUtils.isNotBlank(snapshotJson)) {
      try {
        List<DictItemDTO> snapshotItems =
            YdszJson.fromJson(snapshotJson, java.util.List.class, DictItemDTO.class);
        ...
        log.error(
            "[DictVersion] 快照解析失败: typeCode={}, version={}, error={}",
            typeCode,
            targetVersion,
            e.getMessage());
    ...
    log.info(
        "[DictVersion] 回滚完成: typeCode={}, targetVersion={}, newVersion={}, 恢复 {} 条字典项",
        typeCode,
        targetVersion,
        newVersion,
        insertedCount);
