 ...
 *
 * <p>对 {@code /api/internal/**} 路径的请求进行 IP 白名单校验，防御未授权的跨服务调用。 当 {@code
 * ydsz.system.internal-api-ip-whitelist} 配置为空时<b>不限制</b>（允许所有 IP 访问）。
 *
 * <p><b>升级说明（P0-3）：</b>
 *
 * <ul>
 *   <li>原实现使用自建精确匹配白名单，不支持 CIDR 网段
 *   <li>现委托 {@link IpAccessService}（common-safe 模块），获得 CIDR 网段匹配能力
 *   <li>白名单通过 {@code ydsz.safe.ip-access.static-whitelist} 配置（支持 CIDR 格式）
 *   <li>同时具备 Redis 持久化能力（动态白名单运维接口），满足集群环境一致性
 * </ul>
 *
 * <p><b>典型配置（生产）：</b>
 *
 ...
 * </pre>
 *
 * <p><b>客户端 IP 解析：</b>委托 {@link ClientIpResolver#getClientIp(HttpServletRequest)} 统一解析（含
 * X-Forwarded-For / RemoteAddr 多级回退与防伪造校验）。
 *
...
public class InternalApiIpFilter {
   ...
   *
   * <ul>
   *   <li><b>URL 模式</b>：{@code /api/internal/*}（单星号通配一层，匹配 {@code /api/internal/xxx}）
   *   <li><b>优先级</b>：{@link Ordered#HIGHEST_PRECEDENCE} + 10，确保在鉴权 Filter 之前执行
   *   <li><b>Bean 名称</b>：{@code internalApiIpFilter}，便于在监控平台识别
   * </ul>
   *
   ...
   *
   * <ol>
   *   <li>委托 {@link IpAccessService#isAllowed(String)} 执行校验
   *   <li>校验不通过则返回 403
   *   <li>通过则继续 Filter 链
   * </ol>
   *
  ...
  private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
    ...
    if (!ipAccessService.isAllowed(clientIp)) {
      ...
      response.sendError(HttpServletResponse.SC_FORBIDDEN, "Access denied: IP not in whitelist");
      return;
    }
