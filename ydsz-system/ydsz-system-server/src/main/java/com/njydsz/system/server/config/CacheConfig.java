 ...
 *
 * <p>启用 Spring Cache 注解驱动（{@code @Cacheable} / {@code @CacheEvict}），
 * 以 ydsz-common-cache 的 {@link YdszCacheManager} 作为缓存管理器，
 * 替代原有的 {@code RedisStringOps} 手动缓存管理模式：
 * <ul>
 *   <li>统一缓存编程模型，消除各 ServiceImpl 中重复的 cache-aside 样板代码</li>
 *   <li>利用 ydsz-common-cache 内置的防穿透能力，无需手动维护空值哨兵</li>
 *   <li>跨实例缓存一致性由 {@code ConfigServiceImpl} OutboxService 事件广播 +
 *       {@code CrossModuleEventListener} 本地缓存失效机制保证</li>
 * </ul>
 *
 * <p><b>本地缓存 vs Redis：</b>ydsz-common-cache 为进程内本地缓存（无网络 IO），
 * TTL 和容量通过 {@code ydsz.cache.caches} YAML 配置。跨实例一致性通过
 * OutboxService 事件实现：写操作发布 {@code CONFIG_CHANGED} 事件，
 * 各实例的 {@code CrossModuleEventListener} 接收到事件后清除本地缓存。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class CacheConfig {
     ...
     *
     * <p>当 classpath 同时存在 spring-data-redis 和 ydsz-common-cache 时，
     * 显式声明 {@code @Primary} 的 {@link YdszCacheManager} 确保 Spring Cache
     * 注解使用 ydsz-common-cache（本地缓存）而非 RedisCacheManager。
     *
    ...
    public YdszCacheManager cacheManager(YdszCacheProperties ydszCacheProperties) {
        ...
        manager.setExpireAfterWrite(ydszCacheProperties.getExpireAfterWrite(),
                ydszCacheProperties.getExpireTimeUnit());
        ...
        manager.setExpireAfterAccess(ydszCacheProperties.getExpireAfterAccess(),
                ydszCacheProperties.getExpireTimeUnit());
        manager.setRefreshAfterWrite(ydszCacheProperties.getRefreshAfterWrite(),
                ydszCacheProperties.getExpireTimeUnit());
