 ...
 *   <li><b>密钥校验</b>：{@link #validateClient} — OAuth 2.0 客户端凭证模式核心入口， 使用 BCrypt 匹配（不可逆）
 *   <li><b>分页查询</b>：{@link #page} — 管理后台「应用管理」列表
 ...
 *
 * <ul>
 *   <li>{@code appSecret} 在 Service 层 BCrypt 加密后存储，<b>明文永远不落库</b>
 *   <li>VO 中<b>不暴露</b> {@code appSecret} 字段（即便 BCrypt 哈希也不应泄露）
 *   <li>{@link #validateClient} 调用方需做 <b>QPS 限流</b>（{@code @RateLimit}）防爆破
 *   <li>连续失败 N 次后锁定账号（{@code AppInfo.lockedUntil} 字段）
 * </ul>
 *
...
public interface AppInfoService {
   ...
   * 校验应用密钥（BCrypt）
   *
   * <p>OAuth 2.0 客户端凭证模式核心入口；调用方需配合 <b>QPS 限流</b>（{@code @RateLimit}）和 <b>失败锁定</b>策略防爆破。
   *
   ...
   * 删除应用（逻辑删除）
   *
   * <p>基于 MyBatis-Plus 逻辑删除（{@code @TableLogic}），不物理删除。 删除后已签发的 access_token 在 Redis 黑名单 TTL
   * 到期前仍可使用。
   *
   * @param id 主键 ID
   * @return 是否成功
