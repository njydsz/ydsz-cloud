 ...
 * 字典项创建/更新 DTO
 *
 * <p>对应 {@code ydsz_dict_item} 表的写入参数。字典项是「字典类型」下的具体条目， 通过 {@code typeCode} 外键关联 {@link
 * DictTypeDTO}，形成「类型 → 项」两级结构。
 *
 * <p><b>字段约束：</b>
 *
 * <ul>
 *   <li>{@code typeCode} — 所属字典类型编码，最长 64 字符
 *   <li>{@code itemCode} — 字典项编码，类型内唯一，最长 64 字符
 *   <li>{@code itemValue} — 字典项展示值（前端下拉框 / 单选框显示文本），最长 255 字符
 *   <li>{@code sortOrder} — 同类型内排序号（升序）
 *   <li>{@code parentId} — 父级字典项 ID（{@code 0} 表示根），支持树形结构（如「省 / 市 / 区县」）
 *   <li>{@code extJson} — 扩展属性 JSON（如地区码、颜色值、图标等）
 *   <li>{@code status} — 启用状态：{@code ENABLED / DISABLED}
 * </ul>
 *
 * <p><b>唯一约束：</b>（{@code tenantId}, {@code typeCode}, {@code itemCode}）三联唯一。
 *
 * <p><b>缓存策略：</b>字典项通过 {@code ydsz:dict:item:{typeCode}} 缓存至 Redis， 写入时 {@code @CacheEvict} 主动失效。
 *
 * @author ydsz-team
 * @since 1.0.0
