 ...
 * 系统变量创建/更新 DTO
 *
 * <p>对应 {@code ydsz_variable} 表的写入参数，承担「系统级键值对参数」的 CRUD 入参。 与 {@link ConfigDTO} 的区别：{@code Config}
 * 强绑定分组（{@code configGroup}）， {@code Variable} 是<b>全局</b>无分组的扁平结构，更适合「跨模块共享的环境变量」场景。
 *
 * <p><b>字段约束：</b>
 *
 * <ul>
 *   <li>{@code variableKey} — 变量键，租户内唯一，最长 128 字符
 *   <li>{@code variableValue} — 变量值（按 {@code valueType} 解析）
 ...
 *
 * <p><b>缓存策略：</b>读取时通过 {@code ydsz:variable:{key}} 缓存至 Redis； 写入时 {@code @CacheEvict} 主动失效。常用于业务代码中
