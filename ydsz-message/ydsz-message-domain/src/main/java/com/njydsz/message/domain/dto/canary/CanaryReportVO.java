 ...
 * 灰度 A/B 实验报表（P1-6）。
 *
 * <p>对照组（control）= 未命中灰度的消息：{@code template_code = canaryKey AND canary = 0 AND canary_key IS
 * NULL}。 实验组（treatment）= 命中灰度的消息：{@code canary_key = canaryKey}（canary=1 隐含）。 两组分别统计发送量 / 成功 / 失败 /
 * 重试 / 死信 / 送达 / 已读 / 点击 及对应比率, 供运营对比实验模板/通道与基线模板/通道的效果差异。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
