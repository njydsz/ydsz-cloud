 ...
 * 站内通知 Mapper
 *
 * <p>对应数据表 <code>ydsz_msg_notification</code>。
 *
 ...
 *
 * <ul>
 *   <li>uk_notification_id — 主键唯一索引
 *   <li>idx_user_unread — (用户+已读状态) 复合索引（未读列表）
 *   <li>idx_created_at — 创建时间排序索引
 * </ul>
 *
...
public interface MsgNotificationMapper extends BaseMapper<MsgNotification> {
   ...
   *
   * <p>P2-6: 增加 {@code batchSize} 参数实现分批 UPDATE，避免单次 UPDATE 万级未读通知导致的长事务与行锁堆积。调用方需循环调用直到返回值 &lt;
   * batchSize。
   *
   * @param userId 接收人 ID
