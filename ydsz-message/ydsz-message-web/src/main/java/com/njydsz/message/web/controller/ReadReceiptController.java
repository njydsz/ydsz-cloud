 ...
 *
 * <p>提供<b>邮件追踪像素</b>与<b>短信短链跳转</b>两类 HTTP 回调端点， 用于无登录态场景下收集用户对消息的「已读 / 点击」行为数据， 最终通过 {@link
 * com.njydsz.message.server.service.receipt.ReceiptService} 落地到 {@code ydsz_msg_receipt} 表，更新对应
 * {@code ydsz_msg_log} 的 {@code receiptStatus}。
 *
 ...
 *       邮件客户端加载图片时自动触发，标记消息为「已读」并返回 1x1 透明 PNG
 *   <li><b>短信短链跳转</b>：{@code GET /s/{shortCode}} — 短信中插入短链（避免长 URL 占用字符）, 用户点击时 302 重定向到原始
 *       URL，同时标记消息为「已点击」
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>本 Controller：<b>无登录态</b>，由邮件/短信的端点被动触发（pixel / short link）
 *   <li>ReadStatusController：<b>有登录态</b>，由用户在站内通知中心主动操作（markRead 等）
 * </ul>
 *
 * <p><b>透明 PNG（1x1）：</b>硬编码的 Base64 解码字节，用于邮件追踪回调返回， 避免引入图片资源依赖；最小 67 字节、所有邮件客户端均能正常渲染。
 *
 * <p><b>短链降级：</b>当 {@code handleShortLinkClick} 返回 {@code null}（短链不存在或已过期）， 返回 HTTP 404
 * 提示用户，而非重定向到错误页，避免引入外部跳转风险。
 *
 * <p><b>幂等性：</b>两个端点均按「同一 {@code msgId/shortCode} 多次触发只生效一次」处理， 即使被恶意刷新也不会造成重复计费/统计污染。
 *
 * @author ydsz-team
 * @since 1.0.0
 * @see com.njydsz.message.server.service.receipt.ReadReceiptService 已读回执服务
 */
@Slf4j
