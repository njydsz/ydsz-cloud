 ...
 *
 * <ul>
 *   <li>本 Controller：<b>通道联系方式</b>（用户「绑定」到该通道的具体账号）
 *   <li>PreferenceController：<b>接收偏好</b>（用户在该通道上「是否愿意接收」）
 * </ul>
 *
 ...
 *   <li>邮箱 / 手机号等敏感联系方式在落库前自动脱敏（{@code @Sensitive} 注解）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class UserChannelBindingController {
   ...
   *
   * <p>按 {@link AuthContextUtils#getUserId()} 当前用户寻址，结果已转为脱敏 VO。 需 {@code NOTIF_MESSAGE_LIST} 权限。
   *
   ...
   * 删除指定通道绑定。
   *
   * <p>解除某条用户通道绑定关系；需 {@code NOTIF_MESSAGE_SEND} 权限， 启用 5s 幂等防重与 50 QPS 限流，并异步记录审计日志。 {@code id}
   * 不存在时由服务层按未找到处理（不抛客户端异常）。
   *
