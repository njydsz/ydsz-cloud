 ...
 *
 * <p>提供<b>消息发送流程的 DAG 编排</b>能力，是 P1-9「复杂通知编排」的核心入口。 与单条消息发送不同，编排支持<b>多节点依赖、条件分支、失败策略</b>，
 ...
 *
 * <ul>
 *   <li><b>执行编排流程</b>：{@code POST /execute} — 提交 DAG 定义并同步/异步执行，返回执行结果
 * </ul>
 *
 * <p><b>DAG 节点：</b>编排流程由多个 {@code OrchestrationNode} 组成，每个节点可以是：
 *
 * <ul>
 *   <li><b>消息节点</b>：发送一条消息（指定模板 / 接收人 / 通道）
 *   <li><b>延时节点</b>：等待 N 秒 / 到指定时间后继续
 *   <li><b>条件节点</b>：基于上一步执行结果判断分支
 *   <li><b>回调节点</b>：调用外部 HTTP API（业务系统 webhook）
 * </ul>
 *
 * <p><b>失败策略：</b>每个节点可配置 {@code onFailure}：
 *
 * <ul>
 *   <li>{@code FAIL_FAST}：节点失败立即终止整个流程
 *   <li>{@code CONTINUE}：节点失败不影响后续节点（适合非关键节点）
 *   <li>{@code RETRY_N}：失败后自动重试 N 次
 * </ul>
 *
 * <p><b>典型场景：</b>
 *
 * <ul>
 *   <li>项目立项审批通过 → 通知审批人（站内信）+ 通知申请人（邮件）+ 通知财务（钉钉）
 *   <li>订单支付成功 → 等 5 秒查询支付回执 → 通知用户 + 更新订单状态
 *   <li>定时生日祝福：00:00 触发 → 查询当日生日用户 → 分批发送个性化祝福
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>写接口（execute）启用 {@link Idempotent} 5s 防重（避免重复执行同一 DAG）
 *   <li>写接口（execute）启用 {@link RateLimit} 50 QPS 限流
 *   <li>写接口（execute）启用 {@link Audit} 审计日志（异步持久化）
 *   <li>权限模型：通过 {@code @AuthApiPermission} 校验 {@link PermissionCodes#NOTIF_MESSAGE_SEND} 权限码
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
 * @see com.njydsz.message.server.service.core.OrchestrationService 编排服务
 * @see com.njydsz.message.domain.dto.core.OrchestrationFlowDTO 编排流程 DTO
 */
@Slf4j
...
public class OrchestrationController {
  ...
  @PostMapping("/execute")
  public BaseResponse<OrchestrationResultVO> execute(
      @Valid @RequestBody OrchestrationFlowDTO flow) {
