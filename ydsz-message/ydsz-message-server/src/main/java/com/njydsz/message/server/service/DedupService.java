 ...
 * 智能去重服务（P2-1）。
 *
 * <p>基于 Redis {@code SET NX EX} 原子操作实现短窗口去重：相同 dedupKey 的消息 在配置的 TTL 秒内仅允许通过一次，超时后自动释放。
 *
...
public interface DedupService {
   ...
   *
   * <ul>
   *   <li>dedupKey 为 null / 空白时直接返回 true（无去重维度，放行）
   *   <li>去重总开关关闭时直接返回 true
   *   <li>Redis 异常时 fail-open 返回 true 并记 WARN 日志
   * </ul>
   *
