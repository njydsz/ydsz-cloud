...
public class CanaryReportServiceImpl implements CanaryReportService {
   ...
   */
  private CanaryReportVO.GroupStats buildTreatmentStats(
      String canaryKey, LocalDateTime start, LocalDateTime end) {
   ...
   * <p>D-1: 从8次selectCount改为2次selectMaps GROUP BY查询。
   */
  private CanaryReportVO.GroupStats buildControlStats(
      String canaryKey, LocalDateTime start, LocalDateTime end) {
   ...
   */
  private Map<String, Long> countGroupByStatus(
      boolean isTreatment, String canaryKey, LocalDateTime start, LocalDateTime end) {
    ...
    if (isTreatment) {
      ...
      wrapper.eq("template_code", canaryKey).eq("canary", 0).isNull("canary_key");
    }
  ...
  private Map<String, Long> countGroupByReceipt(
      boolean isTreatment, String canaryKey, LocalDateTime start, LocalDateTime end) {
    QueryWrapper<MsgLog> wrapper = new QueryWrapper<>();
  ...
  /** D-1: 从分组Map组装统计结果。 */
  private CanaryReportVO.GroupStats assembleStatsFromMaps(
      Map<String, Long> statusCounts, Map<String, Long> receiptCounts) {
  ...
  /** 组装分组统计并计算比率。 */
  private CanaryReportVO.GroupStats assembleStats(
      long total,
      long success,
      long failed,
      long retry,
