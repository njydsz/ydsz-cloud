 ...
 * 用户通道绑定解析 Handler。
 *
 * <p>当 receiver 是 userId 而非通道原生联系方式时，自动解析为通道联系方式。
 * 例如：userId=123 → 手机号 138xxxx（SMS）或 user@example.com（EMAIL）。
 *
 * @author ydsz-team
 * @since 1.1.0
 */
...
public class ChannelBindingHandler implements SendHandler {
    ...
    public boolean handle(MessageRequest request, SendContext ctx) {
        ...
        if (resolved != null && !resolved.equals(receiver)) {
            log.debug("[Message] 通道绑定解析: userId={} channel={} -> channelUserId={}",
                    SensitiveUtil.scanAndMask(receiver), channel, resolved);
