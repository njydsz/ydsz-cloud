 ...
 * 限流与频率校验 Handler。
 *
 * <p>多维度限流：通道级 QPS + 接收人频率 + 模板频率 + 租户配额。
 * 限流触发时抛 {@link SysException}（TOO_MANY_REQUESTS），由上层统一处理。
 *
 * @author ydsz-team
 * @since 1.1.0
 */
...
public class RateLimitHandler implements SendHandler {
    ...
    public boolean handle(MessageRequest request, SendContext ctx) {
        ...
        if (!rateLimitService.checkSendLimit(channel, receiver, templateCode,
                ctx.getTenantId(), request.getPriority())) {
    ...
    private String buildRateLimitKey(String channel, String bizType) {
        return "channel:" + (channel != null ? channel : "UNKNOWN")
                + ":biz:" + (bizType != null ? bizType : "DEFAULT");
    }
}
