 ...
 * RocketMQ 消息生产者封装。
 *
 * <p>统一封装 {@link RocketMQTemplate} 同步/异步发送,自动生成雪花 messageId 保证消费端幂等。 条件装配:仅当 classpath 存在
...
public class RocketMQMessageProducer implements MessageQueueOperations {
  ...
  public RocketMQMessageProducer(
      RocketMQTemplate rocketMQTemplate, SnowflakeIdGenerator snowflakeIdGenerator) {
  ...
  public String syncSend(MessageRequest req) {
    ...
    try {
      ...
      log.error(
          "[Producer] syncSend 失败: messageId={} channel={} err={}",
          req.getMessageId(),
          req.getChannel(),
          e.getMessage());
      ...
      rocketMQTemplate.asyncSend(
          ...
          new SendCallback() {
            ...
            public void onSuccess(SendResult result) {
              log.info(
                  "[Producer] asyncSend OK: msgId={} messageId={}",
                  result.getMsgId(),
                  req.getMessageId());
            }
            ...
            public void onException(Throwable e) {
              log.error(
                  "[Producer] asyncSend 失败: messageId={} err={}",
                  req.getMessageId(),
                  e.getMessage());
            }
          });
   ...
   *
   * <ul>
   *   <li>URGENT → tag:URGENT（最高优先级，独立消费线程池）
   *   <li>HIGH → tag:HIGH
   *   <li>NORMAL → tag:NORMAL（默认）
   *   <li>LOW → tag:LOW
   * </ul>
   *
   * @param req 消息请求
   ...
   * 执行本地事务（校验模板/通道）,根据结果 COMMIT / ROLLBACK。 适用于业务侧需要确保通知请求仅在本地事务成功后才投递的场景。
   *
   * @param req 消息请求
    ...
    try {
      TransactionSendResult result =
          rocketMQTemplate.sendMessageInTransaction(
              buildDestination(req), MessageBuilder.withPayload(payload).build(), req);
      log.info(
          "[Producer] sendTransactionMessage: msgId={} messageId={} state={}",
          result.getMsgId(),
          req.getMessageId(),
