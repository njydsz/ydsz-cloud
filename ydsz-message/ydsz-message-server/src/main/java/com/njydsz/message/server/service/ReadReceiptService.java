 ...
 * 全通道已读回执 Service
 *
 * <p>为邮件和短信通道提供主动探测的"已读"回执能力。常规通道(IN_APP/PUSH/IM)的消息已读状态 由前端主动调用 {@code
 * ReadStatusSyncService.markRead} 触发,而邮件和短信因用户离线, 需通过链路追踪技术主动探测"用户是否打开/点击"。
 *
 * <p><b>实现方式：</b>
 *
 * <ul>
 *   <li><b>邮件：</b>在 HTML 正文末尾注入 1x1 透明 GIF 追踪像素 {@code <img src="/msg/track/{msgId}.gif"/>},
 *       用户打开邮件时浏览器请求该像素 URL 触发回执
 *   <li><b>短信：</b>将原始 URL 替换为短链 {@code https://s.ydsz.cn/{shortCode}},用户点击短链时 触发回执后 302 跳转到目标 URL
 * </ul>
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li><b>注入追踪元素</b>：{@link #injectEmailTrackingPixel} / {@link #generateShortLink}
