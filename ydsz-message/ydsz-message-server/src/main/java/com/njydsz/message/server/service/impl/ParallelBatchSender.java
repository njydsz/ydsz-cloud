 ...
 * P2-15: 并行批量发送 + 流控。
 *
 * <p>使用通道级线程池（Bulkhead）并行发送批量消息，同时通过 Semaphore 控制并发度， 避免瞬时高峰压垮下游服务商（如 SMS 网关、SMTP 服务器）。
 *
 * <p>特性：
 *
 * <ul>
 *   <li>并行发送：基于 {@link CompletableFuture} + 通道级线程池
 *   <li>流控：Semaphore 限制单批次最大并发数
 *   <li>超时控制：单条发送超时不影响整体批次
 *   <li>结果聚合：统一收集成功/失败/跳过计数
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class ParallelBatchSender {
  ...
  public BatchSendResult sendBatch(
    ...
    for (int i = 0; i < futures.size(); i++) {
      try {
        ...
        failure++;
        log.error(
            "[ParallelBatch] 发送异常: msgId={} err={}",
            requests.get(i).getMessageId(),
            e.getMessage());
      }
    }
    log.info(
        "[ParallelBatch] 批量发送完成: channel={} total={} success={} failure={} skipped={}",
        channel,
        requests.size(),
        success,
        failure,
   ...
   * @return 批量发送结果
   */
  private BatchSendResult sendSequential(
      List<MessageRequest> requests, Function<MessageRequest, MessageResult> sender) {
    int success = 0;
    int failure = 0;
    for (MessageRequest request : requests) {
      try {
