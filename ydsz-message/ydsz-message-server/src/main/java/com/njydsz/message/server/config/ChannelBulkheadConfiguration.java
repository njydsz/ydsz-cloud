 ...
 *
 * <p>为每个通道类型分配独立的线程池，避免单通道故障（如 SMTP 超时）耗尽 全局线程资源导致其他通道也被拖垮。
 *
 * <p>P0-1: 线程池由 ydsz-common-thread 统一创建管理（配置化 + Micrometer 指标 + 优雅关闭）， 本类仅负责将已注册的线程池按通道类型组装为 Map
 * 供业务使用。
 *
...
public class ChannelBulkheadConfiguration {
  ...
  /** 通道类型 → 线程池 Bean 名称映射 */
  private static final Map<String, String> CHANNEL_POOL_NAMES =
      Map.of(
          "EMAIL", "msgEmail",
          "SMS", "msgSms",
   ...
   * 组装通道级线程池 Map。
   *
   * <p>从 common-thread 已注册的线程池中查找各通道对应的执行器， 提取底层 ThreadPoolExecutor 供业务使用。
   *
    ...
    CHANNEL_POOL_NAMES.forEach(
        (channelType, poolName) -> {
          ...
          if (executor != null) {
            ...
            log.warn(
                "[Bulkhead] 通道 [{}] 线程池未在 common-thread 中注册 (poolName={})，请检查 ydsz.thread.pools 配置",
                channelType,
                poolName);
          }
        });
