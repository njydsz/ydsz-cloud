...
public class CanaryServiceImpl implements CanaryService {
  ...
  public MsgCanary upsert(CanaryUpsertDTO dto) {
    ...
    int total =
        dto.getBucketTotal() == null || dto.getBucketTotal() <= 0
            ? DEFAULT_BUCKET_TOTAL
            : dto.getBucketTotal();
    int percentage =
        dto.getPercentage() == null ? 0 : Math.max(0, Math.min(100, dto.getPercentage()));
    MsgCanary existing =
        msgCanaryMapper.selectOne(
            new LambdaQueryWrapper<MsgCanary>()
    ...
    if (existing == null) {
      ...
      log.info(
          "[Canary] 新建灰度桶: key={} percentage={} expTpl={} expChan={}",
          dto.getCanaryKey(),
          percentage,
          dto.getExperimentTemplateCode(),
          dto.getExperimentChannel());
      return entity;
    }
  ...
  public MsgCanary getByKey(String canaryKey) {
    ...
    return msgCanaryMapper.selectOne(
        new LambdaQueryWrapper<MsgCanary>().eq(MsgCanary::getCanaryKey, canaryKey).last("LIMIT 1"));
  }
  ...
  public Page<MsgCanary> page(PageQuery query) {
    ...
    return msgCanaryMapper.selectPage(
        page, new LambdaQueryWrapper<MsgCanary>().orderByDesc(MsgCanary::getCreatedAt));
  }
