 ...
 * 消息发送监控指标。
 *
 * <p>基于 Micrometer {@link MeterRegistry} 采集发送计数、耗时、重试、死信、回执等指标， 供 Prometheus / Grafana 监控。所有记录方法均
 * try-catch 降级，监控失败不影响业务。
 *
...
public class MessageMetrics extends SentryMetricsAdapter {
   ...
   * P2-5: 记录一条过期丢弃消息。
   *
   * <p>消费者侧 TTL 检查命中（消息 scheduledAt 距今超过 {@code ydsz.message.message-ttl-seconds}） 时调用，用于监控过期丢弃量。按
   * channel + reason 维度统计， reason 当前固定为 TTL_EXPIRED，便于后续扩展其它丢弃原因。
   *
   * @param channel 通道
   ...
   *
   * <p>与 {@link #recordSend} 的区别：recordSend 是业务级指标(由 MessageService 记录), recordChannelError
   * 是通道级指标(由 ChannelRouter 记录),用于按 errorType 维度监控 各 HTTP 通道的失败原因(如
   * CIRCUIT_BREAKER/EXCEPTION/BUSINESS_ERROR)。
   *
   * @param channel 通道
  ...
  public void recordSendFailure(
      String channel, String templateCode, String tenantId, String errorType) {
    incrementCounter(
        "send.total",
        "channel",
        safe(channel),
