 ...
 * 路由规则匹配 Handler。
 *
 * <p>按优先级遍历启用规则，SpEL 求值命中后覆盖通道。
 * 路由规则仅做通道切换，不做拦截（拦截由下游 Handler 负责）。
 *
 * @author ydsz-team
 * @since 1.1.0
 */
...
public class RouteRuleHandler implements SendHandler {
    ...
    public boolean handle(MessageRequest request, SendContext ctx) {
        ...
        if (matchedRule != null && StringUtils.hasText(matchedRule.getTargetChannel())) {
            ...
            log.info("[Message] 路由命中切换通道: orig={} -> target={} ruleCode={}",
                    ctx.getChannel(), newChannel, matchedRule.getRuleCode());
