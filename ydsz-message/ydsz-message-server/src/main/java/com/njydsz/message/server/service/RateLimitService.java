...
public interface RateLimitService {
   ...
   * P2-5: 多维度发送限流检查。
   *
   * <p>按 receiver / templateCode / tenant 三个维度分别做令牌桶限流， 任一维度超限即返回 false。各维度开关与 permits 由 {@code
   * MessageProperties.rateLimit} 配置。 维度间为 AND 关系：所有启用的维度都通过才允许发送。
   *
   * <p>调用方应在限流失败时记录 {@code messageMetrics.recordSend(channel, "RATE_LIMITED", 0)} 并抛出 {@code
   * SysException(RATE_LIMIT)}。
   *
