 ...
 * <ul>
 *   <li><b>站内信 / IM 类</b>（INAPP / DINGTALK / WECOM / FEISHU）：支持调用平台 API 撤回（有限时间窗口）</li>
 *   <li><b>SMS / EMAIL</b>：仅能标记状态为 RECALLED（无法真正撤回已发送物理介质）</li>
 *   <li><b>WEBHOOK / PUSH</b>：仅标记</li>
 * </ul>
 *
...
public interface RecallChannel {
   ...
   * 尝试撤回指定消息。
   *
   * <p>实现类根据通道能力：能真正调用平台 API 的尽量调用（如钉钉/飞书机器人）；
   * 不能的则仅做状态标记，返回的 {@code platformRecallSucceeded=false} 表示仅本地标记。
   *
