 ...
 * 限流与频率控制服务实现。
 *
 * <p>P0-1b 架构优化：令牌桶限流从 Redisson {@code RRateLimiter} 改为委托 {@link
 * RedisRateLimiter}（ydsz-common-redis 公共能力），统一全项目限流技术栈。
 *
 * <p>令牌桶使用 {@link RedisRateLimiter#tryAcquireTokenBucket(String, int, int, Duration, int)}； 每日 /
 * 每小时频率使用 Redis INCR + EXPIRE， 上限取自用户偏好 {@link MsgPreference#getDailyLimit()} / {@code
 * hourlyLimit}。
 *
 * <p>P2-5: {@link #checkSendLimit} 方法，按 receiver / templateCode / tenant 三个维度分别做令牌桶限流，任一维度超限即拒绝发送。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class RateLimitServiceImpl implements RateLimitService {
   ...
   * {@inheritDoc}
   *
   * <p>从用户偏好读取 hourlyLimit/dailyLimit，使用 Redis INCR + EXPIRE 计数， 任一维度超限即返回 false。偏好未配置或 enabled=0
   * 时跳过频率检查。
   *
   * @param userId 用户 ID
   * @param channel 通道类型
  ...
  public boolean checkFrequency(String userId, String channel, String bizType) {
    ...
    if (pref.getHourlyLimit() != null && pref.getHourlyLimit() > 0) {
      ...
      if (cur != null && cur >= pref.getHourlyLimit()) {
        log.info(
            "[RateLimit] 频率超限(小时): user={} channel={} cur={} limit={}",
            userId,
            channel,
            cur,
            pref.getHourlyLimit());
    ...
    if (pref.getDailyLimit() != null && pref.getDailyLimit() > 0) {
      ...
      if (cur != null && cur >= pref.getDailyLimit()) {
        log.info(
            "[RateLimit] 频率超限(日): user={} channel={} cur={} limit={}",
            userId,
            channel,
            cur,
            pref.getDailyLimit());
  ...
  public boolean checkSendLimit(
    ...
    if (cfg.isReceiverEnabled() && receiver != null && !receiver.isBlank()) {
      if (!tryAcquire("receiver:" + receiver, cfg.getReceiverPermits())) {
        log.info(
            "[RateLimit] receiver 维度限流: channel={} receiver={} permits={}/s",
            channel,
            receiver,
    ...
    if (cfg.isTemplateEnabled() && templateCode != null && !templateCode.isBlank()) {
      if (!tryAcquire("template:" + templateCode, cfg.getTemplatePermits())) {
        log.info(
            "[RateLimit] template 维度限流: channel={} template={} permits={}/s",
            channel,
            templateCode,
    ...
    if (cfg.isTenantEnabled() && tenantId != null && !tenantId.isBlank()) {
      if (!tryAcquire("tenant:" + tenantId, cfg.getTenantPermits())) {
        log.info(
            "[RateLimit] tenant 维度限流: channel={} tenant={} permits={}/s",
            channel,
            tenantId,
