 ...
 * 消息发送管线编排引擎。
 *
 * <p>负责按 order 排序并依次执行所有 {@link SendHandler}。 任一 Handler 返回 false 时管线短路，后续 Handler 不再执行。
 *
...
public class SendPipeline implements InitializingBean {
   ...
   * 执行管线预处理，并返回填充完毕的上下文。
   *
   * <p>按 order 依次调用各 Handler，任一 Handler 设置 errorResult 后终止。 全部通过时返回填充完毕的 SendContext。
   *
  ...
  public void afterPropertiesSet() {
    log.info(
        "[Pipeline] 管线初始化完成: handlerCount={} handlers={}",
        handlers.size(),
        handlers.stream()
