 ...
 *
 * <p>当前 {@link #executeLocalTransaction} 仅做发送前校验（通道启用 + 模板存在 + 接收人非空）， 校验通过即
 * COMMIT。<b>不执行业务本地事务</b>（如创建订单、扣减库存等）。 这意味着消息投递与业务事务无关联，业务事务失败后消息已投递，可能造成数据不一致。
 *
 * <p><b>适用场景</b>：仅需发送前校验、不需要消息与业务事务强一致的场景。
 *
 * <p><b>不适用场景</b>：需要消息投递与业务事务强一致的场景。 此时应提供 {@code TransactionExecutor} 接口，业务侧实现：
 *
 * <ul>
 *   <li>{@code executeLocalTransaction}：执行业务 SQL，成功后 COMMIT
 *   <li>{@code checkLocalTransaction}：查询业务表状态，决定 COMMIT/ROLLBACK
 * </ul>
 *
...
public class MessageTransactionListener implements RocketMQLocalTransactionListener {
  ...
  public RocketMQLocalTransactionState executeLocalTransaction(Message message, Object arg) {
    ...
    try {
      ...
      if (reason != null) {
        log.warn(
            "[TxListener] executeLocalTransaction: 校验失败 ROLLBACK, messageId={} reason={}",
            req.getMessageId(),
            reason);
        return RocketMQLocalTransactionState.ROLLBACK;
      }
      log.info(
          "[TxListener] executeLocalTransaction: 校验通过 COMMIT, messageId={} channel={} template={}",
          req.getMessageId(),
          req.getChannel(),
          req.getTemplateCode());
      ...
      log.error(
          "[TxListener] executeLocalTransaction: 异常 ROLLBACK, messageId={} err={}",
          req.getMessageId(),
          e.getMessage());
      return RocketMQLocalTransactionState.ROLLBACK;
    }
  ...
  public RocketMQLocalTransactionState checkLocalTransaction(Message message) {
    ...
    try {
      ...
      if (reason != null) {
        log.warn(
            "[TxListener] checkLocalTransaction: 校验失败 ROLLBACK, messageId={} reason={}",
            req.getMessageId(),
            reason);
        return RocketMQLocalTransactionState.ROLLBACK;
      }
      ...
      log.error(
          "[TxListener] checkLocalTransaction: 异常 UNKNOWN, messageId={} err={}",
          req.getMessageId(),
          e.getMessage());
