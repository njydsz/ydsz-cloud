 ...
 *
 * <ul>
 *   <li>必填变量缺失 → 抛 SysException(MISSING_PARAMETER)
 *   <li>类型不匹配 → 抛 SysException(BAD_REQUEST)
 *   <li>ENUM 值不在可选范围 → 抛 SysException(BAD_REQUEST)
 *   <li>有 defaultValue 的缺失变量 → 自动填充默认值
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class TemplateVariableValidator {
  ...
  public void validateAndFill(
      Map<String, Object> params, List<TemplateVariableDef> varDefs, String templateCode) {
    ...
    for (TemplateVariableDef def : varDefs) {
      ...
      if (value == null || (value instanceof String s && s.isBlank())) {
        ...
        if (StringUtils.hasText(def.getDefaultValue())) {
          ...
          log.debug(
              "[VariableValidator] 填充默认值: template={} var={} default={}",
              templateCode,
              name,
              def.getDefaultValue());
          continue;
