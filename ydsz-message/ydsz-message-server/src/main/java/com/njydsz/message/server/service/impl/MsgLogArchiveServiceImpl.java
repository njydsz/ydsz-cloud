 ...
 *
 * <ol>
 *   <li>归档 90 天前所在月份的分区（DETACH + RENAME）
 *   <li>为下下个月预创建分区（保证至少有一个未来分区可用）
 * </ol>
 *
 * <p>幂等性：
 *
 * <ul>
 *   <li>DETACH 使用 CONCURRENTLY（PG 14+），不阻塞读写
 *   <li>若分区不存在则跳过，记 WARN 日志
 *   <li>CREATE TABLE IF NOT EXISTS 保证幂等
 *   <li>RENAME 失败时（目标表已存在）记 ERROR 但不中断后续流程
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class MsgLogArchiveServiceImpl implements MsgLogArchiveService {
  ...
  /** 父表名 */
  private static final String PARENT_TABLE = "ydsz_msg_log";

  /** 分区名前缀 */
  private static final String PARTITION_PREFIX = "ydsz_msg_log_y";

  /** 归档表名前缀 */
  private static final String ARCHIVE_PREFIX = "ydsz_msg_log_archive_";
  ...
  public List<String> archive(int year, int month) {
    ...
    for (int i = 0; i <= 24; i++) {
      ...
      try {
        ...
        jdbcTemplate.execute(
            String.format(
                ...
                PARENT_TABLE, partitionName, supportsDetachConcurrently() ? " CONCURRENTLY" : ""));
      ...
      try {
        jdbcTemplate.execute(
            String.format("ALTER TABLE %s RENAME TO %s", partitionName, archiveName));
  ...
  public String ensurePartition(int year, int month) {
    ...
    String sql =
        String.format(
            ...
            partitionName, PARENT_TABLE, start.atDay(1), end.atDay(1));
  ...
  private boolean supportsDetachConcurrently() {
    try {
      Integer major =
          jdbcTemplate.queryForObject("SELECT split_part(version(), ' ', 2)::int", Integer.class);
  ...
  /**
   * 生成分区名: {@code ydsz_msg_log_y2026m05}。
   *
   * @param year 年
   * @param month 月(1-12)
   * @return 分区名
  ...
  /**
   * 生成归档表名: {@code ydsz_msg_log_archive_202605}。
   *
   * @param year 年
   * @param month 月(1-12)
   * @return 归档表名
