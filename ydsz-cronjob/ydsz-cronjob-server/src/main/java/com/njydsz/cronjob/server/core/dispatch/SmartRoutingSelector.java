 ...
 *
 * <p>在调度器-执行器分离模式下，根据执行器节点的资源负载（CPU、内存、运行任务数） 和机房亲和性，选择最优的 Worker 节点执行任务。
 *
 * <h3>路由策略</h3>
 *
 * <ul>
 *   <li><b>CPU 负载感知</b>：选择 CPU 使用率最低的节点（通过 JMX 获取）
 *   <li><b>任务负载感知</b>：选择 runningCount 最低的节点（基于心跳上报）
 *   <li><b>机房亲和性</b>：优先选择与 Leader 同机房的节点（降低网络延迟）
 *   <li><b>综合评分</b>：CPU(40%) + 任务负载(40%) + 机房亲和(20%)
 * </ul>
 *
 * <h3>评分公式</h3>
 *
...
public class SmartRoutingSelector {
  ...
  public JobNodeVO selectBestWorker(int maxConcurrentPerWorker) {
    ...
    if (bestNode != null && log.isDebugEnabled()) {
      log.debug(
          "[SmartRouting] 选择最优 Worker: nodeId={} score={:.2f}", bestNode.getNodeId(), bestScore);
    }
  ...
  private double getCpuUsage() {
    try {
      com.sun.management.OperatingSystemMXBean
          osBean = // FQN-OK: name conflict with java.lang.management.OperatingSystemMXBean
          (com.sun.management.OperatingSystemMXBean) // FQN-OK: name conflict with
