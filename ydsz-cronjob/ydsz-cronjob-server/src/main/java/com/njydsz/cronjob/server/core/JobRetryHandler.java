 ...
 *
 * <p>封装任务执行失败后的重试逻辑，支持固定间隔和指数退避两种策略。
 * 消除 DefaultTaskDispatcher 中分散的重试代码。
 *
 * <h3>核心能力</h3>
 * <ul>
 *   <li>计算下次重试的等待时间（FIXED / EXPONENTIAL）</li>
 *   <li>判断是否已达最大重试次数</li>
 *   <li>执行重试等待（阻塞式，带超时保护）</li>
 * </ul>
 *
 * <h3>提取动机</h3>
 * <p>DefaultTaskDispatcher 1592 行代码中约 150 行涉及重试逻辑，
 * 提取后可统一重试策略配置和错误处理。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class JobRetryHandler {
    ...
    public void waitForRetry(int retryCount, Long retryIntervalMs, String backoffStrategy) {
        ...
        try {
            ...
            log.info("[JobRetry] 等待重试: retryCount={} delay={}ms strategy={}",
                    retryCount, cappedDelay, backoffStrategy);
