 ...
 *
 * <ul>
 *   <li>CPU 使用率（权重 35%）：直接反映节点计算压力
 *   <li>内存使用率（权重 25%）：反映 JVM 堆压力，避免 OOM 风险
 *   <li>运行任务数（权重 25%）：反映当前调度压力，避免任务堆积
 *   <li>历史成功率（权重 15%）：反映节点稳定性，优先分配给稳定节点
 * </ul>
 *
 * <h3>评分模型</h3>
 *
...
public class LoadAwareShardingStrategy implements ShardingStrategy {
  ...
  public List<ShardAssignment> assign(int shardTotal, List<String> onlineNodes) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("[LoadAwareSharding] 分片分配完成: shardTotal={} assignments={}", shardTotal, result);
    }
  ...
  public void recordNodeFailure(String nodeId) {
    ...
    log.debug(
        "[LoadAwareSharding] 节点失败计数: nodeId={} streak={}", nodeId, nodeFailStreak.get(nodeId));
  }
