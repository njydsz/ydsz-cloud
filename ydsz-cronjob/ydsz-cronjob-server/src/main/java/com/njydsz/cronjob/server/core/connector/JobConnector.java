 ...
 * 生态连接器接口（P2-3）。
 *
 * <p>定义与外部系统（如 XXL-Job、PowerJob、SchedulerX、Elastic-Job）的标准化集成接口， 支持任务导入导出、双向同步和迁移适配。
 *
 * <h3>支持的外部系统</h3>
 *
 * <ul>
 *   <li>{@code XXL_JOB}: XXL-JOB 调度中心
 *   <li>{@code POWER_JOB}: PowerJob 调度服务器
 *   <li>{@code SCHEDULER_X}: Alibaba SchedulerX
 *   <li>{@code ELASTIC_JOB}: Apache ShardingSphere ElasticJob
 *   <li>{@code SPRING_BATCH}: Spring Batch
 *   <li>{@code QUARTZ}: Quartz Scheduler
 * </ul>
 *
 * <h3>设计目标</h3>
 *
 * <ul>
 *   <li>统一接口：所有外部系统适配同一套接口，业务无感知
 *   <li>可插拔：通过 SPI 机制动态加载连接器实现
 *   <li>双向同步：支持从外部系统导入任务，也支持导出任务到外部系统
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
