 ...
 *
 * <p>将任务调度全链路（调度→派发→执行→完成）接入分布式追踪系统， 实现跨节点、跨服务的调用链可视化。
 *
 * <h3>追踪链路</h3>
 *
 * <pre>
 ...
 *
 * <ul>
 *   <li>SkyWalking：通过 agent 自动注入 TraceId，本组件补充业务标签
 *   <li>OpenTelemetry：通过 OTel SDK 手动创建 Span，本组件提供 Span 创建辅助方法
 *   <li>兼容模式：无 agent 时使用 MDC + TracerUtils 实现简易追踪
 * </ul>
 *
 * <h3>业务标签（Tags）</h3>
 *
 * <ul>
 *   <li>{@code job.key}: 任务 KEY
 *   <li>{@code job.trigger}: 触发类型（CRON/MANUAL/RETRY）
 *   <li>{@code job.shard}: 分片索引
 *   <li>{@code job.duration}: 执行耗时（ms）
 *   <li>{@code job.status}: 执行结果（SUCCESS/FAILED）
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class TraceIntegrationHelper {
   ...
   * 创建任务执行 Span 的业务标签。
   *
   * <p>当 SkyWalking agent 或 OTel SDK 存在时，这些标签会自动附加到当前 Span。 无 agent 时，标签写入 MDC 供日志输出。
   *
   * @param jobKey 任务 KEY
  ...
  public Map<String, String> buildJobTags(String jobKey, String triggerType, int shardIndex) {
    Map<String, String> tags = new LinkedHashMap<>(COLLECTION_CAPACITY);
   ...
   * 记录任务执行完成时的追踪信息。
   *
   * <p>在 SkyWalking 存在时，会自动捕获异常并设置 Span 状态； 无 agent 时，仅记录日志。
   *
   * @param jobKey 任务 KEY
   * @param triggerType 触发类型
   * @param success 是否成功
  ...
  public void recordJobCompletion(
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "[Trace] 任务执行完成: key={} trigger={} success={} duration={}ms",
            jobKey,
            triggerType,
            success,
            durationMs);
