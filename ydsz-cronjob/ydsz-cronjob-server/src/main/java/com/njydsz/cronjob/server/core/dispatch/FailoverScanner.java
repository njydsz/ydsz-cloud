 ...
 * 失败自动转移扫描器（P1-4）。
 *
 * <p>仅当 {@code ydsz.cronjob.leader.enabled=true} 且当前节点是 Leader 时启用。 定时（默认 30s）扫描已下线执行器节点上的 RUNNING
 * 任务日志， 标记为 FAILED 后以 triggerType=FAILOVER 重新派发任务，避免任务因节点宕机而永久卡死。
 *
 * <h3>执行流程</h3>
 *
 * <ol>
 *   <li>检查 Leader 身份（非 Leader 节点直接返回，避免重复扫描）
 *   <li>获取在线节点列表（通过 {@link NodeDiscoveryStrategy}，兼容 Nacos / DB 模式）
 *   <li>查询所有 RUNNING 日志的 exec_node_id，找出不在在线列表中的下线节点
 *   <li>对每个下线节点：
 *       <ul>
 *         <li>调用 {@code selectRunningByNode} 获取 RUNNING 日志
 *         <li>调用 {@code markFailedByNodeOffline} 标记为 FAILED
 *         <li>对每条失败日志，查询对应的 Job，若任务仍为 NORMAL 状态， 调用 {@code taskDispatcher.dispatch(job, null,
 *             FAILOVER)} 重新派发
 *       </ul>
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li><b>Leader 独占</b>：通过 {@link LeaderElector#isLeader(String)} 判定， 避免多实例重复扫描与重复派发
 *   <li><b>节点发现抽象</b>：通过 {@link NodeDiscoveryStrategy} 统一获取在线节点， Nacos 模式对比 Nacos 实例列表，DB 模式对比
 *       ydsz_job_node 心跳表
 *   <li><b>容错</b>：单条任务转移失败不影响其他任务；外层 try-catch 兜底
 *   <li><b>限流</b>：单批最多扫描 {@code scanNodeLimit} 个节点， 单节点最多转移 {@code failoverTaskLimit} 个任务，避免雪崩
 *   <li><b>幂等</b>：{@code markFailedByNodeOffline} 使用 CAS 语义 （WHERE status='RUNNING'），重复扫描不会重复标记
 * </ul>
 *
 * <h3>与 JobNodeReaper 的关系</h3>
 *
 * <p>{@link com.njydsz.cronjob.server.core.executor.JobNodeReaper} 负责 DB 模式下的 节点状态回收（标记 OFFLINE +
 * 物理删除过期记录），其 P1-3 故障转移逻辑仅释放锁和标记 FAILED， 不重新派发任务。本扫描器专注于"标记 FAILED + 重新派发"，二者职责互补：
 *
 * <ul>
 *   <li>FailoverScanner（30s）：快速发现下线节点并重新派发任务，减少任务延迟
 *   <li>JobNodeReaper（5min）：清理节点状态，避免 ydsz_job_node 表膨胀
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class FailoverScanner {
   ...
   *
   * <p>{@code leaderRole} 一次性读入字段，供后续每 30 秒的扫描判定身份， 避免高频穿透配置对象；<b>不支持运行期改角色</b>，需重启生效。
   *
   * <p>当 {@code leader.enabled=false} 时仅打印提示日志，本扫描器的 {@code scan()} 会在运行期直接短路返回，故障转移能力关闭——此时节点宕机遗留的
   * RUNNING 任务 只能依赖 {@code JobNodeReaper} 兜底清理，不会被重新派发。
   */
  @PostConstruct
  public void init() {
    ...
    if (cronjobProperties.getLeader().isEnabled()) {
      log.info(
          "[FailoverScanner] 初始化完成, role={} scanInterval={}s scanNodeLimit={} taskLimit={}",
          leaderRole,
   ...
   *
   * <p>使用 {@code fixedDelayString} 而非 {@code fixedRateString}， 避免上次扫描耗时较长时任务堆积。 配置项 {@code
   * ydsz.cronjob.failover.scan-interval-ms} 为毫秒值（默认 30000）。
   */
  ...
  private void doScan() {
    ...
    log.warn(
        "[FailoverScanner] 发现 {} 个下线节点待故障转移: role={} onlineNodes={} runningNodes={}",
        offlineNodeIds.size(),
        leaderRole,
        onlineNodeIds.size(),
        runningNodeIds.size());
    ...
    for (String nodeId : offlineNodeIds) {
      try {
        ...
        log.error("[FailoverScanner] 节点故障转移异常: nodeId={} reason={}", nodeId, e.getMessage(), e);
      }
    }

    if (totalRedispatched > 0) {
      log.warn(
          "[FailoverScanner] 扫描完成: role={} offlineNodes={} redispatched={}",
          leaderRole,
          offlineNodeIds.size(),
          totalRedispatched);
    }
   ...
   *
   * <p>调用 {@link JobLogMapper#selectRunningNodeIds()} 获取去重后的节点 ID 列表， 避免 MyBatis Plus
   * LambdaQueryWrapper 在无 Spring 上下文环境下（如单元测试）的 lambda 缓存问题。
   *
   ...
   *
   * <ol>
   *   <li>调用 {@link JobLogMapper#selectRunningByNode(String)} 获取 RUNNING 日志
   *   <li>调用 {@link JobLogMapper#markFailedByNodeOffline(String, LocalDateTime)} 标记为 FAILED
   *   <li>对每条失败日志，查询对应的 Job
   *   <li>若任务仍为 NORMAL 状态，调用 {@link TaskDispatcher#dispatch} 重新派发（triggerType=FAILOVER）
   * </ol>
   *
  ...
  private int failoverNode(String nodeId) {
    ...
    log.warn(
        "[FailoverScanner] 节点故障转移开始: nodeId={} runningTasks={} taskLimit={}",
        nodeId,
        runningLogs.size(),
        taskLimit);
    ...
    if (releasedLocks > 0) {
      log.info(
          "[FailoverScanner] 已释放死节点任务锁: nodeId={} releasedLocks={}/{}",
          nodeId,
          releasedLocks,
          runningLogs.size());
    }
    ...
    try {
      ...
      log.error(
          "[FailoverScanner] 标记节点任务 FAILED 失败: nodeId={} reason={}", nodeId, e.getMessage(), e);
    ...
    for (JobLog logEntry : runningLogs) {
      if (redispatched >= taskLimit) {
        log.warn(
            "[FailoverScanner] 达到单节点转移上限 {}, 剩余任务不再派发: nodeId={} total={}",
            taskLimit,
            nodeId,
            runningLogs.size());
        break;
      ...
      try {
        ...
        if (job == null) {
          log.debug(
              "[FailoverScanner] 任务已删除, 跳过: jobId={} logId={}",
              logEntry.getJobId(),
              logEntry.getId());
          continue;
        }
        if (!"NORMAL".equals(job.getStatus())) {
          log.debug(
              "[FailoverScanner] 任务非 NORMAL 状态, 跳过: jobKey={} status={}",
              job.getJobKey(),
              job.getStatus());
          continue;
        }
        ...
        log.info(
            "[FailoverScanner] 故障转移派发: jobKey={} oldLogId={} newLogId={} nodeId={}",
            job.getJobKey(),
            logEntry.getId(),
            newLogId,
            nodeId);
        ...
        log.error(
            "[FailoverScanner] 任务转移失败: logId={} jobKey={} reason={}",
            logEntry.getId(),
            logEntry.getJobKey(),
            e.getMessage(),
            e);
    ...
    log.warn(
        "[FailoverScanner] 节点故障转移完成: nodeId={} runningTasks={} releasedLocks={} markedFailed={} redispatched={}",
        nodeId,
        runningLogs.size(),
        releasedLocks,
        markedFailed,
   ...
   * 安全释放死节点持有的任务锁（P0-10）。
   *
   * <p>使用 Lua 脚本保证"检查 lockHolder 匹配 + delete"的原子性， 避免误删其他节点（如新派发任务的执行器）持有的锁。
   *
   * <p>分片任务（shardIndex >= 0）使用 {@code :shard:{shardIndex}} 后缀的锁 key， 通过 {@link
   * LockKeyUtil#buildJobLockKey(String, Integer)} 统一构造。
   *
  ...
  private boolean releaseLockSafe(JobLog logEntry) {
    ...
    try {
      ...
      if (result != null && result > 0) {
        log.debug("[FailoverScanner] 释放死节点锁成功: lockKey={} holder={}", lockKey, lockHolder);
        return true;
      }
      log.debug(
          "[FailoverScanner] 锁 holder 不匹配或已过期, 跳过释放: lockKey={} holder={}", lockKey, lockHolder);
      return false;
    } catch (Exception e) {
      log.warn(
          "[FailoverScanner] 释放锁失败(将等待 TTL 自动过期): lockKey={} reason={}", lockKey, e.getMessage());
      return false;
    }
  }
}
