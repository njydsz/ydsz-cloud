 ...
 * 慢任务诊断扫描器。
 *
 * <p>仅当 {@code ydsz.cronjob.leader.enabled=true} 且当前节点是 Leader 时启用。 定时（默认 30s）扫描 {@code
 * ydsz_job_log} 中已结束（SUCCESS/FAILED/TIMEOUT） 且耗时超过 {@code ydsz_job.slow_threshold_ms} 的记录，标记 {@code
 * is_slow=1}。
 *
 * <h3>设计要点</h3>
 *
 * <ul>
 *   <li><b>幂等</b>：通过 {@code WHERE is_slow = 0} 过滤已标记的记录， 并在 UPDATE 中二次检查 {@code is_slow = 0} 防止并发竞态
 *   <li><b>时间窗口</b>：仅扫描最近 N 分钟（默认 60min）的日志，避免全表扫描
 *   <li><b>批量查询</b>：通过 selectByIds 一次性获取所有相关 Job，避免 N+1 查询
 *   <li><b>独立失败</b>：每条标记独立 try-catch，单条失败不影响其他
 * </ul>
 *
 * <h3>与告警系统的关系</h3>
 *
 * <p>本扫描器仅负责 <b>标记</b> 慢任务（is_slow=1），用于性能趋势分析。 慢任务 <b>告警</b> 由 {@code
 * DefaultTaskDispatcher.triggerAlerts()} 在执行完成时 实时触发 {@link
...
public class SlowTaskDetector {
   ...
   *
   * <p>仅在 {@code ydsz.cronjob.leader.enabled=true} 时进入扫描启用分支； 否则仅记录 Leaderless 日志，不创建任何资源（扫描方法
   * {@link #scan()} 内部已通过 Leader 身份与 enabled 双重校验拦截，故本方法不负责实际扫描逻辑）。 扫描的时间窗口 {@code
   * LOOKBACK_MINUTES}（60min）固定，不依赖外部配置。
   */
  @PostConstruct
  public void init() {
    ...
    if (cronjobProperties.getLeader().isEnabled()) {
      log.info(
          "[SlowTaskDetector] 初始化完成, role={} lookbackMinutes={}", leaderRole, LOOKBACK_MINUTES);
    } else {
  ...
  private void doScan() {
    ...
    for (JobLogVO log0 : slowLogs) {
      try {
        ...
        log.error(
            "[SlowTaskDetector] 标记慢任务失败: logId={} jobKey={} reason={}",
            log0.getId(),
            log0.getJobKey(),
            e.getMessage(),
            e);
    ...
    log.info(
        "[SlowTaskDetector] 扫描完成: role={} marked={} skipped={} total={}",
        leaderRole,
        marked,
        skipped,
        slowLogs.size());
