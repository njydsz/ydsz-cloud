 ...
 *
 * <ul>
 *   <li>CPU 使用率高 → 缩减 batchSize，降低调度压力
 *   <li>内存使用率高 → 缩减 batchSize，防止 OOM
 *   <li>线程池活跃度高 → 缩减 batchSize，避免任务积压
 *   <li>系统空闲 → 放大 batchSize，提升吞吐量
 * </ul>
 *
 * <h3>负载评分模型</h3>
 *
 ...
 * <h3>安全发布</h3>
 *
 * <p>通过 {@link AtomicInteger} 安全发布当前 batchSize，JobScanner 每次扫描时读取最新值。 调整频率为每 {@code
 * evalIntervalSeconds} 秒一次，避免频繁波动。
 *
...
public class AdaptiveBatchScheduler {
   ...
   *
   * <p>仅当 {@code ydsz.cronjob.adaptive-batch.enabled=true} 且有 {@code cronjobMetrics} Bean 时注册。初始
   * {@code currentBatchSize} 取配置的 {@code scanner.batchSize}，避免在首轮负载评估前 JobScanner 读到无意义的中间值； 后续由
   * {@link #evaluateAndAdjust()} 按实时负载动态覆写。无独立线程池或锁资源需预分配。
   */
  @PostConstruct
  public void init() {
    ...
    log.info(
        "[AdaptiveBatch] 初始化完成, initialBatchSize={} min={} max={} cpuThreshold={} memThreshold={} poolThreshold={}",
  ...
  public void evaluateAndAdjust() {
    try {
      ...
      if (newBatchSize != oldBatchSize) {
        log.info(
            "[AdaptiveBatch] batchSize 调整: {} -> {} (cpu={}%, mem={}%, pool={}%, loadScore={})",
            oldBatchSize,
            newBatchSize,
            String.format("%.1f", cpuUsage),
            String.format("%.1f", memUsage),
            String.format("%.1f", poolActive),
            String.format("%.3f", loadScore));
      }
      ...
      log.warn(
          "[AdaptiveBatch] 评估异常, 保持当前 batchSize={}: {}", currentBatchSize.get(), e.getMessage());
    }
  }
  ...
  private double calculateLoadScore(
      double cpuUsage, double memUsage, double poolActive, CronjobProperties.AdaptiveBatch config) {
    // 归一化各项指标到 0-1
   ...
   *
   * <p>本类不持有线程池或外部连接，无需要主动释放的资源； 通过 {@link AtomicInteger} 发布的 {@code currentBatchSize} 随 Bean
   * 销毁自然失效， JobScanner 在停机前读取到的最后一次评估值即为最终生效值。
   */
  @PreDestroy
