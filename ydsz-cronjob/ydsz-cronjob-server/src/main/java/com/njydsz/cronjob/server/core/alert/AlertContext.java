 ...
 * 告警上下文（P5 告警 + 监控）。
 *
 * <p>封装一次告警触发时的完整上下文信息，由触发点（如 Dispatcher、TimeoutMonitor） 构造并传递给 {@link AlertDispatcher}。Dispatcher
 * 根据 context 匹配告警规则、 执行去重判断并调用 {@link com.njydsz.common.feign.NotificationClient} 派发通知。
 *
 * <p>使用 record 保证不可变性，避免多线程（@Async 监听器）下的可见性问题。
 *
 * <p>P3-1: 新增 {@code recovery} 标志，用于区分告警通知与恢复通知。 恢复通知跳过冷却窗口检查，日志 status 带 {@code _RECOVERY} 后缀。
 *
...
public record AlertContext(
   ...
   *
   * <p>恢复通知用于告警条件解除时通知用户，例如任务从失败恢复为成功、 慢任务恢复为正常耗时。Dispatcher 处理恢复通知时会跳过冷却窗口检查。
   *
   * @param alertType 告警类型
   * @param jobId 任务 ID（NULL 表示全局告警）
   * @param jobKey 任务 KEY
   ...
   * @return 恢复通知上下文
   */
  public static AlertContext recovery(
      AlertType alertType,
      String jobId,
      String jobKey,
      String jobName,
