 ...
 *
 * <ul>
 *   <li>通过配置 {@code ydsz.cronjob.clusters} 定义多个集群端点
 *   <li>任务的 {@code cluster} 字段指定目标集群（null=本地集群）
 *   <li>跨集群派发通过 HTTP API 调用目标集群的 /cronjob/internal/execute 接口
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class CrossClusterDispatcher {
  ...
  public void initClusters(Map<String, String> clusters) {
    ...
    log.info("[CrossCluster] 初始化集群端点: count={} clusters={}", clusters.size(), clusters.keySet());
  }
    ...
    try {
      ...
      if (response.statusCode() == 200) {
        ...
        if (code == 0) {
          ...
          log.info(
              "[CrossCluster] 跨集群派发成功: cluster={} key={} logId={}",
              clusterName,
      ...
      log.warn(
          "[CrossCluster] 跨集群派发失败: cluster={} status={} body={}",
          clusterName,
          response.statusCode(),
          response.body() == null
              ? ""
