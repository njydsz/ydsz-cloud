 ...
 *
 * <ul>
 *   <li>{@code WORKDAY}：仅工作日执行（跳过周末和节假日）
 *   <li>{@code WEEKEND}：仅周末执行
 *   <li>{@code HOLIDAY}：仅节假日执行
 *   <li>{@code ALL}（默认）：不限制，每天均可执行
 * </ul>
 *
 * <h3>配置方式</h3>
 *
 ...
 *
 * <ul>
 *   <li>任务级配置：paramsJson.holidays 数组（手动维护）
    ...
    switch (calendarType.toUpperCase()) {
      case "WORKDAY":
        ...
        if (isWeekend || isHoliday) {
          log.debug(
              "[Calendar] WORKDAY 策略跳过: date={} isWeekend={} isHoliday={}",
              date,
              isWeekend,
              isHoliday);
          return false;
