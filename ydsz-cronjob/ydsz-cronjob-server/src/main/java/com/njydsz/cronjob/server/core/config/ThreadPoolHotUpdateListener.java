 ...
 *
 * <ul>
 *   <li>{@code executor.maxConcurrent}：最大并发数（调整 corePoolSize / maxPoolSize）
 *   <li>{@code executor.queueCapacity}：队列容量
 *   <li>{@code executor.isolationStrategy}：租户隔离策略（none / tenant / job_group）
 *   <li>{@code executor.tenantPoolSize}：租户隔离线程池大小
 *   <li>{@code executor.tenantPoolQueueCapacity}：租户隔离队列容量
 * </ul>
 *
 * <h3>设计要点</h3>
 *
 * <ul>
 *   <li>通过 {@link ThreadPoolExecutor#setCorePoolSize} / {@link
 *       ThreadPoolExecutor#setMaximumPoolSize} 实现线程池参数的运行时调整，无需重启
...
public class ThreadPoolHotUpdateListener {
  ...
  private final CronjobThreadPoolRegistry threadPoolRegistry;
  private final TenantAwareExecutorPool tenantAwareExecutorPool;

  /**
   * Nacos 配置变更监听。
   *
   * <p>监听 {@code ydsz-cronjob.yml}（或对应的 dataId），当检测到 executor 相关配置变更时， 解析新配置并应用到运行中的线程池。
   *
  ...
  public void onConfigChange(String configInfo) {
    ...
    try {
      ObjectNode config = parseConfig(configInfo);
      if (config == null) {
        return;
      }
   ...
   * 解析配置内容（支持 JSON 格式）。
   *
   * <p>YAML 格式由 Nacos 客户端自动转换为 JSON，此处统一按 JSON 解析。 兼容两层嵌套：顶层 ydsz.cronjob.executor 或直接 executor。
   *
  ...
  private ObjectNode parseConfig(String configInfo) {
    try {
      ...
      ObjectNode ydsz = root.getObjectNode("ydsz");
      if (ydsz != null) {
        ObjectNode cronjob = ydsz.getObjectNode("cronjob");
        if (cronjob != null) {
   ...
   */
  private void applyExecutorConfigChanges(ObjectNode executorConfig) {
    ...
    if (newQueueCapacity != null && newQueueCapacity >= 0) {
      ...
      if (newQueueCapacity != oldQueue) {
        ...
        changed = true;
        log.info(
            "[ThreadPoolHotUpdate] queueCapacity: {} -> {} (下次创建线程池时生效)",
            oldQueue,
            newQueueCapacity);
      }
    }
    ...
    if (newStrategy != null && !newStrategy.isBlank()) {
      ...
      if (!newStrategy.equalsIgnoreCase(oldStrategy)) {
        ...
        changed = true;
        log.info(
            "[ThreadPoolHotUpdate] isolationStrategy: {} -> {} (已清空旧隔离池)",
            oldStrategy,
            newStrategy);
      }
    }
    ...
    if (newTenantPoolSize != null && newTenantPoolSize > 0) {
      ...
      if (newTenantPoolSize != oldSize) {
        ...
        changed = true;
        log.info(
            "[ThreadPoolHotUpdate] tenantPoolSize: {} -> {} (已清空旧隔离池)", oldSize, newTenantPoolSize);
      }
    }
    ...
    if (newTenantQueueCap != null && newTenantQueueCap >= 0) {
      ...
      if (newTenantQueueCap != oldCap) {
        ...
        changed = true;
        log.info(
            "[ThreadPoolHotUpdate] tenantPoolQueueCapacity: {} -> {} (已清空旧隔离池)",
            oldCap,
            newTenantQueueCap);
      }
    }
   ...
   * 动态调整全局执行线程池大小。
   *
   * <p>通过 {@link CronjobThreadPoolRegistry} 获取全局执行线程池引用， 调用 setCorePoolSize / setMaximumPoolSize
   * 实现运行时调整。
   *
   * <p><b>P1-A2 改进:</b> 消除对 DefaultTaskDispatcher 的直接依赖和反射强耦合， 统一通过注册表管理线程池生命周期。
   *
  ...
  private void resizeGlobalThreadPool(int newMaxConcurrent) {
    try {
      ...
      log.info(
          "[ThreadPoolHotUpdate] 全局线程池已调整: core={} max={} active={} queue={}",
