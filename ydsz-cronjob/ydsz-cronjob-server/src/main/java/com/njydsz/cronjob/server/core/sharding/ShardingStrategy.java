 ...
 *
 * <ul>
 *   <li>可插拔：业务侧可自定义实现替代默认的 {@link AverageShardingStrategy}
 *   <li>确定性：相同输入（节点列表 + 分片数）应产生相同输出，保证 Leader 切换时分片稳定
 *   <li>均衡：尽量让各节点承担相同数量的分片
 * </ul>
 *
 * <p>典型实现：
 *
 * <ul>
 *   <li>{@link AverageShardingStrategy}：轮询平均分配（默认）
 *   <li>未来扩展：一致性哈希、按标签亲和性、按负载权重等
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
