 ...
 * DAG 工作流定义 Service
 *
 * <p>负责 DAG(有向无环图)工作流定义的完整生命周期：CRUD、状态流转、版本管理、手动触发。 DAG 用于表达"多个任务按依赖关系编排执行"的需求(任务 A 完成后才能执行任务 B,C
 * 失败时跳过 D), 比单任务更适合复杂的业务编排场景。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li><b>CRUD</b>：{@link #createDag} / {@link #updateDag} / {@link #deleteDag}
 *   <li><b>状态流转</b>：{@link #enableDag} / {@link #disableDag} — {@code DRAFT/DISABLED ↔ ENABLED}
 *   <li><b>查询</b>：{@link #getDagById} / {@link #getDagByKey} / {@link #listEnabledDags} / {@link
 *       #listCronEnabledDags}
 *   <li><b>手动触发</b>：{@link #triggerDag} — 立即执行并返回实例 ID
 *   <li><b>版本管理(P1-8)</b>：{@link #listDagVersions} / {@link #getDagVersion} / {@link
 *       #rollbackDagVersion}
 * </ul>
 *
...
public interface JobDagService {
   ...
   * P1-8: 回滚 DAG 到指定版本。
   *
   * <p>将指定版本的 dagDefinition 复制到当前 DAG 定义，并创建新的版本快照。 回滚本身也是一个新版本，版本号继续递增（不重用旧版本号）。
   *
   * @param dagId DAG ID
