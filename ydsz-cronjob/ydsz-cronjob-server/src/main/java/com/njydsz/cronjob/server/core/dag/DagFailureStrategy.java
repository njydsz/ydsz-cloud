 ...
 *
 * <ul>
 *   <li>{@link #ABORT}：前置失败则不触发后继，并标记所有未完成节点为 SKIPPED（默认，关键链路）
 *   <li>{@link #CONTINUE}：前置失败仍触发后继（适用于通知/清理类后继）
 *   <li>{@link #RETRY}：节点失败时自动重试
 *   <li>{@link #SKIP_SUBSEQUENT}：节点失败时跳过该节点的所有直接后继
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
