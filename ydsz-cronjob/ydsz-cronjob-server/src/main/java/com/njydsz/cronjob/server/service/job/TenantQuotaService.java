 ...
 * 租户级配额服务（P7-2 / P7-3）。
 *
 * <p>提供任务数/并发数/日执行量配额检查能力，防止 noisy neighbor 问题。 默认禁用（{@code
 * ydsz.cronjob.quota.enabled=false}），启用后在任务创建/派发时自动校验。
 *
 * <h3>配额优先级</h3>
 *
 * <ol>
 ...
 *
 * <ul>
 *   <li><b>并发计数</b>：{@code ydsz:quota:concurrent:{tenantId}}
 *       <ul>
 *         <li>任务执行开始时 INCR（{@link #recordExecutionStart}）
 *         <li>任务执行结束时 DECR（{@link #recordExecutionEnd}），保证不会为负
 *         <li>TTL 兜底：首次 INCR 时设置 24 小时 TTL，防止节点宕机导致计数泄漏
 *       </ul>
 *   <li><b>日执行计数</b>：{@code ydsz:quota:daily:{tenantId}:{yyyyMMdd}}
 *       <ul>
 *         <li>任务派发时 INCR（{@link #recordExecutionStart}），不释放
 *         <li>TTL：25 小时（跨天自动过期，留 1 小时余量应对时区差异）
 *       </ul>
 * </ul>
 *
...
public interface TenantQuotaService {
   ...
   *
   * <p>当 {@code ydsz.cronjob.quota.enabled=false} 时直接返回（不检查）。 当租户配额记录不存在时，使用全局默认 {@code
   * defaultMaxJobs}。
   *
   * @param tenantId 租户 ID
   ...
   *
   * <ul>
   *   <li>{@code ydsz:quota:concurrent:{tenantId}}（并发计数，需在 {@link #recordExecutionEnd} 中 DECR）
   *   <li>{@code ydsz:quota:daily:{tenantId}:{yyyyMMdd}}（日执行计数，不释放）
   * </ul>
   *
   * <p>首次 INCR 时自动设置 TTL，防止节点宕机导致计数泄漏。 Redis 操作失败时降级放行（不阻塞任务执行）。
   *
   * @param tenantId 租户 ID
   */
   ...
   * 记录任务执行结束（P7-3）。
   *
   * <p>DECR {@code ydsz:quota:concurrent:{tenantId}} 计数器，保证不会为负。 日执行计数器不释放（跨天自动过期）。 Redis 操作失败时仅记录
   * WARN 日志（不影响主流程）。
   *
   * @param tenantId 租户 ID
   */
