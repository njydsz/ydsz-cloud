  ...
  protected void doHealthCheck(Health.Builder builder) {
    ...
    if (leaderElector != null && cronjobProperties.getLeader().isEnabled()) {
      try {
        ...
        Map<String, Object> leaderInfo = new LinkedHashMap<>(COLLECTION_CAPACITY);
    ...
    // 4. 调度器配置摘要
    Map<String, Object> schedulerInfo = new LinkedHashMap<>(COLLECTION_CAPACITY);
    ...
    if (jobNodeRepository != null) {
      try {
        Map<String, Object> nodeHealth = new LinkedHashMap<>(COLLECTION_CAPACITY);
    ...
    if (retryRepo != null) {
      try {
        Map<String, Object> retryInfo = new LinkedHashMap<>(COLLECTION_CAPACITY);
