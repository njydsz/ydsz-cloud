 ...
 *
 * <p>每次 Leader 切换时递增 Fencing Token，旧 Leader 残留的写操作会因 Token 过期被拒绝。 通过 Redis INCR 原子递增保证 Token
 * 的全局单调递增。
 *
 * <h3>脑裂场景</h3>
 *
 * <ol>
 *   <li>Leader A 因网络分区与 Redis 断连，但本地仍认为自己是 Leader
 *   <li>Leader B 抢占成功，获得新的 Fencing Token (N+1)
 *   <li>Leader A 恢复连接后尝试写操作，附带旧 Token (N)
 *   <li>写操作前检查 Token：N &lt; N+1，拒绝 Leader A 的写操作
 * </ol>
 *
 * <h3>使用方式</h3>
 *
...
public class FencingTokenManager {
  ...
  private static final String FENCING_TOKEN_PREFIX = "ydsz:job:fencing:token:";
   ...
   *
   * <p>通过 Redis INCR 原子递增，保证 Token 全局单调递增。 获取成功后更新本地缓存。
   *
   ...
   *
   * <p>对比本地 Token 与 Redis 中的最新 Token，若本地 Token 小于 Redis Token， 说明已发生 Leader 切换，当前节点的写操作应被拒绝。
   *
   * @param role Leader 角色
  ...
  public boolean validateToken(String role, long localToken) {
    ...
    try {
      ...
      if (localToken < redisToken) {
        log.warn(
            "[FencingToken] Token 过期, 拒绝操作: role={} localToken={} redisToken={} (脑裂防护)",
            role,
            localToken,
            redisToken);
        return false;
      ...
      log.error("[FencingToken] Redis Token 格式异常: role={} value={}", role, redisStringOps.get(key));
      return false;
    } catch (Exception e) {
      // Redis 异常时放行（避免 Redis 故障导致整个系统不可用）
      log.warn(
          "[FencingToken] 校验异常, 放行: role={} localToken={} reason={}",
          role,
          localToken,
          e.getMessage());
      return true;
