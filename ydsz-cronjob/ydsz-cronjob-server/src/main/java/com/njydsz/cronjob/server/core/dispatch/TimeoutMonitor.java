 ...
 * 任务执行超时监控器（P2-4）。
 *
 * <p>仅当 {@code ydsz.cronjob.leader.enabled=true} 且当前节点是 Leader 时启用。 定时（默认 10s）扫描 {@code
 * ydsz_job_log} 中 {@code status='RUNNING'} 且 {@code start_time + job.timeout_ms < NOW()} 的日志，标记为
 * TIMEOUT 并主动释放分布式锁。
 *
 * <h3>执行流程</h3>
 *
 * <ol>
 *   <li>检查 Leader 身份（非 Leader 直接返回，避免重复扫描）
 *   <li>查询超时日志（JOIN ydsz_job 取 timeout_ms）
 *   <li>对每条超时日志：
 *       <ul>
 ...
 *
 * <ul>
 *   <li>仅当 timeout_ms 非空时才检测（null 表示不限超时）
 *   <li>标记 TIMEOUT 后任务 status=ERROR，避免下次扫描重复处理
 *   <li>不主动中断执行中的线程（Java 无法安全中断外部线程）， 仅通过释放锁让其他节点可以重新派发
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class TimeoutMonitor {
  ...
  private void scanTimedOutLogs(LocalDateTime now) {
    ...
    for (JobLog log0 : timedOut) {
      try {
        ...
        log.error(
            "[TimeoutMonitor] 处理超时日志失败: logId={} jobKey={} reason={}",
            log0.getId(),
            log0.getJobKey(),
            e.getMessage(),
            e);
  ...
  protected void handleTimeout(JobLog log0, LocalDateTime now) {
    ...
    if (holder != null && !holder.isBlank()) {
      ...
      log.debug(
          "[TimeoutMonitor] 日志无 lockHolder, 跳过锁释放: logId={} jobKey={}",
          log0.getId(),
          log0.getJobKey());
    }
    ...
    try {
      ...
      log.error("[TimeoutMonitor] 更新任务统计失败: jobId={} reason={}", log0.getJobId(), e.getMessage());
    }
    // P5: 触发超时告警
  ...
  private void triggerTimeoutAlert(JobLog log0, long durationMs) {
    ...
    try {
      ...
      log.warn(
          "[TimeoutMonitor] 触发超时告警失败(不影响主流程): logId={} reason={}", log0.getId(), e.getMessage());
    }
  }
}
