 ...
 *
 * <ul>
 *   <li>幂等任务（如状态同步、缓存刷新）：FIRE_NOW
 *   <li>非幂等任务（如发邮件、扣款）：SKIP（避免重复执行）
 *   <li>报表类任务（合并多次数据）：COALESCE
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public enum MisfirePolicy {
   ...
   *
   * <p>适用于非幂等任务（如发邮件、扣款），避免重复执行造成业务影响。 对标 Quartz {@code MISFIRE_INSTRUCTION_DO_NOTHING}。
   */
  SKIP,
   ...
   *
   * <p>适用于报表类任务，需要执行但需记录 Misfire 历史。 对标 Quartz {@code MISFIRE_INSTRUCTION_FIRE_ONCE_NOW}。
   */
  COALESCE;
