 ...
 * </ul>
 *
 * <p>Bean 名称 = {@code searchIndexRebuildJobHandler}，
 * 在 ydsz_job 表插入记录：handler=searchIndexRebuildJobHandler。
 *
 * <p><b>调度建议：</b>
 * <ul>
 *   <li>按需手动触发（运维操作），不建议高频定时执行</li>
 *   <li>如需定时，建议在业务低峰期（如每周日凌晨 03:00）执行一次</li>
 * </ul>
 *
 * <p>参数 JSON 格式：
 * <ul>
 *   <li>{@code null} 或 {@code ""} — 重建全部类型、全租户</li>
 *   <li>{@code {"type":"wiki"}} — 仅重建 wiki 类型</li>
 *   <li>{@code {"type":"wiki","tenantId":"xxx"}} — 仅重建指定租户的 wiki</li>
 * </ul>
 *
 * <p><b>注意事项：</b>全量重建是高耗时操作，底层使用单线程池串行执行，
 * 同时只允许 1 个重建任务运行。提交前请通过 {@code IndexRebuildService.isRebuilding()} 检查是否有任务在执行。
 *
 * @author ydsz-team
 * @since 1.0.0
