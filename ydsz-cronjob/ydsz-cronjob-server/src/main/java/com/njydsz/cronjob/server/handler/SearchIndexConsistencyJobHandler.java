 ...
 * 搜索索引一致性巡检 Job。
 *
 * <p>定时对比数据库文档数与搜索索引文档数，检测索引丢失或冗余，
 * 并对不一致项自动修复（丢失文档重新索引，冗余文档删除索引）。
 *
 * <p><b>双重索引巡检：</b>同时检查 nw_search_index（DB 降级存储）和
 * ydsz_search_index（统一搜索主索引）与实际业务数据的一致性。
 *
 * <p>Bean 名称 = {@code searchIndexConsistencyJobHandler}，
 * 在 ydsz_job 表插入记录：handler=searchIndexConsistencyJobHandler。
 *
 ...
 * <p>参数 JSON 格式：
 * <ul>
 *   <li>{@code null} 或 {@code ""} — 全部租户巡检</li>
 *   <li>{@code {"tenantId":"xxx"} } — 仅巡检指定租户</li>
 *   <li>{@code {"tenantId":"xxx","types":["job","dag"]}} — 仅巡检指定类型</li>
 * </ul>
 *
 * <p><b>依赖触发：</b>仅当 classpath 存在 {@code ydsz-common-search} 时激活。
 * 未引入搜索模块的应用不会装配本 Handler，也不会报错。
 *
 * @author ydsz-team
 * @since 1.0.0
