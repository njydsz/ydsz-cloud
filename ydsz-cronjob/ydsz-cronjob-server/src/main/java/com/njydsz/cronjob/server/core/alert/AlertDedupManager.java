 ...
 *
 * <ul>
 *   <li><b>时间窗口聚合</b>：同一规则在聚合窗口内的多次告警合并为一条
 *   <li><b>频次升级</b>：窗口内告警次数超过阈值时，升级通知渠道（追加短信/电话）
 *   <li><b>自动降级</b>：长时间无告警后恢复原始通知通道
 *   <li><b>同任务去重</b>：同一任务的同类告警在短时间窗口内只通知一次
 * </ul>
 *
 * <h3>工作流程</h3>
 *
 * <ol>
 *   <li>告警事件到达 → 检查聚合窗口
 *   <li>窗口内已有告警 → 计数+1，判断是否需要升级
 *   <li>窗口内无告警 → 通过，创建新窗口
 *   <li>计数超过 maxAggregateCount → 追加升级通道
 *   <li>超过降级冷却时间无告警 → 重置升级状态
 * </ol>
 *
 * <p>仅在 {@code ydsz.cronjob.alert-dedup.enabled=true} 时启用。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class AlertDedupManager {
  ...
  /** Redis key 前缀：告警聚合计数 */
  private static final String AGGREGATE_COUNT_PREFIX = "ydsz:alert:dedup:count:";

  /** Redis key 前缀：升级状态 */
  private static final String ESCALATION_PREFIX = "ydsz:alert:dedup:escalate:";
   ...
   * @return 降噪决策结果
   */
  public DedupDecision checkAndDedup(
      String ruleId, String jobId, String alertType, String origChannels) {
    ...
    try {
      // 原子递增计数
      Long count = redisStringOps.incr(aggregateKey, 1);
      if (count == null) {
        count = 1L;
      }
      ...
      // 首次告警：设置窗口 TTL
      if (count == 1) {
        redisStringOps.expire(aggregateKey, Duration.ofSeconds(config.getAggregateWindowSeconds()));
        // 首次告警，使用原始通道发送
      ...
      if (count > config.getMaxAggregateCount()) {
        ...
        redisStringOps.set(
            escalateKey, "1", Duration.ofSeconds(config.getDowngradeCooldownSeconds()));

        String escalatedChannels = mergeChannels(origChannels, config.getEscalateChannels());
        log.warn(
            "[AlertDedup] 告警升级: ruleId={} alertType={} count={} channels={}",
            ruleId,
            alertType,
            count,
            escalatedChannels);
      ...
      // 窗口内但未超阈值，抑制告警
      log.debug(
          "[AlertDedup] 告警抑制(窗口内): ruleId={} alertType={} count={}", ruleId, alertType, count);
