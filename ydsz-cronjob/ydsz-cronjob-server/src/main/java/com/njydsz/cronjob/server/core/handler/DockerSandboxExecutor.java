 ...
 *
 * <ul>
 *   <li><b>文件隔离</b>：容器内独立文件系统，无法访问宿主机
 *   <li><b>网络隔离</b>：可配置 --network=none 禁止网络访问
 *   <li><b>资源限制</b>：CPU / 内存 / PID 限制，防止资源耗尽（P2-11: 全部可配置）
 *   <li><b>权限降级</b>：以非 root 用户运行（P2-11: 可配置用户）
 *   <li><b>环境变量</b>：支持向容器传递白名单环境变量（P2-11 新增）
 *   <li><b>输出安全</b>：异步读取 stdout，避免大输出导致管道阻塞死锁（P2-11 修复）
 * </ul>
 *
 * <h3>执行命令</h3>
 *
...
public class DockerSandboxExecutor {
  ...
  public SandboxResult execute(
    ...
    List<String> command =
        buildDockerCommand(sandboxConfig, image, interpreter, containerName, envVars);
    ...
    try {
      ...
      int effectiveTimeout =
          timeoutSeconds > 0 ? timeoutSeconds : sandboxConfig.getTimeoutSeconds();
