 ...
 *
 * <ol>
 *   <li>检测线程池满且新任务优先级高于队列中最低优先级任务
 *   <li>通过 Thread.interrupt() 中断低优先级任务的执行线程
 *   <li>释放线程池资源给高优先级任务
 *   <li>被中断的低优先级任务自动重试（标记为 PREEMPTED）
 * </ol>
 *
 * <h3>抢占条件</h3>
 *
 * <ul>
 *   <li>新任务优先级 ≤ 2（高优先级）
 *   <li>线程池活跃线程数 = maxPoolSize（已满）
 *   <li>运行中最任务的优先级 ≥ 5（低优先级）
 *   <li>优先级差 ≥ 3（确保抢占有意义）
 * </ul>
 *
 * <h3>安全措施</h3>
 *
 * <ul>
 *   <li>每个任务最多被抢占 3 次（防止饥饿）
 *   <li>被抢占任务自动调度重试（延迟 1s）
 *   <li>MANUAL 触发的任务不可被抢占
 * </ul>
 *
...
public class PreemptiveScheduler {
  ...
  public boolean tryPreempt(int newJobPriority, String localNodeId) {
    ...
    if (preemptCount >= MAX_PREEMPT_COUNT) {
      log.debug(
          "[Preemptive] 任务被抢占次数已达上限, 跳过: jobKey={} count={}",
          preemptable.getJobKey(),
          preemptCount);
      return false;
    }
    ...
    if (!interrupted) {
      log.warn(
          "[Preemptive] 中断任务线程失败: jobKey={} threadId={}",
          preemptable.getJobKey(),
          preemptable.getExecThreadId());
      return false;
    }
    ...
    log.info(
        "[Preemptive] 抢占成功: newPriority={} preemptedJobKey={} preemptedThreadId={}",
        newJobPriority,
        preemptable.getJobKey(),
        preemptable.getExecThreadId());
   ...
   * 查找可被抢占的低优先级运行中任务。
   *
   * <p>P0-3 修复：原实现将 runningPriority 硬编码为 5，导致抢占逻辑失效。 现在通过 jobId 批量关联查询 Job 获取实际优先级。
   *
   * <p>条件：
   *
   * <ul>
   *   <li>状态为 RUNNING
   *   <li>执行节点为当前节点（P0-3 修复：原实现未使用 localNodeId 参数）
   *   <li>触发类型非 MANUAL
   *   <li>优先级 ≥ PREEMPTABLE_PRIORITY
   *   <li>与新任务优先级差 ≥ PRIORITY_DIFF_THRESHOLD
   * </ul>
   *
  ...
  private JobLog findPreemptableTask(int newJobPriority, String localNodeId) {
    try {
      ...
      List<String> jobIds =
          runningLogs.stream().map(JobLog::getJobId).distinct().collect(Collectors.toList());
