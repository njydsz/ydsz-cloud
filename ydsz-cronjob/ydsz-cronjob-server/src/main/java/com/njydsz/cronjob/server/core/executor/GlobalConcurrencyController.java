 ...
 *
 * <ol>
 *   <li>任务派发前：{@link #tryAcquire} 原子递增全局并发计数器
 *   <li>计数器超过 {@code maxGlobalConcurrent} 时拒绝派发（返回 false）
 *   <li>任务执行完成：{@link #release} 原子递减计数器
 * </ol>
 *
 * <h3>与租户级配额的关系</h3>
 *
...
public class GlobalConcurrencyController {
   ...
   * 尝试获取全局并发配额。
   *
   * <p>原子递增全局并发计数器，如果递增后超过最大值则回滚并返回 false。 全局并发上限 = 单节点并发 × 在线节点数（动态获取，回退到配置值）。
   *
