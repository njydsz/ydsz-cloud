 ...
 *
 * <p>负责 {@link DagDefinition} 与 JSON 字符串之间的转换， 存储/读取 {@code JobDag.dagDefinition} 字段。
 *
 * <p>序列化使用 YdszJson POJO 自动序列化（record 字段已标记 @JsonProperty）， 反序列化保留手工解析以支持结构校验和向后兼容，避免 record
 * 构造函数兼容性问题。
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class DagDefinitionCodec {
   ...
   *
   * <p>1.1.0 重构：DagNode/DagEdge 已添加 @JsonProperty 和 @JsonClass 注解， 直接委托 YdszJson 引擎完成序列化，消除 50+
   * 行手工树构建代码。
   *
  ...
  public DagDefinition fromJson(String json) {
    ...
    // 解析 nodes
    List<DagNode> nodes = new ArrayList<>(COLLECTION_CAPACITY);
    ...
    // 解析 edges（可为空）
    List<DagEdge> edges = new ArrayList<>(COLLECTION_CAPACITY);
    ...
    if (edgesArr != null) {
      for (int i = 0; i < edgesArr.size(); i++) {
        ...
        edges.add(new DagEdge(from, to, e.getString("failStrategy"), e.getString("condition")));
      }
    }
