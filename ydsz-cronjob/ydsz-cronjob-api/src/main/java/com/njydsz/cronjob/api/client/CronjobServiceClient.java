...
public interface CronjobServiceClient {
   ...
   *
   * <p>对应 cronjob 模块: POST /api/cronjob/{id}/trigger?holdLock=false
   *
   * @param jobId 任务 ID
   ...
   *
   * <p>对应 cronjob 模块: GET /api/cronjob/{id}。返回 Map 以解耦 api 模块对 domain VO 的依赖。
   *
   * @param jobId 任务 ID
   ...
   * P2-F8: 暂停任务。
   *
   * <p>对应 cronjob 模块: POST /api/cronjob/{id}/pause。暂停后调度器不再触发该任务，
   * 正在执行的任务继续完成。
   *
   * @param jobId 任务 ID
   * @return 统一响应结果
   ...
   * P2-F8: 恢复任务。
   *
   * <p>对应 cronjob 模块: POST /api/cronjob/{id}/resume。恢复后按 cron 表达式重新排程。
   *
   * @param jobId 任务 ID
   * @return 统一响应结果
   */
