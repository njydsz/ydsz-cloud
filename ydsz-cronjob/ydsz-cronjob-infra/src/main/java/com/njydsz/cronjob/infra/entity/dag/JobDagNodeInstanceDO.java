 ...
 *
 * <p>记录 DAG 实例中每个任务节点的执行状态。一个 DAG 实例包含若干节点实例， 每个节点实例关联一个任务执行日志（ydsz_job_log）。
 *
 * <p>节点状态流转：
 *
 * <ul>
 *   <li>PENDING → RUNNING（开始执行）
 *   <li>RUNNING → SUCCESS / FAILED
 *   <li>RUNNING → RETRYING → RUNNING（节点级重试）
 *   <li>PENDING → SKIPPED（前置失败且 FAIL_FAST 时跳过）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class JobDagNodeInstanceDO extends MpBaseEntity<String> {
