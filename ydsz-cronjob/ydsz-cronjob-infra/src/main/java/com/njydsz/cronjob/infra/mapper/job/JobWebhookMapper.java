...
public interface JobWebhookMapper extends BaseMapper<JobWebhook> {
   ...
   *
   * <p>仅返回 {@code status = 'ACTIVE'} 且未逻辑删除的记录，按事件类型拉取回调配置， 供 {@code WebhookEventDispatcher}
   * 在对应事件触发时批量推送。
   *
   ...
   */
  @Select(
      "SELECT id, name, event_type, job_key, job_group, callback_url, http_method, "
   ...
   *
   * <p>{@code job_key} 为 null 的记录视为「全局 WebHook」，对所有任务生效； 因此查询条件包含 {@code job_key = #{jobKey} OR
   * job_key IS NULL}， 保证任务专属回调与全局回调都能命中。
   *
  ...
  List<JobWebhook> selectActiveByEventAndJob(
      @Param("eventType") String eventType, @Param("jobKey") String jobKey);
}
