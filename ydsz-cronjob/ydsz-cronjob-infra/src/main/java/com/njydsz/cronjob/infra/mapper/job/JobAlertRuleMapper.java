...
public interface JobAlertRuleMapper extends BaseMapper<JobAlertRule> {
   ...
   */
  @Select(
      "SELECT id, rule_name, job_id, job_key, alert_type, alert_level, "
   ...
   *
   * <p>用于 FAIL_RATE / DURATION_P95 等需要周期性统计的告警类型， 由 {@code AlertScanner} 定时调用。
   *
  ...
  int updateLastAlertAtIfNotInCooldown(
