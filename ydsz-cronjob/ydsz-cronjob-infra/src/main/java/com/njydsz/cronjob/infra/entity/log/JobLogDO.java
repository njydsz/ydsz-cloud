 ...
 *
 * <p>P1-2: 新增执行轨迹字段（{@link #queueTime}/{@link #dispatchTime}/ {@link #handlerInitTime}/{@link
...
public class JobLogDO extends MpBaseIdEntity<String> {
   ...
   * 分片索引（P1-4：远程派发支持）。
   *
   * <p>非分片任务为 null；分片任务为 0-based 索引。 供 JobNodeReaper 故障转移时重建分片锁 key（{@code
   * ydsz:job:lock:{jobKey}:shard:{shardIndex}}）。
   */
   ...
   * 慢任务阈值快照（毫秒）。
   *
   * <p>执行时从 {@code ydsz_job.slow_threshold_ms} 快照到日志记录， NULL=未配置慢任务检测。快照保留执行时的阈值，避免后续修改 job
   * 配置影响历史判定。
   */
