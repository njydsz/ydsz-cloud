 ...
 *
 * <p>对应 {@code ydsz_job_task} 表，提供按 logId 查询子任务、统计指定状态子任务数、 更新子任务状态等操作，供 {@code MapTaskExecutor}
 * 管理子任务生命周期。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public interface JobTaskMapper extends BaseMapper<JobTask> {
   ...
   */
  @Select(
      "SELECT id, job_id, log_id, job_key, task_name, task_params, task_type, status, "
   ...
   */
  @Select(
      "SELECT COUNT(1) FROM ydsz_job_task "
          + "WHERE log_id = #{logId} AND status = #{status} AND deleted = 0")
  int countByLogIdAndStatus(@Param("logId") String logId, @Param("status") String status);
   ...
   *
   * <ul>
   *   <li>执行前：status=PENDING → RUNNING（result/errorMessage 为 null）
   *   <li>执行后：status=RUNNING → SUCCESS/FAILED（填充 result/errorMessage）
   * </ul>
   *
   * @param id 子任务 ID
   ...
   */
  @Update(
      "UPDATE ydsz_job_task SET exec_node_id = #{execNodeId}, updated_at = #{now} "
          + "WHERE id = #{id} AND deleted = 0")
  int updateExecNodeId(
      @Param("id") String id,
   ...
   *
   * @param before 过期分界时间（删除 created_at 早于该时间的子任务记录）
   * @param limit 单批最多删除条数
   * @return 实际删除条数
   */
  @Delete(
      "DELETE FROM ydsz_job_task "
          + "WHERE ctid = ANY(ARRAY("
          + "  SELECT ctid FROM ydsz_job_task "
