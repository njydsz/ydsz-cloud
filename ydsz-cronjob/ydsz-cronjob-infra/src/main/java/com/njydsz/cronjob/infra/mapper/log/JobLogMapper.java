...
public interface JobLogMapper extends BaseMapper<JobLog> {
     ...
     *
     * <p>JOIN ydsz_job 取 timeout_ms，筛选 {@code log.status='RUNNING'}
     ...
     *
     * <p>用于 {@link com.njydsz.cronjob.server.core.dispatch.FailoverScanner} 扫描
     * 有 RUNNING 任务但可能已下线的节点。对比在线节点列表后找出下线节点。
     *
     * @return 有 RUNNING 任务的节点 ID 列表（去重）；无记录时返回空列表
     */
    @Select("SELECT DISTINCT exec_node_id FROM ydsz_job_log "
     ...
     * 避免大事务锁表。由 {@link com.njydsz.cronjob.server.core.cleaner.LogCleaner} 循环调用直至无数据。
     *
     * @param before 过期分界时间（此时间之前的记录将被删除）
     * @param limit  单批最多删除条数
     * @return 实际删除条数
     */
    @Delete("DELETE FROM ydsz_job_log "
            + "WHERE id IN ("
