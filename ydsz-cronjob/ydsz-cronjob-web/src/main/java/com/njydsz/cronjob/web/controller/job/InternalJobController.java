...
public class InternalJobController {
   ...
   *
   * <ol>
   *   <li>参数校验：job 和 jobKey 必填
   *   <li>从请求中恢复 traceId 到 MDC（保证全链路追踪）
   *   <li>调用 {@link TaskDispatcher#executeLocally} 在本地执行任务
   *   <li>finally 中清理 traceId，避免线程复用导致 MDC 污染
   * </ol>
   *
   * <p>注意：执行异常时不返回 error，而是返回 success(null)。原因是执行器端已记录 FAILED 日志， 调用方通过日志 ID 即可查询失败原因；返回 error 会导致
   * Leader 误判为派发失败触发重试。
   *
  ...
  public BaseResponse<String> execute(@RequestBody RemoteTaskRequest request) {
    ...
    try {
      log.info(
          "[InternalJob] 接收远程派发: key={} triggerType={} shard={}/{} traceId={}",
   ...
   *
   * <ol>
   *   <li>参数校验：jobKey / handler 必填
   *   <li>从请求中恢复 traceId 到 MDC
   *   <li>从 ApplicationContext 通过 handler Bean 名称获取 MapProcessor
   *   <li>构造子任务 MapContext，调用 processor.process()
   *   <li>返回执行结果（含 success/result/errorMessage）
   * </ol>
   *
   * <p>注意：与 {@link #execute} 不同，本接口始终返回 success（包含执行结果对象）， 因为子任务执行结果是 Leader 端做汇总归并的依据，需要传回。
   *
