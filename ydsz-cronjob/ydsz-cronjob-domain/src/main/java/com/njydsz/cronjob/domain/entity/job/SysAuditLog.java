 ...
 *
 * <p>对应 <code>ydsz_com_audit_log</code> 表（由 ydsz-common-audit 模块管理），
 * 本实体仅用于查询，不执行写入操作。写入由 common-audit 模块的 {@code AuditWriter} 完成。
 *
 * <p>cronjob 模块通过 {@code module = 'cronjob'} 过滤只查看与任务调度相关的审计记录。
 *
 * <p><b>注意</b>：本实体不继承 {@code MpBaseIdEntity}，因为 ydsz_com_audit_log 表可能不存在于
