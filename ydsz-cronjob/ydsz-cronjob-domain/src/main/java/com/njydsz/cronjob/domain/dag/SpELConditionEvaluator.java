 ...
 *
 * <p>用于 DAG 条件分支节点（CONDITION）的表达式评估， 支持从上下文中读取上游节点的执行结果进行条件判断。
 *
 * <p>表达式格式：{@code ${nodeA.result=='success'}} 或 {@code #nodeA.status!='FAILED'}
 *
 * <p><b>缓存机制：</b> 表达式解析结果使用实例级 {@link Cache}（ydsz-common-cache）缓存，避免重复解析相同表达式。
 *
 * <ul>
 *   <li>有界 LRU 淘汰（默认 1024），由 ydsz-common-cache {@code maximumSize} 实现，无全表锁，高并发下性能远优于 {@code
 *       Collections.synchronizedMap(LinkedHashMap)} 方案
 *   <li>缓存为实例级而非静态级，避免 classloader 级内存泄漏
 *   <li>支持 {@link #clearCache()} 在运行时清理缓存
 *   <li>支持 {@link #getCacheStats()} 获取命中率/淘汰数等统计指标（配合健康检查/监控）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class SpELConditionEvaluator {
  ...
  private static Cache<String, Expression> createCache(int maxSize) {
    var builder =
        YdszCache.<String, Expression>newBuilder().name("cronjob:spel-expr").recordStats();
    if (maxSize > 0) {
  ...
  public boolean evaluate(String expression, Map<String, Object> context) {
    ...
    try {
      ...
      log.warn(
          "[SpELConditionEvaluator] 表达式评估失败, 返回 false: expr={} reason={}",
          expression,
          e.getMessage());
      return false;
    }
   ...
   *
   * <p>仅当缓存启用且 {@code recordStats()} 生效时返回有意义的数据； 缓存未启用时返回空统计（全 0）。
   *
