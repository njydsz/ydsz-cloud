 ...
 *
 * <p>使用 {@link TransmittableThreadLocal}（TTL），在线程池场景下配合
 * {@code com.alibaba.ttl.TtlRunnable#get(Runnable)} 包装任务，即可实现
 * 父线程上下文自动传播到线程池工作线程，无需 TaskDecorator。
 *
 * <p><b>线程池使用约束：</b>使用 {@link #setLogger(JobLogger)} /
 * {@link #setShardingContext(ShardingContext)} 后，若需提交到线程池，
 * 必须在提交前通过 {@code TtlRunnable.get(task)} 包装任务；否则线程池内将无法读取上下文。
 * 推荐在任务入口处使用 {@link ExecutionContextScope#of(JobLogger, ShardingContext)}
 * 结构化生命周期管理。
 *
 * @author ydsz-team
...
public final class JobExecutionContext {
  ...
  private static final TransmittableThreadLocal<JobLogger> LOGGER = new TransmittableThreadLocal<>();
