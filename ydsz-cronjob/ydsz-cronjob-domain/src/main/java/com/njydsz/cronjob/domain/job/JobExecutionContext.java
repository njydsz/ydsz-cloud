 ...
 *
 * <p><b>注意：</b>使用 {@link InheritableThreadLocal}，仅在线程首次创建时继承父线程的值， 不会在线程复用时更新。线程池场景请使用
 * TaskDecorator 或在提交任务前 显式调用 {@link #setLogger(JobLogger)} / {@link
 * #setShardingContext(ShardingContext)}， 并在 finally 中 {@link #clear()}，避免上下文泄漏到下一个任务。
 *
 * @author ydsz-team
...
public final class JobExecutionContext {
  ...
  private static final InheritableThreadLocal<ShardingContext> SHARDING_CONTEXT =
