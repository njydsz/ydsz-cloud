 ...
 *
 * <ul>
 *   <li>{@code GET /agent/definitions} - 列出所有活跃 Agent 定义
 *   <li>{@code GET /agent/definitions/{id}} - 按主键 ID 获取 Agent 定义
 *   <li>{@code GET /agent/definitions/code/{code}} - 按业务 code 获取 Agent 定义（推荐入口）
 *   <li>{@code POST /agent/definitions} - 创建 Agent 定义
 *   <li>{@code PUT /agent/definitions} - 更新 Agent 定义
 *   <li>{@code DELETE /agent/definitions/{id}} - 删除 Agent 定义
 * </ul>
 *
 * <h3>核心概念</h3>
 *
 * <ul>
 *   <li>{@code AgentDefinition} - Agent 静态定义（system prompt / 工具列表 / 能力配置 / LLM 偏好）
 *   <li>{@code agentCode} - 业务侧 Agent 唯一编码（如 {@code order-analysis-agent}）
 *   <li>Agent 定义与执行实例解耦：定义存储为配置，运行时由 {@link AgentFactory} 按需加载
 * </ul>
 *
 * <h3>安全与稳定性</h3>
 *
 ...
 *   <li>查询 {@code list} 仅返回 {@code isActive=true} 的活跃定义（{@code listActive} 内部过滤）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class AgentDefinitionController {
   ...
   * 列出所有活跃的 Agent 定义。
   *
   * <p>仅返回 {@code isActive=true} 的 Agent 定义，按业务约定排序（通常按 created_at 倒序）。 供前端 Agent 列表页/下拉选择器使用。
   *
   ...
   *
   * <p>{@code code} 是 Agent 在业务侧的稳定标识（不随主键变化），推荐作为外部调用入口。 与 {@link #getById} 的区别：code
   * 是用户/调用方可见的语义编码，id 是系统内部的主键。
   *
   ...
   * 更新 Agent 定义。
   *
   * <p>全量更新指定 ID 的 Agent 定义（PUT 语义），必须传入完整定义。 局部更新场景建议用 PATCH 接口（暂未提供，必要时基于此方法扩展）。
   *
   ...
   * 删除 Agent 定义（逻辑删除）。
   *
   * <p>调用 {@link AgentDefinitionService#removeById} 删除指定 ID 的 Agent 定义； 是否为物理删除由 Service
   * 层决定（推荐使用逻辑删除 + 状态字段标记）。
   *
   * @param id Agent 定义主键
