 ...
 * MCP 工具适配器
 *
 * <p>将 MCP Server 暴露的工具描述转换为内部 {@link ToolDefinition} 格式，
 * 并注册到 {@link com.njydsz.agent.domain.tool.ToolRegistry} 中。
 *
 * <p>支持多个 MCP Server 的工具统一管理，工具名前缀为 MCP Server 名称以避免冲突。
 *
 * <h3>集成说明</h3>
 * <p>实际 MCP Client SDK（io.modelcontextprotocol:sdk）依赖由 ydsz-agent-infra 的 pom.xml 引入。
 * 本适配器通过 {@link McpClientProvider} 解耦具体 SDK 实现，便于单元测试替换。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
