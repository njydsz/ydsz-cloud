 ...
 * PII 脱敏输出护栏（P1-3 优化）。
 *
 * <p>对 LLM 输出中的个人身份信息（PII）进行脱敏。委托
 * {@link SensitiveUtil#scanAndMask(String)} 完成统一的 PII 识别与脱敏。
 *
 * <p><b>覆盖类型：</b>
 * <ul>
 *   <li>手机号：委托 {@link SensitiveUtil} 标准脱敏（前3后4，中间星号）</li>
 *   <li>身份证号：委托 {@link SensitiveUtil} 标准脱敏（前3后5，中间8位星号）</li>
 *   <li>邮箱：委托 {@link SensitiveUtil} 标准脱敏（首尾字符保留，中间星号）</li>
 *   <li>银行卡号：委托 {@link SensitiveUtil} 标准脱敏（后4位保留，其余星号）</li>
 *   <li>护照号：委托 {@link SensitiveUtil} 标准脱敏（前2后2保留，中间星号）</li>
 * </ul>
 *
 * <p><b>P1-3 重构说明：</b>
 * <ul>
 *   <li>原实现中 5 个 PII 正则（PHONE/ID_CARD/EMAIL/BANK_CARD/PASSPORT）在本类独立定义</li>
 *   <li>现统一委托 {@link SensitiveUtil#scanAndMask(String)}，与 workflow 模块、common-safe 模块
 *       共享同一套 PII 扫描正则，消除三处重复定义</li>
 *   <li>升级 PII 类型或调整正则只需修改 {@link SensitiveUtil} 一处</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
