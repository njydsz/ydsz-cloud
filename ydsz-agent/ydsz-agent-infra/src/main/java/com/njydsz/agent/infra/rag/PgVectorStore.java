 ...
 * </pre>
 *
 * <p><b>多租户隔离（P0 修复）</b>：本实现走 {@link JdbcTemplate} 原生 SQL，不经过 MyBatis
 * 租户拦截器，因此必须在 SQL 层显式追加 {@code tenant_id} 条件，否则跨租户检索会泄露数据。
 * 租户 ID 从 {@link TenantContextHolder} 解析；超级管理员/系统租户/跳过隔离场景不做过滤。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public class PgVectorStore implements VectorStore {
     ...
     * 重复入库同一文档块时更新内容、向量与元数据。启用租户隔离时写入 {@code tenant_id}。</p>
     *
    ...
    public void store(TextChunk chunk) {
        ...
        jdbcTemplate.update(sql,
                ...
                metadataJson,
                tenantId);
        log.debug("[VectorStore] 存储文本块: id={}, docId={}, tokens={}, tenantId={}",
                chunk.getId(), chunk.getDocumentId(), chunk.getTokenCount(), tenantId);
    }
