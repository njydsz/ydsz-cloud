 ...
 * <ol>
 *   <li>接收用户输入</li>
 *   <li>通过 LLM 分析任务，生成执行计划（含子任务列表）</li>
 *   <li>分发子任务到合适的 Worker Agent 执行</li>
 *   <li>汇总各 Worker 结果，生成最终回答</li>
 * </ol>
 *
 * <h3>适用场景</h3>
 * <p>适合需要多种能力协作的复杂任务，例如：
 * "帮我分析项目进度，查询相关文档，然后生成一份报告"
 * 可分解为 REACT（查项目）+ RAG（查文档）→ CHAT（生成报告）。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public class SupervisorAgentExecutor implements AgentExecutor {
    ...
    public SupervisorAgentExecutor(LlmClient llmClient, ConversationMemory memory,
    ...
    public ChatResponse execute(AgentExecutionRequest request) {
        ...
        traceRecorder.recordStep(traceId, "PLAN", "Task plan created",
                userInput, subTasks.size() + " subtasks", 0);

        // 2. 执行子任务并汇总结果
        List<String> results = new ArrayList<>(subTasks.size());
        TokenUsage totalUsage = TokenUsage.zero();
        for (SubTask subTask : subTasks) {
            AgentExecutionRequest subRequest = AgentExecutionRequest.builder()
            ...
            try {
                ...
                results.add(workerResponse.getContent());
                if (workerResponse.getUsage() != null) {
                    totalUsage = totalUsage.add(workerResponse.getUsage());
                }
                traceRecorder.recordStep(traceId, "SUB_TASK_DONE",
                        "Sub-task " + subTask.id() + " completed",
                        subTask.description(), workerResponse.getContent(),
                        0);
            } catch (Exception e) {
                log.error("[Supervisor] 子任务 {} 执行失败: {}", subTask.id(), e.getMessage());
                results.add("[子任务 " + subTask.id() + " 执行失败: " + e.getMessage() + "]");
                traceRecorder.recordStep(traceId, "SUB_TASK_ERROR",
                        "Sub-task " + subTask.id() + " failed",
                        subTask.description(), e.getMessage(), 0);
            }
        }
        ...
        log.info("[Supervisor] 执行完成: convId={}, subTasks={}, tokens={}",
                convId, subTasks.size(), totalUsage.getTotalTokens());
    ...
    public void executeStream(AgentExecutionRequest request, Consumer<ChatChunk> chunkConsumer) {
        ...
        chunkConsumer.accept(ChatChunk.finish(response.getId(), response.getModel(),
                response.getFinishReason(), response.getUsage()));
    }
    ...
    private List<SubTask> planTasks(String userTask, String traceId) {
        ...
        try {
            ...
            traceRecorder.recordStep(traceId, "LLM_PLAN",
                    "Planning LLM call", planPrompt, planResponse.getContent(), 0);
            return parsePlanResponse(planResponse.getContent());
        } catch (Exception e) {
            log.warn("[Supervisor] 任务规划失败，使用单任务降级: {}", e.getMessage());
            // 降级为单 REACT 任务
            return List.of(new SubTask(1, "REACT", "直接处理用户请求: " + userTask));
        }
    }
    ...
    private List<SubTask> parsePlanResponse(String content) {
        ...
        try {
            ...
            com.njydsz.common.json.YdszJson.parseMap(json);
            // 简单解析：提取 type 和 description
            // 注：实际生产环境建议使用结构化输出或 JSON Schema 约束
            List<SubTask> tasks = new ArrayList<>();
            for (AgentDefinition.Type type : AgentDefinition.Type.values()) {
                if (json.contains(type.name()) && tasks.size() < 3) {
                    tasks.add(new SubTask(tasks.size() + 1, type.name(),
                            "执行" + type.name() + "任务"));
                }
            }
            return tasks.isEmpty()
                    ? List.of(new SubTask(1, "REACT", "处理用户请求"))
                    : tasks;
        } catch (Exception e) {
            log.warn("[Supervisor] 计划解析失败，使用默认任务: {}", e.getMessage());
            return List.of(new SubTask(1, "REACT", "处理用户请求"));
        }
    }
    ...
    private AgentExecutor createWorker(String type, AgentExecutionRequest request) {
        AgentDefinition def = new AgentDefinition(
                IdGenerator.nextIdStr(), "supervisor-worker", "Worker",
     ...
     */
    private record SubTask(int id, String type, String description) {
    }
}
