 ...
 * <p>支持以下表达式语法：
 * <ul>
 *   <li><b>变量引用</b>：{@code results['nodeId']} 或 {@code results["nodeId"]}</li>
 *   <li><b>字符串方法</b>：{@code .contains("x")}、{@code .equals("x")}、{@code .startsWith("x")}、{@code .endsWith("x")}、{@code .isEmpty()}</li>
 *   <li><b>逻辑运算</b>：{@code &&} (AND)、{@code ||} (OR)、{@code !} (NOT)</li>
 *   <li><b>比较运算</b>：{@code ==}、{@code !=}、{@code <}、{@code >}、{@code <=}、{@code >=}（数值比较）</li>
 * </ul>
 *
 * <p>表达式示例：
 * <pre>{@code
...
public final class DagConditionEvaluator {
    ...
    private static final Pattern VARIABLE_PATTERN = Pattern.compile("results\\['([^']+)'\\]|results\\[\"([^\"]+)\"\\]");

    /** 字符串方法调用模式 */
    private static final Pattern CONTAIN_PATTERN = Pattern.compile("\\.contains\\([\"']([^\"']+)[\"']\\)");
    private static final Pattern EQUALS_PATTERN = Pattern.compile("\\.equals\\([\"']([^\"']+)[\"']\\)");
    private static final Pattern STARTS_WITH_PATTERN = Pattern.compile("\\.startsWith\\([\"']([^\"']+)[\"']\\)");
    private static final Pattern ENDS_WITH_PATTERN = Pattern.compile("\\.endsWith\\([\"']([^\"']+)[\"']\\)");
     ...
     * @return 方法调用结果
     */
    private static boolean evaluateStringMethod(String expr, Pattern methodPattern,
     ...
     * <p>支持：
     * <ul>
     *   <li>results['nodeId'] → 从结果映射获取</li>
     *   <li>"literal" 或 'literal' → 字符串字面量</li>
     *   <li>其他 → 直接作为字面量返回</li>
     * </ul>
     *
     * @param expr    变量表达式
