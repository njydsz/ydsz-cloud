 ...
 * Prompt 管理服务
 *
 * <p>提供 Prompt 模板的 CRUD、版本管理和变量替换能力。
 * 底层使用数据库持久化，内存缓存加速热点读取。
 *
 * <h3>核心能力</h3>
 * <ul>
 *   <li>Prompt 模板 CRUD（数据库存储，重启不丢失）</li>
 ...
 *
 * <h3>缓存策略</h3>
 * <p>首次读取后缓存在内存中，写操作同步更新缓存与数据库，
 * 确保单实例内读取一致性。多实例部署时依赖数据库保证最终一致性。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class PromptManagementService {
    ...
    public PromptManagementService(PromptTemplateMapper templateMapper,
    ...
    public PromptTemplate create(String code, String name, String content,
        ...
        // 插入模板主表（初始版本号为 1）
        PromptTemplateDO templateDO = PromptTemplateDO.builder()
                .templateCode(code)
                .templateName(name)
                .content(content)
        ...
        // 更新缓存
        PromptTemplate template = new PromptTemplate(code, name, content,
                description, category, 1, now, now);
    ...
    public PromptTemplate update(String code, String content) {
        ...
        PromptTemplate updated = new PromptTemplate(
                existing.getTemplateCode(), existing.getTemplateName(), content,
                existing.getDescription(), existing.getCategory(),
                newVersion, existing.getCreatedAt(), now);
    ...
    public PromptVersion getVersion(String code, int version) {
        PromptVersionDO versionDO = versionMapper.selectOne(
        ...
        return new PromptVersion(code, version, versionDO.getContent(),
    ...
    public List<PromptVersion> listVersions(String code) {
        List<PromptVersionDO> versionDOs = versionMapper.selectList(
                new QueryWrapper<PromptVersionDO>()
                        .eq("template_code", code)
                        .orderByAsc("version"));
    ...
    public PromptTemplate rollback(String code, int targetVersion) {
        ...
        PromptTemplate rolledBack = new PromptTemplate(
                existing.getTemplateCode(), existing.getTemplateName(), pv.content(),
                existing.getDescription(), existing.getCategory(),
                newVersion, existing.getCreatedAt(), now);
        templateCache.put(code, rolledBack);
        log.info("[Prompt] 回滚模板: code={}, targetVersion={}, newVersion={}",
                code, targetVersion, newVersion);
        return rolledBack;
    }
    ...
    private synchronized void warmCacheIfNeeded() {
        ...
        List<PromptTemplateDO> allTemplates = templateMapper.selectList(
                new QueryWrapper<PromptTemplateDO>().eq("deleted", false));
        for (PromptTemplateDO t : allTemplates) {
            templateCache.put(t.getTemplateCode(),
                    new PromptTemplate(t.getTemplateCode(), t.getTemplateName(),
                            t.getContent(), t.getDescription(), t.getCategory(),
                            t.getCurrentVersion(), t.getCreatedAt(), t.getUpdatedAt()));
        }
        cacheWarmed = true;
    ...
    private PromptTemplateDO selectByCode(String code) {
        return templateMapper.selectOne(new QueryWrapper<PromptTemplateDO>()
                .eq("template_code", code)
                .eq("deleted", false)
                .last("LIMIT 1"));
    }
    ...
    private void insertVersion(String code, int version, String content, String changeNote) {
        PromptVersionDO versionDO = PromptVersionDO.builder()
                .templateCode(code)
                .version(version)
                .content(content)
