 ...
 *
 * <ul>
 *   <li>{@code SimpleAgentExecutor} — 单轮 LLM 调用（P0 已在 ChatService 实现）
 *   <li>{@code ReActAgentExecutor} — ReAct 模式（Thought→Action→Observation 循环）
 *   <li>{@code PlanExecuteAgentExecutor} — Plan-and-Execute 模式（P3）
 *   <li>{@code RouterAgentExecutor} — 路由器 Agent（P3）
 * </ul>
 *
