...
public final class ToolRegistration {
   ...
   *
   * <p>简化工具注册流程：名称 + 描述 + 入参声明 + 执行器， 内部自动组装 {@link ToolDefinition}。受简化约定限制， 入参类型固定为
   * string，复杂类型请直接构造 {@link ToolDefinition}。
   */
  public static class Builder {
     ...
     * 追加一个工具入参声明。
     *
     * <p>同名参数后注册者覆盖先注册者。受 Builder 简化约定限制， 参数类型固定生成为 {@code string}；若需要 object / number / array
     * 等复杂类型， 应绕过本 Builder 直接构造 {@link ToolDefinition}。
     *
