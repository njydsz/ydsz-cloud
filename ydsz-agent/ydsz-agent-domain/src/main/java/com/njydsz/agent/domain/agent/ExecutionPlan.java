 ...
 *
 * <p><b>线程安全</b>：非线程安全。status 字段与 steps 列表可变，且 replaceRemainingSteps 会修改列表， 须由单个执行线程独占访问或由外部加锁保护。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public final class ExecutionPlan implements Serializable {
   ...
   * 将计划流转为执行中状态。
   *
   * <p>状态机：{@code PENDING -> EXECUTING}。方法不校验前置状态，重复调用等价于幂等赋值， 由调度方保证只在开始执行时调用一次。
   */
   ...
   * 将计划流转为已完成状态。
   *
   * <p>状态机：{@code EXECUTING -> COMPLETED}。仅标记计划整体结果， 不会级联修改 {@link PlanStep} 的状态；调用前应通过 {@link
   * #isCompleted()} 确认所有步骤均已完成。
   */
   ...
   * 将计划流转为失败状态。
   *
   * <p>状态机：{@code EXECUTING -> FAILED}。终态，不支持回退； 若需要重试应由上层重新生成 {@link ExecutionPlan} 实例，而非复用已失败的计划。
   */
   ...
   * 判断计划是否已全部执行完成。
   *
   * @return {@code true} 当且仅当所有 {@link PlanStep} 的状态均为 {@code COMPLETED}； 只要存在任一 PENDING /
   *     EXECUTING / FAILED 步骤即返回 {@code false}
   */
   ...
   * 获取当前应执行的下一步骤。
   *
   * <p>按索引顺序返回第一个仍处于 {@code PENDING} 状态的步骤，作为执行游标； 步骤标记为 EXECUTING / COMPLETED / FAILED
   * 后即脱离候选队列，游标自动前移。
   *
   * @return 下一个待执行步骤；当所有步骤均已离开 {@code PENDING} 状态时返回 {@code null}， 调用方据此判定计划已执行完毕并结束执行循环
   */
  ...
  public void replaceRemainingSteps(int fromIndex, List<PlanStep> newSteps) {
    ...
    for (int i = 0; i < newSteps.size(); i++) {
      steps.add(
          new PlanStep(
              fromIndex + i, newSteps.get(i).getDescription(), newSteps.get(i).getAction()));
    }
  }
   ...
   * 执行计划中的单个步骤。
   *
   * <p>由 Planner 生成并携带索引、描述与动作；步骤状态独立于计划整体状态流转， 支持在 Plan-and-Execute 执行过程中通过 {@link
   * ExecutionPlan#replaceRemainingSteps(int, List)} 重规划。
   *
  ...
  public static final class PlanStep implements Serializable {
     ...
     * 将步骤流转为已完成状态。
     *
     * <p>状态机：{@code EXECUTING -> COMPLETED}。完成后该步骤不再被 {@link #getCurrentStep()} 选中， 执行游标自动前移到下一个
     * {@code PENDING} 步骤。
     */
    public void markCompleted() {
     ...
     * 将步骤流转为失败状态。
     *
     * <p>状态机：{@code EXECUTING -> FAILED}。失败步骤同样脱离 {@code PENDING} 队列， 因此不会被重复挑选；重试需通过 {@link
     * #replaceRemainingSteps(int, List)} 重规划。
     */
    public void markFailed() {
     ...
     * 将步骤流转为执行中状态。
     *
     * <p>状态机：{@code PENDING -> EXECUTING}。用于标识当前占用执行线程的步骤， 便于链路追踪定位卡住的步骤。
     */
    public void markExecuting() {
