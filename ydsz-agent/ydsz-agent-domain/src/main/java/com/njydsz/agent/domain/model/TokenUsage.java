...
public final class TokenUsage implements Serializable {
   ...
   * 累加另一份用量并返回新实例（不可变语义）。
   *
   * <p>用于多轮对话/多 chunk 流式场景下的用量累计， 原实例保持不变，返回值为两者的逐项和。
   *
  ...
  public TokenUsage add(TokenUsage other) {
    ...
    return new TokenUsage(
        this.promptTokens + other.promptTokens, this.completionTokens + other.completionTokens);
  }
