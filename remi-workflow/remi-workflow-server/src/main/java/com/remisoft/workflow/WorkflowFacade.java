 ...
 *
 * <p>所有业务模块（project / execution / closure 等）只能依赖本接口，
 * 不允许直接引用 FlowEngine 内部服务，便于引擎隔离与升级。
 *
 * <p><b>设计模式：</b>门面模式（Facade Pattern）— 将引擎内部的 {@code FlowInstanceService}、
 * {@code FlowTaskService}、{@code FlowDefinitionService} 等多个 Service 的复杂调用编排
 * 封装为统一的粗粒度接口，降低业务模块与引擎的耦合度。
 *
 * <p><b>引擎实现：</b>基于 {@code remi_flow_*} 自建表（Warm-Flow 风格）的轻量级流程引擎，
 * 兼容 BPMN 2.0 标准流程文件（通过 {@code BpmnXmlParser} 解析 startEvent / userTask /
 * gateway / endEvent / sequenceFlow）。
 *
 * <p><b>能力矩阵：</b>
 * <ul>
 *   <li><b>流程实例</b>：启动 / 终止 / 挂起 / 激活 / 撤回 / 重审 / 批量启动</li>
 *   <li><b>任务办理</b>：通过 / 驳回 / 转办 / 委派 / 签收 / 跳转 / 自由流</li>
 *   <li><b>加签操作</b>：前加签 / 后加签 / 并加签 / 减签 / 追加处理人</li>
 *   <li><b>查询能力</b>：待办 / 已办 / 审批轨迹 / 时间线 / 流程图 / 流程回放</li>
 *   <li><b>批量操作</b>：批量通过 / 一键通过 / 批量驳回 / 批量转办</li>
 *   <li><b>辅助操作</b>：暂存待审 / 已阅 / 沟通 / 任务级挂起激活</li>
 * </ul>
 *
 * @since 1.0.0
 * @author remi-team
 *
 * @see com.remisoft.workflow.server.service.FlowInstanceService 流程实例服务
 * @see com.remisoft.workflow.server.service.FlowTaskService 任务服务
 * @see com.remisoft.workflow.domain.dto.FlowStartProcessDTO 启动参数 DTO
 * @see com.remisoft.workflow.domain.dto.FlowTaskOperateDTO 任务操作 DTO
 */
public interface WorkflowFacade {
     ...
     * 启动流程
     *
     * @return 流程实例 ID（remi_flow_instance.id，字符串形式）
     */
     ...
     * 查询用户待办任务列表（分页）
     *
     * <p>对标钉钉/飞书审批中心「我的待办」Tab，查询当前用户名下所有 PENDING / CLAIMED 状态的任务。
     * 结果按创建时间降序排列，支持分页。
     *
     * @param userId 用户 ID
     * @param page   页码（从 1 开始）
     * @param size   每页大小
     * @return 待办任务 Map 列表（含 taskId / flowCode / instanceId / nodeCode / nodeName / assigneeName / createdAt）
     */
     ...
     * 查询用户已办任务列表（分页）
     *
     * <p>对标钉钉/飞书审批中心「我已审批」Tab，查询当前用户处理过的历史任务（APPROVED / REJECTED / DELEGATED）。
     * 数据来源为 {@code remi_flow_his_task} 归档表，结果按完成时间降序排列。
     *
     * @param userId 用户 ID
     * @param page   页码（从 1 开始）
     * @param size   每页大小
     ...
     * 前加签 — 在当前审批人之前插入额外审批人
     *
     * <p>对标钉钉/飞书「前加签」能力。加签后流程变为：被加签人 → 原审批人 → 下一节点。
     * 被加签人审批通过后，流程回到原审批人继续办理。
     *
     ...
     * 后加签 — 在当前审批人之后插入额外审批人
     *
     * <p>对标钉钉/飞书「后加签」能力。加签后流程变为：原审批人 → 被加签人 → 下一节点。
     * 原审批人审批通过后，流程流转到被加签人办理，而非直接进入下一节点。
     *
     * @param dto 任务操作参数（需含 taskId + targetUserId + targetUserName）
     */
     ...
     * 同时记录催办日志。催办频率受 SLA 配置的 {@code reminderCount} 上限控制。
     *
     ...
     *
     * <p>对标钉钉/飞书「撤回」能力。仅当流程处于 RUNNING 状态且当前待办节点尚未被审批时允许撤回。
     * 撤回后流程回到发起人，发起人可修改表单后重新提交。
     *
     ...
     * 查询审批轨迹（审计日志）
     *
     * <p>返回流程实例从启动到当前的所有操作记录（含通过/驳回/转办/委派/加签/撤回等），
     * 按时间正序排列，用于前端审批详情页的「审批记录」列表展示。
     *
     * @param processInstanceId 流程实例 ID
     ...
     * 获取当前引擎类型标识
     *
     * <p>用于多引擎场景下区分底层实现（当前固定返回 {@code "REMI"}，标识自研引擎）。
     * 业务侧可据此判断引擎能力特性，如是否支持子流程、是否支持 DMN 决策等。
     *
     * @return 引擎类型字符串
     */
    String engineType();
     ...
     * 暂存待审 — 审批人保存审批意见草稿
     *
     * <p>对标钉钉/飞书「暂存」能力。审批人可在正式提交前先保存审批意见和附件，
     * 草稿存储在 Redis 中（TTL 7 天），提交后自动清除。
     *
     ...
     * 追加处理人 — 在已有会签任务中追加审批人
     *
     * <p>对标钉钉/飞书「加人」能力。在任务进入会签节点后，动态追加新的审批人参与会签，
     * 不影响已有审批人的审批状态。
     *
     * @param dto 任务操作参数（需含 taskId + targetUserId + targetUserName）
     */
     ...
     * 减签 — 从会签任务中移除指定审批人
     *
     * <p>对标钉钉/飞书「减签」能力。将会签任务中尚未办理的指定审批人移除，
     * 已办理的审批人不受影响。移除后会签完成条件相应调整。
     *
     ...
     * 已阅 — 标记任务已阅
     *
     * <p>对标钉钉/飞书「已阅」能力。抄送任务（CC）或会签任务中非办理人可以标记已阅，
     * 标记后不再在「待办」列表中展示该任务。
     *
     * @param taskId 任务 ID
     * @param userId 操作人 ID
     ...
     * 沟通 — 在任务下添加沟通评论
     *
     * <p>对标钉钉/飞书「沟通」能力。审批人可在任务下发起沟通，@提及其他同事提供意见，
     * 被沟通人收到通知后可回复评论。沟通不阻塞流程推进。
     *
