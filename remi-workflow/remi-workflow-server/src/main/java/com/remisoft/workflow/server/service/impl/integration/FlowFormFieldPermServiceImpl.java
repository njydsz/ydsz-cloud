 ...
 * 「<b>表单字段权限控制</b>」能力。允许业务方为审批表单的每个字段配置权限，
 * 控制不同节点的「可编辑 / 只读 / 隐藏 / 必填」语义，是大厂 B 端工作流
 * 「精细化表单控制」的标准能力。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li><b>配置解析</b>：{@link #parseFieldPerms} —
 *       解析节点 ext JSON 中的 {@code formFields} 配置为 {@code Map<fieldKey, perm>}</li>
 *   <li><b>权限校验</b>：{@link #validateFieldPerms} —
 *       校验用户提交的表单变量是否符合字段权限（HIDDEN 不允许提交 / READONLY 不允许修改）</li>
 *   <li><b>权限合并</b>：{@link #mergeFieldPerms} —
 *       合并多个权限 Map（多个节点的权限叠加）</li>
 *   <li><b>权限渲染</b>：{@link #renderFieldPermsForNode} —
 *       根据当前节点 + 用户角色返回前端表单的字段权限状态</li>
 * </ul>
 *
 * <p><b>权限类型（{@link #PERM_EDIT} 等常量）：</b>
 * <ul>
 *   <li>{@link #PERM_EDIT} — <b>可编辑</b>：用户可修改字段值（默认）</li>
 *   <li>{@link #PERM_READONLY} — <b>只读</b>：用户可查看但不能修改</li>
 *   <li>{@link #PERM_HIDDEN} — <b>隐藏</b>：字段不展示给用户，且提交时不允许包含该字段</li>
 *   <li>{@link #PERM_REQUIRED} — <b>必填</b>：用户必须填写该字段</li>
 * </ul>
 *
 ...
 * <p><b>设计要点：</b>
 * <ul>
 *   <li><b>空安全</b>：{@code formFieldsConfig} / {@code fieldPerms} 为空时直接返回空 Map，不抛异常</li>
 *   <li><b>解析失败降级</b>：JSON 解析失败时返回空 Map，业务方可重新配置</li>
 *   <li><b>大小写归一化</b>：权限值自动转大写（{@code edit} / {@code EDIT} 等同）</li>
 *   <li><b>无副作用</b>：{@link #validateFieldPerms} 仅做校验，<b>不修改</b>用户提交的变量</li>
 *   <li><b>职责清晰</b>：校验失败立即抛 {@link SysException}，由全局异常处理统一返回 400</li>
 * </ul>
 *
 * <p><b>典型使用：</b>
 * <pre>{@code
 ...
 *
 * <p><b>与流程设计器配合：</b>
 * <p>字段权限由流程设计器的「表单设计」面板配置，存储在节点 ext JSON 的 {@code formFields} 字段中。
 * 设计器根据本服务返回的权限状态控制字段的「可编辑 / 只读 / 隐藏」UI 表现。
 *
 * @author remi-team
 * @since 1.0.0
 *
