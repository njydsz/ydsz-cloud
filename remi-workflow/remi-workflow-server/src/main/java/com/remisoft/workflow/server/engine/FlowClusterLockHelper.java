 ...
 *
 * <p>实现：委托给 remi-common-lock 公共模块的 {@link DistributedLocker} 接口（默认实现为
 * {@link com.remisoft.common.lock.impl.RedisReentrantLock}），通过 Lua 脚本原子性获取可重入锁，
 * 获取失败时直接跳过本次执行（不阻塞等待）。锁 key 以 {@code remi:flow:schedule:} 为前缀，
 * TTL 略大于扫描间隔，防止任务未执行完锁就释放；WatchDog 自动续期由公共模块统一维护。
 *
 * <p>降级策略：{@link LockStrategy} Bean 不存在（单节点/测试环境未装配 remi-common-lock）时，
...
public class FlowClusterLockHelper {
    ...
    /** 锁 key 前缀 */
    private static final String LOCK_PREFIX = "remi:flow:schedule:";
