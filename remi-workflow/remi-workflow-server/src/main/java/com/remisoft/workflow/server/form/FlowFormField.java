package com.remisoft.workflow.server.form;
 ...
 * 字段定义存储在 {@code FlowNode.ext} JSON 的 {@code formSchema.fields} 数组中。
 *
 * <p>支持的字段能力：
 * <ul>
 *   <li>基础类型：文本/数字/金额/日期/选择/附件等（{@link FlowFormFieldType}）</li>
 *   <li>校验规则：必填/最小值/最大值/正则/最大长度/最小长度（{@link ValidationRule}）</li>
 *   <li>字段联动：当某字段值满足条件时，显示/隐藏/设置本字段（{@link LinkageRule}）</li>
 *   <li>子表单：SUB_FORM 类型支持嵌套字段列表 + 动态增删行</li>
 *   <li>公式计算：FORMULA 类型支持表达式自动计算</li>
