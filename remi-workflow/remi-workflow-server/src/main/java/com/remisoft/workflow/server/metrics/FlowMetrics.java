 ...
 * </ul>
 *
 * <p>所有指标前缀 {@code remi_flow_}，便于在 Grafana 看板中筛选。
 *
...
public class FlowMetrics extends AbstractModuleMetrics {
    ...
    /**
     * 待办任务创建计数，累加 {@code remi_flow_task_created_total}。
     *
    ...
    /**
     * 任务审批通过计数，累加 {@code remi_flow_task_passed_total}。
     *
    ...
    /**
     * 任务驳回计数，累加 {@code remi_flow_task_rejected_total}。
     *
    ...
    /**
     * 任务转办计数，累加 {@code remi_flow_task_transferred_total}。
     *
    ...
    /**
     * 任务委派计数，累加 {@code remi_flow_task_delegated_total}。
     *
    ...
    /**
     * 任务催办计数，累加 {@code remi_flow_task_urged_total}。
     *
    ...
    /**
     * 任务认领计数，累加 {@code remi_flow_task_claimed_total}。
     *
    ...
    /**
     * 任务跳过计数，累加 {@code remi_flow_task_skipped_total}。
     *
    ...
    /**
     * 任务自动处理计数，累加 {@code remi_flow_task_auto_handled_total}。
     *
    ...
    /**
     * 流程启动失败计数，累加 {@code remi_flow_start_error_total}。
     *
    ...
    /**
     * 流程撤回计数，累加 {@code remi_flow_recall_total}。
     *
    ...
    /**
     * SLA 超时计数，累加 {@code remi_flow_sla_timeout_total}。
     *
    ...
    private void registerGauges() {
        // 运行中实例数
        registry.gauge("remi_flow_instance_running", Tags.empty(), this, m -> m.getGaugeSnapshot().runningInstances());

        // 待办任务数
        registry.gauge("remi_flow_task_pending", Tags.empty(), this, m -> m.getGaugeSnapshot().pendingTasks());

        // 超期任务数
        registry.gauge("remi_flow_task_overdue", Tags.empty(), this, m -> m.getGaugeSnapshot().overdueTasks());

        // 抄送未读数
        registry.gauge("remi_flow_cc_unread", Tags.empty(), this, m -> m.getGaugeSnapshot().unreadCc());
    }
