...
import com.remisoft.workflow.server.engine.FlowSensitiveMasker;
 ...
 * 消息通知引擎（{@code remi-message}）之间的<b>轻量适配器</b>。
 *
 * <p><b>架构演进：</b>
 * <ul>
 *   <li>早期版本：通知基础设施（{@code outbox / template / channel / preference}）耦合在
 *       {@code remi-workflow} 模块内部，<b>已移除</b></li>
 *   <li>当前架构：通知能力由独立的<b>消息通知引擎</b> {@code remi-message} 承载，
 ...
 *   <li>消息可靠性由 {@code remi-message} 模块的 Outbox 模式保证</li>
 * </ul>
 *
 * <p><b>设计要点：</b>
 * <ul>
 *   <li><b>轻量适配器</b>：本类只做「事件 → 通知请求」转换 + Feign 调用，
 *       <b>不负责</b>通知模板渲染 / 通道选择 / 用户偏好（均由 {@code remi-message} 处理）</li>
 *   <li><b>敏感数据脱敏</b>：通过 {@link FlowSensitiveMasker} 对 {@code content} 字段脱敏，
 *       避免手机号 / 身份证 / 银行卡等敏感信息通过 IM 泄露</li>
 *   <li><b>幂等性</b>：通过 {@code providerTraceId} 实现通知幂等，
 *       同一事件多次通知只会发送一次（由 {@code remi-message} 侧保证）</li>
 ...
 * 在 {@code remi-message} 侧新增模板，本类无需修改即可支持。
 *
 * @author remi-team
 * @since 1.0.0
 *
