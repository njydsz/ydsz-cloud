...
public interface FlowJoinTokenService {
     ...
     *
     * <p>对标 BPMN 2.0 复杂网关和钉钉/飞书"部分到达即推进"能力。
     * 例如 5 个分支中 3 个到达即推进 → requiredCount=3。
     *
     * @param instanceId     流程实例 ID
     * @param joinNodeCode   join 节点编码
     ...
     */
    void initTokensWithRequired(String instanceId, String joinNodeCode,
     ...
     *
     * <p>与 {@link #arriveToken} 的区别：当 join 令牌使用
     * {@link #initTokensWithRequired} 初始化时，返回 true 的条件
     ...
     *
     * <p>当使用 {@link #initTokensWithRequired} 初始化时，
     * 检查到达数是否 &gt;= requiredCount。
     * 未设置 requiredCount 时回退到 {@link #allArrived} 语义。
     *
     * @param instanceId   流程实例 ID
     * @param joinNodeCode join 节点编码
