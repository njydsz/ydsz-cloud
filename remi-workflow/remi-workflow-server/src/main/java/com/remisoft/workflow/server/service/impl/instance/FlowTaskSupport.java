...
public class FlowTaskSupport {
    ...
    /** 运行时任务 Mapper，负责 {@code remi_flow_run_task} 表的查询 / 更新 / 状态扭转 */
    private final FlowRunTaskMapper taskMapper;

    /** 审计日志 Mapper，负责 {@code remi_flow_audit_log} 表的写入，承载任务操作审计轨迹 */
     ...
     * P2-35: Spring 事件发布器
     *
     * <p>用于异步发布 {@link FlowWorkflowEvent} 给跨模块监听器（{@code remi-message} 通知模块、
     * {@code remi-audit} 审计模块等），通过 {@code @EventListener} 或
     ...
     * 供跨模块监听器订阅（如 {@code remi-message} 通知模块的站内信 / 邮件推送、
     * {@code remi-audit} 审计模块的额外审计字段写入等）。
     *
