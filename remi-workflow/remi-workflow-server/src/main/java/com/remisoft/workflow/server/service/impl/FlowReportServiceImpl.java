...
import com.remisoft.workflow.server.service.FlowAnalyticsService;
 ...
 *       避免重复推送（{@code remi:flow:report:weekly:lock} / {@code remi:flow:report:monthly:lock}）</li>
 * </ul>
 *
 * <p><b>报告内容：</b>
 * <ul>
 ...
 * <p><b>数据来源：</b>
 * <ul>
 *   <li>{@code remi_flow_instance} — 流程实例表（活跃实例）</li>
 *   <li>{@code remi_flow_his_instance} — 历史实例表（已完成 / 终止实例）</li>
 *   <li>{@code remi_flow_his_task} — 历史任务表（审批操作轨迹）</li>
 *   <li>{@code remi_flow_audit_log} — 审计日志表（操作审计）</li>
 * </ul>
 *
 * <p><b>事务边界：</b>
 * <ul>
 ...
 *       订阅关系存储在 {@code remi_flow_report_subscription}</li>
