 ...
 * 催办限流器
 *
 * <p>P0-1c 架构优化：委托 {@link RedisRateLimiter} 公共限流能力，
 * 消除手写 Lua 脚本（SET NX EX）和 StringRedisTemplate 直接操作。
 *
 ...
 *   <li>限流失败时通过 {@link com.remisoft.common.exception.custom.SysException}
 *       + {@code RATE_LIMIT} 错误码抛回前端</li>
 *   <li>RedisRateLimiter 不可用时降级放行（ObjectProvider 可选注入）</li>
 * </ul>
 *
 * @since 1.0.0
 * @author remi-team
...
public class FlowUrgeLimiter {
    ...
    public boolean tryAcquire(String userId, Long targetId, String targetType, long cooldownSeconds) {
        ...
        try {
            boolean acquired = rateLimiter.tryAcquireFixedWindow(
