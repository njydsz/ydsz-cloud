...
import com.remisoft.workflow.infra.mapper.FlowThirdPartyAccountMapper;
 ...
 * 第三方审批账号服务实现。
 *
 * <p>管理钉钉/飞书/企业微信审批账号的绑定关系 ({@code remi_flow_thirdparty_account})：
 *
...
public class FlowThirdPartyAccountServiceImpl implements FlowThirdPartyAccountService {
    ...
    /** 三方账号 Mapper，管理 remi_flow_third_party_account 表 */
     ...
     *
     * <p><b>缓存：</b>{@link CacheConstants#FLOW_THIRDPARTY_BY_USER_CACHE}，
     * 键为 {@code userId:platform}。{@code unless = "#result == null"} 使空结果不入缓存——
     * 未绑定用户属常态，缓存空值会占用大量条目；代价是未绑定用户每次都会穿透到 DB，
     * 因该场景 QPS 极低，取舍上可接受。
     *
     * <p><b>事务：</b>只读事务，仅用于走从库/只读优化，无写副作用。
     *
     * <p><b>异常处理：</b>DB 异常被<b>捕获并转为返回 {@code null}</b>，不向上抛出。
     * 设计意图是三方账号缺失只应导致「无法推送到 IM」，而不能让审批主流程失败。
     ...
     *
     * <p>与 {@link #getByUserIdAndPlatform} 方向相反，用于<b>IM 侧回调入口</b>：
     * 用户在钉钉/飞书卡片上点「同意」时，回调只带 openId，需借此还原成本系统用户身份
     * 才能校验其审批权限。因此本方法是权限链路的起点，返回 {@code null} 时调用方
     * <b>必须拒绝该次操作</b>，不得回退为匿名或默认用户。
     *
     * <p><b>缓存：</b>{@link CacheConstants#FLOW_THIRDPARTY_BY_OPENID_CACHE}，
     * 键为 {@code platform:openId}，同样不缓存空值。
     *
     ...
     *
     * <p><b>幂等判定：</b>入参无 {@code id} 时，先按 {@code userId + platform} 反查已有记录，
     * 命中则回填 {@code id} 转为更新。这保证同一用户在同一平台<b>只会有一条绑定</b>，
     * 重复调用不会产生脏数据。新建时默认 {@code status=ACTIVE}。
     *
     * <p><b>缓存：</b>成功后清空两个三方账号缓存的<b>全部</b>条目。之所以用
     * {@code allEntries = true} 而非精确删除，是因为一次写入会同时影响
     * userId 和 openId 两个维度的键，精确失效易遗漏；该表写入频率极低，全清代价可接受。
     *
     * <p><b>事务边界与重要缺陷提示：</b>方法虽标注
     * {@code @Transactional(rollbackFor = Exception.class)}，但内部 {@code try-catch}
     * 吞掉了所有异常，异常不会传播到事务拦截器，因此<b>回滚实际不会触发</b>，
     * 且调用方（返回 {@code void}）也<b>无从得知失败</b>。业务上属「绑定失败可容忍、
     * 不阻断主流程」的取舍，但需要确认绑定结果的场景必须回查
     * {@link #getByUserIdAndPlatform} 校验，不能假定本方法一定成功。
     *
     ...
     * 绑定本系统用户与 IM 平台账号。
     *
     * <p>相对 {@link #saveOrUpdate} 的差异：本方法面向<b>扫码/授权绑定</b>场景，
     * 只接收标识字段，由内部组装实体。已存在绑定时<b>覆盖</b> {@code openId} 与
     * {@code unionId}，用于支持用户更换 IM 账号后重新绑定；不存在则新建并置为
     * {@code ACTIVE}。因此对同一 {@code (userId, platform)} 重复调用是幂等的。
     *
     * <p><b>缓存：</b>成功后全量清空两个三方账号缓存，理由同 {@link #saveOrUpdate}。
     *
     * <p><b>事务边界与失败可见性：</b>同 {@link #saveOrUpdate} —— 内部吞异常导致
     * {@code rollbackFor} 不生效，且返回 {@code void} 使调用方无法感知失败。
     * 绑定是否真正生效需回查 {@link #getByOpenId} 确认。
     *
     * <p><b>安全提示：</b>本方法不校验 openId 的归属真实性，调用方<b>必须</b>先完成
     * IM 侧的授权码换取流程，确认 openId 确属当前登录用户后再调用，
