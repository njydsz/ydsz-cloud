...
import com.remisoft.workflow.infra.mapper.FlowAutoTriggerMapper;
...
import com.remisoft.workflow.server.service.FlowRoutingService;
 ...
 * 使用 {@code remi-literule} 的 {@link com.remisoft.literule.api.expr.ExpressionEvaluator}
 * 评估 {@code conditionExpression}，满足条件则自动启动 {@code targetFlowCode} 对应的目标流程。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li><b>触发规则配置</b>：维护 {@code remi_flow_auto_trigger} 表，
 ...
 *       并透传源流程的上下文变量</li>
 *   <li><b>审计与回溯</b>：所有触发动作写入 {@code remi_flow_audit_log}，
 ...
 * </ul>
 *
 * <p><b>规则降级：</b>如果 {@code remi-literule} 模块未引入，{@code conditionExpression} 降级为
 * 「<b>永远成立</b>」（即不评估条件），所有 enabled 规则都会触发。
 * 生产环境应保证 {@code remi-literule} 已正确引入。
 *
 * @author remi-team
 * @since 1.0.0
 *
 * @see FlowAutoTriggerService 接口定义
 * @see com.remisoft.workflow.domain.entity.FlowAutoTrigger 自动触发规则实体
 * @see WorkflowFacade 工作流门面
 * @see com.remisoft.literule.api.expr.ExpressionEvaluator Aviator 表达式评估器
...
public class FlowAutoTriggerServiceImpl implements FlowAutoTriggerService {
    ...
    /** 自动触发 Mapper，管理 remi_flow_auto_trigger 表 */
