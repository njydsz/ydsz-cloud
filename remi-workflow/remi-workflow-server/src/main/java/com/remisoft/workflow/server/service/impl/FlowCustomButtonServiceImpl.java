 ...
 *   <li>「加签」按钮 — 触发 {@link com.remisoft.workflow.server.service.FlowTaskSignService}</li>
 *   <li>「打印审批单」按钮 — 调用 {@link FlowExportServiceImpl#exportApprovalHtml}</li>
 *   <li>「抄送」按钮 — 触发 {@link com.remisoft.workflow.server.service.FlowCcService}</li>
 ...
 * <p><b>设计要点：</b>
 * <ul>
 *   <li><b>权限分离</b>：按钮可关联 {@code permissionCode}（如 {@code remi:flow:btn:approve_large}），
 ...
 *   <li><b>审计追溯</b>：所有按钮点击记录到 {@code remi_flow_audit_log}，
 ...
 *   <li>HTTP 调用白名单：仅允许调用 {@code remi.callback.allowed-domains} 配置的域名</li>
 ...
 * @see com.remisoft.workflow.domain.entity.FlowNode 流程节点（自定义按钮挂在节点上）
