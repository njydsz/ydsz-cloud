 ...
 * 通知通道配置已移至独立的消息通知引擎 remi-message。
 *
 * @author remi-team
 * @since 1.0.0
 */
...
public class FlowEventController {
     ...
     * @return 触发的订阅数量
     */
    @Idempotent(key = "remi:workflow:FlowEventController:correlateMessage:lock", ttlSeconds = 5)
     ...
     * @return 触发的订阅数量
     */
    @Idempotent(key = "remi:workflow:FlowEventController:throwError:lock", ttlSeconds = 5)
