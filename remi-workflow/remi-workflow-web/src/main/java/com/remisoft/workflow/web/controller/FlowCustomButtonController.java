package com.remisoft.workflow.web.controller.definition;
 ...
 *
 * <p>提供流程节点的<b>自定义操作按钮</b>管理能力，扩展默认的"通过/驳回/转办"等系统按钮。
 * 业务方可为任意节点注册自定义按钮（如"加签财务"、"转交 HR"、"返回修改"），
 ...
 * <p><b>核心能力：</b>
 * <ul>
 *   <li><b>按钮查询</b>：{@code GET /} — 按 (definitionId, nodeCode) 查询按钮列表</li>
 *   <li><b>按钮保存</b>：{@code POST /} — 覆盖式保存节点按钮配置</li>
 *   <li><b>按钮执行</b>：{@code POST /execute} — 运行时执行按钮，触发后端逻辑</li>
 * </ul>
 *
 * <p><b>按钮类型：</b>
 * <ul>
 *   <li><b>JAVA_METHOD</b>：调用 Spring Bean 的指定方法（{@code beanName.methodName(args)}）</li>
 *   <li><b>HTTP_WEBHOOK</b>：调用外部 HTTP 接口（POST/PUT）</li>
 *   <li><b>SCRIPT</b>：执行 QLExpress 脚本</li>
 * </ul>
 *
 * <p><b>安全特性：</b>
 * <ul>
 ...
 *   <li>Java 方法调用使用 {@code @PermissionStrict} 严格模式，禁止反射绕过</li>
 *   <li>Webhook 调用走 OkHttp 客户端，禁止内网穿透（仅允许配置白名单 host）</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.workflow.server.service.FlowCustomButtonService 自定义按钮 Service
 */
@Slf4j
@RestController
...
public class FlowCustomButtonController {
     ...
     * 获取节点的自定义按钮列表
     *
     * <p>按 (definitionId, nodeCode) 查询该节点的全部自定义按钮配置，
     * 含按钮编码、名称、类型、关联逻辑、显示顺序、是否必填等。
     ...
     * <p><b>覆盖式</b>保存：先清空旧配置，再批量插入新配置（避免 N+1 循环）。
     * 业务方传入<b>完整</b>的按钮列表，而非增量。
     *
     * @param definitionId 流程定义 ID
     * @param nodeCode     节点编码
     ...
     * @return 空响应
     */
    @Idempotent(key = "remi:workflow:FlowCustomButtonController:save:lock", ttlSeconds = 5)
     ...
     *
     * <p>幂等保护 5 秒。
     * <p>运行时执行按钮，触发后端 Java 方法 / HTTP Webhook / 脚本逻辑。
     * 执行结果可回填到流程变量（{@code variables} 参数），影响后续分支走向。
     ...
     */
    @Idempotent(key = "remi:workflow:FlowCustomButtonController:execute:lock", ttlSeconds = 5)
    @PostMapping("/execute")
