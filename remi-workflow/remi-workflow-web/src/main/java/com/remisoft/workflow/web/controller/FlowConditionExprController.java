 ...
 * </ul>
 *
 * <p><b>支持的表达式引擎：</b>AVIATOR（默认，高性能轻量级求值器，{@code remi.workflow.engine.condition.default=AVIATOR}）、
 * GROOVY（动态脚本，重量级但表达力强，需开启 {@code remi.workflow.engine.condition.allow-groovy=true}）。
 *
 * <p><b>安全特性：</b>
 * <ul>
 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.workflow.server.service.FlowConditionExprService 条件表达式服务
 * @see com.remisoft.workflow.server.engine.JsonHelper JSON 转换助手
 */
@Slf4j
@RestController
...
public class FlowConditionExprController {
     ...
     * @return 转换后的表达式字符串
     */
    @Idempotent(key = "remi:workflow:FlowConditionExprController:build:lock", ttlSeconds = 5)
     ...
     */
    @Idempotent(key = "remi:workflow:FlowConditionExprController:parse:lock", ttlSeconds = 5)
     ...
     */
    @Idempotent(key = "remi:workflow:FlowConditionExprController:validate:lock", ttlSeconds = 5)
