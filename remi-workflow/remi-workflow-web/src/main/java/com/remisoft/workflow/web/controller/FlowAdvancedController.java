...
import com.remisoft.workflow.server.service.FlowInstanceMergeService;
import com.remisoft.workflow.server.service.FlowReportService;
import com.remisoft.workflow.server.service.impl.instance.FlowAssigneeDedupService;
import com.remisoft.workflow.server.service.impl.instance.FlowCountersignDynamicService;
 ...
 * <ul>
 *   <li><b>P2-4 审批数据周报/月报</b>：自动生成并推送审批效率报告</li>
 *   <li><b>P2-5 多实例合并审批</b>：把多个相同流程的实例合并为一组统一审批</li>
 *   <li><b>P2-6 会签动态完成条件</b>：运行时调整会签的通过率 / 人数阈值</li>
 *   <li><b>P2-7 跨节点办理人去重</b>：检查 / 查询实例已审批人</li>
 *   <li><b>P2-8 催办限流可视化</b>：查询催办冷却剩余时间</li>
 * </ul>
 *
 * <p><b>接口路径：</b>{@code /api/v1/workflow/advanced/**}
 *
 * <p><b>权限模型：</b>所有写接口通过 {@link AuthApiPermission} 校验
 * {@link PermissionCodes#WORKFLOW_INSTANCE_CONTROL} 或
 * {@link PermissionCodes#WORKFLOW_TASK_OPERATE} 权限码。
 *
 * <p><b>安全特性：</b>
 * <ul>
 *   <li>写接口启用 {@link Idempotent} 5s 防重（避免双击重复触发）</li>
 *   <li>写接口启用 {@link RateLimit} 50 QPS 限流</li>
 *   <li>读接口（dedup / urge / mergeable）无锁，可高 QPS 调用</li>
 * </ul>
 *
 * <p><b>设计原则：</b>Controller 仅做参数透传、权限校验、VO 转换；
 * 报表生成、实例合并、会签阈值计算、催办限流逻辑下沉到对应 Service。
 *
 * @author remi-team
 * @since 1.0.0
 *
...
public class FlowAdvancedController {
     ...
     * <p>触发周报推送：按租户下管理员配置的接收人列表，通过 {@code remi-message} 通知中心
     * 发送 IM 消息（钉钉 / 飞书 / 企微）+ 邮件。
     * <p>要求 {@link PermissionCodes#WORKFLOW_INSTANCE_CONTROL} 权限。
     *
     * @return 是否推送成功
     */
    @Idempotent(key = "remi:workflow:FlowAdvancedController:sendWeekly:lock", ttlSeconds = 5)
     ...
     * @return 是否推送成功
     */
    @Idempotent(key = "remi:workflow:FlowAdvancedController:sendMonthly:lock", ttlSeconds = 5)
     ...
     * @return 合并组 ID（VO 形式）
     */
    @Idempotent(key = "remi:workflow:FlowAdvancedController:merge:lock", ttlSeconds = 5)
     ...
     * @return 成功通过实例数
     */
    @Idempotent(key = "remi:workflow:FlowAdvancedController:mergePass:lock", ttlSeconds = 5)
     ...
     * @return 成功驳回实例数
     */
    @Idempotent(key = "remi:workflow:FlowAdvancedController:mergeReject:lock", ttlSeconds = 5)
     ...
     * P2-5: 查询当前用户可合并的实例列表
     *
     * <p>筛选条件：当前用户作为审批人 / 发起人、流程编码相同、状态为 RUNNING、
     * 尚未被合并、且不在其它合并组内的实例。
     ...
     * @return 空响应
     */
    @Idempotent(key = "remi:workflow:FlowAdvancedController:updateCondition:lock", ttlSeconds = 5)
     ...
     * @return 空响应
     */
    @Idempotent(key = "remi:workflow:FlowAdvancedController:updateApproveCount:lock", ttlSeconds = 5)
     ...
     * P2-8: 查询催办剩余冷却时间
     *
     * <p>对标钉钉/飞书审批「催一下」按钮的禁用逻辑：催办后默认冷却期 30 分钟，
     * 此端点返回当前用户对该实例的剩余冷却秒数 / 是否可催办。
     * <p>前端依据返回结果置灰或启用催办按钮。
     *
     * @param instanceId 流程实例 ID
