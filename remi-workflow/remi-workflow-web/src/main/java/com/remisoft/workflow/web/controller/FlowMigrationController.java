 ...
 *   <li>事务内更新 {@code remi_flow_instance} 与 {@code remi_flow_run_task} 的 definitionId 与 nodeCode</li>
 *   <li>触发 {@code remi_flow_audit_log} 写入（标记为「迁移」）</li>
...
public class FlowMigrationController {
    ...
    @Idempotent(key = "remi:workflow:FlowMigrationController:migrateInstances:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:workflow:FlowMigrationController:previewMigration:lock", ttlSeconds = 5)
