...
import com.remisoft.workflow.server.service.FlowTemplateRecommendService;
 ...
 * @see com.remisoft.workflow.server.service.FlowTemplateRecommendService 智能推荐服务
 */
@Slf4j
@Tag(name = "流程模板市场")
@RestController
...
public class FlowTemplateController {
    ...
    @Idempotent(key = "remi:workflow:FlowTemplateController:importTemplate:lock", ttlSeconds = 5)
     ...
     *
     * <p>读取流程定义的节点/跳转数据，生成对应的 BPMN XML 并存入 remi_flow_template 表。
    ...
    @Idempotent(key = "remi:workflow:FlowTemplateController:createNewVersion:lock", ttlSeconds = 5)
    @Audit(module = "流程模板", type = AuditType.OPERATION, action = AuditAction.CREATE,
    ...
    @Idempotent(key = "remi:workflow:FlowTemplateController:cloneTemplate:lock", ttlSeconds = 5)
    @Audit(module = "流程模板", type = AuditType.OPERATION, action = AuditAction.CREATE,
    ...
    @Idempotent(key = "remi:workflow:FlowTemplateController:inheritFromParent:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:workflow:FlowTemplateController:syncFromParent:lock", ttlSeconds = 5)
