package com.remisoft.workflow.web.controller.integration;
...
import com.remisoft.workflow.domain.dto.FlowAutoTriggerCreateDTO;
 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.workflow.server.service.FlowAutoTriggerService 触发规则 Service
 * @see com.remisoft.workflow.server.engine.FlowEventListener 事件监听器（消费者）
 */
@Slf4j
@Tag(name = "流程自动触发规则")
@RestController
...
public class FlowAutoTriggerController {
    ...
    @Idempotent(key = "remi:workflow:FlowAutoTriggerController:create:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:workflow:FlowAutoTriggerController:delete:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:workflow:FlowAutoTriggerController:toggle:lock", ttlSeconds = 5)
