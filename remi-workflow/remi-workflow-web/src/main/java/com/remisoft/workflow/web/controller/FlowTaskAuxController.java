...
import com.remisoft.workflow.server.service.FlowTodoCountPushService;
...
public class FlowTaskAuxController {
     ...
     * @return 统一响应结果
     */
    @Idempotent(key = "remi:workflow:FlowTaskController:countersignRemove:lock", ttlSeconds = 5)
     ...
     * @return 统一响应结果
     */
    @Idempotent(key = "remi:workflow:FlowTaskController:markRead:lock", ttlSeconds = 5)
     ...
     * @return 统一响应结果
     */
    @Idempotent(key = "remi:workflow:FlowTaskController:communicate:lock", ttlSeconds = 5)
     ...
     * @return 统一响应结果
     */
    @Idempotent(key = "remi:workflow:FlowTaskController:saveDraft:lock", ttlSeconds = 5)
     ...
     * @return 统一响应结果
     */
    @Idempotent(key = "remi:workflow:FlowTaskController:addApprover:lock", ttlSeconds = 5)
     ...
     *
     * @param hisTaskId 历史任务 ID（remi_flow_his_task.id）
     * @param comment   取回说明（可选）
     * @return 统一响应结果，包含新创建的待办任务 ID
     */
    @Idempotent(key = "remi:workflow:FlowTaskController:retract:lock", ttlSeconds = 5)
     ...
     * @return 统一响应结果
     */
    @Idempotent(key = "remi:workflow:FlowTaskController:suspendTask:lock", ttlSeconds = 5)
     ...
     * @return 统一响应结果
     */
    @Idempotent(key = "remi:workflow:FlowTaskController:activateTask:lock", ttlSeconds = 5)
     ...
     * @return 是否成功
     */
    @Idempotent(key = "remi:workflow:FlowTaskController:pushMyTodoCount:lock", ttlSeconds = 5)
