package com.remisoft.workflow.web.controller.dmn;
...
import com.remisoft.workflow.server.service.FlowDmnDecisionService;
 ...
 * </ul>
 *
 * <p><b>性能优化：</b>已发布决策表走 Redis 缓存（{@code remi:dmn:decision:published:{tenantId}:{code}}），
...
public class FlowDmnDecisionController {
    ...
    @Idempotent(key = "remi:workflow:FlowDmnDecisionController:createDecision:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:workflow:FlowDmnDecisionController:updateDecision:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:workflow:FlowDmnDecisionController:publish:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:workflow:FlowDmnDecisionController:deprecate:lock", ttlSeconds = 5)
