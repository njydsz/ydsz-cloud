 ...
 * 历史任务实体
 *
 * <p>对应数据库表 {@code remi_flow_his_task}，对标 Warm-Flow {@code flow_his_task}，
 ...
 * 避免运行态主表膨胀。本表按月分区（{@code remi_flow_his_task_2026m07}），超过保留期后由清理调度器删除。
 *
...
@TableName("remi_flow_his_task")
public class FlowHisTask extends MpBaseEntity<String> {

    @Serial
    private static final long serialVersionUID = 1L;

    /** 流程实例 ID（归档时从 {@code remi_flow_run_task.instance_id} 复制） */
    private String instanceId;

    /** 原始任务 ID（指向源 {@code remi_flow_run_task.id}，归档后源表清理前可关联） */
