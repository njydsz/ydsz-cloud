 ...
 * P0-1: DMN 决策表实体
 *
 * <p>对应数据库表 {@code remi_flow_dmn_decision}，对标 BPMN 2.0 DMN（Decision Model and Notation）规范中的决策表概念。
 * 每条决策表包含若干输入列（{@code inputExpressions}）和输出列（{@code outputLabels}），
 * 以及一组规则行（{@link FlowDmnRule}），按 {@code hitPolicy} 匹配规则并输出结果。
 *
 * <p><b>核心使用场景：</b>
 * <ul>
 ...
 * <ul>
 *   <li>{@code UNIQUE}：仅一条规则命中（类似排他网关）</li>
 *   <li>{@code FIRST}：按顺序取第一条命中（默认）</li>
 *   <li>{@code ANY}：多条命中时输出必须相同</li>
 *   <li>{@code COLLECT}：收集所有命中规则的输出（类似包容网关）</li>
 * </ul>
 *
 * <p><b>状态机（{@code status}）：</b>{@code DRAFT} → {@code PUBLISHED} → {@code DEPRECATED}，
 * 同一 {@code decisionCode} 同一时间仅允许一个 {@code PUBLISHED} 版本生效。
 *
 ...
 * <p><b>索引设计：</b>
 * <ul>
 *   <li>唯一索引 {@code uk_decision_code_version}（{@code decision_code}, {@code decision_version}）</li>
 *   <li>普通索引 {@code idx_flow_node}（{@code flow_code}, {@code node_code}）：按流程节点查询</li>
 *   <li>普通索引 {@code idx_status}（{@code status}）：按状态筛选</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
 ...
 * @see com.remisoft.workflow.server.engine.DmnDecisionEngine DMN 决策引擎
 */
@Data
@SuperBuilder
@NoArgsConstructor
@EqualsAndHashCode(callSuper = true)
@TableName("remi_flow_dmn_decision")
