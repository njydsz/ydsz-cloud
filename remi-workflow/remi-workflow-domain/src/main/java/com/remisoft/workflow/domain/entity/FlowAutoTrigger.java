 ...
 * 流程自动触发规则实体
 *
 * <p>对应数据库表 {@code remi_flow_auto_trigger}，实现「流程触发流程」的自动化能力。
 * 当源流程实例完成（终态）时，自动检查本表规则，按权重顺序执行条件匹配，
 * 满足条件的规则自动启动目标流程。
 *
 * <p><b>核心使用场景：</b>
 * <ul>
 *   <li>「立项审批」通过 → 自动触发「合同审批」</li>
 *   <li>「合同审批」通过 → 自动触发「财务付款审批」</li>
 *   <li>「项目验收」通过 → 自动触发「项目结项审批」</li>
 * </ul>
 *
 ...
 *
 * <p><b>变量传递：</b>目标流程默认继承源实例的 {@code variable} JSON，可通过 {@code variableMapping} 配置字段映射（暂存于 {@code ext}，未来扩展）。
 *
 * <p><b>索引设计：</b>
 * <ul>
 *   <li>普通索引 {@code idx_source}（{@code source_flow_code}）：源流程规则查询</li>
 *   <li>普通索引 {@code idx_target}（{@code target_flow_code}）：目标流程反查</li>
 ...
 * @see com.remisoft.workflow.server.listener.FlowAutoTriggerListener 自动触发监听器
 */
@Data
@SuperBuilder
@NoArgsConstructor
@EqualsAndHashCode(callSuper = true)
@TableName("remi_flow_auto_trigger")
