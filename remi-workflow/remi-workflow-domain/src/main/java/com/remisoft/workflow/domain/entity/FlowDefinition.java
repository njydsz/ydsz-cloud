...
import com.remisoft.workflow.domain.vo.FlowViews;

/**
 * 流程定义实体
 *
 * <p>对应数据库表 {@code remi_flow_definition}，对标 Warm-Flow {@code flow_definition}，存储流程模板元数据。
 * 流程定义是工作流引擎的「模板层」，描述一类流程的结构（节点、流转、审批人），
 * 每次发起流程都基于某版本的流程定义生成流程实例。
 *
 * <p><b>核心字段：</b>
 * <ul>
 *   <li>{@code flowCode}：流程编码（业务语义，如 {@code project_initiation} / {@code contract_change}）</li>
 *   <li>{@code flowVersion}：流程版本号（{@code v1} / {@code v2}），支持版本灰度发布</li>
 *   <li>{@code modelValue}：设计器模型（{@code CLASSICS} 经典 / {@code MIMIC} 仿钉钉）</li>
 *   <li>{@code formCustom}：审批表单是否自定义（{@code Y/N}）</li>
 *   <li>{@code activityStatus}：激活状态（{@code 0} 挂起 / {@code 1} 激活）</li>
 *   <li>{@code isPublish}：发布状态（{@code 0} 未发布 / {@code 1} 已发布 / {@code 9} 失效）</li>
 *   <li>{@code listenerType} / {@code listenerPath}：流程监听器配置（Spring Bean 路径）</li>
 *   <li>{@code ext}：扩展字段 JSON（业务侧自定义元数据）</li>
 * </ul>
 *
 * <p><b>P3-1 灰度发布：</b>
 * <ul>
 *   <li>{@code canaryPercent}：灰度比例（{@code 0-100}，{@code 0}=全量稳定版，{@code 100}=全量灰度版）</li>
 *   <li>{@code canaryStatus}：灰度状态（{@code NONE} / {@code CANARYING} / {@code PROMOTED} / {@code ROLLED_BACK}）</li>
 *   <li>{@code canaryStrategy}：灰度切流策略（{@code USER_HASH} / {@code RANDOM} / {@code WHITELIST}）</li>
 *   <li>{@code canaryRolloutLog}：灰度发布历史（JSON 数组）</li>
 * </ul>
 *
 * <p><b>P2-4 协同编辑锁定：</b>
 * <ul>
 *   <li>{@code lockedBy}：当前持锁人 ID（{@code NULL}=未锁定）</li>
 *   <li>{@code lockedAt}：加锁时间（超过 {@code workflow.designer.lock-timeout-minutes} 默认 30 分钟可强制抢占）</li>
 * </ul>
 *
 * <p><b>索引设计：</b>唯一索引 {@code uk_flow_code_version}（{@code flow_code, flow_version}），
 * 普通索引 {@code idx_category}（{@code category}）。
 *
 * @author remi-team
 * @since 1.0.0
 *
 ...
 * @see FlowNode 流程节点
 * @see com.remisoft.workflow.server.service.FlowDefinitionService 流程定义 Service
 */
@Data
@SuperBuilder
@NoArgsConstructor
@EqualsAndHashCode(callSuper = true)
@TableName("remi_flow_definition")
public class FlowDefinition extends MpBaseEntity<String> {
     ...
     */
    @JsonView(FlowViews.Summary.class)
     ...
     * 流程版本号。
     *
     * <p>建议使用 {@code v1} / {@code v2} 格式或语义版本号（{@code 1.0.0} / {@code 1.1.0}）。
     ...
     */
    @JsonView(FlowViews.Detail.class)
     ...
     * {@code N}=使用系统内置表单（基于 {@code remi_form_definition}）。
     */
    @JsonView(FlowViews.Detail.class)
     ...
     * 与 {@code remi_provider_trace_id} 全链路追踪协议对齐。
     */
    @JsonView(FlowViews.Detail.class)
