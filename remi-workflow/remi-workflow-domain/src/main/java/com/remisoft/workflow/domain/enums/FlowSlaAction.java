 ...
 * <ul>
 *   <li>{@link #REMIND}     — 中间态：发送催办通知（站内信/邮件/企微），任务保持活跃</li>
 *   <li>{@link #NOTIFY}     — 最终态：通知管理员/升级人介入，任务保持活跃（等人工处理）</li>
 *   <li>{@link #ESCALATE}   — 最终态：升级到上级（自动转办给 escalateUserId），任务保持活跃</li>
 *   <li>{@link #AUTO_PASS}  — 最终态：自动通过（超时自动审批通过），流程推进到下一节点</li>
 *   <li>{@link #AUTO_REJECT}— 最终态：自动驳回（超时自动驳回），流程终止</li>
 * </ul>
 *
 * <p>REMIND 作为中间态，达到 {@code maxReminders} 后会切换到 {@code finalAction}
 * （默认 NOTIFY）。这样保证 SLA 链路始终闭环：要么任务被人处理，要么被系统自动推进，
