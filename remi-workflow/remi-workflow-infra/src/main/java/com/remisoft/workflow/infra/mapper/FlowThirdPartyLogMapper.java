 ...
 * 三方审批回调日志 Mapper
 *
 * <p>对应数据表 <code>remi_flow_third_party_log</code>（P0-2），存储三方审批回调日志与状态更新。</p>
 ...
 * @since 1.0.0
 *
 * @see com.remisoft.workflow.domain.entity.FlowThirdPartyLog 三方回调日志实体
 * @see com.remisoft.workflow.server.service.FlowThirdPartySyncService 三方同步 Service
...
public interface FlowThirdPartyLogMapper extends BaseMapper<FlowThirdPartyLog> {
     ...
     */
    @Select(
            "SELECT * FROM remi_flow_third_party_log WHERE business_id = #{businessId} " +
     ...
     */
    @Select(
            "SELECT * FROM remi_flow_third_party_log " +
