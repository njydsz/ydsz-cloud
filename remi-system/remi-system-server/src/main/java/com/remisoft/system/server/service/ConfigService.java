 ...
 * 系统配置 Service 接口
 *
 * <p>提供系统配置（{@code remi_config}）的 CRUD、按 key / group 查询、公开配置查询等能力。
 ...
 * <p><b>缓存策略：</b>
 * <ul>
 *   <li>Redis 二级缓存（{@code remi:config:{group}:{key}}），TTL 30min</li>
...
public interface ConfigService {
     ...
     * 自动校验 {@code valueType}（{@link com.remisoft.system.domain.enums.ConfigValueType}）。
     *
     * @param dto 配置 DTO
     * @return 新建配置主键 ID
     */
     ...
     * 按配置键查询配置值（走缓存）
     *
     * <p>典型用法：{@code configService.getConfigValue("remi.workflow.sla-default-hours")}。
     ...
     * 按配置分组批量查询启用的配置项
     *
     * <p>典型用法：{@code configService.getConfigsByGroup("remi.workflow")} 返回工作流相关所有配置。
