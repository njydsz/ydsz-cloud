package com.remisoft.system.server.search;
...
import com.remisoft.common.search.provider.SearchProviderContext;
import com.remisoft.system.domain.entity.Config;
import com.remisoft.system.infra.mapper.ConfigMapper;
 ...
 * 系统配置搜索提供者
 *
 * <p>将 {@link com.remisoft.system.domain.entity.Config} 注册到统一搜索体系，
 * 支持配置键、配置值、配置分组的全文搜索与聚合分析。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li>声明搜索字段 schema（{@code configKey} / {@code configValue} / {@code configGroup} / {@code description}）</li>
 *   <li>提供文档数据源：从 {@code ConfigMapper} 拉取全量配置</li>
 *   <li>权限过滤：{@link #getFilters} 限制非管理员仅搜索 {@code isPublic=1} 的配置</li>
 *   <li>聚合字段：{@code configGroup} 支持按分组聚合（如「工作流」「消息」「功能开关」）</li>
 * </ul>
 *
 * <p><b>字段映射：</b>
 * <ul>
 *   <li>{@code configKey} → {@code title}（{@code FieldType.KEYWORD}）</li>
 *   <li>{@code configGroup} → {@code subtitle}（{@code FieldType.KEYWORD}）</li>
 *   <li>{@code configValue + description} → {@code content}（{@code FieldType.TEXT}）</li>
 * </ul>
 *
 * <p><b>设计要点：</b>
 * <ul>
 *   <li>使用新 {@link SearchField} API（{@code FieldType + searchable + sortable + aggregatable}）</li>
 *   <li>{@code content} 直接设为 {@code configValue + description}，引擎策略自行组合 title + subtitle + content</li>
 *   <li>权限过滤由 {@link #getFilters(SearchProviderContext)} 实现，遵循 RBAC 模型</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
...
public class SystemSearchProvider implements SearchProvider<Config> {
     ...
     * <p>字段映射规则：
     * <ul>
     *   <li>{@code configKey} → {@code title}（用于列表展示 + 高亮）</li>
     *   <li>{@code configGroup} → {@code subtitle}（用于分组聚合）</li>
     *   <li>{@code configValue + description} → {@code content}（用于全文搜索）</li>
     *   <li>{@code description} → {@code snippet}（用于搜索结果摘要）</li>
     *   <li>{@code path} 固定为 {@code /system/config/{id}}，点击搜索结果跳转系统配置详情页</li>
     * </ul>
     *
     ...
     * <p>四个搜索字段：
     * <ul>
     *   <li><b>title</b>（配置键）— {@code KEYWORD} 类型，权重 3.0，可高亮、可排序</li>
     *   <li><b>subtitle</b>（配置分组）— {@code KEYWORD} 类型，权重 2.0，可高亮、<b>可聚合</b></li>
     *   <li><b>content</b>（配置值）— {@code TEXT} 类型，权重 1.0，可高亮</li>
     *   <li><b>status</b>（状态）— {@code KEYWORD} 类型，<b>不可搜索</b>，可聚合、可排序</li>
     * </ul>
     * <p>权重设计：title &gt; subtitle &gt; content &gt; status，<b>关键字命中标题比命中正文分数高</b>，
     * 搜索结果更相关。
     *
     * @return 可搜索字段列表
     */
     ...
     * <p>权限语义：
     * <ul>
     *   <li>管理员（{@link SearchProviderContext#isAdmin()} 返回 {@code true}）—
     *       无任何过滤，可搜索全部配置（含 {@code isPublic=0} 私有配置）</li>
     *   <li>非管理员 — 强制追加 {@code is_public = 1} 过滤条件，<b>只能搜到公开配置</b>，
     *       私有配置（含密钥、连接地址）不会泄露给普通用户</li>
     * </ul>
     * <p>租户隔离：依赖 {@link IndexDocument#tenantId} 字段，搜索服务层在 ES / DB 查询时
     * 自动追加 {@code tenant_id} 条件，本方法不重复处理。
     *
     ...
     * 加载指定租户下的全部配置 ID
     *
     * <p>用于全量重建索引场景：调用方遍历返回的 ID 列表逐条调用 {@link #loadById} 获取实体，
     * 再通过 {@link #toIndexDocument} 转换为索引文档写入 ES。
