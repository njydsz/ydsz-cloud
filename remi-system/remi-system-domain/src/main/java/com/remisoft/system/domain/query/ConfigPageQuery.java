 ...
 * 系统配置分页查询参数
 *
 * <p>对应 {@code remi_config} 表的分页查询条件，由 Controller 接收并透传给
 * {@code ConfigService.page()}。继承自 {@link PageQuery}，自带 {@code pageNum} /
 * {@code pageSize} / {@code orderBy} / {@code sort} 等通用分页参数。
 *
 * <p><b>字段语义：</b>
 * <ul>
 *   <li>{@code configGroup} — 配置分组精确匹配（{@code =}），如 {@code remi.workflow}</li>
 ...
 * @see com.remisoft.system.server.service.ConfigService 系统配置服务
 */
@Data
@SuperBuilder
@NoArgsConstructor
