...
本模块是 REMI 的**系统级基础服务**，承担**横切关注点**。

| 业务域 | 说明 |
|---|---|
| **系统配置** | 参数配置（`remi_config`），支持按 key 查询、按 group 批量查询、公开配置查询、Redis 缓存、缓存穿透防护、值类型校验 |
| **数据字典** | 字典类型（`remi_dict_type`）+ 字典项（`remi_dict_item`），支持树形字典、缓存、版本管理 |
| **应用注册** | OAuth2 应用注册（`remi_app_info`），支持 BCrypt 密钥校验（强度可配置） |
| **系统变量** | 业务级变量管理（`remi_variable`），支持 Redis 缓存、缓存穿透防护 |
| **字典版本** | 字典变更历史快照（`remi_dict_version`），写操作自动记录变更、支持查询审计 |
| **多租户** | 租户（`remi_tenant`）+ 套餐（`remi_tenant_plan`）+ 套餐菜单（`remi_tenant_plan_menu`），支持租户级菜单定制 |
...
remi-system/
├── pom.xml
├── remi-system-api/                    # API 层：Feign Client + Fallback
│   └── src/main/java/com/remisoft/system/api/client/
...
├── remi-system-domain/                 # 领域层：Entity + DTO + VO
│   └── src/main/java/com/remisoft/system/domain/
...
├── remi-system-infra/                  # 基础设施层：MyBatis Mapper
│   └── src/main/java/com/remisoft/system/infra/mapper/
...
│       └── VariableMapper.java
│   └── src/main/java/com/remisoft/system/infra/repository/
│       ├── ConfigRepository.java       # Config 仓储
│       └── DictRepository.java         # Dict 仓储
├── remi-system-server/                 # 应用层：Service + Config + Health + Metrics
│   └── src/main/java/com/remisoft/system/server/
...
└── remi-system-web/                    # Web 层：Controller + Bootstrap
    └── src/main/java/com/remisoft/system/web/
...
| 表名 | 说明 |
|---|---|
| `remi_config` | 系统参数（key-value，支持租户维度） |
| `remi_dict_type` | 字典类型 |
| `remi_dict_item` | 字典项（支持树形 parent_id、扩展 ext_json） |
| `remi_dict_version` | 字典版本（变更历史快照，含 tenant_id） |
| `remi_app_info` | 应用注册（OAuth2 client_id/client_secret） |
| `remi_variable` | 系统变量 |
| `remi_tenant` | 租户（多租户隔离的租户主表） |
| `remi_tenant_plan` | 租户套餐（套餐定义，含菜单配额/功能开关） |
| `remi_tenant_plan_menu` | 套餐菜单关联（套餐 → 菜单多对多） |
...
### 应用密钥安全
- `appSecret` 使用 BCrypt 加密存储，强度可配置（`remi.system.app.bcrypt-strength`）
  ...
  cloud:
    nacos:
      ...
      config:
        ...
        file-extension: yml
        shared-configs:
          - data-id: remi-common.yml
            refresh: true
          - data-id: remi-system.yml
            refresh: true
server:
  port: 9001
```
...
| 属性 | 默认值 | 说明 |
|---|---|---|
| `remi.system.health-enabled` | true | 是否启用健康检查 |
| `remi.system.config.cache-ttl-minutes` | 5 | 配置缓存 TTL（分钟） |
| `remi.system.config.hot-reload-enabled` | true | 配置热加载（预留） |
| `remi.system.dict.cache-ttl-minutes` | 10 | 字典缓存 TTL（分钟） |
| `remi.system.app.bcrypt-strength` | 10 | BCrypt 加密强度（4-31） |
| `remi.system.internal-api-ip-whitelist` | [] | 内部 API IP 白名单 |

## Feign 接口

被以下 Feign 客户端调用（位于 `remi-system-api`）：
...
检查 `remi_config` 表中是否存在对应的 `config_key` 且 `status = 'ENABLED'`、`deleted = 0`。
配置查询走 Redis 缓存，空值会缓存 1 分钟（防穿透），期间重复查询返回 null。

### Q2：应用密钥校验失败

1. 确认 `remi_app_info` 表中 `app_key` 存在且 `status = 'ENABLED'`
2. 确认 `app_secret` 字段存储的是 BCrypt 加密后的哈希值（非明文）
3. 创建应用时通过 API 传入明文密钥，Service 会自动 BCrypt 加密
4. BCrypt 强度可通过 `remi.system.app.bcrypt-strength` 配置
