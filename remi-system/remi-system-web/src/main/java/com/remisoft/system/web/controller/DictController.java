 ...
 * 下游业务模块通过版本号感知字典变更并刷新本地缓存（{@code remi.dict.cache-ttl}）。
 *
 * @author remi-team
 * @since 1.0.0
...
public class DictController {
    ...
    @Idempotent(key = "remi:system:DictController:save:lock", ttlSeconds = 5)
    @PostMapping
    ...
    @Idempotent(key = "remi:system:DictController:update:lock", ttlSeconds = 5)
    @PutMapping
    ...
    @Idempotent(key = "remi:system:DictController:remove:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
