 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.system.server.service.ConfigService 配置业务逻辑
 */
...
public class ConfigController {
     ...
     * <p>创建后会自动失效 Redis 缓存（{@code remi:system:ConfigController:save:lock}），
    ...
    @Idempotent(key = "remi:system:ConfigController:save:lock", ttlSeconds = 5)
    @PostMapping
    ...
    @Idempotent(key = "remi:system:ConfigController:update:lock", ttlSeconds = 5)
    @PutMapping
    ...
    @Idempotent(key = "remi:system:ConfigController:remove:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
     ...
     * <p>常用于业务方高频读取同一配置（如 {@code remi.workflow.sla-default-hours}），
     * 比 {@link #getById} 更高效（无反序列化 VO 开销）。
     *
     * @param configKey 配置键（如 {@code remi.workflow.sla-default-hours}）
     ...
     * 按配置分组批量查询
     *
     * <p>走 Redis 缓存（{@code remi:config:group:{configGroup}}，TTL 5min）。
