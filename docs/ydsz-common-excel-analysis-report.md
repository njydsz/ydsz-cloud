...
14. XLSX 规范覆盖缺口：fast 读引擎 rich text 多 run 解析、SST phonetic、数值型日期单元格识别、1904 窗口用例覆盖
15. fast 写路径 WriteHandler 回调实做（消除与 POI 路径的静默能力差异）
16. EasyExcel 兼容用例矩阵自建（纸面基线参照，round-trip 读写 × 格式 × 类型 × 边界，200+ 用例，fast 默认开启前置门禁）
