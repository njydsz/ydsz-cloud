<!--
  ===========================================================================
  文件名: database-spec.md
  路径:   docs/standards/database-spec.md
  作用:   PMIS 数据库（PostgreSQL 18）表/字段命名、类型、索引、SQL 编写、迁移、性能规范
  适用:   所有 Flyway 迁移脚本 / DDL / 复杂 SQL / DBA 评审
  对标:   阿里《Java 开发手册》数据库规约 / 华为云数据库设计规范
  ===========================================================================
-->
...
> 📌 所有 DDL 必须以 Flyway 迁移脚本形式提交，路径 `deploy/sql/V{version}__{description}.sql`。
...
| 场景 | 策略 | 说明 |
|------|------|------|
| 单库业务 | `BIGSERIAL` | 数据库自增，简单可靠 |
| 分布式/分库 | 雪花算法（`IdType.ASSIGN_ID`） | 64 位 Long，全局唯一 |
| 业务编号 | `pmis:{module}:{yyyyMMdd}:{seq}` | 业务可读，单独序列 |

> 💡 PMIS 默认采用**雪花算法**生成主键，便于未来分库分表。

## 12. 关联关系与外键

- 关联表优先用**逻辑外键**（仅保存外键 ID 列），不创建物理外键约束（性能 + 灵活性）
- 必须的外键约束在命名上以 `_id` 结尾，便于识别
- 多对多关系必须通过关联表（`pmis_<主>_<从>_rel`）实现

## 13. 软删除与归档

- 业务表统一使用 `deleted` 字段做软删除
- 所有查询必须带 `WHERE deleted = 0`
- 物理删除**仅限**字典、日志、临时表
- 归档：超过 1 年的数据按月归档到 `pmis_<table>_archive_yyyyMM`

## 14. 敏感数据加密

- 身份证、银行卡、手机号等使用 AES-256-GCM 加密存储
- 加密字段命名为 `*_encrypted`，避免业务方误用
- 应用层统一通过 [`CryptoUtil.aesGcmEncrypt`](file:///d:/Code/ydsz/ydsz-pmis/ydsz-pmis-backend/ydsz-pmis-common/src/main/java/com/njydsz/pmis/common/security/CryptoUtil.java) 加解密
- 密钥从 Nacos / 环境变量读取，**禁止**入库

## 15. 变更记录

| 日期 | 版本 | 变更人 | 变更内容 |
|------|------|--------|----------|
| 2026-07-03 | 1.1 | DBA + 架构组 | 新增 §11 主键策略、§12 关联关系、§13 软删除、§14 加密 |
