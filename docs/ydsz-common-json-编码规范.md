...
| 性能基准测试 | `ydsz-common-json` 的 `src/test` 中引入 Jackson / Fastjson2 | 用于 JMH 性能对比，test scope 不影响生产 |
| `module-info.java` | `requires static com.fasterxml.jackson.annotation;` | JPMS 模块描述，非业务代码 |
...
| `@JsonAlias` | `@JsonAlias`（Deprecated） | 推荐统一命名约定 |
