...
| 项 | 状态 | 说明 |
|---|---|---|
| 审批通过后「恢复被暂停执行」 | **已实施** | 见 §6.8：会话级暂停/恢复全链路落地（检查点 + 恢复执行 + 审批回调接线） |
| ToolResultEviction 中间件 | **已实施** | 见 §6.9：`ToolResultEvictionMiddleware` 默认关闭、按需启用 |
| 多副本部署 | **已启用** | 见 §6.10：`ExecutionPauseService` 接入 `AgentStateStore` + `application-prod.yml` 样本 + README 章节 |
| DTO 丢失 agentCode | **已修复** | `AgentController#toExecutionRequest` 原未映射 `agentCode`，导致 HTTP 入口按编码路由**从未生效**；已补映射 |
| 单元测试 | **已补** | 见 §6.11：4 个测试类 45 个用例（domain 27 / infra 6 / server 13），全绿 |

### 6.8 会话级暂停/恢复（HITL 闭环）✅

原 P0-3 的「非阻塞 + 登记审批 + 重新发起」语义在**不挂起执行线程**的前提下可用，但
无法表达「审批通过后由服务端**继续**原执行」的语义。本轮补齐暂停/恢复链路：

- **领域层**：
  - `ExecutionCheckpoint`（不可变快照：原请求 / 消息列表 / 待执行工具 / 已耗 Token / 迭代轮次 / 用户输入 / 系统提示词）；
  - `SessionPausedException`（携带 `approvalId` + 待审批工具调用列表）；
  - `ChatResponse` 增加 `metadata` 字段与 `withMetadata(k, v)`（不可变语义），承载暂停响应中的 `approvalId` / `status=PAUSED`；
  - `AgentExecutor` 新增 `resume(checkpoint, approved)` 默认方法（默认返回 null = 不支持恢复），
    避免为不支持的执行器引入强制实现。
- **应用层**：
  - `ReActAgentExecutor#resume(...)`：审批拒绝 → 返回拒绝响应并清理检查点；审批通过 → 执行待执行工具、
    回填 tool 消息、从 `currentIteration + 1` 继续 ReAct 循环；
  - `ExecutionPauseService`：按 `approvalId` 存取检查点，支持内存与 `AgentStateStore`（Redis）双后端；
  - `AgentFacade#resume(approvalId, approved)` + `AgentFacadeImpl` 实现（经 `resolveExecutor` 路由到原执行器）。
- **接口层**：
  - `ToolApprovalMiddleware` 增加 `pause-on-sensitive` 模式（默认 false）；暂停模式抛 `SessionPausedException`；
  - 恢复时上下文注入 `tool-approval.approved-call-ids`，中间件据此跳过已审批调用的重复拦截
    （解决「恢复后又被同一审批门拦截」的死锁）；
  - `HumanApprovalController#approve/reject` 在审批落库后调用 `agentFacade.resume(id, true/false)`；
    非暂停模式下检查点不存在 → `resume` 返回 null，属正常降级（既有非阻塞语义不受影响）。
- **默认关闭**：`ydsz.agent.approval.pause-on-sensitive=false`（默认）时链路与改造前完全一致。

### 6.9 ToolResultEviction 中间件 ✅

- `ToolResultEvictionMiddleware`（`implements AgentMiddleware`，优先级 `TOOL_EVICTION_PRIORITY = 55`）；
- 三种驱逐维度：单结果字符上限、单结果 Token 上限（char/2.5 估算）、批次总字符预算
  （按比例压缩 + 单结果硬上限 = `maxTotalChars / resultCount`，保证总量不越界）；
- 截断保留尾部语义：`truncate()` 保证结果长度 **≤ maxLength**（先扣减标记长度再截断，
  `keep <= 0` 时仅返回标记），解决「截断后反而超出上限」的原实现缺陷；
- `AgentProperties.Tool` 新增 `eviction-enabled` / `eviction-max-result-chars` /
  `eviction-max-total-chars` / `eviction-max-result-tokens`；
- **默认关闭**：`evictionEnabled=false` 时直接透传，零行为变更；与 P1-1 的「先落 Trace 再驱逐」配合，
  证据链不受影响。

### 6.10 多副本部署启用 ✅

- `ExecutionPauseService` 构造改为 `ObjectProvider<AgentStateStore> + ttl`：
  装配了 `RedisAgentStateStore` 时检查点写入 Redis（键
  `ydsz:agent:checkpoint:default:default:{approvalId}`，TTL 可配），未装配时退化为内存；
  读取优先内存缓存、未命中回查存储并回填，保证多副本下审批回调可路由到任意实例恢复；
- 新增 `ydsz-agent-web/src/main/resources/application-prod.yml` 生产样本
  （`pause-ttl-hours` / `pause-on-sensitive` / Nacos 命名空间 / Redis / 日志 MDC traceId）；
- README 新增「多副本部署」章节：状态组件外置要求表 + 启用方式 + 多副本 HITL 恢复流程
  + SSE 粘性会话提示。

> **仍需部署侧确认**：SSE 长连接在多副本下的粘性路由（或客户端断线重连 + `/approvals/pending` 补偿）；
> 代码侧已提供补偿路径，无需额外改动。
...
| 模块 | 测试类 | 用例数 | 覆盖点 |
|---|---|---|---|
| domain | `AgentExecutionRequestTest` | 8 | `deriveForSubAgent` 工具交集 / 上下文清空 / 不可变语义 / builder 默认值 |
| domain | `AgentStateKeyTest` | 11 | 四段键生成 / 空段占位 / 段清洗截断 / 扫描模式 / `parse()` 往返 |
| domain | `ModelSseEventTest` | 8 | 工厂方法 / `withSource` 不可变 / payload 来源字段取舍 |
| infra | `SlidingWindowCompressorTest` | 6 | 预算裁剪 / System 保留 / 至少保留 1 条非 System 消息 |
| server | `ToolResultEvictionMiddlewareTest` | 6 | 关闭透传 / 单结果字符与 Token 截断 / 总量比例压缩 / 优先级 |
| server | `ReActAgentExecutorPauseResumeTest` | 4 | 暂停响应与检查点保存 / 审批通过恢复 / 审批拒绝中止 / Token 跨迭代累计 |
| server | `AbstractAgentExecutorLoadHistoryTest` | 3 | 预置上下文优先 / memory.load 兜底 / null 上下文兜底 |

**修复的真实缺陷（本轮测试暴露）**：`ReActAgentExecutor#runIteration` 原以
`totalUsage.add(response.getUsage())` 累加 Token，但 `TokenUsage` 为**不可变值对象**、
`add()` 返回新实例，返回值被丢弃 → **多轮迭代的 Token 用量从未累计，最终响应只反映最后一轮**。
修复方式：`runIteration` 改为返回 `IterationOutcome(response, totalUsage)` 记录，
调用方按返回值重新赋值 `totalUsage`；同步路径与恢复路径同步修正。
