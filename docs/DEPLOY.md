...
```bash
./deploy/scripts/import-nacos-config.sh pmis dev
```
...
- Group：`dev`
- 配置格式：YAML
- 内容：复制 `deploy/nacos/ydsz-pmis-common.yaml`
...
```bash
# 1. 停止所有服务
./deploy/scripts/stop-all.sh --with-infra
...
```bash
# 启动新版本（不同端口）
SERVER_PORT_GATEWAY=19000 ./deploy/scripts/start-all.sh --backend
