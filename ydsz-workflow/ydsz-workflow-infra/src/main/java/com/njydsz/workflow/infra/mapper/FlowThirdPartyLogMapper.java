 ...
 * 三方审批回调日志 Mapper
 *
 * <p>对应数据表 <code>ydsz_flow_third_party_log</code>（P0-2），存储三方审批回调日志与状态更新。
 *
 ...
 *
 * <ul>
 *   <li>idx_platform_event — (platform+eventId) 索引（幂等去重）
...
public interface FlowThirdPartyLogMapper extends BaseMapper<FlowThirdPartyLog> {
   ...
   */
  int updateStatus(
      @Param("id") String id, @Param("status") String status, @Param("errorMsg") String errorMsg);
  ...
  @Select(
      "SELECT * FROM ydsz_flow_third_party_log "
          ...
          + "LIMIT #{batchSize}")
  List<FlowThirdPartyLog> selectFailedForRetry(
      @Param("maxRetries") int maxRetries, @Param("batchSize") int batchSize);
