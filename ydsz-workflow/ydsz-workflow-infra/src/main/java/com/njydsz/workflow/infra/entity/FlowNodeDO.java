 ...
 *
 * <ul>
 *   <li><b>开始节点（{@code nodeType=0}）</b>：流程入口，每个流程定义唯一
 *   <li><b>审批节点（{@code nodeType=1}）</b>：人工审批环节，需配置 {@code permissionFlag} 办理人规则
 *   <li><b>网关节点（{@code nodeType=2}）</b>：并行/排他/包容网关，控制流转分支
 *   <li><b>结束节点（{@code nodeType=3}）</b>：流程出口，可配置多个
 *   <li><b>子流程节点（{@code nodeType=4}）</b>：嵌套调用其它流程定义
 *   <li><b>抄送节点（{@code nodeType=5}）</b>：仅通知，不阻塞流程
 * </ul>
 *
 * <p><b>办理人解析：</b>{@code permissionFlag} 支持多类前缀：
 *
 * <ul>
 *   <li>{@code role:hr} — 指定角色（如 {@code hr} / {@code pm}）
 *   <li>{@code dept:10} — 指定部门（ID/编码）
 *   <li>{@code user:1001} — 指定用户（ID/账号）
 *   <li>{@code post:dev} — 指定岗位（编码）
 *   <li>{@code initiator} — 发起人本人
 *   <li>{@code initiatorLeader} — 发起人直属上级
 *   <li>{@code ${spel}} — SpEL 表达式动态解析
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>{@code priority} — 节点优先级（默认 50，用于多任务排序）
 ...
 * </ul>
 *
 * <p><b>表单字段权限（{@code formFieldsConfig}）：</b>按节点控制审批表单字段的可编辑/只读/隐藏， 格式为 {@code
 * {"fieldKey":"EDIT|READONLY|HIDDEN",...}}，由前端渲染层解析应用。
 *
 ...
 *
 * <ul>
 *   <li>唯一索引 {@code uk_definition_node_code}（{@code definition_id}, {@code node_code}）：流程内节点编码唯一
 *   <li>普通索引 {@code idx_flow_code}（{@code flow_code}）：加速按流程编码批量查询
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
 * @see FlowDefinitionDO 流程定义
...
public class FlowNodeDO extends MpBaseEntity<String> {
   ...
   * 办理人权限标识。
   *
   * <p>支持的前缀：{@code role:xxx} / {@code dept:xxx} / {@code user:xxx} / {@code post:xxx} / {@code
   * initiator} / {@code initiatorLeader} / {@code ${spel}}。 多个规则用 {@code ,} 分隔，由 {@code
   * AssignmentResolver} 解析展开。
   */
 ...
 *   <li>{@code userWeights}：加权投票权重映射（userId -> weight）
   ...
   * 表单字段权限配置 JSON。
   *
   * <p>按节点控制字段可编辑/只读/隐藏，格式：{@code {"fieldKey":"EDIT|READONLY|HIDDEN",...}}。 由前端审批表单渲染时按当前节点配置应用。
   */
   ...
   *
   * <p>格式：{@code
   * {"timeoutMinutes":120,"action":"REMIND|ESCALATE|AUTO_PASS|AUTO_REJECT","maxUrges":3,"adminUserId":"xxx"}}。
