 ...
 *
 * <p><b>归档机制：</b>任务完成（终态）后由 {@code FlowArchiveScheduler} 从 {@link FlowRunTaskDO} 迁移至本表，
 * 避免运行态主表膨胀。本表按月分区（{@code ydsz_flow_his_task_2026m07}），超过保留期后由清理调度器删除。
 *
 * <p><b>设计要点：</b>{@code createdAt} 复用 {@link MpBaseEntity} 字段，但关闭 MyBatis-Plus 自动填充， 归档时由业务代码显式从源
 * {@code task.createdAt} 复制（保留业务创建时间，非归档时间）。
 *
 * <p><b>索引设计：</b>
 *
 * <ul>
 *   <li>普通索引 {@code idx_instance}（{@code instance_id}）：流程审批历史时间线查询
 *   <li>普通索引 {@code idx_assignee}（{@code assignee_id}）：「我审批过的」查询
 *   <li>普通索引 {@code idx_business}（{@code business_type}, {@code business_id}）：业务侧审批历史
 *   <li>普通索引 {@code idx_finish_at}（{@code finish_at}）：性能分析、报表
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
 * @see FlowRunTaskDO 运行态任务表
...
public class FlowHisTaskDO extends MpBaseIdEntity<String> {
   ...
   *
   * <p>循环节点每条独立 task 对应的集合元素，归档后保留以支持审批历史追溯 （如「这个审批是谁做的？属于哪一轮迭代？」）。 非 FOREACH 节点的 task 该字段为 {@code
   * null}。
   */
