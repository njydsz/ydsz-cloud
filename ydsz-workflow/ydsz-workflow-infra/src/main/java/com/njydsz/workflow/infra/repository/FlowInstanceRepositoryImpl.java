 ...
 *
 * <p>位于基础设施层，实现领域层定义的 {@link FlowInstanceRepository} 接口。
 * 内部委托 MyBatis-Plus Mapper 完成具体数据库操作。
 *
 * <p><b>分层定位：</b>依赖方向为 infra → domain（符合 DDD 依赖倒置原则），
 * domain 层定义接口契约，infra 层提供适配器实现。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class FlowInstanceRepositoryImpl implements FlowInstanceRepository {
    ...
    public long countByStatus(String flowStatus) {
        return instanceMapper.selectCount(new LambdaQueryWrapper<FlowInstance>()
