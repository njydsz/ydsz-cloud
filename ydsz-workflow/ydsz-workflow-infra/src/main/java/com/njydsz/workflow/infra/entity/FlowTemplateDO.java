 ...
 * 流程模板实体
 *
 * <p>对应数据库表 {@code ydsz_flow_template}，存储流程模板市场中的预置模板数据。 每个模板包含 BPMN 2.0 XML 流程定义，支持一键导入为流程草稿，
 * 也可将已发布的流程定义导出为模板复用于跨项目/跨租户场景。
 *
 * <p><b>核心使用场景：</b>
 *
 * <ul>
 *   <li>新租户/新项目接入时，从模板市场快速创建标准化流程
 *   <li>管理员将已稳定运行的流程定义导出为模板，作为最佳实践沉淀
 *   <li>模板市场页面按分类展示，供业务方浏览选用
 * </ul>
 *
 * <p><b>P2-9: 模板继承与版本化：</b>
 *
 * <ul>
 *   <li>{@link #parentTemplateId} + {@link #inheritType} 表达跨模板继承关系 （{@code STANDALONE}=独立 / {@code
 *       CLONE}=克隆 / {@code INHERIT}=继承）
 *   <li>{@link #version} + {@link #versionLabel} + {@link #isLatest} 表达同 {@code templateCode}
 *       下的多版本管理； 同一编码下 {@code isLatest=1} 的记录为当前生效版本
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>唯一索引 {@code uk_template_code_version}（{@code template_code}, {@code version}）
 *   <li>普通索引 {@code idx_category}（{@code category}）：按分类筛选
 *   <li>普通索引 {@code idx_parent}（{@code parent_template_id}）：模板继承关系
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
