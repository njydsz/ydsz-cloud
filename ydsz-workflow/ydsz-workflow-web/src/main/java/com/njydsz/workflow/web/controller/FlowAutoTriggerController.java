 ...
 * 流程自动触发规则 HTTP API
 *
 * <p>提供触发规则的 CRUD 管理接口，支持列表查询、创建、删除、启用/禁用切换。 触发规则在流程实例完成时自动生效，无需手动调用。
 *
 ...
 *
 * <ul>
 *   <li><b>规则列表</b>：{@code GET /list} — 列出全部已注册规则（含启用/禁用状态）
 *   <li><b>规则创建</b>：{@code POST /} — 注册新的触发规则（sourceFlow → targetFlow）
 *   <li><b>规则删除</b>：{@code DELETE /{id}} — 注销规则（不可恢复，建议改用禁用）
 *   <li><b>启停切换</b>：{@code PUT /{id}/toggle} — 启用或禁用规则
 * </ul>
 *
 * <p><b>业务场景：</b>
 *
 ...
 * </pre>
 *
 * <p>触发规则由 {@code FlowEventListener} 在源流程实例 COMPLETED 事件中异步加载并匹配， 满足条件的目标流程由 {@code
 * FlowAutoTriggerService.executeTrigger} 自动启动。
 *
 * <p><b>安全特性：</b>
 *
 * <ul>
 ...
 *   <li>条件表达式使用 QLExpress 沙箱执行，禁止访问外部资源
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class FlowAutoTriggerController {
   ...
   * <p>幂等保护 5 秒。
   *
   * <p>在 {@code enabled=true} 和 {@code enabled=false} 之间切换，<b>不删除规则</b>。 禁用后源流程 COMPLETED 时将不再触发。
   *
   * @param id 规则 ID
