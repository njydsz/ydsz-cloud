 ...
 *
 * <ul>
 *   <li><b>查询</b>：{@code GET /categories}（查询全部分类，按 sortNum 升序）
 *   <li><b>新增</b>：{@code POST /categories}（创建分类）
 *   <li><b>编辑</b>：{@code PUT /categories/{id}}（修改分类）
 *   <li><b>删除</b>：{@code DELETE /categories/{id}}（删除分类，校验是否有子分类或关联定义）
 * </ul>
 *
 * <p><b>权限模型：</b>所有写接口通过 {@link Idempotent} 5s 防重；分类编码唯一性 由 {@code @UniqueCheck} 拦截器在 Service 层校验。
 *
 * <p><b>多租户：</b>所有操作按 {@link TenantContext} 当前租户隔离， 跨租户分类不可见。
 *
 * <p><b>设计原则：</b>Controller 仅做参数透传与租户注入；分类业务逻辑下沉到 {@link FlowCategoryService}。树形结构由前端基于 {@code GET
 * /categories} 扁平结果自行组装。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class FlowCategoryController {
   ...
   *
   * <ul>
   *   <li>有<b>子分类</b>的分类<b>禁止删除</b>（避免悬挂引用）
   *   <li>有<b>流程定义</b>关联的分类<b>禁止删除</b>
   * </ul>
   *
