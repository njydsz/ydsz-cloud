 ...
 * </pre>
 *
 * <p><b>终态判定：</b>{@link #isFinished()} 标识流程已结束，
 * 包括正常完成、强制终止、驳回、已回滚。终态实例不允许再次变更状态（除非回滚撤销）。
 *
 * <p><b>使用场景：</b>
 * <ul>
 *   <li>流程监控页面按状态筛选/统计</li>
 *   <li>历史归档 Job 定时清理非 ERROR/RUNNING 状态实例</li>
 *   <li>业务侧监听（如 {@code ProjectInitiationFlowListener}）根据 ROLLED_BACK 状态做补偿</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public enum FlowInstanceStatus implements BaseStatusEnum<FlowInstanceStatus> {
     ...
     * P2-3: 已回滚
     *
     * <p>原本已 COMPLETED 的实例被发起人/管理员撤销，最终态。
     * 流程不再运行，但保留全部历史轨迹，供业务侧
     * （如 {@code ProjectInitiationFlowListener}）感知并执行回滚补偿逻辑。
     */
    ROLLED_BACK;
     ...
     * 是否为终态
     *
     * <p>终态指流程已结束（无论正常完成、强制终止、驳回还是回滚）。
     * 终态实例不允许再次变更状态（ROLLBACK 除外）。
     *
     * @return true-终态（COMPLETED / TERMINATED / REJECTED / ROLLED_BACK）；false-非终态（RUNNING / SUSPENDED / ERROR）
     */
