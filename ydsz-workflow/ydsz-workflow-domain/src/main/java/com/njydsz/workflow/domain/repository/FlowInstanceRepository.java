 ...
 *
 * <p>领域层定义的持久化抽象，隔离领域模型与具体数据访问技术实现。
 * 应用层 Service 通过此接口操作聚合根，不直接依赖 MyBatis Mapper。
 *
 * <p><b>设计原则：</b>
 * <ul>
 *   <li>以聚合根（{@link FlowInstance}）为操作单位</li>
 *   <li>接口签名使用领域语言（如 {@link #save(FlowInstance)} 而非 insert/update）</li>
 *   <li>实现类位于 {@code ydsz-workflow-infra} 模块</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public interface FlowInstanceRepository {
     ...
     * 保存流程实例（新增 or 更新）
     *
     * <br>新增：entity.id 由 Snowflake 生成后传入
     * <br>更新：根据 entity.id 定位并更新
     *
