 ...
 *
 * <ul>
 *   <li>以聚合根（{@link FlowInstanceDO}）为操作单位
 *   <li>接口签名使用领域语言（如 {@link #save(FlowInstanceDO)} 而非 insert/update）
...
public interface FlowInstanceRepository {
   ...
   */
  Optional<FlowInstanceDO> findById(String id);
   ...
   * @return Optional 包装的流程实例
   */
  Optional<FlowInstanceDO> findByBusiness(String businessType, String businessId);
   ...
   * @return 流程实例列表
   */
  List<FlowInstanceDO> findByInitiatorId(String initiatorId);
   ...
   * @return 子流程实例列表
   */
  List<FlowInstanceDO> findChildren(String parentInstanceId);
   ...
   * @return 实例列表
   */
  List<FlowInstanceDO> findSuspendedBefore(java.time.LocalDateTime before, int limit);
