 ...
 * P0-1: DMN 决策规则行实体
 *
 * <p>对应数据库表 {@code ydsz_flow_dmn_rule}，每条规则对应决策表中的一行， 包含输入条件单元格和输出值单元格。输入条件为逗号分隔的比较表达式列表（与 {@link
 * FlowDmnDecision#getInputDefinitions} 一一对应）， 输出值为逗号分隔的值列表（与 {@link
 * FlowDmnDecision#getOutputDefinitions} 一一对应）。
 *
 * <p><b>输入条件格式示例：</b>
 *
 * <pre>
 ...
 *
 * <ul>
 *   <li>{@code FIRST}：取第一条命中的规则
 *   <li>{@code UNIQUE}：仅一条命中（命中数量校验）
 *   <li>{@code ANY}：多条命中时输出必须一致
 *   <li>{@code COLLECT}：收集所有命中的输出
 * </ul>
 *
 * <p><b>索引设计：</b>
 *
 * <ul>
 *   <li>普通索引 {@code idx_decision_order}（{@code decision_id}, {@code rule_order}）：按决策表 + 顺序查询
