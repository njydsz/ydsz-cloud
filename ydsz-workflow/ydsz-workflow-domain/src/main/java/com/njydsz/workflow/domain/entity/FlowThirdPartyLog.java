 ...
 * 三方审批回调日志实体
 *
 * <p>对应数据库表 {@code ydsz_flow_third_party_log}，P0-2: 三方审批 SDK（钉钉/飞书/企微）回调原始数据落库。 回调入口先以 {@code
 * PENDING} 状态写入，处理完成后更新为 {@code SUCCESS/FAIL}， 由独立重试任务保证最终一致。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li>三方审批回调的<b>全量审计</b>（原始数据持久化，便于问题回溯）
 *   <li>回调处理的<b>最终一致性</b>（失败重试 → 死信队列人工介入）
 *   <li>本地 → 三方的<b>双向同步状态</b>追踪
 * </ul>
 *
 * <p><b>处理状态（{@code handleStatus}）：</b>
 *
 * <ul>
 *   <li>{@code PENDING}：已接收待处理
 *   <li>{@code SUCCESS}：处理成功
 *   <li>{@code FAIL}：处理失败（{@code errorMsg} 记录原因，超过最大重试次数后进入死信）
 * </ul>
 *
 * <p><b>双向同步状态（{@code syncBackStatus}）：</b>本地 → 三方的回撤/取消同步结果。
 *
 * <ul>
 *   <li>{@code NOT_REQUIRED}：无需回撤
 *   <li>{@code PENDING}：回撤中
 *   <li>{@code SUCCESS}：回撤成功
 *   <li>{@code FAIL}：回撤失败（{@code syncBackMsg} 记录原因）
 * </ul>
 *
 * <p><b>重试机制：</b>{@code retryCount} 由 {@code ThirdPartyCallbackRetryJob} 累加， 超过 {@code
 * jobhandler.third-party-callback.max-retry}（默认 5 次）后进入死信队列。
 *
 * <p><b>索引设计：</b>
 *
 * <ul>
 *   <li>普通索引 {@code idx_platform_instance}（{@code platform}, {@code process_instance_id}）
 *   <li>普通索引 {@code idx_status}（{@code handle_status}）：按状态查询待重试
