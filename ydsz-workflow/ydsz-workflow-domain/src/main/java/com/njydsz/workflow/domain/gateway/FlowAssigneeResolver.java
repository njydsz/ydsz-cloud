 ...
 * 办理人解析 SPI
 *
 * <p>业务模块（如 user 模块）可提供实现，将 ROLE/DEPT/LEADER/POSITION 展开为具体用户列表。引擎在创建任务时调用本接口展开多人会签/或签。
 *
...
public interface FlowAssigneeResolver {
   ...
   *
   * <p>从指定用户开始逐级向上查找上级，返回各级上级的用户 ID 列表。 例如 userId=1001, levels=3 → [直属上级, 上上级, 上上上级]
   *
   ...
   * P1-5: 查询部门负责人
   *
   * <p>将部门 ID 解析为该部门的负责人用户 ID。 业务方需提供实现：查询部门主表的 leader_user_id 字段。
   *
   * @param deptId 部门 ID
   * @param variables 流程变量（可用于动态解析）
