 ...
 * 记录系统用户与三方平台账号的映射关系，并缓存访问/刷新令牌（加密存储）， 供三方审批回调时反查系统用户、驱动工作流通过/驳回等操作。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li>系统用户 ↔ 三方平台账号绑定（一个用户可在多平台有账号）
 *   <li>缓存 access/refresh token（加密存储），供审批同步使用
 *   <li>三方审批回调时反查系统用户，触发流程推进
 *   <li>支持「本地 → 三方」双向同步（如本地撤销时调用 cancelWebhookUrl）
 * </ul>
 *
 * <p><b>支持平台（{@code platform}）：</b>
 *
 * <ul>
 *   <li>{@code DINGTALK}：钉钉审批
 *   <li>{@code FEISHU}：飞书审批
 *   <li>{@code WECOM}：企业微信审批
 * </ul>
 *
 * <p><b>状态机（{@code status}）：</b>{@code ACTIVE}（生效中）/ {@code INACTIVE}（未激活）/ {@code REVOKED}（已撤销授权）。
 *
 * <p><b>令牌刷新：</b>由 {@code ThirdPartyTokenRefresher} 调度器在 {@code tokenExpireAt} 前 5 分钟触发刷新， 避免调用三方
 * API 时 token 失效。
 *
 * <p><b>索引设计：</b>
 *
 * <ul>
 *   <li>唯一索引 {@code uk_platform_open}（{@code platform}, {@code openId}）
 *   <li>普通索引 {@code idx_user}（{@code user_id}）：查询用户的绑定
 *   <li>普通索引 {@code idx_token_expire}（{@code token_expire_at}）：token 刷新扫描
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
