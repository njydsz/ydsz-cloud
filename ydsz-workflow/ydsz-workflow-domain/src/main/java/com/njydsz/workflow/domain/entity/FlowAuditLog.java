 ...
 *
 * <p><b>域内职责说明：</b>本类属于<b>工作流领域</b>内部的事件溯源记录，域内职责为
 * 「节点流转追溯」（如：审批轨迹时间线、加签历史、代理操作日志）。
 * <b>与通用操作审计（{@code ydsz-common-audit} 的 {@code AuditLog}）是两套独立体系，不可互相替代：</b>
 * <ul>
 *   <li>本类：工作流域内审计，记录的是流程引擎内部的状态流转事件（START/PASS/REJECT 等），
 *       属性与流程实例/任务/节点强绑定，对业务方查询、效率统计、合规追溯有直接价值</li>
 *   <li>{@code ydsz-common-audit.OperationLog / AuditLog}：横切关注点，记录的是「哪个用户
 *       在哪个应用调用了哪个 API、入参/出参/耗时/结果」，用于安全审计与运维排查</li>
 * </ul>
 *
