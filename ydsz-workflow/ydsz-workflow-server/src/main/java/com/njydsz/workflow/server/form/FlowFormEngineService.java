 ...
 *
 * <p>作为表单 Schema 管理、校验和字段权限协同的统一入口。 在流程任务提交（通过/拒绝/保存草稿）时调用本服务进行表单校验。
 *
 * <p>与 {@link com.njydsz.workflow.server.service.FlowFormFieldPermService} 协作：
 *
 * <ul>
 *   <li>字段权限服务负责控制字段的可见性/可编辑性/必填性（按节点）
 *   <li>表单校验引擎负责校验字段值的格式/范围/长度/正则等（按 Schema 定义）
...
public class FlowFormEngineService {
   ...
   *
   * <ol>
   *   <li>检查 HIDDEN 字段是否被提交（拒绝）
   *   <li>检查 REQUIRED 字段是否为空（拒绝）
   *   <li>执行 Schema 校验（格式/范围/长度等）
   * </ol>
   *
  ...
  public void validateAndThrow(
      FlowFormSchema schema, Map<String, Object> formData, Map<String, String> fieldPerms) {
