 ...
 *
 * <ol>
 *   <li>从 callbackData 反序列化为 Map
 *   <li>解析三方事件 → 工作流动作（PASS/REJECT/WITHDRAW）
 *   <li>通过 openId 反查系统用户
 *   <li>调用 {@link FlowEmbeddedApprovalService#quickAction} 派发
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li>{@link #retryOneInternal} 使用 REQUIRES_NEW 子事务，仅承载 quickAction 等业务调用， 不调用
 *       mapper.updateRetryResult（避免子事务回滚污染状态更新）。
 *   <li>状态更新（SUCCESS/FAIL + retryCount++）统一在外层 {@link #retryBatch} / {@link #retryOne} 中通过 {@link
 *       #updateRetryResultSafely} 调用，无事务包裹，每次更新独立提交。
 * </ul>
 *
 * <p>容错策略与首次处理对齐：
 *
 * <ul>
 *   <li>事件不支持 / 缺 businessType → 标 SUCCESS（与首次一致，避免死循环）
 *   <li>账号未映射 → 标 FAIL（账号映射可能在后续补全，可继续重试）
 *   <li>SysException（找不到任务/流程已结束）→ 标 SUCCESS（容错跳过，与首次一致）
 *   <li>其他系统异常 → 标 FAIL（继续重试直至超阈值进入死信）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class FlowThirdPartyRetryServiceImpl implements FlowThirdPartyRetryService {
  ...
  public RetryResult retryBatch(int maxRetries, int batchSize) {
    ...
    try {
      ...
      log.error(
          "[ThirdPartyRetry] 扫描失败回调异常: maxRetries={} batchSize={} err={}",
          maxRetries,
          batchSize,
          e.getMessage(),
          e);
    ...
    for (FlowThirdPartyLog logEntry : failedList) {
      ...
      try {
        ...
        switch (outcome) {
          case SUCCESS -> {
            result.success++;
            updateRetryResultSafely(
                logEntry, FlowThirdPartyLogService.STATUS_SUCCESS, null, newRetryCount);
          }
          case FAIL_ACCOUNT_NOT_MAPPED -> {
            result.fail++;
            updateRetryResultSafely(
                logEntry,
                FlowThirdPartyLogService.STATUS_FAIL,
                "account not mapped",
                newRetryCount);
            log.warn(
                "[ThirdPartyRetry] 账号未映射 logId={} platform={} openId={} retryCount={}/{}",
                logEntry.getId(),
                logEntry.getPlatform(),
                extractOpenId(logEntry),
                newRetryCount,
                maxRetries);
          }
          case SKIPPED -> {
            ...
            updateRetryResultSafely(
                logEntry, FlowThirdPartyLogService.STATUS_SUCCESS, null, newRetryCount);
            log.info(
                "[ThirdPartyRetry] 容错跳过 logId={} platform={} eventType={}",
                logEntry.getId(),
                logEntry.getPlatform(),
        ...
        updateRetryResultSafely(
            ...
            newRetryCount);
        log.error(
            "[ThirdPartyRetry] 重试单条异常 logId={} platform={} err={}",
            logEntry.getId(),
            logEntry.getPlatform(),
            e.getMessage(),
            e);
    ...
    log.info(
        "[ThirdPartyRetry] 重试批次完成: scanned={} success={} fail={} skipped={} errors={} maxRetries={}",
        result.scanned,
        result.success,
        result.fail,
        result.skipped,
  ...
  public boolean retryOne(FlowThirdPartyLog logEntry) {
    ...
    try {
      ...
      return switch (outcome) {
        case SUCCESS, SKIPPED -> {
          updateRetryResultSafely(
              logEntry, FlowThirdPartyLogService.STATUS_SUCCESS, null, newRetryCount);
          log.info(
              "[ThirdPartyRetry] 手动重试成功 logId={} outcome={} retryCount={}",
              logEntry.getId(),
              outcome,
              newRetryCount);
          yield true;
        ...
        case FAIL_ACCOUNT_NOT_MAPPED -> {
          updateRetryResultSafely(
              logEntry, FlowThirdPartyLogService.STATUS_FAIL, "account not mapped", newRetryCount);
          log.info(
              "[ThirdPartyRetry] 手动重试失败(账号未映射) logId={} retryCount={}",
              logEntry.getId(),
              newRetryCount);
          yield false;
        }
      ...
      updateRetryResultSafely(
          ...
          truncate(e.getMessage(), 512),
          newRetryCount);
      log.error("[ThirdPartyRetry] 手动重试异常 logId={} err={}", logEntry.getId(), e.getMessage(), e);
      return false;
    }
  }
   ...
   *
   * <p>事务边界：本方法仅包裹 {@link FlowEmbeddedApprovalService#quickAction} 等业务调用， 不调用 {@code
   * mapper.updateRetryResult}。这样即使 quickAction 抛出 RuntimeException 导致 子事务回滚，也不会污染外层的状态更新逻辑。
   *
   * @param logEntry 日志记录
   * @return 重试结果枚举
  ...
  protected RetryOutcome retryOneInternal(FlowThirdPartyLog logEntry) {
    ...
    try {
      ...
      log.warn(
          "[ThirdPartyRetry] SysException 容错跳过 logId={} code={} msg={}",
          logEntry.getId(),
          e.getCode(),
          e.getMessage());
      return RetryOutcome.SKIPPED;
    ...
    log.info(
        "[ThirdPartyRetry] 重试成功 logId={} platform={} userId={} action={}",
        logEntry.getId(),
        platform,
        account.getUserId(),
        action);
  ...
  private void updateRetryResultSafely(
      FlowThirdPartyLog logEntry, String status, String errorMsg, int retryCount) {
    try {
      thirdPartyLogMapper.updateRetryResult(logEntry.getId(), status, errorMsg, retryCount);
    } catch (Exception e) {
      log.error(
          "[ThirdPartyRetry] 更新重试结果失败 logId={} status={} retryCount={} err={}",
          logEntry.getId(),
          status,
          retryCount,
          e.getMessage(),
  ...
  private Map<String, Object> parseCallbackData(String callbackData) {
    ...
    try {
      ...
      log.warn(
          "[ThirdPartyRetry] callbackData 解析失败: {} err={}",
