...
public interface FlowEmbeddedApprovalService {
   ...
   * 加载嵌入式审批面板（聚合查询）
   *
   * <p>业务页只需要传入 businessType+businessId+currentUserId， 不需要先查 taskId、不需要单独拉流程图/历史轨迹。
   *
   ...
   *
   * <ul>
   *   <li>PASS — 通过（找到当前用户 mine=true 的待办任务）
   *   <li>REJECT — 驳回（找到 mine=true 的待办任务）
   *   <li>TRANSFER — 转办（需 targetUserId）
   *   <li>DELEGATE — 委派（需 targetUserId）
   *   <li>URGE — 催办（无需 mine）
