 ...
 *
 * <p>当用户禁用、组织架构变更等事件发生时，由 {@code CrossModuleEventListener} 调用本服务，将该用户名下的待办任务转交给代理人或上级，确保审批流程不中断。
 *
 * <p>核心场景：
 *
 * <ul>
...
public interface FlowTaskTransferService {
   ...
   * 用户禁用时转交待办任务。
   *
   * <p>将该用户名下所有 PENDING 状态的待办任务转交给指定代理人。 如果代理人为空，则尝试查找该用户的上级作为默认转交目标。
   *
   ...
   * 组织架构变更时批量调整审批人。
   *
   * <p>当部门合并/拆分/撤销时，批量调整该部门下所有在途流程的审批人配置， 确保审批人指向有效的组织节点。
   *
