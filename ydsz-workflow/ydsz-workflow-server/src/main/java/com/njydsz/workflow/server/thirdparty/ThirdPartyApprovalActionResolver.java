 ...
 *       <ul>
 *         <li>{@code AGREE} → PASS
 *         <li>{@code REFUSE} → REJECT
 *       </ul>
 *   <li>飞书：
 *       <ul>
 *         <li>{@code approval.approved} → PASS
 *         <li>{@code approval.rejected} → REJECT
 *         <li>{@code approval.canceled} → WITHDRAW（三方撤销对应发起人撤回）
 *       </ul>
 *   <li>企微 {@code sys_approval_change}：根据 {@code status} 区分
 *       <ul>
 *         <li>{@code 1}（审批通过）→ PASS
 *         <li>{@code 2}（审批驳回）→ REJECT
 *         <li>{@code 3}（审批撤销）→ WITHDRAW
 *       </ul>
 * </ul>
 *
 * <p>无法映射的事件返回 {@code null}（如钉钉 bpmsInstanceChange 实例级变更、 飞书抄送事件等），由调用方决定是否忽略。
 *
 * @since 1.0.0
 * @author ydsz-team
 */
