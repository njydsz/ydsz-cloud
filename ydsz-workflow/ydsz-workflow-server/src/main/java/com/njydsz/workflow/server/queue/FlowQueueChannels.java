 ...
 * 工作流消息队列通道常量（语义别名）。
 *
 * <p>定义工作流模块使用的所有消息队列通道名称，用于跨服务事件分发。 通过 common-queue 的 IMessagePublisher 发布事件，其他服务（如 project 模块）
 * 可订阅这些通道实现跨服务异步通信。
 *
 ...
 *
 * <ul>
 *   <li>{@link #FLOW_EVENT} - 工作流生命周期事件（任务创建/完成、实例启动/完成/驳回/终止等）
...
public final class FlowQueueChannels {
   ...
   * 工作流生命周期事件通道
   *
   * <p>事件类型包括：INSTANCE_STARTED, INSTANCE_COMPLETED, INSTANCE_REJECTED, INSTANCE_TERMINATED,
   * INSTANCE_RECALLED, TASK_CREATED, TASK_COMPLETED, TASK_URGED, TASK_TRANSFERRED, TASK_DELEGATED,
   * TASK_TIMEOUT 等。
   */
