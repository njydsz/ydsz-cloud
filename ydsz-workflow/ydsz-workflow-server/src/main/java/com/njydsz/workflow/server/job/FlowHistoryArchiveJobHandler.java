 ...
 *
 * <p>每日 03:00 扫描已结束（COMPLETED/TERMINATED/REJECTED）且结束时间超过阈值的流程实例， 将其从主表迁移到 {@code
 * ydsz_flow_his_instance} 冷存储表，同时归档关联的 variable。
 *
 * <p>P2-8 改造点：
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Bean 名称 = {@code flowHistoryArchiveJobHandler}， 可在 ydsz_job
 * 表配置：handler=flowHistoryArchiveJobHandler, cron="0 0 3 * * ?"
 *
 * @since 1.0.0
 * @author ydsz-team
 */
...
public class FlowHistoryArchiveJobHandler implements JobHandler {
   ...
   * 执行归档与清理
   *
   * <p>paramsJson 可包含 days/batchSize/maxProcessMs/purgeDays，覆盖配置默认值。 适用于临时触发一次特殊参数的归档（如手动归档 90
   * 天前的数据）。
   *
