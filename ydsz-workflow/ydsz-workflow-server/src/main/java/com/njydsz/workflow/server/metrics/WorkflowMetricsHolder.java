 ...
 *
 * <p>为流程引擎核心路径提供 Micrometer 指标注册与累加能力， 通过静态方法方便业务代码（如 {@code DefaultFlowAdvancer}、{@code
 * FlowTaskService}）埋点。
 *
 ...
 *
 * <ul>
 *   <li>{@code workflow.start_total{process_def_key}} — 流程启动计数
 *   <li>{@code workflow.task_complete_total{process_def_key}} — 任务完成计数
 *   <li>{@code workflow.execution_duration{process_def_key}} — 流程平均执行耗时分布
 *   <li>{@code workflow.task_timeout_total{process_def_key}} — 流程卡住/超时计数
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public final class WorkflowMetricsHolder extends AbstractMetricsHolder {
  ...
  public static void incrementStart(String processDefKey) {
    registerCounter(METRIC_PREFIX, "start_total", "process_def_key", safe(processDefKey))
        .increment();
  }
  ...
  public static void incrementTaskComplete(String processDefKey) {
    registerCounter(METRIC_PREFIX, "task_complete_total", "process_def_key", safe(processDefKey))
        .increment();
  }
  ...
  public static void recordExecutionDuration(String processDefKey, long millis) {
    recordDuration(
        METRIC_PREFIX, "execution_duration", millis, "process_def_key", safe(processDefKey));
  }
  ...
  public static void incrementTaskTimeout(String processDefKey) {
    registerCounter(METRIC_PREFIX, "task_timeout_total", "process_def_key", safe(processDefKey))
        .increment();
  }
}
