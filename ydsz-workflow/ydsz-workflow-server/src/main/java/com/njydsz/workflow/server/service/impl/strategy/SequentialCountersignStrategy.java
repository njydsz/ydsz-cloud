  ...
  public void onUserPassed(FlowRunTask task, FlowTaskOperateDTO dto) {
    ...
    if (finished < required) {
      ...
      List<FlowUser> unprocessed =
          userMapper.selectUnprocessedByInstanceAndNode(task.getInstanceId(), task.getNodeCode());
      if (!unprocessed.isEmpty()) {
        FlowUser next = unprocessed.get(0);
        taskMapper.updateAssignee(
            task.getId(), next.getUserId(), next.getUserName(), FlowAssigneeType.USER.name());
      }
    } else {
