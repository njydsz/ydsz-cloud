 ...
 * 流程实例 Service
 *
 * <p>流程实例是工作流引擎的<b>运行时核心</b>，本 Service 负责实例的整个生命周期： 启动、查询、终止、挂起/激活、归档、子流程级联等。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li><b>实例生命周期</b>：启动（{@link #start}）/ 终止（{@link #terminate}）/ 挂起（{@link #suspend}）/ 激活（{@link
 *       #activate}）/ 完成（{@link #complete}）
 *   <li><b>批量能力</b>：批量启动（{@link #batchStartInstances}）/ 批量终止（{@link #batchTerminate}）
 *   <li><b>查询能力</b>：按 ID / 按业务关联 / 按发起人 / 按状态 / 待办聚合查询
 *   <li><b>子流程</b>：父-子流程级联（终止时自动终止所有子实例）
 *   <li><b>幂等性</b>：{@link #start} 基于 {@code (businessType, businessId)} 唯一索引保证幂等
 * </ul>
 *
 * <p><b>事务边界：</b>
 *
 * <ul>
 *   <li>{@link #start} 整体事务（含初始化任务、抄送、审计日志）
 *   <li>{@link #batchStartInstances} 每个 {@link FlowStartProcessDTO} 独立事务
 *   <li>跨服务调用（如业务侧）使用 {@code Spring Event} 解耦，避免大事务
 * </ul>
 *
 * <p><b>性能优化：</b>
 *
...
public interface FlowInstanceService {
   ...
   * P2-6: 批量发起流程实例。
   *
   * <p>对标钉钉/飞书"批量发起审批"能力：一次性提交多个流程实例，每个实例独立事务， 单个失败不影响其他实例的发起。适用于"批量立项"、"批量报销"等场景。
   *
   * <p>行为约定：
   *
   * <ul>
   *   <li>每个 {@link FlowStartProcessDTO} 独立事务，失败记录到 failedItems
   *   <li>返回成功发起的 instanceId 列表 + 失败项明细
   *   <li>限制单次批量最大 100 条（防止事务过多）
   *   <li>幂等性由 {@link #start} 内部保证（同 businessType+businessId 已有 RUNNING 实例时返回原 ID）
   * </ul>
   *
   ...
   * 终止流程（管理员强制终止）
   *
   * <p>终止时级联取消所有 PENDING 任务、关闭实例、记录审计日志。 子流程实例会被自动级联终止。终止后实例状态变为 TERMINATED。
   *
   ...
   * 挂起流程实例
   *
   * <p>实例挂起后所有 PENDING/CLAIMED 任务变为 FROZEN 状态， 不会推进、不计超时。激活后任务回到原状态继续处理。
   *
   * @param instanceId 实例 ID
   */
   ...
   * 强制完成（驳回到终态时由调用方使用）
   *
   * <p>主要用于驳回到「结束节点」的终态处理：实例状态变为 COMPLETED， 所有 PENDING 任务被取消。
   *
   * @param instanceId 实例 ID
   ...
   *
   * <p>默认撤回到开始节点下游第一节点（重新生成第一批待办）。 如需撤回到任意历史节点，请使用 {@link #recall(String, String, String)}。
   *
   * @param instanceId 实例 ID
   * @param initiatorId 发起人 ID
   * @return 是否撤回成功
   ...
   *
   * <p>与 {@link #recall(String, String)} 的差异：调用方可显式指定退回到任意历史已办节点， 而非仅能撤回到开始节点下游第一节点。
   *
   * <p>校验规则：
   *
   * <ul>
   *   <li>继承 {@link #recall(String, String)} 的全部校验（发起人、运行中、未处理）；
   *   <li>额外校验 targetNodeCode 必须在 {@link #listRecallableNodes(String, String)} 返回的列表中。
   * </ul>
   *
   * @param instanceId 实例 ID
   * @param initiatorId 发起人 ID
   ...
   * P2-3: 回滚已完成的流程实例（撤销）
   *
   * <p>对标钉钉/飞书的"撤销审批"能力。已完成（COMPLETED）的流程实例， 在回滚时间窗口内（默认 7 天）可由发起人或管理员撤销：
   *
   * <ul>
   *   <li>实例状态置为 ROLLED_BACK，保留全部历史轨迹；
   *   <li>不再重新推进流程，业务侧（如 ProjectInitiationFlowListener）通过 监听 onInstanceRolledBack 事件执行补偿逻辑；
   *   <li>记录回滚操作人、时间、原因到 variable JSON。
   * </ul>
   *
   * <p>校验规则：
   *
   * <ul>
   *   <li>仅 COMPLETED 状态可回滚（TERMINATED/REJECTED 不可回滚，已驳回/已终止为最终态）；
   *   <li>仅发起人或管理员可回滚；
   *   <li>endAt 距当前时间不得超过 maxRollbackDays（默认 7 天）。
   * </ul>
   *
   * @param instanceId 实例 ID
   ...
   *
   * <p>从开始节点出发，按照 variables 中的条件变量模拟条件判断，依次走过每个节点并记录路径。 遇到 END 节点终止；遇到循环或超过 50
   * 个节点时终止（防死循环）。不写数据库，不创建实例/任务。
   *
   * @param flowCode 流程编码
   ...
   */
  List<Map<String, Object>> simulate(
   ...
   * P2-2 (GAP-10): 驳回后快速重审
   *
   * <p>实例处于 REJECTED 终态时，发起人可基于原实例直接重新提交（而非全新发起）， 引擎保留原有审批轨迹与流程变量，将实例状态恢复为 RUNNING 并从开始节点重新推进。
   *
   * <p>校验规则：
   *
   * <ul>
   *   <li>仅 REJECTED 状态可重审（RUNNING/COMPLETED/TERMINATED/ROLLED_BACK 不可重审）；
   *   <li>仅发起人可重审；
   *   <li>重审时传入的新变量会合并覆盖（保留未覆盖的历史变量）。
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>{@code RESTART}（默认，向后兼容）：仅 REJECTED 实例可重做，在原实例上重置状态并从开始节点重新推进。 等价于 {@link
   *       #resubmit(String, String, Map, String)}。
   *   <li>{@code NEW_INSTANCE}：任意终态（COMPLETED / REJECTED / TERMINATED / ROLLED_BACK）均可重做， 创建全新实例（新
   *       instanceId），复用原实例的 flowCode / businessType / businessId / initiator，
   *       合并原变量与传入变量。原实例保持不变，仅追加一条 REDO 审计日志。
   * </ul>
   *
   * <p>校验规则：
   *
   * <ul>
   *   <li>RESTART 模式：仅 REJECTED 可重做（与原 resubmit 一致）；
   *   <li>NEW_INSTANCE 模式：仅终态可重做（非 RUNNING / SUSPENDED）；
   *   <li>两种模式均要求仅发起人可操作。
   * </ul>
   *
