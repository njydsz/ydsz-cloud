 ...
 *
 * <ul>
 *   <li>通过 ydsz_job 表配置：handler=flowThirdPartyRetryJobHandler, cron="0 0/10 * * * ?"（每 10 分钟扫描一次）
 *   <li>paramsJson 可选参数：
 *       <ul>
 *         <li>{@code maxRetries} — 最大重试次数阈值，默认 3（超过则进入死信不再扫描）
 *         <li>{@code batchSize} — 单批扫描条数，默认 50
 *         <li>{@code lockLeaseSec} — 集群锁持有时间（秒），默认 120
 *       </ul>
 * </ul>
 *
 * <p>集群幂等：通过 {@link FlowClusterLockHelper#tryRun} 包装，多节点部署时同一时刻只有一个节点执行。 锁 key = {@code
 * thirdPartyRetry:scan}，TTL 由 lockLeaseSec 控制。
 *
 * <p>容错策略：
 *
 * <ul>
 *   <li>未获取锁（其他节点正在执行）→ 直接返回 skipped
 *   <li>扫描异常 / 重试异常 → 返回 error，但不抛出（避免 JobHandler 框架标记任务失败）
 *   <li>单条重试异常由 {@link FlowThirdPartyRetryService#retryBatch} 内部 try-catch，不影响其他条目
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class FlowThirdPartyRetryJobHandler implements JobHandler {
  ...
  public Object execute(String paramsJson) throws Exception {
    ...
    // 集群幂等：尝试获取锁后执行
    FlowThirdPartyRetryService.RetryResult result =
        clusterLockHelper.tryRun(
            LOCK_KEY, lockLeaseSec, () -> retryService.retryBatch(maxRetries, batchSize));
    ...
    log.info(
        "[ThirdPartyRetry] 执行完成: scanned={} success={} fail={} skipped={} errors={} costMs={}",
        result.scanned,
        result.success,
        result.fail,
        result.skipped,
  ...
  private int parseIntParam(String paramsJson, String key, int defaultValue) {
    ...
    try {
      ...
      log.warn(
          "[ThirdPartyRetry] 参数解析失败，使用默认值: key={} default={} err={}",
          key,
          defaultValue,
          e.getMessage());
      return defaultValue;
