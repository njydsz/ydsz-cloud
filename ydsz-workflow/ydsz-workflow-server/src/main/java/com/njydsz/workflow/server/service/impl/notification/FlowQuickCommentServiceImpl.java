 ...
 * 审批常用语服务实现
 *
 * <p>P1-2: 对标钉钉/飞书审批的"常用语"能力，对 {@link FlowQuickCommentService} 接口的完整实现。 提供审批评论常用语（快捷回复模板）的
 * CRUD、用户隔离、使用统计等完整业务能力。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li><b>查询能力</b>：{@link #listByUser} — 返回「用户自定义 + 系统预设」两类常用语， 按 {@code sortNum} 升序、{@code
 *       useCount} 降序排列
 *   <li><b>CRUD</b>：{@link #create}（创建用户自定义）/ {@link #update}（仅创建者本人）/ {@link
 *       #delete}（仅创建者本人，<b>系统预设不可删</b>）
 *   <li><b>使用统计</b>：{@link #incrementUseCount} — 用户使用常用语后 +1， <b>按使用频次智能排序</b>
 *   <li><b>用户隔离</b>：用户仅可管理自己创建的常用语，系统预设为全局共享
 *   <li><b>多租户</b>：所有数据按 {@code tenantId} 隔离
 * </ul>
 *
 * <p><b>事务边界：</b>
 *
 * <ul>
 *   <li>所有写方法开启 {@code @Transactional(rollbackFor = Exception.class)}，确保数据一致性
 ...
 *   <li>查询走 {@code ydsz_flow_quick_comment} 复合索引（{@code idx_user} + {@code idx_tenant}）
 *   <li>{@link #incrementUseCount} 使用先查后更，<b>并发场景下 useCount 可能丢失更新</b>， 生产环境建议改用 {@code UPDATE ...
 *       SET use_count = use_count + 1} 原子操作
 * </ul>
 *
 * <p><b>防御性编程：</b>
 *
 * <ul>
 *   <li>{@link #listByUser} 当 userId 为空时返回空列表（前端 bug 兜底）
 *   <li>{@link #delete} 当常用语不存在时直接返回（幂等性）
 *   <li>{@link #incrementUseCount} 异常被 try-catch 吞掉，<b>不抛异常</b>， 避免使用统计失败影响主流程评论发布
 *   <li>{@link #update} / {@link #delete} 校验<b>创建者本人</b>才能操作， 防止越权修改他人常用语
 * </ul>
 *
 * <p><b>典型使用：</b>
 *
...
public class FlowQuickCommentServiceImpl implements FlowQuickCommentService {
   ...
   *
   * <p>合并查询：先查 {@code userId=userId} 的自定义常用语，再追加 {@code isSystem=1} 的系统预设， 最终按 {@code sortNum}
   * 升序、{@code useCount} 降序两级排序。
   *
   ...
   * 创建用户自定义常用语
   *
   * <p>仅创建用户自定义常用语（{@code isSystem=0}），{@code useCount} 初始为 {@code 0}。 创建时强制覆盖 {@code
   * userId/tenantId/isSystem=0}，<b>不可通过 DTO 伪造为系统预设</b>。
   *
   ...
   * 更新常用语
   *
   * <p>仅允许<b>创建者本人</b>更新；<b>系统预设不可更新</b>（实际接口层就不应该走到这里， 因为系统预设不暴露 update 接口给前端）。仅更新 DTO 中非空字段。
   *
   * @param dto 常用语 DTO（{@code id} 必传）
   * @param userId 操作人 ID（必须与创建者一致）
   * @throws SysException {@code BAD_REQUEST} — id 为空；{@code NOT_FOUND} — 常用语不存在； {@code FORBIDDEN}
   *     — 操作人非创建者
   */
  @Override
  @Transactional(rollbackFor = Exception.class)
   ...
   *
   * <ul>
   *   <li>系统预设（{@code isSystem=1}）<b>不可删除</b>，抛 {@code BAD_REQUEST}
   *   <li>仅创建者本人可删除，<b>非创建者</b>抛 {@code FORBIDDEN}
   * </ul>
   *
   * @param id 常用语 ID
   * @param userId 操作人 ID
   ...
   * 增加常用语使用次数
   *
   * <p>用户在前端选择常用语时调用，{@code useCount} 自增 1。 异常被 try-catch 吞掉记 WARN，<b>不传播异常</b>——使用统计失败不应阻塞评论发布主流程。
   *
   * <p><b>已知风险：</b>采用「先查后更」非原子操作，<b>高并发场景下 useCount 可能丢失更新</b>。 生产环境建议改用 SQL 原子更新 {@code UPDATE ...
