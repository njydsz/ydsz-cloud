...
public interface FlowTemplateService {
   ...
   * @return 模板列表，每行含 templateCode / templateName / category / description / icon / useCount /
   *     formPath
   */
   ...
   * 获取模板详情（含 BPMN XML）
   *
   * <p>P2-9: 默认返回 {@code is_latest=1} 的最新版本。如需指定历史版本，请使用 {@link #getTemplateVersion(String,
   * Integer)}。
   *
   ...
   * 从模板导入创建流程定义
   *
   * <p>读取模板（最新版本）的 BPMN XML，通过 {@code FlowDefinitionService.deploy} 部署为草稿流程定义。 导入成功后自动增加模板的
   * use_count。
   *
   * @param templateCode 模板编码
   ...
   * 将已发布流程导出为模板
   *
   * <p>读取流程定义的节点/跳转数据，生成对应的 BPMN XML 并写入 ydsz_flow_template 表。 若模板编码已存在则更新（在同一 template_code
   * 下创建新版本，旧版本降级为 is_latest=0）。
   *
   ...
   * <ul>
   *   <li>新版本号 = 当前最大版本号 + 1
   *   <li>旧版本统一降级为 {@code is_latest=0}
   *   <li>新版本 {@code is_latest=1}
   *   <li>{@code version_label} 由调用方指定（可空）
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>新模板 version=1, is_latest=1, inherit_type=CLONE, parent_template_id=源模板 id
   *   <li>新模板独立演进，后续修改互不影响
   *   <li>use_count 重置为 0
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>新模板 inherit_type=INHERIT, parent_template_id=父模板 id
   *   <li>可通过 {@link #listInheritedTemplates} 反查继承关系
   *   <li>新模板独立演进，但保留与父模板的关联，便于后续同步父模板更新
   * </ul>
   *
