...
public interface FlowEfficiencyService {
   ...
   *
   * <ul>
   *   <li><b>STUCK</b>：任务在同一节点停留超过阈值时间（默认 24 小时）
   *   <li><b>HIGH_REJECTION</b>：节点在最近 100 个任务中驳回率超过 50%
   *   <li><b>LONG_RUNNING</b>：流程实例运行时间超过阈值天数（默认 7 天）
   * </ul>
   *
   * @param tenantId 租户 ID
   * @param limit 返回条数上限
   ...
   * @return 长期运行实例列表，每行含 type=LONG_RUNNING / instanceId / flowCode / flowName / startAt /
   *     runningDays
   */
   ...
   *
   * <p>基于效率统计和异常检测的综合评分，对标钉钉/飞书审批的"健康度"看板。 评分维度：
   *
   * <ul>
   *   <li>超期率（30%）：overdueRate 越低越好，最高扣 30 分
   *   <li>代批率（20%）：proxyRate 过高说明审批人不在线，最高扣 20 分
   *   <li>平均耗时（20%）：avgDurationMs 越低越好，最高扣 20 分
   *   <li>异常数（30%）：卡单/高驳回/长期运行实例数，最高扣 30 分
   * </ul>
   *
   * <p>评级标准：
   *
   * <ul>
   *   <li>EXCELLENT（优秀）：≥ 90 分
   *   <li>GOOD（良好）：75-89 分
   *   <li>FAIR（一般）：60-74 分
   *   <li>POOR（较差）：< 60 分
   * </ul>
   ...
   */
  Map<String, Object> healthScore(String tenantId, String startTime, String endTime);
}
