 ...
 * <p>P0-4: 失败回调的最终一致性保证。
 *
 * <p>回调处理失败（如账号未映射、工作流分发异常等）时，日志落库为 FAIL 状态。 本服务负责：
 *
 * <ul>
 *   <li>扫描 FAIL 状态且 retry_count 未超阈值的日志
 *   <li>重新解析 callbackData、反查账号、派发审批动作
 *   <li>更新重试结果（SUCCESS / FAIL + retryCount++）
 *   <li>超过最大重试次数的进入死信（不再扫描，等运维介入）
 * </ul>
 *
 * <p>容错策略：
 *
 * <ul>
 *   <li>单条重试失败不影响其余，每条独立 try-catch
 *   <li>账号仍未映射 → 直接判 FAIL 但不抛异常（与首次处理一致）
 *   <li>工作流业务异常（SysException）→ 容错跳过，标记为 FAIL（避免死循环重试已结束的流程）
 *   <li>系统异常（数据库/网络） → 抛出，由调用方决定后续策略
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
