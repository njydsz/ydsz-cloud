 ...
 * 任务生成与状态回写原子提交。但在极端场景下（如事务提交前 JVM 崩溃、数据库主从切换导致
 * 部分提交），仍可能出现异常态：
 * <ul>
 *   <li>实例状态为 RUNNING 但无 PENDING 任务（任务生成成功但 updateStatus 失败回滚时）</li>
 *   <li>实例状态已更新到下一节点但对应节点无任务（updateStatus 成功但任务生成回滚时）</li>
 *   <li>实例处于 COMPLETED/TERMINATED 终态但仍有 PENDING 任务（完成逻辑异常）</li>
 * </ul>
 *
 * <p><b>待实现（TODO）：</b>
 * <ol>
 *   <li>扫描 {@code ydsz_flow_instance} 中 flow_status = RUNNING 且当前无任何非终态任务的实例，
 *       标记为异常并触发告警（Cat/Mail）；或尝试重新生成任务（需人工确认）</li>
 *   <li>扫描 {@code ydsz_flow_run_task} 中 task_status IN (PENDING, CLAIMED) 但关联的实例
 *       已处于 COMPLETED/TERMINATED/REJECTED 终态的任务，自动取消（CANCELLED）并记录审计日志</li>
 *   <li>输出对账报告：total / anomalyCount / autoFixedCount / needManualCount，
 *       供运维监控大盘展示</li>
 * </ol>
 *
...
public class FlowConsistencyJobHandler implements JobHandler {
     ...
     * <p>TODO: 实现对账逻辑：
     * <ol>
     *   <li>查询 RUNNING 实例列表，逐一检查是否存在 PENDING/CLAIMED 任务</li>
     *   <li>查询 PENDING/CLAIMED 任务列表，逐一检查实例是否已处于终态</li>
     *   <li>对异常态执行修复或告警</li>
     * </ol>
     *
