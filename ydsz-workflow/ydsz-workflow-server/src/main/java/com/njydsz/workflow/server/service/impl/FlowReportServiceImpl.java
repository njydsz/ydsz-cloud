 ...
 * 审批数据周报/月报服务实现
 *
 * <p>对 {@link FlowReportService} 接口的完整实现，承担工作流引擎的<b>数据报告</b>能力。 通过定时任务自动生成「审批数据周报 /
 * 月报」并推送给指定用户（通常是部门负责人 / 管理员）， 是大厂 B 端工作流「数据驱动管理」的标配。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li><b>周报生成（{@link #generateWeeklyReport}）</b>：每周一 9:00 自动生成本周审批数据报告 （发起量 / 通过量 / 驳回量 / 平均耗时 /
 *       TOP 申请人 / TOP 审批人）
 *   <li><b>月报生成（{@link #generateMonthlyReport}）</b>：每月 1 号 9:00 自动生成本月审批数据报告 （环比上月 / 同比去年 / 异常趋势 /
 *       部门对比）
 *   <li><b>报告推送（{@link #sendReport}）</b>：报告生成后通过 {@link FlowNotificationService} 推送给指定用户（站内信 / 邮件 /
 *       企业微信）
 *   <li><b>自定义报告（{@link #generateCustomReport}）</b>：支持业务方按维度自定义报告 （指定时间范围 / 流程类型 / 部门）
 *   <li><b>报告历史（{@link #listReportHistory}）</b>：报告历史记录可查询、可下载
 * </ul>
 *
 * <p><b>定时任务调度：</b>
 *
 * <ul>
 *   <li><b>周报</b>：{@code @Scheduled(cron = "0 0 9 ? * MON")} 每周一 9:00 触发
 *   <li><b>月报</b>：{@code @Scheduled(cron = "0 0 9 1 * ?")} 每月 1 号 9:00 触发
 *   <li>通过 {@link DistributedScheduled} 分布式锁保证集群中只有<b>一个节点</b>执行推送， 避免重复推送
 * </ul>
 *
 * <p><b>报告内容：</b>
 *
 * <ul>
 *   <li><b>核心指标</b>：发起量、通过量、驳回量、超时量、平均审批耗时
 *   <li><b>趋势分析</b>：环比上月 / 同比去年（按日 / 周 / 月维度）
 *   <li><b>TOP 排行</b>：TOP 10 申请人 / 审批人 / 流程类型 / 部门
 *   <li><b>异常告警</b>：超时率 / 驳回率超过阈值的流程 / 部门
 *   <li><b>效率分析</b>：各流程类型的平均耗时分布（P50 / P90 / P99）
 * </ul>
 *
 * <p><b>数据来源：</b>
 *
 * <ul>
 *   <li>{@code ydsz_flow_instance} — 流程实例表（活跃实例）
 *   <li>{@code ydsz_flow_his_instance} — 历史实例表（已完成 / 终止实例）
 *   <li>{@code ydsz_flow_his_task} — 历史任务表（审批操作轨迹）
 *   <li>{@code ydsz_flow_audit_log} — 审计日志表（操作审计）
 * </ul>
 *
 * <p><b>事务边界：</b>
 *
 * <ul>
 *   <li>报告生成涉及多次 SQL 查询，使用 {@code @Transactional(readOnly = true)} 支持只读副本路由
 *   <li>报告写入和通知推送由 {@code REQUIRES_NEW} 子事务隔离
 *   <li>单租户报告生成失败不影响其他租户
 * </ul>
 *
 * <p><b>设计要点：</b>
 *
 * <ul>
 *   <li><b>定时器 + 集群锁</b>：避免集群多节点重复推送，参考大厂分布式调度最佳实践
 *   <li><b>异步推送</b>：报告生成后通过 {@code @Async} 异步推送，避免阻塞定时任务
 *   <li><b>报告缓存</b>：报告内容缓存 24h，避免重复查询 DB
 *   <li><b>报告订阅</b>：用户可订阅 / 退订周报 / 月报， 订阅关系存储在 {@code ydsz_flow_report_subscription}
 *   <li><b>报告国际化</b>：支持中英文报告，根据用户 locale 切换
 * </ul>
 *
 * <p><b>典型使用：</b>
 *
...
public class FlowReportServiceImpl implements FlowReportService {
   ...
   * 定时推送周报：每周一 9:00
   *
   * <p>通过 {@link DistributedScheduled} 保证多节点部署时仅一个节点执行推送， 获取不到锁的节点直接跳过本次执行（非阻塞），避免重复发送。
   */
   ...
   * 生成本周审批数据报告
   *
   * <p>默认统计时间范围：<b>上周一 00:00 ~ 本周一 00:00</b>（即「上一完整自然周」）。 报告内容包含：核心指标（发起量 / 通过量 / 驳回量 / 超时量 /
   * 平均耗时）、 审批趋势（按天）、审批人效率 Top 5、流程效率对比。
   *
   * <p>数据来源：{@link FlowAnalyticsService}，内部已优化查询走只读副本路由。
   *
   * @param tenantId 租户 ID（默认 {@code "1"}）
   * @return 报告数据 Map（含 {@code reportType / period / overview / trend / topApprovers /
   *     flowComparison}）
   */
  @Override
   ...
   * 生成本月审批数据报告
   *
   * <p>默认统计时间范围：<b>上月 1 号 00:00 ~ 上月最后一天 23:59:59</b>（即「上一完整自然月」）。 报告内容与周报类似，但审批人效率 Top
   * 10、趋势按周聚合，更适合月度管理决策。
   *
   * @param tenantId 租户 ID（默认 {@code "1"}）
   * @return 报告数据 Map
   ...
   * 推送周报到管理员（手动触发）
   *
   * <p>由 {@link #scheduledWeeklyReport} 定时任务调用，也可由外部手动触发。 推送通道：{@link
   * FlowNotificationService#send}（默认「admin」用户， 实际应通过租户配置获取收件人列表）。
   *
   ...
   * 推送月报到管理员（手动触发）
   *
   * <p>由 {@link #scheduledMonthlyReport} 定时任务调用，也可由外部手动触发。 推送通道：{@link
   * FlowNotificationService#send}。
   *
   * @param tenantId 租户 ID
