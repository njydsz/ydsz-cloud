 ...
 *   <li>必填校验（REQUIRED）
 *   <li>数值范围校验（MIN/MAX）
 *   <li>长度校验（MIN_LENGTH/MAX_LENGTH）
 *   <li>正则校验（PATTERN）
 *   <li>多选数量校验（MIN_SELECTED/MAX_SELECTED）
 *   <li>附件数量/大小/类型校验（MIN_COUNT/MAX_COUNT/MAX_SIZE_MB/ACCEPT_TYPES）
 *   <li>子表单行数校验（MIN_ROWS/MAX_ROWS）+ 递归校验每行字段
 *   <li>字段联动规则求值（SHOW/HIDE 动态影响校验范围）
 * </ul>
 *
 * <p>使用方式：
 *
...
public class FlowFormValidator {
  ...
  private void validateField(
      ...
      List<FlowFormValidationError> errors,
      String pathPrefix) {
    ...
    // 必填校验
    if (required && isEmpty) {
      errors.add(new FlowFormValidationError(fieldKey, "REQUIRED", field.getLabel() + " 不能为空"));
      return;
    }
  ...
  private void validateSubForm(
    ...
    if (!(value instanceof List<?> rows)) {
      errors.add(
          new FlowFormValidationError(fieldKey, "TYPE_MISMATCH", field.getLabel() + " 格式不正确，应为数组"));
      return;
    }
    ...
    if (rowCount < minRows) {
      errors.add(
          new FlowFormValidationError(
              fieldKey, "MIN_ROWS", field.getLabel() + " 至少需要 " + minRows + " 行数据"));
    }
    if (maxRows > 0 && rowCount > maxRows) {
      errors.add(
          new FlowFormValidationError(
              fieldKey, "MAX_ROWS", field.getLabel() + " 最多允许 " + maxRows + " 行数据"));
    }
    ...
    for (int i = 0; i < rows.size(); i++) {
      ...
      for (FlowFormField subField : field.getSubFields()) {
        validateField(
            subField,
            rowData.get(subField.getFieldKey()),
            rowData,
            errors,
            fieldKey + "[" + i + "].");
      }
  ...
  private void validateNumberRange(
    ...
    if (numVal == null) {
      errors.add(
          new FlowFormValidationError(fieldKey, "TYPE_MISMATCH", field.getLabel() + " 不是有效的数字"));
      return;
    }
    if (validation.getMin() != null && numVal < validation.getMin()) {
      errors.add(
          new FlowFormValidationError(
              fieldKey, "MIN", field.getLabel() + " 不能小于 " + validation.getMin()));
    }
    if (validation.getMax() != null && numVal > validation.getMax()) {
      errors.add(
          new FlowFormValidationError(
              fieldKey, "MAX", field.getLabel() + " 不能大于 " + validation.getMax()));
    }
  }
  ...
  /** P3-2: 量词重叠模式（如 (a+)+、(a*)* ） */
  private static final Pattern OVERLAPPING_QUANTIFIER = Pattern.compile("\\([^)]*[+*][^)]*\\)[+*]");

  /** P3-2: 分支重叠模式（如 (a|a)* ） */
  private static final Pattern ALTERNATION_OVERLAP = Pattern.compile("\\([^)]*\\|[^)]*\\)[*+]");
  ...
  private void validatePattern(
    ...
    if (pattern == null || pattern.length() > MAX_PATTERN_LENGTH) {
      log.warn("[FormValidator] 正则表达式为空或超长（>{}字符），跳过校验: field={}", MAX_PATTERN_LENGTH, fieldKey);
      return;
    }
    if (DANGEROUS_PATTERN.matcher(pattern).find()
      ...
      log.warn("[FormValidator] 检测到危险嵌套量词正则，跳过校验: field={} pattern={}", fieldKey, pattern);
      return;
    }
    try {
      if (!strVal.matches(pattern)) {
        String msg =
            StringUtils.hasText(validation.getPatternMessage())
      ...
      log.warn(
          "[FormValidator] 正则表达式无效: field={} pattern={} err={}", fieldKey, pattern, e.getMessage());
    }
  }

  private void validateCheckboxCount(
      FlowFormField field,
      Object value,
      FlowFormField.ValidationRule validation,
      List<FlowFormValidationError> errors,
    ...
    if (validation.getMaxSelected() != null
        && validation.getMaxSelected() > 0
  ...
  private void validateAttachment(
    ...
    if (validation.getMaxCount() != null
        && validation.getMaxCount() > 0
    ...
    if (validation.getAcceptTypes() != null
        && !validation.getAcceptTypes().isEmpty()
    ...
    if (validation.getMaxSizeMb() != null
        && validation.getMaxSizeMb() > 0
        && value instanceof List<?> list) {
      for (int i = 0; i < list.size(); i++) {
        Object item = list.get(i);
        if (item instanceof Map<?, ?> map) {
  ...
  private boolean isHiddenByLinkage(FlowFormField field, Map<String, Object> allData) {
    ...
    for (FlowFormField.LinkageRule rule : field.getLinkages()) {
      ...
      boolean conditionMet =
          evaluateCondition(rule.getOperator(), triggerVal, rule.getTriggerValue());
      if (conditionMet) {
  ...
  public FlowFormSchema parseSchema(String json) {
    ...
    try {
      return YdszJson.fromJson(json, FlowFormSchema.class);
    } catch (Exception e) {
