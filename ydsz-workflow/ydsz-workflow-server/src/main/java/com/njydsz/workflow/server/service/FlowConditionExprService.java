...
public interface FlowConditionExprService {
   ...
   *
   * <p>仅支持由 {@link #buildExpression} 生成的简单表达式， 不支持嵌套括号或复杂函数调用。
   *
   ...
   *
   * <p>P1-4: 从流程定义的所有节点表单中提取变量，包括表单字段、 系统内置变量（如 initiatorId, currentTime 等）。
   *
   ...
   *
   * <p>P1-4: 给定表达式和示例变量，返回表达式执行结果（true/false）， 用于前端实时预览条件匹配效果。仅支持 Aviator 引擎。
   *
   * @param expression 表达式字符串
   ...
   */
  Map<String, Object> previewExpression(
      String expression, Map<String, Object> variables, String engine);

  /**
   * 获取条件模板列表。
   *
   * <p>P1-4: 提供常用条件模板（如金额判断、部门判断、时间判断等）， 用户选择模板后可快速编辑自定义条件。
   *
