 ...
 *
 * <p>当某个节点执行流程定义发布/停用/迁移/编辑等操作时，通过 Redis Pub/Sub 广播 缓存失效消息到集群所有节点，各节点收到后清除本地缓存。
 *
 * <p>消息格式：{@code definitionId|sourceNodeId}
 *
 * <ul>
 *   <li>{@code definitionId} — 要失效的流程定义 ID
...
public class FlowDefinitionCacheBroadcaster {
  ...
  private static final Logger log = LoggerFactory.getLogger(FlowDefinitionCacheBroadcaster.class);
