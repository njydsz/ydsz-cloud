 ...
 *       统一实现完整闭环（NOTIFY/ESCALATE/AUTO_PASS/AUTO_REJECT），包括提醒计数、间隔控制、 最终动作分发。消除原先两套实现不一致的问题（旧
 *       markTimeout 会标记 TIMEOUT 终态导致流程卡死）。
 *   <li><b>子流程超时检测</b> — 扫描 ydsz_flow_instance 中 due_at 已超期且状态为 RUNNING 的子流程实例，
 *       自动终止子流程并同步终止父流程。此项为本 Handler 独有职责，不委托。
 * </ul>
 *
 * <p>容错策略：每个任务/实例独立 try-catch，单个处理失败不影响其余。
 *
 * <p>Bean 名称 = {@code flowTimeoutJobHandler}， 可在 ydsz_job 表配置：handler=flowTimeoutJobHandler,
 * cron="0 0/5 * * * ?"（每 5 分钟扫描一次）。
 *
 * @since 1.0.0
 * @author ydsz-team
 */
...
public class FlowTimeoutJobHandler implements JobHandler {
   ...
   *
   * <p>FlowSlaServiceImpl 已实现完整的 SLA 闭环（NOTIFY/ESCALATE/AUTO_PASS/AUTO_REJECT）， 此处委托可避免两套实现不一致（如旧
   * markTimeout 会标记 TIMEOUT 终态导致流程卡死）。 FlowTimeoutJobHandler 仅保留"子流程超时检测"独有职责。
   */
  ...
  public Object execute(String paramsJson) throws Exception {
    ...
    if (overdueTasks != null && !overdueTasks.isEmpty()) {
      for (FlowRunTask task : overdueTasks) {
        try {
          ...
          errors++;
          log.error(
              "[FlowTimeout] 处理超期任务异常 taskId={} instanceId={} err={}",
              task.getId(),
              task.getInstanceId(),
              e.getMessage(),
              e);
    ...
    log.info(
        "[FlowTimeout] 扫描完成: processed={} subProcessTimeouts={} costMs={}",
        processed,
        subProcessProcessed,
   ...
   *
   * <p>P1-3 闭环重构：原先在 JobHandler 中重复实现 SLA 动作分发（doYdsznd/doEscalate/
   ...
   *
   * <ul>
   *   <li>提醒计数与间隔控制（urgeCount / lastUrgedAt / maxUrges）
   *   <li>最终动作分发（NOTIFY 保持任务活跃 / ESCALATE 转办 / AUTO_PASS 推进 / AUTO_REJECT 终止）
   *   <li>REQUIRES_NEW 子事务隔离，单条失败不影响主循环
   * </ul>
   *
   * @param task 超期任务
   */
  private void handleOverdueTask(FlowRunTask task) {
    ...
    if (processed) {
      log.debug(
          "[FlowTimeout] SLA 处理完成: taskId={} instanceId={}", task.getId(), task.getInstanceId());
    }
  }
   ...
   *
   * <p>对于超时子流程，自动终止子流程实例并取消其下活跃任务， 同时回调父流程推进（通过 onSubProcessTerminated 终止父流程）。
   *
  ...
  private Map<String, Object> handleSubProcessTimeout(String tenantId) {
    ...
    for (FlowInstance instance : overdueInstances) {
      try {
        ...
        errors++;
        log.error(
            "[FlowTimeout] 处理超期子流程异常 instanceId={} err={}", instance.getId(), e.getMessage(), e);
      }
    }
  ...
  private void handleOverdueSubProcess(FlowInstance instance) {
    ...
    if (latest == null || !FlowInstanceStatus.RUNNING.name().equals(latest.getFlowStatus())) {
      log.info(
          ...
          instanceId,
          latest == null ? "null" : latest.getFlowStatus());
      return;
    }
    // 终止子流程实例
    long instanceDurationMs =
        latest.getStartAt() == null ? 0L : Duration.between(latest.getStartAt(), now).toMillis();
    instanceMapper.updateStatus(
        instanceId, FlowInstanceStatus.TERMINATED.name(), null, null, now, instanceDurationMs);
    // 取消子流程下所有活跃任务
    taskMapper.cancelByInstance(instanceId, FlowTaskStatus.CANCELLED.name());
    log.info(
        "[FlowTimeout] 子流程超时，已终止子流程实例: instanceId={} flowCode={} dueAt={}",
        instanceId,
        latest.getFlowCode(),
        latest.getDueAt());
    // 清除 dueAt 标记
      ...
      if (parent != null && FlowInstanceStatus.RUNNING.name().equals(parent.getFlowStatus())) {
        long parentDurationMs =
            parent.getStartAt() == null
                ? 0L
                : Duration.between(parent.getStartAt(), now).toMillis();
        instanceMapper.updateStatus(
            parentId, FlowInstanceStatus.TERMINATED.name(), null, null, now, parentDurationMs);
        taskMapper.cancelByInstance(parentId, FlowTaskStatus.CANCELLED.name());
        log.info("[FlowTimeout] 子流程超时触发父流程终止: parentId={} childId={}", parentId, instanceId);
      }
    }
  }
