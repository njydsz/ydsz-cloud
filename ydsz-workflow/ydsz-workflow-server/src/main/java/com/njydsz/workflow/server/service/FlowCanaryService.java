...
public interface FlowCanaryService {
   ...
   *
   * <p>要求定义已发布（isPublish=1）且尚未处于灰度状态。灰度开始时， 当前稳定版仍可继续接收流量；按 initialPercent 切流到灰度版。
   *
   ...
   * 回滚：灰度版失效
   *
   * <p>将灰度版 isPublish=9，canaryStatus=ROLLED_BACK，canaryPercent=0。 后续所有请求都走稳定版。
   *
   * @param definitionId 灰度版定义 ID
   * @param operatorId 操作人 ID
   * @param operatorName 操作人姓名
   ...
   * 解析流程启动时实际生效的版本
   *
   * <p>核心切流逻辑：根据稳定版 + 灰度版 + canaryPercent + canaryStrategy + 发起人ID， 决定本次启动应该使用哪个版本的定义。
   *
   * @param flowCode 流程编码
   ...
   */
  FlowDefinition resolveEffectiveDefinition(
