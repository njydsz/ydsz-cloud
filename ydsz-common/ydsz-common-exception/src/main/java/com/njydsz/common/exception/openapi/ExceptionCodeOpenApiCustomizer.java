 ...
 *
 * <p>扫描项目中所有已注册的异常错误码（{@link ErrorCodeTable}），
 * 为每个 OpenAPI 操作（operation）的响应自动追加当前接口可能返回的错误码说明，
 * 实现"错误码即文档"的单一信息源。
 *
 * <p><b>启用条件：</b>classpath 中存在 springdoc-openapi 时由
 * {@link YdszExceptionOpenApiAutoConfiguration} 自动装配；否则本 Bean 不会生效。
 *
...
public class ExceptionCodeOpenApiCustomizer implements OpenApiCustomizer {
    ...
    public ExceptionCodeOpenApiCustomizer(ErrorCodeTable errorCodeTable, MessageSource messageSource) {
    ...
    public void customise(io.swagger.v3.oas.models.OpenAPI openApi) {
        ...
        log.info("[ExceptionCodeOpenApiCustomizer] 已为 OpenAPI 文档注入 {} 个 HTTP 状态的错误码响应模型",
                httpStatuses.size());
    }
    ...
    private String buildStatusDescription(Integer status) {
        ...
        errorCodeTable.allCodes().values().stream()
                ...
                .forEach(code -> sb.append("- `").append(code.getCode()).append("`: ")
