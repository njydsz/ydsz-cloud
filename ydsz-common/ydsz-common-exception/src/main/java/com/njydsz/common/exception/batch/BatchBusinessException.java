...
public class BatchBusinessException extends BusinessException {
   ...
   *
   * <p>每次调用都基于当前 counts 重新生成消息， 确保无论何时获取消息都能反映最新的批量处理结果。
   *
   * <p>使用 {@link MessageFormat} 处理 i18n 模板中的 {@code {0}} / {@code {1}} 占位符， 与 Spring {@link
   * org.springframework.context.MessageSource} 的消息格式保持一致。
   *
   * @return 处理结果
   */
  @Override
  public String getMessage() {
    String template =
        MessageSourceHolder.resolve(
            BATCH_PARTIAL_SUCCESS_KEY, new Object[] {getSuccessCount(), getFailureCount()});
    // 如果 i18n 未配置（返回 key 本身），使用默认文案
    if (BATCH_PARTIAL_SUCCESS_KEY.equals(template)) {
      return String.format(
          "Batch operation partially successful: %d succeeded, %d failed",
   ...
   * 按错误码聚合统计失败子项。
   *
   * <p>便于前端/调用方快速了解"哪种错误最多"，无需遍历全量 failureItems。 返回的 map 保持错误码首次出现的顺序（LinkedHashMap），值为该错误码的出现次数。
   *
   ...
   *
   * <p>与 {@link #getFailureAggregation()} 不同，此方法返回每个错误码对应的子项 ID 列表， 供调用方精确定位哪些子项触发了同类错误。
   *
