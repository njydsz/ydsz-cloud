 ...
 * }</pre>
 *
 * <p><b>精简设计（26.09.01）：</b>仅保留 3 个核心构造函数， 其他参数化构造通过 {@link #builder()} 链式 Builder 完成，消除 20+ 构造函数爆炸问题。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
...
public class SysException extends AbstractYdszException {
   ...
   *
   * <p>会触发国际化消息的懒加载解析，应在请求线程内调用以保证取到正确的 Locale。 系统异常默认 HTTP 状态码为 500，属于基础设施故障， 对外暴露时应避免把 {@code
   * cause} 的原始堆栈信息带给客户端。
   *
  ...
  public static class SysExceptionBuilder extends YdszExceptionBuilder<SysException> {
     ...
     *
     * <p>ResultCode 不包含 HTTP 状态码，使用 {@link #DEFAULT_HTTP_STATUS} 作为默认。 如需精确 HTTP 语义，请使用 {@link
     ...
     *
     * <p>覆盖基类的 {@code params(Object[])} 以提供变长参数调用方式， 与 {@link
     * com.njydsz.common.exception.custom.BusinessException.BusinessExceptionBuilder#params(Object...)}
     * 对齐。
     *
     * @param params 消息参数
     * @return 处理结果
    ...
    @Override
    protected SysException doBuild(
        String code,
        String key,
        Object[] params,
        int httpStatus,
