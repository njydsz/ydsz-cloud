 ...
 * 异常响应信息封装类
 *
 * <p>用于统一异常响应数据的结构化表示，包含异常的完整上下文信息。 该类会被序列化为 JSON 返回给客户端或记录到日志中。
 *
 * <p><b>主要字段：</b>
 *
 * <ul>
 *   <li>code：业务错误码，如 "A01001"
 *   <li>key：国际化消息键，如 "user.not.found"
 *   <li>message：已解析的本地化消息
 *   <li>details：错误详情，结构化键值对
 *   <li>timestamp：异常发生时间
 *   <li>path：发生异常的请求路径
 *   <li>traceId：分布式追踪 ID
 * </ul>
...
public class ExceptionInfo implements Serializable {
   ...
   * 错误详情
   *
   * <p>结构化键值对，序列化为 JSON 时按 {@code @JsonInclude(NON_NULL)} 语义输出。 注意：{@code transient} 标记会误导 Jackson
   * 序列化行为（默认忽略 transient 标记）， 故此处不使用 transient，序列化控制交由 JSON 配置层统一处理。
   */
  ...
  public static ExceptionInfo of(ExceptionCode exceptionCode) {
    return new ExceptionInfo(exceptionCode.getCode(), exceptionCode.getKey(), null);
  }
  ...
  public static class Builder {
     ...
     * 整体替换错误详情集合。
     *
     * <p>直接持有传入引用而非拷贝，调用方在 {@code build()} 之后不应再修改该 Map， 否则会影响已构建的 {@link ExceptionInfo}。需要逐项累积时请改用
     ...
     *
     * <p>底层使用 {@link LinkedHashMap}，保证详情按追加顺序序列化， 前端表单可据此按字段顺序展示校验错误。相同 key 重复追加时后者覆盖前者。
     *
     ...
     * 覆盖异常发生时间。
     *
     * <p>不设置时由 {@link ExceptionInfo} 构造器取当前时间； 仅在补录历史异常或需要与上游时间戳对齐时才显式指定。
     *
