 ...
 *
 * <p>替代静态 {@link MessageSourceHolder} 的可注入方案，适用于需要通过 Spring DI
 * 获取 i18n 消息解析能力的场景（如 Service 层、工具类）。
 *
 * <p>与普通 {@link MessageSource} 相比，额外提供：
 * <ul>
 *   <li>自动取当前请求线程 Locale（{@link LocaleContextHolder}）的便捷方法</li>
 *   <li>无 MessageSource 时的安全兜底（返回 key 本身，不抛异常）</li>
 * </ul>
 *
 * <p><b>使用示例：</b>
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><b>兼容说明：</b>静态 {@link MessageSourceHolder} 仍然可用，
 * {@link com.njydsz.common.exception.config.YdszExceptionCoreAutoConfiguration}
 * 会在启动时将同一个 {@link MessageSource} 注入到本 Bean 和 {@link MessageSourceHolder}，
 * 两者行为一致。
 *
 * @author ydsz-team
 * @since 2.4.0
