 ...
 *
 * <ul>
 *   <li>类路径存在 {@link org.springframework.dao.DataAccessException}
 *   <li>应用类型为 {@link
...
public class JdbcExceptionHandler extends BaseExceptionHandler {
   ...
   * 处理数据完整性违反异常（细粒度分类）
   *
   * <p>通过 SQLState 精确匹配分类，返回 HTTP 409 Conflict。 SQLState 23xxx 对应 Integrity Constraint Violation：
   *
   * <ul>
   *   <li>23505 / 23000：唯一约束冲突
   *   <li>23503：外键约束冲突
   *   <li>23502：非空约束违反
   *   <li>23514 / 23001：检查约束违反
   * </ul>
   *
  ...
  public YdszResponse<?> handleDataAccessException(
    ...
    recordMetrics(e);
    log.error(
        "{}数据访问异常 | 路径: {} | 消息: {}", getLogPrefix(), request.getRequestURI(), e.getMessage(), e);

    // 通过 MessageSource 按当前请求 Locale 解析 i18n 文案，避免向客户端暴露裸 key
    String message =
        resolveMessage(
            CoreExceptionCode.DATABASE_ERROR.getKey(), null, DEFAULT_DATABASE_ERROR_MESSAGE);
    ...
    return errorResponse(
        CoreExceptionCode.DATABASE_ERROR.getCode(), message, includeExceptionInfo() ? info : null);
  }
