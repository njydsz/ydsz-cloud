 ...
 *
 * <p><b>性能优化（v2.5）：</b>首次成功后缓存 {@link Method} 对象，
 * 后续调用直接使用缓存的 Method.invoke，消除重复的 getMethod 查找开销。
...
public class OtelTraceInfoExtractor {
    ...
    @Nullable
    private static volatile Method currentMethod;

    /** 缓存的 getSpanContext() 方法 */
    @Nullable
    private static volatile Method getSpanContextMethod;
    ...
    /** 缓存的 isValid() 方法 */
    @Nullable
    private static volatile Method isValidMethod;

    /** 缓存的 isSampled() 方法 */
    @Nullable
    private static volatile Method isSampledMethod;
    ...
    public static OtelTraceInfo currentTraceInfo() {
        ...
        try {
            ...
            String traceId = getTraceIdMethod != null ? (String) getTraceIdMethod.invoke(spanContext) : null;
