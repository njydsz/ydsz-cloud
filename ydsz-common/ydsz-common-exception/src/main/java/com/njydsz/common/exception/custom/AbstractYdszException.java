 ...
 * 异常抽象基类
 *
 * <p>封装所有异常的公共字段和逻辑，消除子类代码重复。 子类只需通过构造函数传入各自的默认值（如错误码、HTTP 状态码、级别、分类）即可。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li><b>公共字段管理</b>：code / key / params / httpStatus
 *   <li><b>国际化消息解析</b>：通过 {@link MessageSourceHolder} 按请求线程 Locale 解析 i18n 文案
 *   <li><b>异常元数据</b>：level / category / snapshot / extData / timestamp 委托给 {@link ExceptionContext}
 * </ul>
 *
 * <p><b>国际化消息解析：</b>通过 {@link MessageSourceHolder} 静态持有者获取 Spring MessageSource， 避免异常类对 Spring
 * 上下文的硬依赖。{@code getMessage()} 直接按当前请求 Locale 解析 i18n 文案， {@link
...
public abstract class AbstractYdszException extends RuntimeException
    implements IExceptionResultCode {
   ...
   *
   * <p>内部调用 {@link #getMessage()}，会触发国际化文案的懒加载解析。 若 {@link MessageSourceHolder} 已注入 Spring
   * MessageSource，则按当前请求 Locale 解析； 若未注入，则返回 messageKey 本身。
   *
   * <p>投影字段包括：code / key / message / httpStatus / timestamp， 以及 {@link #getSnapshot()}（如有，透写入
   * details 供排查定位）。 {@code traceId} 与 {@code details}（非 snapshot）由子类或调用方补充。
   *
   ...
   *
   * <p>目的是让 {@code params} 字段保持非 null 不变量， 使 i18n 格式化、参数克隆与序列化路径无需重复做空判断。
   *
   ...
   *
   * <ul>
   *   <li>{@link ResultCode#getCode()} → {@code code} 字段
   *   <li>{@link ResultCode#getMsg()} → {@link #getMessage()}（含 i18n 解析）
   *   <li>{@code getKey()} → {@code key} 字段（i18n 消息键，用于二次解析）
   *   <li>{@link ResultCode#getHttpStatus()} → {@code httpStatus} 字段（0 视为 500）
   * </ul>
   *
   * <p>若 {@code code} 与 {@code key} 均为 {@code null}（异常未被初始化）， 视为不参与桥接，返回 {@code null}，由调用方走 UNKNOWN
   * 兜底逻辑。
   *
   ...
   *
   * <p>设置后 {@link #getMessage()} 直接返回此值，优先级最高。 传 {@code null} 清除覆盖，恢复走 i18n 解析逻辑。
   *
   ...
   * 向上下文快照追加单个键值对（链式调用）。
   *
   * <p>惰性初始化内部 {@link java.util.LinkedHashMap}，首次调用时创建快照容器。 适合在 throw 前逐条追加关键业务信息：
   *
   * <pre>{@code
   ...
   * 向上下文快照追加多个条目（链式调用）。
   *
   * <p>等价于多次调用 {@link #snapshot(String, Object)}， 适合批量传入已有 Map：
   *
   * <pre>{@code
