 ...
 *
 * <ul>
 *   <li>错误码文档端点：{@code /actuator/exception-codes}
 *   <li>健康检查指示器：{@code exception}
 * </ul>
 *
 * <p>仅在 Spring Boot Actuator 存在于类路径时激活，无 Actuator 依赖时自动跳过。 错误码扫描注册（{@code
 * ExceptionCodeScanner}）由核心装配 {@link YdszExceptionCoreAutoConfiguration} 负责，与 Actuator 解耦，保证无
...
public class YdszExceptionActuatorAutoConfiguration {
   ...
   * 创建错误码文档端点 Bean
   *
   * <p>暴露 {@code /actuator/exception-codes} 端点，输出全量异常码与文档说明， 便于前端/测试同学检索错误码、运维同学做异常字典管理。
   *
  ...
  @ConditionalOnProperty(
      ...
      name = "doc-endpoint-enabled",
      havingValue = "true",
      matchIfMissing = true)
  public ExceptionCodeDocEndpoint exceptionCodeDocEndpoint(
  ...
  public ExceptionHealthIndicator exceptionHealthIndicator(
    ...
    return new ExceptionHealthIndicator(properties, metricsProvider, errorCodeTableProvider);
  }
}
