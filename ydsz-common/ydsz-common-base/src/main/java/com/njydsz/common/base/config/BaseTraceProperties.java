 ...
 *
 * <p>子类通过 {@code @ConfigurationProperties} 的 prefix 属性指定具体前缀。 提供链路追踪、请求日志、采样率、响应头等通用配置项。
 *
 * <p><b>配置示例：</b>
 *
 * <pre>{@code
...
public abstract class BaseTraceProperties {
   ...
   * 是否在响应头中输出追踪/请求 ID
   *
   * <p>启用后会在每个 HTTP 响应头中添加 X-Request-Id。 默认 true，便于客户端关联日志。
   */
   ...
   * 是否启用请求日志记录
   *
   * <p>控制 BaseRequestLogInterceptor 是否记录请求/响应日志。 默认 true，生产环境可根据需要关闭以减少日志量。
   */
   ...
   * 请求日志格式
   *
   * <p>支持 "simple" 或 "detailed"。 默认为 "detailed"，包含 IP、UA、请求参数等详细信息。
   */
   ...
   * 是否记录请求参数
   *
   * <p>默认 true，会输出 queryString 和 form 参数。 注意：敏感参数（如密码、Token）需自行脱敏。
   */
   ...
   * 慢请求阈值（毫秒）
   *
   * <p>请求耗时超过此阈值时，日志级别升级为 WARN，便于性能监控。 默认 3000ms（3 秒）。
   */
