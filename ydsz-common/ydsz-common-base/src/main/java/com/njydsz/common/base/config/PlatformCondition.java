 ...
 * 平台模式条件判断。
 *
 * <p>根据配置属性 {@code ydsz.platform.mode} 或 classpath 自动探测， 判断当前是否匹配指定的平台模式。
 *
 * <p>自动探测规则：
 *
 * <ul>
 *   <li>classpath 中存在 {@code com.njydsz.common.web.config.WebMvcConfiguration} → WEB
 *   <li>classpath 中存在 {@code com.njydsz.common.app.config.AppMvcConfiguration} → APP
 *   <li>两者都存在 → 抛出异常（需显式配置）
 * </ul>
 *
 * @author ydsz-team
    ...
    if (modeProperty != null && !modeProperty.isBlank()) {
      try {
        ...
        throw new IllegalStateException(
            "无效的平台模式配置: ydsz.platform.mode="
                + modeProperty
                + "，可选值: "
                + Arrays.toString(PlatformMode.values()));
      }
    }
