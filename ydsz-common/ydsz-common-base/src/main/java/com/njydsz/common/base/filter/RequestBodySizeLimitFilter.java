 ...
 * 超过配置的阈值时直接返回 413 错误，避免大请求占用过多内存。
 *
 * <p>注意：此过滤器仅基于 Content-Length header 进行预检查，
 * 对于 chunked 编码的请求需要在 ContentCachingFilter 中二次校验。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public class RequestBodySizeLimitFilter extends OncePerRequestFilter {
    ...
    private void loggerReject(HttpServletRequest request, long contentLength) {
        if (log.isDebugEnabled()) {
            log.debug("请求体过大被拒绝 | uri={} | contentLength={} | maxBodySize={}",
                    request.getRequestURI(), contentLength, maxBodySize);
        }
    }
    ...
    private void rejectRequest(HttpServletResponse response) throws IOException {
        ...
        BaseResponse<?> body = BaseResponse.error("REQUEST_BODY_TOO_LARGE",
                "请求体过大，最大允许 " + (maxBodySize / 1024 / 1024) + "MB");
