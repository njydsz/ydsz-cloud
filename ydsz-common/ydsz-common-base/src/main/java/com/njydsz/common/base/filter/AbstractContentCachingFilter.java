 ...
 * 请求体缓存过滤器抽象基类
 *
 * <p>包装请求为 {@link ContentCachingRequestWrapper}，支持请求体多次读取。 跳过 multipart 请求，避免大文件上传场景下的内存问题。
 *
 * <p>默认缓存容量 512KB，超过部分不缓存（不会 OOM）。可由子类构造器自定义。
 *
...
public abstract class AbstractContentCachingFilter extends OncePerRequestFilter {
   ...
   * 默认缓存容量（字节，512KB）。
   *
   * <p>说明：ContentCachingRequestWrapper 在超过 cacheCapacity 时不会抛错， 只是截断缓存。512KB 适合典型 JSON
   * 请求体；如需支持更大请求， 可由子类传入更大的 cacheCapacity。
   */
  ...
  protected void doFilterInternal(
    ...
    if (isMultipart(request)) {
      ...
      ContentCachingRequestWrapper wrappedRequest =
          new ContentCachingRequestWrapper(request, getCacheCapacity());
