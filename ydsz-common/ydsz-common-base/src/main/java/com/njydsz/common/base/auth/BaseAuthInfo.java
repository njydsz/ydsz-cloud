 ...
 *
 * <p>定义认证上下文的统一抽象，子类覆盖 {@link #getServiceTypeCode()} 返回具体的服务类型编码 （例如 "WEB" 或 "APP"），用于业务层区分请求来源。
 *
 * <p>本类继承自 {@link YdszAuthInfo}，具备完整的认证信息能力，包括：
 *
 * <ul>
 *   <li>用户ID、登录账号、姓名等基础信息
 *   <li>租户ID、公司ID、部门ID、项目ID、区域ID 等多维度隔离信息
 *   <li>数据权限范围、可见列、可编辑列 等权限相关字段
 *   <li>Token、刷新Token、过期时间等会话信息
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
