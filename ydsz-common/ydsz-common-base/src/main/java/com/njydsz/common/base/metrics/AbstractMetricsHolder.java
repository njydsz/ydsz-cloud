 ...
 *
 * <p>为不方便注入 Spring Bean 的核心引擎路径提供静态注册与累加能力。 通过 {@link #bindTo(MeterRegistry)} 在应用启动时将 Spring 容器注入的
 * MeterRegistry 绑定到持有者， 单元测试中注入 {@code SimpleMeterRegistry} 即可验证计数器和计时器行为。
 *
 * <p>子类放置模块指标前缀（如 {@code "cronjob."}）和业务语义方法， 调用 {@link #registerCounter} / {@link #registerTimer}
 * / {@link #recordDuration} 系列方法， 无需关心注册表绑定与缓存去重。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
...
public abstract class AbstractMetricsHolder {
   ...
   * 工具构造器（禁止外部实例化）。
   *
   * <p>因父类构造器抛出异常，子类无需额外声明私有构造器， 但建议在子类显式声明以保持代码可读性和反射安全。
   */
