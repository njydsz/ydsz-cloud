 ...
 *
 * <p>基于 OpenAPI JSON 规范文档，导出为 HTML、Markdown、YAML、JSON 四种格式。 相比 {@link DefaultDocExporter}，本实现的
 * Markdown 导出器会解析 {@code paths} / {@code components} 等节点，生成包含接口路径、方法、参数、 请求体、响应码以及数据模型等结构化信息的
 * Markdown 文档。
 *
 * <p><b>激活条件：</b>
 *
 * <ul>
 *   <li>{@code ydsz.doc.export.enabled=true}（默认 true）
 *   <li>{@code ydsz.doc.exporter=markdown}（显式指定）
 * </ul>
 *
...
public class MarkdownDocExporter extends AbstractDocExporter {
  ...
  private String generateHtml(String apiDocs) {
    ...
    html.append("<p><strong>描述:</strong> ")
        .append(escapeHtml(docInfo.description()))
        .append("</p>");
  ...
  /** 将单个接口信息追加到 Markdown */
  private void appendEndpointMd(
      StringBuilder md, String method, String path, Map<String, Object> operation) {
    ...
    if (parameters != null && !parameters.isEmpty()) {
      ...
      for (Map<String, Object> param : parameters) {
        ...
        String type =
            schema != null && schema.containsKey("type")
                ? asString(schema.get("type"), "string")
                : "string";
   ...
   */
  private void appendSchemaMd(
      StringBuilder md, String name, Map<String, Object> schema, int depth) {
    if (depth > 3) {
      return;
    }
    ...
    if (properties != null && !properties.isEmpty()) {
      ...
      for (Map.Entry<String, Object> propEntry : properties.entrySet()) {
        ...
        String type =
            propSchema != null && propSchema.containsKey("type")
                ? asString(propSchema.get("type"), "object")
                : "object";
        boolean isRequired = required != null && required.contains(propName);
        String propDesc = propSchema != null ? asString(propSchema.get("description"), "") : "";
        md.append("| ")
            .append(propName)
            .append(" | ")
            .append(type)
            .append(" | ")
            .append(isRequired ? "是" : "否")
            .append(" | ")
            .append(propDesc)
            .append(" |\n");
      }
