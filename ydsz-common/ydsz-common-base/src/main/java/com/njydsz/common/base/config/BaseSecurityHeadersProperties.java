 ...
 * Base 模块安全响应头配置属性
 *
 * <p>用于配置 HTTP 响应的安全相关头部，防止常见 Web 安全威胁。 仅在业务方直接使用 base 模块（未引入 web/app/safe 模块）时生效。
 *
 * <p><b>配置示例：</b>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><b>与 safe 模块的关系：</b> 当项目中同时存在 safe/web/app 模块时，本配置不会被使用， 安全响应头由 safe 模块的 {@code
 * SecurityHeaderProperties}（前缀 {@code ydsz.safe.security-headers}）统一管理。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class BaseSecurityHeadersProperties {
   ...
   * XSS 防护头部
   *
   * <p>启用浏览器的 XSS 过滤器。 推荐值："1; mode=block" - 发现 XSS 时阻止页面渲染
   */
