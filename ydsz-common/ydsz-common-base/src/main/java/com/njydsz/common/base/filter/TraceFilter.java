 ...
 *
 * <ul>
 *   <li>生成或提取 traceId（生成复用 {@link TraceIdGenerator}，线程本地高性能实现）
 *   <li>将 traceId 注入 MDC，供日志框架使用
 *   <li>将 traceId 存入 RequestContext
 *   <li>在响应头中返回 traceId
 * </ul>
 *
 * <p><b>安全校验：</b> 对请求头中的 traceId 进行长度限制（最大 64 字符）和字符集校验 （仅允许 {@code [a-zA-Z0-9_-]}），防止日志注入和日志膨胀。
 *
 * <p><b>MDC 清理策略：</b> 本 Filter 不在 finally 中清理 MDC，统一由 {@link RequestContextCleanupFilter}
 * （LOWEST_PRECEDENCE，最外层 Filter）在请求结束时调用 {@code MDC.clear()} 清理， 确保后续 Filter 的后处理日志仍能使用 traceId。
 *
...
public class TraceFilter extends OncePerRequestFilter {
   ...
   * </ul>
   *
   * 校验失败时调用 {@link TraceIdGenerator#generateTraceId()} 重新生成 （ThreadLocalRandom 实现，性能优于
   * UUID），防止日志注入和日志膨胀。
   *
