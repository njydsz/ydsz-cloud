 ...
 * 默认文档导出器实现
 *
 * <p>基于 OpenAPI JSON 规范文档，支持导出为 HTML、Markdown、YAML、JSON 四种格式。 HTML 模板采用内联 CSS，Markdown 导出为简单格式（原始
 * JSON）。 如需结构化 Markdown 导出，请使用 {@link MarkdownDocExporter}。
 *
 * <p><b>激活条件：</b>同时满足以下两个条件时本 Bean 才会被注册：
 *
 * <ul>
 *   <li>{@code ydsz.doc.export.enabled=true}（默认 true）
 *   <li>{@code ydsz.doc.exporter=default}（默认 default）
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
