 ...
 * 配置注册端点 — 暴露运行时配置信息。
 *
 * <p>通过 Actuator 端点暴露所有 ydsz.* 前缀的配置项， 便于运维人员在线排查配置问题。
 *
 * <h3>端点信息</h3>
 *
 * <ul>
 *   <li>{@code GET /actuator/config-registry} — 查看所有 ydsz.* 配置
 *   <li>{@code GET /actuator/config-registry/{prefix}} — 查看指定前缀下的配置
 * </ul>
 *
 * <h3>安全说明</h3>
 *
 * <ul>
 *   <li>敏感属性（password/secret/token/key/cert/private）自动脱敏为 {@code ******}
 *   <li>生产环境应通过 {@code management.endpoint.config-registry.exposure} 控制访问权限
 *   <li>建议仅限内网访问
 * </ul>
 *
 * @author ydsz-team
...
public class ConfigRegistryEndpoint {
  ...
  private static final Set<String> SENSITIVE_KEYS =
      Set.of("password", "secret", "token", "key", "cert", "private", "credential", "auth");
