 ...
 *
 * <ul>
 *   <li>Spring 依赖注入（{@link org.springframework.beans.factory.annotation.Autowired}）
 *   <li>程序化注册（向后兼容 {@link MessageResolverHolder}）
 *   <li>默认降级解析器（未注入时使用 {@link MessageResolverHolder} 的值）
 * </ul>
 *
...
public class MessageResolverRegistry {
   ...
   * 注册消息解析器。
   *
   * <p>允许后续注册覆盖（不同于 setResolverIfAbsent）， 便于测试和动态替换。
   *
   ...
   * 解析国际化消息。
   *
   * <p>优先使用 Spring 注入的解析器， 未注入时降级使用 MessageResolverHolder 中的值。
   *
   * @param key 消息 key
