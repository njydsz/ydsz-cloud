 ...
 *
 * <ul>
 *   <li>X-Content-Type-Options: nosniff - 防止 MIME 类型嗅探
 *   <li>X-Frame-Options: DENY - 防止点击劫持
 *   <li>X-XSS-Protection: 1; mode=block - 启用浏览器 XSS 过滤
 ...
 *
 * <p><b>与 safe 模块的关系：</b> 本过滤器为 base 模块的兜底实现，仅在未引入 safe/web/app 模块时生效。 当项目中存在 web/app 模块时，安全响应头由
...
public class SecurityHeadersFilter extends OncePerRequestFilter {
  ...
  private void addSecurityHeaders(HttpServletResponse response) {
    addHeaderIfNotEmpty(
        response, HEADER_X_CONTENT_TYPE_OPTIONS, properties.getContentTypeOptions());
