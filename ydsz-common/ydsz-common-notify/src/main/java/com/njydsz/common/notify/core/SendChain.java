 ...
 * 通知发送处理链
 *
 * <p>封装单条通知发送的完整处理链路：去重 → 熔断 → 限流 → 执行 → 指标 → 审计 → 降级。
 * 与直接在 {@link NotifyServiceImpl} 中串联调用相比，本类将链路逻辑集中管理，
 * 便于单元测试和步骤调整。
 *
 * <p>使用 {@link SendContext} 在步骤间传递状态，每个步骤可通过 {@link SendContext#hasResult()}
 * 判断是否已短路返回。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class SendChain {
     ...
     */
    public SendChain(Map<NotifyChannel, NotifyChannelStrategy> channelStrategies,
