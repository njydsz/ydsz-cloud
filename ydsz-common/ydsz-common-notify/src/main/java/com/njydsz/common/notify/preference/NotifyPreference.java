 ...
 *
 * <ul>
 *   <li>渠道开关（如关闭短信通知但保留邮件通知）
 *   <li>通知类型开关（如关闭审批提醒但保留告警通知）
 *   <li>免打扰时段配置
 *   <li>聚合策略配置（如多条消息聚合为一条摘要）
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class NotifyPreference {
  ...
  private Map<NotifyChannel, Set<NotifyType>> channelTypeOverrides =
   ...
   * 全量替换启用的通知渠道集合。
   *
   * <p>入参为 {@code null} 时视为清空（空集合）；内部复制为 {@link EnumSet} 防御性拷贝， 避免外部集合后续修改影响内部状态。
   *
  ...
  public void setEnabledChannels(Set<NotifyChannel> enabledChannels) {
    this.enabledChannels =
        enabledChannels != null
            ...
            : EnumSet.noneOf(NotifyChannel.class);
  }
  ...
  public void setEnabledTypes(Set<NotifyType> enabledTypes) {
    this.enabledTypes =
        enabledTypes != null ? EnumSet.copyOf(enabledTypes) : EnumSet.noneOf(NotifyType.class);
  }
  ...
  public void setDndStartHour(int dndStartHour) {
  ...
  public void setDndEndHour(int dndEndHour) {
    this.dndEndHour = dndEndHour;
  }

  public Map<NotifyChannel, Set<NotifyType>> getChannelTypeOverrides() {
    return channelTypeOverrides;
  }
  ...
  public int getAggregateWindowMinutes() {
    return aggregateWindowMinutes;
  }

  public void setAggregateWindowMinutes(int aggregateWindowMinutes) {
