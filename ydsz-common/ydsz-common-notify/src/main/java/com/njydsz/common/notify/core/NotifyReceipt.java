 ...
 * 通知回执
 *
 * <p>表示一条通知的投递状态，用于追踪通知是否成功送达接收者。
 * 回执状态包括：待投递（PENDING）、已投递（DELIVERED）、投递失败（FAILED）、已读（READ）。
 *
...
public record NotifyReceipt(
    ...
    public NotifyReceipt markDelivered() {
        return new NotifyReceipt(messageId, channel, receiver, ReceiptStatus.DELIVERED,
    ...
    public NotifyReceipt markFailed(String errorMessage) {
        return new NotifyReceipt(messageId, channel, receiver, ReceiptStatus.FAILED,
    ...
    public NotifyReceipt markRead() {
        return new NotifyReceipt(messageId, channel, receiver, ReceiptStatus.READ,
                createdAt, Instant.now(), null);
    }
}
