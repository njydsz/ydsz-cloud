 ...
 * 统一消息通知服务接口
 *
 * <p>支持邮件、企业微信、钉钉、飞书等全渠道消息发送。
 * 核心入口为 {@link #send(NotifyRequest)}，其他方法为其便捷封装。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public interface NotifyService {
     ...
     * @return 发送结果
     */
    default NotifySendResult send(NotifyChannel channel, String receiver, String title, String content) {
        NotifyRequest request = NotifyRequest.of(channel, receiver, title, content)
     ...
     * @return 发送结果
     */
    default NotifySendResult sendTemplate(NotifyChannel channel, String receiver,
                                           String templateCode, Object templateParams) {
        NotifyRequest request = NotifyRequest.of(channel, receiver, null, null)
     ...
     */
    CompletableFuture<NotifySendResult> parallelBatchSend(NotifyChannel channel,
     ...
     * @return 异步批量发送结构化结果
     */
    CompletableFuture<BatchSendResult> parallelBatchSendDetailed(NotifyChannel channel,
                                                                  List<String> receivers,
                                                                  String title, String content);
}
