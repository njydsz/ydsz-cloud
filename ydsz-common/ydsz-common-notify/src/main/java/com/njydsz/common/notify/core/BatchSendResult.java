 ...
 * 批量发送结构化结果
 *
 * <p>提供每个接收者的发送明细，便于业务方定位失败接收者并执行定向重试。
 * 与 {@link NotifySendResult} 不同，本对象包含完整的逐条发送结果。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public class BatchSendResult {
    ...
    public BatchSendResult(int totalCount, int successCount, int failureCount,
                           List<ReceiverSendResult> details) {
        this.totalCount = totalCount;
        this.successCount = successCount;
        this.failureCount = failureCount;
        this.details = details != null ? Collections.unmodifiableList(new ArrayList<>(details))
