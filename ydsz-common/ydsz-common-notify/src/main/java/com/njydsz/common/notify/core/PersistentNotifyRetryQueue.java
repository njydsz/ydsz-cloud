 ...
 *
 * <p>内部优先使用 Redis List 存储待重试消息；当 Redis 不可用时自动降级到 内置 {@link ArrayBlockingQueue} 内存队列，保障服务可用性。
 * 超过最大重试次数的消息自动移入死信队列。
 *
 * <h3>重试流程</h3>
 *
 * <ol>
 *   <li>消息发送失败后调用 {@code enqueue} 加入重试队列
 *   <li>后台定时线程以固定间隔 {@code poll} 取出消息重新发送
 *   <li>发送成功则移除；失败则重试次数 +1 后重新入队
 *   <li>重试次数超过 {@code maxRetries} 后移入死信队列
 * </ol>
 *
 * <h3>Redis 降级探测</h3>
 *
...
public class PersistentNotifyRetryQueue implements NotifyRetryQueue {
   ...
   */
  public PersistentNotifyRetryQueue(
    ...
    this.deadLetterHandler =
        deadLetterHandler != null ? deadLetterHandler : new InMemoryDeadLetterHandler();
  ...
  private static class InMemoryFallbackRetryQueue implements NotifyRetryQueue {
    ...
    InMemoryFallbackRetryQueue(
        int maxRetries, int capacity, int batchSize, DeadLetterHandler deadLetterHandler) {
    ...
    public void offer(
      ...
      if (!queue.offer(entry)) {
        ...
        LOG.warn(
            "[NotifyRetryQueue] 队列已满，丢弃消息, channel={}, receiver={}", channel.getName(), receiver);
        return;
      }
      queuedCount.incrementAndGet();
      LOG.warn(
          "[NotifyRetryQueue] 加入重试队列, channel={}, receiver={}, retryCount={}",
          channel.getName(),
          receiver,
          entry.retryCount);
    }
    ...
    public void retry(NotifyService notifyService) {
      ...
      if (entry.nextRetryTime > System.currentTimeMillis()) {
        if (!queue.offer(entry)) {
          droppedCount.incrementAndGet();
          LOG.warn(
              "[NotifyRetryQueue] 队列已满，丢弃延迟重试消息, channel={}, receiver={}",
      ...
      try {
        NotifySendResult result =
            notifyService.send(entry.channel, entry.receiver, entry.title, entry.content);
        if (result.isSuccess()) {
          LOG.info(
              "[NotifyRetryQueue] 重试成功, channel={}, receiver={}",
              entry.channel.getName(),
              entry.receiver);
          return;
        }
    ...
    public int retryBatch(NotifyService notifyService, int maxBatchSize) {
      ...
      while (processed < limit) {
        ...
        try {
          ...
          if (result.isSuccess()) {
            LOG.info(
                "[NotifyRetryQueue] 批量重试成功, channel={}, receiver={}",
                entry.channel.getName(),
                entry.receiver);
          } else {
            handleRetryFailure(entry, result.getErrorMessage());
    ...
    private void handleRetryFailure(RetryEntry entry, String error) {
      ...
      if (entry.retryCount > maxRetries) {
        ...
        LOG.error(
            "[NotifyRetryQueue] 重试超过最大次数，移入死信队列, channel={}, receiver={}, totalRetries={}, lastError={}",
            entry.channel.getName(),
            entry.receiver,
            entry.retryCount,
            entry.lastError);
        ...
        if (deadLetterHandler != null) {
          deadLetterHandler.moveToDeadLetter(
              entry.channel,
              entry.receiver,
              entry.title,
              entry.content,
      ...
      if (!queue.offer(entry)) {
        droppedCount.incrementAndGet();
        LOG.warn(
            "[NotifyRetryQueue] 队列已满，丢弃重试消息, channel={}, receiver={}",
            entry.channel.getName(),
            entry.receiver);
        return;
      }
      LOG.warn(
          "[NotifyRetryQueue] 重试失败，重新入队 ({}/{}), channel={}, receiver={}, nextRetryIn={}ms",
          entry.retryCount,
          maxRetries,
          entry.channel.getName(),
          entry.receiver,
