 ...
 * 渠道降级管理器（P1-6）
 *
 * <p>当主渠道发送失败时，按配置的降级链自动尝试备用渠道发送。 降级链在 {@code ydsz.notify.fallback.chains} 中配置， key 为主渠道枚举，value
 * 为按优先级排序的备用渠道列表。
 *
 * <p><b>配置示例：</b>
 *
 ...
 * <ul>
 *   <li>仅在主渠道发送失败时触发
 *   <li>按降级链顺序依次尝试，直到成功或全部失败
 *   <li>每次降级尝试记录日志，便于追踪降级路径
 *   <li>最多尝试 3 个备用渠道，防止无限降级
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class NotifyFallbackManager {
   ...
   */
  public NotifySendResult fallbackSend(
      NotifyChannel primaryChannel, String receiver, String title, String content) {
    ...
    if (fallbackChain == null || fallbackChain.isEmpty()) {
      return NotifySendResult.failure(
          "主渠道[" + primaryChannel.getName() + "]发送失败且无降级链配置", primaryChannel.getName());
    }
    ...
    return NotifySendResult.failure(
        "所有降级渠道均发送失败, 主渠道=" + primaryChannel.getName(), primaryChannel.getName());
  }
