...
public class SmsNotifySender implements NotifyChannelStrategy {
  ...
  public boolean isEnabled() {
    return smsConfig != null
        && smsConfig.isEnabled()
        && (smsProvider != null
            || (smsConfig.getEndpoint() != null && !smsConfig.getEndpoint().isEmpty()));
  }
  ...
  private NotifySendResult doSend(
      String receiver, String templateCode, Map<String, Object> templateParam) {
    ...
    try {
      ...
      if ("0".equals(code) || "OK".equals(code) || "SUCCESS".equals(code)) {
        ...
        return NotifySendResult.success(
            messageId != null ? messageId : "sent", getChannel().getName());
      }
   ...
   * @return 异步发送结果
   */
  public CompletableFuture<NotifySendResult> sendSmsAsync(
   ...
   * @return 异步发送结果
   */
  public CompletableFuture<NotifySendResult> batchSendSmsAsync(
