 ...
 *
 * <p>提供流式 API 构建 {@link NotifyServiceImpl}，避免直接使用 10 参数构造函数。
 * 必填参数通过构造器传入，可选参数通过 fluent 方法设置。
 *
 * <p><b>使用示例：</b>
 * <pre>{@code
...
public class NotifyServiceImplBuilder {
     ...
     * 设置熔断器注册中心
     */
    public NotifyServiceImplBuilder circuitBreakerRegistry(NotifyCircuitBreakerRegistry circuitBreakerRegistry) {
