...
public class RedisNotifyRetryQueue implements NotifyRetryQueue {
  ...
  public RedisNotifyRetryQueue(
      StringRedisTemplate stringRedisTemplate, int maxRetries, int batchSize) {
  ...
  public RedisNotifyRetryQueue(
    ...
    String prefix =
        (redisKeyPrefix != null && !redisKeyPrefix.isEmpty()) ? redisKeyPrefix : "notify:retry:";
  ...
  public void offer(
    ...
    try {
      RetryMessage msg =
          new RetryMessage(
              channel, receiver, title, content, 1, lastError, System.currentTimeMillis());
      ...
      Long result =
          stringRedisTemplate.execute(
              ...
              msgJson,
              String.valueOf(msg.nextRetryTime),
  ...
  public int retryBatch(NotifyService notifyService, int maxBatchSize) {
    ...
    try {
      ...
      Set<String> ids = stringRedisTemplate.opsForZSet().rangeByScore(zsetKey, 0, now, 0, limit);
      ...
      for (String id : ids) {
        ...
        RetryMessage msg = YdszJson.fromJson(msgJson, RetryMessage.class);
        if (msg == null) {
