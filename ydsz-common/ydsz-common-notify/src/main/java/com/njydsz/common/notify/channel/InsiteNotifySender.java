 ...
 * 站内信通知发送器
 *
 * <p>实现 {@link NotifyChannelStrategy} 接口，将站内信存储到 Redis List，
 * 支持前端通过 WebSocket 或轮询拉取。
 *
 * <p>存储结构（Redis List）：
 * <ul>
 *   <li>Key：{@code notify:insite:{userId}}</li>
 *   <li>Value：JSON 格式的 {@link InsiteMessage}</li>
 *   <li>TTL：{@code ydsz.notify.insite.expire-minutes}（默认 1440 分钟 = 24 小时）</li>
 *   <li>容量上限：{@code ydsz.notify.insite.max-queue-size}（默认 10000 条/用户）</li>
 * </ul>
 *
 ...
 * }</pre>
 *
 * <p><b>收敛定位</b>：作为 common-notify 内置的 INSITE Provider，
 * 当 message 模块不存在时提供默认的站内信存储能力。
 * 若 message 模块存在，其 {@code InAppChannel} 通过
 * {@code NotifyChannelBridgeConfiguration} 桥接后自动覆盖本实现。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class InsiteNotifySender implements NotifyChannelStrategy {
     ...
     */
    public InsiteNotifySender(NotifyProperties notifyProperties,
    ...
    public NotifySendResult batchSend(List<String> receivers, String title, String content) {
        ...
        return NotifySendResult.failure(
                "部分发送失败: 成功" + successCount + "/" + receivers.size(),
                getChannel().getName());
    }
