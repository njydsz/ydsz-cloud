 ...
 * 发送链路上下文
 *
 * <p>封装一次通知发送所需的全部参数和可变状态，供 {@link SendChain} 各步骤传递。
 * 使用 record 保证不可变参数部分的安全性，可变状态通过单独字段维护。
 *
 * @param channel       通知渠道
...
public record SendContext(
     ...
     */
    public static SendContext forSend(NotifyChannel channel, String receiver, String title,
                                       String content, String userId, String templateCode,
                                       NotifyType notifyType, String tenantId) {
        return new SendContext(channel, receiver, title, content, userId, templateCode,
                notifyType, tenantId, null, null, 0L, 0L);
    }

    /**
     * 构造初始上下文（用于 doSendTemplate 路径）
     */
    public static SendContext forTemplate(NotifyChannel channel, String receiver,
                                           String templateCode, Object templateParams,
                                           String title, String tenantId) {
        return new SendContext(channel, receiver, title, null, null, templateCode,
                NotifyType.TEMPLATE, tenantId, templateParams, null, 0L, 0L);
    }
    ...
    public SendContext withResult(NotifySendResult result) {
        return new SendContext(channel, receiver, title, content, userId, templateCode,
                notifyType, tenantId, templateParams, result, durationNanos, startTime);
    }
    ...
    public SendContext withTiming(long startTime, long durationNanos) {
        return new SendContext(channel, receiver, title, content, userId, templateCode,
                notifyType, tenantId, templateParams, sendResult, durationNanos, startTime);
    }
