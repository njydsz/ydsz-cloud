 ...
 * 邮件发送提供商抽象接口（P2-10）
 *
 * <p>将邮件发送能力抽象为接口，支持多 SMTP 提供商（如腾讯企业邮箱、阿里云邮件推送、 AWS SES、SendGrid 等）的动态切换。
 *
 * <p>每个提供商实现此接口，通过 {@code ydsz.notify.email.provider} 配置项选择当前使用的提供商。 在运行时可动态切换提供商，实现故障转移和灰度迁移。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
