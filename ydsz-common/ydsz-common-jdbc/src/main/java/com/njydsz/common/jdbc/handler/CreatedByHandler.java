 ...
 * 创建人字段填充处理器
 *
 * <p>在 INSERT 操作时自动为记录设置创建人标识（created_by 字段）。 从当前请求上下文中获取用户信息，如果获取失败则使用英文标识符 {@value
 * com.njydsz.common.jdbc.constant.AuditFieldConstants#CREATED_BY_SYSTEM}， 由上层 MessageSource 通过 key
 * {@code audit.created_by.system} 解析为展示文本。
 *
 * <h2>使用场景</h2>
 *
 * <ul>
  ...
  protected Expression doGetFieldFillValue() {
    ...
    String uniqueId =
        Optional.ofNullable(authInfo)
            ...
            .orElse(AuditFieldConstants.CREATED_BY_SYSTEM);
