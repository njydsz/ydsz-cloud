 ...
 * 每次调用都会通过 {@code CCJSqlParserUtil.parse()} 重新解析 SQL。在拦截器链中， 多个拦截器（逻辑删除、行权限、列权限）各自独立解析同一条 SQL，造成 N
 * 倍解析开销。
 *
 * <p>本基类覆盖解析入口，通过 {@link SqlAstCache} 复用已解析的 AST：
 *
 * <ul>
 *   <li>首次解析：通过缓存获取（未命中时自动解析并缓存）
 *   <li>后续拦截器处理相同 SQL 模板时：直接从缓存返回深拷贝，零解析开销
 *   <li>所有改写操作在深拷贝上进行，保证线程安全
 * </ul>
 *
 * <p>使用方式：将拦截器继承的父类从 {@code JsqlParserSupport} 改为 {@code CachingJsqlParserSupport}， 并通过构造器注入 {@link
 * SqlAstCache}，即可透明获得缓存能力。
 *
...
public abstract class CachingJsqlParserSupport extends JsqlParserSupport {
   ...
   *
   * <p>覆盖父类实现，将解析步骤替换为缓存查找。缓存命中时返回 AST 深拷贝直接改写； 缓存未命中时解析后存入缓存。后续拦截器处理相同 SQL 模板时零解析。
   *
   * @param sql 原始 SQL 语句
   ...
   *
   * <p>此方法复现了 MyBatis-Plus {@code JsqlParserSupport} 的分发逻辑： 根据 Statement 运行时类型调用对应的 {@code
   * processXxx} 抽象方法， 实现子类只需覆盖特定类型的处理逻辑即可。
   *
