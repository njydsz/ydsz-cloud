...
public final class JSqlParserHelper {
   ...
   *
   * <p>这是 JSqlParser 4.7+ 推荐的方式，通过 getUpdateSets() 获取列和值。 注意：ExpressionList 直接继承 List，因此直接返回
   * ExpressionList 本身。
   *
   * @param update UPDATE 语句对象
   * @return 值表达式列表
   ...
   *
   * <p>这是 JSqlParser 4.7+ 推荐的方式，使用列表存储多个 ON 表达式。
   *
   ...
   * 设置 JOIN 语句的 ON 表达式
   *
   * <p>兼容性别名方法，如果只有一个表达式，设置到列表的第一个位置。
   *
   * @param join JOIN 语句对象
   * @param expr ON 表达式
   */
