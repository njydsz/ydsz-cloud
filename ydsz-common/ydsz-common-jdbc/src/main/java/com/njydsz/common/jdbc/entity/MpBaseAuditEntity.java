 ...
 *
 * <p>包含审计字段（创建人/时间、更新人/时间），由
 * {@code CombinedFieldFillInterceptor} 通过 JSQLParser 改写 SQL 在 INSERT/UPDATE 时自动填充。
 *
...
public class MpBaseAuditEntity<T extends Serializable> extends MpBaseIdEntity<T> {
    ...
    /**
     * 创建人ID
     * <p>由 {@code CombinedFieldFillInterceptor} 在 INSERT 操作时自动填充，
     * 此处不使用 {@code @TableField(fill)} 以避免双重填充。
     */
    ...
    /**
     * 更新人ID
     * <p>由 {@code CombinedFieldFillInterceptor} 在 INSERT/UPDATE 操作时自动填充，
     * 此处不使用 {@code @TableField(fill)} 以避免双重填充。
     */
