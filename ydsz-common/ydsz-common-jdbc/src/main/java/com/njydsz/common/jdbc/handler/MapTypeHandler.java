 ...
 *
 * <p>通过 {@code valueType} 构造参数保留具体值类型，解决泛型擦除导致的
 * 反序列化后值变为 {@code LinkedHashMap} 的问题。无参构造 fallback 到 {@link Object.class}，
 * 保持向后兼容（行为与旧版一致）。</p>
 *
 * <h2>使用场景</h2>
 * <ul>
...
public class MapTypeHandler<V> extends BaseTypeHandler<Map<String, V>> {
     ...
     * 构造 Map 类型处理器（指定值类型）。
     *
     * <p>反序列化时会将 JSON 对象值转换为 {@code valueType} 指定的类型，
     * 而非默认的 {@code LinkedHashMap}。</p>
     *
     ...
     *
     * <p>当通过 {@code @TableField(typeHandler = MapTypeHandler.class)} 等方式使用时，
     * MyBatis 通过反射调用无参构造，此时值类型 fallback 到 {@link Object.class}，
     * 反序列化行为与旧版一致（值为 LinkedHashMap / 基本类型）。</p>
     */
     ...
     */
    @Override
    public void setNonNullParameter(PreparedStatement ps, int i, Map<String, V> parameter, JdbcType jdbcType) throws SQLException {
     ...
     */
    @Override
    public Map<String, V> getNullableResult(CallableStatement cs, int columnIndex) throws SQLException {
     ...
     *
     * <p>当 valueType 为 {@link Object.class}（无参构造 fallback）时，
     * 使用 {@link JsonParserUtil#parseObject(String)} 保持旧版行为；
     * 当指定了具体 valueType 时，使用 {@link YdszJson#fromJsonToMap(String, Class, Class)}
     * 将值转换为目标类型。</p>
     *
     * @param json JSON 字符串
