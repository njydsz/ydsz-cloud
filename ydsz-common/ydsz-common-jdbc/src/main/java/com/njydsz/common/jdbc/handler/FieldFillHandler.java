 ...
 * 字段填充处理器接口
 *
 * <p>定义字段自动填充的标准行为，用于在 SQL 执行前自动为指定字段设置值。 主要应用于审计字段（创建人、创建时间、更新人、更新时间）的自动填充。
 *
 * <p>该接口配合 {@link com.njydsz.common.jdbc.interceptor.FieldFillInterceptor} 使用， 在 INSERT/UPDATE
 * 语句执行前自动注入字段值。
 *
 * <h2>使用示例</h2>
 *
...
public interface FieldFillHandler {
   ...
   * 获取自动填充值表达式
   *
   * <p>返回的表达式将作为 INSERT/UPDATE 语句中对应字段的值。 可以是常量值、函数调用或参数占位符。
   *
   * @return 填充值表达式
   */
   ...
   *
   * <p>某些特殊表可能不需要自动填充审计字段，可以通过此方法进行控制。
   *
   * @param tableName 表名
