 ...
 * <p><b>输出字段：</b>
 * <ul>
 *   <li>{@code timestamp} — ISO-8601 时间戳</li>
 *   <li>{@code sql_id} — MyBatis MappedStatement ID</li>
 *   <li>{@code command_type} — SQL 类型（SELECT/INSERT/UPDATE/DELETE）</li>
 *   <li>{@code elapsed_ms} — 执行耗时（毫秒）</li>
 *   <li>{@code affected_rows} — 影响行数</li>
 *   <li>{@code sql_fingerprint} — SQL 指纹（归一化）</li>
 *   <li>{@code sql} — 完整 SQL 语句</li>
 *   <li>{@code exception} — 异常信息（存在时）</li>
 *   <li>{@code stack_trace} — 异常堆栈（存在时）</li>
 * </ul>
 *
 * <p><b>使用方式：</b>
 * <pre>{@code
...
public final class SqlAuditJsonLogger {
     ...
     */
    public static void log(String sqlId, String commandType, String sql,
                           long elapsedMs, Integer affectedRows, Object parameter) {
     ...
     */
    public static void logError(String sqlId, String commandType, String sql,
                                long elapsedMs, Throwable exception) {
    ...
    private static void appendField(StringBuilder json, String key, Map<String, ?> value, boolean unused) {
        json.append(',').append('"').append(escapeJson(key)).append("\":");
        if (value == null) {
            json.append("null");
        } else {
            ...
            for (Map.Entry<String, ?> entry : value.entrySet()) {
                ...
                first = false;
                appendField(json, entry.getKey(), entry.getValue() != null
                        ? entry.getValue().toString() : null, true);
            }
            json.append('}');
        }
    }
    ...
    private static String formatParameter(Object parameter) {
        ...
        return paramStr.length() > 500
                ? paramStr.substring(0, 500) + "...(已截断)"
                : paramStr;
    }
