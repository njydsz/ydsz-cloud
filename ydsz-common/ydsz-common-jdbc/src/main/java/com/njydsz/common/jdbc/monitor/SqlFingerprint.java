 ...
 * SQL 指纹归一化工具
 *
 * <p>将 SQL 语句中的参数值、IN 列表等替换为占位符，生成稳定的指纹用于 Micrometer tag， 避免高基数标签导致 Prometheus 内存爆炸。
 *
 * <p>归一化规则：
 *
 * <ul>
 *   <li>字符串字面量 {@code 'xxx'} → {@code ?}
 *   <li>数字字面量 {@code 123} → {@code ?}
 *   <li>IN 列表 {@code IN (1,2,3)} → {@code IN (?)}
 *   <li>多空格折叠为单空格
 *   <li>超长指纹截断到 200 字符
 * </ul>
 *
...
public final class SqlFingerprint {
  ...
  private static final Pattern IN_LIST =
      Pattern.compile("IN\\s*\\(\\s*\\?(?:\\s*,\\s*\\?)*\\s*\\)", Pattern.CASE_INSENSITIVE);
