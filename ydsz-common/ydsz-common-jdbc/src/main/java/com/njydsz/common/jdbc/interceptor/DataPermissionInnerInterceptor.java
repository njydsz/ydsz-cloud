 ...
 * 的公共逻辑，消除以下重复代码：
 * <ul>
 *   <li>{@code apply()} — 空上下文处理 + parserSingle 调用</li>
 *   <li>{@code beforePrepare()} — 配置检查 + SQL 类型过滤 + 注解忽略 + 解析权限上下文</li>
 *   <li>{@code shouldApply(Table)} — 表名标准化 + 拦截策略判断</li>
 *   <li>{@code isDataPermissionIgnored()} — @DataPermissionIgnore 注解检查</li>
 *   <li>{@code normalizeTableSet()} — 表名集合标准化</li>
 * </ul>
 *
 * <p>模板方法 {@link #beforePrepare} 定义了通用拦截流程，子类可通过覆盖
 * {@link #shouldCheckBypass()} 和 {@link #isSupportedSqlType} 自定义行为差异。
 *
 * <p>行级拦截器处理 SELECT/UPDATE/DELETE；列级拦截器处理 SELECT/UPDATE/INSERT。
 * 两者在 INSERT vs DELETE 上的差异由 {@link #isSupportedSqlType} 抽象方法承载。
 *
 * @author ydsz-team
 * @since 1.8.0
 */
@Slf4j
public abstract class DataPermissionInnerInterceptor extends CachingJsqlParserSupport implements InnerInterceptor {
     ...
     *   <li>（可选）系统级绕过检查 → 跳过</li>
     *   <li>SQL 类型不支持 → 跳过</li>
     *   <li>@DataPermissionIgnore 注解 → 跳过</li>
     *   <li>解析权限上下文并改写 SQL</li>
     * </ol>
     *
     ...
     * @return 支持时返回 true
     */
    protected abstract boolean isSupportedSqlType(org.apache.ibatis.mapping.SqlCommandType sqlCommandType);
