 ...
 *
 * <p>将 MyBatis-Plus 的 {@link IPage} 分页查询结果转换为 {@link PageResponse} 统一响应体，避免各业务模块重复编写 {@code total /
 * pageNum / pageSize} 的样板转换代码。
 *
 * <p><b>返回值语义：</b>响应体包含 {@code total / pageNum / pageSize} 三个分页字段， data 字段携带当前页记录列表，额外提供 {@link
 * PageResponse#getPages()} 便捷方法。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><b>与 core 模块的关系：</b>core 模块保持零依赖原则（不引入 MyBatis-Plus）， 因此本工具位于 jdbc 模块（MyBatis-Plus 依赖的宿主模块）。
...
public final class PageResponses {
   ...
   *
   * <p>自动提取 total / pageNum / pageSize，并携带记录列表。
   *
   * @param page MyBatis-Plus 分页结果（含记录、总数、页码信息）
   * @param <T> 记录类型
   * @return 成功分页响应
   */
  public static <T> PageResponse<List<T>> success(IPage<T> page) {
    if (page == null) {
    ...
    return PageResponse.success(total, pageNum, pageSize, records);
  }

  /**
   * 将 MyBatis-Plus 分页结果转换为成功分页响应（记录经映射函数转换）。
   *
   * <p>适用于分页查询结果需要 DO → VO 转换的场景， 避免先取出 records 再单独 map 的样板代码。
   *
  ...
  public static <S, T> PageResponse<List<T>> success(
    ...
    return PageResponse.success(page.getTotal(), page.getCurrent(), page.getSize(), mapped);
  }
}
