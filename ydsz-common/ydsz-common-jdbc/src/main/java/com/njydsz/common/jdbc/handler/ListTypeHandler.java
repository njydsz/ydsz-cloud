 ...
 *
 * <p>通过 {@code elementType} 构造参数保留具体元素类型，解决泛型擦除导致的
 * 反序列化后元素变为 {@code LinkedHashMap} 的问题。无参构造 fallback 到 {@link Object.class}，
 * 保持向后兼容（行为与旧版一致）。</p>
 *
 * <h2>使用场景</h2>
 * <ul>
...
public class ListTypeHandler<E> extends BaseTypeHandler<List<E>> {
     ...
     *
     * <p>反序列化时会将 JSON 数组元素转换为 {@code elementType} 指定的类型，
     ...
     *
     * <p>当通过 {@code @TableField(typeHandler = ListTypeHandler.class)} 等方式使用时，
     * MyBatis 通过反射调用无参构造，此时元素类型 fallback 到 {@link Object.class}，
     * 反序列化行为与旧版一致（元素为 LinkedHashMap / 基本类型）。</p>
     */
     ...
     *
     * <p>当 elementType 为 {@link Object.class}（无参构造 fallback）时，
     * 使用 {@link JsonParserUtil#parseArray(String)} 保持旧版行为；
     * 当指定了具体 elementType 时，使用 {@link YdszJson#parseArray(String, Class)}
     * 将元素转换为目标类型。</p>
     *
     * @param json JSON 字符串
