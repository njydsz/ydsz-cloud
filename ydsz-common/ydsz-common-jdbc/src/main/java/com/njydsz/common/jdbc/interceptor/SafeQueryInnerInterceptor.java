 ...
 *
 * <p>从 {@link com.njydsz.common.domain.query.PageQuery} 中抽取 SQL 安全相关职责， 统一在 MyBatis 拦截器层处理，实现关注点分离。
 *
 * <p>拦截规则：
 *
 * <ul>
 *   <li>ORDER BY 字段安全校验 — 防止 SQL 注入（仅允许 {@code [a-zA-Z_][a-zA-Z0-9_.]*} 模式）
 *   <li>深度分页检测 — offset 超过阈值时 WARN 日志或拒绝执行
 * </ul>
 *
 * <p>配置示例：
 *
...
public class SafeQueryInnerInterceptor implements InnerInterceptor {
   ...
   * 检测深度分页。
   *
   * <p><b>优先解析 {@link IPage} 参数：</b>MP 分页主路径的 offset 是绑定参数 （SQL 中为 {@code LIMIT ? OFFSET ?}
   * 占位符），无法通过 SQL 正则匹配， 因此从 {@code StatementHandler} 的绑定参数中提取分页对象计算 offset。 仅在第一页或未携带分页参数（原生字面量
   * LIMIT）时，跳过检测或回退到正则兜底。
   *
   ...
   *
   * <p>MP 分页参数可能直接是 {@code Page} 对象，也可能是包含 {@code page} 键的 参数 Map（如 {@code selectPage(page,
   * wrapper)} 场景）。
   *
   ...
   *
   * <p>复用领域层 {@link DeepPaginationRisk#assess(long, long, long)} 的统一判定， 命中拒绝阈值时抛出携带完整分页信息的 {@link
   * DeepPaginationException}。
   *
   * @param offset 偏移量
  ...
  private void evaluateDeepPagination(long offset, int pageNum, int pageSize) {
    DeepPaginationRisk risk =
        DeepPaginationRisk.assess(offset, cursorWarningThreshold, cursorRejectThreshold);
    ...
    if (risk == DeepPaginationRisk.WARN) {
      log.warn(
          "深度分页警告：offset={}（pageNum={}, pageSize={}）超过警告阈值={}，建议改用游标分页",
          offset,
          pageNum,
          pageSize,
          cursorWarningThreshold);
    }
  }
   ...
   *
   * <p>仅适用于未携带 IPage 参数、SQL 中硬编码 LIMIT 的罕见查询场景， 此时 pageNum/pageSize 无法从拦截器上下文获取，异常中显示为 -1。
   *
   * @param offset 偏移量
   */
  private void evaluateDeepPagination(long offset) {
    ...
    if (risk == DeepPaginationRisk.WARN) {
      log.warn("深度分页警告：offset={}（SQL 字面量 LIMIT）超过警告阈值={}，建议改用游标分页", offset, cursorWarningThreshold);
    }
  }
