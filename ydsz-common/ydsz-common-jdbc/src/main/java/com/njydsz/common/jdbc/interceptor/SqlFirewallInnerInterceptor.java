 ...
 *
 * <ul>
 *   <li>DROP TABLE / DROP DATABASE — 禁止删除表/库
 *   <li>TRUNCATE TABLE — 禁止清空表
 *   <li>DELETE WITHOUT WHERE — 禁止无条件删除
 *   <li>UPDATE WITHOUT WHERE — 禁止无条件更新
 *   <li>ALTER TABLE ... DROP — 禁止删除列（可选）
 *   <li>GRANT / REVOKE — 禁止权限操作
 *   <li>分号分隔的多语句 — 禁止多语句执行
 * </ul>
 *
 * <p>配置示例：
...
public class SqlFirewallInnerInterceptor implements InnerInterceptor {
  ...
  private static final Pattern DROP_PATTERN =
      Pattern.compile("\\bDROP\\s+(TABLE|DATABASE|INDEX|SCHEMA|VIEW)\\b", Pattern.CASE_INSENSITIVE);
  ...
  /** GRANT / REVOKE 正则 */
  private static final Pattern DDL_PERMISSION_PATTERN =
   ...
   * 检测 SQL 是否包含多语句（分号分隔）
   *
   * <p>去除字符串字面量后，若仍存在分号则判定为多语句。 末尾分号（如存储过程定义）也视为多语句特征，由调用方按场景决定放行。
   *
   * @param sql 原始 SQL
   ...
   * 检查表是否在白名单中
   *
   * <p>使用 {@code \b} 边界正则匹配表名，避免 {@code contains()} 导致的前缀误匹配 （如白名单 {@code user} 错误匹配到 {@code users}
   * 表）。
   */
   ...
   * 设置允许执行危险操作的表名白名单。
   *
   * <p>配置来源为 {@code ydsz.jdbc.sql-firewall.allow-tables}。入参为 {@code null} 时视为无白名单（任何 DROP/TRUNCATE
   * 均被拦截）；非空时会对 表名做 trim 并统一转小写存储，与 SQL 检测时的小写化匹配逻辑保持一致， 避免因大小写差异导致白名单失效。
   *
