 ...
 *
 * <p>{@code common-jdbc} 通过 Spring {@code ObjectProvider} 自动发现并加载， 无需 {@code common-jdbc}
 * 硬依赖任何外部模块。
 *
 * <p><b>拦截器链顺序约定（值越小越靠前）：</b>
 *
 * <ul>
 *   <li>100: OptimisticLock（乐观锁）
 *   <li>300: FieldFill（字段填充）
 *   <li>400: TenantIsolation（租户隔离，由 common-tenant 提供）
 *   <li>500: DataPermission（行级 + 列级数据权限）
 *   <li>600: Pagination（分页）
 * </ul>
 *
 * <p><b>注意：</b>逻辑删除（LogicalDelete）自 v1.9.0 起已迁移至 MP 原生 {@code @TableLogic} 注解， 不再占用拦截器链节点。
 *
