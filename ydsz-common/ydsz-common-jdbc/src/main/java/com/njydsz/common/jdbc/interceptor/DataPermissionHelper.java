 ...
 *
 * <ul>
 *   <li>检测 Mapper 方法是否标注了 {@link DataPermissionIgnore}（跳过数据权限拦截）
 *   <li>解析 SQL 中的表名与数据权限规则匹配
 *   <li>缓存忽略标记结果（有界 10000 条），避免重复反射扫描
 * </ul>
 *
 * <h3>缓存设计</h3>
 *
 * <p>使用 ydsz-common-cache 缓存方法级忽略标记，最大容量 10000 条，防止内存泄漏， 并避免 {@code Collections.synchronizedMap}
 * 全局锁带来的并发竞争。
 *
 * @author ydsz-team
 * @since 1.0.0
...
final class DataPermissionHelper {
  ...
  static boolean shouldApply(
      Table table, DataPermissionConfiguration config, Set<String> normalizedTables) {
    if (table == null) {
      return false;
    }
