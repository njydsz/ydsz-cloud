 ...
 *
 * <p>从请求上下文中读取数据权限相关的 HTTP 请求头，构建 {@link DataPermissionContext}， 供 {@link
 * DataPermissionInnerInterceptor} 在 SQL 执行前改写 WHERE/JOIN 条件。
 *
 * <h2>读取的 Header 清单（与 {@link DataPermissionHeaderConstants} 编号对齐）</h2>
 *
 * <ul>
 *   <li>X-Data-Scope (6) - 数据权限范围类型，决定按哪个维度过滤
 *   <li>X-Company-Ids (7) - 公司ID集合（GROUP 范围）
 *   <li>X-Dept-Ids (8) - 部门ID集合（COMPANY/DEPT 范围）
 *   <li>X-Unique-Id (9) - 用户ID（USER 范围）
 *   <li>X-Project-Ids (11) - 项目ID集合（PROJECT 范围）
 *   <li>X-Region-Ids (12) - 区域ID集合（REGION 范围）
 *   <li>X-Visible-Columns (14) - 列可见规则（SELECT 过滤）
 *   <li>X-Editable-Columns (15) - 列可编辑规则（INSERT/UPDATE 过滤）
 * </ul>
 *
 * <p><b>注意：</b>租户ID（X-Tenant-Id）已由独立的 {@code common-tenant} 模块 通过 {@code TenantContextWebFilter} +
 * {@code TenantIsolationInterceptor} 处理， 本解析器不再负责租户上下文。
 *
 * <h2>读取优先级（安全增强）</h2>
 *
 * <ol>
 ...
 *
 * <ul>
 *   <li>group scope：扩展 companyIds -> 下级公司
 *   <li>company/dept scope：扩展 deptIds -> 下级部门
 *   <li>project scope：扩展 projectIds -> 下级项目
 *   <li>region scope：扩展 regionIds -> 下级区域
 * </ul>
 *
...
public class DataPermissionContextResolver {
  ...
  public DataPermissionContext resolve() {
    ...
    context.setDataScope(
        resolveDataScope(resolveHeader(request, DataPermissionHeaderConstants.X_DATA_SCOPE)));
    ...
    context.setUserId(
        trimToNull(
            authUserId != null
                ? authUserId
                : resolveHeader(request, DataPermissionHeaderConstants.X_UNIQUE_ID)));
    context.setCompanyIds(
        splitCsv(resolveHeader(request, DataPermissionHeaderConstants.X_COMPANY_IDS)));
    context.setDeptIds(splitCsv(resolveHeader(request, DataPermissionHeaderConstants.X_DEPT_IDS)));
    context.setProjectIds(
        splitCsv(resolveHeader(request, DataPermissionHeaderConstants.X_PROJECT_IDS)));
    context.setRegionIds(
        splitCsv(resolveHeader(request, DataPermissionHeaderConstants.X_REGION_IDS)));
    context.setVisibleColumnsByTable(
        parseTableColumnsRule(
            resolveHeader(request, DataPermissionHeaderConstants.X_VISIBLE_COLUMNS)));
    context.setEditableColumnsByTable(
        parseTableColumnsRule(
            resolveHeader(request, DataPermissionHeaderConstants.X_EDITABLE_COLUMNS)));
   ...
   *
   * <ul>
   *   <li>group：扩展 companyIds
   *   <li>company / dept：扩展 deptIds
   *   <li>project：扩展 projectIds
   *   <li>region：扩展 regionIds
   *   <li>其他（user）或 idExpander 为 null：不做扩展
   * </ul>
   *
