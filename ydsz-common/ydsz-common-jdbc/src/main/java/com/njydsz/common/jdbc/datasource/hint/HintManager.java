 ...
 *
 * <p>提供编程式 API 强制某次查询走主库或指定数据源， 优先级高于事务上下文（@Transactional）和 {@code @DS} 注解的路由。
 *
 * <p>使用方式：
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><b>线程安全：</b>基于 {@link ThreadLocal} 实现，每个线程独立维护自己的 Hint。 注意：线程池场景下若子线程需继承父线程 Hint，需显式传递。
 *
 * <p><b>实现原理：</b>{@code DynamicRoutingDataSource.determineCurrentLookupKey()} 在路由决策前优先检查当前线程的
...
public final class HintManager {
   ...
   * 强制后续数据库操作走主库。
   *
   * <p>等效于 {@code HintManager.set(new Hint(HintType.MASTER))}。 需在操作完成后调用 {@link #clear()} 或使用
