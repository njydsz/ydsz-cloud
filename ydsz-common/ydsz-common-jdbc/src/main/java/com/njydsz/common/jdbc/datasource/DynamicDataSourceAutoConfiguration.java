 ...
 * 将容器中已创建的默认数据源（HikariDataSource）作为路由数据源的默认目标
 * 与 {@code master} 目标注册，保证 {@link DynamicRoutingDataSource} 可直接使用；
 * 同时注册 {@code @DS} 注解切面与动态多数据源健康检查。
 *
 * <p>配置示例：
 * <pre>
 * ydsz:
...
public class DynamicDataSourceAutoConfiguration {
     ...
     * 注册动态路由数据源
     *
     * <p>将容器中唯一的默认数据源作为路由默认目标与 master 目标，
     * 使 {@link DynamicRoutingDataSource} 在单数据源场景下即可正常路由。
     *
