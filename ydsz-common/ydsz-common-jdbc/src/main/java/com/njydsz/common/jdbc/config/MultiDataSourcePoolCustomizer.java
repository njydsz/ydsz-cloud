 ...
 * 多数据源连接池参数定制器
 *
 * <p>当项目中使用了 {@link DynamicRoutingDataSource} 时，
 * 此 {@link SmartLifecycle} Bean 在所有数据源初始化完成后，遍历所有目标数据源，
 * 对每个 {@link HikariDataSource} 执行连接池参数定制。
 *
 * <p>基础连接池配置由 Spring Boot 原生自动配置处理，
 * 本配置仅服务于多数据源场景下的差异化定制需求。
 *
...
public class MultiDataSourcePoolCustomizer implements SmartLifecycle {
    ...
    public MultiDataSourcePoolCustomizer(DynamicRoutingDataSource dynamicRoutingDataSource,
     ...
     * 依次调用所有 {@link HikariCPPoolConfigurer}，最后将修改后的配置通过 MXBean 热更新。
     *
     * @param dsName       数据源名称
    ...
    private void applyPoolConfig(String dsName, HikariDataSource hikariDs,
        ...
        log.info("数据源 [{}] 连接池已定制: poolName={}, maxPoolSize={}, minIdle={}, connectionTimeout={}ms",
