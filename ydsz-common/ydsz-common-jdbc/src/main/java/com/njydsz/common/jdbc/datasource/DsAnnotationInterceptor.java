...
public class DsAnnotationInterceptor implements MethodInterceptor {
   ...
   *
   * <p>使用 {@link AnnotatedElementUtils#findMergedAnnotation} 替代 {@link
   * java.lang.reflect.Method#getAnnotation}， 自动搜索接口方法和父类上的注解，确保接口方法上标注的 @DS 也能被正确识别。
   */
  ...
  private String resolveDataSourceName(String value, MethodInvocation invocation) {
    ...
    try {
      ...
      String[] paramNames =
          Arrays.stream(invocation.getMethod().getParameters())
              .map(p -> p.getName())
              .toArray(String[]::new);
