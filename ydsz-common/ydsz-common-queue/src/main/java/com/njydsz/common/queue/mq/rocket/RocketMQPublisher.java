 ...
 * RocketMQ 消息发布者
 *
 * <p>使用 RocketMQ Producer API 实现消息发布功能。 支持同步、异步和延迟发送，提供完善的失败重试和状态校验机制。
 *
 * <p><b>技术特点：</b>
 *
 * <ul>
 *   <li>同步发送：带重试机制，失败时抛出 BusinessException
 *   <li>异步发送：基于 CompletableFuture，回调中校验 SendStatus
 *   <li>延迟消息：支持指定延迟级别的消息
 *   <li>高可靠：发送失败自动重试 3 次，SendStatus 非 SEND_OK 时抛异常
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
...
public class RocketMQPublisher implements IMessagePublisher {
  ...
  public RocketMQPublisher(RocketMQProperties properties, String topic, String tag) {
    ...
    this.maxRetryTimes =
        properties.getMaxRetryCount() > 0 ? properties.getMaxRetryCount() : DEFAULT_MAX_RETRY;
    this.sendTimeout = DEFAULT_SEND_TIMEOUT;
    this.producer = createProducer(properties);
    log.info(
        "[RocketMQ] 发布者初始化完成，topic={}, tag={}, namesrvAddr={}, maxRetry={}",
        topic,
        this.tag,
  ...
  public void publish(QueueMessage message) {
    ...
    for (int attempt = 0; attempt <= maxRetryTimes; attempt++) {
      try {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "[RocketMQ] 消息已发送，topic={}, traceId={}, msgId={}, status={}, attempt={}",
              topic,
              message.getTraceId(),
              result.getMsgId(),
        ...
        if (attempt < maxRetryTimes) {
          log.warn(
              "[RocketMQ] 消息发送失败，准备重试，topic={}, traceId={}, attempt={}/{}, error={}",
              topic,
              message.getTraceId(),
              attempt + 1,
              maxRetryTimes + 1,
    ...
    log.error(
        "[RocketMQ] 消息发布失败，已重试 {} 次，topic={}, traceId={}",
        maxRetryTimes,
        topic,
        message.getTraceId(),
        lastException);
    ...
    throw BusinessException.builder()
        .key("RocketMQ 消息发布失败，已重试 " + maxRetryTimes + " 次：" + errorMsg)
        .cause(lastException)
        .build();
  }
   ...
   * 异步发送消息
   *
   * <p>使用 RocketMQ 异步发送 API，返回 CompletableFuture 供调用方处理结果。 回调中会校验 SendStatus，非 SEND_OK 时记录错误日志并在
   * Future 中抛出异常。
   *
          ...
          new SendCallback() {
            ...
            public void onSuccess(SendResult result) {
              try {
                ...
                if (log.isDebugEnabled()) {
                  log.debug(
                      "[RocketMQ] 异步消息发送成功，topic={}, traceId={}, msgId={}, status={}",
                      topic,
                      message.getTraceId(),
                      result.getMsgId(),
                      result.getSendStatus());
                ...
                log.error(
                    "[RocketMQ] 异步消息发送状态异常，topic={}, traceId={}, status={}",
                    topic,
                    message.getTraceId(),
                    result.getSendStatus(),
                    e);
                future.completeExceptionally(
                    BusinessException.builder()
                        .key("RocketMQ 异步消息发送状态异常：" + e.getMessage())
                        .cause(e)
                        .build());
              }
            }
            ...
            public void onException(Throwable e) {
              log.error("[RocketMQ] 异步消息发送失败，topic={}, traceId={}", topic, message.getTraceId(), e);
              future.completeExceptionally(
                  BusinessException.builder()
                      .key("RocketMQ 异步消息发送失败：" + e.getMessage())
                      .cause(e)
                      .build());
            }
          });
    } catch (Exception e) {
      log.error("[RocketMQ] 异步消息发送初始化失败，topic={}, traceId={}", topic, message.getTraceId(), e);
      future.completeExceptionally(
          BusinessException.builder()
              .key("RocketMQ 异步消息发送初始化失败：" + e.getMessage())
              .cause(e)
              .build());
    }
    return future;
  ...
  public void publishDelayed(QueueMessage message, long delayMillis) {
    ...
    for (int attempt = 0; attempt <= maxRetryTimes; attempt++) {
      try {
        ...
        validateSendStatus(result, message.getTraceId(), topic);
        log.debug(
            "[RocketMQ] 延迟消息已发送，topic={}, traceId={}, delayLevel={}, msgId={}, status={}, attempt={}",
            topic,
            message.getTraceId(),
            delayLevel,
            result.getMsgId(),
        ...
        if (attempt < maxRetryTimes) {
          log.warn(
              "[RocketMQ] 延迟消息发送失败，准备重试，topic={}, traceId={}, attempt={}/{}, error={}",
              topic,
              message.getTraceId(),
              attempt + 1,
              maxRetryTimes + 1,
    ...
    log.error(
        "[RocketMQ] 延迟消息发布失败，已重试 {} 次，topic={}, traceId={}",
        maxRetryTimes,
        topic,
        message.getTraceId(),
        lastException);
    String errorMsg = lastException != null ? lastException.getMessage() : "未知错误";
    throw BusinessException.builder()
        .key("RocketMQ 延迟消息发布失败，已重试 " + maxRetryTimes + " 次：" + errorMsg)
        .cause(lastException)
        .build();
  }
  ...
  public void publishBatch(List<QueueMessage> messages) {
    ...
    for (int attempt = 0; attempt <= maxRetryTimes; attempt++) {
      try {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "[RocketMQ] 批量消息已发送，topic={}, count={}, msgId={}, attempt={}",
              topic,
              rocketMessages.size(),
              result.getMsgId(),
              attempt + 1);
        ...
        if (attempt < maxRetryTimes) {
          log.warn(
              "[RocketMQ] 批量消息发送失败，准备重试，topic={}, count={}, attempt={}/{}, error={}",
              topic,
              rocketMessages.size(),
              attempt + 1,
              maxRetryTimes + 1,
    ...
    log.error(
        "[RocketMQ] 批量消息发布失败，已重试 {} 次，topic={}, count={}",
        maxRetryTimes,
        topic,
        rocketMessages.size(),
        lastException);
    String errorMsg = lastException != null ? lastException.getMessage() : "未知错误";
    throw BusinessException.builder()
        .key("RocketMQ 批量消息发布失败，已重试 " + maxRetryTimes + " 次：" + errorMsg)
        .cause(lastException)
        .build();
  }
  ...
  public void publishSequential(QueueMessage message) {
    ...
    for (int attempt = 0; attempt <= maxRetryTimes; attempt++) {
      try {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "[RocketMQ] 顺序消息已发送，topic={}, traceId={}, groupKey={}, msgId={}, status={}, attempt={}",
              topic,
              message.getTraceId(),
              groupKey,
              result.getMsgId(),
        ...
        if (attempt < maxRetryTimes) {
          log.warn(
              "[RocketMQ] 顺序消息发送失败，准备重试，topic={}, traceId={}, attempt={}/{}, error={}",
              topic,
              message.getTraceId(),
              attempt + 1,
              maxRetryTimes + 1,
    ...
    log.error(
        "[RocketMQ] 顺序消息发布失败，已重试 {} 次，topic={}, traceId={}, groupKey={}",
        maxRetryTimes,
        topic,
        message.getTraceId(),
        message.getMessageGroupKey(),
        lastException);
    String errorMsg = lastException != null ? lastException.getMessage() : "未知错误";
    throw BusinessException.builder()
        .key("RocketMQ 顺序消息发布失败，已重试 " + maxRetryTimes + " 次：" + errorMsg)
        .cause(lastException)
        .build();
  }
   ...
   * 校验发送结果状态
   *
   * <p>检查 SendResult.getSendStatus() 是否为 SEND_OK。 非 SEND_OK 时抛出异常，防止消息静默丢失。
   */
  private void validateSendStatus(SendResult result, String traceId, String topicName) {
    if (result == null) {
      throw BusinessException.builder()
          .key("RocketMQ 发送结果为空，topic=" + topicName + ", traceId=" + traceId)
          .build();
    }
    ...
    if (status != SendStatus.SEND_OK) {
      String errorMsg =
          String.format(
              "RocketMQ 发送状态异常，topic=%s, traceId=%s, msgId=%s, status=%s",
   ...
   * 计算延迟级别
   *
   * <p>RocketMQ 支持的延迟级别： 1=1s, 2=5s, 3=10s, 4=30s, 5=1m, 6=2m, 7=3m, 8=4m, 9=5m, 10=6m, 11=7m,
   * 12=8m, 13=9m, 14=10m, 15=20m, 16=30m, 17=1h, 18=2h
   */
