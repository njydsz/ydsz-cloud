 ...
 * 延时消息发送器接口
 *
 * <p>定义延时消息的统一语义抽象，屏蔽底层 MQ 实现差异。
 * 不同 MQ 对延时消息的支持能力不同：
 * <ul>
 *   <li>RocketMQ：原生支持 18 个延迟级别</li>
 *   <li>Kafka：需配合外部延时队列实现</li>
 *   <li>Redis：需配合 ZSet + 轮询实现</li>
 * </ul>
 *
...
public interface DelayedMessageSender {
     ...
     * 发送延时消息
     *
     * <p>消息将在指定的延迟时间后被投递到目标队列。
     * 如果底层 MQ 不支持原生延时，实现类应自动降级到本地定时器方案。
     *
