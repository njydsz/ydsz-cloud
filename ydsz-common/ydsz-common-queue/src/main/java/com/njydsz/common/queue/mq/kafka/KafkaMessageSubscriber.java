 ...
 * Kafka 消息订阅者
 *
 * <p>使用 Kafka Consumer API 实现消息消费功能。 支持同步拉取和异步持续消费两种模式。
 *
 * <p><b>技术特点：</b>
 *
 * <ul>
 *   <li>消费组支持：多个消费者协同消费实现负载均衡
 *   <li>偏移量管理：支持手动提交偏移量实现精确控制
 *   <li>消息回溯：从earliest或latest位置开始消费
 *   <li>分区策略：同一分区消息有序
 * </ul>
 *
 * <p><b>消费模式：</b>
 *
 * <ul>
 *   <li>同步拉取：调用 subscribe() 阻塞等待单条消息
 *   <li>异步持续消费：调用 subscribeAsync(handler) 后台线程持续消费
 * </ul>
 *
 * <p><b>偏移量提交：</b>
 *
 * <ul>
 *   <li>enableAutoCommit=false：手动提交偏移量
 *   <li>消息处理成功后自动提交
 *   <li>保证至少一次消费语义
 * </ul>
 *
...
public class KafkaMessageSubscriber implements IMessageSubscriber {
  ...
  private void processRecord(ConsumerRecord<String, String> record, IMessageHandler handler) {
    ...
    try {
      ...
      lastError.set(null);
      log.debug(
          "[Kafka] 消息处理成功，topic={}, partition={}, offset={}, traceId={}",
          record.topic(),
          record.partition(),
          record.offset(),
          message.getTraceId());
