 ...
 *
 * <p>使用 Redis <b>SUBSCRIBE</b> 命令订阅指定频道，接收发布者广播的消息。 与 {@link RedisStreamSubscriber} 不同，PubSub
 * 模式为「即发即忘」广播， 不支持消息确认、重试和死信队列，适用于实时通知、缓存失效等允许丢消息的场景。
 *
 * <h3>工作原理</h3>
 *
 * <ol>
 *   <li>通过 {@link RedisMessageListenerContainer} 注册 {@link MessageListener}
 *   <li>消息到达时调用 {@link ConsumerRateLimiter#acquire()} 限流后处理
 *   <li>处理异常仅记录日志和 lastError，不重试（PubSub 无 ACK 机制）
 * </ol>
 *
 * <h3>线程安全</h3>
 *
 * <p>{@link AtomicBoolean} 控制运行状态，{@link RedisMessageListenerContainer} 内部使用 Spring TaskExecutor
 * 调度消息回调。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class RedisPubSubSubscriber implements IMessageSubscriber {
   ...
   * {@inheritDoc}
   *
   * <p>创建 {@link RedisMessageListenerContainer} 并注册 {@link MessageListener}， 底层通过 Redis SUBSCRIBE
   * 命令建立长连接。消息到达时先限流再处理。 重复调用时仅记录警告。
   *
   ...
   *
   * <p>消息回调流程：限流 → 解析消息体 → 调用 handler → 记录指标。 异常仅记录 lastError，不重试（PubSub 无 ACK 机制）。
   *
   * @param handler 消息处理回调
   */
   ...
   * 处理单条 PubSub 消息。
   *
   * <p>将原始字符串解析为 {@link QueueMessage}，然后调用 handler.onMessage()。 解析失败时降级为原始字符串消息。
   *
   ...
   *
   * <p>依次调用 stop() 和 destroy()，异常仅记录警告不向上抛出， 确保关闭流程不会因单个订阅者失败而中断。
   */
