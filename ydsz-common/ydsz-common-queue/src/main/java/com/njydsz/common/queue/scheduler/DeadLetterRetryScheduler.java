 ...
 *
 * <ul>
 *   <li>默认每 60 秒执行一次扫描（可通过 deadLetterRetryInterval 配置）
 *   <li>每次扫描附加随机抖动（0~30% 的间隔时间），避免多实例同时扫描造成惊群
 *   <li>每次扫描遍历所有主题的死信消息
 *   <li>超过最大重试次数的消息将被永久删除
 *   <li>记录每次重试的详细日志
 * </ul>
 *
 * <p><b>配置项：</b>
...
public class DeadLetterRetryScheduler implements DisposableBean {
   ...
   * 调度下一次扫描任务
   *
   * <p>使用 programmatic scheduling 替代 {@code @Scheduled}， 以便在每次执行后动态计算带抖动的下次执行时间。
   */
  private void scheduleNext() {
    ...
    scheduledFuture =
        scheduler.schedule(this::scanAndRetryWithReschedule, delay, TimeUnit.MILLISECONDS);
   ...
   * 计算随机抖动值
   *
   * <p>抖动范围为 {@code [0, interval * jitterPercent / 100]}， 多实例部署时各实例的抖动值不同，避免同时扫描造成惊群。
   *
