 ...
 * 消息队列自动配置类
 *
 * <p>提供消息队列模块的 Spring Boot 自动配置能力。 当配置项 {@code ydsz.queue.enabled=true} 时自动启用。
 *
 * <p><b>Redis 连接复用：</b> 优先使用 ydsz-common-redis 的 RedisTemplate 复用 Redis 连接， 当 RedisTemplate 不可用时回退到
 * QueueProperties 中的连接配置自建 JedisPool。
 *
 * <p><b>配置示例：</b>
 *
 * <pre>{@code
...
public class QueueConfiguration {
  ...
  public void init() {
    ...
    if (redisTemplate != null) {
      ...
      log.info(
          "[Queue] 消息队列配置初始化，Redis 连接：自建 JedisPool（{}:{}）",
          queueProperties.getHost(),
          queueProperties.getPort());
      log.info(
          "[Queue] 提示：推荐引入 ydsz-common-redis 模块以复用 Redis 连接，" + "通过 spring.data.redis.* 配置连接信息");
    }
   ...
   * 创建消息队列管理器
   *
   * <p>提供统一的队列生命周期管理和监控指标收集。 负责注册、查询、移除队列实例及其对应的监控指标。
   *
   ...
   *
   * <p>通过 {@code @ConditionalOnMissingBean} 允许业务方通过 {@code ydsz.thread.pools.queueConsumerExecutor}
   * 注入 ydsz-common-thread 统一管理线程池覆盖本默认实现。
   *
   * <p><b>线程池管理规范：</b> 本方法仅在 ydsz-common-thread 未提供 {@code queueConsumerExecutor} Bean 时生效。
   ...
   * 创建消息队列提供者
   *
   * <p>消息队列提供者负责根据 QueueType 创建对应的队列实例。 优先注入 ydsz-common-redis 的 RedisTemplate 复用连接， 当
   * RedisTemplate 不可用时回退到自建 JedisPool 连接。
   *
   ...
   * 创建死信队列自动重试调度器
   *
   * <p>当死信队列服务可用且启用了自动重试时，创建定时调度器实例。 调度器使用带抖动的延迟策略，避免多实例同时扫描造成惊群。
   *
   ...
   * 创建内存消息去重器（单实例场景）
   *
   * <p>当 ydsz.queue.dedup-enabled=true 且 RedisTemplate 不可用时创建。 分布式场景应使用 ydsz-common-redis 的
   ...
   * 创建消息队列健康检查器
   *
   * <p>当 spring-boot-health 模块在 classpath 中时自动注册。 Redis 类型复用 RedisStringOps 连接检查，非 Redis 类型通过 TCP
   * 端口连通性检查。
   *
   * @return 健康检查器实例
   */
   ...
   * 注册消息队列 Actuator 端点
   *
   * <p>提供 /actuator/queues 端点，用于查询消息队列运行状态。 需要 spring-boot-actuator 依赖。
   *
   ...
   *
   * <p>当 classpath 中存在 Micrometer MeterRegistry 时自动注册。 将 QueueManager 中所有队列的 MessageMetrics 暴露为
   * Prometheus 指标。
   *
   * @param queueManager 队列管理器
  ...
  public QueueMetricsBinder queueMetricsBinder(
      QueueManager queueManager, ObjectProvider<MeterRegistry> meterRegistryProvider) {
