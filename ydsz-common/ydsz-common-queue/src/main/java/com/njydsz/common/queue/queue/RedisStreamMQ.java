 ...
 *
 * <p>Redis Stream 是 Redis 5.0 引入的数据结构，提供了更强大的消息持久化和消费组功能。 它是 List 的增强版，支持消息确认、消费组、消息ID排序等高级特性。
 *
...
public class RedisStreamMQ extends AbstractMessageQueue {
  ...
  public RedisStreamMQ(
    ...
    log.info("[RedisStreamMQ] 初始化成功（复用 ydsz-common-redis 连接），消费者组: {}", config.getStreamGroup());
  }
