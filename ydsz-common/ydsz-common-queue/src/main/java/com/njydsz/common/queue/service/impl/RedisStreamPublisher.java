 ...
 *
 * <p>使用 Redis <b>XADD</b> 命令将消息写入 Stream，实现可靠的消息发布。 与 PubSub 的「即发即忘」不同，Stream 模式消息持久化在 Redis 中，
 * 消费者通过消费组读取并确认，保证消息不丢失。
 *
 * <h3>支持的消息属性</h3>
 *
 * <ul>
 *   <li>{@code payload}：消息体（JSON 序列化）
 *   <li>{@code traceId}：链路追踪 ID，贯穿全链路日志
 *   <li>{@code retryCount}：重试计数，由消费端在重试时回写
 *   <li>{@code groupKey} + {@code sequence}：顺序消息分组键和序号
 * </ul>
 *
 * <h3>批量发布</h3>
 *
...
public class RedisStreamPublisher implements IMessagePublisher {
   ...
   * {@inheritDoc}
   *
   * <p>将消息序列化为字段 Map 后通过 XADD 写入 Stream。包含 payload、traceId、 retryCount 以及顺序消息字段（如果消息标记为
   * sequential）。
   *
  ...
  public void publish(QueueMessage message) {
    ...
    ObjectRecord<String, Map<String, String>> record =
        StreamRecords.newRecord().ofObject(fields).withStreamKey(channel);
   ...
   * {@inheritDoc}
   *
   * <p>使用 Redis Pipeline 批量写入多条消息，减少网络往返开销。 每条消息的字段组装逻辑与 {@link #publish(QueueMessage)} 一致。
   *
  ...
  public void publishBatch(List<QueueMessage> messages) {
    ...
    for (QueueMessage message : messages) {
      ...
      MapRecord<String, String, String> record =
          StreamRecords.mapBacked(fields).withStreamKey(channel);
      records.add(record);
    }
