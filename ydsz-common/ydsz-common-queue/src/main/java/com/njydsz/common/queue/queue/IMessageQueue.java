 ...
 * 消息队列接口
 *
 * <p>定义消息队列的标准操作，负责创建消息发布者和订阅者。 该接口是队列实现的抽象层，支持多种消息队列模式（如 Redis List、PubSub、Stream）。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
 ...
 *
 * <ul>
 *   <li>createPublisher: 创建消息发布者，用于发送消息到队列
 *   <li>createSubscriber: 创建消息订阅者，用于从队列接收消息
 *   <li>close: 关闭队列连接，释放相关资源
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public interface IMessageQueue extends AutoCloseable {
   ...
   * 创建消息发布者
   *
   * <p>发布者负责将消息发送到指定的队列通道。 同一队列可以创建多个发布者实例。
   *
   ...
   * 创建消息订阅者
   *
   * <p>订阅者负责从指定的队列通道接收消息。 订阅者支持同步拉取和异步监听两种消费模式。
   *
   ...
   * 获取队列通道列表
   *
   * <p>返回当前队列实例管理的所有通道名称。 该方法为可选实现，某些队列模式可能不支持。
   *
   ...
   * 关闭队列连接
   *
   * <p>释放所有与队列相关的资源，包括连接池、通道等。 关闭后不应再使用该队列实例进行任何操作。
   */
  default void close() {}
   ...
   *
   * <p>在执行发布、订阅等操作前调用此方法， 防止在队列关闭后继续进行操作。
   *
