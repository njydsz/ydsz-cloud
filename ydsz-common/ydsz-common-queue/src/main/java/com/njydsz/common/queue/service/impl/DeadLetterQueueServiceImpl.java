...
public class DeadLetterQueueServiceImpl implements DeadLetterQueueService {
  ...
  public void sendToDeadLetter(
    ...
    log.info(
        "[DeadLetterQueue] 消息进入死信队列: topic={}, messageId={}, reason={}",
        topic,
        messageId,
        failureReason);
  }
  ...
  public boolean retry(String topic, String messageId) {
    ...
    DeadLetterMessage dlqMessage =
        YdszJson.fromJson(dlqMessageObj.toString(), DeadLetterMessage.class);
    ...
    try {
      ...
      QueueMessage toPublish =
          queueMessage != null ? queueMessage : QueueMessage.of(dlqMessage.getMessageBody());
      ...
      log.info(
          "[DeadLetterQueue] 消息重试成功: topic={}, messageId={}, retryCount={}",
          topic,
          messageId,
          currentRetryCount + 1);
      return true;
      ...
      log.error(
          "[DeadLetterQueue] 消息重试失败: topic={}, messageId={}, error={}",
          topic,
          messageId,
          e.getMessage(),
          e);
        ...
        for (Map.Entry<Object, Object> entry : entries.entrySet()) {
          ...
          try {
            DeadLetterMessage msg =
                YdszJson.fromJson(entry.getValue().toString(), DeadLetterMessage.class);
