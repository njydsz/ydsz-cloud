 ...
 *
 * <ul>
 *   <li><b>本类（QueueMetrics）</b>：队列技术层监控，不依赖 Spring/Micrometer，纯 POJO 实现。
 *   <li><b>MessageMetrics</b>：业务层监控（继承 SentryMetricsAdapter + Micrometer），
 *       面向业务运营（send/retry/dead/receipt 等业务语义指标）。
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
...
public class QueueMetrics {
  ...
  public String getSummary() {
    return String.format(
        ...
        queueName,
        queueType,
        publishSuccessCount.sum(),
        publishFailCount.sum(),
        getAvgPublishQps(),
        getAvgPublishLatency(),
        getMaxPublishLatency(),
        consumeSuccessCount.sum(),
        consumeFailCount.sum(),
        getAvgConsumeQps(),
        getAvgConsumeLatency(),
