 ...
 * 跨模块消息通道常量注册中心
 *
 * <p>集中管理所有业务模块的 MQ 通道名称常量，消除各模块重复定义。
 * 通道全局唯一，作为 MQ 中间件的命名契约。
 *
 * <p><b>通道命名规范</b>：{@code ydsz:{模块}:{用途}}
 * <ul>
 *   <li>单词分隔符使用 {@code :}（冒号），与 RocketMQ/Kafka Topic 命名体系兼容</li>
 *   <li>单词内部使用 {@code -}（短横线），符合 DNS 友好的命名习惯</li>
 * </ul>
 *
 * <p><b>新增通道步骤</b>：
 * <ol>
 *   <li>在本类对应分组中定义通道常量</li>
 *   <li>更新对应模块的 {@code application.yml} 配置</li>
 *   <li>在使用方的 {@code ChannelConstants} 中引用本类常量</li>
 * </ol>
 *
 * <p>各业务模块可以保留本地的 {@code XxxQueueChannels} 类作为语义别名，
 * 但值必须引用本类常量而非重复定义字符串。
 *
 * @author ydsz-team
 * @since 1.0.0
