 ...
 * 消费组 Rebalance 监控器
 *
 * <p>周期性扫描 Redis Stream 消费组的消费者列表，检测消费者加入/离开事件，
 * 并触发 {@link ConsumerGroupRebalanceListener} 回调。
 *
 * <p><b>工作原理：</b>
 * <ol>
...
public class RebalanceMonitor implements DisposableBean {
     ...
     */
    public RebalanceMonitor(RedisTemplate<String, Object> redisTemplate,
     ...
     */
    public RebalanceMonitor(RedisTemplate<String, Object> redisTemplate,
                             String channel, String groupName, long scanIntervalSeconds) {
        ...
        this.scanIntervalSeconds = scanIntervalSeconds > 0 ? scanIntervalSeconds : DEFAULT_SCAN_INTERVAL_SECONDS;
    }
    ...
    public void start() {
        ...
        log.info("[RebalanceMonitor] 监控器已启动，channel={}, group={}, interval={}s",
                channel, groupName, scanIntervalSeconds);
    }
    ...
    private void scanAndDetectChanges() {
        try {
            ...
            log.warn("[RebalanceMonitor] 扫描消费组异常，channel={}, group={}, error={}",
    ...
    private void fireEvent(String consumerName, EventType eventType, int totalConsumers) {
        ConsumerGroupEvent event = ConsumerGroupEvent.builder()
                .groupName(groupName)
                .channel(channel)
        ...
        for (ConsumerGroupRebalanceListener listener : listeners) {
            try {
                ...
                log.warn("[RebalanceMonitor] 监听器执行异常, eventType={}, listener={}, error={}",
                        eventType, listener.getClass().getSimpleName(), e.getMessage());
            }
        }
        log.info("[RebalanceMonitor] 消费组事件: type={}, consumer={}, total={}, channel={}, group={}",
                eventType, consumerName, totalConsumers, channel, groupName);
    }
}
