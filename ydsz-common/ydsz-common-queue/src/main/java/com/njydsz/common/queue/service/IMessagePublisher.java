 ...
 * 消息发布者接口（精简版）。
 *
 * <p>定义消息队列发布者的核心能力：单条发布、结构化发布、批量发布。
 * 组合操作（顺序消息、延迟发布等）由 {@link MessagePublisherHelper} 提供。
 *
 * <p><b>线程安全：</b>实现类应保证 publish 方法的线程安全，
 * 支持多线程并发发布。
 *
 * <p><b>使用示例：</b>
 * <pre>{@code
...
public interface IMessagePublisher extends Closeable {
     ...
     * 发布结构化消息。
     *
     * <p>将消息序列化后写入目标队列。消息的 traceId、retryCount、
     * 顺序消息字段等元数据会随消息一起传递。
     *
