 ...
 * 去重 Key 定时清理调度器
 *
 * <p>定期清理内存去重器中的过期记录，避免内存泄漏。 Redis 去重器已使用 TTL 自动过期，无需额外清理。
 *
 * <p><b>清理策略：</b>
 *
 * <ul>
 *   <li>每 10 分钟执行一次清理
 *   <li>清理 {@link MessageDeduplicator} 中超时的去重记录
 *   <li>记录清理前后的记录数量变化
 * </ul>
 *
 * <p><b>注意事项：</b>
 *
 * <ul>
 *   <li>仅对内存去重器有效，Redis 去重器依赖 TTL 自动过期
 *   <li>清理间隔可通过 spring.task.scheduling 配置调整
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
