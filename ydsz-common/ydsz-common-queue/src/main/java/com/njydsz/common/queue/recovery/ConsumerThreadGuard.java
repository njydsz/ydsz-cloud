 ...
 *
 * <p><b>线程池模式（推荐）：</b> 当传入 {@link ExecutorService} 时，任务由 Spring 管理的线程池执行， 避免直接创建裸线程，支持优雅停机。
 *
 * <p><b>裸线程模式（已弃用）：</b> 当未传入 {@link ExecutorService} 时，内部会创建守护线程执行任务， 仅用于向后兼容，不推荐在新代码中使用。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
