 ...
 * 消息中心通道常量定义
 *
 * <p>统一定义消息中心（ydsz-message）使用的所有 MQ Topic 和消费组常量。
 * 遵循通道命名规范：{@code ydsz:message:<用途>}，其中：
 * <ul>
 *   <li>消息 Topic：{@code ydsz:message:topic}</li>
 *   <li>批量消息 Topic：{@code ydsz:message:topic-batch}</li>
 *   <li>死信队列 Topic：{@code ydsz:message:dlq}</li>
 * </ul>
 *
 * <p>Topic 名称全局唯一，作为消息中心与 RocketMQ/Kafka 等 MQ 中间件的契约。
 * 如需新增通道，请同步更新本类和对应业务模块的配置文件。
 *
 * <p><b>命名规范</b>：
 * <ul>
 *   <li>Topic：{@code ydsz:{module}:{topic-name}}</li>
 *   <li>消费组：{@code ydsz:{module}:group-{用途}}</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public final class YdszMessageTopics {
     ...
     * 消息投递 Topic（单条消息）
     *
     * <p>格式：{@code ydsz:message:topic}，支持带优先级 Tag（URGENT/HIGH/NORMAL/LOW），
     * destination 格式为 {@code topic:tag}。
     */
     ...
     * 死信队列 Topic
     *
     * <p>重试耗尽（maxReconsumeTimes）的消息将路由到此 Topic，
     * 由 {@code MessageDlqConsumer} 消费并落库 status=DEAD。
     */
