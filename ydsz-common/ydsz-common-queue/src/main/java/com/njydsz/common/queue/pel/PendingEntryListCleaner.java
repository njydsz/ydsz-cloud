 ...
 *
 * <p>Redis Stream 消费组中，已被读取但未 ACK 的消息会积累在 PEL 中。 当消费者崩溃或处理异常时，PEL 可能无限膨胀导致内存问题。
 *
 * <p>本清理器提供以下能力：
 *
 * <ul>
 *   <li>扫描 PEL 中超时的 Pending 消息
 *   <li>支持清理策略：重试 / ACK 丢弃 / 转死信
 *   <li>生成 PEL 统计报告
 * </ul>
 *
 * @author ydsz-team
...
public class PendingEntryListCleaner {
  ...
  public List<PendingEntryInfo> scanStaleEntries(Duration idleThreshold) {
    ...
    try {
      ...
      for (Map.Entry<String, Long> entry : summary.getPendingMessagesPerConsumer().entrySet()) {
        ...
        if (count != null && count > 0) {
          ...
          PendingMessages pendingMessages =
              redisTemplate.opsForStream().pending(channel, consumer, Range.unbounded(), count);
          if (pendingMessages != null) {
            for (PendingMessage pending : pendingMessages) {
              if (pending.getElapsedTimeSinceLastDelivery().compareTo(idleThreshold) > 0) {
                staleEntries.add(
                    PendingEntryInfo.builder()
                        ...
                        .firstDeliveryTime(
                            LocalDateTime.now().minus(pending.getElapsedTimeSinceLastDelivery()))
                        .build());
              }
            }
          }
      ...
      log.debug(
          "[PEL-Cleaner] 扫描完成，channel={}, group={}, staleCount={}",
          channel,
          groupName,
          staleEntries.size());
    } catch (Exception e) {
      log.warn(
          "[PEL-Cleaner] 扫描 PEL 异常，channel={}, group={}, error={}",
          channel,
          groupName,
          e.getMessage());
    }
  ...
  public int cleanStaleEntries(Duration idleThreshold, int maxRetryAttempts) {
    ...
    try {
      ...
      for (PendingEntryInfo entry : staleEntries) {
        if (entry.getDeliveryCount() > maxRetryAttempts) {
          ...
          cleanedCount++;
          log.info(
              "[PEL-Cleaner] ACK 丢弃超时消息，channel={}, entryId={}, deliveryCount={}",
              channel,
              entry.getEntryId(),
          ...
          if (retryEntry(entry.getEntryId())) {
            cleanedCount++;
            log.info(
                "[PEL-Cleaner] 重试超时消息，channel={}, entryId={}, deliveryCount={}",
                channel,
                entry.getEntryId(),
                entry.getDeliveryCount());
          }
      ...
      log.warn(
          "[PEL-Cleaner] 清理 PEL 异常，channel={}, group={}, error={}",
          channel,
          groupName,
          e.getMessage());
    }
    if (cleanedCount > 0) {
      log.info(
          "[PEL-Cleaner] 清理完成，channel={}, group={}, cleanedCount={}",
          channel,
          groupName,
          cleanedCount);
    }
  ...
  public PelStatistics getStatistics() {
    try {
      ...
      log.warn(
          "[PEL-Cleaner] 获取统计异常，channel={}, group={}, error={}",
          channel,
          groupName,
          e.getMessage());
      return PelStatistics.empty(channel, groupName);
