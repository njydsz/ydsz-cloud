 ...
 * <p>支持两种使用模式：
 * <ul>
 *   <li><b>静态方法</b>：直接注入/提取/清理 traceId（兼容旧 API）</li>
 *   <li><b>try-with-resources</b>：进入上下文时设置 traceId，退出时自动恢复/清除</li>
 * </ul>
 *
 * <p>典型用法：
 * <pre>{@code
...
public class MessageTracer {
     ...
     * 清除当前线程上下文中的 traceId
     *
     * <p>注意：RequestContext 中的 traceId 仅在有上层管理时才清除。
     * 静态清理方法会同时清理 MDC 和 RequestContext。
     */
     ...
     *
     * <p>退出时自动恢复进入前的 traceId 或清除（无原始 traceId 时）。
     * traceId 为 null / 空白时自动生成新 traceId。
     *
     ...
     *
     * <p>进入时保存当前 traceId 并设置新 traceId，
     * 退出时恢复原 traceId 或清除。
     */
