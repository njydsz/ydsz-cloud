...
public class RedisListSubscriber implements IMessageSubscriber {
  ...
  /** 备份队列最大长度限制，超过此值时触发清理和告警 默认 10000 条 */
                ...
                connection -> {
                  byte[] result =
                      connection
                          .listCommands()
                          .bRPopLPush(
                              blockTimeoutSeconds, channel.getBytes(), backupChannel.getBytes());
                  return result;
                },
                true);
        if (message != null) {
  ...
  private void trimBackupQueueIfExceeded() {
    try {
      ...
      if (size != null && size > maxBackupQueueSize) {
        ...
        log.warn(
            "[RedisList] 备份队列长度超限({}>{})，已清理 {} 条最旧消息，channel={}",
            size,
            maxBackupQueueSize,
            trimCount,
            channel);
  ...
  private void processMessage(String message, IMessageHandler handler) {
    ...
    try {
      ...
      if (queueMessage.isExpired()) {
        log.warn(
            "[RedisList] 消息已过期，丢弃处理，channel={}, traceId={}, expireMillis={}",
            channel,
            queueMessage.getTraceId(),
            queueMessage.getHeader("expireMillis"));
        ackSuccess = true;
        return;
      }
      if (handler != null) {
