...
- **v1.1.0**（2026-08-16）：接口精简（IMessagePublisher/Subscriber 10→4 方法）；移除过度设计组件（熔断器、消息轨迹、自动压缩等）；`common-redis` 升为必需依赖；新增 `dead-letter-retry-jitter-percent`（抖动延迟）、`dedup-enabled`（去重开关）、`dedup-window-millis`（去重窗口）配置项；新增 `QueueEndpoint` Actuator 端点、`QueueMetricsBinder` Micrometer 桥接；`@EnableScheduling` 移至 `QueueConfiguration`
