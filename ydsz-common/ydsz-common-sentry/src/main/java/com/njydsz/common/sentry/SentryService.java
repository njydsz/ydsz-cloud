 ...
 * 可观测性服务（Spring Bean）。
 *
 * <p>作为 {@link SentryObservation} 的实例方法版替代，通过构造器注入 SPI 实现， 支持依赖注入、AOP 拦截、Mock 替换等 Spring 生态能力。
 *
 * <p>业务方可选择：
 *
 * <ul>
 *   <li>注入 {@link SentryService}：享受完整的 DI 能力，便于单元测试替换
...
public class SentryService {
   ...
   * 构造器注入所有 SPI 实现。
   *
   * <p>任一参数可为 {@code null}：Spring 在找不到对应 Bean 时会传入 null， 各方法内部已做 null 安全处理。
   *
   ...
   * @param operation 要执行的操作
   */
  public void time(String name, String description, Map<String, String> tags, Runnable operation) {
    long start = System.currentTimeMillis();
    try {
      operation.run();
    } finally {
