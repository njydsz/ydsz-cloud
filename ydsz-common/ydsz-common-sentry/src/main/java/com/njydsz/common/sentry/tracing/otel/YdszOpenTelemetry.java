 ...
 *
 * <ul>
 *   <li>统一的 Tracer 获取（支持命名空间隔离）
 *   <li>W3C TraceContext + B3 + Baggage 多协议上下文传播
 *   <li>HTTP / Feign / 自定义载体的 Propagation 工具
 * </ul>
 *
 * <p>典型用法：
 *
...
public final class YdszOpenTelemetry {
  ...
  public static Tracer tracer(String instrumentationName) {
    ...
    return TRACER_CACHE.computeIfAbsent(
        instrumentationName, name -> openTelemetry().getTracer(name));
  }
