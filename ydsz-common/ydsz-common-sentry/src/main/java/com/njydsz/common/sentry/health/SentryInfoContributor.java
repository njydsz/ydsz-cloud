 ...
 * Sentry 模块运行时元数据暴露。
 *
 * <p>通过 {@code /actuator/info} 端点暴露当前生效的 SPI 实现类名、采样率、队列大小等运行时元数据，
 * 便于运维人员在不停机的情况下确认可观测性模块的实际工作状态。
 *
 * <p>暴露内容包括：
 * <ul>
 *   <li>metrics.collector：指标采集器名称与可用性</li>
 *   <li>logging.publisher：日志发布器名称与方案</li>
 *   <li>tracing.tracer：链路追踪系统名称</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class SentryInfoContributor implements InfoContributor {
    ...
    public void contribute(Info.Builder builder) {
        ...
        if (logPublisher != null) {
            ...
            // 双发子发布器摘要
            LogPublisher delegate = logPublisher instanceof AsyncLogPublisher async
                    ? async.getDelegate() : logPublisher;
