...
/**
 * YDSZ 熔断降级保护器（基于 Resilience4j）。
 *
 * <p>底层委托 Resilience4j {@link CircuitBreaker}，提供滑动窗口失败率统计、状态自动流转、半开探测等标准熔断能力。
 *
 * <p>状态流转：
 *
 * <ul>
 ...
 */
@Slf4j
public class SentryCircuitBreaker {
  ...
  private static final CircuitBreaker NOOP_DELEGATE =
      CircuitBreaker.ofDefaults("noop");
  ...
  public SentryCircuitBreaker(
    ...
    log.info(
        "[Sentry] SentryCircuitBreaker '{}' 初始化完成: threshold={}, window={}s, halfOpenAfter={}ms",
        name,
  ...
  public SentryCircuitBreaker(
    ...
    log.info("[Sentry] SentryCircuitBreaker '{}' 初始化完成（共享 Registry）", name);
  }

  private SentryCircuitBreaker(String name, boolean noop) {
    this.name = name;
   ...
   * @param config Resilience4j 熔断器配置
   * @param registry Resilience4j 注册中心
   * @return SentryCircuitBreaker 实例
   */
  public static SentryCircuitBreaker fromRegistry(
      String name,
      CircuitBreakerConfig config,
      CircuitBreakerRegistry registry) {
    return new SentryCircuitBreaker(name, config, registry);
  }
  ...
  public <T> T execute(Supplier<T> operation, Supplier<T> fallback) {
    try {
      return CircuitBreaker.decorateSupplier(delegate, operation).get();
  ...
  public void execute(Runnable operation, Runnable fallback) {
    try {
      CircuitBreaker.decorateRunnable(delegate, operation).run();
    } catch (CallNotPermittedException e) {
      fallback.run();
    }
  }
  ...
  public void recordFailure(long duration, TimeUnit unit, Throwable throwable) {
    delegate.onError(
        duration,
        unit,
        throwable != null ? throwable : new RuntimeException("SentryCircuitBreaker recorded failure"));
  }
