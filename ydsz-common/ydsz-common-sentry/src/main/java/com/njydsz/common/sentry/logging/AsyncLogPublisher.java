 ...
 *
 * <ul>
 *   <li>有界队列：默认容量 8192，队列满时丢弃最旧日志（背压降级）
 *   <li>批量发送：每 {@code batchSize} 条或每 {@code flushIntervalMillis} 毫秒批量发送
 *   <li>令牌桶限流：限制每秒最大发送量，防止日志风暴打爆下游
 *   <li>优雅关闭：{@code close()} 时 drain 剩余队列并 flush
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class AsyncLogPublisher implements LogPublisher, AutoCloseable {
  ...
  public AsyncLogPublisher(
    ...
    log.info(
        "[Sentry] AsyncLogPublisher 初始化: delegate={}, queueCapacity={}, batchSize={}, "
   ...
   * 令牌桶限流等待。
   *
   * <p>采用配额借贷模式：允许短期突发超过速率上限（透支下一周期配额）， 避免消费线程自旋阻塞；同时设置最大等待时间防止无限等待。
   *
   * @param needed 所需令牌数
   */
  private void waitForTokens(int needed) {
    ...
    while (running) {
      ...
      long waitTime = maxRatePerSecond > 0 ? TOKEN_REFILL_PERIOD_MILLIS / maxRatePerSecond : 1;
      if (waitTime <= 0) {
  ...
  public void close() {
    ...
    log.info(
        "[Sentry] AsyncLogPublisher 已关闭, 丢弃总数={}, 发布总数={}",
        droppedCount.get(),
        totalPublished.get());
  }
}
