 ...
 *
 * <p><b>注意</b>：本类不直接 import OTLP Exporter 类，避免在缺少 otel-exporter-* 依赖时无法启动。 Exporter 实例由上层模块（已添加
 * otel-exporter-otlp / otel-exporter-zipkin / otel-exporter-jaeger 依赖） 通过 {@link
 * SpanExporterProvider} SPI 注入；本工厂负责包装成 {@link BatchSpanProcessor} 或 {@link SimpleSpanProcessor}。
 *
 * <p>典型集成方式：
 *
 * <pre>{@code
...
public final class OtelExporterFactory {
  ...
  public static SpanProcessor batchProcessor(SpanExporter exporter, BatchConfig config) {
    ...
    log.info(
        "[Sentry] BatchSpanProcessor 创建完成：queue={}, batch={}, delay={}ms",
  ...
  public static class BatchConfig {
     ...
     *
     * <p>面向高 QPS 服务，通过放大队列降低 Span 因队列满而被丢弃的概率， 代价是常驻内存更高（队列按 Span 对象计数，非字节数）。
     *
     * @return 全新的配置实例，可安全修改后使用
     */
     ...
     *
     * <p>面向排障与灰度验证场景，让链路数据尽快可见；高流量下极易触发队列溢出丢 Span， 且导出请求频次高，不建议在生产主链路长期开启。
     *
     * @return 全新的配置实例，可安全修改后使用
     */
