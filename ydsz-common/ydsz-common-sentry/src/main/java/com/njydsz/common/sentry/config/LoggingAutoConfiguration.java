 ...
 *
 * <ol>
 *   <li>按 {@code logging.elk.enabled}、{@code logging.loki.enabled} 收集启用的通道， 并按熔断器名称（{@code
 *       elk-logstash} / {@code loki}）为各通道绑定对应熔断器
 *   <li>一个通道都未启用时兜底使用 Loki 默认配置，保证日志不会完全无出口
 *   <li>多通道时包装为 {@link DualLogPublisher} 双写， 是否要求全部成功由 {@code logging.dual.failOnAllError} 决定
 *   <li>开启 {@code logging.async.enabled} 时再包一层 {@link AsyncLogPublisher}： 有界队列 + 批量 flush +
 *       限速，队列满时丢弃日志而非阻塞业务线程
 * </ol>
 *
 * @author ydsz-team
...
public class LoggingAutoConfiguration {
  ...
  public LogPublisher logPublisher(
    ...
    if (elkConfig.isEnabled()) {
      ...
      publishers.add(
          new ElkLogPublisher(
              elkConfig.getHost(),
              elkConfig.getPort(),
              elkConfig.getProtocol(),
              elkConfig.getConnectTimeoutMillis(),
              elkConfig.getReadTimeoutMillis(),
              elkConfig.getMaxRetryAttempts(),
              elkCb));
    }
    ...
    if (lokiConfig.isEnabled()) {
      CircuitBreaker lokiCb =
          circuitBreakers.stream()
              ...
              .orElse(null);
      publishers.add(
          new LokiLogPublisher(
              lokiConfig.getUrl(),
              lokiConfig.getConnectTimeoutSeconds(),
              lokiConfig.getMaxRetryAttempts(),
              lokiCb));
    }
    ...
    if (publishers.size() == 1) {
      ...
      delegate =
          new DualLogPublisher(publishers, properties.getLogging().getDual().isFailOnAllError());
    }
