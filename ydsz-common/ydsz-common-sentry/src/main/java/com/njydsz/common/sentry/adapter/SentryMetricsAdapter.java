 ...
 * 本类将传统 Counter/Timer/Gauge 调用桥接到 {@link MetricsCollector} 统一入口，
 * 底层由 {@code MicrometerMetricsCollector} 执行实际的 Micrometer 注册。
 *
 * <p><b>v2.1.0 变更</b>：移除 {@link MeterRegistry} 构造参数，改为内部通过
 * {@link SentryService} 获取 {@link MetricsCollector}，业务模块不再直接依赖 Micrometer API。
 * 符合《云顶编码规范》第 27.2.1 节「禁止直接操作 MeterRegistry」的强制要求。
 *
 * <h3>迁移路径</h3>
 * <ol>
 *   <li><b>当前阶段</b>：继承本类替换 {@code AbstractModuleMetrics}，获得统一入口能力</li>
 *   <li><b>最终阶段</b>：直接调用 {@code SentryService} 的 count/time/gauge 方法</li>
 * </ol>
 *
 * <h3>使用示例</h3>
 * <pre>{@code
...
public abstract class SentryMetricsAdapter {
    ...
    protected void gauge(String name, Supplier<Number> supplier, String... tags) {
        ...
        if (collector != null) {
            ...
            String key = cacheKey(prefix + name, tags);
            AtomicReference<Double> ref = gaugeRefCache.computeIfAbsent(key, k -> {
