 ...
 * 自监控自动配置。
 *
 * <p>周期性上报 Sentry 各组件的可用性指标到 MetricsCollector，
 * 解决"监控系统本身挂了没人知道"的问题。
 *
 * <p>Periodic reporting includes:
 * <ul>
 *   <li>指标采集器 / 日志发布器 / 告警发布器可用性</li>
 *   <li>异步日志队列积压数、丢弃总数、已发布总数</li>
 *   <li>告警收敛的抑制率与总量</li>
 * </ul>
 *
 * @author ydsz-team
...
@AutoConfigureAfter({MetricsAutoConfiguration.class, LoggingAutoConfiguration.class,
...
public class SelfMonitorAutoConfiguration {
    ...
    public SentrySelfMonitor sentrySelfMonitor(MetricsCollector metricsCollector,
        ...
        return new SentrySelfMonitor(metricsCollector, logPublisher, alertPublisher,
                traceContext, slaCollector);
    }

    /**
     * 自监控指标上报器。
     *
     * <p>定时上报 Sentry 各组件的自监控 Gauge 指标到 MetricsCollector，
     * 供 Prometheus 告警规则使用。
     */
    @Slf4j
        ...
        private void reportLoggingMetrics() {
            ...
            if (logPublisher instanceof AsyncLogPublisher async) {
                metricsCollector.setGauge("ydsz.sentry.logging.queue_size",
                        "异步日志队列积压数", EMPTY_TAGS, async.getQueueSize());
                metricsCollector.setGauge("ydsz.sentry.logging.dropped_total",
                        "异步日志丢弃总数", EMPTY_TAGS, async.getDroppedCount());
                metricsCollector.setGauge("ydsz.sentry.logging.published_total",
                        "异步日志已发布总数", EMPTY_TAGS, async.getTotalPublished());
            }
        }
        ...
        private void reportAlertingMetrics() {
            ...
            if (alertPublisher instanceof AlertConverger converger) {
                metricsCollector.setGauge("ydsz.sentry.alert.suppression_rate",
                        "告警抑制率", EMPTY_TAGS, converger.getSuppressionRate());
                metricsCollector.setGauge("ydsz.sentry.alert.total",
                        "告警总数", EMPTY_TAGS, converger.getTotalAlerts());
                metricsCollector.setGauge("ydsz.sentry.alert.suppressed",
                        "被抑制告警数", EMPTY_TAGS, converger.getSuppressedAlerts());
            }
        }
    }
}
