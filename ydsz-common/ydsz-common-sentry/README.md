...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `ydsz.sentry.enabled` | true | 是否启用 Sentry 模块 |
| `ydsz.sentry.app-name` | `ydsz` | 应用名 |
| `ydsz.sentry.hostname` | `auto` | 主机名（auto 自动探测） |
| `ydsz.sentry.profile` | `dev` | 环境标识 |
| `ydsz.sentry.metrics.primary` | `micrometer` | 主指标采集器（micrometer / memory） |
| `ydsz.sentry.metrics.enable-system-metrics` | true | 是否启用系统资源指标采集 |
| `ydsz.sentry.metrics.system-metrics-interval-seconds` | 15 | 系统资源指标采集间隔（秒） |
| `ydsz.sentry.metrics.circuit-breaker.enabled` | true | 是否启用指标采集熔断器 |
| `ydsz.sentry.metrics.circuit-breaker.failure-rate-threshold` | 0.3 | 失败率阈值 |
| `ydsz.sentry.metrics.circuit-breaker.sliding-window-size` | 100 | 滑动窗口大小 |
| `ydsz.sentry.metrics.circuit-breaker.half-open-after-seconds` | 30 | 半开恢复时间（秒） |
| `ydsz.sentry.logging.primary` | `loki` | 主日志方案（elk / loki / dual） |
| `ydsz.sentry.logging.elk.enabled` | false | 是否启用 ELK |
| `ydsz.sentry.logging.elk.host` | `logstash` | Logstash 主机 |
| `ydsz.sentry.logging.elk.port` | 5044 | Logstash 端口 |
| `ydsz.sentry.logging.elk.protocol` | `tcp` | 协议（tcp / udp） |
...
| `ydsz.sentry.logging.async.batch-size` | 100 | 批量发送大小 |
| `ydsz.sentry.logging.async.flush-interval-millis` | 1000 | 刷新间隔（毫秒） |
| `ydsz.sentry.logging.async.max-rate-per-second` | 0 | 令牌桶限流（0=不限流） |
| `ydsz.sentry.tracing.primary` | `skywalking` | 主追踪系统（skywalking / opentelemetry / default） |
| `ydsz.sentry.tracing.slow-trace-threshold-millis` | 3000 | 慢追踪阈值（毫秒） |
| `ydsz.sentry.tracing.otel.enabled` | false | 是否启用 OTel SDK 自动初始化 |
| `ydsz.sentry.tracing.otel.service-name` | - | 服务名（默认使用 sentry.appName） |
| `ydsz.sentry.tracing.otel.service-version` | `26.09.01` | 服务版本 |
| `ydsz.sentry.tracing.otel.service-namespace` | `ydsz` | 服务命名空间 |
...
| `ydsz.sentry.alerting.dingtalk-receiver` | - | 钉钉告警接收者 |
| `ydsz.sentry.alerting.email-receiver` | - | 邮件告警接收者 |
| `ydsz.sentry.sla.enabled` | true | 是否启用 SLA 框架 |

## 配置验证

26.09.01 起，`SentryProperties` 通过 JSR-303 Bean Validation 约束配置合法范围，启动时自动校验：
