...
> ydsz-common-json 模块是 ydsz-cloud 项目唯一的 JSON 序列化/反序列化引擎。  
...
1. **唯一数据源**：全项目 JSON 操作 **必须且仅能** 使用 `ydsz-common-json`，**禁止** 引入 Jackson/Gson/Fastjson/org.json 等任何第三方 JSON 库。
...
### 2.1 YdszJson vs JsonMapper 的选用原则

| 场景 | 推荐 API | 理由 |
|------|---------|------|
| 大多数日常 JSON 操作 | `YdszJson` 静态方法 | 零配置，全局共享默认 Mapper，心智负担最低 |
...
| 对象类型转换（Map→POJO 等） | `YdszJson.convertValue()` | 语义明确，优于 `fromJson(toJson())` 链式调用 |

### 2.2 常用 API 速查

```java
// — 序列化 —
YdszJson.toJson(obj)              // 对象 → JSON 字符串
YdszJson.toJsonBytes(obj)         // 对象 → byte[]（推荐代替 .getBytes(UTF_8)）
YdszJson.format(obj)              // 对象 → 格式化 JSON（调试用）

// — 反序列化 —
YdszJson.fromJson(json, User.class)               // JSON → 对象
YdszJson.fromJson(json, new JsonType<List<User>>(){}) // JSON → 泛型对象
YdszJson.fromJsonBytes(bytes, User.class)          // byte[] → 对象
YdszJson.parseMap(json)                            // JSON → Map<String, Object>
YdszJson.parseArray(json, User.class)              // JSON → List<User>

// — 类型转换（1.1.0 新增）—
YdszJson.convertValue(map, User.class)             // Map → POJO（优先使用）
YdszJson.convertValue(list, new JsonType<List<User>>(){}) // 泛型转换

// — 树模型 —
YdszJson.parseObject(json)        // JSON → ObjectNode
YdszJson.readTree(json)           // JSON → JsonNode
ObjectNode obj = new ObjectNode();
obj.put("name", "value");         // 构建树
obj.getObjectNode("child");       // 获取嵌套对象节点
obj.getArrayNode("items");        // 获取嵌套数组节点
String json = YdszJson.toJson(obj); // 树 → JSON

// — 安全校验 —
YdszJson.isValidJson(json)        // 校验字符串是否为合法 JSON
```
...
User user = YdszJson.fromJson(YdszJson.toJson(map), User.class);

// ✅ 正确用法（1.1.0 新增）
User user = YdszJson.convertValue(map, User.class);
...
Map<String, Object> ext = YdszJson.parseMap(json);
String val = (String) ext.get("key");

// ✅ 正确用法：优先用 POJO 反序列化
MyDto dto = YdszJson.fromJson(json, MyDto.class);
...
// 泛型 Map
Map<String, Order> map = YdszJson.fromJson(json,
        new JsonType<Map<String, Order>>() {});

// 复杂嵌套
Map<String, List<Order>> complex = YdszJson.fromJson(json,
...
1. **必须** 通过 `JsonModule` SPI 注册，不得直接调用 `YdszJson.register()`
...
参考示例：`ydsz-agent-domain` 中的 `AgentJsonModule`，`ydsz-common-safe` 中的 `SafeJsonModule`。
...
- `ydsz-common-json` 已内置全面的反序列化安全防护（黑名单 + 白名单），业务模块无需额外配置
- 标记为安全的实体类需添加 `@JsonClass` 注解（系统启动时会自动扫描 `com.njydsz` 包）
...
ydsz-common-json 提供 Jackson 兼容注解层，迁移路径：

| Jackson 注解 | YdszJson 等价注解 | 说明 |
|-------------|-----------------|------|
| `@com.fasterxml.jackson.annotation.JsonProperty` | `@com.njydsz.common.json.annotation.JsonProperty` | 直接替换 import |
| `@com.fasterxml.jackson.annotation.JsonIgnore` | `@com.njydsz.common.json.annotation.JsonIgnore` | 直接替换 import |
| `@JsonFormat` | `@com.njydsz.common.json.annotation.JsonFormat` | 直接替换 import |

### 9.2 从 Fastjson2 迁移

- `JSON.parseObject(json, Class.class)` → `YdszJson.fromJson(json, Class.class)`
- `JSON.toJSONString(obj)` → `YdszJson.toJson(obj)`
...
- [ ] 序列化使用 `YdszJson.toJsonBytes()` 而非 `toJson().getBytes()`
...
> **文档版本**: 1.1.0 | **最后更新**: 2026-08-06 | **维护者**: ydsz-team
