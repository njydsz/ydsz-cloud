 ...
 * 自定义序列化器注册中心。
 *
 * <p>支持用户注册和管理自定义序列化器，实现类似 Jackson Module 的扩展机制。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>
 ...
 *
 * <ul>
 *   <li>使用 ConcurrentHashMap 保证并发安全
 *   <li>注册操作原子执行
 * </ul>
 *
 * @author ydsz-team
...
public final class SerializerRegistry {
   ...
   * 模块序列化器时使用，保证"先注册优先"语义：用户直接注册（{@link #register}） 的序列化器优先于模块注册，反之亦然，按注册时序决定。
   *
   * @param type 目标类型
   * @param serializer 序列化器
   ...
   *
   * <p>语义与 {@link #registerIfAbsent(Class, JsonSerializer)} 一致，用于模块系统 回填模块反序列化器。
   *
   * @param type 目标类型
   * @param deserializer 反序列化器
  ...
  private static <T> JsonDeserializer<T> castDeserializer(JsonDeserializer<?> deserializer) {
