 ...
 * Jackson 注解兼容桥（P1-8）。
 *
 * <p><b>解决迁移期静默失效问题：</b>本模块注解与 Jackson 同名
 * （{@code @JsonProperty} / {@code @JsonIgnore} 等），从 Jackson 迁移到 YdszJson 的
 * 业务代码若未批量替换 import，注解会因包名不同而静默失效（字段名、忽略规则不生效）。
 * 本桥通过反射读取 Jackson 注解（类名反射解析，<b>不 import 任何 Jackson 类</b>，
 * 不触发 checkstyle JSON 生态红线），在原生注解缺失时以 Jackson 注解语义兜底。</p>
 *
 * <p><b>优先级规则：</b>原生注解（{@code com.njydsz.common.json.annotation}）优先，
 * Jackson 注解仅在原生注解未标注时生效。对标 FastJSON2 的
 * {@code JSONReader.Feature.SupportAutoType} 式兼容策略。</p>
 *
 * <p><b>兼容范围（Jackson annotations 2.x）：</b></p>
 * <ul>
 *   <li>{@code @JsonProperty.value()} - 字段重命名（读/写双向）</li>
 *   <li>{@code @JsonIgnore} - 字段忽略（读/写双向）</li>
 *   <li>{@code @JsonIgnoreProperties.value()} - 类级字段忽略</li>
 *   <li>{@code @JsonValue} - 方法级整体值序列化（枚举常用）</li>
 *   <li>{@code @JsonAlias.value()} - 反序列化多命名兼容</li>
 * </ul>
 *
 * <p><b>运行期可选：</b>jackson-annotations 不在 classpath 时桥自动禁用
 * （{@link #isAvailable()} 返回 false），所有方法返回空值，零开销降级。</p>
 *
 * @author ydsz-team
 * @since 1.2.3
 */
public final class JacksonAnnotationBridge {
    ...
    private static final Class<? extends Annotation> JSON_IGNORE_PROPERTIES = resolve("JsonIgnoreProperties");
    ...
    public static String[] ignoreProperties(Class<?> clazz) {
        ...
        try {
            ...
            LOGGER.debug("JacksonAnnotationBridge failed to read @JsonIgnoreProperties on {}: {}",
    ...
    public static String[] aliases(Field field) {
        ...
        try {
            ...
            LOGGER.debug("JacksonAnnotationBridge failed to read @JsonAlias on {}.{}: {}",
                field.getDeclaringClass().getName(), field.getName(), e.toString());
            return new String[0];
        }
    }
     ...
     */
    private static String stringValue(AnnotatedElement element,
            Class<? extends Annotation> annotationType, String member) {
        ...
        try {
            ...
            LOGGER.debug("JacksonAnnotationBridge failed to read @{} on {}: {}",
                annotationType.getSimpleName(), element, e.toString());
            return null;
        }
    }
}
