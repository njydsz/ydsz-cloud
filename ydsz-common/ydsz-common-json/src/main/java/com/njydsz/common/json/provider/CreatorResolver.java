 ...
 *
 * <p>负责处理带 {@code @JsonCreator} 注解的构造函数反序列化逻辑。 当目标类没有默认无参构造函数时，通过注解标记的构造函数进行反序列化， 类似 Jackson 的
 * {@code @JsonCreator} 和 Gson 的 {@code @SerializedName} 机制。
 *
 * <h3>参数名解析策略</h3>
 *
 * <ol>
 *   <li>优先使用 {@code @JsonCreator(parameterNames=...)} 显式指定的参数名数组
 *   <li>降级为通过类字段名 + {@code @JsonProperty} 注解映射 JSON 字段名
 * </ol>
 *
 * @author ydsz-team
 * @since 1.0.0
...
final class CreatorResolver {
   ...
   *
   * <p>如果多个构造函数都标注了该注解，优先选择 {@code defaultCreator=true} 的构造函数。 其次选择最后遍历到的已标注构造函数。
   *
   * @param clazz 目标类
   ...
   * <ol>
   *   <li>将 JSON 解析为 Map
   *   <li>如果注解指定了 {@code parameterNames}，按名称从 Map 中取值并转换类型后传参
   *   <li>否则降级为字段反射赋值 + Setter 方式创建实例
   *   <li>通过反射调用构造函数创建实例
   * </ol>
   *
   * @param json JSON 字符串
  ...
  static Object deserializeWithCreator(String json, Constructor<?> constructor) {
    ...
    if (map == null || map.isEmpty()) {
      try {
        ...
        LOGGER.warn(
            "Constructor invocation failed for {} (empty map)",
    ...
    String[] parameterNames =
        annotation != null && annotation.parameterNames().length > 0
    ...
    try {
      ...
      LOGGER.warn(
          "Constructor invocation failed for {}", constructor.getDeclaringClass().getName(), e);
      return null;
    }
  }
   ...
   *
   * <p>字段名映射优先级：{@code @JsonProperty(value)} > Java 字段名。 值通过 {@link TypeConverter#convertValue}
   * 进行类型转换。
   *
      ...
      for (Field field : fields) {
        ...
        if (value != null) {
          try {
            ...
            LOGGER.warn(
                "Field setter failed for {}.{}, skipping field",
                clazz.getName(),
                field.getName(),
                e);
          }
