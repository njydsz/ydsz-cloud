 ...
 *
 * <p>JSON Merge Patch 是一种基于 JSON 文档的增量更新协议，通过对目标文档递归合并的方式
 * 表示修改操作。语义等价于将 patch 文档"覆盖"到目标文档上：</p>
 * <ul>
 *   <li>patch 中的 {@code null} 值表示删除目标中对应字段</li>
 *   <li>patch 中的对象值与目标中对应对象值递归合并</li>
 *   <li>其他 patch 值直接替换目标值</li>
 * </ul>
 *
 * <p><b>使用示例：</b></p>
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>与 {@link JsonPatch RFC 6902} 相比，Merge Patch 不支持对数组的精确操作和移动/复制语义，
 * 但其文档形式更简洁，适合 RESTful API 的部分更新场景（如 HTTP PATCH 方法）。</p>
 *
 * @author ydsz-team
 * @since 1.2.0
 * @see JsonPatch
...
public final class JsonMergePatch {
     ...
     * <p>算法语义（RFC 7396 §2）：</p>
     * <ol>
     *   <li>如果 patch 不是 JSON 对象，则结果就是 patch 本身。</li>
     *   <li>如果目标不是 JSON 对象，则用空白对象替换目标。</li>
     *   <li>遍历 patch 的所有字段：
     *       <ul>
     *         <li>若 patch 值为 {@code null}，则删除目标中对应字段。</li>
     *         <li>若 patch 对象为 JSON 对象且目标对应字段也为 JSON 对象，则递归合并。</li>
     *         <li>否则直接用 patch 值替换目标字段。</li>
     *       </ul>
     *   </li>
     * </ol>
     *
     ...
     * @return 修改后的目标节点（可能是 {@code target} 本身，也可能是 {@code patch} 本身——
     *         当 patch 不是对象或目标不是对象时返回 patch）
     */
