 ...
 *
 * <p>提供 token-by-token 的 JSON 解析能力，适合：
 *
 * <ul>
 *   <li>超大 JSON 流式处理（避免全部加载到内存）
 *   <li>部分字段提取（跳过无关数据）
 *   <li>混合解析（部分树解析、部分流解析）
 * </ul>
 *
...
public class JsonParser implements Closeable {
  ...
  public JsonToken nextToken() {
    ...
    try {
        ...
        default:
          if (c == '-' || (c >= '0' && c <= '9')) {
            ...
            throw new JsonException(
                "Unexpected character: '" + c + "' at position " + reader.getPosition());
          }
      }
      return currentToken;
      ...
      throw new JsonException(
          "Parse error at position " + reader.getPosition() + ": " + e.getMessage(), e);
    }
  }
