...
/** validation error */
   ...
   *
   * <p>与 {@link #setFieldName(String)} 功能一致，返回 {@code this} 支持链式调用。 推荐在创建异常后立即链式调用，例如：
   *
   * <pre>
   ...
   *
   * <p>如果 {@link #line} 和 {@link column} 尚未设置（值为 -1）， 尝试从 {@code position}（通过 {@link
   * #getPosition()} 获取）从 JSON 源中重新计算。 若无法计算则返回 {@code (unknown)}。
   *
  ...
  public static JsonDeserializationException missingField(String fieldName) {
    return new JsonDeserializationException(MISSING_FIELD, "Missing required field: " + fieldName);
  }
   ...
   * @return 反序列化异常
   */
  public static JsonDeserializationException typeMismatch(
      String fieldName, Class<?> expected, Class<?> actual) {
  ...
  public static JsonDeserializationException invalidValue(String fieldName, String value) {
    return new JsonDeserializationException(
        INVALID_VALUE, "Invalid value '" + value + "' for field '" + fieldName + "'");
  }
  ...
  public static JsonDeserializationException parseError(String json, int position) {
    return new JsonDeserializationException(
        PARSE_ERROR, "Failed to parse JSON at position " + position, position, json);
  }
