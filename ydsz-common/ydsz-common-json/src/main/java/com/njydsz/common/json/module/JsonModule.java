 ...
 *
 * <ul>
 *   <li>模块化注册 - 类似 Jackson Module，可自由组合
 *   <li>类型绑定 - 为指定类型注册序列化器和反序列化器
 *   <li>自动发现 - Spring Boot 环境下自动发现并注册模块
 *   <li>优先级控制 - 支持设置模块优先级，优先级高的先注册
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
 ...
 * </pre>
 *
 * <p><b>规范要求（R6）：</b>每个业务模块若有自定义序列化需求，必须实现 {@code XxxJsonModule extends JsonModule} 并通过
 * {@code @Component} 注册为 Spring Bean， 由 {@link com.njydsz.common.json.spring.JsonModuleRegistrar}
 * 自动发现并注册到引擎。 <b>禁止</b>在业务代码中散落调用 {@code YdszJson.register()}，所有自定义序列化器/反序列化器 必须通过 JsonModule
 * 收敛，确保可发现、可治理、可测试。
 *
 * @author ydsz-team
 * @since 26.09.01
...
public interface JsonModule {
   ...
   * Spring 工厂接口
   *
   * <p>实现此接口的模块会在 Spring Boot 环境下自动注册
   */
