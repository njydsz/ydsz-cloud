 ...
 * JSON 运行时预计算配置（不可变快照）。
 *
 * <p>从 {@link JsonConfig} 派生的预计算配置对象，用于替代 ThreadLocal 中的配置字段传递。
 * 所有字段均为 {@code final}，天然线程安全，读取无锁无 ThreadLocal 开销。</p>
 *
 * <p><b>设计目标：</b></p>
 * <ul>
 *   <li>将不可变配置从 {@code ThreadLocal<SerializationContext>} 中分离出来，
 *       减少 ThreadLocal 体积和内存泄漏风险</li>
 *   <li>允许显式传递配置给序列化/反序列化组件（{@code JSONWriter} / {@code JSONReader}），
 *       消除对 ThreadLocal 查询的依赖</li>
 *   <li>生命周期绑定到 {@link JsonMapper} 实例，而非线程，支持多配置并行</li>
 * </ul>
 *
 * <p><b>不可变语义：</b>对标 Jackson {@code SerializationConfig} / {@code DeserializationConfig}，
 * 一次构建后只读，如需修改配置请构建新的 {@link JsonConfig} 并重新生成。</p>
 *
...
public record JsonRuntimeConfig(
     ...
     *
     * <p>不可变，可安全全局共享；对应无自定义配置时的默认序列化/反序列化行为。</p>
     */
     ...
     * 适用于 {@link JsonMapper} 实例化时一次性捕获配置状态。</p>
     *
     ...
     *
     * <p>当 Mapper 配置开启 prettyPrint 时为 true；调用级别可通过
     * {@code override} 参数强制控制。</p>
     *
