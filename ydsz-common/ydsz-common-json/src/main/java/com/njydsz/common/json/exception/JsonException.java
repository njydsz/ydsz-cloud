 ...
 *
 * <p>所有 YdszJson 相关异常的基类，携带错误码、源位置信息和可选的 JSON 字段路径，
 * 便于快速定位序列化/反序列化失败的具体字段和原因。</p>
 *
 * <p><b>字段路径格式（对标 Jackson {@code JsonProcessingException.getPath()}）：</b></p>
 * <ul>
 *   <li>{@code user.address.street} — 嵌套对象字段</li>
 *   <li>{@code items[2].name} — 数组元素字段</li>
 *   <li>{@code null} — 未设置路径</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class JsonException extends RuntimeException {
     ...
     *
     * <p>例如 {@code "user.addresses[0].zipCode"}，用于快速定位错误字段。
     * 为 {@code null} 表示未设置。</p>
     *
     * @since 1.2.0
     */
     ...
     * @since 1.2.0
     */
    public JsonException(int errorCode, String message, Throwable cause, int position, CauseType causeType) {
        super(message, cause);
        this.errorCode = errorCode;
        this.position = position;
     ...
     * <p>返回格式：</p>
     * <ul>
     *   <li>{@code "(position: 42)"} — 仅知道字符位置</li>
     *   <li>{@code "(line 3, column 15)"} — 行号和列号可用时（子类实现）</li>
     *   <li>{@code ""} — 位置未知</li>
     * </ul>
     *
     ...
     *
     * <p>对标 Jackson 的 {@code JsonProcessingException.getMessage()} 行为：
     * 在原始消息后附加 <code> [field: xxx]</code> 结构化后缀，
     * 便于日志聚合和根因定位。</p>
     *
