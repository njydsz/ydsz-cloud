 ...
 *
 * <p>字段元数据缓存：避免重复反射获取字段信息。
 *
 * <p><b>缓存隔离说明（P0-1 并发安全修复，2026-08-04）：</b>
 *
 * <p>修复前以外层仅以 Class 为 Key，导致同一类的不同命名策略共享同一个 FieldMeta[]， jsonName 首次加载时被"烘焙固化"，后续不同策略的 Mapper
 * 无法隔离——先加载者决定命名。 修复后内层以 PropertyNamingStrategy 引用为 Key，不同策略独立缓存各自的 FieldMeta[]。
 * PropertyNamingStrategy 使用引用相等（==）语义——内置策略常量为接口静态字段， 同一常量引用天然相等；自定义策略实例各自独立，行为正确。
 *
...
public final class SerializerCache {
  ...
  /** 字段元数据缓存（双层：外层 Class -> 内层 命名策略 -> FieldMeta[]）， 外层有界 LRU（容量 1024），防止动态类加载场景下无界增长。 */
  private static final BoundedLruCache<Class<?>, ConcurrentMap<PropertyNamingStrategy, FieldMeta[]>>
      FIELD_META_CACHE = new BoundedLruCache<>(1024);
   ...
   * @param metas 字段元数据数组
   */
  public static void putFieldMeta(
      Class<?> clazz, PropertyNamingStrategy strategy, FieldMeta[] metas) {
    FIELD_META_CACHE.computeIfAbsent(clazz, k -> new ConcurrentHashMap<>()).put(strategy, metas);
  }
