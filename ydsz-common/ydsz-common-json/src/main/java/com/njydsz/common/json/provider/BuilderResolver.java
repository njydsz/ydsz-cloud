 ...
 * 本类通过反射定位 Builder 类及其 setter 方法（如 {@code withXxx()} 或 {@code fieldName()}），
 * 逐字段设置值后调用 {@code build()} 方法生成目标实例。
 *
 * <h3>支持的 Builder 模式</h3>
 * <ul>
 *   <li><b>内部 Builder</b>：Builder 作为目标类的静态内部类，自动发现</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
final class BuilderResolver {
     ...
     *
     * <p>自动检测内部 Builder 类：查找方法名为 fieldName（如 {@code name()}）的 setter，
     * 设置所有字段值后调用 {@code build()} 方法生成目标实例。
     *
     * @param json         JSON 字符串
     * @param clazz        目标类
