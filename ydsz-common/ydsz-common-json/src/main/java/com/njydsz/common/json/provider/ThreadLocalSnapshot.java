 ...
 *
 * <p>使用 {@link SerializationContext} 合并多个 ThreadLocal 为单一实例， 构造时捕获当前线程的 SerializationContext
 * 配置字段快照， 调用 {@link #restore()} 恢复原始值。避免修改全局单例。
 *
 ...
 * 不保存运行时状态字段（sbPool、fastWriterPool、serializingObjects、currentViewClass）， 因为运行时状态仅在单次序列化调用内有意义。
 *
 * <p><b>namingStrategy 说明：</b>命名策略存放在 {@link FieldMetadataLoader#NAMING_STRATEGY} 这个独立 ThreadLocal
 * 中（不在 SerializationContext 内）。此前快照未保存它， 导致 {@code YdszJson.toJson(obj, config)} 用不同命名策略序列化后未回滚，
 * 后续默认调用仍使用旧命名策略（配置泄漏）。现已补全。
 *
 * <p><b>useBigDecimal 说明（P0-2 并发安全修复，2026-08-04）：</b>useBigDecimal 从全局 volatile static 改为
 * ThreadLocal，快照中保存/恢复其值， 确保不同配置的 Mapper 在使用后相互隔离，避免某 Mapper 开启 BigDecimal 后永久影响所有线程、所有 Mapper
 * 的解析行为。
 *
...
public final class ThreadLocalSnapshot {
   ...
   *
   * <p>保存调用前的线程级 maxDepth / maxGenericDepth / maxParseDepth 覆盖值 （null 表示未设置），restore
   * 时原样写回，保证嵌套调用与调用后状态一致。
   */
