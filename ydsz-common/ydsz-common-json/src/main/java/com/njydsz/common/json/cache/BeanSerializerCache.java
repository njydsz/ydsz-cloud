 ...
 *
 * <p>为每个 Bean 类 + 命名策略缓存预计算的序列化器，避免重复创建。
 *
 * <p><b>缓存策略（P0-1 并发安全修复，2026-08-05）：</b>
 *
 * <p>修复前仅以 Class 为 Key，导致不同命名策略的 BeanSerializer 被覆盖—— 先加载的线程的 BeanSerializer（含特定命名字段名）被后加载的线程复用，
 * 导致字段名输出与当前命名策略不一致。修复后内层以 PropertyNamingStrategy 引用为 Key， 不同策略独立缓存各自的 BeanSerializer。
 *
 ...
 *
 * <ul>
 *   <li>双层 Key：外层 Class -> 内层 命名策略 -> BeanSerializer
 *   <li>外层有界 LRU（1024），内层 ConcurrentHashMap
 *   <li>BeanSerializer 为 Value - 预计算的序列化器
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public final class BeanSerializerCache {
  ...
  private static final BoundedLruCache<
          Class<?>, ConcurrentMap<PropertyNamingStrategy, BeanSerializer>>
      CACHE = new BoundedLruCache<>(1024);
  ...
  static {
    ...
    JsonConfig.addChangeListener(
        (oldConfig, nextConfig, newVersion) -> {
          if (oldConfig != null
              && oldConfig.getNamingStrategy() == nextConfig.getNamingStrategy()) {
            return; // 命名策略未变更，跳过清理
          }
          CACHE.clear();
        });
   ...
   * @return Bean 序列化器实例
   */
  public static BeanSerializer getOrCreate(
      Class<?> clazz, FieldMeta[] fieldMetas, PropertyNamingStrategy strategy) {
   ...
   *
   * <p>用于序列化热路径中快速判断 primitiveOnly 标记，避免不必要的 IdentityHashSet 循环引用检测。
   *
   * @param clazz Bean 类
   * @param strategy 命名策略
