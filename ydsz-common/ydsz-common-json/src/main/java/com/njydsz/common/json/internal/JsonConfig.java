 ...
 * YdszJson 全局配置类（不可变）。
 *
 * <p><b>内部 API：</b>此类主要供 {@code JsonAutoConfiguration} 和框架内部使用。 业务代码请通过 {@code ydsz.json.*} 配置属性调整
 * JSON 行为，不要直接操作 JsonConfig。
 *
 * <p><b>配置功能：</b>
 *
 * <ul>
 *   <li>全局命名策略 - 序列化时的字段命名转换
 *   <li>循环引用处理策略 - REF/IGNORE/ERROR
 *   <li>空值处理策略 - 是否输出 null 值
 *   <li>日期格式配置 - 全局日期格式
 *   <li>枚举序列化策略 - ordinal 或 name
 * </ul>
 *
 * <p><b>不可变设计：</b>所有字段均为 {@code final}，通过 Builder 一次性构建后不可修改。 如需修改配置，请通过 {@link
 * Builder#from(JsonConfig) Builder.from()} 创建新实例， 或使用 {@link #install(JsonConfig)} 原子替换全局单例。 对标
 * Jackson {@code ObjectMapper} 实例级别的不可变语义。
 *
 * <p><b>Builder 模式（框架内部使用）：</b>
 *
 * <pre>
...
public final class JsonConfig implements Serializable {
   ...
   *
   * <p>缓存组件可通过 {@link #getConfigVersion()} 或轮询此值判断配置是否变更， 从而实现自动失效（如命名策略切换后清理 SerializerCache
   * 中已烘焙的字段名）。
   */
   ...
   *
   * <p>标榜"构建后不可变"：外部通过 {@code JsonConfig.builder().build()} 构建完整配置后， 调用此方法原子替换全局单例，后续只读不修改。
   * 如确需修改配置，请重新构建新的 {@link JsonConfig} 并再次调用 {@code install(newConfig)}。
   *
   ...
   * 注册配置变更监听器。
   *
   * <p>监听器在 {@link #install(JsonConfig)} 完成后回调，典型的使用场景：
   *
   * <ul>
   *   <li>SerializerCache：检测到命名策略变更时清理已烘焙字段名的缓存条目
   *   <li>监控指标：记录配置变更审计日志
   * </ul>
   *
  ...
  private static void notifyConfigChanged(
      JsonConfig oldConfig, JsonConfig newConfig, long newVersion) {
   ...
   * 获取配置实例（单例）
   *
   * <p><b>推荐用法：</b>业务代码只读获取配置。如需修改全局配置，请使用 {@link #install(JsonConfig)} 替换为新的不可变实例。
   *
   ...
   * 获取泛型递归深度上限。
   *
   * <p>防止恶意构造的嵌套泛型类型（如 {@code List<List<List...>>}）导致 {@link
   * com.njydsz.common.json.provider.DeserializationProvider DeserializationProvider} 递归过深触发
   * StackOverflow。默认 64，与 FastJSON2 默认值对齐。
   *
   * @return 泛型递归深度上限
   */
   ...
   *
   * <p>启用后，包含小数点的数字将被解析为 {@link java.math.BigDecimal}， 避免精度丢失，适用于金融场景。
   *
   ...
   *
   * <p>推荐使用 Builder 替代 getInstance() + setter 链式调用。 Builder 构建的实例所有字段均为 final，天然线程安全，无需
   * ThreadLocalSnapshot。
   *
   * <pre>
   * JsonConfig config = JsonConfig.builder()
  ...
  public static final class Builder {
     ...
     *
     * <p>通过 getter 读取源配置值，而非直接访问字段， 保持封装性并兼容未来的字段变更。
     *
     * @param config 源配置
     ...
     *
     * <p>通过全参数构造函数一次性写入所有字段，构建后字段值不可修改。 线程安全，无锁可见性保证（final 字段语义）。
     *
