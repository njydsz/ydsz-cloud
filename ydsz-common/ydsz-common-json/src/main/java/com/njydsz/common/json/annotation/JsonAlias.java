 ...
 * 序列化时仍输出主名称（{@code @JsonProperty} 值或字段名），
 * 反序列化时 JSON 中出现主名称或任一别名均可匹配到该字段。
 * 适用于对接外部系统的多命名兼容（如 {@code user_id} / {@code userId}）。</p>
 *
 * <p><b>示例：</b></p>
 * <pre>
 * public class User {
 ...
 * </pre>
 *
 * <p><b>F-3 恢复说明：</b>v1.2.x 曾将该注解标记废弃并移除了实现，因外部 API
 * 多命名兼容为刚需，v1.2.2 恢复支持（见 BeanReader 别名字段匹配）。</p>
 *
 * @author ydsz-team
 * @since 1.2.2
 */
