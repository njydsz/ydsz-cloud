 ...
 * 高性能 JSON 读取器
 *
 * <p>直接操作 char[] 数组解析 JSON，避免 String 分配和方法调用开销
 *
 * <p><b>性能优势：</b>
 *
 * <ul>
 *   <li>直接字符比较，避免 String 创建
 *   <li>预计算数值解析，避免 Double.parseDouble
 *   <li>FNV-1a 哈希字段名匹配，避免 String 创建
 * </ul>
 *
 * <p><b>特性系统（Feature）：</b>
 *
 * <ul>
 *   <li>{@link Feature#LimitDepth} - 默认开启，防止栈溢出攻击（最大深度 256）
 *   <li>{@link Feature#LimitStringLength} - 防止大字符串 OOM（最大 1MB）
 *   <li>{@link Feature#LimitObjectSize} - 防止大对象 OOM（最大 10000 字段）
 *   <li>{@link Feature#LimitArraySize} - 防止大数组 OOM（最大 10000 元素）
 *   <li>{@link Feature#SafeMode} - 安全模式，启用所有安全限制
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public final class JSONReader {
  ...
  public enum Feature {
     ...
     *
     * <p><b>默认开启</b>，最大深度 256，可通过 {@link JSONReader#setMaxDepth(int)} 调整
     */
    LimitDepth(true),
     ...
     * 安全模式（启用所有安全限制）
     *
     * <p>启用后自动开启：LimitDepth、LimitStringLength、LimitObjectSize、LimitArraySize
     */
    SafeMode(true),
     ...
     *
     * <p>用于和特性组合值按位与（{@code features & mask}）判断是否启用， 或被 {@code of(...)} 按位或组合多个特性。位序依赖枚举声明顺序，
     * 请勿随意调整枚举常量位置，否则会破坏已持久化/传输的特性位组合。
     *
     * @return 64 位长整型位掩码
     */
    public long mask() {
   ...
   *
   * <p>为 null 时回退到静态 {@link #maxDepth}，保持向后兼容。
   */
  ...
  /**
   * 线程级调用深度覆盖（P0-3 修复：多 Mapper 实例隔离）。
   *
   * <p>历史问题：{@code JsonConfig.install()} 将 maxDepth/maxGenericDepth 传播到 本类静态全局字段，多个 {@code
   * JsonMapper} 实例配置不同深度时互相覆盖 （后 install 者生效），违背 Mapper 实例隔离承诺。
   *
   * <p>修复方式：{@code JsonMapper} 在每次序列化/反序列化调用期间将自身配置的 深度限制写入线程级覆盖（通过 {@link ThreadLocalSnapshot}
   * 保存/恢复）， {@link #resolveMaxDepth()} / {@link #resolveMaxGenericDepth()} 优先读取覆盖值。 优先级：线程级调用覆盖
   * &gt; 实例级显式传参 &gt; 静态全局默认。
   *
  ...
  public static void clearThreadLocals() {
    READER_POOL.remove();
    CALL_MAX_DEPTH.remove();
    CALL_MAX_GENERIC_DEPTH.remove();
  }
   ...
   * 构造函数（char[] 模式 + 显式深度配置）
   *
   * <p>通过传入预计算的深度限制，避免静态 volatile 字段读取和 ThreadLocal 查询。 当 {@code maxDepth} 或 {@code
   * maxGenericDepth} 为 null 时回退到静态全局值。
   *
   * @param buf 字符缓冲区
   * @param offset 起始偏移
   * @param length 有效长度
  ...
  /**
   * 设置线程级调用深度覆盖（框架内部使用，供 JsonMapper 调用期间隔离实例配置）。
   *
   * <p>传入 null 表示清除对应覆盖，回退静态全局值。调用方负责在 finally 中 恢复原值（通常由 {@code ThreadLocalSnapshot} 统一管理）。
   *
  ...
  public static void setCallDepthOverride(Integer maxDepth, Integer maxGenericDepth) {
    ...
    if (maxGenericDepth != null && maxGenericDepth <= 0) {
      throw new IllegalArgumentException("maxGenericDepth must be > 0, got: " + maxGenericDepth);
    }
    if (maxDepth != null) {
      CALL_MAX_DEPTH.set(maxDepth);
    } else {
      CALL_MAX_DEPTH.remove();
    }
    if (maxGenericDepth != null) {
      CALL_MAX_GENERIC_DEPTH.set(maxGenericDepth);
    } else {
      CALL_MAX_GENERIC_DEPTH.remove();
    }
  }
  ...
  /**
   * 解析当前线程生效的泛型递归深度上限（静态入口，供 DeserializationProvider 使用）。
   *
  ...
  public static int resolveCallMaxGenericDepth() {
    Integer callDepth = CALL_MAX_GENERIC_DEPTH.get();
    return callDepth != null ? callDepth : maxGenericDepth;
  }
   ...
   *
   * <p>当 {@link com.njydsz.common.json.provider.DeserializationProvider} 递归解析泛型参数（ParameterizedType
   * / GenericArrayType / WildcardType）超过此值时抛出 {@link
   * com.njydsz.common.json.exception.JsonDeserializationException}。 默认值 {@link
  ...
  /**
   * 池化读取器允许保留的最大 char[] 容量（64K 字符 ≈ 128KB）。
   *
   * <p>P1-7：超出此容量的缓冲区在归还池时不保留（置 null，下次 reset 重新分配）， 防止偶发超大 JSON 把线程池中每条线程的常驻内存永久抬高到峰值水平。
   */
  static final int MAX_RETAINED_BUFFER_CHARS = 65536;

  /**
   * 归还池化的 JSONReader。
   *
   * <p>P1-7：超限缓冲区不保留（与 {@code JSONWriter} 的 MAX_RESET_CAPACITY 缩容策略对齐），保证线程池常驻内存有界。
   *
   * @param reader 需归还的 JSONReader 实例
   */
  public static void returnPooledReader(JSONReader reader) {
    if (reader != null && reader.buf != null && reader.buf.length > MAX_RETAINED_BUFFER_CHARS) {
      reader.buf = null;
    }
   ...
   *
   * <p>内部先 {@code skipWhitespace()} 定位，记录字符后再恢复 pos，因此不会推进读取位置； 与 {@link #peekChar()}
   * 的区别在于本方法会跳过前置空白。
   *
   ...
   *
   * <p>用于跳到 {@code ':'} 等结构字符：遇到目标字符即停（不消费）， 遇到非空白非目标字符也立即返回，避免越过值内容。典型用途是定位字段名后的冒号。
   *
   ...
   *
   * <p>先跳过空白并确认当前为引号起始的字段名，再逐字符比对 {@code fieldName}； 不匹配则 {@code skipValue()} 跳过该值并返回
   * false。匹配成功后跳过结束引号、 空白与冒号，使读取位置停在字段值起始处，便于后续直接读取。
   *
   ...
   *
   * <p>用于 {@code @JsonAnySetter} 等需保留原始片段的场景：对象/数组按括号配对截取， 字符串读取到结束引号，标量读取到逗号/括号/空白。已到末尾或当前为 null
   * 时返回 {@code "null"}。 返回的是未解析的文本片段，调用方需自行决定如何解析。
   *
   * @return 原始 JSON 文本片段
   */
   ...
   *
   * <p>先 {@code nextChar()} 定位引号，再委托 {@code readStringContent} 读取内容（自动处理转义）。 非标准单引号由
   * SupportSingleQuotes 特性兼容。
   *
   ...
   *
   * <p>用于已知字段为 null 时显式推进读取位置；若当前非 {@code "null"} 则抛异常。 通常配合 {@link #isNull()} 先判断再调用，避免误消费。
   *
   ...
   *
   * <p>只做前瞻判断，不推进读取位置。若返回 {@code true}， 可调用 {@link #readNull()} 消费该字面量。
   *
   ...
   *
   * <p>定位到引号起始的字符串并读取；若遇到 {@code '}'} 或已到末尾则返回 null， 表示对象结束。用于通用 Map 反序列化等无法预知字段名的路径。
   *
   ...
   *
   * <p>用于字段名预计算哈希，与解析时实时计算的 {@code readFieldNameHash()} 结果比对， 从而以 {@code long} 比较替代字符串相等判断，避免字段名
   * String 分配。 种子 {@code 0x811c9dc5}，乘子 {@code 0x100000001b3L}。
   *
   * @param name 字段名
   * @return FNV-1a 哈希值
   */
   ...
   *
   * <p>对象/数组按括号配对递归跳过（字符串内跳过转义），标量读取到逗号/括号/空白。 用于字段名未匹配时丢弃未知字段，保持容错。
   */
   ...
   *
   * <p>正确处理转义引号 {@code \"} 和转义反斜杠 {@code \\}， 避免字符串内容中的引号导致解析错位。
   *
   ...
   *
   * <p>跳过前置空白并确认 {@code '['}；空数组返回空列表。每个元素交由 {@code readArrayElement} 按类型解析（已知类型走快速路径，未知回退 {@code
   * readAnyValue}）。 用于通用反序列化，持续读取直到 {@code ']'}。
   *
