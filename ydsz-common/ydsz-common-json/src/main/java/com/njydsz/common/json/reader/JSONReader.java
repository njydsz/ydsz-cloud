...
public final class JSONReader {
     ...
     *
     * <p>历史问题：{@code JsonConfig.install()} 将 maxDepth/maxGenericDepth 传播到
     * 本类静态全局字段，多个 {@code JsonMapper} 实例配置不同深度时互相覆盖
     * （后 install 者生效），违背 Mapper 实例隔离承诺。</p>
     *
     * <p>修复方式：{@code JsonMapper} 在每次序列化/反序列化调用期间将自身配置的
     * 深度限制写入线程级覆盖（通过 {@link ThreadLocalSnapshot} 保存/恢复），
     * {@link #resolveMaxDepth()} / {@link #resolveMaxGenericDepth()} 优先读取覆盖值。
     * 优先级：线程级调用覆盖 &gt; 实例级显式传参 &gt; 静态全局默认。</p>
     *
     * <p>此机制为 ThreadLocal 配置模型（P0-5/P0-6 显式传参改造）的过渡方案，
     * 随配置显式传递落地后将随调用参数传递，不再依赖 ThreadLocal。</p>
     *
     * @since 1.2.3
     */
     ...
     * 释放池化的 JSONReader 实例及其 char[] 缓冲区，防止内存泄漏
     * （P1-7：同时清理线程级深度覆盖，避免池化线程残留上一任务的覆盖配置）。</p>
     *
     * @since 1.2.1
     */
     ...
     * 获取当前生效的最大嵌套深度。
     *
     * <p>优先级（P0-3）：线程级调用覆盖 &gt; 实例级显式传参 &gt; 静态全局值。</p>
     *
     * @return 最大嵌套深度
     * @since 1.1.0
     */
     ...
     *
     * <p>传入 null 表示清除对应覆盖，回退静态全局值。调用方负责在 finally 中
     * 恢复原值（通常由 {@code ThreadLocalSnapshot} 统一管理）。</p>
     *
     ...
     *
     * <p>优先返回线程级调用覆盖（P0-3），未设置时返回静态全局值。</p>
     *
     * @return 泛型递归深度上限
     * @since 1.2.3
     */
     ...
     *
     * <p>P1-7：超出此容量的缓冲区在归还池时不保留（置 null，下次 reset 重新分配），
     * 防止偶发超大 JSON 把线程池中每条线程的常驻内存永久抬高到峰值水平。</p>
     */
     ...
     * 归还池化的 JSONReader。
     *
     * <p>P1-7：超限缓冲区不保留（与 {@code JSONWriter} 的 MAX_RESET_CAPACITY
     * 缩容策略对齐），保证线程池常驻内存有界。</p>
     *
