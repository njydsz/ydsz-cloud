 ...
 * 自动预热 Runner（P1-E2）。
 *
 * <p>应用启动时自动扫描 Spring 容器中的 {@code @RestController}/{@code @Controller} Bean，
 * 提取 {@code @RequestBody} 参数类型和 {@code @ResponseBody} 返回类型，
 * 调用 {@link YdszJson#warmup(Class...)} 触发字段元数据与序列化器缓存构建。</p>
 *
 * <p>避免首次真实请求时的冷启动延迟尖峰（无参构造→反射→元数据加载 + 字节码生成）。</p>
 *
 * <p><b>前置条件：</b>需配置 {@code ydsz.json.warmup-enabled=true}（默认 false）。</p>
 *
 * <p><b>对标行业实践：</b></p>
 * <ul>
 *   <li>对标 Jackson 的 {@code ObjectMapper.copy()} 启动预加载</li>
 *   <li>对标 FastJSON2 {@code JSON.registerTypeConverter} 初始化</li>
 *   <li>对标 Spring Boot {@code ApplicationRunner} 启动任务模式</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.2.1
