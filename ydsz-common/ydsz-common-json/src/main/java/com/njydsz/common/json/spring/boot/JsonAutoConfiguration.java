 ...
 *
 * <p><b>与 Spring Boot Jackson 的关系（默认共存策略，A-3 修复）：</b> 本配置通过 {@code @AutoConfigureBefore} 声明在 {@code
 * JacksonAutoConfiguration} 之前加载， 并通过 {@code @ConditionalOnMissingBean} 占位 HTTP 消息转换器，使业务 REST 接口走
 * YdszJson。 默认<b>不</b>排除 {@code JacksonAutoConfiguration}——Spring 容器仍注册 {@code ObjectMapper} Bean，供
 * Actuator / springdoc-openapi 等内部组件使用，构成"可控并存"的双引擎格局。
 *
 * <p>如需强隔离（全仓库唯一 JSON 底座，容器不注册 {@code ObjectMapper} Bean）， 可在配置文件中显式设置 {@code
 * ydsz.json.disable-jackson-auto-configuration=true}， 此时 {@link
 * JacksonExclusionEnvironmentPostProcessor} 会将 {@code JacksonAutoConfiguration} 加入 {@code
...
public class JsonAutoConfiguration {
  ...
  /**
   * 命名策略转换器，使 {@code ydsz.json.naming-strategy=SNAKE_CASE} 等 YAML 配置 能绑定到 {@link
   ...
   * 自动预热 Runner（P1-E2）。
   *
   * <p>仅在 {@code ydsz.json.warmup-enabled=true} 时注册，应用启动阶段自动扫描 Spring 容器中的 Controller Bean，提取
   * {@code @RequestBody}/{@code @ResponseBody} 类型 并调用 {@link
   * com.njydsz.common.json.YdszJson#warmup(Class...)} 触发缓存构建。
   *
   ...
   * YdszJson 配置 Bean。
   *
   * <p>使用 {@code @PostConstruct} 在依赖注入完成后初始化全局配置， 确保所有 {@code JsonModule} Bean 已就绪后再注册到 {@link
   * JsonModuleRegistrar}。
   */
  public static class JsonConfigBean {
     ...
     *
     * <ul>
     *   <li>确保 {@code springModules} 依赖注入完成后再注册模块
     *   <li>避免构造函数中调用可被重写的方法（构造函数陷阱）
     *   <li>更好的可测试性：可在测试中构造 Bean 而不触发初始化
     * </ul>
     */
    @PostConstruct
    public void init() {
      ...
      try {
        strategy =
            JsonConfig.CircularReferenceStrategy.valueOf(properties.getCircularReferenceStrategy());
      ...
      JsonConfig newConfig =
          JsonConfig.builder()
              ...
              .writeNulls(properties.getIsWriteNulls())
              .prettyPrint(properties.getIsPrettyPrint())
              .serializeEnumUsingOrdinal(properties.getIsSerializeEnumUsingOrdinal())
              ...
              .useBigDecimal(properties.getIsUseBigDecimal())
              .wrapRootValue(properties.getIsWrapRootValue())
              .failOnError(properties.getIsFailOnError())
              .failOnUnknownProperties(properties.getIsFailOnUnknownProperties())
              .build();
