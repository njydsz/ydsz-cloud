 ...
 *
 * <p>暴露 JSON 引擎运行时配置与缓存状态，便于运维通过 JConsole / VisualVM / Prometheus 采集。</p>
 *
 * <p><b>ObjectName：</b>{@code com.njydsz.common.json:type=JsonConfigViewer}</p>
 *
 * @author ydsz-team
 * @since 1.2.1
 */
public interface JsonConfigViewerMBean {
     ...
     * 获取当前全局配置版本号。
     *
     * <p>每次 {@code JsonConfig.install()} 自增，可用于检测配置热更新是否生效。</p>
     *
     * @return 配置版本号
     */
     ...
     *
     * <p>用于配置热更新后手动失效旧缓存。操作后 {@link #getBeanSerializerCacheSize()} 应返回 0。</p>
     */
    void clearAllCaches();
}
