 ...
 *
 * <p>将 JSON 引擎内部状态（全局配置版本、命名策略、循环引用策略、SerializerCache 大小、
 * 估算的 ThreadLocal 内存占用）暴露到 JMX，便于运维排查配置泄漏、内存异常等问题。</p>
 *
 * <p><b>运维场景：</b></p>
 * <ul>
 *   <li>配置热更新验证：对比修改前后 {@link #getConfigVersion()} 是否自增</li>
 *   <li>缓存膨胀诊断：通过 {@link #getBeanSerializerCacheSize()} 判断是否有类加载器泄漏</li>
 *   <li>ThreadLocal 泄漏检测：通过 JMX 监控 {@code estimateThreadLocalMemory()}</li>
 * </ul>
 *
 * <p>由 {@link JsonAutoConfiguration.JsonConfigBean} 在 {@code @PostConstruct} 阶段
 * 自动注册到平台 MBeanServer，无需业务代码介入。</p>
 *
 * @author ydsz-team
 * @since 1.2.1
 */
public class JsonConfigViewer implements JsonConfigViewerMBean {
     ...
     *
     * <p>若同一 MBean 已注册（如容器热部署场景），先注销旧实例再注册新实例，
     * 避免 {@link javax.management.InstanceAlreadyExistsException}。</p>
     */
    public void register() {
        try {
            ...
            LOGGER.warn("JsonConfigViewer JMX MBean 注册失败，ObjectName={}，reason={}",
                    OBJECT_NAME, e.getMessage());
        }
    }
    ...
    public void unregister() {
        try {
            ...
            LOGGER.warn("JsonConfigViewer JMX MBean 注销失败，ObjectName={}，reason={}",
                    OBJECT_NAME, e.getMessage());
        }
    }
