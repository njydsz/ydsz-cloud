 ...
 * 高性能 JSON 写入器
 *
 * <p>直接操作 char[] 数组，避免 StringBuilder 的方法调用开销
 *
 * <p><b>性能优势：</b>
 *
 * <ul>
 ...
 * <p><b>Feature 系统：</b>
 *
 * <p>通过 {@link Feature} 枚举控制序列化行为，参考 FastJSON2 和 Jackson 的 Feature 设计。 使用 {@link #of(Feature...)} 或
 * {@link #of(Set)} 计算特性标志位。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public final class JSONWriter {
   ...
   * 写入特性枚举
   *
   * <p>用于控制序列化行为，参考 FastJSON2 和 Jackson 的 Feature 设计。
   */
  public enum Feature {
    /** 输出 null 值字段 */
    WriteNulls(false),
   ...
   * 构造函数（指定缓冲区大小和预计算特性位）
   *
   * <p>通过传入预计算的 features 位掩码，避免运行时 ThreadLocal 查询开销。 参见 {@link #of(Feature...)} 或 {@link
   * com.njydsz.common.json.internal.JsonRuntimeConfig}。
   *
   * @param capacity 初始容量
   ...
   *
   * <ul>
   *   <li>整数 BigDecimal（scale=0 或可通过缩放转为 long）：直接写入 long 数值，避免 toPlainString()
   *   <li>小 scale（≤10）且 unscaled 值在 long 范围内：整数部分 + 小数部分分别写入
   *   <li>其他情况：回退 toPlainString()
   * </ul>
   *
   * <p>使用 {@link BigDecimal#toPlainString()} 而非 {@link BigDecimal#toString()}， 避免科学计数法输出（如 1E+2），保证
   * JSON 数字格式合法。
   *
   * <p>当 {@link Feature#WriteBigDecimalAsString} 启用时， BigDecimal 将作为 JSON 字符串（带引号）写入，避免 JavaScript
   * 精度丢失。
   *
   ...
   *
   * <ul>
   *   <li>几何增长：新容量 = max(当前容量 * 2, 所需容量)，摊还 O(1) 扩容成本
   *   <li>上限保护：超过 MAX_BUF_SIZE 时停止扩容
   * </ul>
   */
   ...
   *
   * <p>根据对象类型估算 JSON 输出大小，一次性分配足够容量：
   *
   * <ul>
   *   <li>Bean：字段数 * 64 + 32
   *   <li>Collection：元素数 * 64 + 16
   *   <li>Map：条目数 * 64 + 16
   * </ul>
   *
   * @param obj 待序列化对象
   ...
   *
   * <p>避免 {@code new String(buf).getBytes(UTF_8)} 的双重分配： 先创建 String 再创建 byte[]。本方法对于纯 ASCII 内容直接
   ...
   *
   * <p>当缓冲区容量超过 MAX_RESET_CAPACITY 时，缩容到默认大小， 防止偶尔序列化大对象后，缓冲区一直占用大量内存
   */
  public void reset() {
    pos = 0;
   ...
   *
   * <p>除控制字符、引号、反斜杠外，还需转义 U+2028/U+2029（裸置于 {@code <script>} 中会导致 JS 语法错误）以及代理码元（D800-DFFF，需经
   * {@code writeStringWithEscape} 校验是否为合法代理对）。
   *
   * @param c 待判断字符
   ...
   *
   * <p>优化：对于能放入 long 范围内的 BigInteger，直接使用 writeLong 写入。
   */
   ...
   *
   * <p>优化：对于 AtomicLong/AtomicInteger/LongAdder/DoubleAdder 等原子类型， 直接拆箱原始类型写入，避免 toString() 分配。
   */
