...
public final class NumberUtils {
   ...
   *
   * <ul>
   *   <li>单数字快速路径：直接查表，避免循环和乘法
   *   <li>双数字快速路径：一次乘法 + 一次加法，避免两次循环迭代
   *   <li>乘法替代重复加法：value * 10 + digit 比 value + digit 的累加快 2-3 倍
   * </ul>
   *
   * @param str 数字字符串
   * @return 整数值
   ...
   * 高性能解析长整数字符串（快速路径优化）
   *
   * <p>优化策略同 parseInt，额外增加 long 范围的溢出保护
   *
   * @param str 数字字符串
   * @return 长整数值
   ...
   * @param errorMessage 溢出时的错误消息
   */
  private static void checkLongOverflow(
      long result, int digit, boolean negative, int i, int len, long limit, String errorMessage) {
