 ...
 *
 * <ul>
 *   <li>首次获取锁：设置 Hash 字段值为 1
 *   <li>重入获取：原子性递增计数
 *   <li>释放锁：原子性递减计数，计数为 0 时删除整个 Hash
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class RedisReentrantLock extends AbstractRedisDistributedLock {
   ...
   * 设置最大重入深度
   *
   * <p>超过该深度后，同一客户端再次重入时将返回 null（获取失败）， 防止编程错误导致的无限重入（最终锁无法释放）。 默认值为 {@value
   * #DEFAULT_MAX_REENTRANCE_DEPTH}。
   *
  ...
  public void setMaxReentranceDepth(int maxReentranceDepth) {
    if (maxReentranceDepth < 1) {
      throw new IllegalArgumentException(
          "maxReentranceDepth must be >= 1, got: " + maxReentranceDepth);
    }
  ...
  @Override
  protected String doAcquireLock(
    ...
    try {
      ...
      if (result != null && result == -1L) {
        log.warn(
            "[ydsz-lock]超过最大重入深度 | lockKey={} | clientId={} | maxDepth={}",
            lockKey,
            clientId,
            maxReentranceDepth);
        return null;
  ...
  public int getHoldCount(String lockKey, String lockValue) {
    try {
      Long result =
          stringRedisTemplate.execute(
              getHoldCountScript, Collections.singletonList(lockKey), lockValue);
