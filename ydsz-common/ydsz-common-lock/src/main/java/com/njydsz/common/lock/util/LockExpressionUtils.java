 ...
 * <ul>
 *   <li><b>模板模式</b>：{@code "order:#{#orderId}"}，逐段替换 {@code #{...}} 占位符，
 *       保留模板中的常量前缀</li>
 *   <li><b>整串 SpEL 模式</b>：{@code "'order:' + #orderId"}，将整个 key 作为 SpEL 表达式求值，
 *       解析失败时回退为原字符串</li>
 * </ul>
 *
...
public final class LockExpressionUtils {
     ...
     * 解析锁键表达式
     *
     * <p>按 {@code #{...}} 占位符或整串 SpEL 两种模式解析，参数通过方法参数名绑定。</p>
     *
    ...
    private static Object evaluate(String expression, Method method, Object[] args) {
        Expression parsed = EXPRESSION_CACHE.computeIfAbsent(expression, EXPRESSION_PARSER::parseExpression);
