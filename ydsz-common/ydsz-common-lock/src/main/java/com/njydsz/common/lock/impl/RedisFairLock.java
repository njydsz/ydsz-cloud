 ...
 * Redis 公平分布式锁实现
 *
 * <p>基于 Redis List 队列实现公平调度，按客户端请求顺序获取锁（先到先得）。 内部使用 Lua 脚本保证入队、出队、锁获取的原子性。
 *
 * <p><b>实现机制：</b>
 *
 * <ul>
 *   <li>队列管理：通过 Redis List 维护等待队列，新请求追加到队尾
 *   <li>原子调度：Lua 脚本检查队首客户端，仅队首客户端可获取锁
 *   <li>可重入支持：同一客户端可多次获取锁，内部维护重入计数
 *   <li>LPOS 快速路径：Redis 6.2+ 使用 LPOS O(1) 检查客户端是否已在队列中， 低版本自动回退到 LINDEX O(N) 遍历
 *   <li>原子 EXPIRE：队列 TTL 续期合并到 Lua 脚本中，避免非原子操作窗口
 * </ul>
 *
