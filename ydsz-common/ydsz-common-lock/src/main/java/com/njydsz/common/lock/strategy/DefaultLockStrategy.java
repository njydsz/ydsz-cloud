 ...
 * 默认锁策略实现
 *
 * <p>基于锁类型缓存的锁策略实现，根据 LockType 创建并缓存对应的分布式锁实例， 避免重复创建锁对象，提高性能。
 *
 * <p><b>支持的锁类型：</b>
 *
 * <ul>
 *   <li>REENTRANT - 可重入锁（默认）
 *   <li>FAIR - 公平锁
 * </ul>
 *
 * <p><b>不支持的锁类型：</b>{@link LockType#READ_WRITE} 与 {@link LockType#SEMAPHORE} 为键维度实例，无法通过注解方式创建，请使用
...
public class DefaultLockStrategy implements LockStrategy {
  ...
  public DefaultLockStrategy(
      ...
      String namespace,
      LockProperties.MultiLock multiLockConfig,
      LockReleaseNotifier lockReleaseNotifier) {
  ...
  public RedisSemaphore getSemaphore(String key, int permits) {
    ...
    return new RedisSemaphore(
        redisStringOps, redisTemplate, key, permits, DEFAULT_EXPIRE_MILLIS, scheduler, namespace);
  }
  ...
  public RedisReadWriteLock getReadWriteLock(String key, long expireMillis, long waitMillis) {
    ...
    return new RedisReadWriteLock(
        redisStringOps, redisTemplate, key, expireMillis, waitMillis, namespace, scheduler);
  }
  ...
  public void stopWatchDog(String lockKey) {
    ...
    String namespacedKey =
        (namespace != null && !namespace.isEmpty()) ? namespace + ":lock:" + lockKey : lockKey;
