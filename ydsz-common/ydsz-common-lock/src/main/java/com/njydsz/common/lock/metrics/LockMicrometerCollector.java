 ...
 *
 * <p>独立的顶级类，通过 {@code @ConditionalOnClass(MeterRegistry.class)} 控制加载， 避免对 Micrometer 的编译期硬依赖。
 *
 * <p>所有 Counter/Timer 实例在首次使用时懒加载并缓存，record 方法直接调用 缓存实例的 {@code .increment()} / {@code
 * .record()}，避免每次创建 Builder 对象。
 *
 * <p>注册的指标：
 *
 * <ul>
 *   <li>{@code lock.acquire.total} - 获取锁成功总数（Counter，标签: lock_type）
 *   <li>{@code lock.acquire.failed.total} - 获取锁失败总数（Counter，标签: lock_type）
 *   <li>{@code lock.release.total} - 释放锁总数（Counter，标签: lock_type）
 *   <li>{@code lock.hold.duration} - 锁持有耗时（Timer，标签: lock_type，P50/P90/P99/P999）
 *   <li>{@code lock.wait.duration} - 锁等待时间分布（Timer，标签: lock_type，P50/P90/P99/P999）
 *   <li>{@code lock.active.locks} - 当前活跃锁数量（Gauge）
 *   <li>{@code lock.timeout.count} - 锁超时次数（Counter，标签: lock_type）
 *   <li>{@code lock.watchdog.renew.count} - 续期次数（Counter，标签: lock_type）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class LockMicrometerCollector {
  ...
  void recordAcquireSuccess(long waitTimeMillis, String lockType) {
    counter("lock.acquire.total", lockType, "Total number of successful lock acquisitions")
        .increment();
  ...
  void recordAcquireFail(String lockType) {
    counter("lock.acquire.failed.total", lockType, "Total number of failed lock acquisitions")
        .increment();
  }
