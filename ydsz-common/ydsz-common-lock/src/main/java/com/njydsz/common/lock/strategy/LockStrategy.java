 ...
 * 锁策略工厂接口
 *
 * <p>根据 {@link LockType} 创建并返回对应的分布式锁实例， 同时提供读写锁、信号量与多 Key 联锁的创建方法。
 *
...
public interface LockStrategy {
   ...
   * 根据锁类型获取对应的分布式锁实例
   *
   * <p>仅支持 {@link LockType#REENTRANT} 与 {@link LockType#FAIR}； 读写锁与信号量请使用 {@link #getReadWriteLock}
   * / {@link #getSemaphore} 获取。
   *
   ...
   * 获取 Redis 多 Key 联锁实例
   *
   * <p>至少需要 2 个底层锁，全部获取成功才算成功，任一失败回滚全部。 续期参数（间隔/次数上限）取 {@code ydsz.lock.multi-lock} 配置。
   *
   ...
   * 停止指定锁键的看门狗续期
   *
   * <p>当 {@code @YdszDistributedLock(autoRenew = false)} 时调用， 停止看门狗对指定锁的续期任务。
   *
