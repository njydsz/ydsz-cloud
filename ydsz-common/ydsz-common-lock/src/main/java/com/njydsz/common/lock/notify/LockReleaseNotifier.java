 ...
 * 锁释放通知器
 *
 * <p>对标 Redisson 的发布订阅唤醒机制：锁释放时向 Redis 频道广播通知，
 * 等待锁的线程通过订阅收到通知后立即重新尝试，替代高竞争场景下的
 * 指数退避轮询，降低无效 Redis QPS 与等待延迟。
 *
 * <p><b>工作机制：</b>
 * <ul>
 *   <li>启动时按 {@code ydsz:lock:release:*} 模式订阅一次，收到消息后唤醒对应锁键的本地等待者</li>
 *   <li>{@link #awaitRelease} 注册等待者并以 {@link CompletableFuture} 阻塞等待，被唤醒或超时后返回</li>
 *   <li>{@link #notifyRelease} 发布 Redis 消息并同时唤醒本地等待者（双保险，兼容订阅延迟）</li>
 *   <li>等待上限 {@value #MAX_AWAIT_MILLIS} ms，即使错过通知也能在限定时间内重新探测锁状态</li>
 * </ul>
 *
...
public class LockReleaseNotifier {
     ...
     */
    private final ConcurrentHashMap<String, List<CompletableFuture<Void>>> waiters = new ConcurrentHashMap<>();
     ...
     */
    public LockReleaseNotifier(StringRedisTemplate redisTemplate,
     ...
     * 等待指定锁的释放通知
     *
     * <p>阻塞至收到释放通知或超时。超时后调用方应重新尝试获取锁。</p>
     *
     * @param lockKey       锁的键
     ...
     * 发布锁释放通知
     *
     * <p>向 Redis 频道广播释放消息，并直接唤醒本 JVM 内的等待者（兼容订阅建立延迟）。</p>
     *
     * @param lockKey 锁的键
     */
