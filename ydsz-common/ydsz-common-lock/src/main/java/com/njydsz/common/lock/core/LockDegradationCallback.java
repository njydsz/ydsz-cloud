 ...
 * 锁降级回调接口
 *
 * <p>当分布式锁因 Redis 不可用而降级为本地 {@link java.util.concurrent.locks.ReentrantLock} 时，
 * 触发此回调通知业务方。降级后仅保证单 JVM 内的互斥，分布式一致性无法保证。
 *
 * <p>业务方可实现此接口执行自定义逻辑，如：
 * <ul>
 *   <li>发送告警通知（短信/邮件/钉钉）</li>
 *   <li>切换到只读模式</li>
 *   <li>触发业务熔断</li>
 *   <li>记录降级审计日志</li>
 * </ul>
...
public interface LockDegradationCallback {
     ...
     * 默认空实现（不做任何事）
     */
    LockDegradationCallback NO_OP = new LockDegradationCallback() {
        @Override
