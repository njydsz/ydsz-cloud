 ...
 * 编程式锁操作模板 - 简化分布式锁的使用
 *
 * <p>提供统一的 try-with-resources 风格 API，自动管理锁的获取与释放， 避免手动 try-finally 样板代码。
 *
 * <h3>使用示例</h3>
 *
 * <pre>{@code
...
public class LockTemplate {
  ...
  public <T> T executeOrDefault(
      String lockKey, long leaseTime, TimeUnit timeUnit, Supplier<T> action, T defaultValue) {
    return tryExecuteOrDefault(
        lockKey, LockRequest.withoutWait(leaseTime, timeUnit), action, defaultValue);
  }
  ...
  public <T> T executeOrDefault(
      String lockKey, long waitTime, long leaseTime, TimeUnit timeUnit, Supplier<T> action) {
    return tryExecuteOrDefault(
        lockKey, LockRequest.of(waitTime, leaseTime, timeUnit), action, null);
  }
   ...
   * @return 业务逻辑的返回值或默认值
   */
  private <T> T tryExecuteOrDefault(
      String lockKey, LockRequest request, Supplier<T> action, T defaultValue) {
    DistributedLocker lock = lockStrategy.getLock(LockType.REENTRANT);
    String lockValue;
    try {
      lockValue =
  ...
  private String acquireLock(
      DistributedLocker lock, String lockKey, long waitTime, long leaseTime, TimeUnit timeUnit) {
    try {
      String lockValue =
          waitTime > 0
