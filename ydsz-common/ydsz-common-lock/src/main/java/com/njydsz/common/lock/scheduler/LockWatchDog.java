 ...
 *
 * <ul>
 *   <li>防止业务执行时间超过锁过期时间导致锁被自动释放
 *   <li>续期操作使用 Lua 脚本保证原子性
 *   <li>使用独立线程池管理续期任务
...
public class LockWatchDog {
   ...
   * 最大续期次数限制
   *
   * <p>使用 volatile 保证可见性：该字段可能由主线程通过 {@link #setMaxRenewTimes} 修改， 而续期任务在调度线程中读取。缺少 volatile
   * 可能导致调度线程读到陈旧值。
   */
   ...
   * 启动续期任务的互斥锁
   *
   * <p>使用 ReentrantLock 替代 synchronized，避免 JDK 21 虚拟线程被固定（pinning）。 synchronized 块在 JDK 21
   * 中会导致虚拟线程固定到载体平台线程， ReentrantLock 则基于 AQS，虚拟线程可以正常 unpark。
   */
   ...
   *
   * <ul>
   *   <li>REENTRANT：使用 HEXISTS 检查 clientId 作为 Hash field
   *   <li>FAIR：使用 HGET 'owner' 检查 clientId 作为 owner 值
   * </ul>
   *
   * @param lockKey 锁的键
   * @param clientId 客户端标识
  ...
  public void startWatch(String lockKey, String clientId, long leaseTime, LockType lockType) {
    ...
    try {
      ...
      log.info(
          "[ydsz-lock] [watchdog]启动续期任务 | lockKey={} | leaseTime={}ms | interval={}ms | lockType={}",
          lockKey,
          leaseTime,
          renewInterval,
          lockType);
   ...
   * 判断指定锁是否正在被看门狗续期。
   *
   * <p>仅当续期任务存在、运行标记为 true 且底层定时任务未完成/未取消时返回 {@code true}； 用于锁泄漏检测与资源清理判定。
   *
   * @param lockKey 锁的键
   ...
   * 周期扫描活跃续期任务，检测锁泄漏
   *
   * <p>当某把锁的续期次数超过最大续期限制的 80% 时，记录 WARN 日志； 达到最大续期限制时，记录 ERROR 日志。
   */
  ...
  public void detectLockLeak() {
    ...
    for (Map.Entry<String, WatchTask> entry : activeTasksSnapshot.entrySet()) {
      ...
      if (renewCount >= maxRenewTimesLocal) {
        log.error(
            "[ydsz-lock] [watchdog]锁续期次数已达最大限制，可能泄漏 | lockKey={} | renewCount={}/{} | leaseTime={}ms",
            lockKey,
            renewCount,
            maxRenewTimesLocal,
            task.getLeaseTime());
        ...
        log.warn(
            "[ydsz-lock] [watchdog]锁续期次数接近最大限制，可能泄漏 | lockKey={} | renewCount={}/{} | leaseTime={}ms",
            lockKey,
            renewCount,
            maxRenewTimesLocal,
            task.getLeaseTime());
    ...
    if (lockMetrics != null && !activeTasksSnapshot.isEmpty()) {
      log.debug(
          "[ydsz-lock] [watchdog]活跃续期任务数={} | maxRenewTimes={}",
          activeTasksSnapshot.size(),
          maxRenewTimesLocal);
    }
  }
  ...
  private void renewLockWithRetry(
      String lockKey, String clientId, long leaseTime, LockType lockType) {
    WatchTask currentTask = activeTasks.get(lockKey);
    if (currentTask != null && currentTask.renewCount >= maxRenewTimes) {
      log.warn(
          "[ydsz-lock] [watchdog]续期次数超过最大限制（{}次），停止续期，锁将自动过期 | lockKey={}", maxRenewTimes, lockKey);
      stopWatch(lockKey);
      return;
    }
    ...
    for (int retry = 0; retry < MAX_RETRY_COUNT; retry++) {
      try {
        boolean success =
            renewalService.renew(stringRedisTemplate, lockKey, clientId, leaseTime, lockType);
        if (success) {
        ...
        log.warn(
            "[ydsz-lock] [watchdog]锁续期失败，可能锁已释放 | lockKey={} | lockType={}", lockKey, lockType);
        stopWatch(lockKey);
        return;
      } catch (Exception e) {
        log.warn(
            "[ydsz-lock] [watchdog]锁续期异常 | lockKey={} | lockType={} | retry={}/{} | error={}",
            lockKey,
            lockType,
            retry + 1,
            MAX_RETRY_COUNT,
