 ...
 * 分布式锁自动配置类
 *
 * <p>基于 Spring Boot AutoConfiguration 机制，自动装配分布式锁相关的 Bean， 包括锁策略、看门狗、指标收集器、AOP 切面等组件。
 *
 * <p><b>配置项：</b>
 *
 * <ul>
 *   <li>ydsz.lock.enabled - 是否启用分布式锁（默认 true）
 *   <li>ydsz.lock.fallback-enabled - 是否启用降级策略
 *   <li>ydsz.lock.watchdog-enabled - 是否启用看门狗自动续期
 *   <li>ydsz.lock.max-renew-times - 最大续期次数
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class DistributedLockAutoConfiguration {
   ...
   * 创建锁事件监听器 Bean
   *
   * <p>业务方可实现 {@link LockEventListener} 接口并注册为 Bean， 感知锁生命周期事件（获取、释放、超时、续期失败）。
   *
   ...
   * 创建统一的锁续期服务 Bean。
   *
   * <p>v1.2.0 引入，将散落在各锁实现中的续期 Lua 脚本统一收口，消除双锁冗余。 业务方可覆盖此 Bean 以注入自定义 {@link
   ...
   * 创建看门狗 Bean。
   *
   * <p>v1.2.0 变更：通过 {@link LockRenewalService} 统一执行续期， 不再在本地维护续期 Lua 脚本。
   *
  ...
  public LockWatchDog lockWatchDog(
      @Qualifier("lockWatchDogScheduler") TaskScheduler lockWatchDogScheduler,
   ...
   * 创建锁释放通知器 Bean。
   *
   * <p>依赖 common-redis 提供的 {@link RedisMessageListenerContainer}， 基于 Redis pub/sub
   * 在锁释放时唤醒等待线程，减少高竞争场景下的空轮询。 容器未配置时该 Bean 不注册，等待路径自动退化为指数退避轮询。
   *
   * @param stringRedisTemplate Redis 模板
  ...
  public LockReleaseNotifier lockReleaseNotifier(
      StringRedisTemplate stringRedisTemplate, RedisMessageListenerContainer listenerContainer) {
   ...
   * LockReleaseNotifier）通过 {@link LockOptionalDependencies} 聚合注入， 避免 Bean 方法参数超过 5 个（云顶编码规范 5.4 节）。
   *
   * @param stringRedisTemplate Redis 模板
  ...
  public LockStrategy lockStrategy(
    ...
    DefaultLockStrategy strategy =
        new DefaultLockStrategy(
            stringRedisTemplate,
            lockWatchDog,
  ...
  public LockOptionalDependencies lockOptionalDependencies(
    ...
    return new LockOptionalDependencies(
        stringOpsProvider,
        redisTemplateProvider,
        schedulerProvider,
        renewalServiceProvider,
        notifierProvider,
        lockEventListener,
        waitTimePolicyProvider);
  }
  ...
  @Bean
  @ConditionalOnMissingBean
  public YdszDistributedLockAspect distributedLockAspect(
      LockStrategy lockStrategy, LockMetrics lockMetrics, LockProperties lockProperties) {
    YdszDistributedLockAspect aspect =
        new YdszDistributedLockAspect(lockStrategy, lockProperties.isFallbackEnabled());
   ...
   * 确保多节点部署时同一 @Scheduled 任务同一时刻只有一个节点执行。 获取不到锁的节点直接跳过本次执行（非阻塞）。
   *
  ...
  @Bean
  @ConditionalOnMissingBean
  public DistributedScheduledAspect distributedScheduledAspect(
   ...
   * 锁策略可选依赖聚合
   *
   * <p>用于收敛 {@link LockStrategy} Bean 方法的参数数量 （云顶编码规范 5.4 节：方法参数不超过 5 个）。
   *
   * @param stringOpsProvider RedisStringOps 提供者
   * @param redisTemplateProvider RedisTemplate 提供者
   * @param schedulerProvider TaskScheduler 提供者
