 ...
 * 分布式锁 AOP 切面
 *
 * <p>拦截带有 @YdszDistributedLock 注解的方法，在方法执行前后进行加锁和解锁操作。 支持 SpEL 表达式解析锁的键（委托 {@link
 * LockExpressionUtils}）。
 *
 * <p>执行流程： 1. 解析方法上的 @YdszDistributedLock 注解 2. 使用 SpEL 解析锁的键 3. 根据锁类型获取对应的锁实例 4. 尝试获取锁，失败则按配置重试 5.
 * 执行目标方法 6. 方法执行完成后释放锁
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class YdszDistributedLockAspect {
   ...
   */
  private final Map<LockType, FallbackDistributedLock> fallbackLockCache =
  ...
  public Object around(ProceedingJoinPoint joinPoint, YdszDistributedLock lockAnn) {
    ...
    if (fallbackEnabled && !(lock instanceof FallbackDistributedLock)) {
      lock =
          fallbackLockCache.computeIfAbsent(
              lockType, lt -> new FallbackDistributedLock(lockStrategy.getLock(lt), true));
    }
   ...
   * 带重试机制的锁获取
   *
   * <p>采用指数退避策略：每次重试间隔 = retryInterval * 2^attempt， 重试次数上限 {@value #MAX_RETRY_CAP} 次。
   *
   * @param lock 锁实例
   * @param lockKey 锁键
  ...
  private String acquireLockWithRetry(
      DistributedLocker lock, String lockKey, YdszDistributedLock lockAnn) {
    long waitTime = lockAnn.waitTime();
    long leaseTime = lockAnn.leaseTime();
    TimeUnit timeUnit = lockAnn.timeUnit();
    int retryCount = lockAnn.retryCount();
    ...
    while (attempt <= maxRetries) {
      ...
      if (attempt <= maxRetries) {
        ...
        log.debug(
            "[ydsz-lock]获取锁失败，第 {} 次重试，等待 {} ms | lockKey={}", attempt, backoffDelay, lockKey);
        try {
   ...
   *
   * <ul>
   *   <li>模板模式：{@code "order:#{#orderId}"}
   *   <li>整串 SpEL 模式：{@code "'order:' + #orderId"}
   * </ul>
   *
