 ...
 *
 * <ul>
 *   <li>tryLock 时先尝试 Redis 锁，捕获异常后降级为本地锁
 *   <li>unlock 时先尝试 Redis 释放，失败则释放本地锁
 *   <li>通过配置开关 ydsz.lock.fallback-enabled 控制是否启用降级（默认 true）
 * </ul>
 *
 * <p><b>自动恢复：</b>
 *
 * <ul>
 *   <li>连续失败达到阈值后，全局标记 Redis 不可用，直接走本地锁
 *   <li>Redis 恢复后，自动探测并切换回分布式锁模式
 *   <li>通过 {@code ydsz.lock.fallback-recovery-threshold} 控制探测间隔
 * </ul>
 *
 * <p><b>注意事项：</b>
 *
 * <ul>
 *   <li>降级为本地锁后，分布式一致性无法保证，仅保证服务不因 Redis 故障而中断
 *   <li>适用于对一致性要求不高、但可用性优先的场景
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class FallbackDistributedLock implements DistributedLocker {
  ...
  private final Cache<String, ReentrantLock> localLocks =
      YdszCache.<String, ReentrantLock>newBuilder()
  ...
  public void setDegradationCallback(LockDegradationCallback degradationCallback) {
    this.degradationCallback =
        degradationCallback != null ? degradationCallback : LockDegradationCallback.NO_OP;
  }
   ...
   */
  private String acquireLocalLock(String lockKey, long waitTime, TimeUnit timeUnit)
      throws InterruptedException {
    ReentrantLock localLock = localLocks.get(lockKey, k -> new ReentrantLock());
  ...
  private void onRedisFailure(String lockKey, Exception e) {
    ...
    if (failures >= FAILURE_THRESHOLD && redisAvailable.compareAndSet(true, false)) {
      log.error(
          "[ydsz-lock] [fallback]Redis 连续失败 {} 次，标记为不可用，全局切换到本地锁模式 | "
              + "lockKey={} | lastError={}",
          failures,
          lockKey,
          e.getMessage());
      ...
      try {
        ...
        log.warn(
            "[ydsz-lock] [fallback]降级回调执行异常 | lockKey={} | error={}",
            lockKey,
            callbackEx.getMessage());
      }
    }
    log.error(
        "[ydsz-lock] [fallback]Redis 锁操作异常，降级为本地锁 | lockKey={} | error={}",
        lockKey,
        e.getMessage());
    degradedKeys.put(lockKey, Boolean.TRUE);
  }
    ...
    try {
      ...
      if (lockValue != null) {
        ...
        try {
          ...
          log.warn(
              "[ydsz-lock] [fallback]恢复回调执行异常 | lockKey={} | error={}",
              lockKey,
              callbackEx.getMessage());
        }
        return lockValue;
  ...
  /** 探测 Redis 是否恢复（带等待版本） */
  private String probeRedisRecoveryWithWait(
