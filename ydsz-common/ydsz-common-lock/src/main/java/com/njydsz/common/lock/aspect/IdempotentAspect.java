 ...
 *         <li>抛出 {@code BusinessException}（含子类）：立即释放幂等锁， 让客户端修正参数后可重试提交（项目硬约束 P2-9）
 ...
 *
 * <ul>
 *   <li>前缀：从 {@code ydsz.lock.idempotent.key-prefix} 读取（默认 {@code ydsz:idem:}）
 *   <li>可选命名空间：{@code ${ydsz.lock.namespace}:}（多服务共享 Redis 时隔离）
 *   <li>完整 key：{@code ${keyPrefix}${namespace}:${userKey}}
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class IdempotentAspect {
  ...
  public Object around(ProceedingJoinPoint joinPoint, Idempotent idempotent) {
    ...
    if (method.isAnnotationPresent(IdempotentExempt.class)) {
      log.debug(
          "[ydsz-lock] [idempotent] 方法标注 @IdempotentExempt，跳过幂等检查 method={}", method.getName());
    ...
    try {
      ...
      // 防止下游异常时客户端重试风暴击穿系统
      log.warn(
          "[ydsz-lock] [idempotent] 非 BusinessException 抛出，保留幂等锁 key={} cause={}",
          redisKey,
          e.getClass().getSimpleName());
      throw e;
    } catch (Throwable ex) {
      log.warn(
          "[ydsz-lock] [idempotent] 检查型异常包装后抛出，保留幂等锁 key={} cause={}",
          redisKey,
   ...
   *   <li>{@code #{...}} 包裹的 SpEL 表达式：解析后作为 key
  ...
  private String resolveUserKey(String keyExpression, Method method, Object[] args) {
    ...
    try {
      ...
      log.warn(
          "[ydsz-lock] [idempotent] SpEL 解析失败，降级为自动 key expr={} cause={}",
          keyExpression,
          e.getMessage());
      return generateAutoKey(method, args);
    }
