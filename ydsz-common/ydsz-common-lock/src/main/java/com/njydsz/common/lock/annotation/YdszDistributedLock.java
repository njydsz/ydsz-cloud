 ...
 *
 * <ul>
 *   <li>key：锁的键，支持 SpEL 表达式（模板 {@code "order:#{#orderId}"} 或整串 {@code "'order:' + #orderId"}）
 *   <li>lockType：锁类型，仅支持 REENTRANT / FAIR，默认 REENTRANT
 *   <li>waitTime：最大等待时间，默认 0（不等待）
 *   <li>leaseTime：锁的自动释放时间，默认 30 秒
 *   <li>timeUnit：时间单位，默认秒
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public @interface YdszDistributedLock {
   ...
   * 是否抛出异常
   *
   * <p>true：获取锁失败时抛出异常 false：获取锁失败时跳过方法执行，返回 null 或默认值
   *
   * @return 是否抛出异常
   */
