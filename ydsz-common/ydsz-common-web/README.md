...
| `ydsz.web.cors.enabled` | `true` | CORS 跨域开关 |
| `ydsz.web.cors.allow-credentials` | `false` | 是否允许 Cookie |
| `ydsz.web.cors.allowed-origin-patterns` | — | 允许的来源模式列表 |
| `ydsz.web.cors.allowed-headers` | `["*"]` | 允许的请求头 |
| `ydsz.web.cors.allowed-methods` | `["*"]` | 允许的请求方法 |
| `ydsz.web.cors.exposed-headers` | `[]` | 暴露的响应头 |
| `ydsz.web.cors.max-age` | `3600` | 预检缓存秒数 |
| `ydsz.web.cors.path-pattern` | `/**` | CORS 路径模式 |
| `ydsz.web.cors.order` | `0` | 过滤器顺序 |
...
| `ydsz.web.trace.log-level` | `INFO` | 日志级别（INFO / DEBUG） |
...
| `ydsz.api.version.deprecated-versions` | `[]` | 已废弃版本列表（返回 410 Gone） |
| `ydsz.api.version.sunset-headers` | `true` | 是否输出 Deprecation / Sunset 头（RFC 8594） |
| `ydsz.api.version.sunset-duration-days` | `90` | 废弃过渡期天数 |
...
| `ydsz.web.multipart.max-request-size` | `100MB` | 整个请求最大大小（含所有文件 + 表单字段） |
| `ydsz.web.multipart.file-size-threshold` | `0` | 写入磁盘的阈值（0 = 全部内存） |
| `ydsz.web.multipart.resolve-lazily` | `false` | 是否延迟解析 |
| `ydsz.web.multipart.location` | `""` | 临时文件目录（空 = Servlet 容器默认） |
...
- `ACCEPT`：`GET /api/users` + `Accept: application/vnd.ydsz.v1+json`
