...
public class FilterIgnoreProperties {
   ...
   * 是否用配置值整体替换内置默认值。
   *
   * <p>{@code false}（默认）：配置值与内置默认值合并； {@code true}：仅使用配置值，忽略内置默认值。
   */
   ...
   * 获取合并后的公共忽略 URL 集合。
   *
   * <p>{@code replaceBuiltin=true} 时仅返回配置值。
   *
   ...
   * 获取认证过滤器忽略服务名称集合。
   *
   * <p>默认与内置默认值合并；{@code replaceBuiltin=true} 时仅使用配置值。 若配置值非空，无论开关如何，均以配置值为主（合并时内置值打底）。
   *
   * @return 服务名称集合
   */
