 ...
 * Webhook 投递器默认实现。
 *
 * <p>使用内存 ConcurrentHashMap 管理订阅，RestTemplate 投递事件， HMAC-SHA256 签名，简单重试（3 次指数退避）。
 *
 * <p>连接池优化：
 *
 * <ul>
 *   <li>当 Apache HttpClient 在 classpath 时，使用 {@code HttpComponentsClientHttpRequestFactory} 启用连接池
 *   <li>否则降级为 {@code SimpleClientHttpRequestFactory}（无连接池）
 *   <li>连接超时、读取超时、最大连接数均通过 {@link WebhookProperties} 配置
 * </ul>
 *
...
public class DefaultWebhookDispatcher implements WebhookDispatcher {
  ...
  public DefaultWebhookDispatcher(
      ObjectProvider<RestTemplate> restTemplateProvider, WebhookProperties webhookProperties) {
  ...
  private List<WebhookSubscription> findMatchingSubscriptions(String eventType) {
    ...
    for (WebhookSubscription sub : subscriptions.values()) {
      if (Boolean.FALSE.equals(sub.getIsEnabled())) {
        continue;
      }
   ...
   *
   * <p>当 Apache HttpClient 在 classpath 时使用连接池实现， 否则降级为 SimpleClientHttpRequestFactory。
   */
  private RestTemplate configureRestTemplate(
      RestTemplate restTemplate, WebhookProperties properties) {
