 ...
 *       值为 {@code HMAC-SHA256(payload, secret)} 的 Base64 编码</li>
 *   <li><b>重试策略</b>：投递失败时按指数退避重试（默认 3 次）</li>
 *   <li><b>事件过滤</b>：仅向订阅了对应 {@code eventType} 的 URL 投递</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public interface WebhookDispatcher {
     ...
     * <p>投递逻辑：
     * <ol>
     *   <li>筛选 {@code enabled=true} 且 {@code eventTypes} 包含 {@code eventType} 的订阅</li>
     *   <li>对每个匹配的订阅，将 {@code payload} 序列化为 JSON 后 POST 到 {@code callbackUrl}</li>
     *   <li>在 HTTP Header 中附带 {@code X-Webhook-Signature}（HMAC-SHA256 签名）</li>
     *   <li>投递失败时按指数退避重试（默认 3 次），最终失败记录日志</li>
     * </ol>
     *
