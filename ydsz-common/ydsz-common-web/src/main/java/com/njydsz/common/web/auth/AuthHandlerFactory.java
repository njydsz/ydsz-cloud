 ...
 * Web 端认证处理器工厂
 *
 * <p>支持根据服务类型编码动态路由到对应的 {@link AuthHandler} 实现。
 * 通过 Spring 的依赖注入机制自动收集所有 {@link AuthHandler} 实现类，
 * 并按 Bean 名称进行路由分发。
 *
 * <p><b>路由规则：</b>
 * <ul>
 *   <li>{@code "appService"} → Bean 名称 {@code appAuthHandler}</li>
 *   <li>其他（{@code "webService"} 等）→ Bean 名称 {@code webAuthHandler}</li>
...
public class AuthHandlerFactory {
     ...
     *
     * <p>路由规则：{@code "appService"} → appAuthHandler，其余 → webAuthHandler；
    ...
    public AuthHandler getAuthHandler(String serviceTypeCode) {
        ...
        if (handler == null) {
            throw new IllegalStateException(
                    "无法为服务类型 [" + serviceTypeCode + "] 获取可用的 AuthHandler");
        }
        return handler;
    }
