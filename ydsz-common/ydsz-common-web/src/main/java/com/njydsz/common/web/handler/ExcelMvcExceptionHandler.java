 ...
 * Excel 模块异常处理器
 *
 * <p>在 Web 层桥接 Excel 自包含异常体系到全局统一响应格式。
 * 设置 {@code @Order(Ordered.HIGHEST_PRECEDENCE + 10)} 确保在
 * {@link com.njydsz.common.exception.handler.MvcExceptionHandler} 之前拦截，
 * 避免被通用 {@code Exception.class} 处理器兜底。</p>
 *
 * <p><b>设计意图：</b></p>
 * <ul>
 *   <li>Excel 模块保持零反向依赖，不感知全局异常体系</li>
 *   <li>Web 层作为上层模块，同时依赖 excel 和 exception，负责桥接</li>
 *   <li>如需 Excel 异常注册到全局错误码表，可在 web 层通过适配器实现</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class ExcelMvcExceptionHandler {
    ...
    private BaseResponse<?> handleExcelException(
            ExcelException e, HttpServletRequest request, HttpServletResponse response, String operation) {
        ...
        // 日志记录
        if (httpStatus >= 500) {
            log.error("Excel{}异常 | 路径: {} | 错误码: {} | 消息: {}",
                    operation, request.getRequestURI(), code, message, e);
        } else {
            log.warn("Excel{}异常 | 路径: {} | 错误码: {} | 消息: {}",
                    operation, request.getRequestURI(), code, message);
        }
