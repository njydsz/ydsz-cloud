 ...
 *
 * <p>自定义 HandlerMapping，在构建 RequestMappingInfo 时注入 {@link ApiVersionCondition}，
 * 实现基于版本的接口路由。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class ApiVersionRequestMappingHandlerMapping extends RequestMappingHandlerMapping {
    ...
    protected RequestCondition<?> getCustomTypeCondition(Class<?> handlerType) {
        ...
        if (apiVersion != null) {
            log.debug("发现类级 @ApiVersion 注解 | class={}, version={}", handlerType.getSimpleName(), apiVersion.value());
            return new ApiVersionCondition(apiVersion.value(), properties);
        }
        return null;
    }
