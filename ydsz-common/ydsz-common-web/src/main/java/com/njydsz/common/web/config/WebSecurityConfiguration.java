...
public class WebSecurityConfiguration {
   ...
   * 注册 403 访问拒绝处理器。
   *
   * <p>认证用户访问无权限资源时由 Spring Security 回调，统一返回标准 JSON 错误响应。 仅在容器中不存在其他 {@link AccessDeniedHandler}
   * 时装配（{@code @ConditionalOnMissingBean}）， 便于业务侧自定义覆盖。
   *
   ...
   * 注册 401 未认证入口点。
   *
   * <p>匿名请求触及受保护资源、认证信息缺失或失效时由 Spring Security 回调， 统一返回标准 JSON 错误响应。仅在容器中不存在其他 {@link
   * AuthenticationEntryPoint} 时装配， 便于业务侧自定义覆盖。
   *
   ...
   * 构建 Web 安全过滤器链。
   *
   * <p>项目采用<b>无状态 JWT Bearer 认证</b>（由自定义 {@code WebAuthFilter} 完成鉴权）， 因此本链：
   *
   * <ul>
   *   <li>关闭 CSRF（无 Cookie 会话，无 CSRF 攻击面）
   *   <li>会话策略设为 {@code STATELESS}，与 JWT 无状态模型一致，不创建/使用 HttpSession
   *   <li>所有请求放行（{@code permitAll}）：真正的鉴权与 401/403 响应由自定义 {@code WebAuthFilter}
   *       在链内完成，本链仅兜底接入异常处理器，避免业务侧 自定义 {@link SecurityFilterChain} 被覆盖
   * </ul>
   *
   * <p><b>注意：</b>若业务需接入 Spring Security 原生鉴权（如注解 {@code @PreAuthorize}）， 应自行提供 {@link
   * SecurityFilterChain}（本 Bean 通过 {@code @ConditionalOnMissingBean} 自动让位）， 并在自定义链中配置真正的 {@code
