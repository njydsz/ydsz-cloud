 ...
 * SpEL 模板处理器
 *
 * <p>负责解析和执行审计日志 content 字段中的 SpEL 表达式，支持动态内容拼接。 例如 {@code @Audit(content = "'删除用户:' + #userId")}
 * 中的 {@code #userId} 会被替换为实际方法参数。
 *
 * <p><b>性能与安全：</b>
 *
 * <ul>
 *   <li>使用 {@link SimpleEvaluationContext}（只读），禁止执行任意方法/构造器，避免安全风险
 *   <li>表达式解析结果按模板字符串做 LRU 风格缓存（容量上限 256）
 *   <li>SpEL 解析失败时降级返回原模板字符串，不影响审计主流程
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class AuditTemplateProcessor {
   ...
   * 判断模板是否需要 SpEL 解析
   *
   * <p>只有包含 {@code #}（引用变量）或 {@code T(}（类型引用）才会进行解析， 其余纯文本模板直接返回。
   *
   ...
   *
   * <p>使用 {@link SimpleEvaluationContext#forReadOnlyDataBinding()} 构建只读上下文， 避免执行任意 Java 方法带来的安全风险。
   *
   * @param method 目标方法
   * @param args 方法参数
   * @return 评估上下文
