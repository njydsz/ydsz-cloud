 ...
 * 健康信息数据载体
 *
 * <p>封装审计记录器的运行时健康状态，由 {@link AuditRecorder#health()} 方法返回，
 * 供 {@link com.njydsz.common.audit.health.AuditHealthIndicator} 直接使用。
 *
 * <p>不同实现可扩展额外字段（如 {@code queueSize}、{@code usageRatio}），
 * 通过 {@link #getDetails()} 传递给健康检查端点。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public class HealthInfo {
    ...
    public static HealthInfo up(Map<String, Object> details) {
        return new HealthInfo(Status.UP, details != null ? new LinkedHashMap<>(details) : new LinkedHashMap<>());
    }
    ...
    public static HealthInfo down(Map<String, Object> details) {
        return new HealthInfo(Status.DOWN, details != null ? new LinkedHashMap<>(details) : new LinkedHashMap<>());
    }
