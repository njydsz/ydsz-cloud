 ...
 * 用于监控审计记录器的运行状态，包括队列/缓冲区状态、丢弃计数等。
 * </p>
 *
 * <p>健康状态由 {@link AuditRecorder#health()} 多态分发，
 * 无需在指示器中做 instanceof 类型判断。</p>
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class AuditHealthIndicator implements HealthIndicator {
     ...
     * 执行健康检查
     *
     * <p>通过多态调用 {@link AuditRecorder#health()} 获取健康信息，
     * 再映射为 Spring Boot Actuator 的 {@link Health} 对象。
     *
    ...
    public Health health() {
        ...
        Health.Builder builder = healthInfo.getStatus() == HealthInfo.Status.DOWN
                ? Health.down()
                : Health.up();
