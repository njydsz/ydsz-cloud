 ...
 * JDBC 审计日志存储实现
 *
 * <p>将审计日志写入数据库表，支持分表策略。内部复用 Spring 容器中的 {@link NamedParameterJdbcTemplate}，避免每次创建新实例。
 *
 * <p><b>依赖说明：</b>本类使用 {@code javax.sql.DataSource}，该接口属于 JDK 标准库， 不受 Jakarta EE 迁移影响，在 Spring Boot
 * 3.x 中无需修改。
 *
 * <p><b>分表支持：</b>
 *
 * <ul>
 *   <li>写入时根据操作时间自动路由到对应分表
 *   <li>批量写入时按分表分组，避免跨表批量问题
 * </ul>
 *
 * <p><b>DDL 示例（MySQL）：</b>
 *
...
public class JdbcAuditStorage implements AuditWriter {
  ...
  private void saveBatchWithSharding(List<AuditLog> auditLogs) {
    Map<String, List<AuditLog>> grouped =
        auditLogs.stream().collect(Collectors.groupingBy(this::resolveTableName));
   ...
   * 从指定表中清理过期审计日志
   *
   * <p>使用 Java 计算过期时间点并传参给 SQL，避免使用 MySQL 专有的 {@code DATE_SUB(NOW(), INTERVAL ? DAY)} 函数，保证在
   * PostgreSQL/Oracle/SQL Server 等数据库上兼容。
   *
   * @param tableName 表名
   * @param retentionDays 日志保留天数
   * @return 清理的记录数
  ...
  private int cleanFromTable(String tableName, int retentionDays) {
    ...
    Timestamp expireTime =
        new Timestamp(System.currentTimeMillis() - (long) retentionDays * 24L * 60L * 60L * 1000L);
    try {
