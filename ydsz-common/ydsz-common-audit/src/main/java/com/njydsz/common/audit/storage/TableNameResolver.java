 ...
 * 审计日志分表名解析器
 * <p>
 * 根据配置的分表类型（monthly/daily/yearly）和时间动态计算目标表名。
 * 替代原有的 {@code TableShardingStrategy} 策略接口，通过配置驱动而非多态实现。
 * </p>
 *
 * <p>分表命名规则：</p>
 * <ul>
 *   <li>monthly：{@code baseTableName_yyyyMM}，如 {@code sys_audit_log_202601}</li>
 *   <li>daily：{@code baseTableName_yyyyMMdd}，如 {@code sys_audit_log_20260101}</li>
 *   <li>yearly：{@code baseTableName_yyyy}，如 {@code sys_audit_log_2026}</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class TableNameResolver {
    ...
    public Set<String> resolveInRange(LocalDateTime startTime, LocalDateTime endTime) {
        ...
        LocalDateTime current = switch (shardType) {
            ...
            case YEARLY -> startTime.withMonth(1).withDayOfYear(1).withHour(0).withMinute(0).withSecond(0);
        };
        while (!current.isAfter(endTime)) {
            tables.add(resolve(current));
            current = switch (shardType) {
