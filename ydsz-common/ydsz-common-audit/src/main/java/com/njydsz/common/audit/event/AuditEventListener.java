 ...
 * 审计事件监听器
 *
 * <p>消费业务模块通过 {@code ApplicationEventPublisher} 发布的审计事件，
 * 将 {@link OperationLogEvent} / {@link DataExportAuditEvent} 转换为
 * 统一的 {@link AuditLog} 实体并通过 {@link AuditRecorder} 异步落库。
 *
 * <p>设计要点：
 * <ul>
 *   <li>使用 {@link TransactionalEventListener}(phase = AFTER_COMMIT) 确保事件在业务事务提交后才消费，
 *       避免业务回滚后审计日志残留</li>
 *   <li>事件消费逻辑异常被隔离捕获，绝不反向影响业务主链路</li>
 *   <li>事件携带的审计字段（userId / module / action 等）优先使用，
 *       缺失字段使用 {@code null} 占位，避免 NPE</li>
 *   <li>支持 {@link Async} 异步消费（需业务主类或配置类显式启用 {@code @EnableAsync}）</li>
 * </ul>
 *
 * <h3>事件 → 审计字段映射</h3>
 * <table border="1">
...
public class AuditEventListener {
     ...
     * 消费操作日志事件（事务提交后）
     *
     * <p>将 {@link OperationLogEvent} 转换为 {@link AuditLog} 并异步落库。
     * 业务模块通过在代码中 {@code publishEvent(new OperationLogEvent(...))} 发布事件，
     * 即可将操作审计数据统一汇入 sys_audit_log 表。
     *
     * @param event 操作日志事件
     */
    ...
    public void onOperationLog(OperationLogEvent event) {
        ...
        try {
            ...
            log.debug("[AuditEvent] 操作日志事件已消费: userId={}, module={}, action={}",
                    event.getUserId(), event.getModule(), event.getAction());
        } catch (Exception e) {
    ...
    public void onDataExport(DataExportAuditEvent event) {
        ...
        try {
            ...
            auditRecorder.record(auditLog);
            log.debug("[AuditEvent] 数据导出事件已消费: userId={}, module={}, rowCount={}",
                    event.getUserId(), event.getExportModule(), event.getRowCount());
        } catch (Exception e) {
    ...
    private AuditLog convertFromOperationLog(OperationLogEvent event) {
        ...
        auditLog.setStatus(isSuccess(event.getStatus())
    ...
    private AuditLog convertFromDataExport(DataExportAuditEvent event) {
        ...
        auditLog.setContent((event.getRowCount() != null)
     ...
     *
     * <p>若 action 字符串可匹配已知枚举则返回对应编码，
     * 否则使用原字符串的 hashCode 取正数作为兜底编码。
     *
