...
> **职责边界说明**：v1.10.0 起，本模块聚焦"运行时隔离"职责。租户生命周期管理（上下线/状态转换）已剥离至独立运营域模块；字段加密能力下沉至 `common-safe`；PostgreSQL RLS DDL 生成工具迁移至 devops 脚本目录。
...
- **v1.1.0**（2026-08-14）：安全加固与架构优化。新增 SCHEMA 模式、`@TenantColumn` 启动扫描、Feign/WebFilter header 契约统一、SQL IN 表达式树防注入、Caffeine 缓存替代 ConcurrentHashMap、双路径上下文统一为 `TenantContextHolder`、`TenantIndexValidator` 启动校验。
- **v1.0.0**（2026-08-02）：初始版本。提供统一租户上下文、SQL 隔离拦截器、多级租户支持（SINGLE/MULTI/ISOLATE_DB）、全链路传播、Redis Key 隔离、per-table 列名覆盖、租户限流、Micrometer 指标、Fail-Closed 保护、健康检查。
