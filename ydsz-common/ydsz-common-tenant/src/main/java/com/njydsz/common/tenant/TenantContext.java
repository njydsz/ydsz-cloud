...
public final class TenantContext {
  ...
  private TenantContext(
    ...
    this.schema = schema;
    this.sharedTenantIds =
        sharedTenantIds != null
   ...
   * 获取字段值（多值）。
   *
   * <p>通过遍历 {@code List<?>} 并对每个元素进行 {@code instanceof String} 检查来构建 {@code List<String>}，避免使用
   * {@code @SuppressWarnings("unchecked")} 注解和未经检查的强制类型转换。非 String 元素会被跳过。
   *
   * @param claim 字段名
  ...
  public static final class Builder {
    ...
    public TenantContext build() {
      ...
      return new TenantContext(
          tenantId, fields, systemTenant, superAdmin, skipIsolation, schema, sharedTenantIds);
    }
  }
}
