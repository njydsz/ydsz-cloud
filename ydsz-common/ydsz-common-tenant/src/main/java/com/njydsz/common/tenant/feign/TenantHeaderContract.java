 ...
 *
 * <p>确保 {@code TenantContextFeignInterceptor}（写入端）与
 * {@code TenantContextWebFilter}（读取端）对同一字段使用完全一致的 header 名称
 * 和 claim 回退优先级。
 *
 * <p><b>header 名计算规则：</b>
 * <ol>
 *   <li>字段显式配置了 {@code header} → 使用显式值（如 {@code X-Company-Id}）</li>
 *   <li>否则 → {@code X-Tenant-{fieldKey}}（fieldKey = claim 名或列名）</li>
 * </ol>
 *
 * <p><b>effecitiveClaim（值来源 key）：</b>
 * <ul>
 *   <li>字段配置了 {@code claim} → 使用 claim 名（JWT 读取 + Map key）</li>
 *   <li>否则 → 使用 {@code column} 名（Map key）</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.10.0
