 ...
 * 注解扫描结果回填器。
 *
 * <p>在 {@link TenantProperties} 实例创建后自动将其与 {@link TenantColumnScanner} 的扫描结果合并：
 *
 * <ul>
 *   <li>先读取 classpath 中 {@code @TenantColumn} 注解声明的 per-table 列名映射
 *   <li>将扫描结果回填到 {@link TenantProperties#getTableColumnMapping()} Map
 *   <li>YAML 显式配置的映射优先级更高（不覆盖已有 entry）
 * </ul>
 *
 * <p>实现为 {@link BeanPostProcessor} 而非 {@code @PostConstruct}， 因为需要拿到完整装配后的 {@link
...
public class TenantPropertiesAnnotationPopulator implements BeanPostProcessor {
  ...
  public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
    if (bean instanceof TenantProperties properties) {
      ...
      scannedMappings.forEach(
          (table, column) -> {
            // YAML 显式配置优先，不覆盖
            properties.getTableColumnMapping().putIfAbsent(table.toLowerCase(), column);
          });
    }
    return bean;
  }
