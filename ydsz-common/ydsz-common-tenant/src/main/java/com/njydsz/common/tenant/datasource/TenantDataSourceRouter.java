 ...
 *
 * <p><b>路由缓存：</b>租户 ID → 数据源 Key 的映射缓存在 {@link ConcurrentHashMap} 中， 避免每次请求都遍历 {@link
 * DynamicRoutingDataSource#getDataSources()}。 缓存生命周期与 Bean 一致（应用启动时预热，运行时只读）。
 *
...
public class TenantDataSourceRouter {
  ...
  public void routeToTenantDataSource(String tenantId) {
    ...
    if (!dataSources.containsKey(datasourceKey)) {
      log.warn("租户 {} 的数据源 [{}] 不存在，使用默认数据源。已移除失效缓存。", tenantId, datasourceKey);
      // 移除失效缓存
   ...
   *
   * <ul>
   *   <li>命名约定：{@code tenant_{tenantId}}
   *   <li>配置映射：从 ydsz.tenant.datasource.mapping 读取
   *   <li>自定义：业务模块实现 DatasourceKeyResolver 接口
   * </ul>
   *
