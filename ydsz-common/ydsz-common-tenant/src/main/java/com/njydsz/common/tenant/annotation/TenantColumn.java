 ...
 * 租户列名覆盖注解。
 *
 * <p>标注在 DO 类上，指定该表使用的租户列名（覆盖全局默认 {@code tenant_id}）。 支持 per-table 自定义列名，满足不同表使用不同列名的场景。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>
...
public @interface TenantColumn {
   ...
   *
   * <p>每个元素是 JWT claim 名（如 "tenantId"、"companyId"、"deptId"）， 拦截器会从 {@code TenantContext} 中按 claim
   * 名取值。
   *
   * @return claim 名数组
   */
