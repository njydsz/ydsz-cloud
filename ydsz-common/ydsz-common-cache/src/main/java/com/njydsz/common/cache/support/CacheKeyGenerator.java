...
public final class CacheKeyGenerator {
  ...
  public static final class Builder {
     ...
     *
     * <p>多个应用共用一套 Redis 时必须设置，否则不同模块的同名 key 会互相覆盖。 前缀参与 SCAN 匹配模式的构造，因此选定后不宜随意变更， 否则历史 key
     * 将无法被批量清理。
     *
     ...
     *
     * <p>缓存值的结构发生不兼容变更时，递增命名空间（如 {@code v1} → {@code v2}） 即可让全部旧 key 自然失效，无需停机清理，是灰度发布的常用手法。
     *
     ...
     * 开启超长 key 的哈希压缩，默认关闭。
     *
     * <p>开启后，业务部分长度超过 {@link #maxLength(int)} 的 key 会被替换为 MD5 摘要的 URL-safe Base64 编码（固定 22 字符）， 以控制
     * Redis 内存占用与网络开销。
     *
     * <p><b>代价</b>：压缩<b>不可逆</b>，从 Redis 里看到的 key 无法还原为原始业务 key， 排查问题时需要自行复算摘要；且 MD5 存在理论碰撞可能，
     ...
     *
     * <p>仅在 {@link #hashLongKeys(boolean)} 为 {@code true} 时生效， 且只统计业务 key 本身的长度，<b>不含</b>前缀与命名空间。
     ...
     *
     * <p>沿用 Redis 社区约定的冒号分层习惯可让 RedisInsight 等工具 自动把 key 渲染成树形结构；改成其他字符会丧失这一便利。
     *
     ...
     *
     * <p>未显式设置的前缀、命名空间与分隔符分别回退为空串与默认的 {@code ":"}； 此处不校验参数组合合法性，入参 null 契约在 {@link
     * #generate(Object)} 调用时才生效。
     *
