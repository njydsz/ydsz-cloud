 ...
 * Redis Key 过期事件监听注解
 *
 * <p>标注在 Spring Bean 的方法上，当匹配的 Redis Key 过期时自动回调。 底层基于 Redis Keyspace Notifications（{@code
 * __keyevent@*__:expired}）， 需要 Redis 服务端开启 notify-keyspace-events 配置（建议 {@code Ex}）。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
 ...
 *
 * <ul>
 *   <li>方法签名必须为 {@code void method(String expiredKey)} 或 {@code void method(RedisKeyExpirationEvent
 *       event)}
 *   <li>Redis 服务端需配置 {@code notify-keyspace-events Ex}
 *   <li>过期事件不保证实时性，Redis 采用惰性删除 + 定期删除策略
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public @interface RedisKeyExpireListener {
   ...
   *
   * <ul>
   *   <li>{@code user:*} - 匹配所有以 "user:" 开头的 key
   *   <li>{@code order:*:lock} - 匹配中间任意段的 key
   *   <li>{@code *} - 匹配所有 key（慎用，性能开销大）
   * </ul>
   *
   * @return key 匹配模式
   */
