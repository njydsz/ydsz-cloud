 ...
 *   <li>PING 延迟测量</li>
 *   <li>Redis 版本信息（server section）</li>
 *   <li>内存使用情况（memory section）</li>
 *   <li>连接数、Key 数量等元信息（clients + stats section）</li>
 * </ul>
 *
 * <p><b>性能优化：</b>
 * <ul>
 *   <li>INFO 调用限制为指定 section（server/memory/clients/stats），避免全量 INFO 返回过大内容</li>
 *   <li>健康结果缓存 30 秒，避免高频监控端点反复触发 INFO 查询</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class RedisHealthIndicator implements HealthIndicator {
     ...
     * <p>执行步骤：
     * <ol>
     *   <li>检查缓存：若 30 秒内有有效缓存则直接返回缓存结果</li>
     *   <li>从连接工厂获取连接并发送 PING 命令</li>
     *   <li>解析 PONG 响应与延迟</li>
     *   <li>收集指定 section 的服务端元信息（限制 INFO 范围，避免全量返回）</li>
    ...
    public Health health() {
        ...
        try {
            ...
            try {
                ...
                // 缓存结果（下次在 TTL 内可直接复用）
                cachedHealth.set(new CachedHealth(health,
                        System.currentTimeMillis() + INFO_CACHE_TTL_MS));
                return health;
            } finally {
                connection.close();
            }
    ...
    private Health refreshPingOnly(Health cached) {
        try {
            ...
            try {
                ...
                cached.getDetails().forEach((key, value) -> {
     ...
     *
     * <p>仅查询 server、memory、clients、stats 四个关键 section，
     * 避免无参数 INFO 命令返回全部监控数据（生产环境可能非常大）。</p>
     *
    ...
    private void fetchLimitedSections(RedisConnection connection, Health.Builder builder) {
        try {
            ...
            if (memoryInfo != null) {
                builder.withDetail("used_memory_human", memoryInfo.getProperty("used_memory_human", "unknown"));
                builder.withDetail("max_memory_policy", memoryInfo.getProperty("maxmemory_policy", "unknown"));
            ...
            if (clientsInfo != null) {
                builder.withDetail("connected_clients",
                        clientsInfo.getProperty("connected_clients", "unknown"));
            }

            // stats section: 全局统计
            Properties statsInfo = connection.serverCommands().info("stats");
            if (statsInfo != null) {
                builder.withDetail("total_commands_processed",
                        statsInfo.getProperty("total_commands_processed", "unknown"));
                builder.withDetail("keyspace_hits",
                        statsInfo.getProperty("keyspace_hits", "unknown"));
                builder.withDetail("keyspace_misses",
                        statsInfo.getProperty("keyspace_misses", "unknown"));
            }
