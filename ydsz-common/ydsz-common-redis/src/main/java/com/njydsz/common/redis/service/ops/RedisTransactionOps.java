 ...
 *
 * <ul>
 *   <li>事务执行（executeInTransaction）
 * </ul>
 *
 * <p>使用 RedisTemplate 的 {@link SessionCallback} 实现事务， 所有在回调中的操作将作为一个原子事务执行。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
...
public class RedisTransactionOps {
   ...
   */
  public RedisTransactionOps(
      RedisTemplate<String, Object> redisTemplate, RedisMetricsCollector metricsCollector) {
   ...
   * 在 Redis 事务中执行操作
   *
   * <p>使用 MULTI/EXEC 包裹回调中的所有操作，保证原子性。 回调中通过传入的 {@link RedisTemplate} 执行的所有命令将在一个事务中执行。
   *
   * <p><b>注意：</b>事务中的命令不会立即执行，而是在 EXEC 时批量执行， 因此事务中无法获取中间结果（如 GET 的返回值）。
   *
  ...
  public <T> List<Object> executeInTransaction(
    ...
    if (metricsCollector != null) {
      return metricsCollector.recordOperation(
          "transaction_exec", () -> doExecuteInTransaction(callback));
    }
    return doExecuteInTransaction(callback);
  }

  private <T> List<Object> doExecuteInTransaction(
      Function<RedisTemplate<String, Object>, T> callback) {
    try {
   ...
   *
   * <p>使用 MULTI/EXEC 包裹回调中的所有操作，保证原子性。 适用于不需要处理事务返回结果的场景。
   *
   * @param callback 事务回调函数，参数为 RedisTemplate 实例
