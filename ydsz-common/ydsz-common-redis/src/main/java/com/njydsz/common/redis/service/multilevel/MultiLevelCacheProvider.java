 ...
 * <ul>
 *   <li><b>L1</b>：Caffeine 本地缓存，命中率极高、零网络开销，适合热点数据</li>
 *   <li><b>L2</b>：Redis 远程缓存，跨实例共享，适合全局数据</li>
 * </ul>
 *
 * <p><b>读取流程：</b> L1 → L2 → Supplier（回源）
 * <ul>
 *   <li>L1 命中：直接返回（微秒级）</li>
 *   <li>L1 未命中、L2 命中：回填 L1 后返回（毫秒级）</li>
 *   <li>L1+L2 均未命中：调用 supplier 获取数据，写入 L2 和 L1 后返回</li>
 * </ul>
 *
 ...
 * <p><b>注意事项：</b>
 * <ul>
 *   <li>L1 TTL 应显著小于 L2 TTL，保证数据新鲜度</li>
 *   <li>本组件仅在 Caffeine 位于 classpath 时自动装配</li>
 *   <li>不适用于写多读少或数据实时性要求极高的场景</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.4.0
...
public class MultiLevelCacheProvider implements CacheProvider {
     ...
     */
    public MultiLevelCacheProvider(RedisStringOps redisStringOps,
