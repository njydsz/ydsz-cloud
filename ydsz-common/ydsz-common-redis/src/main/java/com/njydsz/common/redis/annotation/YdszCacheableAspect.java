 ...
 *
 * <ul>
 *   <li>缓存穿透防护（空值缓存）—— {@code preventPenetration}
...
public class YdszCacheableAspect {
   ...
   *
   * <ol>
   *   <li>解析 SpEL Key 后检查缓存命中
   *   <li>缓存未命中：执行方法体，结果写入缓存（含 TTL 抖动防雪崩）
   *   <li>空值缓存：方法返回 null 时写入空值标记，防止缓存穿透
   * </ol>
   *
   * @param joinPoint 切点
   ...
   * 解析 SpEL 缓存键表达式
   *
   * <p>使用 {@link SimpleEvaluationContext} 替代 {@code StandardEvaluationContext}， 禁止访问 Bean
   * 引用、类型引用和方法引用，防止 SpEL 注入攻击。 仅支持读取变量和属性访问，满足缓存键解析需求。
   *
   ...
   *
   * <p>方法执行成功后将返回值写入 Redis 缓存。与 {@link #around} 不同， 此方法不查询缓存，始终执行方法体。
   *
   * @param joinPoint 切点
   * @return 方法返回值
   * @throws Throwable 原方法抛出的异常
