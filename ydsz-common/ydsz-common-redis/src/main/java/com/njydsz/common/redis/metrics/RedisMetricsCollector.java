 ...
 * Redis 操作指标收集器
 *
 * <p>为 Redis 操作提供可选的 Micrometer 指标采集，仅当 Micrometer 在 classpath 中时启用。 使用 {@link Timer.Sample}
 * 模式进行低开销测量。
 *
 * <p><b>Meter 缓存优化：</b> 原实现每次调用 recordOperation 都执行 {@code Timer.builder().register()}， 导致重复注册
 * Meter 和 ConcurrentHashMap 竞争。现改为缓存已注册的 Meter， 运行时仅调用 {@code sample.stop(cachedTimer)}，符合
 * Micrometer 最佳实践。
 *
 * <p>注册的指标：
 *
 * <ul>
 *   <li>{@code redis.operation.latency} - Redis 操作延迟 Timer
 *   <li>{@code redis.operation.errors} - Redis 操作错误 Counter
 *   <li>{@code redis.operation.slow} - Redis 慢操作 Counter（P2 可观测性增强）
 * </ul>
 *
 * <p>指标标签：
 *
 * <ul>
 *   <li>{@code operation_type} - 操作类型（如 get, set, del 等）
 *   <li>{@code error_type} - 错误类型（仅 errors 指标，如 ConnectionException, TimeoutException 等）
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
...
public class RedisMetricsCollector {
   ...
   * 记录 Redis 操作的延迟
   *
   * <p>使用 Timer.Sample 模式，开销极低。 当配置了慢操作阈值时，超阈值的操作会递增 {@code redis.operation.slow} Counter。
   *
   ...
   * 记录 Redis 操作错误
   *
   * <p><b>性能优化：</b>使用缓存的 Counter 实例，避免每次调用都执行 {@code register()}。 缓存 key 为 {@code
   * operationType:errorType} 组合，确保不同错误类型有独立的 Counter。
   *
   * @param operationType 操作类型
  ...
  public void recordError(String operationType, String errorType) {
    ...
    Counter counter =
        errorCounterCache.computeIfAbsent(
            cacheKey,
            key ->
   ...
   *
   * <p>当 Redis 操作耗时超过配置阈值时递增此计数器， 标签包含操作类型，便于按操作维度告警。
   *
  ...
  public void recordSlowOperation(String operationType, long duration) {
    Counter counter =
        slowCounterCache.computeIfAbsent(
            operationType,
            op ->
