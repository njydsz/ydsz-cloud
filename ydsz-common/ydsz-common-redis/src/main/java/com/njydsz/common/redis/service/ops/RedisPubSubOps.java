...
public class RedisPubSubOps {
  ...
  public String subscribe(String channel, Consumer<PubSubMessage> listener) {
    ...
    try {
      ...
      MessageListener springListener =
          (message, pattern) -> listener.accept(toPubSubMessage(message, pattern));
  ...
  public static class PubSubMessage {
     ...
     * 按指定类型获取消息体（安全转换）。
     *
     * <p>仅当消息体是目标类型的实例时才返回转换结果，否则返回 {@code null}， 避免 ClassCastException 向上传播。
     *
