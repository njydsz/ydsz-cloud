 ...
 *
 * <p>按数据类型拆分而来的细粒度操作组件，职责单一，便于维护与测试。 集成 {@link RedisMetricsCollector} 进行操作指标采集，与 {@link
 * RedisStringOps} 保持一致。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class RedisCollectionOps {
  ...
  public <T> T sRandomMember(String key, Class<T> clazz) {
    ...
    try {
      ...
      return metricsCollector != null
          ? metricsCollector.recordOperation("sRandomMember", op)
          : op.get();
    } catch (Exception e) {
  ...
  public <T> List<T> sRandomMembers(String key, long count, Class<T> clazz) {
    ...
    try {
      ...
      return metricsCollector != null
          ? metricsCollector.recordOperation("sRandomMembers", op)
          : op.get();
    } catch (Exception e) {
          ...
          () -> {
            Set<Object> result =
                redisTemplate
                    ...
                    .intersect(
                        formattedKeys[0],
                        Arrays.asList(formattedKeys).subList(1, formattedKeys.length));
            if (result == null) {
              return Collections.emptySet();
            }
  ...
  public boolean lSet(String key, long index, Object value) {
    ...
    try {
      if (metricsCollector != null) {
        metricsCollector.recordOperation(
            "lSet", () -> redisTemplate.opsForList().set(formattedKey, index, value));
      } else {
  ...
  public boolean lTrim(String key, long start, long end) {
    ...
    try {
      if (metricsCollector != null) {
        metricsCollector.recordOperation(
            "lTrim", () -> redisTemplate.opsForList().trim(formattedKey, start, end));
      } else {
  ...
  public <T> Set<T> zReverseRange(String key, long start, long end, Class<T> clazz) {
    ...
    try {
      ...
      return metricsCollector != null
          ? metricsCollector.recordOperation("zReverseRange", op)
          : op.get();
    } catch (Exception e) {
      recordError("zReverseRange", e);
      log.error(
          "【Redis】ZREVRANGE 操作失败 | key={} | start={} | end={} | error={}", key, start, end, e);
      return Collections.emptySet();
    }
  }
  ...
  public <T> Set<T> zRangeByScore(String key, double min, double max, Class<T> clazz) {
    ...
    try {
      ...
      return metricsCollector != null
          ? metricsCollector.recordOperation("zRangeByScore", op)
          : op.get();
    } catch (Exception e) {
      recordError("zRangeByScore", e);
      log.error(
          "【Redis】ZRANGEBYSCORE 操作失败 | key={} | min={} | max={} | error={}", key, min, max, e);
      return Collections.emptySet();
    }
  }
  ...
  public Set<ZSetOperations.TypedTuple<Object>> zRangeByScoreWithScores(
    ...
    try {
      ...
      return metricsCollector != null
          ? metricsCollector.recordOperation("zRangeByScoreWithScores", op)
          : op.get();
    } catch (Exception e) {
      recordError("zRangeByScoreWithScores", e);
      log.error(
          "【Redis】ZRANGEBYSCORE WITHSCORES 操作失败 | key={} | min={} | max={} | error={}",
          key,
          min,
          max,
          e);
  ...
  public Long zRank(String key, Object value) {
    ...
    try {
      return metricsCollector != null
          ? metricsCollector.recordOperation(
              "zRank", () -> redisTemplate.opsForZSet().rank(formattedKey, value))
  ...
  public Long zReverseRank(String key, Object value) {
    ...
    try {
      return metricsCollector != null
          ? metricsCollector.recordOperation(
              "zReverseRank", () -> redisTemplate.opsForZSet().reverseRank(formattedKey, value))
  ...
  public long zRemoveRange(String key, long start, long end) {
    ...
    try {
      ...
      return metricsCollector != null
          ? metricsCollector.recordOperation("zRemoveRange", op)
          : op.get();
    } catch (Exception e) {
      recordError("zRemoveRange", e);
      log.error(
          "【Redis】ZREMRANGEBYRANK 操作失败 | key={} | start={} | end={} | error={}",
          key,
          start,
          end,
          e);
  ...
  public long zRemoveRangeByScore(String key, double min, double max) {
    ...
    try {
      ...
      return metricsCollector != null
          ? metricsCollector.recordOperation("zRemoveRangeByScore", op)
          : op.get();
    } catch (Exception e) {
      recordError("zRemoveRangeByScore", e);
      log.error(
          "【Redis】ZREMRANGEBYSCORE 操作失败 | key={} | min={} | max={} | error={}", key, min, max, e);
      return 0;
    }
  }
