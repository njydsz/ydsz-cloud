 ...
 * Redis Key 枚举类
 *
 * <p>定义系统中使用的所有 Redis Key 模板，提供统一的 Key 管理。 支持 Key 前缀、过期时间和批量 Key 生成。
 *
 * <p><b>扩展说明：</b>当前 Key 定义在枚举中不可扩展，业务模块如需自定义 Key 前缀和过期时间， 可通过 RedisStringOps 直接操作或自定义 Key 常量类。
 *
 * <p><b>设计原则：</b>
 *
 * <ul>
 *   <li>统一前缀：所有 Key 使用 "ydsz:" 作为基础前缀
 *   <li>模板化：使用占位符 {} 定义 Key 结构，便于维护
 *   <li>分组管理：按业务模块分组，同一模块的 Key 集中定义
 *   <li>过期时间：每个 Key 都有默认过期时间，便于内存管理
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
...
public enum RedisKeysEnum {
  ...
  public List<String> batchJoin(Collection<?> argsCollection) {
    ...
    return argsCollection.stream().map(arg -> join(arg)).collect(Collectors.toList());
  }
