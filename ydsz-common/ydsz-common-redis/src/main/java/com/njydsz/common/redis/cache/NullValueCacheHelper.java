 ...
 * 缓存空值保护工具类
 *
 * <p>防止缓存穿透：当数据库查询返回 null 时，缓存一个空占位符， 避免后续请求直接打到数据库。
 *
 * <p>用法：
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>与 {@link com.njydsz.common.redis.config.RedisProperties#getKeyPrefix()} 配合使用： 如果配置了
...
public class NullValueCacheHelper {
   ...
   *
   * <p><b>并发保护：</b>使用 {@code setIfAbsent} 原子写入空值占位符， 避免多线程同时回源导致并发穿透。
   *
   * <p><b>类型安全：</b>使用传入的 {@code clazz} 进行反序列化， 避免发生 {@link ClassCastException}。
   *
  ...
  public static <T> T getOrLoad(
      StringRedisTemplate cache, String key, Supplier<T> loader, Duration ttl, Class<T> clazz) {
    ...
    if (cached != null) {
      ...
      try {
        ...
        log.warn(
            "【NullValueCacheHelper】反序列化失败，将回源加载 | key={} | targetClass={}",
            key,
            clazz.getName(),
            e);
      }
   ...
   * 删除空值占位符缓存
   *
   * <p>当数据库中新创建了数据后，可以调用此方法清除空值缓存， 确保下次读取能获取到最新数据。
   *
   * @param cache StringRedisTemplate 实例
   * @param key 缓存 Key
   */
