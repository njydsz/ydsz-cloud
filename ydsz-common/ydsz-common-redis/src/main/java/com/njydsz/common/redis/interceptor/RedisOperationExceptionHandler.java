 ...
 *
 * <ol>
 *   <li>将 Spring Data Redis 常见异常统一转换为内部异常体系
 *   <li>区分可恢复异常（连接失败/超时）与不可恢复异常（序列化/参数错误）
 *   <li>结构化日志记录，包含操作类型、方法名、key 信息
 * </ol>
 *
 * <p><b>异常转换规则：</b>
 *
 * <ul>
 *   <li>RedisConnectionFailureException → RedisConnectionException（可重试）
 *   <li>QueryTimeoutException → RedisConnectionException（可重试）
 *   <li>SerializationException → RedisBusinessException（不可重试）
 *   <li>IllegalArgumentException → RedisBusinessException（不可重试）
 * </ul>
 *
 * <p><b>注意：</b>此拦截器仅作为全局兜底，不替代各组件内部的精细化异常处理。 对于已有 FailOpenPolicy 策略的组件（如
