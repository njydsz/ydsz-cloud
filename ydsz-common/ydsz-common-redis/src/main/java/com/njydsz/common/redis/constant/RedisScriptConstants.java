 ...
 * <p><b>脚本变体说明：</b>
 * <ul>
 *   <li>{@link #TOKEN_BUCKET_LUA_MS} — 毫秒精度版，适用于同步 {@link org.springframework.data.redis.core.RedisTemplate}，
 *       使用 {@code PEXPIRE}（毫秒 TTL）和毫秒时间戳</li>
 *   <li>响应式栈版本（基于秒精度）由 {@code ydsz-gateway} 的 {@code RateLimitFilter} 持有，
 *       差异源于 {@link org.springframework.data.redis.core.ReactiveStringRedisTemplate} 的调用模型；
 *       两者算法同源，修改时请保持语义一致</li>
 * </ul>
 *
 * <p><b>使用约束：</b>
 * <ul>
 *   <li>脚本文本仅应在此处修改 — 业务模块通过 {@code RedisScript.of(...)} 引用，禁止内联复制</li>
 *   <li>修改脚本前必须运行 {@code RedisRateLimiter} 单测验证三类算法行为不退化</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public final class RedisScriptConstants {
     ...
     * <p>逻辑：
     * <ol>
     *   <li>读取桶中当前令牌数与上次刷新时间（毫秒）</li>
     *   <li>计算自上次刷新以来应补充的令牌数（floor(elapsed * rate / periodMs)）</li>
     *   <li>更新令牌数（不超过 capacity），可选仅在有新增令牌时推进 lastRefill</li>
     *   <li>若令牌数 >= requested，扣减并返回 {1, tokens}；否则返回 {0, tokens}</li>
     * </ol>
     *
     * <p>参数：KEYS[1]=key, ARGV[1]=capacity, ARGV[2]=rate, ARGV[3]=periodMs, ARGV[4]=now_ms, ARGV[5]=requested
     ...
     *   <li>删除超出窗口的旧桶</li>
     *   <li>统计所有存活桶的总计数</li>
     *   <li>若总计数 ≥ limit，拒绝并返回 {0, totalCount}</li>
