 ...
 * 线程池编程式工厂工具类。
 *
 * <p>提供全面的线程池创建和管理方法，支持常规线程池、VirtualThread、CPU 密集型有界队列、 自定义命名、守护线程、拒绝策略、TTL 上下文透传等能力。
 *
 * <p><b>模块归属说明：</b>该类位于 {@code ydsz-common-thread} 模块，为配置驱动 （{@code
 * ydsz.thread.pools.<name>}）之外的编程式补充。Spring 环境优先使用配置驱动方式， 本工具类适用于非 Spring
 * 工具方法、短生命周期临时线程池或需要按需创建的场景。
 *
 * <h2>快速选择指南</h2>
 *
 * <table border="1">
 ...
 */
@Slf4j
// CHECKSTYLE.OFF: RegexpSinglelineJava — ExecutorUtils 为线程池编程式工厂（云顶规范 15.4 授权实现层），统一封装线程池创建
public final class ExecutorUtils {
   ...
   *
   * <p>适用于大量 IO 操作场景，如 HTTP 请求、数据库查询等。 VirtualThread 由 JVM 调度，可创建数百万个而不会耗尽系统线程资源。
   *
   * <p><b>与 {@link #newPlatformThreadExecutor()} 的区别：</b> VirtualThread 不支持线程局部变量池化（{@code
   ...
   *
   * <p>明确表达"使用 OS 线程"的意图，与 {@link #newVirtualThreadExecutor()} 互补。 底层实现为缓存线程池（核心 0、最大 {@link
   * #DEFAULT_MAX_POOL_SIZE}、60s 空闲回收）， 适合 CPU 密集型或混合负载场景。
   *
   * @return 平台线程执行器
   */
   ...
   *
   * <p>适用于使用 {@link com.alibaba.ttl.TransmittableThreadLocal} 的场景（如 RequestContext、 链路 TraceId、租户
   * ID 等），确保父线程上下文正确传播到子线程。
   *
   * <p>典型用途：
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>依赖条件：classpath 上需存在 {@code transmittable-thread-local} 库（ydsz-common-core 已声明为 optional 依赖）。
   *
  ...
  public static ThreadPoolExecutor newCustomThreadPool(
    ...
    return newCustomThreadPool(
        corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue, null, null);
  }
   ...
   * @return 线程池实例
   */
  public static ExecutorService newPriorityThreadPool(
      int nThreads, Comparator<Runnable> comparator) {
    BlockingQueue<Runnable> queue = new PriorityBlockingQueue<>(DEFAULT_QUEUE_CAPACITY, comparator);
    return new ThreadPoolExecutor(
        nThreads,
        nThreads,
        0L,
   ...
   * 线程未捕获异常处理器。
   *
   * <p>记录线程名和异常堆栈，避免 {@code execute()} 提交的任务异常被静默吞没。 {@code submit()} 提交的任务异常会被封装到 Future
   * 中，不会触发此处理器。
   *
   * @param t 抛出未捕获异常的线程
   * @param e 异常
  ...
  public static <T> T submitWithTimeout(
      ExecutorService executor, Callable<T> task, long timeout, TimeUnit unit) {
