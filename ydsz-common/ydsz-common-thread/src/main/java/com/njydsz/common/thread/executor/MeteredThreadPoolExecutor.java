 ...
 *
 * <ul>
 *   <li>Micrometer 指标自动注册（active.threads、pool.size、queue.size、completed.tasks、rejected.count）
 *   <li>任务执行耗时统计（P50/P95/P99 percentile）
 *   <li>慢任务检测（超过阈值时输出 warn 日志，预留 Hook 可接入告警系统）
 *   <li>任务失败计数 + 异常溯源
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>{@link #onTaskFailed(Runnable, Throwable)} — 接入告警系统
 *   <li>{@link #onSlowTask(String, long)} — 慢任务告警回调
 *   <li>队列积压告警（队列深度 &gt; highWaterMark 时触发回调）
 * </ul>
 *
 * <p>使用示例：
 *
 ...
 *
 * <ul>
 *   <li>{@code executor_active_threads{pool_name="order-process"}}
 *   <li>{@code executor_queue_size{pool_name="order-process"}}
 *   <li>{@code executor_rejected_count_total{pool_name="order-process"}}
 *   <li>{@code executor_task_duration_seconds{pool_name="order-process", quantile="0.99"}}
 * </ul>
 *
 * @author ydsz-team
 * @since 3.0.0
...
public class MeteredThreadPoolExecutor extends ThreadPoolExecutor {
  ...
  private void registerMicrometerMetrics() {
    ...
    rejectedCounter =
        Counter.builder("executor.rejected.count")
            .tag("pool.name", poolName)
            .description("累计拒绝任务数")
            .register(meterRegistry);

    failedTaskCounter =
        Counter.builder("executor.failed.tasks")
            .tag("pool.name", poolName)
            .description("累计执行失败的任务数")
            .register(meterRegistry);

    slowTaskCounter =
        Counter.builder("executor.slow.tasks")
            .tag("pool.name", poolName)
            .description("累计慢任务数（耗时超过阈值）")
            .register(meterRegistry);

    taskDurationTimer =
        Timer.builder("executor.task.duration")
            .tag("pool.name", poolName)
  ...
  protected void onSlowTask(String taskName, long elapsedMs) {
    LOG.warn(
        "Slow task detected in pool [{}]: {} took {}ms (threshold: {}ms)",
        poolName,
        taskName,
        elapsedMs,
        slowTaskThresholdMs);
   ...
   *
   * <p>失败统计放在包装器内部而非 afterExecute：submit(Runnable) 路径的异常 会被 Future 吞掉（afterExecute
   * 收不到），只有包装器自身能可靠捕获。
   */
