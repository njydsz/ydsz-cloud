...
1. **注解启用**：26.09.01 起新增 `@EnableYdszFeign` 注解，显式开启所有 Feign 增强能力，推荐替代手动配置 `FeignConfiguration` 导入方式。
