...
public class CircuitBreakerStatePersistence {
     ...
     */
    public CircuitBreakerStatePersistence(ObjectProvider<RedisStringOps> redisStringOpsProvider) {
        this(redisStringOpsProvider, Duration.ofHours(1));
    }
     ...
     * @param ttl                    状态持久化 TTL
     */
    public CircuitBreakerStatePersistence(ObjectProvider<RedisStringOps> redisStringOpsProvider, Duration ttl) {
    ...
    public void persistState(String serviceName, FeignCircuitBreakerStrategy.CircuitBreakerState state) {
        if (redisStringOps == null) {
            log.debug("RedisStringOps 未提供，跳过熔断器状态持久化");
            return;
        }
    ...
    public FeignCircuitBreakerStrategy.CircuitBreakerState restoreState(String serviceName) {
        ...
        try {
            String key = buildKey(serviceName);
            Object value = redisStringOps.get(key);
            if (value == null) {
                return null;
            }
    ...
    public void clearState(String serviceName) {
        ...
        try {
            redisStringOps.del(buildKey(serviceName));
        } catch (Exception e) {
