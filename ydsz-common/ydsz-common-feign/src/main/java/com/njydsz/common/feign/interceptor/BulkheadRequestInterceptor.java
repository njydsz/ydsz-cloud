 ...
 *
 * <p>使用信号量隔离模式，按服务维度限制最大并发请求数。 当并发请求超过限制时，快速失败而非排队等待，防止某个下游服务变慢 耗尽连接池资源影响其他服务。
 *
 * <p><b>许可释放机制：</b>
 *
 * <ul>
 *   <li>{@link #apply(RequestTemplate)} 在请求发起前获取信号量许可，并通过 {@link #currentServiceName} ThreadLocal
 *       记录当前线程持有的服务名；
 *   <li>{@link #releaseCurrentPermit()} 在 Feign 调用完成（无论成功或失败）后释放许可， 通常由 {@link
 *       FeignResponseInterceptor} 在 finally 块中调用；
 *   <li>若 {@link #apply} 因信号量耗尽或线程中断抛出异常，<b>不会</b>写入 ThreadLocal， 因此 {@link
 *       #releaseCurrentPermit()} 不会误释放许可。
 * </ul>
 *
 * <p><b>配置示例（YAML）：</b>
 *
...
public class BulkheadRequestInterceptor implements RequestInterceptor {
   ...
   *
   * <p>仅在 {@link #apply} 成功获取许可后写入，调用 {@link #releaseCurrentPermit} 后清除。 使用 {@link
  ...
  public BulkheadRequestInterceptor(
    ...
    this.defaultMaxConcurrent =
        defaultMaxConcurrent > 0 ? defaultMaxConcurrent : DEFAULT_MAX_CONCURRENT;
    this.acquireTimeoutMillis =
        acquireTimeoutMillis > 0 ? acquireTimeoutMillis : DEFAULT_ACQUIRE_TIMEOUT_MS;
    this.perClientMaxConcurrent =
        perClientMaxConcurrent != null
   ...
   *
   * <p>按服务维度获取信号量，超过最大并发数时快速失败。 成功获取许可后，将服务名写入 ThreadLocal 供后续释放。
   *
   ...
   *
   * <p>热更新机制：比较当前配置值与创建信号量时的值，若不一致则创建新信号量替换旧值。 注意：重建信号量时旧信号量中已持有的许可不会被迁移，需等待自然释放后新请求才能获取新配额。
   *
   ...
   *
   * <p>必须在 Feign 调用完成（无论成功或失败）后在 finally 块中调用， 否则会导致信号量永久占用，最终所有请求都被拒绝。
   *
   * <p>若当前线程未持有许可（例如 {@link #apply} 抛出异常未获取许可）， 此方法为空操作，不会误释放。
   */
   ...
   *
   * <p>推荐使用 {@link #releaseCurrentPermit()} 自动管理 ThreadLocal。 此方法仅在调用方明确知道服务名时使用。
   *
   * @param serviceName 服务名称
   */
  ...
  private String extractServiceName(RequestTemplate requestTemplate) {
    try {
      ...
      return requestTemplate.feignTarget() != null
