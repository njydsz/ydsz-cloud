 ...
 * SkyWalking 链路追踪处理器。
 *
 * <p>当 SkyWalking agent 在 classpath 中时，通过反射调用 SkyWalking API 获取当前上下文的 traceId 和 spanId，实现与
 * SkyWalking 的无缝集成。
 *
...
public class SkyWalkingTraceHandler implements FeignTraceHandler {
  ...
  public SkyWalkingTraceHandler() {
    ...
    try {
      ...
      traceIdMH =
          lookup.findStatic(
              contextManagerClass, "getGlobalTraceId", MethodType.methodType(String.class));
      globalTraceIdMH =
          lookup.findStatic(
