 ...
 *
 * <p>在 {@link Retryer.Default} 的基础上增加 HTTP 方法白名单过滤， 仅对配置的方法（如 GET）进行重试，避免对非幂等操作（如 POST/PUT/DELETE）
 * 产生重复提交副作用。
 *
 * <p><b>重试逻辑：</b>
 *
 * <ol>
 *   <li>从 {@link RetryableException} 中提取 HTTP 请求方法
 *   <li>若方法不在白名单中，直接抛出异常（不重试）
 *   <li>若方法在白名单中，委托给 {@link Retryer.Default} 执行指数退避重试
 * </ol>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class MethodAwareRetryer implements Retryer {
  ...
  public MethodAwareRetryer(
      long period, long maxPeriod, int maxAttempts, Set<String> retryableMethods) {
    this.period = period;
