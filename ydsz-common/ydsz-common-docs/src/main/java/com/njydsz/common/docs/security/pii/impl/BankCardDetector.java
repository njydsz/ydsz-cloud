 ...
 *
 * <p>脱敏策略为"前 4 后 4"（保留发卡行标识与末 4 位），
 * 与 {@link SensitiveUtil#bankCard} 的"后 4 位保留，其余脱敏"策略不同，
...
public class BankCardDetector implements PiiDetector {
     ...
     * 过滤出 {@link SensitiveType#BANK_CARD} 类型的匹配。
     *
     ...
     *
     * <p>与 {@link SensitiveUtil#bankCard} 的区别：SensitiveUtil 仅保留后 4 位，
     * 此处额外保留前 4 位（发卡行标识），供文档泄露分析时识别涉及的发卡机构。
     * 这是文档场景特有的脱敏策略。
     *
