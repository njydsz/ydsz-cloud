...
import com.njydsz.common.docs.domain.DocumentImage;
 ...
 * HTML 文档解析器
 *
 * <p>解析 HTML 文件，提取标题、段落、表格、图片等结构化内容。
 *
 * @author ydsz-team
 * @since 26.09.01
 */
...
public class HtmlDocumentParser implements DocumentParser {
   ...
   *
   * <p><b>安全前置：</b>先用 {@link Safelist#relaxed()} 过一遍 {@link Jsoup#clean}， 剥离 {@code
   * <script>}、{@code on*} 事件属性等可执行内容后再重新解析。 这是防御 XSS 的关键一步——解析产物可能被回显到前端， 不清洗等于把攻击载荷透传到浏览器。副作用是被
   * Safelist 拒绝的标签会连同其文本一起消失。
   *
   * <p><b>输出顺序不等于文档顺序：</b>为实现简单，抽取按"全部 h1~h6 → 全部 p → 全部 li"的批次进行，所以 {@code sections} 是按元素类型聚簇排列的，
   * 标题与其所辖正文并不相邻。依赖阅读顺序的下游（如按标题切分章节）不能直接使用该列表。
   *
   * <p>HTML 无分页概念，所有内容的 {@code pageNumber} 统一记为 1。 图片只登记 {@code src} 与 {@code alt}
   * 元信息，<b>不下载二进制</b>， 因此相对路径的 {@code src} 在脱离原站点后不可解析。
   *
   * @param inputStream HTML 字节流，由调用方负责关闭；固定按 UTF-8 解码， 不识别 {@code <meta charset>} 声明，非 UTF-8 页面会乱码
   * @param fileName 原始文件名，仅用于失败日志定位；元数据标题取自 {@code <title>} 而非此值
   * @param options 解析选项，读取 {@code extractTables} 与 {@code extractImages} 两个开关； 传 {@code null}
   *     时两者均视为开启
   * @return 文档内容，含分节、表格、图片元信息；页数恒为 1
   * @throws DocumentException 入参流为 {@code null} 时错误码 {@code DOCUMENT_EMPTY}； 流读取失败时错误码 {@code
   *     PARSE_FAILED}。 HTML 语法错误不会抛异常，Jsoup 会容错解析
   */
  @Override
  public DocumentContent parse(InputStream inputStream, String fileName, ParseOptions options) {
    ...
    for (Element p : paragraphs) {
      ...
      if (!text.isEmpty()) {
        sections.add(
            DocumentSection.builder().type("paragraph").content(text).pageNumber(1).build());
        fullText.append(text).append('\n');
      }
    }
    ...
    if (options == null || options.isExtractImages()) {
      ...
      for (Element img : imgElements) {
        ...
        if (!src.isEmpty()) {
          images.add(DocumentImage.builder().url(src).altText(alt).build());
        }
      }
    }
    ...
    return DocumentContent.builder()
        ...
        .tables(tables)
        .images(images)
        .metadata(DocumentMetadata.builder().title(doc.title()).charCount(text.length()).build())
   ...
   * 声明本解析器在注册中心占据的格式槽位。
   *
   * <p>本类未覆写 {@code supports}，故仅精确匹配 HTML； 结构相近的 {@link DocumentFormat#XML} 需另行处理，不会路由到这里。
   *
