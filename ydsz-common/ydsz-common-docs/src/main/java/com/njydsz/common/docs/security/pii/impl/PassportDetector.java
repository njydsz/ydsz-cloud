 ...
 * 护照号码检测器（中国因私护照）
 * <p>
 * 检测中国因私护照号码（E/G 开头 + 8 位数字，共 9 位）。
 *
 * <p><b>注意：</b>仅匹配格式不做校验位验证（护照号无公开校验算法），
 * 因此置信度只给 0.7；且纯数字前导 E/G 的正则可能命中业务编码、
...
public class PassportDetector implements PiiDetector {
     ...
     * 扫描全文中的疑似中国因私护照号。
     *
     * <p>无法做真实校验位验证，仅凭首字母 + 8 位数字判定；
     * 同时要求文本中出现"护照""passport"等关键词以提升准确率
     * 为可选项（未强制），避免无上下文的纯编码误报。
     *
     * <p>返回的下标基于预处理后的文本。
     *
     ...
     * 对护照号做保留首尾脱敏。
     *
     * <p>保留首字母（E/G，区分护照类型）与末 2 位，中间全部遮蔽。
     * 如 {@code E12345678} 脱敏为 {@code E******78}。
     *
    ...
    public String mask(String matchedText) {
        ...
        return matchedText.charAt(0) + "******"
                + matchedText.substring(matchedText.length() - 2);
    }

    /**
     * 检查命中位置前后 30 字符内是否包含护照相关关键词。
     *
     * <p>用于在无校验位可用的场景下，通过上下文提升判断准确率，
     * 降低纯格式匹配造成的误报。匹配不区分大小写。
     *
     * @param text  全文本
    ...
    private boolean containsPassportKeyword(String text, int start, int end) {
        ...
        return context.contains("护照") || context.contains("passport") || context.contains("traveldocument");
    }
}
