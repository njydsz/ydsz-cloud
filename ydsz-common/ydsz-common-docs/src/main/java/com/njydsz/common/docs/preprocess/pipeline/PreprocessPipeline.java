 ...
 * 文档预处理流水线
 *
 * <p>按 {@link DocumentPreprocessor#getOrder()} 排序依次执行所有预处理器。
 *
 * <p><b>默认处理链：</b>
 *
 * <ol>
 *   <li>{@link com.njydsz.common.docs.preprocess.impl.TextNormalizer} (order=10) - 文本归一化
 *   <li>{@link com.njydsz.common.docs.preprocess.impl.TextCleaner} (order=20) - 噪声清洗
 *   <li>{@link com.njydsz.common.docs.preprocess.impl.TextChunker} (order=30) - 文本分块
 * </ol>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class PreprocessPipeline {
  ...
  public PreprocessPipeline(
      List<DocumentPreprocessor> preprocessors, ObjectProvider<DocsMetrics> metricsProvider) {
    ...
    this.preprocessors =
        preprocessors.stream()
            ...
            .toList();
    log.info(
        "[PreprocessPipeline] 预处理流水线已初始化，共 {} 个处理器: {}",
        this.preprocessors.size(),
