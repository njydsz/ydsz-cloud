 ...
 *
 * <p><b>流式解析：</b>通过 {@link #parseStreaming} 方法支持逐页回调，
 * 适用于超大 PDF 的增量处理场景（如全文索引构建），避免一次性载入全文。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class PdfDocumentParser implements DocumentParser {
     ...
     * <p><b>输出轮廓影响：</b>
     * <ul>
     *   <li>{@link ParseProfile#TEXT_ONLY} - 仅填充 {@code text}，{@code sections} 为空</li>
     *   <li>{@link ParseProfile#STRUCTURED} - 填充 {@code text}、{@code sections}、{@code metadata}</li>
     *   <li>{@link ParseProfile#FULL} - 完整输出（图片除外，尚未实现）</li>
     * </ul>
     *
     * @param inputStream PDF 字节流，由调用方负责关闭；为 {@code null} 时视为空文档
     * @param fileName    原始文件名，在 PDF 未内嵌标题时作为元数据标题的兜底值
     * @param options     解析选项，读取 {@code maxPages}、{@code mode}、{@code profile}、{@code extractMetadata}；
     ...
     *
     * <p>适用于超大 PDF 的增量处理场景（如全文索引构建、分页 PII 扫描）。
     * 回调接收的 {@link PageContent} 包含页码与文本，_consumer_ 可将内容即时推送到
     * 下游（消息队列、索引器），无需等待全文解析完成。
     *
     * <p><b>与 {@link #parse} 的差异：</b>
     * <ul>
     *   <li>不返回完整 {@link DocumentContent}，通过回调增量输出</li>
     *   <li>内存占用恒定为单页大小，与文档总页数无关</li>
     *   <li>不读取元数据（流式场景无需）</li>
     * </ul>
     *
     ...
     */
    public void parseStreaming(InputStream inputStream, String fileName, Consumer<PageContent> consumer) {
