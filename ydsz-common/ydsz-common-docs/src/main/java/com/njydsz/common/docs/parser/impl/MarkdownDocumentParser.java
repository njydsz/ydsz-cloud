...
public class MarkdownDocumentParser implements DocumentParser {
     ...
     *
     * <p>采用<b>流式逐行</b>而非构建 AST 的方式：目标只是提取纯文本与粗粒度结构以供检索，
     * 不需要还原完整语法树，逐行处理可保持内存占用与文件大小无关。
     * 代价是只识别行首语法，行内的加粗、链接、行内代码等标记会<b>原样保留</b>在文本中。
     *
     * <p><b>已知边界：</b>不做围栏代码块（{@code ```}）状态跟踪，
     * 代码块内以 {@code #} 或 {@code -} 开头的行会被误判为标题或列表；
     * 同理不支持 Setext 式下划线标题、表格语法与 YAML front matter。
     * 有序列表项去掉序号后与无序列表统一标记为 {@code list} 类型，原始序号不保留。
     *
     * @param inputStream Markdown 字节流，由调用方负责关闭；
     *                    固定按 UTF-8 解码，不受 {@code options.charset} 影响
     ...
     * 声明本解析器在注册中心占据的格式槽位。
     *
     * <p>本类未覆写 {@code supports}，仅精确匹配 MARKDOWN。
     * 由于 {@link DocumentFormat} 只为该格式登记了 {@code md} 扩展名，
