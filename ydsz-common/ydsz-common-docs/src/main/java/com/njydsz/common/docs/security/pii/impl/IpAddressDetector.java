 ...
 * IP 地址检测器（IPv4）
 *
 * <p>检测 IPv4 地址（如 {@code 192.168.1.1}），不检测 IPv6。
 *
 * <p><b>使用场景：</b>服务器日志、错误堆栈中常包含内网 IP， 外发文档时需识别并脱敏，避免泄露网络拓扑。对 RFC1918 私网段
...
public class IpAddressDetector implements PiiDetector {
   ...
   * 对 IPv4 地址做保留首尾段脱敏。
   *
   * <p>例如 {@code 192.168.1.100} 脱敏为 {@code 192.***.***.100}， 使运维仍可辨识大致网段，同时隐藏主机位。
   *
   ...
   *
   * <p>覆盖 {@code 10.0.0.0/8}、{@code 172.16.0.0/12}、{@code 192.168.0.0/16} 三个区间。 {@code 127.0.0.0/8}
   * 回环地址不视为私网（非敏感）。
   *
