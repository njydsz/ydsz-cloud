 ...
 * PII 发现结果
 * <p>
 * 表示在文档中检测到的敏感信息项，支持多种格式的定位模型。
 *
 * <p><b>定位模型：</b>
 * <ul>
 *   <li>文本定位（{@link #startIndex} / {@link #endIndex}）：适用于纯文本格式</li>
 *   <li>二进制定位（{@link #binaryLocation}）：适用于 PDF/DOCX/XLSX 等结构化格式</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class PiiFinding {
     ...
     * <p>
     * 当检测对象为二进制格式时，此字段携带页面坐标或单元格定位；
     * 纯文本场景下为 {@code null}，此时使用 {@link #startIndex}/{@link #endIndex}。
     */
     ...
     * 二进制文档中的坐标定位。
     * <p>
     * 用于 PDF 页面区域、DOCX 段落范围、Excel 单元格等场景。
     */
    @Data
    @Builder
