 ...
 * 纯文本文档解析器
 *
 * <p>解析 .txt 文件，按行切分为段落分节。
 *
 * <p><b>编码约定：</b>不做编码嗅探，字符集完全由 {@code ParseOptions.charset} 指定， 未指定时一律按 UTF-8 解码。GBK 等非 UTF-8
...
public class TxtDocumentParser implements DocumentParser {
   ...
   *
   * <p>流式逐行读取，内存占用与文件大小无关（除最终拼接的全文外）。 空白行被跳过而不生成分节，避免检索索引里出现大量无意义空条目； 但重建全文时以单个 {@code \n}
   * 连接，因此<b>原文中的空行会丢失</b>， 需要保真还原原始排版的场景不适用本解析器。
   *
   ...
   *     PARSE_FAILED}。 注意字节与所选编码不符时不会报错，而是产生替换字符（乱码）
   */
  @Override
  public DocumentContent parse(InputStream inputStream, String fileName, ParseOptions options) {
    ...
    try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream, charset))) {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        sections.add(
            DocumentSection.builder().type("paragraph").content(line).pageNumber(1).build());
   ...
   * @throws java.nio.charset.UnsupportedCharsetException 配置的编码名 JVM 不认识时抛出， 该异常不会被 {@code parse}
   *     捕获转换，会直接向上传播
   */
