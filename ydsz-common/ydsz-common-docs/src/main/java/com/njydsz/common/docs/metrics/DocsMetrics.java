 ...
 *
 * <p>采集文档解析、PII 检测、安全扫描和预处理等关键路径的指标， 通过 {@link MeterRegistry} 暴露到 Prometheus 供 Grafana 仪表盘展示。
 *
 * <h3>指标列表</h3>
 *
 * <ul>
 *   <li>{@code docs.parse.total}（Counter）：文档解析次数（按格式/状态标签）
 *   <li>{@code docs.parse.duration}（Timer）：文档解析耗时
 *   <li>{@code docs.pii.detected}（Counter）：PII 检测命中次数（按类型标签）
 *   <li>{@code docs.security.scan}（Counter）：安全扫描次数（按级别标签）
 *   <li>{@code docs.preprocess.duration}（Timer）：预处理耗时
 *   <li>{@code docs.async.queue.size}（Gauge）：异步解析队列大小
 * </ul>
 *
...
public class DocsMetrics {
   ...
   * 记录一次文档解析的次数与耗时。
   *
   * <p>次数 Counter 带 {@code format} + {@code result} 双标签，可直接算出各格式解析失败率； 耗时 Timer 只带 {@code format}
   * 标签，成功与失败的耗时合并统计。 {@link MeterRegistry} 缺失时静默返回，调用方无需判空。
   *
  ...
  public void recordParse(DocumentFormat format, boolean success, long durationMs) {
    ...
    Counter.builder("docs.parse.total")
        .tags(
            Tags.of(
                Tag.of("format", format.name()), Tag.of("result", success ? "success" : "failure")))
        .register(r)
        .increment();
   ...
   * 累加某一类 PII 的命中条数。
   *
   * <p>只上报类型与数量，不上报命中的敏感内容本身，避免指标系统二次泄露隐私数据。 {@link MeterRegistry} 缺失时静默返回。
   *
   ...
   *
   * <p>HIGH / CRITICAL 桶的突增通常意味着遭遇恶意文件投放，应据此配置告警。 {@link MeterRegistry} 缺失时静默返回。
   *
   ...
   *
   * <p>{@code processorName} 会直接作为标签值，必须使用有限枚举集（处理器类名/别名）， 切勿传入文件名等高基数字符串，否则会导致时序库指标爆炸。 {@link
   * MeterRegistry} 缺失时静默返回。
   *
   ...
   * 刷新异步解析队列积压量 Gauge。
   *
   * <p>Gauge 在构造函数中已绑定到 {@link AtomicLong} 实例，此处只更新数值， 不会重复注册 meter。基于 {@link
   * AtomicLong}，可由采集线程与业务线程并发调用。 该值持续增长说明消费能力不足，是扩容或限流的判断依据。
   *
