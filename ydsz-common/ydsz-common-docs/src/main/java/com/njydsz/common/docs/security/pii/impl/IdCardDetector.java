 ...
 *
 * <p>脱敏策略为"前 6 后 4"（保留行政区划码以供区域分析），
 * 与 {@link SensitiveUtil#idCard} 的"中间 8 位遮蔽"策略不同，
...
public class IdCardDetector implements PiiDetector {
     ...
     * 过滤出 {@link SensitiveType#ID_CARD} 类型的匹配。
     *
     ...
     *
     * <p>与 {@link SensitiveUtil#idCard} 的区别：SensitiveUtil 遮蔽中间 8 位
     * （前 3 后 5），此处保留前 6 位（行政区划码），供文档泄露分析时
     * 快速定位影响范围。这是文档场景特有的脱敏策略。
     *
