 ...
 *
 * <p>认证策略解耦：通过 {@link AuthHandler} 解析请求头中的认证信息， 业务方可注入自定义 {@link AuthHandler} Bean 覆盖默认策略。
 *
 * <p>认证埋点：通过 {@link AuthMetrics} 接口（默认由 {@code AppMetrics} 实现） 上报认证成功/失败耗时指标到 Micrometer，便于监控 App
 * 端认证质量。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class AppAuthFilter extends BaseAuthFilter {
   ...
   * 解析当前请求的认证信息
   *
   * <p>使用 {@link AuthHandler} 解析请求头中的认证信息。 认证成功/失败均通过 {@link AuthMetrics} 上报耗时指标。
   *
   * @param request 当前 HTTP 请求
   * @param response 当前 HTTP 响应
   * @return 解析得到的认证信息
  ...
  protected AuthInfo resolveAuthInfo(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if (authMetrics != null) {
        authMetrics.recordAuthFailure(
            USER_TYPE_APP, resolveFailureReason(e), System.nanoTime() - startNanos);
      }
      throw e;
    }
  }
