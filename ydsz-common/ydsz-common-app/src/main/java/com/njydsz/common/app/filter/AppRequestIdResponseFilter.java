 ...
 * App 端请求 ID 响应头过滤器
 *
 * <p>继承 {@link BaseRequestIdResponseFilter}，将当前请求的 RequestId 写入响应头 {@code
 * X-Request-Id}，便于客户端关联服务端日志。
 *
 * <p>优先复用鉴权阶段在 {@link RequestContext} 中缓存的 RequestId， 缺失时再调用 {@link
 * RequestIdGenerator#generateId()} 兜底生成。
 *
