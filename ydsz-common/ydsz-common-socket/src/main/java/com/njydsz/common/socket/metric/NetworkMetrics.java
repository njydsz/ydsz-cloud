 ...
 * 网络连接指标顶层契约（P2-9）。
 *
 * <p>定义网络连接模块（WebSocket / Netty / 长连接等）的公共指标操作规范，
 * 各实现类应确保线程安全，所有计数操作支持高并发场景。
 *
 * <p>涵盖的核心指标维度：
 * <ul>
 *   <li>连接维度：活跃连接数、累计连接数、累计断开数</li>
 *   <li>消息维度：消息接收数、消息发送数</li>
 *   <li>流量维度：累计读取字节数、累计写入字节数</li>
 * </ul>
 *
 * <p>实现类通过 Micrometer 注册到 {@code MeterRegistry}，
 * 当 {@code MeterRegistry} 不在 classpath 时降级为空操作（no-op）。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
