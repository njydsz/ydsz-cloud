 ...
 * WebSocket 集群广播自动装配。
 *
 * <p>当 classpath 存在 {@link StringRedisTemplate} 且 {@code ydsz.websocket.cluster.enabled=true}
 * 时自动生效。
 *
 * <p>自动注册：
 *
 * <ul>
 *   <li>{@link WebSocketClusterPublisher} — Redis Pub/Sub 发布者（含熔断保护）
 *   <li>{@link WebSocketClusterSubscriber} — Redis Pub/Sub 订阅者
 *   <li>{@link RedisMessageListenerContainer} — Redis 监听容器
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class WebSocketClusterAutoConfiguration {
  ...
  public WebSocketClusterPublisher webSocketClusterPublisher(
    ...
    log.info(
        "[WS-Cluster] 注册 WebSocketClusterPublisher, channel={}",
        properties.getCluster().getChannel());
  ...
  public RedisMessageListenerContainer wsClusterListenerContainer(
    ...
    container.addMessageListener(
        subscriber, new ChannelTopic(properties.getCluster().getChannel()));
