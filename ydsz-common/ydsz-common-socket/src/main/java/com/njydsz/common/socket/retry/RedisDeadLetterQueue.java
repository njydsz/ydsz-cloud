...
public class RedisDeadLetterQueue implements DeadLetterQueue {
  ...
  private static final String DEAD_LETTER_KEY = "ydsz:ws:retry:deadletter";
  ...
  public void enqueue(RetryableMessage message) {
    ...
    try {
      ...
      log.warn(
          "[WS-DeadLetter] 消息移入死信队列: messageId={}, retryCount={}",
          message.getMessageId(),
          message.getRetryCount());
    } catch (Exception e) {
      log.error(
          "[WS-DeadLetter] 死信入队失败: messageId={}, err={}", message.getMessageId(), e.getMessage());
    }
  }
  ...
  public List<RetryableMessage> list(int offset, int limit) {
    try {
      List<String> raw =
          redisTemplate.opsForList().range(DEAD_LETTER_KEY, offset, offset + limit - 1);
      ...
      for (String json : raw) {
        try {
          result.add(YdszJson.fromJson(json, RetryableMessage.class));
        } catch (Exception e) {
