 ...
 * <p>支持两种鉴权模式：
 * <ol>
 *   <li><b>JWT Token</b>：从查询参数 {@code token} 或请求头 {@code Authorization: Bearer xxx}
 *       提取 JWT，通过 {@link TokenService} 校验合法性（默认模式）</li>
 *   <li><b>网关透传</b>（P1-5）：浏览器无法在 WebSocket 升级请求中设置自定义头，
 *       因此网关认证后注入 {@code X-User-Id} / {@code X-Username} / {@code X-Gateway-Secret}。
 *       后端验证共享密钥或 IP 白名单后信任这些头部</li>
 * </ol>
 *
...
public class WebSocketAuthInterceptor implements HandshakeInterceptor {
     ...
     * @return 是否通过
     */
    private boolean authenticateByJwt(ServerHttpRequest request, ServerHttpResponse response,
        ...
        return completeAuthentication(request, response, attributes, userInfo.getUserId(), userInfo.getUsername());
    }

    /**
     * 网关透传认证（P1-5）。
     *
     * <p>验证 {@code X-Gateway-Secret} 共享密钥或来源 IP 白名单，通过后信任
     * {@code X-User-Id} / {@code X-Username} 头部。
     *
     * @param request    HTTP 请求
     * @param response   HTTP 响应
     * @param attributes 握手属性
     * @return 是否通过
     */
    private boolean authenticateByGateway(ServerHttpRequest request, ServerHttpResponse response,
        ...
        if (!verifyGatewayTrust(request)) {
            log.warn("[WS-Auth] 网关认证失败: 共享密钥不匹配且 IP 不在白名单, remote={}",
        ...
        return completeAuthentication(request, response, attributes, userId, username != null ? username : userId);
    }
     ...
     * @return 是否通过（连接数检查）
     */
    private boolean completeAuthentication(ServerHttpRequest request, ServerHttpResponse response,
                                           Map<String, Object> attributes, String userId, String username) {
        // 连接数限制检查（P2-1）
     ...
     * <p>满足以下任一条件即通过：
     * <ul>
     *   <li>请求头 {@code X-Gateway-Secret} 与配置的共享密钥一致</li>
     *   <li>请求来源 IP 在 {@code trustedIps} 白名单内</li>
     * </ul>
     *
     * @param request HTTP 请求
     * @return 是否受信任
