 ...
 *
 * <p>使用 Redis Hash 跟踪用户在线状态：key = {@code ydsz:ws:online:{userId}}， field = sessionId，value =
 * 时间戳。支持同一用户多端登录（多个 sessionId 共存）。
 *
 * <p>判定在线策略：用户至少有一个活跃 session 即视为在线；断开时移除对应 sessionId， 当 Hash 为空时判定离线。
 *
