 ...
 *
 * <ul>
 *   <li>{@code local.activeConnections} — 本节点活跃 WebSocket 连接数
 *   <li>{@code cluster.enabled} — 集群广播是否启用
 *   <li>{@code cluster.redisReachable} — Redis 连通性（PING 探测）
 *   <li>{@code offline.enabled} — 离线消息存储是否启用
 *   <li>{@code rateLimit.enabled} — 速率限制是否启用
 *   <li>{@code heartbeat.serverInterval} — 服务端心跳间隔（ms）
 *   <li>{@code circuitBreaker.state} — 熔断器状态（CLOSED/OPEN/HALF_OPEN）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class WebSocketHealthIndicator implements HealthIndicator {
  ...
  public Health health() {
    ...
    try {
      ...
      builder.withDetail(
          "heartbeat.serverIntervalMs", properties.getHeartbeat().getServerInterval());
      builder.withDetail(
          "heartbeat.clientIntervalMs", properties.getHeartbeat().getClientInterval());
      builder.withDetail(
          "heartbeat.staleSessionTimeoutMs", properties.getHeartbeat().getStaleSessionTimeout());
