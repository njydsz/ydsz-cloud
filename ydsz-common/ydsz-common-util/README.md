...
> YDSZ 通用工具类库（L2 工具层）— 99+ 工具类覆盖 ID 生成、加密、HTTP、字符串、日期、文件、集合、Bean 拷贝、Spring 集成等领域
...
| 类 | 说明 |
|---|---|
| `ChaCha20Utils` | ChaCha20-Poly1305 AEAD 对称加密（2.0.0 起标记废弃，256 位密钥、96 位 Nonce，纯软件优于 AES-GCM；可迁移至 ydsz-common-crypto-advanced） |
| `Ed25519Utils` | Ed25519 签名算法（2.0.0 起标记废弃，Curve25519、EdDSA、确定性签名、64 字节签名 / 32 字节密钥；可迁移至 ydsz-common-crypto-advanced） |

> JDK 要求：ChaCha20-Poly1305 需要 JDK 12+，Ed25519 需要 JDK 15+（推荐 JDK 17+）。
> **迁移建议：** ChaCha20/Ed25519 属于国际前沿算法，不在国密合规场景要求内，建议迁移至独立的 `ydsz-common-crypto-advanced` 模块，减少 `ydsz-common-util` 模块体积。
...
```java
import com.njydsz.common.util.security.ChaCha20Utils;
import com.njydsz.common.util.security.Ed25519Utils;
...
- **v2.0.0**（2026-08-06）：架构重构
  - **加密模块**：ChaCha20Utils / Ed25519Utils / Rsa2Utils 标记废弃（@Deprecated since 2.0.0），推荐迁移至 ydsz-common-crypto-advanced 模块
