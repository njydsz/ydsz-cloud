 ...
 * 非 K8s 环境使用 IP 哈希（{@link IpHashWorkerIdAllocator}）。
 * 生产环境如需强唯一性，可自定义实现（如 Redis/DB 注册表）通过
 * {@link WorkerIdAllocatorChain#prepend} 前置到策略链头部。
 *
 * <p>多个实现通过 {@link WorkerIdAllocatorChain} 组合为策略链，
 * 首个成功分配即返回。
 *
