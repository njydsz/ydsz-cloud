 ...
 * HTTP 响应渲染工具类
 *
 * <p>封装向 Servlet Response 写入内容的标准化逻辑，
 * 统一处理状态码、Content-Type 和异常降级。
 *
 * <h2>渲染能力</h2>
 * <ul>
 *   <li>字符串渲染：直接写入文本内容</li>
 *   <li>对象渲染：自动序列化为 JSON 字符串后写入</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 2.0.0
...
public final class HttpResponseUtils {
     ...
     * 将字符串渲染到客户端。
     *
     * <p>统一设置状态码 200、Content-Type 为 application/json;charset=UTF-8，
     * 然后写入 body 内容。
     *
     ...
     *
     * <p>内部使用 {@link YdszJson#toJson(Object)} 序列化，再调用 {@link #renderString} 渲染。
     * 若 object 为 null，不进行任何操作（避免写入 null 字符串）。
     *
     * @param response HTTP 响应
     ...
     */
    public static void renderJson(HttpServletResponse response, Object object, HttpStatus httpStatus) {
        if (object == null) {
            // 空 body 渲染
            render(response, "", httpStatus.value(), DEFAULT_CONTENT_TYPE, DEFAULT_CHARSET);
            return;
        }
        render(response, YdszJson.toJson(object), httpStatus.value(), DEFAULT_CONTENT_TYPE, DEFAULT_CHARSET);
    }
