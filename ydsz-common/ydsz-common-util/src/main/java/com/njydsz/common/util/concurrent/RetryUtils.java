 ...
 * 重试工具类（Unchecked 风格）。
 *
 * <p>提供简洁易用的重试能力，支持固定间隔、指数退避、自定义异常判断等策略。 适用于网络调用、消息发送、外部 API 调用等需要容错处理的场景。
 *
...
public final class RetryUtils {
   ...
   * 固定间隔重试（所有异常均触发重试）。
   *
   * <p>在 action 失败时以固定的时间间隔进行重试，最多重试 maxRetries 次。 重试耗尽后抛出 {@link RetryException}，其中包含最后一次失败的异常。
   *
  ...
  public static <T> T executeWithRetry(
    ...
    for (int attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries && retryOn.test(e)) {
          log.debug(
              "Retry attempt {}/{} after delay {} due to: {}",
              attempt + 1,
              maxRetries,
              delay,
              e.getMessage());
    ...
    throw new RetryException(
        "Retry exhausted after " + (maxRetries + 1) + " attempts", lastException);
  }
        ...
        if (attempt < maxRetries && retryOn.test(e)) {
          ...
          long jitterMs =
              ThreadLocalRandom.current()
                  .nextLong(0, Math.max(JITTER_MIN_UPPER_BOUND, currentDelayMs / 2));
          long sleepMs = Math.min(currentDelayMs + jitterMs, maxDelay.toMillis());
          log.debug(
              "Backoff retry attempt {}/{} after {}ms due to: {}",
              attempt + 1,
              maxRetries,
              sleepMs,
              e.getMessage());
   ...
   * 重试配置。
   *
   * <p>默认值：最大重试 3 次，初始延迟 500ms，最大延迟 30s，退避乘数 2.0， 重试条件为所有异常。
   */
  @Getter
  @Builder
  public static class RetryConfig {
    ...
    @Builder.Default private final Duration initialDelay = Duration.ofMillis(500);
    ...
    @Builder.Default private final Consumer<Integer> onRetry = attempt -> {};
  }
}
