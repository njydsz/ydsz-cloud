 ...
 * 字段差异计算器
 *
 * <p>对比两个同类型实体对象的字段值差异，仅处理标注了 {@link DiffField} 注解的字段。 支持敏感字段脱敏、自定义格式化器、忽略字段等特性。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
 ...
 *
 * <ul>
 *   <li>两个对象必须为同一类型，否则抛出 IllegalArgumentException
 *   <li>仅对比标注了 {@link DiffField} 且 ignore=false 的字段
 *   <li>敏感字段会自动脱敏（保留前 2 后 2 位）
 *   <li>字段元数据会被缓存，避免重复反射
 * </ul>
 *
 * @author ydsz-team
...
public class DiffCalculator {
  ...
  private final Map<Class<? extends DiffValueFormatter>, DiffValueFormatter> formatterCache =
  ...
  public <T> DiffReport calculate(T oldObj, T newObj) {
    ...
    if (oldObj.getClass() != newObj.getClass()) {
      throw new IllegalArgumentException(
          "oldObj and newObj must be of the same type: "
    ...
    for (FieldMeta meta : metas) {
      ...
      try {
        ...
        String label =
            meta.annotation.fieldName().isEmpty()
                ? meta.field.getName()
                : meta.annotation.fieldName();
        diffs.add(
