 ...
 * SM2 椭圆曲线公钥密码算法工具类
 *
 * <p>基于 256 位素数域椭圆曲线，安全性对标 RSA-3072，加解密/签名性能优于 RSA， 符合国密标准 GM/T 0003-2012。纯 BouncyCastle 实现。
 *
 * <h2>核心能力</h2>
 *
 * <ul>
 *   <li>密钥对生成：生成 SM2 密钥对（sm2p256v1 曲线）
 *   <li>公钥加密/私钥解密：C1C3C2 密文格式（国密标准推荐）
 *   <li>私钥签名/公钥验签：SM3withSM2 签名算法
 *   <li>支持 Hex/Base64 编码的密钥导入导出
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>算法强度 256 位，相当于 RSA-3072
 *   <li>私钥长度远小于 RSA，签名速度更快
 *   <li>国密合规场景下优先使用 SM2 替代 RSA
 *   <li>签名默认使用 SM3 摘要算法
 * </ul>
 *
 * @author ydsz-team
...
public final class Sm2Utils {
  ...
  // CHECKSTYLE.ON: RegexpSinglelineJava
          () -> {
            try {
              ...
              throw new IllegalStateException(
                  "Failed to initialize SM3withSM2 Signature (ensure bcprov-jdk18on is on classpath)",
                  e);
            }
          });
   ...
   *
   * <p>密文格式为 C1C3C2（国密标准推荐），C1 为随机公钥点， C3 为 SM3 摘要，C2 为密文数据。
   *
  ...
  public static boolean verify(String content, String base64Sig, PublicKey publicKey) {
    Objects.requireNonNull(content, "content must not be null");
    return verify(
        content.getBytes(StandardCharsets.UTF_8), Base64.getDecoder().decode(base64Sig), publicKey);
  }
  ...
  public static String[] getPublicKeyCoordinates(PublicKey publicKey) {
    ...
    try {
      ...
      return new String[] {
        toHex(padTo32Bytes(x.toByteArray())), toHex(padTo32Bytes(y.toByteArray()))
      };
    } catch (Exception e) {
...
/** privateKey */
  ...
  public static KeyPairHex generateKeyPairHex() {
    ...
    return new KeyPairHex(encodePublicKeyHex(kp.getPublic()), encodePrivateKeyHex(kp.getPrivate()));
  }
  ...
  public static KeyPairHex generateKeyPairBase64() {
    ...
    return new KeyPairHex(
        encodePublicKeyBase64(kp.getPublic()), encodePrivateKeyBase64(kp.getPrivate()));
  }
}
