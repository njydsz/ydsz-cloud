 ...
 *
 * <p>算法：对完整 IP 字节序列做 FNV-1a 哈希，映射到 [0, 1024) 区间。 相比仅取末段字节（只能产生 0~255），全 IP 哈希将 workerId 均匀分布到
 * 0~1023， 显著降低跨子网/同末字节场景的冲突概率。
 *
 * <p>适用场景：虚拟机、裸机、开发机等非 K8s 环境（作为 {@link PodOrdinalWorkerIdAllocator} 的兜底）。
 *
 * <p><b>冲突风险与边界：</b>本策略基于哈希，无法保证全局唯一。 生产环境如需强唯一性，应优先使用 {@link
 * PodOrdinalWorkerIdAllocator}（StatefulSet 序数，确定性） 或接入外部协调器（Redis/DB 注册表）通过 {@link
