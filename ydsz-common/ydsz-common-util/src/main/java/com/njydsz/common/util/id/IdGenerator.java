 ...
 * 分布式 ID 生成器统一门面。
 *
 * <p>为非 Spring 管理的类（Domain Model、工具类、非 Bean Pojo）提供 便捷的 ID 生成入口，替代 {@code UUID.randomUUID()}。
 *
 * <p><b>两种使用方式：</b>
 *
 * <ol>
 *   <li><b>静态门面（本类）：</b>适用于非 Spring 托管场景，通过注册的 {@link Supplier} 懒获取 {@link SnowflakeIdGenerator}
 *       Bean，未注册时安全降级为伪随机 long
 *   <li><b>依赖注入：</b>Spring Bean 直接注入 {@link SnowflakeIdGenerator}（由 {@code UtilAutoConfiguration}
 *       注册）
 * </ol>
 *
 * <p><b>降级策略：</b>
 ...
 *       比特分布均匀，不会产生 B+Tree 热点
 *   <li>ydsz.util.snowflake.fallback-to-uuid=true：行为与旧版一致，使用 {@code UUID.getMostSignificantBits()}
 * </ul>
 *
...
public final class IdGenerator {
   ...
   * 上次获取失败的时间戳（毫秒）。
   *
   * <p>区别于"尚未解析"与"确实不存在"：失败仅记录冷却时间， 冷却期过后会再次尝试获取 Bean，避免启动期上下文未就绪时 被一次性判定为"不可用"而永久降级为 UUID（可能与
   * Snowflake 主键空间重叠）。
   */
   ...
   *     {@link ThreadLocalRandom#nextLong()} 降级
   */
  ...
  private static SnowflakeIdGenerator getGenerator() {
    ...
    if (supplier == null) {
      warnFallback("SnowflakeIdGenerator 未注册（Spring 容器尚未初始化或未引入自动装配），ID 降级为随机数");
      return null;
    }
  ...
  /**
   * 记录降级告警并刷新失败冷却时间，避免每次生成 ID 都重复打印日志。
   *
   * @param reason 降级原因
   */
  private static void warnFallback(String reason) {
    ...
    if (DEGRADED_WARNED.compareAndSet(false, true)) {
      log.warn(
          "{}。请确认已引入 ydsz-common-util 自动装配且未禁用 snowflake。" + "当前生成的 ID 为随机数，可能与 Snowflake 主键空间重叠！",
          reason);
    }
  }
