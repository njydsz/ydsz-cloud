 ...
 *   <li><b>依赖注入：</b>Spring Bean 直接注入 {@link SnowflakeIdGenerator}（由 {@code UtilAutoConfiguration} 注册）</li>
 * </ol>
 *
 * <p><b>降级策略：</b>
 * <ul>
    ...
    private static SnowflakeIdGenerator getGenerator() {
        ...
        synchronized (IdGenerator.class) {
            ...
            try {
                gen = supplier.get();
                if (gen == null) {
                    warnFallback("SnowflakeIdGenerator Bean 不可用（可能被 ydsz.util.snowflake.enabled=false 禁用），ID 降级为随机数");
                    return null;
                }
                cached = gen;
                return gen;
