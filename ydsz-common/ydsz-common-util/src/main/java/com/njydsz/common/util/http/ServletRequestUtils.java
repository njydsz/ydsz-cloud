 ...
 *
 * <p>本类为无状态工具类，所有方法均为纯操作传入的 {@link HttpServletRequest}， * * 不涉及静态全局配置（可信代理集合使用 Spring Bean 方式管理）。
 *
 * <h2>可信代理配置</h2>
 *
...
public final class ServletRequestUtils {
  ...
  private static final StaticBridge<TrustedProxyConfiguration> TRUSTED_PROXY_BRIDGE =
      new StaticBridge<>();
   ...
   *
   * <p>可信条件：内网/回环地址（RFC 1918 + 127.0.0.0/8 + ::1）， 或通过 {@link TrustedProxyConfiguration} 显式配置的代理
   * IP。
   *
