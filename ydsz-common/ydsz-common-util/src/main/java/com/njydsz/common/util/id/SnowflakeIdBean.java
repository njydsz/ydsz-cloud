 ...
 *
 * <p>将核心算法类 {@link SnowflakeIdGenerator} 注册为 Spring Bean， 使其支持依赖注入和 Spring IoC 容器生命周期管理。
 *
 * <p>注册条件：
 *
 * <ul>
 *   <li>{@code ydsz.util.snowflake.enabled=true}（或缺省，matchIfMissing=true）时注册
 *   <li>{@code ydsz.util.snowflake.enabled=false} 时跳过注册
 * </ul>
 *
 * <p>sequenceBits 配置优先级：
 *
 * <ol>
 *   <li>{@code ydsz.util.snowflake.sequence-bits} 显式配置
 *   <li>默认值 {@value SnowflakeIdGenerator#DEFAULT_SEQUENCE_BITS} 位
 * </ol>
 *
...
public class SnowflakeIdBean {
   ...
   *
   * <p>使用 {@link WorkerIdAllocator} 策略链自动分配 workerId， 支持显式配置覆盖（{@code
   * ydsz.util.snowflake.worker-id}）。
   *
