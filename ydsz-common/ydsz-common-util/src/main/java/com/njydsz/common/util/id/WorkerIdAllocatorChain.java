 ...
 *
 * <p>业务方可通过 {@link #prepend(WorkerIdAllocator)} 插入自定义策略（如基于 Redis/DB 的注册表实现）：
 *
 * <pre>{@code
 * WorkerIdAllocatorChain chain = WorkerIdAllocatorChain.defaults();
 * chain.prepend(new MyRedisWorkerIdAllocator(redisClient)); // 自定义实现，优先级最高
 * }</pre>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public final class WorkerIdAllocatorChain implements WorkerIdAllocator {
  ...
  public static WorkerIdAllocatorChain defaults() {
    return new WorkerIdAllocatorChain(
        List.of(new PodOrdinalWorkerIdAllocator(), new IpHashWorkerIdAllocator()));
  }
  ...
  public int allocate(String nodeId) {
    ...
    throw new WorkerIdExhaustedException(
        "All WorkerId allocation strategies failed. Tried: "
            + chain.stream()
  ...
  public String name() {
    return "Chain("
        + chain.stream().map(WorkerIdAllocator::name).reduce((a, b) -> a + " → " + b).orElse("")
        + ")";
  }
