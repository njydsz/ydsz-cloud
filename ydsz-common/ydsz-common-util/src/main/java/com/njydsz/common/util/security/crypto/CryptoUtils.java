 ...
 *
 * <ol>
 *   <li>注册表默认值：{@code AES-256-GCM}
 *   <li>Spring 配置：{@code ydsz.util.crypto.default-algorithm}（通过 {@link CryptoAutoConfiguration} 注入）
 *   <li>系统属性：{@code crypto.algorithm}（最高优先级，用于覆盖）
 * </ol>
 *
 * <h2>扩展能力</h2>
 *
...
public final class CryptoUtils {
   ...
   *
   * <p>注入后会清空已有缓存，下次 {@link #provider()} 重新解析。仅允许注入一次， 重复调用将被忽略并打印 warn 日志。
   *
  ...
  public static String encrypt(String plaintext, byte[] key) {
    ...
    byte[] ciphertext = provider().encrypt(plaintext.getBytes(StandardCharsets.UTF_8), key, null);
  ...
  public static String encryptWithAad(String plaintext, byte[] key, byte[] aad) {
    Objects.requireNonNull(plaintext, "plaintext must not be null");
    byte[] ciphertext = provider().encrypt(plaintext.getBytes(StandardCharsets.UTF_8), key, aad);
    return Base64.getEncoder().encodeToString(ciphertext);
  }
   ...
   * 获取当前默认算法提供者。
   *
   * <p>解析优先级：系统属性 {@code crypto.algorithm} 优先，未设置时使用 {@link CryptoAutoConfiguration} 注入值。
   *
