...
public final class DigestUtils {
  ...
  private DigestUtils() {
    throw new UnsupportedOperationException(
        "DigestUtils is a utility class and cannot be instantiated");
  }
   ...
   *
   * <p>注意：缓冲区仅在 digest 方法内部借用，方法返回前不被修改或清空。 由于方法执行期间线程独占使用，不会出现并发安全问题。
   *
  ...
  /**
   * 自研迭代哈希（非标准 KDF，仅限特殊场景使用）。
   *
   * <p>⚠️ <b>安全警告：</b>本方法为自研迭代哈希，非标准 PBKDF2/bcrypt/scrypt， <b>绝对不可用于密码存储</b>。密码存储请使用 {@link
   * PwdUtils}。
   *
   * <p>每次迭代均混入 salt，确保 salt 对最终哈希值的充分影响。 迭代公式：H_0 = H(salt || input)，H_i = H(salt || H_{i-1})
   *
   * @param input 待散列的数据
  ...
  public static byte[] sha256(byte[] input) {
    return digestIterativeRaw(input, "SHA-256", null, 1);
  }
  ...
  public static byte[] sha512(byte[] input) {
    return digestIterativeRaw(input, "SHA-512", null, 1);
  }
  ...
  public static String hmacSha256Hex(String input, String key) {
    ...
    return hmacSha256Hex(
        input.getBytes(StandardCharsets.UTF_8), key.getBytes(StandardCharsets.UTF_8));
  }
  ...
  public static String hmacSha256Base64(String data, String secret) {
    ...
    byte[] hmac =
        hmacSha256(data.getBytes(StandardCharsets.UTF_8), secret.getBytes(StandardCharsets.UTF_8));
  ...
  public static boolean verifySignature(
      String data, String secret, String signature, SignatureEncoding encoding) {
