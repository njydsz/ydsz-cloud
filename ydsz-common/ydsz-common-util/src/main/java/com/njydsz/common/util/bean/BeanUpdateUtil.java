 ...
 * Bean 动态更新工具类。
 *
 * <p>核心能力：复制源对象中 <b>非 null</b> 的属性到目标对象，避免覆盖目标对象已有值。 用于「PATCH 语义」的部分更新场景（PUT/POST 请求中 DTO
 * 只携带需要变更的字段）。
 *
 * <h3>背景</h3>
 *
 * <p>Spring 的 {@link org.springframework.beans.BeanUtils#copyProperties(Object, Object, String...)}
 * 默认会复制源对象的所有属性（包括 null），导致目标对象的已有字段被 null 覆盖。 常见做法是手动传入 {@code getNullPropertyNames(source)}
 * 数组作为忽略列表， 但每个 Service 都重复实现一遍 {@code getNullPropertyNames} 既冗余又容易遗漏。
 *
 * <h3>统一方案</h3>
 *
 * <p>本工具提供 {@link #copyNonNull(Object, Object, String...)} 方法，封装「忽略 null 属性 + 可选固定忽略属性」语义， 替代各
 * Service 中分散的 {@code BeanUtils.copyProperties + getNullPropertyNames} 模式。
 *
 * <h3>使用示例</h3>
 *
 * <pre>{@code
