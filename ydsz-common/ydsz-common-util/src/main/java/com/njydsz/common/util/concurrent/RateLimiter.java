...
public final class RateLimiter {
     ...
     * 动态调整速率（每秒令牌数）。
     *
     * <p>调整后立即生效：先按旧速率补充存量令牌，再以新速率重算令牌间隔与桶容量。
     * 桶容量保持 maxBurstSeconds 秒的突发余量。
     *
    ...
    public void setRate(double newPermitsPerSecond) {
        if (newPermitsPerSecond <= 0) {
            throw new IllegalArgumentException("permitsPerSecond must be positive: " + newPermitsPerSecond);
        }
        lock.lock();
        try {
            refill(System.nanoTime());
