 ...
 * URL 路径匹配工具类
 *
 * <p>提供 Ant 风格路径模式匹配能力，供安全过滤器（XSS / CSRF / SQL 注入 / IP 访问控制 / 安全响应头 / API
 * 签名等）与认证过滤器判断请求路径是否命中白名单或排除清单。
 *
 * <p>Ant 风格规则示例：
 *
 * <ul>
 *   <li>{@code /api/**} — 匹配 {@code /api/} 下所有路径
 *   <li>{@code /actuator/*} — 匹配 {@code /actuator/} 下一级路径
 *   <li>{@code /login} — 精确匹配
 * </ul>
 *
 * @since 1.0.0
