 ...
 *
 * <ul>
 *   <li>单节点理论峰值（默认 7-bit）：12.8 万 ID/s（每毫秒 128 个）
 *   <li>单节点理论峰值（13-bit）：81.9 万 ID/s（每毫秒 8192 个）
 *   <li>实际吞吐量取决于 CAS 竞争程度，普通服务器 5-10 万 ID/s
 * </ul>
 *
 * <h2>时钟回拨处理</h2>
 *
 * <ul>
 *   <li>≤ 5ms：循环等待恢复
 *   <li>> 5ms：抛出 {@link ClockBackwardException} 强制报错
 * </ul>
 *
...
public class SnowflakeIdGenerator {
   ...
   *
   * <p>workerId 分配策略链：PodOrdinal → IpHash。 业务方可通过声明自定义 {@link WorkerIdAllocator} Bean 插入更高优先级的策略。
   *
  ...
  SnowflakeIdGenerator(
    ...
    if (sequenceBits < 1 || sequenceBits > MAX_SEQUENCE_BITS) {
      throw new IllegalArgumentException(
          String.format(
              "sequenceBits must be between 1 and %d, got %d", MAX_SEQUENCE_BITS, sequenceBits));
    }
    ...
    this.datacenterId =
        properties.getDatacenterId() != null ? properties.getDatacenterId() : computeDatacenterId();
  ...
  /**
   * 便捷构造器：使用默认策略链（PodOrdinal → IpHash）。 适用于非 Spring 托管场景（如单元测试、独立工具）。
   *
   * <p>使用默认序列号位数（{@value #DEFAULT_SEQUENCE_BITS} 位）。
   *
   */
   ...
   * 生成下一个唯一 ID（线程安全）。
   *
   * <p>使用 CAS 无锁重试，在单节点 100 万 QPS 下 CAS 失败率 < 1%， 绝大多数场景下 1 次 CAS 即可成功。
   *
   * @return 生成的唯一 ID
   ...
   *       避免忙等消耗 CPU
   *   <li>超过最大等待时间：抛出 {@link ClockBackwardException}
   * </ul>
   *
  ...
  private long tilNextMillis(long lastTimestamp) {
    ...
    while (timestamp <= lastTimestamp) {
      ...
      if (offset > CLOCK_BACKWARD_TOLERANCE_MILLIS) {
        log.error(
            "Clock moved backwards by {} ms, exceeds tolerance {} ms",
            offset,
            CLOCK_BACKWARD_TOLERANCE_MILLIS);
        throw new ClockBackwardException(offset, lastTimestamp, timeGen());
      }
      if (totalWaited >= CLOCK_BACKWARD_MAX_WAIT_MILLIS) {
        log.error(
            "Clock moved backwards, waited {} ms, exceeds max wait {} ms",
            totalWaited,
            CLOCK_BACKWARD_MAX_WAIT_MILLIS);
        throw new ClockBackwardException(offset, lastTimestamp, timeGen());
      }
   ...
   *
   * <p>ID 内只存相对毫秒，配合 {@code epoch} 偏移量即可反解出绝对时间。 这样 41 位时间戳字段寿命从 1970 年起算延长至约 2090 年， 也保证 {@link
   * #getLastTimestamp()} / {@link #parseTimestamp(long)} 的反解语义正确。
   *
   * @return 处理结果
   */
   ...
   * 处理时间戳解析（含时钟回拨容忍）。
   *
   * <p>统一使用 {@link LockSupport#parkNanos} 等待策略，替代原来的 100ms 截止时间自旋循环， 减少 CPU 空转。所有时间戳均处于相对 epoch
   * 的毫秒域内（见 {@link #currentTimeRelative()}）。
   *
    ...
    if (timestamp < lastTimestamp) {
      ...
      while (currentTimeRelative() < lastTimestamp) {
        if (totalWaited >= CLOCK_BACKWARD_MAX_WAIT_MILLIS) {
          ...
          log.error(
              "Clock still moved backwards after waiting {} ms, remaining offset: {} ms",
              totalWaited,
              remainingOffset);
        ...
        long parkMillis =
            Math.min(
                Math.max(lastTimestamp - currentTimeRelative(), 1L),
   ...
   *
   * <p>用于健康检查等场景快速获取最后一次 ID 生成时间，不会触发 ID 生成。 如果尚未生成过 ID，返回当前时间。
   *
