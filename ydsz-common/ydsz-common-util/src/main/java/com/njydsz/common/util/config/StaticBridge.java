 ...
 *
 * <p>简化"非 Spring 管理的静态工具类安全获取 Spring Bean"的通用模式。
 * 各静态工具类通过本类持有 Bean {@link Supplier}，由 AutoConfiguration
 * 在容器就绪后调用 {@link #registerSupplier} 注入。</p>
 *
 * <p>解析策略（DCL + volatile）：</p>
 * <ul>
 *   <li>首次解析成功后缓存结果，后续调用直接返回缓存（跳过 Supplier 查找）</li>
 *   <li>注意：仅缓存成功的结果。Supplier 返回 null 或抛异常时不缓存，下次调用会重试</li>
 *   <li>Supplier 未注册时返回 null</li>
 * </ul>
 *
 * <p>使用示例：</p>
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>典型生命周期：{@code registerSupplier → getIfAvailable(解析+缓存) → 后续直接返回缓存}。</p>
 *
 * <p>线程安全：volatile 保证 happens-before，synchronized 保证原子性。</p>
 *
 * @param <T> Bean 类型
 * @author ydsz-team
 * @since 4.2.0
...
public final class StaticBridge<T> {
     ...
     * 注册 Bean 提供者。
     *
     * <p>由 AutoConfiguration 在容器初始化时调用。重新注册时会清空已有缓存，
     * 以便下次 {@link #getIfAvailable()} 重新解析（适用于测试重置场景）。</p>
     *
     ...
     * 获取 Bean 实例。
     *
     * <p>仅成功结果被永久缓存；Supplier 未注册、返回 null 或抛异常时均返回 null，
     * 调用方应自行降级处理。</p>
     *
