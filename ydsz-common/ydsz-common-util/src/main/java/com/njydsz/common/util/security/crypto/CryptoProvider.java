 ...
 * 对称加密算法统一契约。
 *
 * <p>所有对称加密实现（AES-GCM、SM4-GCM、SM4-CBC）都遵循此接口， 上层业务通过 {@link CryptoProviderRegistry#get(String)}
 * 获取实例， 切换算法时业务代码无需任何改动。
 *
 * <p><b>密文格式统一：</b>IV || ciphertext || tag（无编码，原始字节）， 编解码由 {@link CryptoUtils} 负责。
 *
 * <p><b>线程安全：</b>所有实现必须保证线程安全，可在多线程环境下复用实例。
 *
 * <p><b>密钥生命周期：</b>本接口的实现不会自动擦除传入的密钥字节数组。 调用方负责密钥的安全管理，使用完毕后应调用 {@link
...
public interface CryptoProvider {
   ...
   *
   * <p>该值作为注册表的 key，需保证全局唯一且具有可读性， 推荐格式：算法名-密钥长度-模式。
   *
   * @return 算法唯一标识字符串
   */
  @Nonnull
   ...
   *
   * <ul>
   *   <li>生成的密文前 {@link #ivLength()} 字节为随机 IV
   *   <li>AEAD 模式下后缀包含认证标签（GCM 为 16 字节）
   *   <li>实现内部每次生成新的随机 IV，保证相同明文产生不同密文
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>从密文前 {@link #ivLength()} 字节提取 IV
   *   <li>剩余部分为 ciphertext + tag（AEAD 模式）
   *   <li>认证失败时必须抛出 {@link CryptoException}，不得返回部分解密结果
   * </ul>
   *
