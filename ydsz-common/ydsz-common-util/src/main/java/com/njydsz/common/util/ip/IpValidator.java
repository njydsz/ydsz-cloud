...
public final class IpValidator {
  ...
  /** IPv6 私有/链路本地地址前缀 */
  private static final String[] PRIVATE_IPV6_PREFIXES = {"fe80:", "fc", "fd", "::1", "::ffff:"};

  private IpValidator() {
    throw new UnsupportedOperationException(
        "IpValidator is a utility class and cannot be instantiated");
  }
   ...
   *
   * <p>JDK 的 {@link InetAddress#getHostAddress()} 返回全 8 组形式（不压缩）， 本方法在此基础上折叠最长的连续全零组为 {@code ::}。
   *
   * @param ipv6 ipv6
   * @return 处理后的字符串
   */
   ...
   *
   * <p>IPv4 字面量直接解析为 4 字节后通过 {@link InetAddress#getByAddress(byte[])} 构造， 彻底规避 DNS 解析与反向查询；IPv6
   * 字面量经 {@link InetAddress#getByName(String)} 时， JDK 会先尝试按字面量解析，仅对主机名才触发 DNS，故合法 IPv6
   * 字面量同样不会发起网络请求。
   *
