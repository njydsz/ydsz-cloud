 ...
 *
 * <p>通过 JDK {@link java.util.ServiceLoader} 机制支持业务方自定义密码强度规则， 替代 {@link
 * PwdUtils#checkPasswordStrength(String)} 内置的简单评分逻辑。
 *
 * <p><b>典型扩展场景：</b>
 *
 * <ul>
 *   <li>校验密码不在常见弱密码字典中（如 123456、password）
 *   <li>校验密码不含用户名/手机号等个人信息
 *   <li>校验密码不含连续字符（如 123、abc）或重复字符（如 aaa）
 *   <li>NIST SP 800-63B / 等保 2.0 / OWASP ASVS 等合规规则
 * </ul>
 *
 * <p><b>接入方式：</b> 在 {@code
 * META-INF/services/com.njydsz.common.util.password.PasswordStrengthChecker}
 * 文件中填写实现类的全限定名（每行一个）。Utils 模块默认提供 {@link DefaultPasswordStrengthChecker}， 业务方提供自己的实现后会自动覆盖默认实现。
 *
 * <p><b>实现约定：</b>
 *
 * <ul>
 *   <li>实现类必须提供 public 无参构造器（ServiceLoader 要求）
 *   <li>实现必须线程安全（无实例变量或使用 ThreadLocal）
 *   <li>入参 password 可能为 null，需 null-safe 处理
 * </ul>
 *
 * <pre>{@code
 * // 1. 实现自定义校验器
...
public interface PasswordStrengthChecker {
   ...
   * 密码强度等级枚举。
   *
   * <p>与原 {@link PwdUtils.PasswordStrength} 语义一致但独立定义， 便于 SPI 演进时互不影响。
   */
