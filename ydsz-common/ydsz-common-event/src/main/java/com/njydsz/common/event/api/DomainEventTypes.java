 ...
 * 统一领域事件类型注册表
 *
 * <p>定义所有跨模块事件的标准类型名称，作为模块间事件契约的单一来源。 各模块发布事件时使用此处定义的常量，消费方按类型订阅。
 *
 * <p><b>命名规范：</b>
 *
 * <ul>
 *   <li>格式：{@code MODULE_ENTITY_ACTION}（如 {@code ORDER_CREATED}、{@code USER_DISABLED}）
 *   <li>使用过去时或完成时，表达"已发生的事实"
 *   <li>每个事件类型注明发布方模块和消费方模块
 * </ul>
 *
 * @author ydsz-team
 * @since 1.6.0
