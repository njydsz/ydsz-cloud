 ...
 *
 * <ul>
 *   <li>分页查询死信列表（按时间 / 事件类型过滤）
 *   <li>手动重试（将 DEAD_LETTER 重置为 PENDING）
 *   <li>安全删除（仅允许删除 SENT / DEAD_LETTER 状态的消息）
 *   <li>队列深度统计
 * </ul>
 *
 * <p><b>安全约束：</b>
 *
 * <ul>
 *   <li>禁止删除 PENDING / PROCESSING 状态的消息（避免消息丢失）
 *   <li>重试操作通过 CAS 更新确保幂等（仅 DEAD_LETTER 可被重置）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class OutboxAdminService {
   ...
   * 手动重试死信消息
   *
   * <p>将指定消息从 {@link OutboxStatus#DEAD_LETTER} 重置为 {@link OutboxStatus#PENDING}， 使其被轮询器重新投递。操作通过
   * CAS 更新确保幂等——仅当消息当前状态为 DEAD_LETTER 时才成功。
   *
   ...
   * 安全删除已终态消息
   *
   * <p>仅允许删除 {@link OutboxStatus#SENT} 或 {@link OutboxStatus#DEAD_LETTER} 状态的消息。 禁止删除 PENDING /
   * PROCESSING 状态的消息以避免消息丢失。
   *
   * @param messageId 消息 ID
