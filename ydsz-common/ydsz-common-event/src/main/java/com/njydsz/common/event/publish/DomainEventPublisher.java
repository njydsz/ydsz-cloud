 ...
 *
 * <ul>
 *   <li>将 {@link DomainEvent} 转换为 {@link OutboxMessage} 并委托 {@link OutboxService} 写入
 *   <li>发布失败仅告警不影响主流程（异步投递由轮询器兜底）
 *   <li>OutboxService 不可用时静默降级（模块未引入 event 包时不影响业务）
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
 ...
 * }</pre>
 *
 * <p><b>编码规范要求</b>：业务模块发布跨模块事件时， <b>必须使用本门面</b>，禁止直接注入 {@link OutboxService} 构建 {@link
 * OutboxMessage}。
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class DomainEventPublisher {
   ...
   * 发布领域事件到 Outbox。
   *
   * <p>内部将 {@link DomainEvent} 转换为 {@link OutboxMessage.Builder}， 委托 {@link
   * OutboxService#appendToOutbox} 完成持久化与发布。
   *
  ...
  public void publish(DomainEvent event) {
    ...
    if (outboxService == null) {
      log.debug(
          "[DomainEventPublisher] OutboxService not available, skip event: type={}, id={}",
          event.getEventType(),
