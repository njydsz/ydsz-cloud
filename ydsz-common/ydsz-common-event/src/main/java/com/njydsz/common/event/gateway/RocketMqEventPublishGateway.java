 ...
 * RocketMQ 事件投递网关
 *
 * <p>基于 {@link RocketMQTemplate} 实现，将 Outbox 消息投递到 RocketMQ。 仅当 classpath 存在 {@code
 * RocketMQTemplate} 且容器中有 {@link RocketMQTemplate} Bean 时自动装配。
 *
 * <p>投递策略：
 *
 * <ul>
...
public class RocketMqEventPublishGateway implements EventPublishGateway {
   ...
   * 批量投递消息到 RocketMQ
   *
   * <p>使用 RocketMQ 原生批量发送能力（{@code DefaultMQProducer.send(Collection<Message>)}）。 当消息总大小超过 4MB
   * 时自动分包，每包独立发送。
   *
  ...
  private List<Boolean> publishBatchInternal(List<OutboxMessage> batch) throws Exception {
    ...
    for (OutboxMessage msg : batch) {
      ...
      Message mqMsg =
          new Message(
              topic, destination.split(":")[1], msg.getPayload().getBytes(StandardCharsets.UTF_8));
