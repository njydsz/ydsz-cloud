 ...
 * 空操作事件投递网关（降级实现）
 *
 * <p>当容器中不存在其他 {@link EventPublishGateway} 实现时使用。 记录 WARN 日志但不实际投递消息，返回 true 使消息被标记为 SENT 并清理。
 *
 * <p>生产环境应配置实际的 {@link EventPublishGateway} 实现（如 RocketMQ/Kafka）， 否则事件将丢失。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
