...
| 类 | 说明 |
|---|---|
| `EventAutoConfiguration` | Spring Boot 自动配置，`ydsz.event.outbox.enabled=true`（默认）且 `JdbcTemplate` 存在时装配；含 `@PostConstruct` 校验 Noop 网关、`@PreDestroy` 停止处理器 |
| `EventProperties` | 配置属性（`ydsz.event.outbox.*`） |
...
| `RocketMqEventPublishGateway` | RocketMQ 实现：Topic=`ydsz-outbox-events`，Tag=`eventType`，Body=`payload`，headers 作为用户属性；支持批量发送，失败自动降级为逐条投递 |
...
| `ydsz.outbox.queue.size` | Gauge | 队列深度（按 status 标签：PENDING / PROCESSING / DEAD_LETTER） |
...
-- 表名默认 ydsz_outbox，可通过 ydsz.event.outbox.table-name 覆盖
```
...
| `ydsz.event.outbox.table-name` | `ydsz_outbox` | Outbox 表名（仅允许字母/数字/下划线） |
| `ydsz.event.outbox.poll-interval-seconds` | 5 | 轮询间隔（秒） |
| `ydsz.event.outbox.batch-size` | 100 | 每批最大条数 |
| `ydsz.event.outbox.max-retries` | 5 | 最大重试次数（超过则进入死信） |
| `ydsz.event.outbox.base-backoff-seconds` | 10 | 基础退避秒数（指数退避基数） |
| `ydsz.event.outbox.max-backoff-seconds` | 3600 | 最大退避秒数（退避上限） |
| `ydsz.event.outbox.sent-retention-days` | 7 | 已投递消息保留天数（0=不清理） |
| `ydsz.event.outbox.auto-cleanup` | true | 是否启用自动清理 |
| `ydsz.event.outbox.cleanup-interval-hours` | 6 | 清理间隔（小时） |
| `ydsz.event.outbox.max-payload-size-bytes` | 4194304 (4MB) | payload 最大字节数 |
| `ydsz.event.outbox.default-priority` | 5 | 默认优先级（0-9，9 最高） |
| `ydsz.event.outbox.default-schema-version` | `v1.0.0` | 默认 Schema 版本号 |
| `ydsz.event.outbox.stale-processing-threshold-minutes` | 5 | PROCESSING 超时阈值（分钟），超时回收为 PENDING |
| `ydsz.event.outbox.pending-alert-threshold` | 10000 | PENDING 积压告警阈值 |
| `ydsz.event.outbox.dead-letter-alert-threshold` | 10 | DEAD_LETTER 告警阈值 |
| `ydsz.event.outbox.enable-tenant-isolation` | true | 是否启用租户隔离（自动注入 tenantId） |
| `ydsz.event.outbox.enable-sync-publish` | false | 是否启用同步投递（事务提交后立即投递） |
| `ydsz.event.outbox.auto-dedup` | false | 是否自动生成幂等去重 ID（基于 payload SHA-256） |
| `ydsz.event.outbox.worker-threads` | 1 | 投递工作线程数 |
| `ydsz.event.outbox.fail-on-noop` | true | 检测到 NoopEventPublishGateway 时是否阻止启动 |
...
public class KafkaEventPublishGateway implements EventPublishGateway {
    ...
    public boolean publish(OutboxMessage message) throws Exception {
        // 投递到 Kafka
        kafkaTemplate.send("ydsz-outbox-events", message.getEventType(), message.getPayload());
        return true;
    }
...
1. **fail-on-noop 生产必开**：生产环境必须设置 `ydsz.event.outbox.fail-on-noop=true`，否则检测到 `NoopEventPublishGateway` 时不报错，消息将静默丢失。
