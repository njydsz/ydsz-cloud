 ...
 * 请求上下文持有者
 *
 * <p>基于 TransmittableThreadLocal 的请求上下文传递机制，用于在请求处理链中传递：
 *
 * <ul>
 *   <li>userId - 用户ID
 *   <li>tenantId - 租户ID
 ...
 * </ul>
 *
 * <p>TransmittableThreadLocal 支持线程池场景下的自动上下文传递， 配合 TtlExecutors 或 Java Agent 使用时，无需手动
 * capture/restore。
 *
 * <p><b>使用示例：</b>
 *
 ...
 * </pre>
 *
 * <p><b>try-with-resources 用法（推荐）：</b>
 *
 * <pre>
 ...
 *
 * <ul>
 *   <li>必须在请求结束时调用 {@link #clear()} 防止内存泄漏
 *   <li>建议在拦截器或过滤器的 finally 块中统一管理生命周期
 *   <li>使用 TTL 线程池时，配合 {@link com.alibaba.ttl.TtlExecutors} 或 {@link com.alibaba.ttl.TtlRunnable}
 *       自动传播上下文
 * </ul>
 *
 * @author ydsz-team
...
public final class RequestContext {
   ...
   * 请求上下文存储（懒初始化）。
   *
   * <p>使用 {@link TransmittableThreadLocal} 支持线程池场景下的上下文传递。 采用懒初始化策略：仅当首次 {@link #put(String,
   * Object)} 时才创建 Map， 避免仅读取上下文（如仅调用 {@link #getUserId()} 判断）时无谓分配 HashMap。 初始容量 8，适配内置 6 个键的典型场景。
   */
  private static final TransmittableThreadLocal<Map<String, Object>> CONTEXT_HOLDER =
      new TransmittableThreadLocal<Map<String, Object>>() {
        ...
        /** 传播上下文到子线程时创建防御性拷贝（浅拷贝，值以引用传递）， 避免父线程与子线程共享同一 HashMap 导致并发修改异常。 */
        @Override
   ...
   * 设置租户隔离跳过标记。
   *
   * <p>当 Web 层拦截器判断当前请求 URL 在 anon-urls 白名单中时， 调用此方法标记跳过租户隔离，SQL 拦截器将不注入租户条件。
   *
   ...
   * 设置属性（字符串键）。
   *
   * <p>首次调用时创建上下文 Map（懒初始化）。 传入 {@code null} 值等同于 {@link #remove(String)}。
   *
   * <p>键名建议使用 {@code BizContextKeys.KEY_*} 常量（如 {@code BizContextKeys.KEY_AUTH_INFO}），
   * 保证整个项目的键名来源统一、避免散落的字符串字面量。
   *
   * @param key 属性键
   * @param value 属性值
   * @since 26.09.01
   ...
   * 获取属性（字符串键）。
   *
   * <p>上下文未初始化时返回 null，不触发 Map 创建。
   *
   * <p>键名建议使用 {@code BizContextKeys.KEY_*} 常量（如 {@code BizContextKeys.KEY_AUTH_INFO}），
   * 并通过显式强制类型转换获取值：{@code (AuthInfo) get(BizContextKeys.KEY_AUTH_INFO)}。
   *
   * @param key 属性键
   ...
   * 检查属性是否存在。
   *
   * <p>对标 {@link java.util.Map#containsKey(Object)} 语义，避免调用方 通过 {@link #get(String)} 取 null 后再做
   * null 判断（null 哨兵有歧义）。
   *
   * @param key 属性键
   ...
   * 写入一个数据权限相关的虚拟请求头。
   *
   * <p>由数据权限解析器写入，SQL 拦截器通过 {@link #getExtraHeader(String)} 读取。
   *
   ...
   *
   * <p>快照是<b>防御性拷贝</b>：保留原始值类型（如 Boolean、Long）， 对快照的修改不会影响当前线程上下文。 配合 {@link #restore(Map)}
   ...
   * 恢复上下文快照。
   *
   * <p>会用快照中的<b>全部键值</b>覆盖当前线程上下文（快照中不存在的 key 会被保留， 不会自动清除）。如需清空当前上下文再恢复，请调用方在执行前先 {@link
   * #clear()}。
   *
   ...
   * 将当前上下文桥接到 SLF4J MDC。
   *
   * <p>桥接 tenantId / userId / traceId / requestId 四个常用字段， 使日志框架的 {@code %X{tenantId}} /
   * %X{traceId}} 占位符生效。 一般在 Filter/Interceptor 入口与 {@link RequestContext} 赋值后调用一次。
   *
   ...
   * 创建当前线程上下文的诊断快照。
   *
   * <p>返回当前线程上下文的<b>不可变浅拷贝</b>（Map 本身不可变，值为原引用）， 用于开发诊断、日志输出、链路排查等场景。快照不影响原上下文。
   *
   * <p><b>使用示例：</b>
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>相比直接使用 {@link #newCleanupGuard()}，该方法会在清理阶段额外调用 {@link #clearMdc()}， 避免 {@link
   ...
   *
   * <p>适用于 {@code CompletableFuture.runAsync()}、{@code ExecutorService.submit()}、 {@code
   * Thread.start()} 等异步场景。内部完成：
   *
   * <ol>
   *   <li>捕获当前线程的 TTL 上下文快照
   *   <li>桥接 MDC 到子线程
   *   <li>执行 {@code task}
   *   <li>最终清理子线程的上下文与 MDC（防止复用线程污染）
   * </ol>
   *
   * <p><b>使用示例：</b>
   *
   ...
   *
   * <p>适用于 {@code CompletableFuture.supplyAsync()}、{@code ExecutorService.submit(Callable)} 等返回值场景。
   *
   * <p><b>使用示例：</b>
   *
   * <pre>{@code
   ...
   *
   * <p>内部委托每一步给 {@link #async(Runnable)}，无需再对每个 task 手动包装。
   *
   * <p><b>使用示例：</b>
   *
   * <pre>{@code
   ...
   *
   * <p>在 try 块结束时（无论正常或异常），自动调用 {@link #clear()} 清理当前线程的上下文。
   *
   * <p><b>使用示例：</b>
   *
   * <pre>
   ...
   *
   * <p>在 close() 时自动调用 {@link #clear()} 清理当前线程的上下文。
   */
