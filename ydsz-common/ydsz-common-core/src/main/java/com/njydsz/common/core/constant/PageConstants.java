 ...
 * 分页参数常量。
 *
 * <p>定义分页请求的默认值和上限。 编译期常量（{@link #DEFAULT_PAGE_SIZE} / {@link #MAX_PAGE_SIZE}）仅作为注解等需要
 * 编译时常量场景的固定值；运行时实际值统一从 {@link CoreProperties} 读取， 通过 {@link #init(CoreProperties)} 在启动时注入。
 *
 * <h3>两类值的使用场景</h3>
 *
 * <table>
 ...
 * </table>
 *
 * <p><b>初始化：</b>启动时由 {@code CoreAutoConfiguration} 调用 {@link #init(CoreProperties)} 注入运行时配置值，替代旧版
...
public final class PageConstants {
  ...
  /** 编译期默认每页记录数，用于注解等需要编译时常量的场景。 运行时请使用 {@link #getDefaultPageSize()} 获取配置值。 */
  public static final int DEFAULT_PAGE_SIZE = 20;

  /**
   * 编译期最大每页记录数上限（5000），用于 {@code @Max} 等注解的编译时常量。 运行时实际上限由 {@code ydsz.core.max-page-size} 配置控制，请使用
   * {@link #getMaxPageSize()}。
   */
   ...
   *
   * <p>启动后由 {@code CoreAutoConfiguration} 注入，采用一次性设置语义， 确保分页配置在应用生命周期内不可变。
   */
   ...
   * 标准化 offset（偏移量）。
   *
   * <p>适用于 offset/limit 分页模式（常用于大数据量深度分页场景）。 统一 offset 的归一化规则：
   *
   * <ul>
   *   <li>null 或小于 0 → 返回 0
   *   <li>其余原样返回
   ...
   *
   * <ul>
   *   <li>null 或小于 1 → 返回运行时默认值 {@link #getDefaultPageSize()}
   *   <li>大于运行时上限 → 截断为 {@link #getMaxPageSize()}
   *   <li>其余原样返回
   * </ul>
   *
   ...
   * 标准化页大小并返回是否被归一化的结果。
   *
   * <p>与 {@link #normalizePageSize(Integer)} 语义一致，但额外返回归一化是否发生， 便于调用方在响应中标记"分页参数已被框架调整"。
   *
   * @param pageSize 原始页大小（可为 null）
   ...
   *
   * <ul>
   *   <li>null 或小于 1 → 返回 {@link #DEFAULT_PAGE_NUM}
   *   <li>其余原样返回
   * </ul>
   *
   ...
   *
   * <p>统一分页查询的 offset 计算：{@code (pageNum - 1) * pageSize}， 页码和页大小均先经过归一化。
   *
   ...
   *
   * <p>计算公式：{@code pageNum = (offset / pageSize) + 1} 当 offset 不能被 pageSize 整除时，向上取整。
   *
   ...
   *
   * <p>计算公式：{@code (total + pageSize - 1) / pageSize}
   *
   * @param total 总记录数
   * @param pageSize 页大小（可为 null，按默认值处理）
   * @return 总页数
   ...
   *
   * <p>当 offset 超过 {@link #MAX_SAFE_OFFSET} 时，建议改用 游标分页（cursor-based pagination）或其他优化方案。
   *
   * @param offset 偏移量（可为 null，按 0 处理）
