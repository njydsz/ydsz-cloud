 ...
 * 业务级上下文键名常量。
 *
 * <p>承载认证、租户、列权限、审计、缓存等业务域通用上下文键名。
 * 这类键名已从 {@link RequestContext} 下沉到此专用类，使 core 模块
 * 保持对业务语义的零直接引用（仅通过 {@code BizContextKeys} 常量名
 * 引用自身的 key 字符串，不引入业务类型依赖）。</p>
 *
 * <p>业务 Filter（认证/数据权限/审计）应在入口处往 {@link RequestContext}
 * 写这些 key，后续业务代码通过此常量类引用（避免硬编码字符串）。</p>
 *
 * <p><b>注意：</b>core 模块不持有这些 key 对应的值类型 —— 值类型
 * （如 {@code AuthInfo}、{@code TenantContext}）定义在各自业务模块，
 * core 仅提供 String 键名常量。</p>
 *
 * @author ydsz-team
 * @since 1.9.0
 * @see RequestContext
