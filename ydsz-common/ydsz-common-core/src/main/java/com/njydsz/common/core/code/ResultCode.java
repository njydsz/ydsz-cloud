 ...
 *
 * <p>定义 API 错误响应 envelope 中错误码的三要素：错误码字符串、国际化消息键、默认兜底消息。 HTTP 状态码等异常下沉语义定义在 {@code ExceptionCode}
 * 子接口中。
 *
 * <p><b>继承体系：</b>
 *
 ...
 * </pre>
 *
 * <p><b>扩展指引：</b>业务模块自定义错误码请直接实现 {@code ExceptionCode} 接口并使用 {@code @YdszExceptionCode}
 * 注解注册，不要再直接实现此接口。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public interface ResultCode {
   ...
   *
   * <p>默认返回 {@code "core"}；业务模块枚举可覆盖本方法返回模块名 （如 {@code "workflow"}、{@code "user"}、{@code "cronjob"}
   * 等）， 使未显式覆盖 {@link #getKey()} 的枚举也能自动获得正确的 i18n key。
   *
   ...
   * </ul>
   *
   * 业务枚举也可通过显式声明 {@code key} 字段 + {@code @Getter} 覆盖此方法， 提供更精确的语义化 key（如 {@code
