 ...
 * }</pre>
 *
 * <p><b>异步解耦设计：</b> 为避免事件处理链中的循环依赖（事件聚合器 → IP 封禁 → 可能的后续事件）， 自动封禁操作通过 {@link BlockingQueue}
...
public class SecurityEventAggregator {
  ...
  private final ConcurrentHashMap<String, ConcurrentLinkedDeque<Long>> ipEventTimestamps =
  ...
  /** 异步封禁命令队列 */
  private final BlockingQueue<BlockCommand> blockQueue =
   ...
   * @param ip ip 参数
   * @param blockSeconds blockSeconds 参数
   * @param ip ip 参数
   * @param blockSeconds blockSeconds 参数
   * @param ip ip 参数
   * @param blockSeconds blockSeconds 参数
  ...
  public void onSecurityEvent(SecurityEvent event) {
    ...
    ConcurrentLinkedDeque<Long> timestamps =
        ipEventTimestamps.computeIfAbsent(sourceIp, k -> new ConcurrentLinkedDeque<>());
   ...
   *
   * <p>每 60 秒执行一次，清理超过滑动窗口的事件时间戳， 避免内存泄漏。同时清理已过期的封禁记录。
   */
  @Scheduled(
