...
public class NonceCache {
   ...
   * 存入 nonce。
   *
   * <p>如果 nonce 已存在，返回 false（表示请求重放）。 如果 nonce 不存在，存入并返回 true。
   *
   ...
   * 校验并消费 nonce（原子操作）。
   *
   * <p>用于接口签名防重放场景，将"检查 nonce 是否存在"和"存入 nonce"合并为原子操作， 避免并发请求同时通过检查。如果 nonce 已存在，表示请求重放，返回 false。
   *
   * <p><b>使用示例：</b>
   *
   * <pre>{@code
