 ...
 * 字段加密 TypeHandler
 *
 * <p>MyBatis TypeHandler，在写入数据库时自动加密，查询时自动解密。 配合 {@link EncryptField} 注解使用。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><b>解密失败处理：</b>读取数据库字段时，若值符合密文格式但解密失败 （GCM 认证失败 / 密钥版本缺失 / 密文被篡改），按 {@link
 * DecryptFailureStrategy} 处理：
 *
 * <ul>
 *   <li>{@link DecryptFailureStrategy#THROW}（默认）— 抛异常，阻止读取链路继续；
 *   <li>{@link DecryptFailureStrategy#RETURN_MASKED} — 返回打码值 {@code ****}；
 *   <li>{@link DecryptFailureStrategy#RETURN_ORIGINAL} — 返回原值并记录告警（不推荐）。
 * </ul>
 *
 * <p><b>历史明文兼容：</b>对于不符合密文格式的值（如历史未加密的明文数据）， 直接原样返回，不触发解密流程，便于灰度迁移。
 *
 * @author ydsz-team
 * @author ydsz-team
 * @since 1.0.0
...
public class EncryptTypeHandler extends BaseTypeHandler<String> {
   ...
   *
   * <p>Base64 编码后约为 40 个字符（29 字节 → 4 个 3 字节组向上取整 = 40 字符含 padding）。 此常量用于 {@link
   * #looksLikeCiphertext(String)} 启发式判断，避免对历史明文调用解密逻辑。
   */
  ...
  public static void setFailureStrategy(DecryptFailureStrategy strategy, String maskedValue) {
    ...
    EncryptTypeHandler.maskedValue =
        (maskedValue == null || maskedValue.isEmpty()) ? "****" : maskedValue;
  }
   ...
   * <ol>
   *   <li>空值：直接返回；
   *   <li>非密文格式（历史明文）：直接返回，不触发解密；
   *   <li>密文格式但解密失败：按 {@link #failureStrategy} 策略处理。
   * </ol>
   *
   ...
   * <ul>
   *   <li>能被 Base64 解码；
   *   <li>解码后字节数 >= {@value #MIN_CIPHERTEXT_BYTES}（1B 版本 + 12B IV + 16B Tag）；
   *   <li>首字节（版本号）落在 [1, 127] 区间（密钥版本通常为正整数）。
   * </ul>
   *
   * <p>此判断仅用于区分「历史明文」与「密文」，不保证 100% 准确。 对于极少数符合上述格式的明文（如长随机串），可能误判为密文并触发解密失败， 此时可通过 {@link
   * DecryptFailureStrategy#RETURN_ORIGINAL} 策略兜底。
   *
   * @param value 待判断的字符串
