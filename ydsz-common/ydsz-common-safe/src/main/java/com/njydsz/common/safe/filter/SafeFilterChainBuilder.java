 ...
 * 安全过滤器链构建器
 *
 * <p>集中编排安全过滤器链的注册顺序与条件，替代各 {@link FilterRegistrationBean} 分散注册的方式。
 * 每个安全能力通过 {@link FilterRegistrationDescriptor} 声明自身位置与启用条件，
 * 由本构建器统一排序后输出为 Spring {@link FilterRegistrationBean}。
 *
 * <p><b>设计目标：</b>
 * <ul>
 *   <li>单一入口管理过滤器顺序，避免散落在 {@code @Bean} 方法中难以全局调整</li>
 *   <li>支持声明式条件注册（仅当某条件满足时才启用过滤器）</li>
 *   <li>内置顺序冲突检测：相同 order 值时给出警告</li>
 * </ul>
 *
 * <p><b>当前演进说明：</b>
 * 本构建器为过滤器链统一编排提供基础抽象。当前实现以收集描述符 + 排序校验为主，
 * 后续可演进为完全替代分散 {@link FilterRegistrationBean} 的集中式注册。
 *
 * @author ydsz-team
 * @since 1.2.0
 */
public class SafeFilterChainBuilder {
    ...
    private void detectOrderConflict(List<FilterRegistrationDescriptor<?>> sorted) {
        for (int i = 1; i < sorted.size(); i++) {
            ...
            if (prev.order() == current.order()) {
                System.err.printf("[SafeFilterChain] WARN: order conflict between '%s' and '%s' (both order=%d)%n",
    ...
    public record FilterRegistrationDescriptor<T extends jakarta.servlet.Filter>(
        ...
        public static <T extends jakarta.servlet.Filter> FilterRegistrationDescriptor<T> of(
                String name, int order, List<String> urlPatterns,
                Supplier<Boolean> enabled, Supplier<FilterRegistrationBean<T>> supplier) {
