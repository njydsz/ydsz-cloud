...
public class RepeatSubmitTokenService {
  ...
  public boolean validateAndConsume(String userId, String token) {
    if (!StringUtils.hasText(token)) {
      log.warn("[ydsz-safe] [repeat-submit] Token 为空");
      return false;
    }
    if (!StringUtils.hasText(userId)) {
      log.warn("[ydsz-safe] [repeat-submit] 用户 ID 为空，无法校验 Token");
      return false;
    }
    ...
    if (Boolean.TRUE.equals(deleted)) {
      log.debug("[ydsz-safe] [repeat-submit] Token 校验通过并消费 | userId={}, token={}", userId, token);
      return true;
    } else {
      log.warn("[ydsz-safe] [repeat-submit] Token 无效或已过期 | userId={}, token={}", userId, token);
      return false;
    }
  }
