 ...
 * 统一请求体缓存过滤器
 *
 * <p>在过滤器链最前端将请求体读取到内存，包装为 {@link CachedBodyHttpServletRequestWrapper}
 * 后传递给下游过滤器。后续 XSS / SQL 注入 / API 签名等过滤器可直接复用已缓存的请求体，
 * 消除各自独立读取和包装导致的重复 I/O 与内存拷贝。
 *
 * <p>仅对包含请求体的请求（POST/PUT/PATCH 等）执行缓存，GET 等无 Body 请求直接放行。
 *
 * <p><b>设计考量：</b>
 * <ul>
 *   <li>优先级设为 {@link Ordered#HIGHEST_PRECEDENCE}，确保在安全过滤器链中第一个执行</li>
 *   <li>请求体大小限制为 10MB，超过此阈值不缓存（避免大文件上传场景内存溢出）</li>
 *   <li>包装后的请求通过 {@link CachedBodyHttpServletRequestWrapper#getCachedBody()} 提供字节访问</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.2.0
