 ...
 * 熔断器抽象基类（三态状态机）。
 *
 * <p>封装熔断器核心状态流转逻辑，提供 CLOSED → OPEN → HALF_OPEN → CLOSED/OPEN 的标准状态机。
 * 子类通过实现 {@link #evaluateThreshold()} 定义触发熔断的具体策略（如连续失败次数、滑动窗口失败率等）。
 *
 * <p><b>状态流转：</b>
 * <ul>
 *   <li>CLOSED → OPEN：调用 {@link #evaluateThreshold()} 返回 true 时自动触发</li>
 *   <li>OPEN → HALF_OPEN：{@link #halfOpenAfterMillis} 到达后首个请求 CAS 转换</li>
 *   <li>HALF_OPEN → CLOSED：探测成功（{@link #recordSuccess()}）</li>
 *   <li>HALF_OPEN → OPEN：探测失败（{@link #recordFailure()}）</li>
 * </ul>
 *
...
public abstract class AbstractCircuitBreaker {
    ...
    private void evaluateAndTransition() {
        if (stateRef.get() == State.CLOSED && evaluateThreshold()) {
            if (stateRef.compareAndSet(State.CLOSED, State.OPEN)) {
                openedAt = System.currentTimeMillis();
                log.warn("[CircuitBreaker] '{}' 触发熔断, threshold={}", config.getName(), config.getFailureThreshold());
            }
        }
    }
    ...
    public static class Config {
         ...
         */
        public Config(String name, double failureThreshold, long halfOpenAfterMillis, int permittedHalfOpenCalls) {
            this.name = name;
