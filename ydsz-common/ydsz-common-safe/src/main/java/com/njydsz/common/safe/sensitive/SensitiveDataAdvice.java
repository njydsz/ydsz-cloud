 ...
 * 敏感数据脱敏 AOP 拦截器
 *
 * <p>基于 Spring {@link ResponseBodyAdvice} 实现，在 Controller 方法返回值 写入 HTTP 响应体之前，自动对返回值中的敏感字段进行脱敏处理。
 *
 * <p><b>工作原理：</b>
 *
 * <ul>
 *   <li>拦截所有带有 {@link SensitiveData} 注解字段的方法返回值
 *   <li>支持全局脱敏规则（通过字段名匹配）
 *   <li>在序列化前调用 {@link SensitiveDataProcessor} 进行脱敏
 *   <li>支持配置开关 {@code ydsz.safe.sensitive.enabled} 控制是否启用
 *   <li>使用缓存机制避免重复检查同一个类
 * </ul>
 *
 * <p><b>使用示例：</b>
...
public class SensitiveDataAdvice implements ResponseBodyAdvice<Object> {
  ...
  /** 缓存类的敏感字段检查结果，避免重复反射检查 Key: Class对象，Value: 是否包含敏感字段 */
  ...
  private boolean containsSensitiveAnnotation(Class<?> clazz, MethodParameter methodParameter) {
    // 使用缓存避免重复检查
    return sensitiveClassCache.computeIfAbsent(
        clazz, c -> doCheckSensitiveAnnotation(c, methodParameter, 0));
  }

  /** 实际执行敏感注解检查 */
  private boolean doCheckSensitiveAnnotation(
      Class<?> clazz, MethodParameter methodParameter, int depth) {
