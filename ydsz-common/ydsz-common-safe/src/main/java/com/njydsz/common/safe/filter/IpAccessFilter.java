 ...
 * IP 黑白名单过滤器
 *
 * <p>基于 {@link IpAccessService} 实现 IP 访问控制，在请求进入安全过滤器链之前 执行 IP 黑白名单检查。命中黑名单的 IP 返回 403 Forbidden。
 *
 * <p><b>过滤器优先级：</b>IP 访问控制过滤器优先级最高（HIGHEST_PRECEDENCE）， 确保恶意 IP 在进入 XSS/SQL 注入/限流等过滤器之前就被拦截。
 *
