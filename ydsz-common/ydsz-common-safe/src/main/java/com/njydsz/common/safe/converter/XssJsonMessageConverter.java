 ...
 *
 * <p>继承 {@link JsonHttpMessageConverter}，在反序列化 JSON 请求体时对字符串值进行 XSS 过滤。 通过重写 {@link #readInternal}
 * 方法，在 YdszJson 反序列化前对原始 JSON 字符串进行清洗， 确保所有字符串类型的值都经过 XSS 过滤。
 *
 * <p><b>过滤规则：</b>
 *
 * <ul>
 *   <li>移除 {@code <script>} 标签及其内容
 *   <li>移除 {@code javascript:}、{@code vbscript:}、{@code data:} 等危险协议
 *   <li>移除 {@code on*} 事件属性（如 onclick、onload 等）
 *   <li>HTML 实体编码特殊字符：{@code < > " ' &}
 * </ul>
 *
 * <p><b>使用场景：</b> 当 {@code ydsz.safe.xss.mode=converter} 时，此转换器会替换默认的 JSON 转换器， 在反序列化阶段完成 XSS 过滤，与
...
public class XssJsonMessageConverter extends JsonHttpMessageConverter implements Ordered {
   ...
   * 读取并反序列化 JSON 请求体
   *
   * <p>重写父类方法，在反序列化前对 JSON 字符串值进行 XSS 过滤。 使用 {@link EscapeUtils#cleanJsonValue} 进行流式 JSON 解析和清洗，
  ...
  protected Object readInternal(Class<?> clazz, HttpInputMessage inputMessage)
    ...
    if (originalBytes == null || originalBytes.length == 0) {
      HttpInputMessage emptyInput =
          new XssByteArrayInputMessage(new byte[0], inputMessage.getHeaders());
    ...
    HttpInputMessage cleanedInput =
        new XssByteArrayInputMessage(cleanedBytes, inputMessage.getHeaders());
