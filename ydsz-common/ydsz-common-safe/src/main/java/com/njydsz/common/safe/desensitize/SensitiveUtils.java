 ...
 * 敏感数据脱敏工具类（字段级）。
 *
 * <p>提供各种类型的脱敏策略实现。手机号/身份证/邮箱脱敏规则与 原 {@code StringUtils}（ydsz-common-util）保持一致，本类提供更高层次的 {@link
 * SensitiveType} 枚举 API 以及姓名/地址/全掩码等场景。
 *
 * <p>与 {@link ColumnDesensitizationRule}（列级正则脱敏）互补： 本工具类面向字段级（编程式），列级规则面向数据库结果集。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
...
public final class SensitiveUtils {
   ...
   * 按指定脱敏类型对文本进行脱敏。
   *
   * <p>MOBILE / ID_CARD / EMAIL 由本类实现（规则与原 StringUtils 统一）， BANK_CARD / NAME / ADDRESS / MASK_ALL
   * 由本类实现。
   *
