 ...
 * 限流规则
 *
 * <p>定义一条完整的限流规则，是限流决策的核心配置。 支持 Sentinel 风格的多种限流模式（QPS、并发数、热点参数、集群限流等）。
 *
 * <p><b>使用示例：</b>
 *
 * <pre>{@code
...
public class RateLimitRule implements Serializable {
  ...
  /** 限流维度 */
  @Builder.Default private RateLimitDimension dimension = RateLimitDimension.API;

  /** 限流算法 */
  @Builder.Default private RateLimitAlgorithm algorithm = RateLimitAlgorithm.TOKEN_BUCKET;
  ...
  /** 令牌桶容量（允许的最大突发流量） */
  @Builder.Default private long burstCapacity = 200;

  /** 排队等待超时时间（仅 QUEUEING 模式） */
  @Builder.Default private Duration queueTimeout = Duration.ZERO;
  ...
  /** 关联的限流错误码（默认 RateLimitExceptionCode.RATE_LIMIT） */
   ...
   *
   * <ul>
   *   <li>{@code resource} 不能为空
   *   <li>{@code threshold} 必须大于 0
   *   <li>{@code window} 必须大于 0
   *   <li>{@code burstCapacity} 必须大于 0
   * </ul>
   *
