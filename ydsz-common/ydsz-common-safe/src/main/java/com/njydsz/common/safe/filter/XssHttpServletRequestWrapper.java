 ...
 * XSS 过滤请求包装器
 *
 * <p>对 HTTP 请求参数和 JSON 请求体进行 XSS 安全过滤。 继承 HttpServletRequestWrapper，拦截并清洗用户输入。
 *
 * <p><b>过滤内容：</b>
 *
 * <ul>
 *   <li>表单参数：通过 getParameterValues 获取并过滤
 *   <li>JSON 请求体：通过 getInputStream 获取并过滤
 * </ul>
 *
...
public class XssHttpServletRequestWrapper extends HttpServletRequestWrapper {
  ...
  public XssHttpServletRequestWrapper(
      HttpServletRequest request, XssFilter.CachedRequestBody cachedBody) {
    super(request);
    if (cachedBody != null) {
   ...
   * 获取或初始化请求体
   *
   * <p>懒加载模式，首次调用时读取并清洗请求体。 清洗使用 EscapeUtils.clean() 方法。
   *
   * <p>使用双重检查锁定替代 synchronized 方法，降低并发竞争。
   *
   * @return 清洗后的请求体字节数组
