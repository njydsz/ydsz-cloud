 ...
 *
 * <ul>
 *   <li>classpath 存在 {@link RateLimitManager}
 *   <li>{@code ydsz.safe.ratelimit.enabled=true}（默认 true）
 * </ul>
 *
...
public class RateLimitAutoConfiguration {
  ...
  public RateLimitRuleProvider rateLimitRuleProvider(RateLimitProperties properties) {
    log.info(
        "Initializing rate limit rule provider, configured rules={}",
  ...
  public ClusterRateLimiter clusterRateLimiter(
      RateLimitProperties properties, ObjectProvider<StringRedisTemplate> redisTemplateProvider) {
    StringRedisTemplate redisTemplate = redisTemplateProvider.getIfAvailable();
    return new RedisClusterRateLimiter(
        redisTemplate, properties.getClusterKeyPrefix(), properties.getFallbackOnError());
  }
  ...
  public RateLimitManager rateLimitManager(
    ...
    log.info(
        "Initializing rate limit manager, mode={}, clusterLimiter={}",
        properties.getDefaultMode(),
        clusterLimiter == null ? "null" : clusterLimiter.getClass().getSimpleName());
   ...
   * 注册限流指标采集器（可选）。
   *
   * <p>当 {@code ydsz.safe.ratelimit.metrics-enabled=false} 或 classpath 无 {@code MeterRegistry} 时返回
   * null， 即不采集指标，不影响限流主链路。注入后作为监听器挂到 {@link RateLimitManager}，采集 Counter/Timer。
   *
   ...
   *
   * <p>默认随限流模块启用（{@code ydsz.safe.ratelimit.aop-enabled=true}）。拦截标注 {@code @RateLimit} 的方法， 命中限流时抛
   * {@code RateLimitExceededException}。{@code @ConditionalOnMissingBean} 允许自定义覆盖。
   *
