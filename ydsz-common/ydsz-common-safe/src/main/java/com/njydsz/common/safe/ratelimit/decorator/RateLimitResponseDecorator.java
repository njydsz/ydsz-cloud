 ...
 * <p><b>标准化响应头：</b>
 * <ul>
 *   <li>{@code Retry-After} - 建议客户端等待的秒数（RFC 7231）</li>
 *   <li>{@code X-RateLimit-Limit} - 当前规则的总阈值</li>
 *   <li>{@code X-RateLimit-Remaining} - 剩余可用配额</li>
 *   <li>{@code X-RateLimit-Reset} - 配额重置时间（Unix 时间戳）</li>
 * </ul>
 *
 * <p><b>示例响应头：</b>
 * <pre>{@code
...
public class RateLimitResponseDecorator {
     ...
     */
    public void decorateBlockedResponse(HttpServletRequest request,
        ...
        try {
            // Retry-After：建议等待秒数
            long waitSeconds = decision.getWaitTimeMillis() > 0
            ...
            if (decision.getThreshold() > 0) {
                response.setHeader("X-RateLimit-Limit",
                        String.valueOf((int) decision.getThreshold()));
            }
        ...
        try {
            if (decision.getThreshold() > 0) {
                ...
                response.setHeader("X-RateLimit-Remaining",
                        String.valueOf(Math.max(0, decision.getRemaining())));
            }
        } catch (Exception e) {
