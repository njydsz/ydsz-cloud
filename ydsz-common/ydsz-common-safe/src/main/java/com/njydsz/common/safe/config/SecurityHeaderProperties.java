 ...
 *
 * <ul>
 *   <li>X-XSS-Protection：启用浏览器 XSS 过滤器
 *   <li>X-Content-Type-Options：防止 MIME 类型嗅探
 *   <li>X-Frame-Options：防止点击劫持攻击
 *   <li>Strict-Transport-Security：强制 HTTPS 连接
...
public class SecurityHeaderProperties {
   ...
   * 过滤器注册顺序
   *
   * <p>数值越小，优先级越高。 建议在 XssFilter 之前执行，以便尽早设置安全头部。
   */
  private int order = 1;
   ...
   * XSS 防护头部
   *
   * <p>启用浏览器的 XSS 过滤器。 推荐值："1; mode=block" - 发现 XSS 时阻止页面渲染 旧版值："0" - 禁用 XSS 过滤器（不推荐）
   */
   ...
   * 内容类型选项头部
   *
   * <p>防止浏览器 MIME 类型嗅探。 推荐值："nosniff" - 不猜测内容类型
   */
   ...
   *
   * <ul>
   *   <li>"no-referrer"：不发送 Referer
   *   <li>"same-origin"：仅同源请求发送 Referer
   *   <li>"strict-origin-when-cross-origin"：跨域时仅发送协议+主机
   * </ul>
   */
   ...
   * 权限策略头部
   *
   * <p>控制浏览器功能 API 的使用权限。 例如：geolocation、microphone、camera 等
   */
