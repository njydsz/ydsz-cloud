 ...
 *
 * <p>实现 YdszJson {@link JsonSerializer}，在序列化 JSON 时自动对 标注了 {@link SensitiveData} 注解的字段进行脱敏处理。
 *
 * <p><b>使用方式：</b>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><b>注册方式：</b>通过 {@link com.njydsz.common.safe.xss.SafeJsonModule}（实现 {@link
 * com.njydsz.common.json.module.JsonModule.SpringFactory}）自动注册到 YdszJson 引擎， 禁止在业务代码中散落调用 {@code
 * YdszJson.register()}。
 *
 * <p><b>注意事项：</b>
 *
 * <ul>
 *   <li>仅对 String 类型字段生效
 *   <li>如果字段值为 null，保持 null 不处理
 *   <li>脱敏失败时保留原始值，不影响其他字段
 *   <li>配合 {@link SensitiveDataProcessor} 使用可实现更全面的脱敏
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
