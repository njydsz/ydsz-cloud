 ...
 *
 * <p>业务模块（注册/修改密码）应直接使用此类校验密码强度，避免各自实现正则规则。
 * 内部委托给 {@link PwdUtils#checkPasswordStrengthLevel(String)}，
 * 支持通过 {@code META-INF/services} 注册自定义校验器覆盖默认策略。
 *
 * <h3>使用示例</h3>
 * <pre>{@code
 * // 注入
 ...
 *
 * <h3>对比业务模块自实现</h3>
 * <p>历史版本中，userinfo 模块的 {@code PasswordPolicyValidator} 自写了正则规则
 * （如 {@code HAS_LOWER}、{@code HAS_UPPER}），与 common-util 的 {@link PasswordStrengthChecker}
 * 存在策略不一致风险。现统一委托后：
 * <ul>
 *   <li>common-util 的弱密码字典、连续字符、重复字符等规则由 common-util 统一维护</li>
 *   <li>业务模块仅保留业务特有逻辑（如密码历史校验、用户名包含检查）</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.1.0
...
public class PasswordStrengthValidator {
     ...
     * 默认最低密码强度要求。
     *
     * <p>MEDIUM（中等）为大多数业务场景的推荐阈值，
     * 金融/高安全场景可通过构造时配置提升为 STRONG。
     */
     ...
     * 校验密码强度是否达到最低要求。
     *
     * <p>校验失败时抛出 {@link BusinessException}，
     * 错误码为 {@link SecurityExceptionCode#PASSWORD_TOO_WEAK}（C01071）。
     *
     * @param password 明文密码
    ...
    public void validate(String password, PasswordStrengthChecker.PasswordStrengthLevel customLevel) {
        ...
        if (actualLevel.ordinal() < customLevel.ordinal()) {
            ...
            log.debug("[PasswordStrengthValidator] 密码强度不足: actual={}, required={}, suggestion={}",
                    actualLevel, customLevel, suggestion);
     ...
     * 构建错误消息。
     */
    private String buildErrorMessage(PasswordStrengthChecker.PasswordStrengthLevel actual,
                                      PasswordStrengthChecker.PasswordStrengthLevel required,
                                      String suggestion) {
        StringBuilder sb = new StringBuilder("密码强度不足（当前：").append(actual).append("，要求：≥").append(required).append("）");
