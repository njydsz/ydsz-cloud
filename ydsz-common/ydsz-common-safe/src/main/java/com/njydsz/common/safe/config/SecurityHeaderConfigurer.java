 ...
 *
 * <p>将安全响应头的策略计算逻辑（CSP 构建、HSTS 拼接、高级头策略）集中在此类，
 * 消除 Gateway 模块约 80 行的安全头注入代码与 common-safe 模块的 {@link SecurityHeaderFilter}
 * 之间的重复。
 *
 * <p><b>解决的核心问题：</b>
 * <ul>
 *   <li>历史版本中，Gateway（WebFlux）与业务服务（Servlet）各自实现安全头注入，
 *       CSP/COOP/COEP/CORP 等高级头仅在 Gateway 层注入，业务服务侧无兜底</li>
 *   <li>安全头策略分散维护，配置变更需同步两处</li>
 * </ul>
 *
 * <p><b>使用方式：</b>
 * <pre>{@code
...
public final class SecurityHeaderConfigurer {
     ...
     *
     * <p>返回 Map 而非直接写入响应头，以便不同技术栈（Servlet/WebFlux）
     * 用各自的方式写入。
     *
     * <h4>向后兼容说明</h4>
     ...
     * 构建 CSP 策略字符串。
     *
     * <p>默认策略限制脚本/样式/图片/连接来源，防止 XSS 注入。
     * 可通过 {@code ydsz.safe.security-headers.csp.*} 配置项完整定制。
     *
     * <h4>向后兼容</h4>
    ...
    public static String buildHstsHeader(SecurityHeaderProperties properties) {
        if (properties.getHsts() != null && properties.getHsts().isEnabled()) {
            StringBuilder hstsValue = new StringBuilder()
                    .append("max-age=").append(properties.getHsts().getMaxAge());
