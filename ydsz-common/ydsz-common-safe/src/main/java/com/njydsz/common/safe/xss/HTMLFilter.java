...
public final class HTMLFilter {
  ...
  public HTMLFilter() {
    ...
    vSelfClosingTags =
        new HashSet<>(
            Arrays.asList("img", "br", "hr", "input", "meta", "link", "area", "base", "col"));
    vNeedClosingTags =
        new HashSet<>(
            Arrays.asList(
                "a",
    ...
    vProtocolAtts =
        new HashSet<>(
            Arrays.asList("src", "href", "action", "formaction", "data", "poster", "background"));
    vRemoveBlanks =
        new HashSet<>(
            Arrays.asList(
                "a", "b", "strong", "i", "em", "span", "p", "div", "h1", "h2", "h3", "h4", "h5",
                "h6"));
    vAllowedEntities =
        new HashSet<>(
            Arrays.asList(
                "amp", "gt", "lt", "quot", "nbsp", "copy", "reg", "trade", "mdash", "ndash",
  ...
  public HTMLFilter(final Map<String, Object> conf) {
    ...
    vSelfClosingTags =
        new HashSet<>(Arrays.asList(castToStringArray(conf.get("vSelfClosingTags"))));
    vNeedClosingTags =
        new HashSet<>(Arrays.asList(castToStringArray(conf.get("vNeedClosingTags"))));
    vDisallowed = new HashSet<>(Arrays.asList(castToStringArray(conf.get("vDisallowed"))));
    vAllowedProtocols =
        new HashSet<>(Arrays.asList(castToStringArray(conf.get("vAllowedProtocols"))));
    ...
    vAllowedEntities =
        new HashSet<>(Arrays.asList(castToStringArray(conf.get("vAllowedEntities"))));
   ...
   * HTML 过滤器配置构建器。
   *
   * <p>通过链式方法配置标签/属性白名单、禁止标签、允许协议、去空白标签等策略， 最后调用 {@code build()} 生成对应的 {@link HTMLFilter}
   * 实例；标签名匹配均不区分大小写。
   */
  public static class Builder {
    ...
    public static Builder relaxed() {
      ...
      builder.allowTagWithAttributes(
      ...
      builder.allowEntities(
          "amp", "gt", "lt", "quot", "nbsp", "copy", "reg", "trade", "mdash", "ndash", "hellip");
    ...
    public static Builder strict() {
      Builder builder = new Builder();
      builder.allowTags(
          "b", "strong", "i", "em", "u", "s", "strike", "del", "ins", "sub", "sup", "span", "p",
          "br", "hr", "ul", "ol", "li");
