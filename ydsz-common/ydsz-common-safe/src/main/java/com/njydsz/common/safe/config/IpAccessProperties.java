 ...
 *
 * <ul>
 *   <li>BLACKLIST（黑名单模式）：黑名单中的 IP 被拒绝，其他 IP 放行
 *   <li>WHITELIST（白名单模式）：白名单中的 IP 放行，其他 IP 被拒绝
 * </ul>
 *
 * <p><b>配置示例：</b>
 *
...
public class IpAccessProperties {
  ...
  private List<String> staticWhitelist = new ArrayList<>(4);

  /** 排除路径列表（支持 Ant 风格路径匹配，这些路径跳过 IP 检查） */
  ...
  public enum AccessMode {
    /** 黑名单模式：黑名单中的 IP 被拒绝，其他 IP 放行 */
    BLACKLIST,
    /** 白名单模式：白名单中的 IP 放行，其他 IP 被拒绝 */
    WHITELIST
  }
}
