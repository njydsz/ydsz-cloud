 ...
 *
 * <p><b>P2-6 新增</b>：增加 {@code timeoutMs} 属性，支持步骤级超时设置。 当正向操作执行超过指定时间未完成时，编排器将中断该步骤并触发补偿。
 *
 * <p>使用示例：
 *
 * <pre>{@code
...
public class SagaStep<T> {
  ...
  public static <T> SagaStep<T> of(
      String name, Callable<T> forwardAction, Runnable compensation, long timeoutMs) {
