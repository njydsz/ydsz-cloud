 ...
 *
 * <p>使用 HMAC-SHA256 算法对 XID + 时间戳进行签名，格式为： {@code base64(xid):base64(timestamp):base64(signature)}
 *
 * <p>验证时重新计算签名并比对，同时检查时间戳是否在有效窗口内（默认 5 分钟）， 防止重放攻击。
 *
...
public class HmacXidSigner implements XidSigner {
  ...
  public String sign(String xid) {
    ...
    try {
      ...
      return Base64.getEncoder().encodeToString(xid.getBytes(StandardCharsets.UTF_8))
          + SEPARATOR
          + Base64.getEncoder().encodeToString(timestamp.getBytes(StandardCharsets.UTF_8))
          + SEPARATOR
   ...
   *
   * <ol>
   *   <li>解析格式，提取 xid/timestamp/signature
   *   <li>验证签名有效性
   *   <li>验证时间戳在有效窗口内
   *   <li>验证通过返回原始 XID
   * </ol>
  ...
  private String hmacSha256(String data) {
    try {
      ...
      SecretKeySpec secretKeySpec =
          new SecretKeySpec(signKey.getBytes(StandardCharsets.UTF_8), HMAC_SHA256_ALGORITHM);
