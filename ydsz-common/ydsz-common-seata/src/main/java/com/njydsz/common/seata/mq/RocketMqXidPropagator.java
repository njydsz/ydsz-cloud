 ...
 * RocketMQ XID 传播器实现
 *
 * <p>实现 {@link MqXidPropagator} 接口，提供 RocketMQ 消息的 XID 透明传播。 发送消息时自动注入 XID 到消息头，确保下游消费者能通过 XID
 * 恢复事务上下文。
 *
 * @author ydsz-team
 * @since 1.4.0
...
public class RocketMqXidPropagator implements MqXidPropagator {
  ...
  public SendResult send(
      String topic, String tag, String keys, byte[] body, Map<String, String> properties) {
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "[SeataMQ-RocketMQ] Sent message: topic={}, tag={}, keys={}, msgId={}, xid={}",
            topic,
            tag,
            keys,
            result.getMsgId(),
      ...
      throw new MessagingException(
          "Failed to send Seata-aware RocketMQ message to topic: " + topic, e);
    }
  }
  ...
  private Message buildMessage(
      String topic, String tag, String keys, byte[] body, Map<String, String> extraProperties) {
