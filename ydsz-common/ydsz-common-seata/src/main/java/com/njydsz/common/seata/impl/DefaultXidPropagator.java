 ...
 * 默认 XID 传播器实现
 *
 * <p>使用独立的 {@link XidContextHolder} 存储 XID，支持 HTTP Header 和 MQ 属性的序列化/反序列化。 通过 SPI 注入 {@link
 * XidSigner} 实现签名校验，防止 XID 伪造注入。
 *
 * <p><b>P0-4 修复</b>：集成签名校验机制，当配置签名密钥时对 XID 进行 HMAC-SHA256 签名， 下游服务验证签名有效后才绑定到上下文，防止恶意 XID 注入。
 *
 * <p><b>P2-3 修复</b>：不再委托 {@code AbstractTransactionManager} 包级私有方法， 改为直接依赖 {@link
 * XidContextHolder}，消除循环依赖。
 *
 * <h3>XID 传播流程：</h3>
 *
 * <ol>
 *   <li>上游服务：{@code currentXid()} 获取 XID → {@code serialize(xid)} 序列化（含签名）
 *   <li>HTTP Header：{@code X-XID} 或事务上下文特定 Header 传输
 *   <li>下游服务：{@code deserialize(header)} 反序列化（校验签名）→ {@code bind(xid)} 绑定到线程
 * </ol>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class DefaultXidPropagator implements XidPropagator {
   ...
   * 将 XID 序列化为传输格式
   *
   * <p>当签名器可用时，对 XID 进行签名后再序列化，格式为： {@code base64(xid):base64(timestamp):base64(signature)}。
   * 无签名器时直接返回原值（向后兼容）。
   *
   * @param xid 全局事务 ID
   * @return 序列化后的字符串
   ...
   * 从传输格式反序列化 XID
   *
   * <p>当签名器可用时，验证签名有效性后才提取 XID。 签名无效返回 null，上层应拒绝绑定。 无签名器时直接返回原值（向后兼容）。
   *
   * @param header 传输内容
