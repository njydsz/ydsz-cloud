 ...
 *
 * <ul>
 *   <li>当前事务模式（LOCAL/TCC/SEATA_AT/SAGA）
 *   <li>Seata TC 连通性（当 SeataTransactionManager 可用时）
 *   <li>TCC 挂起事务数（TRIED 状态超时未完成）
 *   <li>恢复扫描配置
 * </ul>
 *
 * <p><b>P0-2 修复</b>：使用 {@link TccTransactionLogStore#countTimeoutPending} 接口， 避免全量查询挂起事务导致健康检查响应超时。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public class SeataHealthIndicator implements HealthIndicator {
   ...
   * 检查 Seata TC 连通性
   *
   * <p>通过检测 Seata RootContext 是否正常初始化来判断连通性， 无需发起远程调用，避免健康检查产生外部依赖。
   */
   ...
   * 检查 TCC 挂起事务数量
   *
   * <p>使用 {@link TccTransactionLogStore#countTimeoutPending} 高效获取计数， 避免全量查询挂起事务列表带来的内存和性能开销。
   */
  private void checkTccPendingTransactions(Health.Builder builder) {
    ...
    LocalDateTime threshold =
        LocalDateTime.now().minusNanos(properties.getRecoveryTimeoutThresholdMs() * 1_000_000);
