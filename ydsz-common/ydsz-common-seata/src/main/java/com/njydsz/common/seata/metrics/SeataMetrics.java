 ...
 *
 * <ul>
 *   <li>{@code seata.tx.count{type,result}} - 事务执行计数
 *   <li>{@code seata.tx.duration{type}} - 事务执行耗时 Timer（P50/P90/P99）
 *   <li>{@code seata.tcc.confirm.retry} - TCC Confirm 重试次数
 *   <li>{@code seata.tcc.cancel.retry} - TCC Cancel 重试次数
 *   <li>{@code seata.saga.compensation.count} - SAGA 补偿次数
 *   <li>{@code seata.tx.active} - 活跃事务数 Gauge
 * </ul>
 *
 * <p><b>降级策略</b>：当 MeterRegistry 不可用时，内部仍维护计数器和活跃事务数， 并通过 {@link #getActiveTxCount()} 暴露。一旦
 * Registry 可用（如动态加载）， 后续指标上报自动恢复。Gauge 在首次 Registry 可用时注册。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public class SeataMetrics {
  ...
  public void recordConfirmRetry() {
    MeterRegistry registry = registryProvider.getIfAvailable();
    if (registry != null) {
      Counter.builder("seata.tcc.confirm.retry").register(registry).increment();
    }
  }
  ...
  public void recordCancelRetry() {
    MeterRegistry registry = registryProvider.getIfAvailable();
    if (registry != null) {
      Counter.builder("seata.tcc.cancel.retry").register(registry).increment();
    }
  }
  ...
  public void recordSagaCompensation() {
    MeterRegistry registry = registryProvider.getIfAvailable();
    if (registry != null) {
      Counter.builder("seata.saga.compensation.count").register(registry).increment();
    }
  }
   ...
   * 获取当前活跃事务数
   *
   * <p>无论 MeterRegistry 是否可用，此方法始终返回当前活跃事务数量。 当 Registry 可用时，该值还会通过 {@code seata.tx.active} Gauge
   * 自动上报。
   *
   * @return 活跃事务数
   */
   ...
   *
   * <p>采用懒注册策略：首次事务开始时检查 Registry 可用性并注册 Gauge。 如果 Registry 后续才可用，下次事务开始时自动注册。
   */
