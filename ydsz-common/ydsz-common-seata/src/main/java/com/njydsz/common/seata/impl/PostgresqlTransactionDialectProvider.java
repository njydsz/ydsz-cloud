 ...
 *
 * <p>使用 PostgreSQL 专有的 {@code INSERT ... ON CONFLICT (columns) DO UPDATE SET} 语法实现 UPSERT， 兼容
...
public class PostgresqlTransactionDialectProvider implements TccTransactionDialectProvider {
   ...
   *
   * <p>使用 {@code INSERT ... ON CONFLICT (xid, branch_id) DO UPDATE SET} 语法， 需保证表上有 UNIQUE (xid,
   * branch_id) 约束。
   */
  @Override
  ...
  public String getCountTimeoutPendingSql() {
    return String.format(
        "SELECT COUNT(*) FROM %s WHERE status = ? AND try_completed_at < ?", tableName);
  }

  /** 获取删除事务日志的 SQL */
  @Override
  public String getDeleteSql() {
    return String.format("DELETE FROM %s WHERE xid = ? AND branch_id = ?", tableName);
  }
}
