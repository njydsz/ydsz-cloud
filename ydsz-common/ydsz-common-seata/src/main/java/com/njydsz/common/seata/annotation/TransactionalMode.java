 ...
 *
 * <p><b>P1-6 新增</b>：解决不同业务场景需要不同事务模式时的动态切换问题。
 * 通过 AOP 拦截 {@code @TransactionalMode} 注解的方法，自动路由到对应的事务类型。
 *
 * <p>使用方式：
 * <pre>{@code
 * &#64;Service
...
public @interface TransactionalMode {
     ...
     *
     * <ul>
     *   <li>{@link TransactionType#LOCAL} - 本地事务（默认）</li>
     *   <li>{@link TransactionType#TCC} - TCC 分布式事务</li>
     *   <li>{@link TransactionType#SEATA_AT} - Seata AT 自动补偿事务</li>
     *   <li>{@link TransactionType#SAGA} - SAGA 长事务</li>
     * </ul>
     */
