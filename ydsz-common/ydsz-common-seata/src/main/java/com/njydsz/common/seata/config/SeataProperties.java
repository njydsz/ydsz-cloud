 ...
 * 分布式事务配置属性
 *
 * <p><b>P2-2 修复</b>：增加 JSR-303 参数校验注解，在应用启动时即验证配置合法性， 避免运行时才发现配置错误导致事务异常。
 *
 * <p>校验规则：
 *
 * <ul>
 *   <li>{@code tcc-retry-count} / {@code saga-max-retries} ∈ [0, 10]
 *   <li>{@code tcc-retry-interval-ms} ≥ 100（避免重试过于频繁）
 *   <li>{@code recovery-batch-size} ∈ [1, 1000]
 *   <li>{@code recovery-timeout-threshold-ms} ≥ 5000（避免误扫描刚创建的事务）
 *   <li>{@code xid-sign-enabled=true} 时 {@code xid-sign-key} 不能为空且长度 ≥ 16
 * </ul>
 *
...
public class SeataProperties {
   ...
   *
   * <ul>
   *   <li>{@code memory} - 内存实现（默认，单机/测试）
   *   <li>{@code redis} - Redis 实现（生产环境，跨服务共享）
   *   <li>{@code db} - 数据库实现（生产环境，强持久化，无需 Redis）
   * </ul>
   */
   ...
   *
   * <ul>
   *   <li>{@code mysql} - MySQL / MariaDB（使用 ON DUPLICATE KEY UPDATE）
   *   <li>{@code postgresql} - PostgreSQL 9.5+（使用 ON CONFLICT DO UPDATE）
   * </ul>
   *
   ...
   * XID 签名开关（P0-4 新增）
   *
   * <p>开启后，XID 跨服务传播时携带 HMAC-SHA256 签名， 下游服务验证签名有效后才绑定到上下文，防止 XID 伪造注入。
   *
   ...
   *
   * <p>建议使用 32 字节以上的随机字符串，通过环境变量或配置中心注入， 确保所有参与服务的密钥一致。
   */
   ...
   *
   * <p>启用后每次扫描仅处理 {@link #recoveryBatchSize} 条记录， 下次扫描从上次结束位置继续，渐进式处理所有超时事务。 避免一次性加载全部超时事务到内存。
   */
   ...
   * 按事务名称配置的超时覆盖（可选）
   *
   * <p>key 为事务名称（与 XID 前缀匹配），value 为超时时间毫秒。 例如：
   *
   * <pre>{@code
   ...
   * TCC 事务日志存储类型。
   *
   * <p>决定 TCC 一阶段确认信息持久化在哪里，直接影响分布式事务的恢复能力： 生产环境跨服务共享时必须选择 {@link #REDIS} 或 {@link
   * #DB}，否则事务恢复扫描无法跨实例工作。
   */
