 ...
 *
 * <p>基于 Seata 2.x 原生 API 实现自动补偿型分布式事务。 当 Seata 在类路径时由 {@link
 * com.njydsz.common.seata.config.SeataAutoConfiguration.SeataAtConfiguration} 条件注册。
 *
 * <p>设计说明：本管理器作为 ydsz-common-seata 框架与 Seata 原生 API 之间的适配器， 提供统一的 {@link
 * com.njydsz.common.seata.api.DistributedTransactionManager} 接口。 <b>业务代码必须通过 {@code
 * DistributedTransactionManager} 统一接口使用</b>， 禁止直接 import {@code org.apache.seata.*} 原生
 * API（参见《云顶编码规范》分布式事务章节）。
 *
 * <p>职责：
 *
 * <ul>
 *   <li>启动/提交/回滚全局事务（委托 Seata 原生 API）
 *   <li>管理 XID 上下文（与 Seata RootContext 协同）
 *   <li>提供可观测性支持（指标、审计日志）
 * </ul>
 *
 * @author ydsz-team
...
public class SeataTransactionManager extends AbstractTransactionManager {
   ...
   * 执行 Seata AT 分布式事务
   *
   * <p>通过 Seata {@link GlobalTransactionContext} 操作全局事务生命周期。 业务操作成功时提交全局事务，失败时回滚。
   *
   ...
   *
   * <p>当 Seata AT 全局事务回滚后，执行业务层提供的补偿动作。 注意：Seata AT 模式下补偿通常不需要（undo_log 已自动回滚），此方法主要
