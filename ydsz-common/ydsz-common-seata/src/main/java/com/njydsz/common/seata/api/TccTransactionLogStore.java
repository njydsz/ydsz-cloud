 ...
 *
 * <ul>
 *   <li><b>空回滚</b>：Cancel 前检查分支状态，若 Try 未执行则跳过
 *   <li><b>悬挂</b>：Try 前检查分支状态，若已有 Cancel 则跳过
 *   <li><b>幂等</b>：Confirm/Cancel 前检查是否已为终态
 * </ul>
 *
 * <p>提供三种实现：
 *
 * <ul>
 *   <li>{@code InMemoryTccTransactionLogStore} - 内存实现，适用于单机/测试
 *   <li>{@code RedisTccTransactionLogStore} - Redis 实现，适用于生产环境（跨服务共享）
 *   <li>{@code DbTccTransactionLogStore} - 数据库实现，适用于生产环境（强持久化）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public interface TccTransactionLogStore {
   ...
   *
   * <p>相比 {@link #findTimeoutPending}，此方法仅返回指定数量的超时事务记录， 适用于恢复扫描场景下的分批处理，避免一次性加载过多事务到内存。
   *
   * @param threshold 超时阈值，早于此时间的 TRIED 状态分支需要恢复
   ...
   *
   * <p>相比 {@link #findTimeoutPending}，此方法仅返回计数而不加载完整事务日志， 适用于健康检查、监控等仅需计数的场景，避免全量加载带来的内存和性能开销。
   *
   * @param threshold 超时阈值，早于此时间的 TRIED 状态分支需要恢复
   * @return 超时未完成的分支事务数量
   */
