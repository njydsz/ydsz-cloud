 ...
 *   <li>事务类型</li>
 *   <li>XID / branchId</li>
 *   <li>traceId（P1-7 新增：自动从 MDC 或 X-Trace-Id 提取）</li>
 ...
 *
 * <p><b>P1-7 新增</b>：traceId 自动注入，支持与分布式链路系统（如 MDD/SkyWalking）关联。
 * 注入优先级：
 * <ol>
 *   <li>SLF4J MDC 中的 {@code traceId} 或 {@code trace_id}</li>
 *   <li>HTTP 请求头 {@code X-Trace-Id}（需前置拦截器写入 MDC）</li>
 * </ol>
 *
 * <h3>MDC 配置建议</h3>
...
public class TransactionAuditLogger {
     ...
     * 解析当前线程的链路追踪 ID
     *
     * <p>依次从 MDC 中查找 {@code traceId}、{@code trace_id}、{@code X-Trace-Id}，
     * 找到任一非空值即返回；全部无值时返回 null。
     *
