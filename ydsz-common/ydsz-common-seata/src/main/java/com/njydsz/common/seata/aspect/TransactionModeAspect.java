 ...
 * 事务模式切面
 *
 * <p>拦截 {@link TransactionalMode} 注解声明的方法，在方法执行前自动切换 到对应的事务类型，并在方法结束后清除上下文。
 *
 ...
 *
 * <ul>
 *   <li>基于 {@link XidContextHolder} 的 TransmittableThreadLocal 标记事务上下文
 *   <li>事务执行器通过 {@link XidContextHolder#getRequiredType()} 获取当前声明类型
 *   <li>提供方法级别事务声明不影响不相关的其它调用
 * </ul>
 *
 * @author ydsz-team
 * @since 1.3.0
...
public class TransactionModeAspect implements Ordered {
   ...
   */
  @Around(
      value = "execution(* *(..)) && (@annotation(annotation) || @within(annotation))",
      argNames = "joinPoint,annotation")
  public Object around(ProceedingJoinPoint joinPoint, TransactionalMode annotation)
      throws Throwable {
