 ...
 *
 * <ul>
 *   <li><b>悬挂保护</b>：Try 前检查状态，若已 CANCELLED 则跳过 Try
 *   <li><b>空回滚保护</b>：Cancel 前检查状态，若为 INIT/TRYING 则跳过 Cancel
 *   <li><b>幂等保护</b>：Confirm/Cancel 前检查是否已为终态，是则跳过
 * </ul>
 *
 * <p><b>P0-12 修复</b>：Confirm/Cancel 失败时按配置重试 ({@code ydsz.seata.tcc-retry-count} / {@code
 * ydsz.seata.tcc-retry-interval-ms})。
 *
 * <p>注意：此实现为本地 TCC 协调器，适用于单服务内的多资源操作。 跨服务的 TCC 需要配合 Seata TCC 模式使用。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
public class TccTransactionManager extends AbstractTransactionManager
   ...
   * 设置异步执行器（P1-3 新增）
   *
   * <p>用于异步 Confirm 模式，执行 Confirm 操作的后台线程池。 建议注入专用的业务线程池，避免与 RPC 线程池竞争。
   *
  ...
  public <T> T executeWithCompensation(
    ...
    try {
      ...
      LOG.error(
          "TCC+SAGA transaction failed, executing compensation: name={}, xid={}",
          transactionName,
          xid,
          e);
   ...
   * 执行 TCC 事务
   *
   * <p>完整执行 Try → Confirm 流程，Try 或 Confirm 失败时执行 Cancel。 Try 前进行悬挂检查，Confirm/Cancel 前进行幂等检查。
   *
   * @param transactionName 事务名称
  ...
  public <T> T executeTcc(String transactionName, String actionBeanName, TccAction<T> tccAction)
    ...
    try {
      if (logStore != null && isSuspended(xid, branchId)) {
        LOG.warn(
            "TCC Try skipped (suspension): already cancelled: xid={}, branch={}", xid, branchId);
        endXid();
        return null;
      }
   ...
   * 异步执行 TCC 事务
   *
   * <p>Try 阶段同步执行，完成后立即返回。Confirm 阶段异步执行， 通过回调通知执行结果。
   *
   * <p>回调签名：{@code BiConsumer<TccContext, Throwable>} - 成功时：{@code callback.accept(context, null)}
  ...
  public <T> CompletableFuture<T> executeTccAsync(
    ...
    try {
      if (logStore != null && isSuspended(xid, branchId)) {
        LOG.warn(
            "TCC Async Try skipped (suspension): already cancelled: xid={}, branch={}",
            xid,
            branchId);
        endXid();
    ...
    LOG.info(
        "TCC Async Confirm phase scheduled: name={}, xid={}, branch={}, executor={}",
        transactionName,
        xid,
        branchId,
    ...
    CompletableFuture<T> future =
        CompletableFuture.supplyAsync(
            () ->
                doAsyncConfirm(
                    transactionName, xid, branchId, txLog, tccAction, context, finalResult),
            executor);
  ...
  private <T> T doAsyncConfirm(
    ...
    try {
      ...
      LOG.error(
          "TCC Async Confirm failed, executing Cancel: name={}, xid={}", transactionName, xid, e);
      try {
        executeCancelWithRetry(transactionName, xid, branchId, txLog, tccAction, context);
  ...
  private <T> void notifyCallback(
      BiConsumer<TccContext, Throwable> callback, TccContext context, Throwable error) {
    if (callback == null) {
      return;
    }
    try {
  ...
  private <T> void executeCancelWithGuard(
      String transactionName,
      String xid,
      String branchId,
      TccTransactionLog txLog,
    ...
    if (logStore != null) {
      ...
      if (existing.isPresent()) {
        ...
        if (currentStatus.isFinal()) {
          LOG.info(
              "TCC Cancel skipped (idempotent): already final: xid={}, branch={}, status={}",
              xid,
              branchId,
              currentStatus);
          return;
        ...
        if (currentStatus == TccBranchStatus.INIT || currentStatus == TccBranchStatus.TRYING) {
          LOG.warn(
              "TCC Cancel skipped (empty rollback): Try not completed: xid={}, branch={}, status={}",
              xid,
              branchId,
              currentStatus);
  ...
  /** 执行 Confirm（带幂等检查 + 重试） */
  private <T> void executeConfirmWithRetry(
      String transactionName,
      String xid,
      String branchId,
      TccTransactionLog txLog,
    ...
    if (logStore != null) {
      ...
      if (existing.isPresent() && existing.get().getStatus() == TccBranchStatus.CONFIRMED) {
        LOG.info(
            "TCC Confirm skipped (idempotent): already confirmed: xid={}, branch={}",
            xid,
            branchId);
        return;
      }
    ...
    Exception lastException =
        new IllegalStateException("TCC Confirm exhausted retries without a specific exception");
  ...
  /** 执行 Cancel（带幂等检查 + 重试） */
  private <T> void executeCancelWithRetry(
      String transactionName,
      String xid,
      String branchId,
      TccTransactionLog txLog,
    ...
    if (logStore != null) {
      ...
      if (existing.isPresent()) {
        ...
        if (currentStatus == TccBranchStatus.CANCELLED) {
          LOG.info(
              "TCC Cancel skipped (idempotent): already cancelled: xid={}, branch={}",
              xid,
              branchId);
          return;
        }
    ...
    Exception lastException =
        new IllegalStateException("TCC Cancel exhausted retries without a specific exception");
    for (int attempt = 0; attempt <= maxRetries; attempt++) {
      try {
  ...
  public void recoverCancel(TccTransactionLog txLog) throws Exception {
    ...
    if (action != null) {
      ...
      LOG.warn(
          "TCC recovery Cancel skipped: no TccAction found for xid={}, beanName={}",
          txLog.getXid(),
          txLog.getActionBeanName());
    }
    if (logStore != null) {
  ...
  private <T> void runTccCancelQuiet(
      String transactionName, String xid, TccAction<T> tccAction, TccContext context) {
    try {
      tccAction.cancelAction(context);
