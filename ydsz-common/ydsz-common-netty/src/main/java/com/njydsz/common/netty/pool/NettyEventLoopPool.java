 ...
 *
 * <ul>
 *   <li><b>共享模式</b>（默认）：所有 Server/Client 复用同一个 boss/worker EventLoopGroup， 通过引用计数管理生命周期，减少线程上下文切换开销
 *   <li><b>隔离模式</b>：每个 Server/Client 创建独立的 EventLoopGroup， 避免高吞吐服务饿死低频服务
 * </ul>
 *
 * <p>使用自定义线程命名（{@code ydsz-netty-boss-N} / {@code ydsz-netty-worker-N}）， 便于在线程 dump 中定位问题。
 *
...
public class NettyEventLoopPool {
   ...
   */
  public NettyEventLoopPool(
      long shutdownQuietPeriodSeconds, long shutdownTimeoutSeconds, TransportType transportType) {
  ...
  public synchronized EventLoopGroup acquireWorkerGroup(int threads) {
    if (workerGroup == null) {
      ...
      workerGroup =
          NativeTransportDetector.createEventLoopGroup(transportType, n, "ydsz-netty-worker");
  ...
  public EventLoopGroup createIsolatedBossGroup(int threads) {
    ...
    EventLoopGroup group =
        NativeTransportDetector.createEventLoopGroup(transportType, n, "ydsz-netty-boss-iso");
  ...
  public EventLoopGroup createIsolatedWorkerGroup(int threads) {
    int n = threads <= 0 ? Runtime.getRuntime().availableProcessors() * 2 : threads;
    EventLoopGroup group =
        NativeTransportDetector.createEventLoopGroup(transportType, n, "ydsz-netty-worker-iso");
  ...
  public synchronized void releaseBossGroup() {
    if (bossRefCount.decrementAndGet() <= 0 && bossGroup != null) {
      try {
        bossGroup
            .shutdownGracefully(
                shutdownQuietPeriodSeconds, shutdownTimeoutSeconds, TimeUnit.SECONDS)
            .await();
