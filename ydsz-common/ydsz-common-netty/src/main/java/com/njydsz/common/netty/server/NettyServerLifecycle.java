 ...
 *
 * <p>实现 {@link SmartLifecycle}，随 Spring 容器启动自动启动所有注册的 {@link AbstractNettyServer}， 容器关闭时优雅停止（释放
 * EventLoopGroup）。
 *
 * <p>自动扫描 Spring 容器中所有 {@link AbstractNettyServer} 子类 Bean， 无需手动调用 {@code start()} / {@code
 * stop()}。
 *
 * <p>支持 fail-fast 模式：当 {@code failFast=true} 时，任一 Server 启动失败 将抛出异常终止 Spring 容器启动；{@code
