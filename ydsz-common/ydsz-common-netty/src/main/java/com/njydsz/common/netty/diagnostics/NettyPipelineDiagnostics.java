 ...
 * <ul>
 *   <li>输出 Pipeline 中所有 Handler 名称及顺序</li>
 *   <li>检测 Handler 是否存在</li>
 *   <li>获取 Channel 基础信息（本地/远端地址、是否活跃、是否可写）</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class NettyPipelineDiagnostics {
    ...
    public static List<String> dumpPipeline(Channel channel) {
        ...
        pipeline.names().forEach(name ->
                handlers.add(name + "(" + pipeline.get(name).getClass().getSimpleName() + ")"));
        return handlers;
    }
    ...
    public static String getChannelInfo(Channel channel) {
        ...
        return String.format("Channel{id=%s, local=%s, remote=%s, active=%s, writable=%s, open=%s}",
