...
public class IndexConsistencyChecker {
  ...
  public ConsistencyReport check(String tenantId) {
    ...
    for (SearchProvider<?> provider : providers) {
      ...
      try {
        ...
        if (dbCount > indexCount) {
          ...
          log.warn(
              "[IndexConsistency] 索引丢失: type={}, db={}, index={}, missing={}",
              type,
              dbCount,
              indexCount,
              dbCount - indexCount);
          ...
          log.warn(
              "[IndexConsistency] 索引冗余: type={}, db={}, index={}, orphan={}",
              type,
              dbCount,
              indexCount,
              indexCount - dbCount);
    ...
    if (!missingFromIndex.isEmpty() || !orphanInIndex.isEmpty()) {
      log.warn(
          "[IndexConsistency] 巡检完成，发现不一致: missing={}, orphan={}", missingFromIndex, orphanInIndex);
    } else {
    ...
    for (Map.Entry<String, Long> entry : report.orphanInIndex().entrySet()) {
      ...
      try {
        ...
        if (rawProvider != null) {
          SearchProvider<Object> provider = ProviderTypeBridge.cast(rawProvider);
          Set<String> dbIdSet =
              provider.loadAll(tenantId).stream()
