 ...
 *
 * <p>当引擎模块（如 {@code ydsz-common-search-es}、{@code ydsz-common-search-redis}）
 * 作为独立 Starter 发布时，各模块只需实现本接口并在 {@code AutoConfiguration.imports} 中注册，
 * 即能被核心模块的 {@link SearchAutoConfiguration.ModularEngineConfiguration} 发现并装配。
 *
 ...
 * }</pre>
 *
 * <p>核心模块仅保留 PG + Memory 引擎作为默认策略，ES/Redis/Solr/OpenSearch 等重型引擎
 * 按需引入。这种拆分带来三个好处：
 * <ul>
 *   <li>减少默认依赖体积（不含 heavyweight 客户端）</li>
 *   <li>各引擎版本独立演进，避免依赖冲突</li>
 *   <li>业务方只引入实际使用的引擎，避免 unused beans 占用内存</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public interface EngineStarterConfigurer {
     ...
     *
     * <p>每个 {@link ImportSelector} 可以返回一组自动配置类的全限定名，
     * Spring 容器会将它们纳入装配流程。通过这种方式，
     * 引擎模块可以在不改动核心 {@code SearchAutoConfiguration} 的前提下，
     * 将自己的 {@code SearchStrategy} 实现注册到引擎注册表。
     *
     ...
     *
     * <p>当多个引擎模块并存时，优先级高的引擎更先被注册到
     * {@link com.njydsz.common.search.core.SearchEngineRegistry}。
     * 核心模块的 PG 引擎默认优先级为 0。
     *
     * @return 优先级，默认为 0
     */
     ...
     *
     * <p>典型实现：检查 classpath 是否存在引擎客户端类（如 {@code RestHighLevelClient}），
     * 或检查 {@code ydsz.search.primary} 配置是否匹配。
     *
