 ...
 * 搜索提供者注册中心
 *
 * <p>管理所有 {@link SearchProvider} 实例，按类型查找。 搜索引擎通过注册中心获取各业务模块的 Provider。
 *
...
public class SearchProviderRegistry {
  ...
  public SearchProviderRegistry(List<SearchProvider<?>> providers) {
    ...
    log.info(
        "[SearchProviderRegistry] 自动注册完成: count={}, types={}",
        providerMap.size(),
        providerMap.keySet());
  }
  ...
  public void register(SearchProvider<?> provider) {
    ...
    if (existing != null) {
      log.warn(
          "[SearchProviderRegistry] 类型 {} 的 Provider 被覆盖: {} -> {}",
          type,
          existing.getClass().getSimpleName(),
          provider.getClass().getSimpleName());
    } else {
      log.info(
          "[SearchProviderRegistry] 注册 Provider: type={}, class={}",
          type,
          provider.getClass().getSimpleName());
    }
  }
  ...
  public List<SearchProvider<?>> getProviders(List<String> types) {
    ...
    return types.stream().map(providerMap::get).filter(Objects::nonNull).toList();
  }
}
