 ...
 * 搜索索引事件桥接器
 *
 * <p>业务模块通过此桥接器将数据变更同步到搜索索引，无需直接依赖 {@code IndexSyncService} 和 {@code SearchProvider}。
 *
 * <p><b>使用方式：</b>
 *
 * <pre>{@code
...
public class SearchIndexEventBridge {
   ...
   * 异步索引实体（UPSERT）
   *
   * <p>根据 type 查找对应的 {@link SearchProvider}，将实体转换为 {@link IndexDocument}， 然后委托 {@link
   * IndexSyncService} 异步写入搜索引擎。
   *
