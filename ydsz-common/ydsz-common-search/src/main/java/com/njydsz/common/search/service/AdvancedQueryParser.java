 ...
 * <ul>
 *   <li>{@code word1 AND word2} — 同时包含 word1 和 word2</li>
 *   <li>{@code word1 OR word2} — 包含 word1 或 word2</li>
 *   <li>{@code NOT word} — 排除 word</li>
 *   <li>{@code (word1 OR word2) AND word3} — 括号分组优先</li>
 *   <li>{@code "exact phrase"} — 精确短语匹配</li>
 *   <li>{@code field:value AND keyword} — 与字段过滤组合</li>
 * </ul>
 *
 * <p>示例：
 * <pre>
 ...
 * <p>对标行业：
 * <ul>
 *   <li>Elasticsearch Query DSL：bool query 嵌套 must/should/must_not</li>
 *   <li>Lucene Query Parser：业界标准布尔语法（Google/百度基于此演进）</li>
 *   <li>Algolia：filters 语法 + 分面过滤 + 数值比较</li>
 * </ul>
 *
...
public class AdvancedQueryParser {
    ...
    public ParseResult parse(String rawQuery) {
        ...
        while (fieldMatcher.find()) {
            ...
            String value = fieldMatcher.group(2) != null
            ...
            if ("type".equals(field)) {
                ...
                filters.add(SearchFilter.builder()
                        .field(field).values(List.of(value))
                        .operator(SearchFilter.Operator.EQ).build());
            }
        ...
        // 剩余文字
        String remainingText = phraseCleaned.length() > 0
        ...
        return new ParseResult(
                ...
                filters, types, mustTerms, shouldTerms, mustNotTerms
        );
    }
     ...
     * <p>语法规则（简化递归下降）：
     * <ul>
     *   <li>无操作符：全部归入 mustTerms（隐式 AND）</li>
     *   <li>AND 连接：左侧词归入 must</li>
     *   <li>OR 连接：两侧词归入 should</li>
     *   <li>NOT 后词：归入 mustNot</li>
     * </ul>
     *
     * @param text          清理后的文本
     ...
     */
    private void parseBooleanTokens(String text, List<String> mustTerms,
                                     List<String> shouldTerms, List<String> mustNotTerms) {
    ...
    private void addToTargetList(String term, BooleanOperator op,
                                  List<String> must, List<String> should, List<String> mustNot) {
        switch (op) {
     ...
     * <ul>
     *   <li>mustTerms → 用空格连接（AND 语义）</li>
     *   <li>shouldTerms → 用 " OR " 连接</li>
     *   <li>mustNotTerms → 附加 " -term" 排除语法</li>
     * </ul>
     *
    ...
    public SearchRequest applyTo(SearchRequest request) {
        ...
        // 检查是否有增强布尔逻辑
        boolean hasBooleanLogic = !result.mustTerms().isEmpty()
        ...
        // 合并 types 和 filters
        List<String> combinedTypes = new ArrayList<>(request.getTypes() != null ? request.getTypes() : new ArrayList<>());
        combinedTypes.addAll(result.types());

        List<SearchFilter> combinedFilters = new ArrayList<>(request.getFilters() != null ? request.getFilters() : new ArrayList<>());
        combinedFilters.addAll(result.filters());

        log.debug("[AdvancedParser] 原始关键词={}, 布尔关键词={}, filters={}",
                request.getKeyword(), finalKeyword, combinedFilters);
