...
public class SearchHealthIndicator implements HealthIndicator {
  ...
  public SearchHealthIndicator(
      SearchEngineRegistry engineRegistry, SearchCacheService cacheService, SearchMetrics metrics) {
  ...
  public Health health() {
    ...
    try {
      ...
      builder.withDetail(
          "engines",
          engineRegistry.getAllEngines().stream()
      ...
      if (metrics != null) {
        ...
        builder.withDetail(
            "zeroResultRate", String.format("%.2f%%", metrics.getZeroResultRate() * 100));
        builder.withDetail("totalIndexOps", metrics.getTotalIndexOps());
        builder.withDetail(
            "indexFailureRate", String.format("%.2f%%", metrics.getIndexFailureRate() * 100));
      }
