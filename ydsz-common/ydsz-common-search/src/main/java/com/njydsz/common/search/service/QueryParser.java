 ...
 * 查询理解器
 * <p>
 * 解析用户输入的高级搜索语法，提取结构化过滤条件：
 * <ul>
 *   <li>{@code field:value} — 精确匹配字段值</li>
 *   <li>{@code field:"multi word"} — 精确匹配含空格的值</li>
 *   <li>{@code type:project} — 指定搜索类型</li>
 *   <li>剩余文本作为关键词搜索</li>
 * </ul>
 *
 * <p>示例：
...
public class QueryParser {
    ...
    public SearchRequest applyTo(SearchRequest request) {
        ...
        log.debug("[QueryParser] 原始关键词={}, 解析后关键词={}, 过滤={}, 类型={}",
                request.getKeyword(), result.keyword(), result.filters(), result.types());
