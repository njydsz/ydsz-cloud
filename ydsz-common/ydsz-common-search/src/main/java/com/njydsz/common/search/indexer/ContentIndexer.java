 ...
 * 内容索引器
 *
 * <p>将文件内容解析后写入搜索索引。通过 {@link ContentExtractor} SPI 接口解析文档内容， 当无可用实现时仅索引文件名等元数据。
 *
 * <p><b>设计要点：</b>
 *
 * <ul>
 *   <li>通过 {@code ContentExtractor} 接口注入，避免反射调用和硬依赖
 *   <li>内容解析失败不影响元数据索引
 *   <li>支持内容长度限制，防止超大文件撑爆索引
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class ContentIndexer {
   ...
   */
  public IndexDocument enrichWithContent(
      IndexDocument document, InputStream inputStream, String fileName) {
    ...
    try {
      ...
      log.warn("[ContentIndexer] 内容解析失败，仅索引元数据: file={}, error={}", fileName, e.getMessage());
    }
