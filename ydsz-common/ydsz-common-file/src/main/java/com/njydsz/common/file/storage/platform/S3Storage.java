...
import software.amazon.awssdk.services.s3.model.CompleteMultipartUpload;
 ...
 * AWS S3 / S3 兼容对象存储实现。
 *
 * <p>继承 {@link AbstractFileStorage}，将操作翻译为 AWS S3 SDK v2 的原生 API 调用。 支持所有 S3 兼容存储服务（AWS
 * S3、MinIO、Ceph、Cloudflare R2 等）。
 *
 * <h3>分片上传协议</h3>
 *
...
public class S3Storage extends AbstractFileStorage {
  ...
  protected boolean doBucketExists(String bucketName) {
    try {
      ...
      log.error(
          "[S3] bucketExists failed, bucket={}, code={}, message={}",
          bucketName,
          e.statusCode(),
          e.getMessage());
      return false;
      ...
      log.error(
          "[S3] bucketExists unexpected error, bucket={}, message={}", bucketName, e.getMessage());
      return false;
    }
  }
  ...
  protected boolean doFolderExists(String bucketName, String folderName) {
    try {
      ...
      log.debug(
          "[S3] folderExist failed, bucket={}, folder={}, code={}, message={}",
          bucketName,
          folderName,
          e.statusCode(),
          e.getMessage());
      ...
      log.debug(
          "[S3] folderExist unexpected error, bucket={}, folder={}, message={}",
          bucketName,
          folderName,
          e.getMessage());
      return false;
  ...
  protected void doMakeFolder(String bucketName, String folderName) {
    try {
      ...
      log.error(
          "[S3] make Folder failed, bucket={}, folder={}, message={}",
          bucketName,
          folderName,
          e.getMessage());
  ...
  protected void doPutObject(
    ...
    try {
      ...
      log.error(
          "[S3] doPutObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
  ...
  protected InputStream doGetObject(
    ...
    try {
      ...
      log.error(
          "[S3] doGetObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected void doRemoveObject(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[S3] doRemoveObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected String buildPrivateUrl(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[S3] generate private url failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      return "";
  ...
  protected ChunkedUploadResult doInitiateMultipartUpload(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[S3] doInitiateMultipartUpload failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
  ...
  protected void doUploadPart(
    ...
    try {
      ...
      log.info(
          "[S3] chunk uploaded, bucket={}, chunk={}, part={}",
          bucketName,
          chunkObjectName,
          partNumber);
    } catch (Exception e) {
      log.error(
          "[S3] doUploadPart failed, bucket={}, chunk={}, part={}, message={}",
          bucketName,
          chunkObjectName,
          partNumber,
          e.getMessage());
  ...
  protected void doCompleteMultipartUpload(
    ...
    try {
      ...
      log.info(
          "[S3] chunked upload completed, bucket={}, object={}, parts={}",
          bucketName,
          objectName,
          completedParts.size());
    } catch (BusinessException e) {
      ...
      log.error(
          "[S3] doCompleteMultipartUpload failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.MULTIPART_UPLOAD_FAILED);
  ...
  protected void doAbortMultipartUpload(String bucketName, String objectName, String uploadId) {
    try {
      ...
      log.warn(
          "[S3] abort multipart upload failed, bucket={}, object={}, uploadId={}, message={}",
          bucketName,
          objectName,
          uploadId,
          e.getMessage());
  ...
  protected ObjectMetadata doGetMetadata(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[S3] doGetMetadata failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      return null;
  ...
  protected ListObjectsResult doListObjects(
    ...
    try {
      ...
      log.error(
          "[S3] doListObjects failed, bucket={}, prefix={}, message={}",
          bucketName,
          prefix,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  public PolicyResult generateUploadPolicy(
    ...
    try {
      ...
      log.error(
          "[S3] generateUploadPolicy failed, bucket={}, prefix={}, message={}",
          bucketName,
          objectNamePrefix,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_UPLOAD_FAILED);
