 ...
 * MinIO 对象存储实现。
 *
 * <p>继承 {@link AbstractFileStorage}，将操作翻译为 MinIO Java SDK 的原生 API 调用。 MinIO 兼容 S3 协议，但使用独立的 Java
 * SDK，API 更为简洁。
 *
 * <h3>分片上传策略</h3>
 *
 ...
 *
 * <ol>
 *   <li>每个分片作为独立对象上传到临时前缀目录
 *   <li>所有分片上传完成后，调用 {@code composeObject} 将分片合并为目标对象
 *   <li>合并成功后清理临时分片对象
 *   <li>失败时调用 {@code abortChunkedUpload} 清理已上传的分片
 * </ol>
 *
 * <h3>预签名 URL</h3>
 *
...
public class MinioStorage extends AbstractFileStorage {
  ...
  protected boolean doFolderExists(String bucketName, String folderName) {
    try {
      ...
      log.error(
          "[Minio] folderExist failed, bucket={}, folder={}, message={}",
          bucketName,
          folderName,
          e.getMessage());
      return false;
    ...
    try {
      ...
      if (!doFolderExists(bucketName, resolvedFolderName)) {
        ...
        PutObjectArgs putObjectArgs =
            PutObjectArgs.builder().bucket(bucketName).object(resolvedFolderName).stream(
                    emptyStream, 0, -1)
                .build();
  ...
  protected void doPutObject(
    ...
    try {
      ...
      log.error(
          "[Minio] doPutObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
  ...
  protected InputStream doGetObject(
    ...
    try {
      GetObjectArgs.Builder argsBuilder =
      ...
      log.error(
          "[Minio] doGetObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected void doRemoveObject(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[Minio] doRemoveObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected String buildPrivateUrl(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[Minio] generate private url failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected void doUploadPart(
    ...
    try {
      PutObjectArgs putObjectArgs =
          PutObjectArgs.builder().bucket(bucketName).object(chunkObjectName).stream(
                  inputStream, size, -1)
              .build();
      minioClient.putObject(putObjectArgs);
    } catch (Exception e) {
      log.error(
          "[Minio] doUploadPart failed, chunk={}, part={}, message={}",
          chunkObjectName,
          partNumber,
          e.getMessage());
    ...
    try {
      ...
      for (Integer partNumber : partNumbers) {
        ...
        minioClient.statObject(
            StatObjectArgs.builder().bucket(bucketName).object(chunkObjectName).build());
        sources.add(ComposeSource.builder().bucket(bucketName).object(chunkObjectName).build());
      }
      ...
      for (Integer partNumber : partNumbers) {
        ...
        try {
          minioClient.removeObject(
              RemoveObjectArgs.builder().bucket(bucketName).object(chunkObjectName).build());
        } catch (Exception e) {
          log.warn(
              "[Minio] cleanup chunk object failed after compose, object={}, message={}",
              chunkObjectName,
              e.getMessage());
        }
      }
      ...
      log.error(
          "[Minio] doCompleteMultipartUpload failed, object={}, message={}",
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.MULTIPART_UPLOAD_FAILED);
    }
    ...
    try {
      ...
      for (PartInfo part : existingParts) {
        ...
        try {
          ...
          log.warn(
              "[Minio] abort remove chunk failed, object={}, message={}",
              chunkObjectName,
              e.getMessage());
        }
      }
      log.info(
          "[Minio] doAbortMultipartUpload cleaned {} chunks, object={}",
          existingParts.size(),
          objectName);
    } catch (Exception e) {
      log.warn(
          "[Minio] doAbortMultipartUpload failed, bucket={}, object={}, uploadId={}, message={}",
          bucketName,
          objectName,
          uploadId,
          e.getMessage());
  ...
  protected ObjectMetadata doGetMetadata(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[Minio] doGetMetadata failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      return null;
  ...
  protected ListObjectsResult doListObjects(
    ...
    try {
      ...
      log.error(
          "[Minio] doListObjects failed, bucket={}, prefix={}, message={}",
          bucketName,
          prefix,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected String doGeneratePresignedUrl(String bucketName, String objectName, int expireSeconds) {
    try {
      ...
      log.error(
          "[MinIO] generatePresignedUrl failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage(),
          e);
  ...
  public PolicyResult generateUploadPolicy(
    ...
    try {
      ...
      mac.init(
          new SecretKeySpec(
              fileProperties.getSecretKey().getBytes(StandardCharsets.UTF_8), "HmacSHA256"));
      ...
      log.error(
          "[Minio] generateUploadPolicy failed, bucket={}, prefix={}, message={}",
          bucketName,
          objectNamePrefix,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_UPLOAD_FAILED);
   ...
   *
   * <p>大文件分片上传中断后，MinIO 不会自动回收已写入的分片，长期堆积会占用大量存储并增加列举开销。 本方法按 {@code CHUNK_DIR_PREFIX}
   * 前缀列举疑似分片对象，将超过 {@code maxAgeHours} 阈值的对象删除。
   *
   * <p><b>容错策略：</b>单条对象删除失败仅 {@code warn} 记录并继续后续对象，不中断整体清理； 列举或整体流程异常仅 {@code error}
   * 记录后吞掉，<b>不得向外抛出</b>，避免清理任务失败影响主存储链路。 桶名经 {@code resolveBucketName} 归一化，{@code null}/空桶名由调用方保证。
   *
    ...
    try {
      ...
      for (Result<Item> result : results) {
        ...
        if (lastModified != null
          ...
          try {
            minioClient.removeObject(
                RemoveObjectArgs.builder().bucket(resolvedBucket).object(objectName).build());
            cleanedCount++;
          } catch (Exception e) {
            log.warn(
                "[Minio] cleanup stale multipart chunk failed, object={}, message={}",
                objectName,
                e.getMessage());
          }
        }
      ...
      if (cleanedCount > 0) {
        log.info(
            "[Minio] cleanupStaleMultipartUploads completed, bucket={}, cleaned={}",
            resolvedBucket,
            cleanedCount);
      }
    } catch (Exception e) {
      log.error(
          "[Minio] cleanupStaleMultipartUploads failed, bucket={}, message={}",
          resolvedBucket,
          e.getMessage());
    }
  }
