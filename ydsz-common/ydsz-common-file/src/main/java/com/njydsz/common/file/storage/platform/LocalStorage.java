 ...
 * 本地磁盘存储实现
 *
 * <p>继承 {@link AbstractFileStorage}， 将文件存储到配置指定的本地目录（bucket 路径）。
 *
...
public class LocalStorage extends AbstractFileStorage {
  ...
  protected void doUploadPart(
    ...
    try {
      ...
      try (OutputStream os =
          Files.newOutputStream(
              chunkPath, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING)) {
        IOUtils.copy(inputStream, os);
      }
    } catch (BusinessException e) {
      throw e;
      ...
      log.error(
          "[Local] doUploadPart failed, chunk={}, message={}", chunkObjectName, e.getMessage());
      throw new BusinessException(FileExceptionCode.MULTIPART_UPLOAD_FAILED);
    }
  }
  ...
  protected void doCompleteMultipartUpload(
    ...
    try {
      ...
      log.error(
          "[Local] doCompleteMultipartUpload failed, object={}, message={}",
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.MULTIPART_UPLOAD_FAILED);
    }
          ...
          .forEach(
              p -> {
                ...
                try {
                  ...
                  log.warn(
                      "[Local] listParts get size failed, path={}, message={}", p, e.getMessage());
                }
              });
    } catch (IOException e) {
      log.warn(
          "[Local] listParts failed, uploadRoot={}, message={}", uploadRootPath, e.getMessage());
    }
    return parts;
  }
    ...
    try {
      ...
      metadata.setLastModified(
          LocalDateTime.ofInstant(
              Instant.ofEpochMilli(localFile.lastModified()), ZoneId.systemDefault()));
    ...
    try {
      ...
      try (Stream<Path> stream = Files.walk(searchPath, 1)) {
        ...
        if (endIndex < sortedFiles.size()) {
          nextCursor =
              bucketPath.relativize(sortedFiles.get(endIndex)).toString().replace("\\", "/");
        }
      ...
      log.error(
          "[Local] doListObjects failed, bucket={}, prefix={}, message={}",
          bucketName,
          prefix,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  public void download(
    ...
    try {
      ...
      response.setStatus(
          start == 0 && (length == null || length >= fileSize)
      ...
      log.info(
          "[Local] file download success, object={}, offset={}, length={}",
          objectName,
          start,
          end - start);
    } catch (BusinessException e) {
      ...
      log.error(
          "[Local] file download failed, object={}, message={}", objectName, e.getMessage(), e);
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
    }
  }
   ...
   * @return 分片文件的绝对路径
   */
  private Path resolveChunkPath(
      String bucketName, String objectName, String uploadId, int partNumber) {
    String chunkObjectName =
        CHUNK_DIR_PREFIX
  ...
  private Path resolveUploadRootPath(String bucketName, String objectName, String uploadId) {
    String chunkRootObjectName =
        CHUNK_DIR_PREFIX + FileConstant.DIR_SPLIT + objectName + FileConstant.DIR_SPLIT + uploadId;
  ...
  private void deleteIfEmpty(Path directoryPath) {
    try {
      ...
      log.warn(
          "[Local] delete empty chunk dir failed, path={}, message={}",
          directoryPath,
          e.getMessage());
    }
  }
