 ...
 *
 * <ol>
 *   <li>{@code ApiUploadV2InitUpload}：初始化分片上传，获取 uploadId
 *   <li>{@code ApiUploadV2UploadPart}：上传单个分片
 *   <li>{@code ApiUploadV2CompleteUpload}：合并所有分片为最终对象
 *   <li>{@code ApiUploadV2AbortUpload}：取消上传并清理已上传分片
 * </ol>
 *
 * <h3>私有空间下载</h3>
 *
...
public class QiniuStorage extends AbstractFileStorage {
  ...
  protected boolean doFolderExists(String bucketName, String folderName) {
    try {
      ...
      log.debug(
          "[Qiniu] folderExists failed, bucket={}, folder={}, code={}, message={}",
          bucketName,
          folderName,
          e.code(),
          e.getMessage());
      ...
      log.debug(
          "[Qiniu] folderExist unexpected error, bucket={}, folder={}, message={}",
          bucketName,
          folderName,
          e.getMessage());
      return false;
  ...
  protected void doMakeFolder(String bucketName, String folderName) {
    try {
      ...
      log.error(
          "[Qiniu] make Folder failed, bucket={}, folder={}, message={}",
          bucketName,
          folderName,
          e.getMessage());
  ...
  protected void doPutObject(
    ...
    try {
      ...
      log.error(
          "[Qiniu] doPutObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
  ...
  protected InputStream doGetObject(
    ...
    try {
      ...
      log.error(
          "[Qiniu] doGetObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected void doRemoveObject(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[Qiniu] doRemoveObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected String buildPrivateUrl(String bucketName, String objectName) {
    ...
    try {
      ...
      log.error(
          "[Qiniu] generate private url failed, object={}, message={}", objectName, e.getMessage());
      return "";
    }
  }
  ...
  protected ChunkedUploadResult doInitiateMultipartUpload(String bucketName, String objectName) {
    try {
      ...
      ApiUploadV2InitUpload.Request request =
          new ApiUploadV2InitUpload.Request(bucketName, upToken).setKey(objectName);
      ...
      log.error(
          "[Qiniu] doInitiateMultipartUpload failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
  ...
  protected void doUploadPart(
    ...
    try {
      ...
      if (context == null) {
        context =
            new MultipartContextStore.MultipartContextData(uploadId, bucketName, chunkObjectName);
      }
      ...
      multipartContextStore.save(
          ...
          24 * 3600);
      log.info(
          "[Qiniu] chunk uploaded, bucket={}, chunk={}, part={}",
          bucketName,
          chunkObjectName,
          partNumber);
      ...
      log.error(
          "[Qiniu] doUploadPart failed, bucket={}, chunk={}, part={}, message={}",
          bucketName,
          chunkObjectName,
          partNumber,
          e.getMessage());
  ...
  protected ObjectMetadata doGetMetadata(String bucketName, String objectName) {
    try {
      ...
      metadata.setLastModified(
          fileInfo.putTime > 0
              ? LocalDateTime.ofInstant(
                  Instant.ofEpochMilli(fileInfo.putTime / 10000), ZoneId.systemDefault())
              : null);
      ...
      log.error(
          "[Qiniu] doGetMetadata failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      return null;
