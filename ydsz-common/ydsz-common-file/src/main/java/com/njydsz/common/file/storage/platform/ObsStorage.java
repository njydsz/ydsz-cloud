 ...
 * 华为云 OBS 对象存储实现。
 *
 * <p>继承 {@link AbstractFileStorage}，将操作翻译为华为云 OBS Java SDK 的原生 API 调用。 实现 {@link AutoCloseable}
 * 以在应用关闭时释放 OBS 客户端资源。
 *
 * <h3>分片上传协议</h3>
 *
...
public class ObsStorage extends AbstractFileStorage implements AutoCloseable {
  ...
  public ObsStorage(FileProperties config, FileUploadProperties uploadProps) {
    ...
    try {
      ...
      this.obsClient = new ObsClient(config.getAccessKey(), config.getSecretKey(), obsConfig);
    } catch (Exception e) {
  ...
  protected boolean doFolderExists(String bucketName, String folderName) {
    try {
      ...
      log.debug(
          "[OBS] folderExists failed, bucket={}, folder={}, message={}",
          bucketName,
          folderName,
          e.getMessage());
      return false;
  ...
  protected void doMakeFolder(String bucketName, String folderName) {
    try {
      ...
      log.error(
          "[OBS] make Folder failed, bucket={}, folder={}, message={}",
          bucketName,
          folderName,
          e.getMessage());
  ...
  protected void doPutObject(
    ...
    try {
      ...
      ObjectMetadata metadata =
          new ObjectMetadata(); // FQN-OK: name conflict with ObjectMetadata
      ...
      log.error(
          "[OBS] doPutObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
  ...
  protected InputStream doGetObject(
    ...
    try {
      ...
      log.error(
          "[OBS] doGetObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected void doRemoveObject(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[OBS] doRemoveObject failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
  ...
  protected String buildPrivateUrl(String bucketName, String objectName) {
    try {
      TemporarySignatureRequest request =
          new TemporarySignatureRequest(HttpMethodEnum.GET, bucketName, objectName, null, 3600);
      TemporarySignatureResponse response = obsClient.createTemporarySignature(request);
      return response.getSignedUrl();
    } catch (Exception e) {
      log.error(
          "[OBS] generate private url failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      return "";
  ...
  protected ChunkedUploadResult doInitiateMultipartUpload(String bucketName, String objectName) {
    try {
      ...
      log.error(
          "[OBS] doInitiateMultipartUpload failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
  ...
  protected void doUploadPart(
    ...
    try {
      ...
      log.info(
          "[OBS] chunk uploaded, bucket={}, chunk={}, part={}",
          bucketName,
          chunkObjectName,
          partNumber);
    } catch (Exception e) {
      log.error(
          "[OBS] doUploadPart failed, bucket={}, chunk={}, part={}, message={}",
          bucketName,
          chunkObjectName,
          partNumber,
          e.getMessage());
  ...
  protected void doCompleteMultipartUpload(
    ...
    try {
      ...
      Map<Integer, Multipart> uploadedPartMap =
          listPartsResponse.getMultipartList().stream()
      ...
      log.info(
          "[OBS] chunked upload completed, bucket={}, object={}, parts={}",
          bucketName,
          objectName,
          partETags.size());
      ...
      log.error(
          "[OBS] doCompleteMultipartUpload failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.MULTIPART_UPLOAD_FAILED);
  ...
  protected void doAbortMultipartUpload(String bucketName, String objectName, String uploadId) {
    try {
      ...
      log.warn(
          "[OBS] abort multipart upload failed, bucket={}, object={}, uploadId={}, message={}",
          bucketName,
          objectName,
          uploadId,
          e.getMessage());
  ...
  protected List<PartInfo> listParts(String bucketName, String objectName, String uploadId) {
    try {
      ...
      return response.getMultipartList().stream()
          .map(
              multipart ->
                  new PartInfo(multipart.getPartNumber(), multipart.getEtag(), multipart.getSize()))
          .toList();
    } catch (Exception e) {
  ...
  protected ObjectMetadata doGetMetadata(String bucketName, String objectName) {
    try {
      ...
      ObjectMetadata obsMetadata =
          obsObject.getMetadata(); // FQN-OK: name conflict with ObjectMetadata
      ...
      log.error(
          "[OBS] doGetMetadata failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
      return null;
  ...
  protected ListObjectsResult doListObjects(
    ...
    try {
      ...
      log.error(
          "[OBS] doListObjects failed, bucket={}, prefix={}, message={}",
          bucketName,
          prefix,
          e.getMessage());
      throw new BusinessException(FileExceptionCode.FILE_OPERATE_FAILED);
