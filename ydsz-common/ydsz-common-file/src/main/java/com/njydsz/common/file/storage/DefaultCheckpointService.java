...
public class DefaultCheckpointService implements CheckpointService {
  ...
  public DefaultCheckpointService(
      CheckpointStore checkpointStore, MultipartLister multipartLister) {
   ...
   */
  public DefaultCheckpointService(
      CheckpointStore checkpointStore, MultipartLister multipartLister, long checkpointTtlSeconds) {
  ...
  public void saveCheckpoint(UploadCheckpoint checkpoint) {
    ...
    try {
      ...
      checkpointStore.save(
          checkpoint.getBucketName(), checkpoint.getObjectName(), json, checkpointTtlSeconds);
    } catch (Exception e) {
      log.warn(
          "[Storage] saveCheckpoint failed, bucket={}, object={}, message={}",
          checkpoint.getBucketName(),
          checkpoint.getObjectName(),
          e.getMessage());
    }
  }
  ...
  public UploadCheckpoint loadCheckpoint(String bucketName, String objectName) {
    try {
      ...
      if (json != null) {
        return YdszJson.fromJson(json, UploadCheckpoint.class);
      }
    } catch (Exception e) {
      log.warn(
          "[Storage] loadCheckpoint failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
    }
  ...
  @Override
  public UploadCheckpoint validateAndRecoverCheckpoint(
    ...
    if (checkpoint.getTotalSize() == null || !checkpoint.getTotalSize().equals(file.getSize())) {
      log.warn(
          "[Storage] checkpoint totalSize mismatch, expected={}, actual={}",
          checkpoint.getTotalSize(),
          file.getSize());
    ...
    try {
      ...
      log.warn(
          "[Storage] validateAndRecoverCheckpoint failed, uploadId={}, message={}",
          uploadId,
          e.getMessage());
      deleteCheckpoint(checkpoint);
      return null;
  ...
  @Override
  public void updateChunkMd5InCheckpoint(
      String bucketName, String objectName, int partNumber, String chunkMd5, long chunkSize) {
    try {
      ...
      log.warn(
          "[Storage] updateChunkMd5InCheckpoint failed, bucket={}, object={}, part={}, message={}",
          bucketName,
          objectName,
          partNumber,
          e.getMessage());
  ...
  public void updateAccumulatedMd5(String bucketName, String objectName, String accumulatedMd5) {
    try {
      ...
      log.debug(
          "[Storage] updateAccumulatedMd5 success, bucket={}, object={}, md5={}",
          bucketName,
          objectName,
          accumulatedMd5);
    } catch (Exception e) {
      log.warn(
          "[Storage] updateAccumulatedMd5 failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
    }
  ...
  @Override
  public void validateFileMd5(
      String bucketName,
      String objectName,
      boolean md5CheckEnabled,
    ...
    try {
      ...
      if (actualMd5 == null || actualMd5.isEmpty()) {
        log.warn(
            "[Storage] accumulatedMd5Hex is null, falling back to re-download for MD5 validation, bucket={}, object={}",
            bucketName,
            objectName);
        try (InputStream is = objectDownloader.download(bucketName, objectName)) {
          ...
          if (actualMd5 == null) {
            log.warn(
                "[Storage] validateFileMd5 compute failed, bucket={}, object={}",
                bucketName,
                objectName);
            return;
          }
      ...
      if (!actualMd5.equalsIgnoreCase(checkpoint.getFileMd5())) {
        log.error(
            "[Storage] validateFileMd5 mismatch, bucket={}, object={}, expected={}, actual={}",
            bucketName,
            objectName,
            checkpoint.getFileMd5(),
            actualMd5);
      ...
      log.warn(
          "[Storage] validateFileMd5 failed, bucket={}, object={}, message={}",
          bucketName,
          objectName,
          e.getMessage());
    }
