 ...
 * 文件存储模块定时任务调度器。
 *
 * <p>承载所有文件存储相关的定时清理任务，与 {@link FileConfiguration} 自动配置分离， 避免定时任务逻辑污染配置类，也便于单独测试和维护。
 *
 * <p>当前包含的定时任务：
 *
 * <ul>
 *   <li>分片上传上下文过期清理（默认每小时执行一次）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class FileScheduler {
   ...
   *
   * <p>对于超时未完成的分片上传任务，其上下文数据会持续堆积在存储后端（如 Redis）， 定期清理可避免内存/存储资源的无意义占用。
   */
