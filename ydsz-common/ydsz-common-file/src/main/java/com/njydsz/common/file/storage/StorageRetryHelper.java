 ...
 *
 * <p>对存储操作（上传/下载等）提供自动重试能力，采用指数退避算法并附加随机抖动，
 * 避免在云存储服务短暂不可用时直接失败，同时减少并发重试时的惊群效应。
 *
 * <p><b>重试策略：</b>
 * <ul>
 *   <li>首次失败后等待约 {@code initialBackoffMillis} 毫秒</li>
 *   <li>每次重试等待时间翻倍（指数退避）并附加 0~50% 随机抖动</li>
 *   <li>退避上限为 {@code maxBackoffMillis}（默认 30 秒）</li>
 ...
 *   <li>可通过 {@link #setRetryablePredicate(Predicate)} 自定义可重试异常类型，
 *       默认所有非业务异常均可重试</li>
 *   <li>线程中断时立即退出重试循环</li>
 * </ul>
 *
...
public class StorageRetryHelper {
     ...
     */
    public StorageRetryHelper(int maxRetries, long initialBackoffMillis,
                              long maxBackoffMillis, Predicate<Exception> retryablePredicate) {
     ...
     * 带重试执行操作（有返回值）
     *
     * <p>若操作成功则直接返回结果；若操作失败且为可重试异常则自动重试；
     * 若为业务异常或不可重试异常则直接抛出。
     *
     * @param <T>           返回值类型
     * @param action        待执行的操作
    ...
    public <T> T executeWithRetry(Supplier<T> action, String operationName) {
        ...
        while (attempts <= maxRetries) {
            try {
                ...
                if (!isRetryable(e)) {
                    log.warn("[StorageRetry] {} non-retryable exception, abort: {}", operationName, e.getMessage());
                ...
                log.warn("[StorageRetry] {} attempt {} failed, retrying in {}ms: {}",
                        operationName, attempts, backoff, e.getMessage());
                try {
