 ...
 *
 * <ul>
 *   <li>菜单权限（menuPermissions）：用于控制用户可见的菜单入口
 *   <li>按钮权限（buttonPermissions）：用于控制用户可执行的操作按钮
 *   <li>接口权限（apiPermissions）：用于控制接口级别的访问权限
 * </ul>
 *
 * <p>该类是不可变的（Immutable），所有字段在构造后不可修改， 适合在多角色权限合并场景下安全使用。
 *
 * <p><b>使用场景：</b>
 *
 * <ul>
...
public final class RolePermissions {
  ...
  public RolePermissions(
      Set<String> menuPermissions, Set<String> buttonPermissions, Set<String> apiPermissions) {
    this.menuPermissions =
        menuPermissions != null
            ? Collections.unmodifiableSet(menuPermissions)
            : Collections.emptySet();
    this.buttonPermissions =
        buttonPermissions != null
            ? Collections.unmodifiableSet(buttonPermissions)
            : Collections.emptySet();
    this.apiPermissions =
        apiPermissions != null
            ? Collections.unmodifiableSet(apiPermissions)
            : Collections.emptySet();
  }
