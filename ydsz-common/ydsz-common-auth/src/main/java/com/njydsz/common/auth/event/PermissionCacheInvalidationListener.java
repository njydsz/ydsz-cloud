 ...
 *
 * <p><b>多实例同步：</b> 当 ydsz-common-redis 可用时，通过 Redis Keyspace Notification 将缓存失效消息广播到其他实例，
 * 确保多实例部署场景下各节点缓存一致性。
 *
 * <p><b>触发场景：</b>
 *
 * <ul>
 *   <li>业务代码发布权限变更事件后自动清理对应角色的权限缓存
 *   <li>角色权限被修改后自动清理缓存
 *   <li>用户角色被分配后自动清理缓存
 * </ul>
 *
...
public class PermissionCacheInvalidationListener {
  ...
  private void handleInvalidation(
      PermissionChangedEvent.PermissionChangeType changeType, String roleCode) {
    switch (changeType) {
