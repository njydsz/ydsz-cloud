 ...
 * 权限快照 —— 请求级别不可变权限缓存。
 *
 * <p>在请求开始时一次性从 Redis 加载所有权限信息，封装为不可变对象，
 * 避免后续在鉴权流程中多次查询 Redis。该类是线程安全的（所有字段均为
 * final 且不可变），可在多线程环境下安全共享。
 *
 * <p><b>包含的权限信息：</b>
 * <ul>
 *   <li>用户 ID 与租户 ID</li>
 *   <li>用户拥有的角色编码集合</li>
 *   <li>菜单权限码集合（合并自所有角色）</li>
 *   <li>按钮权限码集合（合并自所有角色）</li>
 *   <li>接口权限码集合（合并自所有角色）</li>
 * </ul>
 *
...
public final class PermissionSnapshot {
    ...
    private PermissionSnapshot(
        ...
        this.menuPermissions = menuPermissions != null
                ? Collections.unmodifiableSet(new HashSet<>(menuPermissions))
                : Collections.emptySet();
        this.buttonPermissions = buttonPermissions != null
                ? Collections.unmodifiableSet(new HashSet<>(buttonPermissions))
                : Collections.emptySet();
        this.apiPermissions = apiPermissions != null
     ...
     *
     * <p>该方法将所有 {@link RolePermissions} 中的权限集合合并到同一个
     * 快照中，产生去重后的全量权限视图。
     *
     * @param userId        用户 ID
     ...
     * 判断当前用户是否为超级管理员。
     *
     * <p>当用户拥有的角色编码集合与 {@code ignoreRoles} 列表存在交集时，
     * 判定为超级管理员，即拥有所有权限，跳过鉴权检查。
     *
    ...
    @Override
    public int hashCode() {
        return Objects.hash(userId, tenantId, userRoles, menuPermissions, buttonPermissions, apiPermissions);
    }
    ...
    public static final class Builder {
        ...
        public PermissionSnapshot build() {
            return new PermissionSnapshot(userId, tenantId, userRoles, menuPermissions, buttonPermissions, apiPermissions);
        }
    }
}
