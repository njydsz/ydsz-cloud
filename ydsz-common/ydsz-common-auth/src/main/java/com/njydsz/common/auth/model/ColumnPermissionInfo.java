 ...
 * 列权限信息集合。
 *
 * <p>以 Map 形式存储和管理多个字段的列权限信息， 提供权限的添加、合并、查询等操作。
 *
 * <p><b>与 ColumnScopeInfo 的区别：</b>
 *
 * <ul>
 *   <li>{@link ColumnScopeInfo}：按表组织，关注的是"哪些表有哪些可见/可编辑字段"
 *   <li>{@link ColumnPermissionInfo}：按字段组织，关注的是"某个字段是否可读/可写"
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class ColumnPermissionInfo {
   ...
   * 添加单个字段的列权限。
   *
   * <p>如果该字段已存在权限，则与现有权限合并（取并集）。 列名会自动转为小写进行标准化。
   *
   * @param column 字段名
