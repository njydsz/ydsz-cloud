 ...
 *
 * <p>自 v2.0.0 起从 util 层迁移至 common-auth 服务层， 已移除对旧版弃用类的继承依赖，新代码应继承本类。
 *
 * @author ydsz-team
 * @since 1.0.0
 * @see AuthHandler
...
public abstract class AbstractAuthHandler implements AuthHandler {
   ...
   * 创建认证信息实例（模板方法模式）
   *
   * <p>子类实现此方法返回具体的 AuthInfo 实现类实例， 如 WebAuthInfo 或 AppAuthInfo。
   *
   ...
   * 按逗号分割字符串并过滤空值
   *
   * <p>委托给 {@link com.njydsz.common.auth.util.PermissionUtils#splitCsv} 统一处理 CSV 解析， 消除重复逻辑。
   *
   * @param value 待分割的字符串
