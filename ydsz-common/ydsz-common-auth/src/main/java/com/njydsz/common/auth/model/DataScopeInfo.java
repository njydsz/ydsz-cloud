 ...
 *
 * <ul>
 *   <li>组织维度：scope + companyIds + deptIds（集团/公司/部门）
 *   <li>项目维度：projectIds
 *   <li>区域维度：regionIds
 *   <li>自定义维度：customSqlCondition（自定义 SQL 条件）
 * </ul>
 *
 * <p><b>使用场景：</b>
 *
 * <ul>
 *   <li>scope：权限范围（tenant/group/company/dept/user/project/region/custom）
 *   <li>companyIds：可访问公司 ID 集合
 *   <li>deptIds：可访问部门 ID 集合
 *   <li>projectIds：可访问项目 ID 集合
 *   <li>regionIds：可访问区域 ID 集合
 *   <li>customSqlCondition：自定义 SQL 条件片段
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class DataScopeInfo {
  ...
  public DataScopeInfo(
    ...
    this.userId = userId;
    this.companyIds =
        companyIds != null ? Collections.unmodifiableSet(companyIds) : Collections.emptySet();
   ...
   * 返回空数据权限实例。
   *
   * <p>所有维度集合为空、scope 为 {@code null}，表示无任何额外数据范围约束（等同于"全部可见"）。 每次返回新建实例，调用方不可依赖其唯一性。
   *
   * @return 空数据权限实例
   */
   ...
   * 判断是否存在生效的自定义 SQL 条件。
   *
   * <p>模板（{@code customSqlConditionTemplate}）或直给片段（{@code customSqlCondition}）任一非空即视为存在。
   *
   ...
   *
   * <p>若配置了 {@code customSqlConditionTemplate} 则按模板注入当前用户/租户及各维度 ID 并做 SQL 注入清洗后返回； 否则回退到直接给定的
   * {@code customSqlCondition}。模板含注入风险字符时抛出 {@link SecurityException} 阻断拼接。
   *
   ...
   *
   * <p>所有 setter 均<b>不做校验</b>，集合类字段允许传 {@code null}， 由 {@link #build()} 交给构造器统一包装为不可变空集合。
   */
  public static class Builder {
     ...
     * 设置自定义 SQL 条件模板。
     *
     * <p>支持 <code>{{userId}}</code> 等占位符，解析时注入实际值并执行防注入清洗， 因此比 {@link #customSqlCondition(String)}
     * 更安全，应优先使用。
     *
     ...
     *
     * <p>各集合字段会在构造器中包装为不可变集合，{@code null} 统一归一化为空集合， 因此产物的集合类 getter 永不返回 {@code null}。
     *
     * @return 构建完成的数据权限信息
     */
