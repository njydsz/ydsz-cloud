 ...
 * 权限预检注解。
 *
 * <p>标注在方法上，在业务逻辑执行前预先校验用户是否拥有所需权限。 与 {@link com.njydsz.common.auth.annotation.AuthApiPermission}
 * 不同，预检不会直接抛出异常， 而是返回详细的预检结果或注入到方法参数中，由业务方自行决定如何处理。
 *
 * <p><b>使用场景：</b>
 *
 * <ul>
 *   <li>前端根据预检结果动态显示/隐藏操作按钮
 *   <li>批量操作前检查用户是否有权限执行
 *   <li>需要根据权限校验结果执行不同业务逻辑的场景
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
...
public @interface PermissionPreCheck {
   ...
   *
   * <ul>
   *   <li>{@link PreCheckMode#RETURN}：返回模式（默认）
   *   <li>{@link PreCheckMode#THROW}：抛出异常模式
   * </ul>
   *
   * @return 预检模式
   */
