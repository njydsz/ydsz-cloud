 ...
 *
 * <ol>
 *   <li>Spring ApplicationEvent：来自本地节点的权限变更事件
 *   <li>Redis Pub/Sub：来自其他集群节点的权限变更通知
 * </ol>
 *
...
public class PermissionChangeCacheInvalidator {
   ...
   *
   * <p>在容器启动阶段向 {@link RedisMessageListenerContainer} 注册监听器， 以接收其他集群节点通过 Pub/Sub
   * 下发的权限变更通知。订阅失败仅记录错误日志， 不影响本地 Spring {@code ApplicationEvent} 通道的缓存失效能力。
   */
  @PostConstruct
  public void init() {
  ...
  private void subscribeToRedisChannel() {
    try {
      ...
      redisMessageListenerContainer.addMessageListener(
          adapter, new ChannelTopic(PERMISSION_CHANGE_CHANNEL));
   ...
   * 处理本地节点发出的权限变更事件。
   *
   * <p>作为 Spring {@code @EventListener} 监听 {@link PermissionChangedEvent}； 事件为 {@code null}
   * 时直接忽略。最终委派 {@link #handlePermissionChange} 按变更类型失效对应角色的各权限缓存。
   *
   ...
   *
   * <p>订阅 {@value #PERMISSION_CHANGE_CHANNEL} 频道，接收其他集群节点下发的权限变更通知； 消息格式为 {@code
   * roleCode|changeType|affectedTypes|...|sourceNode}，解析失败或格式非法时 记录告警日志并忽略，不影响本地缓存状态。
   */
    ...
    private PermissionChangedEvent parseMessage(String message) {
      ...
      try {
        ...
        PermissionChangedEvent.PermissionChangeType changeType =
            PermissionChangedEvent.PermissionChangeType.valueOf(parts[1].trim());
        String affectedTypesStr = parts[2].trim();
        Set<String> affectedTypes =
            affectedTypesStr.isEmpty()
                ? null
