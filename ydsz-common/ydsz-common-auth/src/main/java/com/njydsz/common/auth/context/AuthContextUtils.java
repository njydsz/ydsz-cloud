 ...
 *   <li>列权限：{@link #getColumnPermission()} / {@link #setColumnPermission(ColumnPermissionInfo)}</li>
 *   <li>请求级用户信息缓存：{@link #getCachedUserInfoMap()} / {@link #setCachedUserInfoMap(Map)}</li>
 * </ul>
 *
 * <p><b>使用示例：</b>
 * <pre>{@code
...
public final class AuthContextUtils {
     ...
     * 设置当前登录用户。
     *
     * <p>同步关键信息（userId/tenantId）到 {@link RequestContext}，
     * 便于跨模块统一访问。
     *
     * @param user 登录用户
     */
     ...
     * 获取请求级缓存的用户信息 Map。
     *
     * <p>由 RbacPermissionEvaluator 首次加载后写入，后续同一请求内直接读取，
     * 避免反复 Redis 调用。
     *
