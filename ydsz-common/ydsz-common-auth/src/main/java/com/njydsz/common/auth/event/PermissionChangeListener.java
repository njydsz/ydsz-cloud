 ...
 * 权限变更事件 SPI 接口。
 *
 * <p>用于扩展权限变更回调，实现类可通过 SpringFactoriesLoader 自动注册。 适用于需要在权限变更时执行自定义逻辑的场景，如：
 *
 * <ul>
 *   <li>审计日志记录
 *   <li>第三方系统同步
 ...
 *       或 {@code META-INF/spring.factories} 中注册
 *   <li>实现 {@code onPermissionChanged} 方法处理权限变更事件
 * </ol>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public interface PermissionChangeListener {
   ...
   * 权限变更回调方法。
   *
   * <p>当权限发生变更时，此方法会被调用。实现类应确保该方法的幂等性， 避免重复处理导致副作用。
   *
