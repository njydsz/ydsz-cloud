 ...
 * </pre>
 *
 * <p><b>与 ydsz-common-jdbc 联动：</b>
 *
 * <p>行级数据权限会自动透传到 SQL 拦截层，实现自动的数据过滤。 详见 {@link
...
public @interface AuthRowPermission {
   ...
   *
   * <p>当方法参数为 Map 类型时，数据权限信息会以此 key 存入 Map。 默认值为 {@code rowPermission}。
   *
   * @return Map key 名称
   */
   ...
   *
   * <ul>
   *   <li>{@code true}：若解析不到有效的数据权限，抛出业务异常
   *   <li>{@code false}：若解析不到数据权限，静默跳过（默认）
   * </ul>
   *
