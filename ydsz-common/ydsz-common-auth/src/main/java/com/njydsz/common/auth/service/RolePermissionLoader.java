 ...
 * 角色权限加载器接口。
 *
 * <p>定义根据 roleCode 加载角色权限集合的行为。 数据来源可为 Redis/DB/远程权限中心等。
 *
 * <p><b>实现类职责：</b>
 *
 * <ul>
 *   <li>根据 roleCode 加载该角色的三类权限集合（菜单/按钮/接口）
 *   <li>实现应考虑缓存策略，提高频繁权限校验的性能
 *   <li>返回空权限而非 null，保证调用方稳定
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
...
public interface RolePermissionLoader {
   ...
   * 批量加载多个角色的权限集合。
   *
   * <p>默认实现逐个调用 {@link #loadByRoleCode(String)}，实现类可覆盖此方法以使用 Redis MGET/Pipeline 等批量操作，将 N 次 Redis
   * 往返减少为 1-2 次。
   *
