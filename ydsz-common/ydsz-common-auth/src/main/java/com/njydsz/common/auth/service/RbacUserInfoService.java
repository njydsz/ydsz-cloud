 ...
 * RBAC 用户信息加载器接口。
 *
 * <p>定义根据 accessToken 加载用户信息的行为。 用户信息必须包含 {@code roleCode} 字段，用于后续权限解析。
 *
 * <p><b>实现类职责：</b>
 *
 * <ul>
 *   <li>从请求上下文（ThreadLocal/请求头）获取 accessToken
 *   <li>根据 accessToken 加载用户信息（可来自 Redis/DB/远程服务）
 *   <li>确保返回的 UserInfo 中包含 {@code roleCode} 字段
 * </ul>
 *
 * <p><b>使用示例：</b>
 *
...
public interface RbacUserInfoService {
   ...
   *
   * <p>推荐使用此方法，返回类型安全的 UserInfo DTO， 避免 Map&lt;String, Object&gt; 带来的类型转换错误和安全隐患。
   *
   ...
   *
   * <p>此方法仅供内部实现使用，外部调用请使用 {@link #loadUserInfo(String)}。
   *
   * @param accessToken 访问令牌
