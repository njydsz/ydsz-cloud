 ...
 * 认证信息快捷读取工具类
 *
 * <p>封装 {@link RequestContext} 中认证信息的常用读取逻辑，提供空值安全的快捷方法：
 *
 * <ul>
 *   <li>基础身份：用户语言、访问令牌、用户ID、身份类型、服务类型
 *   <li>行级数据权限：数据范围类型、租户ID
 *   <li>行级数据权限维度ID集合：公司ID、部门ID、项目ID、区域ID
 *   <li>列级数据权限：表级可见列规则、表级可编辑列规则
 * </ul>
 *
 * <p>所有方法均做空值判断，底层 {@link AuthInfo} 为 null 时返回安全默认值：
 *
 * <ul>
 *   <li>返回类型为 Set：返回 {@link Collections#emptySet()}
 *   <li>返回类型为 Map：返回 {@link Collections#emptyMap()}
 *   <li>返回类型为 String：返回 null
 * </ul>
 *
 * <h3>使用场景</h3>
 *
 * <ul>
 *   <li>Feign 远程调用：透传数据权限 header
 *   <li>SQL 拦截器：读取上下文构建过滤条件
 *   <li>业务代码：判断当前用户权限范围
 * </ul>
 *
 * <p><b>设计说明：</b>自 26.09.01 起，本工具类下沉至 common-auth 服务层， 直接依赖 core 的 RequestContext。util 层不再承载认证业务逻辑。
 *
 * @author ydsz-team
 * @since 26.09.01
 */
...
public final class AuthInfoUtils {
  ...
  public static Set<String> getHasPermissionCompanyIds() {
    ...
    return (auth != null && auth.getHasPermissionCompanyIds() != null)
        ? auth.getHasPermissionCompanyIds()
  ...
  public static Set<String> getHasPermissionProjectIds() {
    ...
    return (auth != null && auth.getHasPermissionProjectIds() != null)
        ? auth.getHasPermissionProjectIds()
        : Collections.emptySet();
  }
  ...
  public static Set<String> getHasPermissionRegionIds() {
    ...
    return (auth != null && auth.getHasPermissionRegionIds() != null)
        ? auth.getHasPermissionRegionIds()
        : Collections.emptySet();
  }
   ...
   * 获取公司级认证信息
   *
   * <p>类型转换为 YdszAuthInfo，以访问行级权限维度ID集合（companyIds/deptIds/projectIds/regionIds）。 使用 {@code
   * instanceof} 类型安全保护，避免 ClassCastException。
   *
  ...
  public static Map<String, Set<String>> getEditableColumnsByTable() {
    ...
    return auth != null && auth.getEditableColumnsByTable() != null
        ? auth.getEditableColumnsByTable()
        : Collections.emptyMap();
  }
   ...
   *
   * <ul>
   *   <li>{@code tenantId} → {@link AuthInfo#getTenantId()}
   *   <li>{@code uniqueId} / {@code userId} → {@link AuthInfo#getUniqueId()}
   *   <li>{@code companyIds} → 逗号拼接的公司 ID 集合
   *   <li>{@code deptIds} → 逗号拼接的部门 ID 集合
   *   <li>{@code projectIds} → 逗号拼接的项目 ID 集合
   *   <li>{@code regionIds} → 逗号拼接的区域 ID 集合
   * </ul>
   *
  ...
  public static String getClaim(String claim) {
    ...
    switch (claim) {
      ...
      default:
        log.debug(
            "未识别的 claim 名: {}，支持的 claim: tenantId, uniqueId, userId, "
                + "companyIds, deptIds, projectIds, regionIds",
            claim);
        return null;
    }
  }
