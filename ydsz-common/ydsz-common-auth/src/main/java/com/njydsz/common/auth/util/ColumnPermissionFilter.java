 ...
 *
 * <p>相比纯反射字段拷贝，Cglib BeanCopier 通过字节码生成直接访问 getter/setter，
 * 性能提升约 3-10 倍（取决于字段数量与 JVM 内联优化程度）。
 *
 * <p><b>核心功能：</b>
 * <ul>
 *   <li>filterColumns：单对象列权限过滤（浅拷贝 + 不可见字段置 null）</li>
 *   <li>filterColumnsList：批量列表列权限过滤</li>
 * </ul>
 *
 * <p><b>降级策略：</b>
 * <p>当 Cglib BeanCopier 创建失败或类无可访问无参构造器时，
 * 自动降级到 {@link BeanUtils#copyProperties} 完成属性拷贝。
 *
 * <p><b>缓存策略：</b>
 * <ul>
 *   <li>BeanCopier 实例按 Class 缓存，避免重复生成字节码</li>
 *   <li>Field[] 按 Class 缓存，避免重复反射扫描类层次结构</li>
 * </ul>
 *
 * <p><b>线程安全：</b>
 * <p>BeanCopier 实例创建后无状态，可安全缓存并发使用；
 * 字段数组缓存为不可变结构，多线程读取安全。
 *
 * @author ydsz-team
 * @since 1.0.0
 *
...
public final class ColumnPermissionFilter {
    ...
    private static final ConcurrentMap<Class<?>, BeanCopier> COPIER_CACHE = new ConcurrentHashMap<>(64);
     ...
     * 过滤单个对象的列权限字段。
     *
     * <p>创建源对象的浅拷贝，然后将不在 visibleColumns 中的字段置为 null。
     * 浅拷贝优先使用 Cglib BeanCopier，失败时降级到 Spring BeanUtils。
     *
     * <p>visibleColumns 为 null 或空时，表示无可见列限制，返回的拷贝中所有字段保留原值。
     *
     * @param source 源对象（不可为 null）
     * @param visibleColumns 可见列名集合（大小写不敏感，自动转为小写比较）；
     *                       null 或空表示无可见列限制
     ...
     * 批量过滤列表中的列权限字段。
     *
     * <p>对 sourceList 中的每个元素执行 {@link #filterColumns}，
     * 返回等大的新列表；列表中的 null 元素在结果中仍为 null。
     *
     ...
     * 创建对象的浅拷贝。
     *
     * <p>优先使用 Cglib BeanCopier（字节码生成，高性能）；
     * 若 BeanCopier 创建失败或类缺少无参构造器，降级到 Spring BeanUtils。
     *
     * @param source 源对象
    ...
    private static void nullifyInvisibleFields(Object target, Set<String> visibleColumns) {
        ...
        Field[] fields = FIELD_CACHE.computeIfAbsent(target.getClass(),
                ColumnPermissionFilter::collectInstanceFields);
