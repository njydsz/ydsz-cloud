 ...
 * <ul>
 *   <li>缓存查询：{@link #getCachedPermissions} 获取已缓存的权限集合</li>
 *   <li>缓存写入：{@link #cachePermissions} 将权限集合写入缓存并维护反向索引</li>
 *   <li>缓存失效：{@link #invalidateByRoleCodes} 按角色编码精确清理，
 *       {@link #invalidateAll} 清理全部缓存</li>
 * </ul>
 *
 * <p>内部维护 {@code roleCode → Set&lt;cacheKey&gt;} 反向索引，
 * 确保即使缓存 Key 使用不可逆的 Hash 算法，也能按角色精确定位并清理缓存条目。
 *
 * <p>使用 ydsz-common-cache 的 {@link Cache} 接口，配合 {@link RolePermissionsExpiry}
 * 实现基于条目内容的动态 TTL（空权限短 TTL 防穿透，非空权限加随机抖动防雪崩）。
 *
 * @author ydsz-team
 * @since 1.0.0
 *
...
public class RolePermissionCacheService {
     ...
     *
     * <p>由于 cacheKey 使用 SHA-256 Hash，无法从 Key 反解角色，
     * 需维护此映射以实现精确的按角色失效。
     */
    ...
    public RolePermissionCacheService(AuthProperties properties) {
        ...
        this.cache = YdszCache.<String, RolePermissions>newBuilder()
     ...
     */
    public void cachePermissions(String cacheKey, Set<String> roleCodes,
     ...
     * 按角色编码集合精确清理缓存。
     *
     * <p>通过 roleCode → cacheKey 反向索引定位所有关联的缓存条目并使其失效，
     * 同时从索引中移除对应的角色映射。
     *
