 ...
 *   <li>缓存时间由 {@code roleColumnCacheSeconds} 配置
...
public class RedisRoleColumnPermissionResolver implements ColumnPermissionResolver {
  ...
  public RedisRoleColumnPermissionResolver(
   ...
   * 解析当前用户的列权限信息。
   *
   * <p>从当前请求上下文获取 token，加载用户信息，解析角色编码， 然后合并所有角色的列权限规则。
   *
   ...
   * 根据角色编码集合解析列权限信息。
   *
   * <p>遍历每个角色编码，从缓存或 Redis 中加载对应的列权限规则， 合并所有角色的可见/可编辑字段集合。
   *
  ...
  private ColumnScopeInfo parseScope(String json) {
    ...
    try {
      ...
      Map<String, Set<String>> visibleColumns =
          parseTableColumns(object, "visibleColumns", "visible");
  ...
  private Map<String, Set<String>> parseTableColumns(
      ObjectNode object, String primaryKey, String fallbackKey) {
