 ...
 * 接口权限校验注解。
 *
 * <p>用于标注在 Controller 或具体接口方法上，表示调用该接口所需的接口权限码。 支持多角色和多权限码的组合校验，并提供 AND/OR 两种校验模式。
 *
 * <p><b>校验链路：</b>
 *
 * <ol>
 *   <li>从请求头 X-Access-Token 获取 token
 *   <li>根据 token 从 Redis 获取用户信息（含 roleCode）
 *   <li>根据 roleCode 加载角色关联的接口权限集合
 *   <li>按注解配置的 mode（AND/OR）进行权限码匹配
 * </ol>
 *
 * <p><b>使用示例：</b>
 *
 ...
 *   <li>示例：{@code sys:user:*} 可匹配 {@code sys:user:add}、{@code sys:user:delete} 等
 * </ul>
 *
 * @author ydsz-team
 * @since 26.09.01
...
public @interface AuthApiPermission {
   ...
   * 必需的接口权限码集合。
   *
   * <p>用户必须拥有列表中指定的权限码才能访问被注解标注的接口。 权限码格式建议采用 {@code 领域:资源:操作} 的命名规范， 如 {@code sys:user:add}
   * 表示系统模块-用户资源-新增操作。
   *
   * @return 接口权限码数组
   */
   ...
   *
   * <p>与 {@link #apiCodes()} 互补：{@code apiCodes} 基于权限码校验， {@code pathPatterns} 基于 URL 路径模式匹配（支持 Ant
   * 风格通配符）。 例如：{@code "/api/user/**"} 匹配所有 /api/user/ 下的接口。
   *
   * <p>当 {@code pathPatterns} 和 {@code apiCodes} 同时配置时， 遵循 {@link #mode()} 指定的 AND/OR 逻辑。
   *
   * @return API 路径模式数组
   */
