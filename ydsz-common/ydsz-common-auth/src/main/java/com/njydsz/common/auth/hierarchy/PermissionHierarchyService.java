 ...
 *
 * <p>支持按租户隔离，每个租户拥有独立的权限层级结构。
 * 使用 {@code Map<tenantId, Map>} 结构实现租户间数据隔离。
 *
...
public class PermissionHierarchyService {
    ...
    /**
     * 默认租户 ID，用于向后兼容全局单例模式。
     * 静态 {@link PermissionHierarchy} 门面委托到此租户。
     */
    public static final String DEFAULT_TENANT_ID = "__DEFAULT__";

    /**
     * 租户 ID → 租户层级数据的映射。
     * 每个租户拥有独立的 parentToChildren 和 childToParents 索引。
     */
     ...
     * 注册权限继承关系。
     *
     * <p>拥有 {@code code} 自动拥有 {@code impliedCodes}。
     * 例如：{@code registerPermission("tenant1", "sys:user", "sys:user:list", "sys:user:add")}
     * 表示拥有 {@code sys:user} 自动拥有 {@code sys:user:list} 和 {@code sys:user:add}。
     *
    ...
    public void registerPermission(String tenantId, String code, String... impliedCodes) {
        if (tenantId == null || code == null || code.isBlank()
                || impliedCodes == null || impliedCodes.length == 0) {
            return;
        }
        String parent = code.trim();
        TenantHierarchy hierarchy = tenantHierarchies.computeIfAbsent(tenantId, k -> new TenantHierarchy());
        Set<String> childSet = hierarchy.parentToChildren.computeIfAbsent(parent, k -> ConcurrentHashMap.newKeySet());
        for (String implied : impliedCodes) {
            if (implied != null && !implied.isBlank()) {
                String child = implied.trim();
                childSet.add(child);
                hierarchy.childToParents.computeIfAbsent(child, k -> ConcurrentHashMap.newKeySet()).add(parent);
            }
        }
    }
     ...
     * 获取指定权限的所有隐含权限（递归）。
     *
     * <p>返回拥有 {@code code} 后自动拥有的所有子权限。
     * 例如：{@code getImpliedPermissions("tenant1", "sys:user")} 返回
     ...
     * @return 拥有权限返回 true
     */
    public boolean hasPermission(String tenantId, Set<String> granted, String required, boolean wildcardEnabled) {
        if (tenantId == null || granted == null || granted.isEmpty() || required == null || required.isBlank()) {
            return false;
        }
        // 1. 直接匹配
     ...
     */
    private void collectChildren(TenantHierarchy hierarchy, String parent, Set<String> result, Set<String> visited) {
