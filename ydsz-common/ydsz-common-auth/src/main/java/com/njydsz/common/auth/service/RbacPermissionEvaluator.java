  ...
  private boolean hasPermission(Set<String> granted, String required) {
    if (hierarchyService != null) {
      return hierarchyService.hasPermission(
          resolveTenantIdOrDefault(), granted, required, properties.isIsWildcardEnabled());
    }
    // 向后兼容：回退到 PermissionUtils 静态调用
    return PermissionUtils.hasPermission(granted, required, properties.isIsWildcardEnabled());
  }
