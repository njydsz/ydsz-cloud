...
public class PageQuery extends BaseQuery {
   ...
   *
   * <p>与 offset 分页互斥：当 cursor 非空时，业务层应按 {@code WHERE id > :cursor} 模式查询，跳过 offset 扫描。
   */
    ...
    if (items != null) {
      ...
      for (OrderItem item : items) {
        if (item != null && item.getColumn() != null) {
          this.orderItems.add(
              OrderItem.of(item.getColumn(), item.getDirection() == OrderItem.Direction.ASC));
        }
      }
    }
    return this;
   ...
   *
   * <p>归一化规则：null 或小于 1 → 返回 1（{@link PageConstants#DEFAULT_PAGE_NUM}）。
   *
   ...
   *
   * <p>委托 {@link PageQueryRiskAssessor#assess(PageQuery)} 执行评估。 纯函数设计，不产生副作用、不做结果缓存。
   *
