package com.njydsz.common.domain.experimental;
 ...
 * 即使底层都是 {@code Long}，也不能混用。
 *
 * <p><b>不可变语义：</b>一旦创建，ID 值不可修改，天然线程安全。
 *
 * <p><b>使用方式：</b>
 * <pre>{@code
 * // 定义实体时
 * public class Project implements EventRegistry {
 *     private TypedId<Project> id;
 *     // ...
 * }
 *
 * // 创建时
 ...
 * }</pre>
 *
 * <p><b>对标：</b>Axon 的 {@code Identifier}、Vlad Mihalcea 的 {@code DomainId}。
 *
 * @param <T>   实体类型（phantom type，仅用于编译期区分，运行时擦除）
 * @param value 底层 ID 值（Long），不可为 null 且必须为正数
 * @author ydsz-team
 * @since 1.2.0
 * @deprecated 当前项目尚未全面实践 DDD 值对象模式，此类保留供未来深度 DDD 采用。
 *             当前实体主键建议使用 {@code Long} 或 {@code String} 原始类型，避免引入额外包装复杂度。
 */
