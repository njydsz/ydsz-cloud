 ...
 * 树节点基础类
 *
 * <p>用于构建树形结构数据，如组织架构、菜单分类、商品分类、区域管理等场景。 支持泛型继承，允许自定义扩展节点属性。
 *
 * <p><b>字段说明：</b>
 *
 * <table>
 ...
 * }</pre>
 *
 * <p><b>泛型安全说明：</b>此类使用了递归泛型模式 {@code TreeNode<T extends TreeNode<T, ID>, ID>}。 内部存在 {@code (T)
 * this} 的未经检查强转。由于 Java 类型擦除机制，{@code ClassCastException} 不会在强转时立即抛出，而是在返回值被使用时才可能触发。使用时必须确保泛型参数
 * {@code T} 与具体子类类型一致，例如 {@code class Menu extends TreeNode<Menu, Long>} 是安全的。
 *
...
public class TreeNode<T extends TreeNode<T, ID>, ID extends Serializable> implements Serializable {
   ...
   * 父节点ID
   *
   * <p>顶级节点的父ID为null。 也可使用特定值（如 0 或 -1）作为根节点标识。
   */
  @Setter private ID parentId;
   ...
   * 排序字段
   *
   * <p>用于同级别节点的排序，数值越小排序越靠前。 支持 null 值，null 值排在最后。
   */
   ...
   * </ul>
   */
  @Builder.Default @Setter private Integer level = ROOT_LEVEL;
