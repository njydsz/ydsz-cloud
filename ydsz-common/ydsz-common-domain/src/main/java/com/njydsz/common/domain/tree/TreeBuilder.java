...
public class TreeBuilder<T extends TreeNode<T, ID>, ID extends Serializable> {
  ...
  /**
   * 将节点挂到父节点下（含子节点列表懒初始化、层级计算与路径自动填充）； 父节点缺失时按根节点处理（多根容错）。
   *
   ...
   *
   * <ul>
   *   <li>{@code parentId == null}：天然根
   *   <li>指定了 {@link #rootId} 且 {@code parentId == rootId}：虚拟根下的根
  ...
  private boolean isRootNode(T node, Set<ID> knownIds) {
    ...
    return parentId == null || Objects.equals(rootId, parentId) || !knownIds.contains(parentId);
  }
   ...
   *
   * <p>适用于不希望或无法让 VO 类继承 {@link TreeNode} 的场景（如已有 VO 结构不便修改）。 通过函数式接口传入 ID/parentId/children/sort
   * 的获取和设置方式。
   *
   ...
   * </ul>
   *
   * <p>若业务模块使用其他根标识（如 {@code -1}），请使用实例方法 {@link #build(Object, List, Comparator)} 并传入自定义 rootId。
   *
   * @param flatList 扁平列表
