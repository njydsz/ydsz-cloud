 ...
 * 查询对象基类型
 *
 * <p>所有查询参数对象的顶层基类，提供序列化支持和通用查询字段。 子类可通过 {@link SuperBuilder} 继承 Builder 能力，实现链式构建查询参数。
 *
 * <p><b>继承体系：</b>
 *
 * <pre>
...
public abstract class BaseQuery implements Serializable {
   ...
   * 模糊搜索关键字
   *
   * <p>用于对多个字段进行模糊匹配搜索。 具体搜索字段由业务子类或 MyBatis XML 定义。
   */
   ...
   * 状态过滤
   *
   * <p>用于过滤数据状态，子类可按需覆盖为具体业务状态枚举值。 默认值为空，由各子类根据业务语义自行定义。
   */
   ...
   * 租户ID
   *
   * <p>用于多租户场景下的数据隔离。 通常在 SaaS 应用中使用，确保不同租户的数据互不干扰。
   */
  ...
  public void validateTimeRange() {
    if (!isValidTimeRange()) {
      throw new IllegalArgumentException(
          "startDateTime must not be after endDateTime: start="
              + startDateTime
              + ", end="
              + endDateTime);
    }
