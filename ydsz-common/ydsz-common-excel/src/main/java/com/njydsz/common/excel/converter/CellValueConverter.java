 ...
 * 单元格值转换器SPI接口
 *
 * <p>定义单元格值的类型转换契约。每个转换器负责一种或多种目标类型的转换。 通过{@link #supports(Class)}声明自己支持的目标类型， 通过{@link
 * #priority()}控制转换器在链中的优先级（数值越小优先级越高）。
 *
 * <h3>扩展方式</h3>
 *
 * <pre>{@code
...
public interface CellValueConverter {
   ...
   * 转换器优先级
   *
   * <p>数值越小优先级越高。默认优先级为100。 自定义转换器可通过覆盖此方法提供更高优先级来替代内置转换器。
   *
   * @return 优先级数值
   */
