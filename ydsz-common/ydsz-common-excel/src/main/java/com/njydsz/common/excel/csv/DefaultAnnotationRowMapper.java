...
public class DefaultAnnotationRowMapper<T> implements TabularRowMapper<T> {
   ...
   * 静态工厂方法。
   *
   * <p>每次调用构造一个新的 Mapper 实例。{@code DefaultAnnotationRowMapper} 不再使用全局缓存，因为泛型类型 {@code Class<T> →
   * DefaultAnnotationRowMapper<T>} 在 Java 类型系统中无法以类型安全的方式缓存（需要未经检查的强制类型转换）。 如需复用 Mapper
   * 实例，由调用方自行持有引用。
   *
   * @param clazz 目标类型
   * @param <T> 目标泛型
