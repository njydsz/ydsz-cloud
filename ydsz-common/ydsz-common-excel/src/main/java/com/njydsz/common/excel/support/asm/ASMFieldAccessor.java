 ...
 *
 * <p>使用 Java 原生 MethodHandle 替代反射，获得接近直接调用的性能。
 * MethodHandle 由 JVM 内联优化，在热点场景下可达到与 ASM 字节码相当的性能，
 * 同时避免了动态类加载带来的 Metaspace 压力与维护成本。</p>
 *
 * <h3>技术原理</h3>
 * <ul>
 *   <li>使用 {@link MethodHandles#unreflectGetter(Field)} / {@link MethodHandles#unreflectSetter(Field)} 获取访问器</li>
 *   <li>实例化回退为 {@code Constructor.newInstance()}</li>
 *   <li>缓存已构建的访问器避免重复创建</li>
 * </ul>
 *
 * <h3>性能对比</h3>
 * <table border="1">
