 ...
 *
 * <p><b>命名澄清</b>：类名中的 {@code ASM} 为历史遗留——本类<b>从不生成或加载字节码</b>，
 * 实际基于 {@code java.lang.invoke.MethodHandle} 实现。保留类名仅为避免公共 API 破坏性变更。
 *
 * <p><b>缓存策略</b>：使用 {@link ClassValue} 实现类级缓存，每个目标类对应一个 {@code FieldGetter[]}/
 * {@code FieldSetter[]} 数组（以字段声明下标为索引），天然线程安全且不受 GC 影响。
 * 此方案替代旧版 {@code ConcurrentHashMap + SoftReference} 缓存，消除了 Full GC 后缓存失效、
 * 以及 check-then-act race condition 导致的重复构建问题。
 *
...
public class ASMFieldAccessor {
  ...
  public static void clearCache() {
    ...
    LOGGER.debug("ASMFieldAccessor: ClassValue caches are cleared via ClassLoader GC");
  }
