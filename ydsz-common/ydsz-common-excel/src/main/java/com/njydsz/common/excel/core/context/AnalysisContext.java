 ...
 * 分析上下文 - 读写过程中状态信息的载体
 *
 * <p>贯穿整个Excel读写流程,用于在各组件间传递状态信息。 主要在监听器回调时使用,让调用方了解当前的读取进度。
 *
 * <h3>用途</h3>
 *
 * <ul>
 *   <li>监听器回调 - 传递读取上下文信息
 *   <li>进度追踪 - currentRow、totalRows
 *   <li>性能监控 - readTime、writeTime
 *   <li>元数据访问 - 提供ReadMetadata/WriteMetadata的访问
 * </ul>
 *
 * @see ReadListener
 * @see ExcelReader
...
public class AnalysisContext {
   ...
   * 将当前行号前进一行。
   *
   * <p>由解析流程在每完成一行回调后调用，是 {@link #getCurrentRow()} 的唯一推进方式。 仅自增不做上界校验，与 {@code totalRows}
   * 无联动，越界与否由调用方保证。
   *
   * <p><b>线程安全性</b>：非原子操作，本上下文假定单线程串行解析， 多线程共享同一实例会丢失计数。
   */
