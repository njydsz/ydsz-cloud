...
public final class ReactiveExcelWriter implements Flow.Publisher<byte[]> {
  ...
  // 共享 OOXML 常量引用（实质指向 OoxmlPackShared 的字节数组，消除跨写入器重复定义）
  private static final byte[] RELS_BYTES = ROOT_RELS;
  private static final byte[] THEME_BYTES = THEME;
  private static final byte[] STYLES_BYTES = STYLES;
