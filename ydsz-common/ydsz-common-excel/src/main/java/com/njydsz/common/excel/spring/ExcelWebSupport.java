 ...
 * Excel Web 导出支持
 *
 * <p>提供直接将 Excel 写入 {@link HttpServletResponse} 的便捷方法，
 * 适用于 Controller 层直接下载场景。仅当 servlet API 在 classpath 上时生效。</p>
 *
 * <h3>使用示例</h3>
 * <pre>{@code
 * @GetMapping("/export")
...
public class ExcelWebSupport {
     ...
     */
    public <T> void write(HttpServletResponse response, Class<T> clazz, List<T> data, String sheetName)
            throws IOException {
     ...
     * @throws IOException 写入失败时抛出
     */
    public <T> void write(HttpServletResponse response, Class<T> clazz, List<T> data, DownloadContext context)
            throws IOException {
