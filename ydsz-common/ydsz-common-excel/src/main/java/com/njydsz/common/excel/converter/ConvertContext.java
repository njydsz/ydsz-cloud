...
public class ConvertContext {
     ...
     *
     * <p><b>注意</b>：构建器内部复用同一个 {@link ConvertContext} 实例，
     * {@link #build()} 返回的正是该实例而非副本。因此一个构建器只应构建一次；
    ...
    public static class Builder {
         ...
         * 设置日期解析/格式化模式。
         *
         * @param dateFormat 日期模式串（如 {@code yyyy-MM-dd}），
         *                   为 {@code null} 时由具体转换器回退到各自的默认模式
         * @return 当前构建器，便于链式调用
         */
         ...
         * 设置数字转换是否采用严格模式。
         *
         * <p>严格模式下无法解析的数值将转换失败并上报错误；
         * 宽松模式下则降级为 {@code null} 或默认值继续处理，适合容忍脏数据的批量导入。
         *
         ...
         * 设置是否采用 1904 日期系统。
         *
         * <p>Mac 版 Excel 早期以 1904-01-01 为纪元，与默认的 1900 系统相差 1462 天。
         * 若源文件工作簿标记了 1904 窗口而此处未开启，所有日期都会整体偏移约 4 年。
         *
