 ...
 *
 * <p>用于监听 Excel 读取过程中的关键事件，包括读取开始、数据行读取、读取结束等。 * 每读取一行数据会触发 {@link #onData} 回调，适合大数据量的流式处理场景。
 *
 * <h3>事件触发顺序</h3>
 *
 * <ol>
 *   <li>{@link #onStart} - 读取开始时调用
 *   <li>{@link #onData} - 每读取一行数据调用一次
 *   <li>{@link #onEnd} - 读取结束时调用
 *   <li>{@link #onError} - 发生错误时调用(可选覆盖)
 * </ol>
 *
 * <h3>使用示例</h3>
 *
...
public interface ReadListener<T> {
   ...
   * 读取开始时调用
   *
   * <p>此时尚未开始解析数据,可在此进行初始化操作, 如创建数据库连接、初始化缓存等。
   *
   * @param context 分析上下文
   */
   ...
   * 每读取一行数据时调用
   *
   * <p>这是最核心的回调方法,每解析一行Excel数据调用一次。 在此进行数据的处理、存储等业务逻辑。
   *
   ...
   * 读取结束时调用
   *
   * <p>无论读取是否成功都会调用。 可在此进行资源清理、统计汇总等操作。
   *
   * @param context 分析上下文
   */
   ...
   * 发生错误时的回调
   *
   * <p>默认实现会抛出RuntimeException。 如需自定义错误处理,可覆盖此方法。
   *
   * @param context 分析上下文
   * @param e 发生的异常
   */
   ...
   * 进度回调
   *
   * <p>在大文件读取时可以提供进度信息,方便UI更新或日志记录。 默认实现为空。
   *
   * @param context 分析上下文
   ...
   * 批量数据回调
   *
   * <p>当使用批量模式读取时,每积累到指定数量调用一次。 适合需要批量处理的场景,如批量入库。
   *
   * <h3>注意</h3>
   *
   * <p>如果使用批量回调,则 {@link #onData} 不会被调用。 默认实现会遍历批量数据逐条调用 {@link #onData}。
   *
   * @param context 分析上下文
   * @param batch 批量数据列表
   */
