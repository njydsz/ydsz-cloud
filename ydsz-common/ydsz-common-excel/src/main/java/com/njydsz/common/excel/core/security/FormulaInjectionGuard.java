 ...
 * 公式注入防护工具类
 *
 * <p>检测并清理可能被利用进行 CSV / Excel 公式注入的危险字符串前缀。 当单元格值以 {@code =}、{@code +}、{@code -}、{@code @} 开头时，
 * 恶意用户可能通过构造特殊值注入公式执行攻击。
 *
 * <p>防护方式：在危险值前添加单引号前缀 ({@code '})，使 Excel 将其识别为文本而非公式。
 *
 * <p>检测前缀可通过系统属性 {@code ydsz.excel.formula-injection-prefixes} 配置， 逗号分隔。默认值为 {@code
 * =,+}（仅拦截最常见的两个前缀），避免对以 {@code -} 或 {@code @} 开头的合法业务数据进行过度转义。
 *
 * @author ydsz-team
 * @since 1.0.0
...
public final class FormulaInjectionGuard {
   ...
   * 解析系统属性中的危险前缀配置。
   *
   * <p>系统属性 {@code ydsz.excel.formula-injection-prefixes} 格式为逗号分隔， 如 {@code "=,+,@",-}。解析失败时回退到默认值
   * {@code ["=", "+"]}。
   *
   * @return 不可变的危险前缀列表
   */
   ...
   * 检测给定值是否为潜在的公式注入
   *
   * <p>当值以 {@code =}、{@code +}、{@code -}、{@code @} 开头时返回 {@code true}
   *
   ...
   * 清理潜在的公式注入
   *
   * <p>如果值被检测为潜在的公式注入，则在值前添加单引号({@code '})前缀， 使Excel将其识别为文本而非公式。否则原样返回。
   *
