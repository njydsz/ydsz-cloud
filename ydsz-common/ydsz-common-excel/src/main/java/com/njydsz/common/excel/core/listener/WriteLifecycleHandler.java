 ...
 * 即可在写入过程的关键节点介入，常用于埋点、审计、动态样式调整、敏感数据脱敏等场景。</p>
 *
 * <p>所有方法均为 {@code default} 空实现，按需覆写即可，无需实现全部钩子。</p>
 *
 * <h3>生命周期顺序</h3>
 * <ol>
 *   <li>{@link #afterWorkbookCreate} — 工作簿初始化完成、首个 Sheet 就绪后</li>
 *   <li>{@link #afterSheetCreate} — 每个 Sheet 创建后（含 newSheet 切 Sheet 场景）</li>
 *   <li>{@link #afterHeaderWrite} — 表头行写入完成后</li>
 *   <li>{@link #afterRowWrite} — 每行数据写入完成后</li>
 *   <li>{@link #afterCellWrite} — 每个单元格写入完成后（高频回调，实现应尽量轻量）</li>
 *   <li>{@link #beforeWorkbookFlush} — 工作簿刷出到输出流之前</li>
 * </ol>
 *
 * <h3>注册示例</h3>
 * <pre>{@code
...
public interface WriteLifecycleHandler {
     ...
     * 工作簿与首个 Sheet 就绪后回调。
     *
     * <p>此时工作簿已初始化、目标 Sheet 已创建，可在此处注入全局 Sheet 级别的配置
     * （如自定义打印设置、文档属性等）。</p>
     *
     ...
     * 每个单元格写入完成后回调。
     *
     * <p>该钩子在每次单元格写入后调用，回调频次极高；实现应避免耗时操作
     * （如数据库访问、远程调用），仅做轻量级内存内变换。</p>
     *
     * @param cell     刚写入的单元格
     ...
     * 工作簿刷出到输出流之前的回调。
     *
     * <p>可在最终刷盘前做尾部校验/修改（如调整共享公式、注入水印标记等）。</p>
     *
