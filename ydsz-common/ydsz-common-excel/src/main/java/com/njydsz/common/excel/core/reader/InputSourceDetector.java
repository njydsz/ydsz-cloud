...
public class InputSourceDetector {
   ...
   * 获取输入流
   *
   * <p>输入流优先级: InputStream > FilePath > File。 根据配置的来源创建合适的输入流。
   *
   * @return 输入流对象
   ...
   * 检测输入流是否为XLSX格式
   *
   * <p>通过读取文件头魔数(PK\x03\x04)判断是否为ZIP格式(XLSX本质是ZIP)。 如果输入流不支持mark/reset，会自动包装为SequenceInputStream。
   *
   * @param is 输入流
