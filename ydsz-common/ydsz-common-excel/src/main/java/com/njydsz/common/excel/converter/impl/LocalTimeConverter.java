 ...
 * LocalTime类型转换器
 *
 * <p>处理目标类型为LocalTime的转换。支持从String等原始值转换。
 *
 * @author ydsz-team
    ...
    try {
      ...
      try {
        DateTimeFormatter formatter =
            FORMATTER_CACHE.computeIfAbsent("HH:mm:ss", DateTimeFormatter::ofPattern);
        return LocalTime.parse(timeStr, formatter);
      } catch (Exception e2) {
        try {
          DateTimeFormatter formatter =
              FORMATTER_CACHE.computeIfAbsent("HH:mm", DateTimeFormatter::ofPattern);
          return LocalTime.parse(timeStr, formatter);
        } catch (Exception e3) {
          return null;
        }
