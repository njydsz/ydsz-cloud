 ...
 * 预计算列属性缓存 - 避免运行时重复计算
 *
 * <p>在写入前预计算所有列的样式、格式等属性， 在写入循环中直接使用，避免运行时的重复查找和转换。
 *
 * <h3>预计算内容</h3>
 *
 * <ul>
 *   <li>CellStyle - 预转换的POI样式对象
 *   <li>列索引 - 预计算的列位置
 *   <li>公式模板 - 预编译的公式模板
 *   <li>日期格式 - 预解析的日期格式器
 * </ul>
...
public class PrecomputedColumnProperties {
   ...
   */
  public PrecomputedColumnProperties(
      List<WriteHeaderProperty> properties, WriteStyleHandler styleHandler) {
