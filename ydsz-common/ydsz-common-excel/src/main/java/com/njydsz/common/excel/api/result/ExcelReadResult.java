...
public class ExcelReadResult<T> {
  ...
  public String toString() {
    return String.format(
        "ExcelReadResult{success=%s, totalRows=%d, elapsedTime=%dms, speed=%.2f rows/s}",
  ...
  public static class Builder<T> {
     ...
     * 设置读取到的数据列表。
     *
     * <p>直接持有入参引用而非拷贝，构建后请勿再修改该列表。 允许传 {@code null}，此时 {@link ExcelReadResult#getOrDefault()}
     * 会兜底为空列表， 但 {@link ExcelReadResult#getData()} 仍会原样返回 {@code null}。
     *
     ...
     * 设置总行数。
     *
     * <p>此值不会由 {@link #data(List)} 自动推导，需调用方显式指定； 若与实际数据条数不一致，{@link
     * ExcelReadResult#getRowsPerSecond()} 的统计结果也会随之偏差。
     *
     ...
     * 设置失败原因。
     *
     * <p>异常在此被封装为结果字段而非向外抛出，调用方通过 {@link ExcelReadResult#getError()} 主动获取，避免异常穿透业务代码。
     *
     ...
     * 绑定底层异步任务。
     *
     * <p>绑定后 {@link ExcelReadResult#get()}、{@link ExcelReadResult#thenAccept} 等方法会转发到该
     * future；未绑定（{@code null}）时这些方法降级为直接返回已有的同步数据， 因此同步结果对象也能安全地按异步风格使用。
     *
