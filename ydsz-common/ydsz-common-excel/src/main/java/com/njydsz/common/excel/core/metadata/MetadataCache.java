 ...
 * 元数据预计算缓存 - 避免重复解析注解
 *
 * <p>通过缓存类级别的注解解析结果，避免每次写入时重复的反射和注解解析开销。 在大数据量写入场景下，可显著降低CPU开销。
 *
 * <h3>缓存内容</h3>
 *
 * <ul>
 *   <li>字段属性列表 - 已排序和过滤的字段元数据
 *   <li>字段访问器映射 - 预计算的FieldGetter
 *   <li>样式配置映射 - 预解析的样式信息
 *   <li>列宽配置 - 预计算的列宽信息
 * </ul>
 *
 * <h3>性能收益</h3>
 *
 * <p>对于相同类型的多次写入，可避免90%以上的注解解析开销， 首次解析后后续写入性能提升约10-20%。
 *
 * <h3>设计模式</h3>
 *
 * <ul>
