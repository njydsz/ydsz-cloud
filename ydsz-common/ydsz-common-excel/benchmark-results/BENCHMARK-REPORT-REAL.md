...
| **引擎版本** | YdszExcel (YDSZ 自研 26.09.x) / EasyExcel 4.0.3 / Apache POI 5.3.0 (XSSF + SXSSF) |
...
| 行数 | YdszExcel avg (ms) | YdszExcel ops/s | EasyExcel 4.0.3 avg (ms) | EasyExcel ops/s | POI XSSF 5.3.0 avg (ms) | POI XSSF ops/s | POI SXSSF 5.3.0 avg (ms) | POI SXSSF ops/s |
|----|----|----|----|----|----|----|----|----|
...
| 行数 | YdszExcel | EasyExcel 4.0.3 | POI XSSF 5.3.0 | POI SXSSF 5.3.0 |
|----|----|----|----|----|
...
> 📌 EasyExcel 文件极小，是因为它默认做了字符串去重（共享字符串表 SST）+ 更高压缩级别。YdszExcel 输出了完整未优化的 SST，后续可优化。
...
| 行数 | YdszExcel avg (ms) | YdszExcel ops/s | EasyExcel 4.0.3 avg (ms) | EasyExcel ops/s | POI XSSF 5.3.0 avg (ms) | POI XSSF ops/s |
|----|----|----|----|----|----|----|
...
| 行数 | YdszExcel | EasyExcel 4.0.3 | POI XSSF 5.3.0 |
|----|----|----|----|
...
- **1k 行**：YdszExcel 碾压全场（11.6 ms），EasyExcel 慢 2.6×，POI XSSF 慢 12×。
- **10k 行**：EasyExcel 反超 YdszExcel（20.3 ms vs 23.1 ms），得益于 EasyExcel 的批量 flush 和游标窗口优化。YdszExcel 仍领先 POI SXSSF 12.5×。
- **100k 行**：EasyExcel 大幅领先（52 ms vs YdszExcel 214 ms），是其内部行批量 + SXSSF 窗口调优的结果。YdszExcel 仍然比 POI SXSSF（1046 ms）快约 5×。
- **写内存**：YdszExcel 100k 仅增 81.5 MB（远低于 SXSSF 的 143 MB 和 XSSF 的未测）。这得益于字节行缓冲 + 流式 ZIP 输出。

### 读取维度

- EasyExcel 10k 行读取仅 **7.3 ms**，性能极好，体现行事件回调 + 高效字段映射的优势。
- YdszExcel 10k 行读取 31.3 ms，约为 EasyExcel 4×，尚可接受，主要开销在 正则解析 SST + byte-tag XML 手工遍历（后续可升级为状态机 XML 解析器）。
...
| 维度 | 表现 | 排名 |
|------|------|------|
| 写入吞吐量（小数据） | ⭐⭐⭐⭐⭐ YdszExcel 最优 | 1. YdszExcel 2. EasyExcel 3. SXSSF 4. XSSF |
| 写入吞吐量（大数据 100k） | ⭐⭐⭐⭐ EasyExcel 最优 | 1. EasyExcel 2. YdszExcel 3. SXSSF |
| 写入内存 | ⭐⭐⭐⭐ YdszExcel & EasyExcel 相当 | 1. YdszExcel 81MB / EasyExcel 96MB / SXSSF 143MB |
| 读取吞吐量 | ⭐⭐⭐⭐ EasyExcel 大幅领先 | 1. EasyExcel 2. YdszExcel 3. XSSF |
| 读取内存 | ⭐⭐⭐⭐ EasyExcel 极低 | 1. EasyExcel 16MB / YdszExcel 76MB / XSSF 164MB |
| 文件体积 | ⭐⭐⭐⭐ EasyExcel 最优 | 1. EasyExcel 266KB / POI-SXSSF 4.6MB / YdszExcel 5.8MB |

---

## 五、后续优化方向

基于本次实测数据，YdszExcel 可行的性能提升点：
...
3. **写入批量 flush**：当前 YdszExcel 在缓冲区满时可能未及时 flush，优化后预期 100k 行写入降到 ~80-100 ms。
...
==============================================================
  ...
  Max Heap   : 8064 MB
  Engines    : YdszExcel (YDSZ) vs EasyExcel 4.0.3 vs POI XSSF/SXSSF 5.3.0
...
*本报告所有 YdszExcel / EasyExcel / POI 的耗时和内存数据均来自同机器、同 JVM 进程的实测。*
