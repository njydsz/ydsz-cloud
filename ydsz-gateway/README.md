...
2. **鉴权拦截**：解析 JWT（Caffeine 防击穿缓存 + 自适应 TTL，验签切出事件循环）、Token 黑名单校验、转发 `X-User-Id` / `X-Tenant-Id` / `X-Trace-Id` 内部头
...
9. **API 版本协商**：`X-API-Version` 头（Path > Header > Query 优先级）+ 弃用版本 Sunset 头
...
- ✅ 缓存：Redis（限流计数 / IP 白名单缓存 / Token 黑名单）
- ❌ 业务 DB：**不持有任何 `ydsz_*` 表**
...
└── src/main/
    ...
    │   ├── GatewayApplication.java              # 启动类 + @EnableConfigurationProperties + Token 黑名单服务注册
    │   ├── config/
    │   │   ├── GatewayConstants.java            # 内部头常量
    │   │   ├── GatewayErrorCode.java            # 网关错误码
    ...
    │   │   ├── GatewayFilterOrder.java          # 过滤器执行顺序统一常量（enum）
    ...
    │   │   ├── AuthGlobalFilter.java            # JWT 解析 + 内部头注入（验签切出事件循环）+ Token 黑名单校验
...
```
  ...
  1        AccessLogGlobalFilter                   结构化访问日志
  3        IpAccessControlFilter                   IP 黑名单 + 白名单
  4        PayloadValidationFilter                 请求体大小校验
  8        WebSocketAuthFilter                     WebSocket 独立鉴权
  10       AuthGlobalFilter                        主鉴权 + 内部头注入 + Token 黑名单
  15       ApiKeyAuthFilter                        API Key 备选认证
  20       GrayLoadBalancerRequestFilter           灰度标识注入
  30       RateLimitFilter                        令牌桶限流
  ...
  ───      ─────────────────────────────────────  ──────────────────────────
  +100     ReactiveLoadBalancerClientFilter       Spring Cloud 负载均衡
```
