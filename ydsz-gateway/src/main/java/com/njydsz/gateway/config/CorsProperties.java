 ...
 * 网关 CORS 跨域配置属性
 *
 * <p>对标 Spring 官方推荐（CorsWebFilter）+ OWASP CORS 安全规范：
 * 使用单一可信 Origin 模式，杜绝使用 {@code *} + 凭据的组合（浏览器规范禁止，
 * 且存在 CSRF / 数据泄露风险）。
 *
 * <p>配置示例（Nacos 共享配置下发）：
 * <pre>{@code
 ...
 *
 * <p><b>凭据模式与通配符互斥校验：</b>
 * 当 {@code allow-credentials=true} 时，{@code allowed-origin} 不得为 {@code *}，
...
public class CorsProperties {
     ...
     *
     * <p>示例：{@code https://ydsz.example.com}。
     * 默认值 {@code https://ydsz.example.com} 仅作为占位，生产环境必须通过 Nacos 配置替换。
     */
