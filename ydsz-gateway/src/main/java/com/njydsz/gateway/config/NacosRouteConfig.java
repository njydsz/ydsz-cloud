 ...
 *
 * <p>当 {@code ydsz.gateway.dynamic-routes.enabled=true} 时， 注册 {@link
 * NacosRouteDefinitionRepository} 为首选路由定义源， 替代 Spring Cloud Gateway 默认的属性路由加载。
 *
 * <p>Java 代码路由（{@link RouteConfig}）作为兜底： 当 Nacos 中无路由配置时，自动回退到 Java 路由。
 *
 * <h3>P2-12 增强项</h3>
 *
...
public class NacosRouteConfig {
  ...
  public RouteDefinitionRepository nacosRouteDefinitionRepository(
    ...
    try {
      ...
      log.warn(
          "[NacosRouteConfig] 未配置托管线程池 nacosRouteListenerExecutor，" + " Nacos 配置变更回调将使用客户端默认线程");
    }
