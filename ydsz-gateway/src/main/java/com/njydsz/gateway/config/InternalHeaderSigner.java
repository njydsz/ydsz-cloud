 ...
 *
 * <ul>
 *   <li>{@code sign(..., nonce)}: 网关为每个请求生成唯一 nonce（UUID）， 纳入 HMAC 签名 payload 后透传给下游
 *   <li>{@code verify(..., nonce)}: 下游服务调用 {@link
 *       com.njydsz.common.safe.crypto.NonceCache#verifyAndConsume(String)} 校验 nonce
 *       是否重复，配合时间戳窗口形成"一次性签名"
 * </ul>
 *
 * <h3>P2-12 已有能力</h3>
 *
 * <ul>
 *   <li>{@code verify()}: 下游服务验证内部头签名的方法
 *   <li>{@code validateTimestamp()}: 时间戳防重放（拒绝超过 N 秒的签名）
 *   <li>{@code slowEquals()}: 恒定时间比较，防计时攻击
 * </ul>
 *
 * @since 1.0.0
 * @author ydsz-team
...
public final class InternalHeaderSigner {
   ...
   * <ol>
   *   <li>校验时间戳窗口（防重放）
   *   <li>用相同密钥重新计算 HMAC-SHA256（含 nonce）
   *   <li>对比签名是否一致（恒定时间比较）
   * </ol>
   *
   * <p><b>nonce 防重放配合：</b>调用方应在签名校验通过后，立即调用 {@link
   * com.njydsz.common.safe.crypto.NonceCache#verifyAndConsume(String)} 校验 nonce
   * 是否首次出现。这样可防止攻击者在时间戳窗口内重放同一签名。
   *
  ...
  public static boolean verify(
    ...
    // 重新计算签名（含 nonce）
    String expectedSig =
        sign(secret, traceId, userId, username, roles, permissions, tsSeconds, nonce);
