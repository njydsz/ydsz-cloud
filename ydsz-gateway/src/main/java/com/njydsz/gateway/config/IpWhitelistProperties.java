 ...
 *
 * <p>通过 {@code @RefreshScope} 支持从 Nacos 动态刷新： 当 Nacos 配置变更触发 {@code RefreshEvent} 时，本 Bean 会被重建，
 * 过滤器在下一次请求读取到最新配置，无需重启服务。
 *
 * <p>对应配置项（ydsz-common.yaml）:
 *
 * <pre>
 * ydsz:
 *   security:
...
public class IpWhitelistProperties {
   ...
   *
   * <p>为空表示不启用白名单功能（即使 enabled=true 也放行所有 IP）。 示例: {@code "192.168.1.0/24,10.0.0.1,172.16.0.0/12"}
   */
