 ...
 *   <li>检查 Token 黑名单（Redis）
 *   <li>将 userId/username/roles/permissions 写入 X-User-* 头透传给下游
 *   <li>注入 {@code X-Internal-Sig} + {@code X-Internal-Ts} + {@code X-Internal-Nonce} 签名头，下游可校验（防伪造
 *       + 防重放）
 * </ol>
 *
 ...
 * 攻击者可伪造内部头绕过下游服务的身份信任。本版本引入独立的 {@code ydsz.gateway.internal-sign-secret} 配置项，与 JWT 密钥隔离。
 *
 * <h3>P0-6: nonce 防重放</h3>
 *
 * <p>历史版本仅有时间戳防重放（60 秒窗口），攻击者可在窗口内重放同一签名。 本版本为每个请求生成唯一 nonce（UUID），纳入 HMAC 签名 payload 后透传给下游。
 * 下游服务调用 {@link com.njydsz.common.safe.crypto.NonceCache#verifyAndConsume(String)} 校验 nonce
 * 是否重复，形成"一次性签名"。
 *
 * @since 1.0.0
 * @author ydsz-team
...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
   ...
   *
   * <p>历史版本硬编码 {@code TOKEN_BLACKLIST_PREFIX = "ydsz:token:blacklist:"} 并直接使用 {@code
   * redisTemplate.hasKey()} 检查黑名单，存在以下问题：
   *
   * <ul>
   *   <li>key 前缀与 common-auth 的 {@code auth:token:blacklist:} 不一致，跨服务黑名单不生效
   *   <li>每个请求都查 Redis，无 Bloom Filter 前置过滤，高 QPS 下 Redis 压力大
   *   <li>Redis key 使用完整 JWT（500+ 字节），浪费内存
   * </ul>
   *
   * <p>改为注入 {@link ReactiveTokenBlacklistService}，复用公共模块能力：
   *
   * <ul>
   *   <li>统一 key 前缀 + SHA-256 摘要
   *   <li>Bloom Filter 前置过滤，减少 90%+ Redis 查询
   *   <li>配置化黑名单开关和 TTL
   * </ul>
   */
   ...
   * 白名单(不校验 Token)。
   *
   * <p>P0-C5 改为精确匹配：仅路径完全相等才放行， 杜绝 {@code /auth/login/../users/list} 等 startsWith 绕过。
   *
   ...
   *
   * <p>网关为每个请求生成唯一 nonce（UUID），存入 NonceCache（TTL=5min）， 同时通过 X-Internal-Nonce 头透传给下游服务。
   *
   * <p>下游服务调用 {@code NonceCache.verifyAndConsume(nonce)} 校验 nonce 是否重复， 形成完整的"一次性签名"防重放机制。
   *
   * <p>网关侧存储 nonce 的目的是：当同一请求被重试/重放时，网关自身也能检测到 nonce 重复（无需等待下游反馈）。
   */
   ...
   * P0-2: 启动时校验内部头签名密钥强度
   *
   * <p>HMAC-SHA256 安全要求密钥长度 ≥ 32 字节（256 bit），且非空、非弱密钥。 校验失败时记录 ERROR 日志（dev/sit 环境允许启动以方便调试，prod
   * 环境建议通过部署校验拦截）。
   */
  @PostConstruct
  private void validateSecret() {
    if (internalSignSecret == null || internalSignSecret.isEmpty()) {
      log.error(
          "[AuthFilter] 内部头签名密钥未配置 (ydsz.gateway.internal-sign-secret)，"
              + "下游服务将无法验证内部头签名，存在身份伪造风险");
      return;
    }
    if (internalSignSecret.length() < MIN_INTERNAL_SECRET_LENGTH) {
      log.error(
          "[AuthFilter] 内部头签名密钥长度不足 (current={}, min={})，" + "HMAC-SHA256 安全要求密钥长度 ≥ 32 字节",
          internalSignSecret.length(),
          MIN_INTERNAL_SECRET_LENGTH);
    }
    // 提示密钥来源（用于运维排查"密钥未分离"问题）
    String source =
        System.getenv("YDSZ_GATEWAY_INTERNAL_SIGN_SECRET") != null
  ...
  /**
   * 核心过滤逻辑：路径规范化 → 链路追踪 → 白名单放行 → Token 校验 → 黑名单检查 → 剥离伪造头 → 注入签名头（含 nonce） → 用户信息透传
   *
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
          ...
          AlertEvent.builder()
              ...
              .labels(
                  Map.of(
                      ...
                      rawPath,
                      "trace_id",
                      existingTraceId != null ? existingTraceId : "n/a"))
              .build());
    ...
    final String traceId =
        (existingTraceId != null && !existingTraceId.isBlank())
            ? existingTraceId
    ...
    return tokenBlacklistService
        ...
        .flatMap(
            blacklisted -> {
              ...
              return withSecurityHeaders(
                  exchange, chain.filter(exchange.mutate().request(mutated).build()));
            });
  }
   ...
   *
   * <p>历史版本在 Gateway 中硬编码 80 行安全头策略计算逻辑，与 common-safe 的 {@code SecurityHeaderFilter}
   * 存在策略不一致风险。现统一委托后，Gateway 侧 仅负责配置转换与 WebFlux 响应写入，策略计算集中在 common-safe。
   *
   * <h3>注入头清单</h3>
   *
   * <ul>
   *   <li>X-Content-Type-Options: nosniff — 阻止 MIME 嗅探
   *   <li>X-Frame-Options: DENY — 阻止点击劫持(Clickjacking)
   *   <li>X-XSS-Protection: 1; mode=block — 启用浏览器 XSS 过滤器
   *   <li>Referrer-Policy: strict-origin-when-cross-origin — 限制 Referrer 泄漏
   *   <li>X-CSRF-Protection: 1 — 声明已启用 CSRF 防护
   *   <li>Content-Security-Policy — 限制脚本/样式/图片/连接来源,防 XSS 注入
   *   <li>Permissions-Policy — 限制浏览器 API 权限(摄像头/麦克风/地理位置等)
   *   <li>P2-12: Cross-Origin-Opener-Policy (COOP) — 防止窗口名攻击
   *   <li>P2-12: Cross-Origin-Embedder-Policy (COEP) — 隔离跨域资源
   *   <li>P2-12: Cross-Origin-Resource-Policy (CORP) — 限制资源跨域访问
   * </ul>
   *
   * <p><b>向后兼容：</b>仍使用 Gateway 原配置类 {@link SecurityHeadersProperties}， 内部转换为 common-safe 的 {@link
   * SecurityHeaderProperties} 后调用。
   *
   * @param exchange 服务器 Web 交换上下文
   ...
   */
  private static SecurityHeaderProperties toCommonProperties(
      SecurityHeadersProperties gatewayProps) {
