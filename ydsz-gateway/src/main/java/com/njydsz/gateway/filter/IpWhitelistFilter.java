 ...
 *
 * <ol>
 *   <li>从 Nacos 配置动态加载 IP 白名单（支持 CIDR）
 *   <li>校验客户端真实 IP（优先从 X-Forwarded-For / X-Real-IP 获取）
 *   <li>白名单为空时默认放行（不启用白名单功能）
 *   <li>支持 CIDR 表示法（如 192.168.1.0/24）
 *   <li>支持单个 IP 精确匹配
 * </ol>
 *
 * <p>执行顺序先于 {@link AuthGlobalFilter}，在认证前即拒绝非法 IP， 避免无效请求消耗 JWT 解析与 Redis 查询资源。
 *
 * @since 1.0.0
 * @author ydsz-team
 */
...
public class IpWhitelistFilter implements GlobalFilter, Ordered {
   ...
   *
   * <p>使用 AtomicReference 保证线程安全的缓存切换。 当 @RefreshScope 刷新 properties 时，下次请求会重新解析。
   */
   ...
   *
   * <p>AuthGlobalFilter 的 order 为 {@code GatewayFilterOrder.AUTH}， 本过滤器设为 {@code
   * GatewayFilterOrder.IP_WHITELIST}，确保认证前完成 IP 拦截。
   *
   * @return 过滤器顺序值
   */
  @Override
   ...
   *
   * <p>当配置字符串与上次相同时直接返回缓存，避免每次请求都执行 split + stream + collect。 当 @RefreshScope 刷新 properties
   * 后，配置字符串会变化，触发重新解析。
   *
   * @param raw 原始配置字符串
   * @return 白名单条目集合
