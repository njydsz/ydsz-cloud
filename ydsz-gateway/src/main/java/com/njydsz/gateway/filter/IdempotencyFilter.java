 ...
 * <h3>工作原理</h3>
 * <ol>
 *   <li>客户端在请求头中携带 {@code Idempotency-Key}（UUID 或业务唯一标识）</li>
 *   <li>网关检查 Redis 中是否已有该 Key 的缓存结果</li>
 *   <li>如果命中缓存：直接返回缓存的响应（短路，不转发下游）</li>
 *   <li>如果未命中：放行请求，响应成功后缓存结果（TTL 可配置）</li>
 * </ol>
 *
 * <h3>Redis 键设计</h3>
 * <pre>
 ...
 *
 * <h3>幂等键生成</h3>
 * <p>使用 {@code userId + idempotencyKey} 的 SHA-256 摘要作为 Redis key，
 * 避免不同用户使用相同 idempotencyKey 时冲突。
 *
 * <h3>执行顺序</h3>
 * <p>{@code HIGHEST_PRECEDENCE + 28}，在限流（+30）之前执行，
 * 确保重复请求在限流检查之前被拦截，避免浪费限流配额。
 *
 * @since 3.7.0
 * @author ydsz-team
 */
...
@ConditionalOnProperty(prefix = "ydsz.gateway.idempotency", name = "enabled", havingValue = "true", matchIfMissing = true)
public class IdempotencyFilter implements GlobalFilter, Ordered {
    ...
    private boolean isIdempotencyMethod(HttpMethod method) {
        return properties.getMethods().stream()
    ...
    private void cacheResponse(String redisKey, byte[] content) {
        ...
        redisTemplate.opsForValue().set(redisKey, bodyStr,
                        Duration.ofSeconds(properties.getKeyTtlSeconds()))
                .onErrorResume(e -> {
