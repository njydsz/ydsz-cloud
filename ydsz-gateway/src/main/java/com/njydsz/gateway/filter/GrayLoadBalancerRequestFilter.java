 ...
 *
 * <ol>
 *   <li>请求头 {@code X-Gray-Tag}(值: {@code gray} / {@code stable})
 *   <li>查询参数 {@code gray=true}(命中则灰度,{@code gray=false} 则稳定)
 *   <li>路径模式 {@code /canary/**}(自动走灰度)
 *   <li>P1-3: 比例灰度 — 当用户身份信息存在时，按 {@code gray-ratio-percent} 自动计算灰度归属
 * </ol>
 *
 * <h3>用户粘性（一致性哈希）</h3>
 *
 * <p>P1-3: 比例灰度模式下，基于 userId（认证用户）或 traceId（未认证）计算哈希， 确保同一用户始终路由到灰度或稳定实例组，避免用户在不同刷新间"跳跃"。
 *
 * <h3>标识写入位置</h3>
 *
 * <ul>
 *   <li>exchange attribute {@code X-Gray-Tag}(供 LoadBalancer 通过 RequestData 读取)
 *   <li>请求头 {@code X-Gray-Tag}(确保下游服务可读取,且不被 AuthGlobalFilter 剥离)
 * </ul>
 *
 * <h3>执行顺序</h3>
 *
 * <p>{@link Ordered#HIGHEST_PRECEDENCE} + 20,晚于 {@link AuthGlobalFilter}(+10), 确保 AuthFilter
 * 完成鉴权后再注入灰度标识,避免白名单请求干扰。
 *
 * @since 1.0.0
 * @author ydsz-team
 */
...
public class GrayLoadBalancerRequestFilter implements GlobalFilter, Ordered {
   ...
   * P1-3: 比例灰度阈值（百分比）。
   *
   * <p>默认 0（禁用比例灰度，仅通过 X-Gray-Tag 显式灰度）。 配置为 10 表示 10% 的流量按比例自动路由到灰度实例。 基于 userId（已认证）或
   * traceId（未认证）的一致性哈希，保证用户粘性。
   */
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
    ...
    if (grayTag != null
      ...
      ServerHttpRequest mutated =
          request.mutate().header(GrayLoadBalancer.GRAY_TAG_HEADER, grayTag).build();
      if (log.isDebugEnabled()) {
        log.debug("[GrayFilter] 路径 {} 注入灰度标识 {} (header 已补写)", request.getURI().getPath(), grayTag);
      }
      return chain.filter(exchange.mutate().request(mutated).build());
    }

    if (log.isDebugEnabled() && grayTag != null) {
      log.debug("[GrayFilter] 路径 {} 灰度标识 {} (header 已存在)", request.getURI().getPath(), grayTag);
    }
   ...
   * P1-3: 基于比例的用户粘性灰度路由。
   *
   * <p>使用 userId（已认证用户）或 traceId（未认证请求）对 100 取模， 若结果小于 grayRatioPercent 则路由到灰度实例。同一用户/traceId
   * 的哈希值稳定， 保证用户不会在多次请求间在灰度和稳定实例组之间"跳跃"。
   *
   * <p>灰色地带：未被灰度命中的用户始终走稳定实例， 灰度实例故障时灰度命中的用户也会受影响（通过 GrayLoadBalancer 降级兜底）。
   *
  ...
  private String resolveRatioGrayTag(ServerHttpRequest request) {
    ...
    if (userId != null && !userId.isEmpty()) {
      ...
      if (hash < grayRatioPercent) {
        if (log.isDebugEnabled()) {
          log.debug(
              "[GrayFilter] 比例灰度命中(用户) userId={} hash={} ratio={}%",
    ...
    if (traceId != null && !traceId.isEmpty()) {
      ...
      if (hash < grayRatioPercent) {
        if (log.isDebugEnabled()) {
          log.debug(
              "[GrayFilter] 比例灰度命中(trace) traceId={} hash={} ratio={}%",
