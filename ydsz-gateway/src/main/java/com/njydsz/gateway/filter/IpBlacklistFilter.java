 ...
 *
 * <ol>
 *   <li>L1: ydsz-common-cache 本地缓存（TTL=10s）— 拦截 99% 的恶意 IP 请求，无网络开销
 ...
 *
 * <ul>
 *   <li>安全系统自动写入（检测到暴力破解 / CC 攻击）
 *   <li>运维通过 Redis CLI 手动写入：{@code SET ydsz:ip:blacklist:1.2.3.4 1 EX 86400}
 *   <li>未来可通过管理后台 API 写入
 * </ul>
 *
 * <h3>执行顺序</h3>
 *
 * <p>{@code HIGHEST_PRECEDENCE + 3}，在 {@link IpWhitelistFilter}(+5) 之前执行， 黑名单优先于白名单检查（恶意 IP
 * 即使在白名单中也应被拒绝）。
 *
 * @since 1.0.0
 * @author ydsz-team
...
public class IpBlacklistFilter implements GlobalFilter, Ordered {
   ...
   *
   * <ul>
   *   <li>{@code fail-open}（默认）：Redis 异常时放行，保证可用性
   *   <li>{@code fail-closed}：Redis 异常时拒绝，保证安全性（生产环境推荐）
   * </ul>
   */
   ...
   *
   * <p>无法获取客户端 IP 时放行；先查 L1 本地缓存（TTL 10s，命中即拒绝或放行）， 未命中再查 Redis 黑名单键并回填本地缓存；Redis 异常时降级放行。
    ...
    if (Boolean.TRUE.equals(cached)) {
      ...
      SentryObservation.alert(
          AlertEvent.builder()
              ...
              .category("security")
              .labels(
                  Map.of("ip", clientIp, "path", request.getURI().getPath(), "cache_level", "L1"))
              .build());
              ...
              if (Boolean.TRUE.equals(blacklisted)) {
                ...
                SentryObservation.alert(
                    AlertEvent.builder()
                        ...
                        .build());
                log.warn(
                    "[IpBlacklist] L2 命中黑名单 ip={} path={}", clientIp, request.getURI().getPath());
                return forbidden(exchange, clientIp);
              }
              return chain.filter(exchange);
            })
        .onErrorResume(
            e -> {
              ...
              if ("fail-closed".equalsIgnoreCase(failMode)) {
                log.warn(
                    "[IpBlacklist] Redis 查询异常，fail-closed 拒绝 ip={} err={}",
                    clientIp,
                    e.getMessage());
                return forbidden(exchange, clientIp);
              }
              log.warn(
                  "[IpBlacklist] Redis 查询异常，fail-open 降级放行 ip={} err={}", clientIp, e.getMessage());
              return chain.filter(exchange);
            });
  }
