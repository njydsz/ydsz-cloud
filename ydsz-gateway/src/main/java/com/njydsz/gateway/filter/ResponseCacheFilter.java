 ...
 * <ul>
 *   <li>仅缓存 GET 请求（幂等安全）
 *   <li>默认 TTL 60 秒，可通过 Nacos 配置按路径覆盖
 *   <li>缓存键 = MD5(method + path + query + userId + Vary headers)
 *   <li>仅缓存 2xx 响应
 *   <li>熔断：Redis 异常时直接放行（降级为无缓存）
 * </ul>
 *
 * <h3>缓存控制</h3>
 *
 * <ul>
 *   <li>{@code Cache-Control: no-store} → 跳过缓存
 *   <li>{@code Cache-Control: no-cache} → 回源并写入新缓存
 *   <li>{@code X-Cache: HIT/MISS} 响应头告知客户端命中状态
 * </ul>
 *
 * <h3>执行顺序</h3>
 *
 * <p>{@code HIGHEST_PRECEDENCE + 25}，在限流(+30)之后、负载均衡(+100)之前。 早于代理请求执行，命中缓存时直接返回，避免建立下游连接。
 *
 * @since 1.0.0 (P2-2)
 * @author ydsz-team
 */
...
public class ResponseCacheFilter implements GlobalFilter, Ordered {
  ...
  private static final Set<String> CACHEABLE_CONTENT_TYPES =
      Set.of("application/json", "application/xml", "text/");

  /** 不可缓存路径前缀 */
  private static final Set<String> UNCACHEABLE_PREFIXES =
      Set.of("/actuator", "/auth", "/realtime", "/ws");
  ...
  private Mono<Void> serveFromCache(ServerWebExchange exchange, String cachedBody) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("[ResponseCache] 缓存命中: path={}", exchange.getRequest().getURI().getPath());
    }
   ...
   * 放行请求并异步缓存响应
   *
   * <p>使用 {@link ServerHttpResponseDecorator} 装饰器模式， 在响应体写入客户端前拦截并异步写入 Redis。
   *
   * @param exchange 服务器 Web 交换上下文
   * @param chain 网关过滤器链
   * @param cacheKey 缓存键
   * @return 放行后的完成信号
   */
  private Mono<Void> passAndCache(
      ServerWebExchange exchange, GatewayFilterChain chain, String cacheKey) {
  ...
  private boolean canCacheResponse(ServerHttpResponse response) {
    ...
    return CACHEABLE_CONTENT_TYPES.stream().anyMatch(contentType::startsWith);
  }
  ...
  private String buildCacheKey(ServerHttpRequest request) {
    ...
    keyBuilder.append(CACHE_KEY_PREFIX).append(method).append(":").append(path);
