 ...
 * <h3>工作机制</h3>
 * <ol>
 *   <li>从 X-User-Roles 头获取用户角色列表</li>
 *   <li>按 Path 匹配找到该路径所需的角色列表</li>
 *   <li>检查用户角色是否满足路径要求（any=OR / all=AND）</li>
 *   <li>不满足返回 403 FORBIDDEN</li>
 * </ol>
 *
 * <h3>路径匹配</h3>
 * <p>使用 Spring {@link AntPathMatcher} 支持 Ant 风格通配符：
 * <ul>
 *   <li>{@code /api/admin/**} — 匹配 /api/admin/ 开头的所有路径</li>
 *   <li>{@code /api/user/*} — 匹配 /api/user/ 下一级路径</li>
 *   <li>{@code /api/**/list} — 匹配任意层级下的 list 端点</li>
 * </ul>
 *
 * <h3>执行顺序</h3>
 * <p>{@code HIGHEST_PRECEDENCE + 12}，在认证（+10）之后、API Key（+15）之前执行。
 * 确保已获取用户角色后再进行权限校验。
 *
 * @since 3.7.0
 * @author ydsz-team
...
@ConditionalOnProperty(prefix = "ydsz.gateway.authorization", name = "enabled", havingValue = "true")
public class AuthorizationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (!authorized) {
            if (properties.isLogFailures()) {
                log.warn("[Authorization] 鉴权失败: path={} requiredRoles={} userRoles={}",
    ...
    private Mono<Void> rejectForbidden(ServerWebExchange exchange, List<String> requiredRoles) {
        ...
        BaseResponse<Void> body = BaseResponse.error(BaseResultCode.FORBIDDEN,
