 ...
 *
 * <p>注册 {@link HealthCheckServiceInstanceListSupplier} 作为实例列表供给者的装饰器, 实现主动健康检查：定期向后端实例发送 HTTP 探活请求，
 * 连续失败 N 次后自动标记为不可用，避免请求被路由到故障实例。
 *
 * <p>配置项：
 *
 * <ul>
 *   <li>{@code spring.cloud.loadbalancer.health-check.initial-delay} — 初始延迟（默认 0s）
 *   <li>{@code spring.cloud.loadbalancer.health-check.interval} — 检查间隔（默认 25s）
 *   <li>{@code spring.cloud.loadbalancer.health-check.path.default} — HTTP 探活路径（默认
 *       /actuator/health）
 *   <li>{@code spring.cloud.loadbalancer.health-check.refetch-instances} — 是否刷新实例列表（默认 true）
 * </ul>
 *
...
public class GrayLoadBalancerConfig {
   ...
   * 注册灰度负载均衡器 Bean
   *
   * <p>Bean 名称 {@code reactorServiceInstanceLoadBalancer} 与 Spring Cloud LoadBalancer 默认实现一致,配合
   * {@code @ConditionalOnMissingBean} 覆盖默认的 RoundRobinLoadBalancer。
   *
   ...
   *
   * <p>以 Nacos 服务发现为基础，叠加 {@link HealthCheckServiceInstanceListSupplier} 装饰器： 网关按 {@code
   * spring.cloud.loadbalancer.health-check.interval}（默认 25s）周期探活， 探活路径默认 {@code
   * /actuator/health}。连续失败实例从候选列表中剔除， 故障发现延迟从 Nacos 心跳（15s）缩短到探活周期级别。
   *
   * <p>说明：spring-cloud-loadbalancer 5.0.2 无静态 {@code decorator} 工厂方法， 改为手动构造并注入 HTTP 探活函数（GET
   * /actuator/health，2xx 视为存活； 探活异常视为不可用，fail-open 语义）。
   *
  ...
  ServiceInstanceListSupplier serviceInstanceListSupplier(
    ...
    WebClient webClient =
        WebClient.builder().codecs(c -> c.defaultCodecs().maxInMemorySize(8 * 1024)).build();
    ...
    log.info(
        "[GrayLB] 已启用主动健康检查（间隔 {}ms，探活路径 /actuator/health）",
