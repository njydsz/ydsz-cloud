...
function createAppRouter(basename?: string) {
  return createRouter({
    history: createWebHistory(basename || '/ydsz-elem-web'),
    routes,
...
async function bootstrap() {
  console.warn('[elem-web] bootstrap');
}

async function mount(props: Record<string, unknown>) {
  console.warn('[elem-web] mount', props);
  ...
  const router = createAppRouter('/ydsz-elem-web');
  initRoutes(router);
  app.use(router);
...
async function unmount() {
  console.warn('[elem-web] unmount');
  app?.unmount();
  app = null;
}

async function update(props: Record<string, unknown>) {
  console.warn('[elem-web] update', props);
}
