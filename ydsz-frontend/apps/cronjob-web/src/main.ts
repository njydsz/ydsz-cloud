...
async function initSharedAuth() {
  ...
  initSharedRequest(
    ...
    async () => {
      console.warn('[cronjob-web] Access token expired, re-authenticating...');
...
function createAppRouter(basename?: string) {
  return createRouter({
    history: createWebHistory(basename || '/ydsz-cron'),
    routes,
...
async function bootstrap() {
  console.warn('[cronjob-web] bootstrap');
}

async function mount(props: Record<string, unknown>) {
  console.warn('[cronjob-web] mount', props);
  ...
  const router = createAppRouter('/ydsz-cron');
  initRoutes(router);
  app.use(router);
...
async function unmount() {
  console.warn('[cronjob-web] unmount');
  app?.unmount();
  app = null;
}
