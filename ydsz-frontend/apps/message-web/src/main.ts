...
function createAppRouter(basename?: string) {
  return createRouter({
    history: createWebHistory(basename || '/ydsz-msg'),
    routes,
...
export async function bootstrap() {
  console.warn('[message-web] bootstrap');
}

export async function mount(props: Record<string, unknown>) {
  console.warn('[message-web] mount', props);
  ...
  const router = createAppRouter('/ydsz-msg');
  initRoutes(router);
  app.use(router);
...
export async function unmount() {
  console.warn('[message-web] unmount');
  app?.unmount();
  app = null;
}

export async function update(props: Record<string, unknown>) {
  console.warn('[message-web] update', props);
}
