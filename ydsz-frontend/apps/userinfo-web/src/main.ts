 ...
 * 应用入口文件，创建并挂载 Vue 实例
 *
 * @path apps\userinfo-web\src\main.ts
 * @author ydsz-team
 * @since 1.0.0
 */
...
function createAppRouter(basename?: string) {
  return createRouter({
    history: createWebHistory(basename || '/ydsz-user'),
    routes,
...
async function bootstrap() {
  console.warn('[userinfo-web] bootstrap');
}

async function mount(props: Record<string, unknown>) {
  console.warn('[userinfo-web] mount', props);
  ...
  await setupSharedAuth('userinfo-web');
  ...
  const router = createAppRouter('/ydsz-user');
  initRoutes(router);
  app.use(router);
...
async function unmount() {
  console.warn('[userinfo-web] unmount');
  app?.unmount();
  app = null;
}

async function update(props: Record<string, unknown>) {
  console.warn('[userinfo-web] update', props);
}
