...
async function initSharedAuth() {
  ...
  initSharedRequest(
    ...
    async () => {
      console.warn('[workflow-web] Access token expired, re-authenticating...');
...
function createAppRouter(basename?: string) {
  return createRouter({
    history: createWebHistory(basename || '/ydsz-flow'),
    routes,
...
async function bootstrap() {
  console.warn('[workflow-web] bootstrap');
}

async function mount(props: Record<string, unknown>) {
  console.warn('[workflow-web] mount', props);
  ...
  const router = createAppRouter('/ydsz-flow');
  initRoutes(router);
  app.use(router);
...
async function unmount() {
  console.warn('[workflow-web] unmount');
  app?.unmount();
  app = null;
}

async function update(props: Record<string, unknown>) {
  console.warn('[workflow-web] update', props);
}
