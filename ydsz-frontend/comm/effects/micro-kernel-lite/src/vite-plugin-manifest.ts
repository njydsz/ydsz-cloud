/**
 * Vite 插件：构建时生成 manifest.json 供 lite-kernel 加载。
 *
 * 约定：子应用必须输出 manifest.json（含 entry、css、版本号），
 * lite-kernel 通过 fetch manifest.json 获取入口信息，免去 HTML entry 解析。
 *
...
/** 创建构建期生成 manifest.json 的 Vite 插件，供 lite-kernel 加载子应用入口与样式 */
