...
const style = computed((): CSSProperties => {
  ...
  return {
    ...
    height: `calc(100% - ${marginTop + (footerEnable && footerFixed ? footerHeight : 0)}px)`,
...
function calcMenuWidthStyle(isHiddenDom: boolean): CSSProperties {
  ...
  // 隐藏 dom 且鼠标悬浮时，也使用 collapseWidth
  else if (isHiddenDom && expandOnHovering.value && !expandOnHover.value) {
    widthValue = `${collapseWidth}px`;
  }
