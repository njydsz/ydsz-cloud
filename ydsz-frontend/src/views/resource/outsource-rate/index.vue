...
import {
  ...
  updateOutsourceRate,
  deleteOutsourceRate,
} from '@/api/resource/outsource-rate'
import type { OutsourceRateVO, OutsourceRateCreateDTO } from '@/api/resource/outsource-rate/types'

const loading = ref(false)
const rateList = ref<OutsourceRateVO[]>([])
const total = ref(0)
...
const form = ref<OutsourceRateCreateDTO & { id?: string }>({
  rateCode: '',
  rateName: '',
...
async function fetchData() {
  loading.value = true
  try {
    const { data } = await pageOutsourceRates({
...
function openEdit(row: OutsourceRateVO) {
...
async function submitForm() {
  ...
  try {
    if (dialogMode.value === 'create') {
      await createOutsourceRate(form.value)
      ElMessage.success('创建成功')
    } else {
      await updateOutsourceRate(editingId.value, form.value)
...
async function handleDelete(row: OutsourceRateVO) {
  await ElMessageBox.confirm(`确认删除外包费率 ${row.rateCode}（${row.rateName}）？`, '提示', {
    type: 'warning',
  })
  await deleteOutsourceRate(row.id)
...
<template>
  <div class="outsource-rate-page">
      ...
      <vxe-table :data="rateList" :loading="loading" border>
        ...
        <vxe-column field="rateCode" title="级别编码" width="90" align="center">
          <template #default="{ row }">
            <el-tag size="small" type="success">{{ row.rateCode }}</el-tag>
          </template>
        </vxe-column>
    ...
    <el-dialog
      ...
      :title="dialogMode === 'create' ? '新增外包职级费率' : '编辑外包职级费率'"
      width="640px"
    >
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="级别编码" required>
              <el-input v-model="form.rateCode" placeholder="如 V5" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="级别名称" required>
              <el-input v-model="form.rateName" placeholder="如 外包高级工程师" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="16">
