 ...
 *
 * 约定：每个子应用挂载时，lite-kernel 在容器上设置 `data-lite-app="app-name"` 属性。
 * 本插件将子应用所有 CSS 规则的选择器前追加 `[data-lite-app="app-name"]`，
 * 使样式仅作用于对应子应用的 DOM 树，防止跨应用样式污染。
 *
 * 仅处理构建产物（Vite build），开发模式下 Vite dev server 自带 HMR 作用域隔离。
 *
 * @path conf/vite-config/src/plugins/postcss-lite-scoped.ts
 * @author ydsz-team
 * @since 3.0.0
 */
...
export interface LiteCssScopeOptions {
  /** 子应用名称（如 'project-web'），作为 data-lite-app 属性值 */
  appName: string;
}
 ...
 * 示例：`.btn { color: red; }` → `[data-lite-app="project-web"] .btn { color: red; }`
 *
 * @param options - 插件选项（应用名）
 * @returns PostCSS 插件实例
 */
export function postcssLiteScopedPlugin(options: LiteCssScopeOptions): AcceptedPlugin {
  const prefix = `[data-lite-app="${options.appName}"]`;

  return {
    postcssPlugin: 'ydsz:lite-css-scoped',
...
postcssLiteScopedPlugin.postcss = true;
