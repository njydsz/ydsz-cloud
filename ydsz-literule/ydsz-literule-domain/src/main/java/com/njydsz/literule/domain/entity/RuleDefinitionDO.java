 ...
 * LiteRule 规则定义 DO
 *
 * <p>映射 ydsz_rule_def 表，存储可配置规则的全部元信息。
 * 封装规则生命周期状态流转行为，避免逻辑散落在 Service 层。
 *
 * <p><b>状态流转：</b>
 * <pre>
...
public class RuleDefinitionDO extends MpBaseEntity<String> {
     ...
     * 生命周期状态
     *
     * <p>数据库存储为字符串，通过 {@link #getStatusEnum()} / {@link #setStatusEnum(RuleStatusEnum)}
     * 提供枚举视图。
     */
    ...
    public void publish(String reviewer) {
        ...
        if (!current.canTransitTo(RuleStatusEnum.PUBLISHED)) {
            throw new IllegalStateException(
                    String.format("规则[%s]当前状态[%s]不允许发布", ruleCode, current));
        }
    ...
    public void disable() {
        ...
        if (current == null) {
            throw new IllegalStateException(
                    String.format("规则[%s]状态为空，无法停用", ruleCode));
        }
        if (!current.canTransitTo(RuleStatusEnum.DISABLED)) {
            throw new IllegalStateException(
                    String.format("规则[%s]当前状态[%s]不允许停用", ruleCode, current));
        }
    ...
    public void revertToDraft() {
        ...
        if (current == null) {
            throw new IllegalStateException(
                    String.format("规则[%s]状态为空，无法回退", ruleCode));
        }
        if (!current.canTransitTo(RuleStatusEnum.DRAFT)) {
            throw new IllegalStateException(
                    String.format("规则[%s]当前状态[%s]不允许回退到草稿", ruleCode, current));
        }
     ...
     * 判断规则是否处于生效窗口内。
     *
     * <p>检查当前时间是否在 {@link #effectiveFrom} 和 {@link #effectiveTo} 之间。
     * 边界情况：effectiveFrom 为 null 表示立即生效，effectiveTo 为 null 表示永不过期。
     *
     * @param now 当前时间
    ...
    public boolean isActive() {
        return RuleStatusEnum.PUBLISHED.name().equals(status)
