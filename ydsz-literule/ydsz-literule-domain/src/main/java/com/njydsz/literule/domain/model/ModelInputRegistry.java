 ...
 *
 * <ul>
 *   <li>线程安全：使用 {@link CopyOnWriteArrayList} + {@link ConcurrentHashMap}，支持运行时动态注册/注销
 *   <li>超时控制：每个 provider 调用受 {@link #timeoutMs} 限制，避免模型延迟拖垮规则引擎
 *   <li>异常隔离：单个 provider 异常/超时不影响其他 provider，符合"规则兜底模型异常"设计
 *   <li>降级策略：{@link #fallbackOnError} 控制模型异常时的行为（继续 vs 中断）
 * </ul>
 *
 * <h3>key 命名规范</h3>
 *
 * <p>{@link #collectAllModelOutputs} 返回的 Map 的 key 统一带 {@code "model."} 前缀， 例如 {@code
 * "model.score"}、{@code "model.probability"}。 规则引擎在合并到 facts 时会转换为嵌套结构 {@code {"model": {"score":
...
public class ModelInputRegistry {
   ...
   * 构造注册中心（降级方案）。
   *
   * <p><b>注意：</b>此构造方法在未配置 common-thread {@code ydsz.thread.pools.modelInput} 时兜底使用，
   * 生产环境应使用 {@link #ModelInputRegistry(long, boolean, ExecutorService)} 注入共同管理的线程池。
   *
