 ...
 *
 * <ul>
 *   <li><b>模式管理</b>：列出 / 注册 / 注销 CEP 模式
 *   <li><b>事件投递</b>：单条 / 批量投递事件，触发模式匹配
 *   <li><b>命中查询</b>：返回最近命中的模式记录（内存暂存，最多 200 条）
 ...
 *
 * <ol>
 *   <li>命中模式后存入 {@code recentHits} 供运维查询（最多 200 条）
 *   <li>将命中事件作为事实投递给规则引擎，触发与 {@code pattern.ruleCode} 关联的规则评估
 * </ol>
 *
...
public class CEPController {
   ...
   * 启动时注册 CEP 命中监听器。
   *
   * <p>命中模式后：① 存入 recentHits 供运维查询；② 将命中事件作为事实 投递给规则引擎，触发与 pattern.ruleCode 关联的规则评估，形成 "CEP 命中 →
   * 规则评估 → 预警"的完整闭环。
   */
  @PostConstruct
  public void init() {
        ...
        hit -> {
          ...
          if (ruleEngine != null && hit.getRuleCode() != null) {
            try {
              ...
              facts.put(
                  "matchedCount",
                  hit.getMatchedEvents() != null ? hit.getMatchedEvents().size() : 0);
