 ...
 *
 * <p>提供 CEP 模式的即时测试能力：注册一个临时模式，按顺序投递测试事件， 收集命中结果后立即注销该模式。用于可视化编辑器中的「测试」按钮。
 *
 * <p><b>拆分说明：</b>本类从原 {@link CEPController} 拆分而来，仅保留模式测试接口。 模式管理 / 事件投递 / 命中查询 / 引擎状态见 {@link
 * CEPController}。
 *
 * <h3>使用场景</h3>
 *
 * <p>用户在可视化编辑器中配置好 CEP 模式后，投递模拟事件流，即时查看是否命中及命中详情， 无需持久化模式定义。测试完成后临时模式自动注销，不影响生产环境。
 *
 * <h3>请求体示例</h3>
 *
 * <pre>
 ...
 *
 * <ol>
 *   <li>解析请求体中的 pattern 和 events
 *   <li>注册临时模式到 CEP 引擎
 *   <li>注册临时监听器收集命中
 *   <li>按顺序投递测试事件
 *   <li>投递完成后注销监听器和临时模式（finally 块保证清理）
 *   <li>返回命中列表、命中数、投递事件数
 * </ol>
 *
 * @author ydsz-team
...
public class CEPTestController {
   ...
   * 测试 CEP 模式（P2-7）。
   *
   * <p>注册一个临时模式，按顺序投递测试事件，收集命中结果后立即注销该模式。 用于可视化编辑器中的「测试」按钮：用户配置好模式后投递模拟事件流，
