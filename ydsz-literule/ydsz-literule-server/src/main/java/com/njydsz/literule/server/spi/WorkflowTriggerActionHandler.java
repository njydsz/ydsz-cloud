 ...
 * 工作流触发动作处理器
 *
 * <p>规则触发后自动启动关联的工作流流程实例，实现规则与工作流深度联动。 依赖 {@code ydsz-workflow-api} 模块提供的 {@link
  ...
  public void handle(List<RuleResult> triggered, RuleContext context) {
    for (RuleResult result : triggered) {
      try {
        ...
        variables.put(
            "ruleSeverity", result.getSeverity() != null ? result.getSeverity().name() : null);
        ...
        log.warn(
            "[LiteRule-Action] 工作流触发失败: ruleCode={}, error={}",
            result.getRuleCode(),
            e.getMessage());
      }
    }
