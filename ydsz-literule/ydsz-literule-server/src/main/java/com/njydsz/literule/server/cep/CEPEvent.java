 ...
 * 复杂事件（CEP）原子事件
 *
 * <p>时间窗口、序列模式、聚合操作都基于事件流处理。 每个事件具备：
 *
 * <ul>
 *   <li>id：事件唯一标识（默认 UUID）
 *   <li>type：事件类型（用于模式匹配，如 "ORDER_CREATED"、"PAYMENT_RECEIVED"）
 *   <li>timestamp：事件时间戳（Instant）
 *   <li>partitionKey：分区键（用于多租户/多用户隔离的窗口）
 *   <li>attributes：事件属性（用于聚合、模式匹配时的字段比较）
 * </ul>
 *
 * @since 1.0.0
 * @author ydsz-team
...
public class CEPEvent implements Serializable {
  ...
  /** 事件唯一 ID */
  @Builder.Default private String id = IdGenerator.nextIdStr();
