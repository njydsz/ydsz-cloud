 ...
 *
 * <ul>
 *   <li>TIME_WINDOW：时间窗口（滚动/滑动）。当窗口内匹配的事件数达到阈值时触发
 *   <li>SEQUENCE：序列模式。按步骤顺序匹配事件 A → B → C，全部在窗口内匹配则触发
 *   <li>AGGREGATE：聚合模式。窗口内对数值属性做 SUM/AVG/COUNT/MIN/MAX，达到阈值时触发
 *   <li>ABSENCE：缺失模式。期望某类型事件在窗口内出现，否则触发（用于告警）
 * </ul>
 *
 * <p>例如：
 *
...
public class CEPPattern implements Serializable {
  ...
  @Builder.Default private int countWindow = 0;
  ...
  public static CEPPattern fromMap(Map<String, Object> map) {
    ...
    if (raw instanceof List<?> list) {
      eventTypes = list.stream().filter(o -> o instanceof String).map(o -> (String) o).toList();
    }
    return CEPPattern.builder()
        ...
        .aggregateFunction(
            map.get("aggregateFunction") != null
                ? AggregateFunction.valueOf((String) map.get("aggregateFunction"))
                : null)
