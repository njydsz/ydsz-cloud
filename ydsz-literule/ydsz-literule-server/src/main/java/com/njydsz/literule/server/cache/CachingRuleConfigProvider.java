 ...
 *
 * <ul>
 *   <li>L1（本地内存）：TTL 60s，最大 1000 条，命中后直接返回
 *   <li>L2（Redis 分布式）：TTL 300s，L1 未命中时查询，命中后回填 L1
 *   <li>DB：L1/L2 均未命中时查询数据库，命中后回填 L1 和 L2
 * </ul>
 *
 * <p>缓存失效策略：
 *
 * <ul>
 *   <li>写操作（save/toggleEnabled/delete）：本地 L1 清除 + Redis 版本号递增
 *   <li>监听 {@link RuleConfigRefreshEvent}：本地 L1 清除
 *   <li>Redis 版本号变更检测：每次 L1 命中前检查版本号，变化则清除 L1（1 秒限流）
 * </ul>
 *
 * <p>降级策略：
 *
...
public class CachingRuleConfigProvider implements RuleConfigProvider {
  ...
  public List<RuleDefinition> loadEnabledRulesByTenant(String tenantId) {
    ...
    return listCache.get(
        cacheKey,
        k -> loadListFromL2OrDb(cacheKey, () -> delegate.loadEnabledRulesByTenant(tenantId)));
  }
   ...
   * 成功提交后才清缓存，回滚时不清（避免回滚后缓存被清空导致额外 DB 回源）。 {@code fallbackExecution=true} 确保非事务上下文（如 Redis
   * 跨节点广播回调线程） 中发布的事件仍能正常触发。
   *
   * <p>由 {@link com.njydsz.literule.server.config.RuleHotReloader} 同源事件触发， 也可由分布式广播器（{@code
   * RedisRuleConfigBroadcaster}）转发跨节点事件触发。
   *
   * @param event 刷新事件
   */
  ...
  public void onConfigRefresh(RuleConfigRefreshEvent event) {
    log.info(
        "[LiteRule-Cache] 收到刷新事件，清除 L1 缓存: type={}, ruleCode={}",
        event.getChangeType(),
        event.getRuleCode());
    invalidateL1();
  }
   ...
   * <ol>
   *   <li>清除本节点 L1（立即生效）
   *   <li>递增 Redis 版本号（其他节点在下次版本检查时清除其 L1）
   * </ol>
   *
