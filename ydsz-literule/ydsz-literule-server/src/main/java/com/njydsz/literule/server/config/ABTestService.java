 ...
 *
 * <ul>
 *   <li>触发状态（triggered）差异
 *   <li>严重度（severity）差异
 *   <li>标题（title）差异
 *   <li>描述（description）差异
 * </ul>
 *
 * <p>典型场景：
 *
 * <ul>
 *   <li>规则条件表达式调整后，验证是否产生意外触发/未触发
 *   <li>严重度表达式调整后，验证严重度变化范围
 *   <li>模板调整后，验证标题/描述输出是否符合预期
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class ABTestService {
  ...
  public ABTestReport test(
    ...
    boolean descriptionChanged =
        !safeEquals(currentResult.getDescription(), candidateResult.getDescription());
    ...
    String summary = String.format("A/B 测试完成：%s", hasDiff ? "检测到差异" : "无差异");

    log.info(
        "[LiteRule] A/B 测试: ruleCode={}, hasDiff={}, triggeredChanged={}, severityChanged={}",
        currentDef.getCode(),
        hasDiff,
        triggeredChanged,
        severityChanged);
