 ...
 *
 * <p>将 {@link RuleChain} 包装为 {@link Rule} 接口实例， 使嵌套子链可作为规则节点被父链引用。
 *
 * <p>评估时委托给内部的 {@link RuleChain#evaluate(RuleContext, ExpressionEngine)}， 返回全部已触发结果中的第一条（作为
 * RuleResult）。 若有多个触发结果，其余通过 {@link RuleResult#getCollectedResults()} 收集。
 *
...
public class ChainAsRule implements Rule {
  ...
  public RuleResult evaluate(RuleContext context) {
    ...
    try {
      ...
      // 隔离失败，不阻断整个 Chain 执行
      log.warn("[LiteRule] ChainAsRule {} 内部 Rule 求值器为 null，已跳过: {}", getCode(), e.getMessage());
      return notTriggered();
    }
  ...
  private RuleResult notTriggered() {
    return RuleResult.builder().ruleCode(getCode()).ruleName(getName()).triggered(false).build();
  }
