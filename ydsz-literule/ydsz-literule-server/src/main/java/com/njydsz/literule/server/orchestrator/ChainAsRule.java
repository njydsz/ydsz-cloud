 ...
 *
 * <p>评估时委托给内部的 {@link RuleChain#evaluate(RuleContextVO, ExpressionEngine)}， 返回全部已触发结果中的第一条（作为
 * RuleResultVO）。 若有多个触发结果，其余通过 {@link RuleResultVO#getCollectedResults()} 收集。
 *
...
public class ChainAsRule implements Rule {
  ...
  public RuleResultVO evaluate(RuleContextVO context) {
    ...
    RuleResultVO main = results.get(0);
