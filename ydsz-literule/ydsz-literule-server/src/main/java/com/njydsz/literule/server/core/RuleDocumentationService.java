 ...
 *
 * <ul>
 *   <li><b>单规则文档</b>：为指定规则生成完整文档（含变更历史、关联规则）
 *   <li><b>批量文档</b>：为全部规则生成文档目录
 *   <li><b>多格式输出</b>：Markdown（默认）、HTML、纯文本
 *   <li><b>条件表达式说明</b>：自动将表达式转换为人类可读描述
 *   <li><b>关联规则识别</b>：自动发现同分类、同互斥组的关联规则
 *   <li><b>效果指标嵌入</b>：如有效果评估数据，嵌入 Precision/Recall/F1
 * </ul>
 *
 * <h3>使用示例</h3>
 *
...
public class RuleDocumentationService {
   ...
   */
  public RuleDocumentationService(
  ...
  public RuleDocumentation generateDocumentation(String ruleCode, String generatedBy) {
    ...
    RuleDocumentation.RuleDocumentationBuilder builder =
        RuleDocumentation.builder()
            ...
            .defaultSeverity(
                rule.getDefaultSeverity() != null ? rule.getDefaultSeverity().name() : null)
  ...
  /** 填充效果指标 */
  private void fillEffectivenessMetrics(
      RuleDocumentation.RuleDocumentationBuilder builder, String ruleCode) {
  ...
  /** 填充变更历史 */
  private void fillVersionHistory(
      RuleDocumentation.RuleDocumentationBuilder builder, String ruleCode) {
    ...
    try {
      ...
      for (RuleVersion v : versions) {
        history.add(
            RuleDocumentation.VersionSummary.builder()
  ...
  /** 填充关联规则 */
  private void fillRelatedRules(
      RuleDocumentation.RuleDocumentationBuilder builder, RuleDefinition rule) {
    try {
      ...
      for (RuleDefinition other : allRules) {
        ...
        if (relationType != null) {
          related.add(
              RuleDocumentation.RelatedRule.builder()
  ...
  private String toMarkdown(RuleDocumentation doc) {
    ...
    sb.append("## 生命周期\n\n");
    sb.append("| 属性 | 值 |\n|---|---|\n");
    sb.append("| 生效时间 | ").append(formatDateTime(doc.getEffectiveFrom())).append(" |\n");
    sb.append("| 失效时间 | ").append(formatDateTime(doc.getEffectiveTo())).append(" |\n");
    sb.append("| 审核人 | ").append(safe(doc.getReviewedBy())).append(" |\n");
    sb.append("| 审核时间 | ").append(formatDateTime(doc.getReviewedAt())).append(" |\n");
    ...
    if (doc.isHasStats()) {
      ...
      sb.append("| 触发率 | ")
          .append(String.format("%.2f%%", doc.getTriggerRate() * 100))
          .append(" |\n");
      sb.append("| 错误率 | ")
          .append(String.format("%.2f%%", doc.getErrorRate() * 100))
          .append(" |\n");
      sb.append("| 平均耗时 | ")
          .append(String.format("%.2f ms", doc.getAvgElapsedMs()))
          .append(" |\n\n");
    }
    ...
    if (doc.isHasEffectivenessMetrics()) {
      sb.append("## 效果指标\n\n");
      sb.append("| 指标 | 值 |\n|---|---|\n");
      sb.append("| Precision（精确率） | ")
          .append(String.format("%.4f", doc.getPrecision()))
          .append(" |\n");
    ...
    if (doc.getVersionHistory() != null && !doc.getVersionHistory().isEmpty()) {
      ...
      for (RuleDocumentation.VersionSummary v : doc.getVersionHistory()) {
        sb.append("| v")
            .append(v.getVersion())
            .append(" | ")
            .append(safe(v.getOperator()))
            .append(" | ")
            .append(safe(v.getChangeDesc()))
            .append(" | ")
    ...
    if (doc.getRelatedRules() != null && !doc.getRelatedRules().isEmpty()) {
      ...
      for (RuleDocumentation.RelatedRule r : doc.getRelatedRules()) {
        sb.append("| `")
            .append(safe(r.getRuleCode()))
            .append("`")
            .append(" | ")
            .append(safe(r.getRuleName()))
            .append(" | ")
            .append(safe(r.getRelationType()))
            .append(" | ")
            .append(r.isEnabled() ? "✅" : "❌")
            .append(" |\n");
      }
    ...
    sb.append("---\n\n");
    sb.append("*文档生成时间：")
        .append(doc.getGeneratedAt())
        .append("，生成人：")
        .append(safe(doc.getGeneratedBy()))
        .append("*\n");
  ...
  private String toHtml(RuleDocumentation doc) {
    ...
    sb.append(
        ".meta{color:#999;font-size:12px;margin-top:40px;border-top:1px solid #eee;padding-top:10px;}\n");
    sb.append("</style>\n</head>\n<body>\n");

    sb.append("<h1>规则文档：").append(escapeHtml(doc.getRuleName())).append("</h1>\n");
    sb.append("<p><strong>规则编码：</strong><code>")
        .append(escapeHtml(doc.getRuleCode()))
        .append("</code></p>\n");
    ...
    sb.append("<h3>条件表达式</h3>\n<pre><code>")
        .append(escapeHtml(doc.getConditionExpression()))
        .append("</code></pre>\n");
    if (doc.getConditionExplanation() != null && !doc.getConditionExplanation().isBlank()) {
      sb.append("<p><strong>说明：</strong>")
          .append(escapeHtml(doc.getConditionExplanation()))
          .append("</p>\n");
    }
    ...
    if (doc.getVersionHistory() != null && !doc.getVersionHistory().isEmpty()) {
      ...
      for (RuleDocumentation.VersionSummary v : doc.getVersionHistory()) {
        sb.append("<tr><td>v")
            .append(v.getVersion())
            .append("</td><td>")
            .append(escapeHtml(safe(v.getOperator())))
            .append("</td><td>")
            .append(escapeHtml(safe(v.getChangeDesc())))
            .append("</td><td>")
            .append(v.getCreatedAt() != null ? v.getCreatedAt().toString() : "")
            .append("</td></tr>\n");
      }
    ...
    if (doc.getRelatedRules() != null && !doc.getRelatedRules().isEmpty()) {
      ...
      for (RuleDocumentation.RelatedRule r : doc.getRelatedRules()) {
        sb.append("<tr><td><code>")
            .append(escapeHtml(r.getRuleCode()))
            .append("</code></td><td>")
            .append(escapeHtml(safe(r.getRuleName())))
            .append("</td><td>")
            .append(escapeHtml(safe(r.getRelationType())))
            .append("</td><td>")
            .append(r.isEnabled() ? "✅" : "❌")
            .append("</td></tr>\n");
      }
    ...
    sb.append("<div class=\"meta\">文档生成时间：")
        .append(doc.getGeneratedAt())
        .append("，生成人：")
        .append(escapeHtml(safe(doc.getGeneratedBy())))
        .append("</div>\n");
    sb.append("</body>\n</html>\n");
    return sb.toString();
  }
  ...
  public String generateIndex(String generatedBy) {
    ...
    for (RuleDefinition rule : allRules) {
      sb.append("| ")
          .append(idx++)
          .append(" | `")
          .append(safe(rule.getCode()))
          .append("`")
          .append(" | ")
          .append(safe(rule.getName()))
          .append(" | ")
          .append(safe(rule.getCategory()))
          .append(" | ")
          .append(safe(rule.getStatus()))
          .append(" | v")
          .append(rule.getVersion())
          .append(" | ")
          .append(rule.isEnabled() ? "✅" : "❌")
          .append(" |\n");
    }
    sb.append("\n---\n\n*文档生成时间：")
        .append(LocalDateTime.now())
        .append("，生成人：")
        .append(safe(generatedBy))
        .append("*\n");
    return sb.toString();
  }
   ...
   * 将条件表达式转换为人类可读的说明
   *
   * <p>简化实现：识别常见的比较运算符和逻辑运算符， 将其转换为中文描述。对于复杂表达式，返回原始表达式。
   *
