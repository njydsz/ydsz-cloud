 ...
 * 规则测试执行器
 *
 * <p>提供规则回归测试能力：执行测试用例，对比实际触发与预期触发， 产出结构化的测试报告。支持 CI/CD 集成和 SDK 嵌入式测试。
 *
 * <h3>基本用法</h3>
 *
 * <pre>{@code
...
public class RuleTestRunner {
  ...
  public RuleTestReport runSuite(String suiteName, List<RuleTestCase> testCases) {
    ...
    List<RuleTestResult> failedResults =
        results.stream().filter(r -> !r.isPassed()).collect(Collectors.toList());
  ...
  public static class TestCaseBuilder {
     ...
     * 设置预期触发的规则编码集合。
     *
     * <p>不传参（{@code .expect()}）表示预期本次评估<strong>无规则触发</strong>， 实际有触发时将判为误触发（false positive）。
     *
     ...
     * 完成当前用例定义并交回套件构建器。
     *
     * <p>幂等：内部通过 {@code added} 标记保证即使用例链中途重复调用 {@code end()}，{@link RuleTestCase} 也只提交一次，避免重复用例。
     *
