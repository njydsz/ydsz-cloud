 ...
 *   <li>{@link #FOR} - 循环执行：遍历集合，对每个元素执行规则链
 *   <li>{@link #WHILE} - 条件循环：条件满足时持续执行规则链
 *   <li>{@link #BREAK} - 终止执行：在循环中终止当前链的执行
 *   <li>{@link #CATCH} - 异常捕获：执行主节点，异常时执行补偿节点（2.0.0）
 *   <li>{@link #RETRY} - 重试执行：执行节点失败时自动重试，达到上限后执行回滚（2.0.0）
 * </ul>
 *
 * @since 1.0.0
 * @author ydsz-team
