 ...
 * LiteExpr AST 节点体系
 *
 * <p>所有 AST 节点实现 {@link ExprNode} 接口，支持 {@link ExprNodeVisitor} 访问者模式。 节点携带行列位置信息，用于错误定位和追踪树可视化。
 *
 * <p>节点类型：
 *
 * <ul>
 *   <li>{@link LiteralNode} — 字面值（数字/字符串/布尔/null）
 *   <li>{@link VariableNode} — 变量引用
 *   <li>{@link BinaryOpNode} — 二元运算（算术/比较/逻辑）
 *   <li>{@link UnaryOpNode} — 一元运算（! / -）
 *   <li>{@link TernaryNode} — 三元条件 (cond ? a : b)
 *   <li>{@link FunctionCallNode}— 函数调用
 *   <li>{@link MemberAccessNode}— 属性访问 a.b.c
 *   <li>{@link IndexNode} — 索引 a[0] / map["key"]
 *   <li>{@link ListNode} — 列表 [1, 2, 3]
 *   <li>{@link MapNode} — 字典 {k: v}
 *   <li>{@link LambdaNode} — Lambda x -> x > 100
...
/** 变量引用节点 */
record VariableNode(String name, int line, int column) implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
 ...
 * @param right 右操作数
 */
record BinaryOpNode(String operator, ExprNode left, ExprNode right, int line, int column)
    implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
 ...
 * @param operand 操作数
 */
record UnaryOpNode(String operator, ExprNode operand, int line, int column) implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
...
record TernaryNode(ExprNode condition, ExprNode thenExpr, ExprNode elseExpr, int line, int column)
    implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
 ...
 */
record FunctionCallNode(String functionName, List<ExprNode> arguments, int line, int column)
    implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
  ...
  public String exprText() {
    return functionName
        + "("
        + arguments.stream().map(ExprNode::exprText).reduce((a, b) -> a + ", " + b).orElse("")
        + ")";
  }
}
 ...
 * @param index 索引表达式
 */
record IndexNode(ExprNode target, ExprNode index, int line, int column) implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
...
/** 列表字面量节点 [1, 2, 3] */
record ListNode(List<ExprNode> elements, int line, int column) implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
  ...
  public String exprText() {
    return "["
        + elements.stream().map(ExprNode::exprText).reduce((a, b) -> a + ", " + b).orElse("")
        + "]";
  }
}

/** 字典字面量节点 {key: value, ...} */
record MapNode(Map<ExprNode, ExprNode> entries, int line, int column) implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
 ...
 * @param body Lambda 体
 */
record LambdaNode(String parameter, ExprNode body, int line, int column) implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
 ...
 */
record TemplateStringNode(List<ExprNode> parts, int line, int column) implements ExprNode {
  @Override
  public <R> R accept(ExprNodeVisitor<R> visitor) {
