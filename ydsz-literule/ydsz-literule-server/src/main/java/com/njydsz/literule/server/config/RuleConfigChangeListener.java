 ...
 *
 * <p>监听规则引擎相关的配置中心变更（{@code ydsz.literule.*}），将 Spring Cloud
 * 配置变更事件桥接到 {@link RuleHotReloader} 执行规则热刷新。
 *
 * <p><b>适用范围：</b>通过 Nacos / Apollo 动态调整规则引擎行为参数
 * （如热加载开关、dry-run 模式、缓存 TTL 等），无需重启服务即可生效。
 *
 * <p><b>设计说明：</b>本监听器实现 {@link ConfigChangeListener} 接口，
 * 由 {@code ydsz-common-config} 的 {@code ConfigChangeBridge} 自动分发配置变更事件，
 * 替代原有的自建配置变更监听机制（保留 {@link RuleHotReloader} 中的业务逻辑）。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
