...
public class LiteRuleProperties {
   ...
   * 断点调试配置（P0-2）
   *
   * <p>控制 {@link com.njydsz.literule.server.core.DefaultBreakpointHook} 的 SUSPEND 超时时间。 默认启用（{@code
   * enabled=true}），超时 60 秒。
   *
   * @since 1.0.0
   */
  ...
  public static class DebugConfig {
     ...
     * SUSPEND 超时时间（秒）
     *
     * <p>断点命中后，规则评估线程最多阻塞该时长等待外部指令（resume/stepOver）， 超时后自动放行，避免调试端断线导致规则评估永久挂起。 默认 60 秒。
     */
    @Min(1)
