 ...
 * 规则执行轨迹记录器（SPI）
 *
 * <p>由消费方提供实现，将执行轨迹写入 {@code ydsz_rule_execution_trace} 表。 literule 模块本身不依赖持久层，通过此接口反转依赖。
 *
 * <p>实现建议：
 *
 * <ul>
 *   <li>使用异步批量写入（如 BlockingQueue + 后台线程）避免阻塞主流程
 *   <li>factsSnapshot/resultSnapshot 应序列化为 JSONB
 *   <li>支持按 traceId/ruleCode/scenario 查询
 *   <li>支持历史 Trace 回放对比
 * </ul>
 *
 * @since 1.0.0
