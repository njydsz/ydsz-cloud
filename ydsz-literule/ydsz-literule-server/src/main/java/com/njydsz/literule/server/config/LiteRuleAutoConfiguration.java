...
public class LiteRuleAutoConfiguration {
     ...
     *   <li>当 circuitBreakerMinEvaluations > 0 时启用 {@link RuleCircuitBreaker}（P2-14: openStateMs 由 {@code ydsz.literule.circuit-breaker-open-state-ms} 配置）</li>
     ...
     * {@code ydsz.literule.cache.enabled=true}（默认 true）时，
     ...
     *   <li>{@code ydsz.literule.cache.l2-enabled=true}（默认 true）</li>
     * </ul>
     * 任一不满足则仅启用 L1。
     *
    ...
    @ConditionalOnBean(RuleConfigProvider.class)
    @ConditionalOnProperty(prefix = "ydsz.literule.cache", name = "enabled", havingValue = "true", matchIfMissing = true)
    @Primary
     ...
     * 文件规则数据源 Bean（P2-3）
     *
     * <p>当 {@code ydsz.literule.file-source.enabled=true} 时自动装配
    ...
    @ConditionalOnMissingBean
    @ConditionalOnProperty(
            prefix = "ydsz.literule.file-source", name = "enabled", havingValue = "true")
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "ydsz.literule.model", name = "mock-enabled", havingValue = "true")
     ...
     *
     * <p>当 {@code ydsz.literule.fact.enabled=true} 时自动装配，聚合所有
     ...
     * 规则动作分发器 Bean（P1-1）
     *
     * <p>当 {@code ydsz.literule.action.enabled=true}（默认 true）时自动装配，
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "ydsz.literule.action", name = "enabled",
            havingValue = "true", matchIfMissing = true)
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "ydsz.literule.action", name = "default-alert-enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     *
     * <p>当 classpath 中存在 {@code CronjobServiceClient}（由 ydsz-cronjob-api 提供）且
     * {@code ydsz.literule.action.cronjob-trigger-enabled=true}（默认 true）时自动装配。
    ...
    @ConditionalOnProperty(prefix = "ydsz.literule.action", name = "cronjob-trigger-enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     *
     * <p>当 classpath 中存在 {@code WorkflowServiceClient}（由 ydsz-workflow-api 提供）且
     * {@code ydsz.literule.action.workflow-trigger-enabled=true}（默认 true）时自动装配。
     * 规则触发后自动启动关联的工作流流程实例。
     *
    ...
    @ConditionalOnProperty(prefix = "ydsz.literule.action", name = "workflow-trigger-enabled",
            havingValue = "true", matchIfMissing = true)
    ...
    @ConditionalOnMissingBean
    @ConditionalOnProperty(
            prefix = "ydsz.literule.effectiveness", name = "enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     * 规则分组并行评估器（P2-3）
     *
     * <p>当 {@code ydsz.literule.performance.parallel-enabled=true} 时装配，
     * 将候选规则按互斥组分组并行评估。
     *
     * @param properties 配置属性
     ...
     * 生成 {@link com.njydsz.literule.api.RetirementSuggestion} 建议列表。
     *
    ...
    @ConditionalOnBean(RuleConfigProvider.class)
    @ConditionalOnProperty(
            prefix = "ydsz.literule.lifecycle", name = "enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     * <p>当存在 {@link RuleAdminService} 时自动装配，
     * 记录规则全生命周期操作的审计日志。
     * 默认使用内存存储，可通过 {@link com.njydsz.literule.server.audit.RuleAuditLogService.AuditLogStore}
     * SPI 提供持久化实现。
     *
