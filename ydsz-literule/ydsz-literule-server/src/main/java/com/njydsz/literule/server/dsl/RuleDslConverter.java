 ...
 * DSL 模型到引擎可执行对象的转换器
 *
 * <p>将 {@link RuleDsl} 转换为引擎可执行的 {@link Rule} 列表与 {@link RuleChain} 列表。 转换过程会根据规则类型（type 字段）分派到对应的
 * Definition + Rule 实现类。
 *
 * <p><b>使用示例</b>：
 *
...
public final class RuleDslConverter {
   ...
   * @return RuleChain 列表
   */
  public static List<RuleChain> toChains(
      RuleDsl dsl, Map<String, Rule> ruleMap, ExpressionEngine evaluator) {
   ...
   * @return RuleChain 实例
   */
  public static RuleChain toChain(
      ChainDslEntry entry, Map<String, Rule> ruleMap, ExpressionEngine evaluator) {
  ...
  public static RuleDefinitionDTO toRuleDefinition(RuleDslEntry entry) {
    ...
    return RuleDefinitionDTO.builder()
        ...
        .canaryRatio(entry.getCanaryRatio() != null ? BigDecimal.valueOf(entry.getCanaryRatio()) : BigDecimal.ZERO)
  ...
  private static ScorecardDefinitionDTO toScorecardDefinition(RuleDslEntry entry) {
    var b = ScorecardDefinitionDTO.builder()
            ...
            .category(entry.getCategory())
            .description(entry.getDescription())
            .baseScore(entry.getBaseScore() != null ? BigDecimal.valueOf(entry.getBaseScore()) : new BigDecimal("100"))
            .redThreshold(entry.getRedThreshold() != null ? BigDecimal.valueOf(entry.getRedThreshold()) : BigDecimal.ZERO)
            .yellowThreshold(entry.getYellowThreshold() != null ? BigDecimal.valueOf(entry.getYellowThreshold()) : BigDecimal.ZERO)
            .minScore(entry.getMinScore() != null ? BigDecimal.valueOf(entry.getMinScore()) : BigDecimal.ZERO)
      ...
      for (RuleDslEntry.FactorDsl f : entry.getFactors()) {
        factors.add(
            ScorecardDefinitionDTO.ScoreFactor.builder()
                ...
                .score(f.getScore() != null ? BigDecimal.valueOf(f.getScore()) : BigDecimal.ZERO)
                .scoreExpression(f.getScoreExpr())
                .weight(f.getWeight() != null ? BigDecimal.valueOf(f.getWeight()) : new BigDecimal("1.0"))
  ...
  private static Rule toScriptRule(RuleDslEntry entry) {
    String language =
        entry.getScriptLanguage() == null ? "groovy" : entry.getScriptLanguage().toLowerCase();
    RuleSeverity defaultSeverity = parseSeverity(entry.getSeverity(), RuleSeverity.INFO);
