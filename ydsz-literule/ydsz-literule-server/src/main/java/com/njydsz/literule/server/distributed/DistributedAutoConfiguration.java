 ...
 *
 * <p><b>依赖说明</b>：使用 ydsz-common-redis 的 {@link RedisHashOps} 和 {@link RedisPubSubOps}，
...
public class DistributedAutoConfiguration {
  ...
  public NodeRegistry redisNodeRegistry(
      RedisHashOps redisHashOps, String nodeId, LiteRuleProperties properties) {
    RedisNodeRegistry registry =
        new RedisNodeRegistry(
            redisHashOps, nodeId, properties.getDistributed().getHeartbeatTimeoutMs());
    // 注册自身
  ...
  /**
   * 规则配置广播器（Redis Pub/Sub 实现）
   *
   * <p>生产环境推荐使用此实现，确保多实例规则配置一致。
   */
  @Bean
  @ConditionalOnBean(RedisPubSubOps.class)
  @ConditionalOnMissingBean(RuleConfigBroadcaster.class)
  public RuleConfigBroadcaster redisRuleConfigBroadcaster(
      RedisPubSubOps redisPubSubOps, String nodeId, ApplicationEventPublisher eventPublisher) {
    RedisRuleConfigBroadcaster broadcaster =
        new RedisRuleConfigBroadcaster(redisPubSubOps, nodeId, eventPublisher);
    // 启动时订阅 Topic
