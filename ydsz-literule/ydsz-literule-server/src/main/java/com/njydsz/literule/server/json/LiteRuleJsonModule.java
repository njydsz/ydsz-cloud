 ...
 *
 * <p>通过 {@link JsonModule.SpringFactory} 机制将 LiteRule 领域的核心 DSL 模型 （{@link RuleDsl} / {@link
 * RuleDslEntry} / {@link ChainDslEntry} / {@link RuleNode} / {@link RuleChain} / {@link
 * RuleChainGraph} / {@link RuleDefinitionDO} / {@link RuleDefinition} / {@link
 * RuleConfigRefreshEvent} / {@link RuleChainType}）的自定义序列化器注册到 YdszJson 引擎， 使规则 DSL 配置文件（JSON/YAML
 * 持久化、分布式缓存广播、审计日志持久化）在全局 {@code toJson/toObject} 路径中统一产出，避免业务代码散落手工拼装 JSON。
 *
 * <p><b>序列化策略要点：</b>
 *
 * <ul>
 *   <li><b>DSL 配置模型</b>（RuleDsl / RuleDslEntry / ChainDslEntry）：序列化为 JSON 时需保证 snake_case 字段名与 YAML
 *       DSL 惯例一致，反序列化时支持类型字段（{@code type}）的 缺省兼容（默认 {@code expression}）。
 *   <li><b>编排运行时模型</b>（RuleNode / RuleChain / RuleChainGraph）：包含递归嵌套结构 （RuleNode 可引用
 *       RuleChain，RuleChain 可引用 RuleNode），序列化需处理循环引用； RuleChain 的内部线程池字段（{@code
 *       WHEN_FALLBACK_EXECUTOR}）为 static transient，应跳过序列化。
 *   <li><b>持久化实体</b>（RuleDefinitionDO）：映射数据库表 ydsz_rule_def， 序列化时需处理 {@link
 *       java.time.LocalDateTime} 时间字段与乐观锁 version 字段的完整输出。
 *   <li><b>领域事件</b>（RuleConfigRefreshEvent）：不可变对象（final 字段）， 序列化应输出 ruleCode / changeType(name) /
 *       operator 三件套。
 *   <li><b>枚举</b>（RuleChainType）：序列化为枚举 name（THEN/WHEN/IF 等），而非中文描述。
 * </ul>
 *
 * <p><b>当前状态：</b>所有序列化器为空占位注册（空实现），实现类留待后续按优先级填充。 已通过 @Component 注册为 Spring Bean，由 {@code
 * JsonAutoConfiguration.JsonConfigBean} 自动发现并注册到 YdszJson 引擎（与 AgentJsonModule / SafeJsonModule
 * 同源机制）。
 *
 * @author ydsz-team
 * @since 1.0.0
