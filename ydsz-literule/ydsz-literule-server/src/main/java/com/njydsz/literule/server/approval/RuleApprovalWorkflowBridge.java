 ...
 *
 * <p>允许 literule 模块的 {@link RuleApprovalService} 在需要时将审批流程 委托到 workflow 模块的统一工作流引擎，避免两套审批系统完全割裂。
 *
 * <h3>设计理念</h3>
 *
 * <p>literule 的 {@link RuleApprovalService} 是规则审批的领域服务， 关注规则状态流转（DRAFT → REVIEW_L1 → REVIEW_L2 →
 * PUBLISHED）。 而 workflow 模块是通用工作流引擎，关注业务流程编排。 两者通过本 SPI 桥接，保持各自的领域聚焦，同时打通审批数据。
 *
 * <h3>实现方</h3>
 *
 * <p>由 project 模块（或专门的 integration 模块）提供实现， 将 literule 审批事件转发到 workflow 引擎：
 *
 * <ul>
 *   <li>提交审核时创建对应的工作流实例
 *   <li>审批通过/驳回时推进/回退工作流节点
 *   <li>委托时更新工作流审批人
 * </ul>
 *
 * <h3>使用方式</h3>
