 ...
 *
 * <ul>
 *   <li>触发 {@link ExpressionValidationService} 的 UNDEFINED_VARIABLE 校验
 *   <li>前端表达式编辑器的自动补全提示
 *   <li>dryRun 时填充默认 facts（基于 sampleValue）
 * </ul>
 *
 * <p>实现方式：
 *
 * <ul>
 *   <li>默认实现 {@link EmptyVariableRegistry}：空实现，向后兼容（不启用 UNDEFINED_VARIABLE 校验）
 *   <li>数据库实现（project 模块）：从 {@code ydsz_rule_variable_def} 表加载
 *   <li>编程式实现：通过 {@link #register(VariableDefinition)} 动态注册
 * </ul>
 *
...
public interface VariableRegistry {
  ...
  default List<VariableDefinition> listByCategory(String category) {
    return listAll().stream().filter(v -> category.equals(v.getCategory())).toList();
  }
   ...
   * 注销变量定义
   *
   * <p>用于编程式动态注销。数据库实现从 DB 删除并刷新缓存； 内存实现从缓存移除。
   *
   * @param name 变量名
   */
