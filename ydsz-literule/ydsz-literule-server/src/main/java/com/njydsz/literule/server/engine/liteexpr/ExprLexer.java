 ...
 *
 * <ul>
 *   <li>整数（十进制 / 十六进制 0x / 八进制 0 / long 后缀 L/l）
 *   <li>浮点数（小数点 / 科学计数法 / BigDecimal 后缀 BD/bd）
 *   <li>字符串（双引号 / 单引号 / 反引号模板字符串）
 *   <li>布尔值 true/false
 *   <li>null
 *   <li>关键字 and/or/not（等价于 &&/||/!）
 *   <li>行注释 // 和块注释 /* *\/
 *   <li>Unicode 标识符（中文变量名）
 *   <li>箭头运算符 ->
 * </ul>
 *
...
public class ExprLexer {
   ...
   * 扫描模板字符串的一部分
   *
   * <p>模板字符串 `Hello ${name}!` 会被拆分为： TEMPLATE_STR("Hello "), TEMPLATE_VAR_START,
   * IDENTIFIER("name"), TEMPLATE_VAR_END, TEMPLATE_STR("!") 最后的 ` 产生一个 STRING 类型的合并结果（在 Parser
   * 中处理）。
   */
  private List<Token> scanTemplatePart() {
    ...
    if (!sb.isEmpty()) {
      tokens.add(
          new Token(
              TokenType.TEMPLATE_STR, sb.toString(), sb.toString(), startLine, startCol, startPos));
    }
    return tokens;
  }
  ...
  private Token scanNumber(int startLine, int startCol, int startPos) {
    ...
    if (pos < length) {
      // BD / bd → BigDecimal
      if (pos + 1 < length
          && (source.charAt(pos) == 'B' || source.charAt(pos) == 'b')
  ...
  private Token scanIdentifier(int startLine, int startCol, int startPos) {
    ...
    return switch (lexeme) {
      case "true" ->
          new Token(TokenType.BOOLEAN, lexeme, Boolean.TRUE, startLine, startCol, startPos);
      case "false" ->
          new Token(TokenType.BOOLEAN, lexeme, Boolean.FALSE, startLine, startCol, startPos);
  ...
  private Token scanOperator(int startLine, int startCol, int startPos) {
    ...
    if (c == '=' && next == '=') {
      advance();
      advance();
      return Token.of(TokenType.EQ, "==", startLine, startCol, startPos);
    }
    if (c == '!' && next == '=') {
      advance();
      advance();
      return Token.of(TokenType.NEQ, "!=", startLine, startCol, startPos);
    }
    if (c == '>' && next == '=') {
      advance();
      advance();
      return Token.of(TokenType.GTE, ">=", startLine, startCol, startPos);
    }
    if (c == '<' && next == '=') {
      advance();
      advance();
      return Token.of(TokenType.LTE, "<=", startLine, startCol, startPos);
    }
    if (c == '&' && next == '&') {
      advance();
      advance();
      return Token.of(TokenType.AND, "&&", startLine, startCol, startPos);
    }
    if (c == '|' && next == '|') {
      advance();
      advance();
      return Token.of(TokenType.OR, "||", startLine, startCol, startPos);
    }
    if (c == '-' && next == '>') {
      advance();
      advance();
      return Token.of(TokenType.ARROW, "->", startLine, startCol, startPos);
    }
        ...
        switch (c) {
          ...
          default ->
              throw new LiteExprException(
                  "无法识别的字符: '" + c + "' (ASCII=" + (int) c + ")", startLine, startCol);
        };
    advance();
  ...
  private void skipWhitespaceAndComments() {
    while (pos < length) {
      ...
      if (Character.isWhitespace(c)) {
        ...
        advance(); // /*
        while (pos < length
            && !(peek() == '*' && pos + 1 < length && source.charAt(pos + 1) == '/')) {
          advance();
        }
        if (pos < length) {
          advance();
