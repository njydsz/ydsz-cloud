 ...
 *
 * <p>当规则引擎服务不可用时降级返回统一错误码
 * ({@link FeignClientConstants#FEIGN_SERVICE_UNAVAILABLE})，
 * 仅记录 WARN 日志，保证调用方主流程不受影响。
 *
 * <p>注意：必须返回 error 而非 success(emptyList)，
 * 否则调用方通过 {@code isSuccess()} 检查会误判为评估成功（无规则触发）。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
    ...
    public LiteRuleClient create(Throwable cause) {
        ...
        return new LiteRuleClient() {
            ...
            public BaseResponse<List<RuleResult>> dryRun(String ruleCode, Map<String, Object> facts) {
                ...
                return BaseResponse.error(FeignClientConstants.FEIGN_SERVICE_UNAVAILABLE, "规则引擎服务不可用");
            }
