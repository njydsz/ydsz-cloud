 ...
 *
 * <ul>
 *   <li>{@link #globalMetrics}：全局汇总指标（所有规则合并后的 TP/FP/FN/TN）
 *   <li>{@link #perRuleMetrics}：按规则编码分组的明细指标
 *   <li>{@link #poorRules}：效果较差的规则列表（F1 < 0.60），按 F1 升序排列
 *   <li>{@link #topRules}：效果最好的规则列表（F1 ≥ 0.75），按 F1 降序排列
 *   <li>{@link #lowDataRules}：反馈样本不足的规则列表（< 30 条）
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class EffectivenessReport implements Serializable {
  ...
  @Builder.Default private List<RuleEffectivenessMetrics> poorRules = new ArrayList<>();

  /** 效果最好的规则列表（F1 ≥ 0.75），按 F1 降序排列 */
  @Builder.Default private List<RuleEffectivenessMetrics> topRules = new ArrayList<>();

  /** 反馈样本不足的规则列表（totalSamples < 30） */
  @Builder.Default private List<String> lowDataRules = new ArrayList<>();
