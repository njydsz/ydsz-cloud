...
public interface ExpressionEngine {
   ...
   * 求值表达式（通用类型）
   *
   * <p>返回 {@link Object}，由调用方根据业务语义自行类型转换。 早期版本使用 {@code <T> T eval(...)} 泛型签名，但表达式求值本质是动态类型，
  ...
  default ExpressionValidationResult validateDetailed(String expression) {
    ...
    return ExpressionValidationResult.fail(
        expression, ExpressionValidationResult.ErrorType.UNKNOWN, "表达式非法", elapsed);
  }
   ...
   *
   * <p>对标 QLExpress4 的 ExpressionTrace 能力，将表达式执行过程转换为计算树。 用于规则归因分析、短路排查和中间结果可视化。
   *
