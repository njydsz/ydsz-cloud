 ...
 *
 * <p>对应 {@code remi_job_task} 表，提供按 logId 查询子任务、统计指定状态子任务数、
...
public interface JobTaskMapper extends BaseMapper<JobTask> {
     ...
     * @return 子任务数
     */
    @Select("SELECT COUNT(1) FROM remi_job_task "
     ...
     * @return 受影响行数
     */
    @Update("UPDATE remi_job_task SET exec_node_id = #{execNodeId}, updated_at = #{now} "
            + "WHERE id = #{id} AND deleted = 0")
     ...
     * @return 实际删除条数
     */
    @Delete("DELETE FROM remi_job_task "
            + "WHERE id IN ("
            + "  SELECT id FROM remi_job_task "
