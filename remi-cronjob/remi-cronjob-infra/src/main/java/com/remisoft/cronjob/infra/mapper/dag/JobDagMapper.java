 ...
 * 任务 DAG Mapper
 *
 * <p>对应数据表 <code>remi_job_dag</code>。
 * <p>DAG 把多个 Job 编排为有向无环图，支持串行/并行/条件分支，是复杂任务的编排核心。
 *
 * <p><b>主要索引：</b>
 * <ul>
 *   <li>uk_dag_code — DAG 编码唯一索引</li>
 ...
 * @since 1.0.0
 *
 * @see com.remisoft.cronjob.domain.entity.dag.JobDag DAG 实体
...
public interface JobDagMapper extends BaseMapper<JobDag> {
    ...
    @Select("SELECT id, dag_key, dag_name, dag_definition, status, trigger_type, cron_expression, "
            ...
            + "FROM remi_job_dag WHERE dag_key = #{dagKey} AND deleted = 0")
    ...
    @Select("SELECT id, dag_key, dag_name, dag_definition, status, trigger_type, cron_expression, "
            ...
            + "       fire_count, success_count, fail_count, version, "
            + "       created_by, created_at, updated_by, updated_at, deleted, tenant_id "
            + "FROM remi_job_dag WHERE status = 'ENABLED' AND deleted = 0")
    List<JobDag> selectEnabledDags();

    /**
     * 更新 DAG 触发统计（fire_count +1，last_fire_time 更新）。
     */
    @Update("UPDATE remi_job_dag SET fire_count = fire_count + 1, last_fire_time = #{fireTime}, "
     ...
     */
    @Update("UPDATE remi_job_dag SET "
