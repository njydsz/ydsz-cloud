 ...
 * 任务定义 Mapper
 *
 * <p>对应 remi_job 表，提供按 jobKey 查询、启动加载 NORMAL 任务、统计字段更新。
 *
 * @author remi-team
 * @since 1.0.0
 */
...
public interface JobMapper extends BaseMapper<Job> {
     ...
     */
    @Update("UPDATE remi_job SET consecutive_fail_count = 0 WHERE id = #{id}")
    int resetConsecutiveFail(@Param("id") String id);

    /**
     * P1-6: 递增连续失败计数（任务执行失败时调用）。
     */
    @Update("UPDATE remi_job SET consecutive_fail_count = consecutive_fail_count + 1 WHERE id = #{id}")
    int incrementConsecutiveFail(@Param("id") String id);

    /**
     * P1-6: 标记任务为 AUTO_PAUSED（熔断自动暂停）。
     */
    @Update("UPDATE remi_job SET status = 'AUTO_PAUSED' WHERE id = #{id} AND status = 'NORMAL'")
    int markAutoPaused(@Param("id") String id);

    /**
     * P1-6: 查询连续失败计数。
     */
    @Select("SELECT consecutive_fail_count FROM remi_job WHERE id = #{id}")
     ...
     * @return 受影响行数
     */
    @Update("UPDATE remi_job SET status = 'NORMAL', "
