 ...
 * 任务执行节点 Mapper
 *
 * <p>对应数据表 <code>remi_job_node</code>。
 ...
 * @since 1.0.0
 *
 * @see com.remisoft.cronjob.domain.entity.job.JobNode 节点实体
 * @see com.remisoft.cronjob.server.service.JobNodeService 节点 Service
...
public interface JobNodeMapper extends BaseMapper<JobNode> {
     ...
     * @return 受影响行数
     */
    @Update("UPDATE remi_job_node SET status = 'OFFLINE' " +
     ...
     */
    @Select("SELECT node_id FROM remi_job_node " +
            "WHERE status = 'ONLINE' AND last_heartbeat < #{cutoff}")
     ...
     * 避免 remi_job_node 表无限膨胀。
     *
     * @param cutoff 心跳截止时间（早于此时间的离线节点将被删除）
     * @return 受影响行数
     */
    @Delete("DELETE FROM remi_job_node " +
