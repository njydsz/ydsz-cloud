 ...
 * 任务 DAG 节点实例 Mapper
 *
 * <p>对应数据表 <code>remi_job_dag_node_instance</code>。
 ...
 * @since 1.0.0
 *
 * @see com.remisoft.cronjob.domain.entity.dag.JobDagNodeInstance 节点实例实体
...
public interface JobDagNodeInstanceMapper extends BaseMapper<JobDagNodeInstance> {
    ...
    @Select("SELECT id, dag_instance_id, dag_id, job_id, job_key, node_status, log_id, "
            ...
            + "FROM remi_job_dag_node_instance "
     ...
     */
    @Update("UPDATE remi_job_dag_node_instance SET node_status = 'RUNNING', started_at = #{startedAt}, "
     ...
     */
    @Update("UPDATE remi_job_dag_node_instance SET node_status = #{finalStatus}, "
     ...
     */
    @Update("UPDATE remi_job_dag_node_instance SET node_status = 'SKIPPED', "
            + "       updated_at = CURRENT_TIMESTAMP "
            + "WHERE id = #{id} AND node_status = 'PENDING' AND deleted = 0")
     ...
     */
    @Update("UPDATE remi_job_dag_node_instance SET node_status = 'PENDING', "
