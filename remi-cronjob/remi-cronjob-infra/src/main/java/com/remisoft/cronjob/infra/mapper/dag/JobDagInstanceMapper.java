package com.remisoft.cronjob.infra.mapper.dag;
 ...
 * 任务 DAG 实例 Mapper
 *
 * <p>对应数据表 <code>remi_job_dag_instance</code>。
 * <p>DAG 实例记录一次完整执行的节点状态、上下文、耗时，是 DAG 运维的事实表。
 *
 * <p><b>主要索引：</b>
 * <ul>
 *   <li>uk_instance_id — 实例 ID 唯一索引</li>
 *   <li>idx_dag_id — DAG 维度查询索引</li>
 ...
 * @since 1.0.0
 *
 * @see com.remisoft.cronjob.domain.entity.dag.JobDagInstance DAG 实例实体
 * @see com.remisoft.cronjob.server.service.JobDagService DAG Service
...
public interface JobDagInstanceMapper extends BaseMapper<JobDagInstance> {
     ...
     */
    @Update("UPDATE remi_job_dag_instance SET status = #{toStatus}, updated_at = CURRENT_TIMESTAMP "
     ...
     */
    @Update("UPDATE remi_job_dag_instance SET status = 'RUNNING', started_at = #{startedAt}, "
     ...
     */
    @Update("UPDATE remi_job_dag_instance SET status = #{finalStatus}, finished_at = #{finishedAt}, "
     ...
     */
    @Update("UPDATE remi_job_dag_instance SET context_json = #{contextJson}, "
            + "       updated_at = CURRENT_TIMESTAMP "
     ...
     */
    @Update("UPDATE remi_job_dag_instance "
     ...
     */
    @Select("SELECT COUNT(1) FROM remi_job_dag_instance "
     ...
     */
    @Update("UPDATE remi_job_dag_instance SET status = 'PAUSED', updated_at = CURRENT_TIMESTAMP "
            + "WHERE id = #{instanceId} AND status = 'RUNNING' AND deleted = 0")
     ...
     */
    @Update("UPDATE remi_job_dag_instance SET status = 'RUNNING', updated_at = CURRENT_TIMESTAMP "
     ...
     * @return 受影响行数
     */
    @Update("UPDATE remi_job_dag_instance SET status = 'CANCELED', finished_at = #{finishedAt}, "
