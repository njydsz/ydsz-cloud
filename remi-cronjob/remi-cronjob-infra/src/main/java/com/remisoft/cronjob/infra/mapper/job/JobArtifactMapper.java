 ...
 * 任务产物 Mapper
 *
 * <p>对应数据表 <code>remi_job_artifact</code>。
 ...
 * @since 1.0.0
 *
 * @see com.remisoft.cronjob.domain.entity.job.JobArtifact 产物实体
 * @see com.remisoft.cronjob.server.service.JobArtifactService 产物 Service
...
public interface JobArtifactMapper extends BaseMapper<JobArtifact> {
     ...
     * 清理过期产物（硬删除）。
     */
    @Delete("DELETE FROM remi_job_artifact WHERE expire_at IS NOT NULL AND expire_at < #{before} LIMIT #{limit}")
