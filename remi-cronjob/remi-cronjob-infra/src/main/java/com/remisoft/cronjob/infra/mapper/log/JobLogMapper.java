 ...
 * 任务执行日志 Mapper
 *
 * <p>对应数据表 <code>remi_job_log</code>。
 ...
 * @since 1.0.0
 *
 * @see com.remisoft.cronjob.domain.entity.log.JobLog 执行日志实体
...
public interface JobLogMapper extends BaseMapper<JobLog> {
     ...
     *
     * <p>JOIN remi_job 取 timeout_ms，筛选 {@code log.status='RUNNING'}
    ...
    @Select("SELECT l.id, l.job_id, l.job_key, l.start_time, l.end_time, l.duration_ms, "
            ...
            + "FROM remi_job_log l "
            + "INNER JOIN remi_job j ON j.id = l.job_id AND j.deleted = 0 "
     ...
     * 现改为直接检查 remi_job_log.is_slow 字段, 消除独立表关联。
     *
     ...
     *
     * <p>用于 {@link com.remisoft.cronjob.server.core.dispatch.FailoverScanner} 扫描
     * 有 RUNNING 任务但可能已下线的节点。对比在线节点列表后找出下线节点。
     *
     * @return 有 RUNNING 任务的节点 ID 列表（去重）；无记录时返回空列表
     */
    @Select("SELECT DISTINCT exec_node_id FROM remi_job_log "
     ...
     * 避免大事务锁表。由 {@link com.remisoft.cronjob.server.core.cleaner.LogCleaner} 循环调用直至无数据。
     *
     * @param before 过期分界时间（此时间之前的记录将被删除）
     * @param limit  单批最多删除条数
     * @return 实际删除条数
     */
    @Delete("DELETE FROM remi_job_log "
            + "WHERE id IN ("
            + "  SELECT id FROM remi_job_log "
