 ...
 *
 * <p>展示 {@link MapReduceProcessor} 的标准用法：Root 任务按租户拆分子任务，
 * 子任务独立统计，reduce 汇总所有子任务结果。
 *
 * <p><b>使用方式：</b>在任务配置中注册本类为 MapReduce 类型任务，
 * 由 {@code MapTaskExecutor} 按分片调度执行：
 * <ul>
 *   <li>Root 上下文：{@link #process(MapContext)} 中 {@code ctx.isRoot()==true}，
 *       通过 {@code addSubTask(taskName, taskParams)} 声明子任务</li>
 *   <li>子任务：{@code ctx.isRoot()==false}，各自执行统计并写入 {@code ctx.getResults()}</li>
 *   <li>汇总：{@link #reduce(List, MapContext)} 聚合所有子任务结果</li>
 * </ul>
 *
 * <p><b>业务场景：</b>按租户维度统计各租户任务执行量，供报表/配额分析使用。
 * 实际业务中可按需改造（如按城市、按部门、按日期分片）。
 *
 * @author remi-team
 * @since 1.0.0
 */
