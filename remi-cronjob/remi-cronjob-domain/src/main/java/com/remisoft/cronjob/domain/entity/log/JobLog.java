package com.remisoft.cronjob.domain.entity.log;
 ...
 * 任务执行日志
 *
 * <p>对应 remi_job_log 表，记录每次任务执行的开始/结束/耗时/状态/结果。
 *
...
public class JobLog extends MpBaseEntity<String> {
     ...
     * 供 {@link com.remisoft.cronjob.server.core.dispatch.TimeoutMonitor} 超时后
     ...
     * 供 JobNodeReaper 故障转移时重建分片锁 key（{@code remi:job:lock:{jobKey}:shard:{shardIndex}}）。
     */
     ...
     * 慢任务阈值快照（毫秒）。
     *
     * <p>执行时从 {@code remi_job.slow_threshold_ms} 快照到日志记录，
