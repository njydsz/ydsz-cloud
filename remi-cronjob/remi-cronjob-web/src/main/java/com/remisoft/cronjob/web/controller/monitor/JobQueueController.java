package com.remisoft.cronjob.web.controller.monitor;
 ...
 * <h3>核心能力</h3>
 * <ul>
 *   <li>{@link #getQueueStatus} - 查询执行线程池实时状态（活跃数/队列大小/已完成数/拒绝数）</li>
 * </ul>
 *
 * <h3>核心指标说明</h3>
 * <ul>
 *   <li>{@code activeCount} - 当前正在执行的任务数（线程池活跃线程数）</li>
 *   <li>{@code poolSize} - 线程池当前线程数（含空闲线程）</li>
 *   <li>{@code maximumPoolSize} - 线程池最大线程数（容量上限）</li>
 *   <li>{@code queueSize} - 工作队列中等待执行的任务数</li>
 *   <li>{@code queueRemainingCapacity} - 工作队列剩余容量</li>
 *   <li>{@code completedTaskCount} - 历史已完成任务总数（线程池生命周期内累计）</li>
 *   <li>{@code taskCount} - 历史提交任务总数</li>
 * </ul>
 *
 * <h3>使用建议</h3>
 * 运维可基于 {@code queueSize / queueRemainingCapacity} 评估积压情况；
 * 基于 {@code activeCount / maximumPoolSize} 评估负载率；
 * 当 {@code queueSize} 持续接近 {@code queueRemainingCapacity} 时应触发扩容告警。
 *
 * <h3>安全</h3>
 * 接口加 {@link AuthApiPermission} 权限控制（{@link PermissionCodes#CRONJOB_STATS_VIEW}），
 * 只读不写，无需幂等/限流/审计。
 *
 * @author remi-team
 * @since 1.0.0
...
public class JobQueueController {
     ...
     * 查询执行队列实时状态。
     *
     * <p>读取 {@link DefaultTaskDispatcher#getTaskExecutorPool} 线程池的所有指标，
     * 转换为前端友好的 Map 返回。当 TaskDispatcher 或 ThreadPoolExecutor 未初始化时
     * 返回空 Map（避免 null pointer）。
     *
