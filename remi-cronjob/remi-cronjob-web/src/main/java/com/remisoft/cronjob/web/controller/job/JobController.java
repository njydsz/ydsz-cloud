...
import com.remisoft.cronjob.domain.dto.job.JobBatchDTO;
 ...
 * <h3>架构位置</h3>
 * <pre>
 *   前端 (PC Web) → remi-gateway → remi-cronjob-web (本 Controller)
 *                                           ↓
 *                                   remi-cronjob-server (JobService)
 *                                           ↓
 *                                   remi-cronjob-infra (MyBatis-Plus Mapper)
 * </pre>
 *
 * @author remi-team
 * @since 1.0.0
...
public class JobController {
    ...
    @Idempotent(key = "remi:cronjob:JobController:create:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:cronjob:JobController:update:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:cronjob:JobController:batchDelete:lock", ttlSeconds = 5)
     ...
     * 关联的 DAG 关系（{@code remi_job_relation}）由数据库外键级联删除。
     *
     * @param id 任务 ID
     * @return 统一响应结果
     */
    ...
    @AuthApiPermission(apiCodes = PermissionCodes.CRONJOB_JOB_DELETE)
    @Idempotent(key = "remi:cronjob:JobController:delete:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:cronjob:JobController:pause:lock", ttlSeconds = 5)
    ...
    @AuthApiPermission(apiCodes = PermissionCodes.CRONJOB_JOB_PAUSE)
    @Idempotent(key = "remi:cronjob:JobController:resume:lock", ttlSeconds = 5)
    ...
    @AuthApiPermission(apiCodes = PermissionCodes.CRONJOB_JOB_UPDATE)
    @Idempotent(key = "remi:cronjob:JobController:batchPause:lock", ttlSeconds = 5)
    ...
    @AuthApiPermission(apiCodes = PermissionCodes.CRONJOB_JOB_UPDATE)
    @Idempotent(key = "remi:cronjob:JobController:batchResume:lock", ttlSeconds = 5)
     ...
     * 重新加载所有任务
     *
     * <p>从数据库 remi_job 表重新加载全部任务到内存调度器。
    ...
    @Idempotent(key = "remi:cronjob:JobController:reload:lock", ttlSeconds = 5)
