package com.remisoft.cronjob.web.controller.dag;
...
import com.remisoft.cronjob.domain.dto.dag.JobDagTriggerDTO;
import com.remisoft.cronjob.domain.entity.dag.JobDag;
import com.remisoft.cronjob.domain.entity.dag.JobDagVersion;
import com.remisoft.cronjob.server.core.dag.DagDefinition;
import com.remisoft.cronjob.server.core.dag.DagDefinitionCodec;
import com.remisoft.cronjob.server.core.dag.DagDefinitionValidator;
import com.remisoft.cronjob.server.service.dag.JobDagService;
...
import com.remisoft.cronjob.domain.converter.CronjobConverter;
import com.remisoft.cronjob.domain.vo.JobDagVO;
import com.remisoft.cronjob.domain.vo.JobDagVersionVO;
import com.remisoft.cronjob.domain.dto.post.JobDagPostDTO;
import com.remisoft.cronjob.domain.dto.put.JobDagPutDTO;
 ...
 * <ul>
 *   <li>CRUD：创建 / 更新 / 删除 / 详情 / 按 KEY 查</li>
 *   <li>状态：启用 / 禁用</li>
 *   <li>触发：手动触发（同步返回实例 ID）</li>
 *   <li>校验：编辑器保存前的实时校验</li>
 *   <li>版本：版本历史查询、回滚到指定版本</li>
 * </ul>
 *
 * <h3>核心组件</h3>
 * <ul>
 *   <li>{@link JobDagService} - DAG 业务编排（CRUD/触发/版本）</li>
 *   <li>{@link DagDefinitionValidator} - 节点/边/环/根节点等结构校验</li>
 *   <li>{@link DagDefinitionCodec} - DAG 定义 JSON 序列化/反序列化</li>
 * </ul>
 *
 * <h3>安全与稳定性</h3>
 * <ul>
 *   <li>CRUD 全部 {@link Idempotent} 防重（5s TTL）</li>
 *   <li>权限按 DAG 维度细分（CRONJOB_DAG_CREATE/UPDATE/DELETE/VIEW/TRIGGER）</li>
 *   <li>所有写操作 {@link RateLimit} 限流 50 QPS / IP</li>
...
public class JobDagController {
     ...
     * 创建 DAG 工作流。
     *
     * <p>在保存前会先做基础结构校验（dagDefinition 非空、节点数 > 0）。
     * 完整的 DAG 结构校验（环/根/边）由 {@link #validateDag} 在编辑器保存前完成。
     * 持久化时初始化 version=1，状态按入参设置（默认 DRAFT）。
     *
    ...
    @Idempotent(key = "remi:cronjob:JobDagController:createDag:lock", ttlSeconds = 5)
     ...
     * 更新 DAG 工作流。
     *
     * <p>保存新版本时自动 +1 version，并同步生成 {@code remi_job_dag_version} 快照记录。
     * 已启动的 DAG 实例继续按旧版本执行；新触发的实例使用新版本。
     *
     * @param dagId DAG ID
     * @param dto   DAG 保存请求体
     * @return 统一响应结果
    ...
    @Idempotent(key = "remi:cronjob:JobDagController:updateDag:lock", ttlSeconds = 5)
     ...
     * 删除 DAG 工作流。
     *
     * <p>逻辑删除（status 置为 DELETED）。如果 DAG 有运行中的实例，禁止删除并抛业务异常。
     * 历史实例数据保留，不受影响。
     *
     * @param dagId DAG ID
     * @return 统一响应结果
    ...
    @Idempotent(key = "remi:cronjob:JobDagController:deleteDag:lock", ttlSeconds = 5)
    ...
    @AuthApiPermission(apiCodes = PermissionCodes.CRONJOB_DAG_UPDATE)
    @Idempotent(key = "remi:cronjob:JobDagController:enableDag:lock", ttlSeconds = 5)
     ...
     * 禁用 DAG 工作流。
     *
     * <p>禁用后调度器不再按 cron 触发新实例，但运行中的实例继续执行完成。
     * 手动触发也会被拒绝。
     *
     * @param dagId DAG ID
     * @return 统一响应结果
    ...
    @AuthApiPermission(apiCodes = PermissionCodes.CRONJOB_DAG_UPDATE)
    @Idempotent(key = "remi:cronjob:JobDagController:disableDag:lock", ttlSeconds = 5)
     ...
     * 查询 DAG 工作流详情。
     *
     * <p>按 ID 查最新版本。返回的 VO 经过 {@link CronjobConverter} 转换，
     * 包含 dagDefinition 完整定义（节点/边/参数）、当前版本号、状态等。
     *
     * @param dagId DAG ID
     * @return DAG 详情 VO
     */
     ...
     * 根据 KEY 查询 DAG 工作流。
     *
     * <p>外部系统集成时常用 dagKey（业务语义化的唯一标识）而非雪花 ID。
     * dagKey 在租户内唯一，由创建者保证。
     *
     ...
     * 手动触发 DAG 工作流。
     *
     * <p>立即创建一个新的 DAG 实例，返回实例 ID 用于追踪执行进度。
     * 被 {@link IdempotentExempt} 豁免（手动触发本身允许重复），但通过 {@link RateLimit} 限流。
     *
    ...
    @Idempotent(key = "remi:cronjob:JobDagController:triggerDag:lock", ttlSeconds = 5)
    @PostMapping("/trigger")
     ...
     * 校验通过返回 true，校验失败抛业务异常并返回错误信息（{@code BizException}）。
     *
    ...
    @AuthApiPermission(apiCodes = PermissionCodes.CRONJOB_DAG_VIEW)
    @Idempotent(key = "remi:cronjob:JobDagController:validateDag:lock", ttlSeconds = 5)
     ...
     * P1-4: 查询 DAG 版本历史列表。
     *
     * <p>返回指定 DAG 的所有历史版本快照（按版本号降序，最多 50 条）。
     * 用于版本回滚 UI：用户选择某个历史版本 → 调用 {@link #rollbackDag}。
     *
     * @param dagId DAG ID
     ...
     * P1-4: 回滚 DAG 到指定版本。
     *
     * <p>将目标历史版本的定义复制为新版本（version = 当前 version + 1），
     * 保留完整的版本谱系（不会删除中间版本），支持后续再次回滚。
     * 已运行的实例继续按其启动时的版本执行。
     *
     * @param dagId   DAG ID
    ...
    @AuthApiPermission(apiCodes = PermissionCodes.CRONJOB_DAG_UPDATE)
    @Idempotent(key = "remi:cronjob:JobDagController:rollbackDag:lock", ttlSeconds = 5)
