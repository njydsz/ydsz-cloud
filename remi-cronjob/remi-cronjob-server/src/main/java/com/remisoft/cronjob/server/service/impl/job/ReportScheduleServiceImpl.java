...
import com.remisoft.cronjob.server.service.job.ReportScheduleService;
 ...
 *   <li>{@link #distributeReport} 落库 remi_export_record（source='SUBSCRIPTION'，P0-3 合并），
 *       并通过 Feign 调用 message 模块发送 EMAIL 通知</li>
 * </ul>
 *
 * <p>P0-3: 原 remi_report_export_record 已并入 remi_export_record，
...
public class ReportScheduleServiceImpl implements ReportScheduleService {
    ...
    private void executeReportsByFrequency(String frequency) {
        String sql = "SELECT * FROM remi_report_subscription "
    ...
    /**
     * 分发报表：落库 remi_export_record（source='SUBSCRIPTION'，P0-3 合并），
    ...
    public void distributeReport(Long subId, String reportType, String fileKey, String recipients, String channels) {
        // 1. 落库 remi_export_record，source='SUBSCRIPTION' 标记订阅触发
        //    字段顺序与 remi_export_record 完全对齐（避免 SQL 字段错位）
        String sql = "INSERT INTO remi_export_record ("
     ...
     * 根据订阅 ID 解析订阅人 ID。
     *
     * <p>用于回填 remi_export_record.user_id，使前端下载中心可以按 user_id 筛选
    ...
    private Long resolveSubscriberId(Long subId) {
        ...
        try {
            Long subscriberId = jdbcTemplate.queryForObject(
                    "SELECT subscriber_id FROM remi_report_subscription WHERE id = ?",
                    Long.class, subId);
            return subscriberId;
        } catch (Exception e) {
     ...
     *
     * <p>存储后端由 {@code remi.file.type} 配置决定（local/minio/s3/oss/cos/qiniu/obs），
     * 业务层不感知具体实现。返回 {@link FileStorage#getUuidName()} 作为对象 key，
     * 供后续 remi_export_record.file_key 落库使用。
     *
    ...
    private void sendEmailNotification(String reportType, String fileKey, String recipients,
        ...
        try {
            ...
            request.setSubject("【REMI报表】" + (reportType == null ? "" : reportType) + " 报表已生成");
