...
import com.remisoft.cronjob.domain.entity.job.TenantQuota;

/**
 * 租户级配额服务（P7-2 / P7-3）。
 *
 * <p>提供任务数/并发数/日执行量配额检查能力，防止 noisy neighbor 问题。
 * 默认禁用（{@code remi.cronjob.quota.enabled=false}），启用后在任务创建/派发时自动校验。
 *
 * <h3>配额优先级</h3>
 * <ol>
 *   <li>DB 记录（{@code remi_tenant_quota} 表对应租户的配置）</li>
 *   <li>全局默认（{@code remi.cronjob.quota.default-max-*} 配置）</li>
 *   <li>Unlimited（未启用配额检查或上限为 null）</li>
 * </ol>
 *
 * <h3>P7-3 Redis 计数器设计</h3>
 * <ul>
 *   <li><b>并发计数</b>：{@code remi:quota:concurrent:{tenantId}}
 *       <ul>
 ...
 *       </ul>
 *   </li>
 *   <li><b>日执行计数</b>：{@code remi:quota:daily:{tenantId}:{yyyyMMdd}}
 *       <ul>
...
public interface TenantQuotaService {
     ...
     *
     * <p>当 {@code remi.cronjob.quota.enabled=false} 时直接返回（不检查）。
     ...
     *
     * <p>通过 Redis 实时计数器 {@code remi:quota:concurrent:{tenantId}} 获取当前并发数，
     * 与 {@link TenantQuota#getMaxConcurrent()} 或全局默认比较。
     *
     * @param tenantId 租户 ID
     ...
     *
     * <p>通过 Redis 日计数器 {@code remi:quota:daily:{tenantId}:{yyyyMMdd}} 获取当日执行数，
     * 与 {@link TenantQuota#getMaxDailyExecutions()} 或全局默认比较。
     *
     * @param tenantId 租户 ID
     ...
     * <ul>
     *   <li>{@code remi:quota:concurrent:{tenantId}}（并发计数，需在 {@link #recordExecutionEnd} 中 DECR）</li>
     *   <li>{@code remi:quota:daily:{tenantId}:{yyyyMMdd}}（日执行计数，不释放）</li>
     * </ul>
     *
     ...
     * 记录任务执行结束（P7-3）。
     *
     * <p>DECR {@code remi:quota:concurrent:{tenantId}} 计数器，保证不会为负。
     * 日执行计数器不释放（跨天自动过期）。
