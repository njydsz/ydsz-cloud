package com.remisoft.cronjob.server.core.map;
...
import com.remisoft.cronjob.domain.job.JobLogger;
...
import com.remisoft.cronjob.domain.job.MapReduceProcessor;
import com.remisoft.cronjob.domain.job.MapTask;
...
import com.remisoft.cronjob.server.core.dispatch.RemoteTaskClient;
 ...
 *
 * <h3>分布式并行执行（P0-1）</h3>
 * <p>当 {@code remi.cronjob.map-reduce.enabled=true} 时，子任务将被分发到多个在线节点并行执行：
 * <ul>
...
public class MapTaskExecutor {
     ...
     *
     * @param job         任务定义
     * @param log0        执行日志（已插入 remi_job_log，status=RUNNING）
     ...
     *
     * @param taskId 子任务 ID（remi_job_task.id）
