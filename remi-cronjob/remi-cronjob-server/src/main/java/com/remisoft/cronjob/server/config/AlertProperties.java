 ...
 * 告警通知配置属性（P5 告警 + 监控）。
 *
 * <p>支持在 application.yml / Nacos 中通过 {@code remi.cronjob.alert.*} 前缀进行动态覆盖。
 *
 * <h3>通道配置</h3>
 * <ul>
 *   <li>{@link #getEmail()} 邮件通道（SMTP 或 message-service 转发）</li>
...
@Data
@Configuration
@ConfigurationProperties(prefix = "remi.cronjob.alert")
public class AlertProperties {
    ...
    public static class Email {
        ...
        /** 发件人邮箱地址（如 alert@remisoft.com） */
        private String from = "alert@remisoft.com";

        /** 邮件服务转发 URL（NULL 时尝试本地 SMTP） */
        private String serviceUrl;

        /** 邮件主题前缀（如 [REMI 告警]） */
        private String subjectPrefix = "[REMI 告警]";
    }
