 ...
 * 定时（默认 30s）扫描 {@code remi_job_log} 中已结束（SUCCESS/FAILED/TIMEOUT）
 * 且耗时超过 {@code remi_job.slow_threshold_ms} 的记录，标记 {@code is_slow=1}。
 *
 * <h3>设计要点</h3>
 * <ul>
 ...
 * 实时触发 {@link com.remisoft.cronjob.server.core.alert.AlertType#SLOW} 告警，
 * 二者关注点正交。
 *
 * @author remi-team
 * @since 1.0.0
...
public class SlowTaskDetector {
     ...
     *
     * <p>仅在 {@code remi.cronjob.leader.enabled=true} 时进入扫描启用分支；
     ...
     * 避免上次扫描耗时较长时任务堆积。
     */
    @Scheduled(fixedDelayString = "${remi.cronjob.slow-task-detector.interval-ms:30000}")
    public void scan() {
        if (!cronjobProperties.getLeader().isEnabled()) {
            return;
        }
