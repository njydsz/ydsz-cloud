package com.remisoft.cronjob.server.service.impl.dag;
...
import com.remisoft.cronjob.infra.mapper.dag.JobDagInstanceMapper;
import com.remisoft.cronjob.infra.mapper.dag.JobDagMapper;
import com.remisoft.cronjob.infra.mapper.dag.JobDagVersionMapper;
import com.remisoft.cronjob.server.core.dag.DagDefinition;
import com.remisoft.cronjob.server.core.dag.DagDefinitionCodec;
import com.remisoft.cronjob.server.core.dag.DagEdge;
import com.remisoft.cronjob.server.core.dag.DagFailureStrategy;
import com.remisoft.cronjob.server.core.dag.DagInstanceExecutor;
import com.remisoft.cronjob.server.core.dag.DagNode;
import com.remisoft.cronjob.server.core.dag.DagParser;
...
public class JobDagServiceImpl implements JobDagService {
    ...
    private void validateDagDefinition(String dagDefinitionJson) {
        ...
        if (dagParser.hasCycle(adj)) {
            throw new SysException(BaseResultCode.BAD_REQUEST, "error.cronjob.msg_dag_has_cycle");
        }
    }
    ...
    private void validateCronExpression(String triggerType, String cronExpression) {
        if ("CRON".equals(triggerType) && !StringUtils.hasText(cronExpression)) {
            throw new SysException(BaseResultCode.BAD_REQUEST, "error.cronjob.msg_dag_cron_required");
        }
    }
