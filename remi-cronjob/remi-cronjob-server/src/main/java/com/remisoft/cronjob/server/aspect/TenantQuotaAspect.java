package com.remisoft.cronjob.server.aspect;
...
import com.remisoft.cronjob.server.annotation.TenantQuotaCheck;
import com.remisoft.cronjob.server.annotation.TenantQuotaCheck.QuotaType;
 ...
 * 租户配额检查切面（P0-5）。
 *
 * <p>拦截 {@link TenantQuotaCheck} 注解标注的方法，在方法执行前自动进行租户配额校验。
 * 支持三种配额类型：任务数配额、并发配额、日执行量配额。
 *
 * <p>租户 ID 提取逻辑：
 * <ol>
 ...
 * </ol>
 *
 * <p>配额检查失败时抛出 {@link com.remisoft.common.exception.custom.SysException}，
 * 错误码为 {@code QUOTA_EXCEEDED}。
 *
 * @author remi-team
 * @since 1.0.0
 */
...
public class TenantQuotaAspect {
    ...
    public void checkQuota(JoinPoint joinPoint, TenantQuotaCheck annotation) {
        ...
        if (tenantId == null || tenantId.isBlank()) {
            log.warn("[TenantQuota] 租户 ID 为空，跳过配额检查: method={}",
                    joinPoint.getSignature().toShortString());
            return;
        }

        QuotaType quotaType = annotation.type();
        log.debug("[TenantQuota] 开始配额检查: tenant={} type={} method={}",
                tenantId, quotaType, joinPoint.getSignature().toShortString());
