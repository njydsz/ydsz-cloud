 ...
 *       Nacos 模式对比 Nacos 实例列表，DB 模式对比 remi_job_node 心跳表</li>
 ...
 * <p>{@link com.remisoft.cronjob.server.core.executor.JobNodeReaper} 负责 DB 模式下的
 ...
 *   <li>JobNodeReaper（5min）：清理节点状态，避免 remi_job_node 表膨胀</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
...
public class FailoverScanner {
     ...
     * 避免上次扫描耗时较长时任务堆积。
     * 配置项 {@code remi.cronjob.failover.scan-interval-ms} 为毫秒值（默认 30000）。
     */
    @DistributedScheduled(lockKey = "cronjob:failover-scan")
    @Scheduled(fixedDelayString = "${remi.cronjob.failover.scan-interval-ms:30000}")
    public void scan() {
    ...
    private void doScan() {
        ...
        // 1. 获取在线节点列表（Nacos 模式查 Nacos 实例，DB 模式查 remi_job_node 心跳表）
