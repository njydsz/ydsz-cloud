 ...
 *   <li>每个分区有独立的 Leader 选举（Redis 锁 key: {@code remi:job:leader:{role}-{partition}}）</li>
...
@ConditionalOnProperty(prefix = "remi.cronjob.leader.partition", name = "enabled", havingValue = "true")
public class PartitionLeaderManager {
     ...
     * 仅当 {@code remi.cronjob.leader.partition.enabled=true} 时本 Bean 才注册。
     */
    @PostConstruct
    public void init() {
     ...
     */
    @Scheduled(fixedDelayString = "${remi.cronjob.leader.renew-interval-seconds:10}s")
     ...
     * @return 角色名（如 {@code remi-job-scheduler-0}）
     */
