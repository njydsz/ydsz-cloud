package com.remisoft.cronjob.server.core.alert;
...
import com.remisoft.cronjob.infra.mapper.job.JobAlertRuleMapper;
 ...
 * 定时（默认 5 分钟）扫描启用的 FAIL_RATE / DURATION_P95 类型告警规则，
 * 统计规则配置的时间窗口内的失败率 / P95 耗时，超过阈值时调用
 * {@link AlertTrigger#trigger(AlertContext)} 触发告警。
 *
 * <h3>设计要点</h3>
 * <ul>
 *   <li><b>Leader 独占</b>：通过 {@link LeaderElector#isLeader(String)} 判定，
 *       避免多实例重复扫描与重复告警</li>
 *   <li><b>解耦</b>：FAIL / SLOW 等单次触发的告警由
 *       {@code DefaultTaskDispatcher} 在任务执行完成时实时触发；
 *       本扫描器仅负责需要周期性聚合统计的告警类型</li>
 *   <li><b>容错</b>：单条规则评估异常不影响其他规则；外层 try-catch 兜底</li>
 *   <li><b>去重</b>：冷却窗口由 {@link AlertDispatcher} 在事件处理阶段统一控制，
 *       本扫描器不做去重，每次扫描只要超过阈值即触发（由 Dispatcher CAS 去重）</li>
 * </ul>
 *
 * <h3>对标</h3>
...
public class AlertScanner {
    ...
    public void init() {
        ...
        if (cronjobProperties.getLeader().isEnabled()) {
            log.info("[AlertScanner] 初始化完成, role={} scanIntervalMs={}",
                    leaderRole, cronjobProperties.getAlert().getScanIntervalMs());
        } else {
    ...
    @Scheduled(fixedDelayString = "${remi.cronjob.alert.scan-interval-ms:300000}")
    public void scan() {
        if (!cronjobProperties.getLeader().isEnabled()) {
            return;
        }
     ...
     *
     * <p>失败率 = 失败次数 / 总次数 * 100（百分比，与 threshold 单位一致）。
     * 失败率 &gt;= threshold 时触发告警。
     */
    void scanFailRateRules() {
        ...
        for (JobAlertRule rule : rules) {
            try {
                ...
                log.error("[AlertScanner] 评估 FAIL_RATE 规则失败: ruleId={} jobId={} reason={}",
     ...
     *
     * <p>P95 耗时仅统计 {@code status='SUCCESS'} 的执行（避免失败/超时任务拉高 P95）。
    ...
    void scanDurationP95Rules() {
        ...
        for (JobAlertRule rule : rules) {
            try {
                ...
                log.error("[AlertScanner] 评估 DURATION_P95 规则失败: ruleId={} jobId={} reason={}",
                        rule.getId(), rule.getJobId(), e.getMessage(), e);
            }
        }
    }
    ...
    private void evaluateFailRateRule(JobAlertRule rule) {
        ...
        log.info("[AlertScanner] FAIL_RATE 告警触发: ruleId={} jobId={} failRate={} threshold={}",
    ...
    private void evaluateDurationP95Rule(JobAlertRule rule) {
        ...
        alertTrigger.trigger(context);
        log.info("[AlertScanner] DURATION_P95 告警触发: ruleId={} jobId={} p95={}ms threshold={}ms",
