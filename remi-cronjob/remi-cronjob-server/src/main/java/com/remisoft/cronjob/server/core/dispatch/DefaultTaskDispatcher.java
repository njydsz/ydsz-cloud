...
import com.remisoft.cronjob.domain.job.JobExecutionContext;
import com.remisoft.cronjob.domain.job.JobHandler;
import com.remisoft.cronjob.domain.job.ProcessResult;
import com.remisoft.cronjob.domain.job.ShardingContext;
...
import com.remisoft.cronjob.infra.mapper.job.JobNodeMapper;
import com.remisoft.cronjob.infra.mapper.log.JobLogMapper;
import com.remisoft.cronjob.server.config.CronjobProperties;
import com.remisoft.cronjob.server.core.LockKeyUtil;
import com.remisoft.cronjob.server.core.TaskCompletedEvent;
import com.remisoft.cronjob.server.core.alert.AlertContext;
import com.remisoft.cronjob.server.core.alert.AlertTrigger;
import com.remisoft.cronjob.server.core.alert.AlertType;
import com.remisoft.cronjob.server.core.discovery.NodeDiscoveryStrategy;
import com.remisoft.cronjob.server.core.executor.JobNodeHeartbeat;
import com.remisoft.cronjob.server.core.executor.TenantAwareExecutorPool;
import com.remisoft.cronjob.server.core.handler.GlueJobHandler;
import com.remisoft.cronjob.server.core.handler.HttpJobHandler;
import com.remisoft.cronjob.server.core.handler.ScriptJobHandler;
import com.remisoft.cronjob.server.core.logger.JobLoggerImpl;
import com.remisoft.cronjob.server.core.logger.LogStreamManager;
import com.remisoft.cronjob.server.core.map.MapTaskExecutor;
import com.remisoft.cronjob.server.core.sharding.ShardAssignment;
import com.remisoft.cronjob.server.core.sharding.ShardingStrategy;
import com.remisoft.cronjob.server.metrics.CronjobMetrics;
import com.remisoft.cronjob.server.service.job.TenantQuotaService;
import com.remisoft.cronjob.server.service.log.JobLogContentService;
 ...
 *   <li>写开始日志（remi_job_log, status=RUNNING）</li>
...
public class DefaultTaskDispatcher implements TaskDispatcher {
     ...
     * 线程池大小和线程名通过硬编码配置（2 线程 + remi-job-retry 前缀）。
     */
     ...
     * <ul>
     *   <li>锁 key 含分片索引: {@code remi:job:lock:{jobKey}:shard:{shardIndex}}</li>
...
try {
     ...
     * <ol>
     *   <li>从 remi_job_log 查询当前 RUNNING 的日志（jobKey + status=RUNNING）</li>
    ...
    private void initNodeId() {
        ...
        this.retryScheduler = Executors.newScheduledThreadPool(
                2, r -> {
                    Thread t = new Thread(r, "remi-job-retry");
                    t.setDaemon(true);
                    return t;
                });
