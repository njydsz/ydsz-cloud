 ...
 * <h3>核心能力</h3>
 * <ul>
 *   <li>{@link #createLog}：创建执行日志记录（PENDING 状态）</li>
 *   <li>{@link #markRunning}：标记为 RUNNING + 记录派发时间（P1-2 执行轨迹）</li>
 *   <li>{@link #markSuccess}：标记为 SUCCESS + 记录结束时间 + 耗时</li>
 *   <li>{@link #markFailed}：标记为 FAILED + 记录错误信息</li>
 *   <li>{@link #markTimeout}：标记为 TIMEOUT（超时终止）</li>
 * </ul>
 *
 * <h3>提取动机</h3>
 * <p>DefaultTaskDispatcher 1592 行代码中约 200 行涉及日志记录，
 * 提取后可统一日志格式和执行轨迹字段管理。
 *
 * @author remi-team
 * @since 1.0.0
