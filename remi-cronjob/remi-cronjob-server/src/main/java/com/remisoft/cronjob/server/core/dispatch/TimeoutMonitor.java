...
public class TimeoutMonitor {
     ...
     *
     * <p>仅在 {@code remi.cronjob.leader.enabled=true} 时进入监控启用分支；
    ...
    @Scheduled(fixedDelayString = "${remi.cronjob.timeout-monitor.interval-ms:10000}")
    public void scan() {
        if (!cronjobProperties.getLeader().isEnabled()) {
            return;
        }
