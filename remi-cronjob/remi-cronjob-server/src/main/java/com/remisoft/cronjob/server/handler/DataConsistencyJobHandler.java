package com.remisoft.cronjob.server.handler;
 ...
 * 在 remi_job 表插入记录：handler=dataConsistencyJobHandler。
 *
 * @author remi-team
 * @since 1.0.0
 */
    ...
    private int checkInvoiceVsPayment() {
        try {
            String sql = """
                ...
                FROM remi_invoice i
                LEFT JOIN remi_payment p ON i.initiation_id = p.initiation_id AND p.deleted = 0
    ...
    private int checkBudgetVsActualCost() {
        try {
            String sql = """
                ...
                FROM remi_budget_item b
                LEFT JOIN remi_expense e ON b.initiation_id = e.initiation_id AND e.deleted = 0 AND e.status = 'CONFIRMED'
                WHERE b.deleted = 0
        ...
        try {
            String sql = """
                SELECT w.id, w.task_name, w.progress,
                       (SELECT COUNT(*) FROM remi_time_entry te WHERE te.wbs_task_id = w.id AND te.deleted = 0 AND te.status = 'APPROVED') AS entry_count
                FROM remi_wbs_task w
                WHERE w.deleted = 0 AND w.progress = 100
                HAVING (SELECT COUNT(*) FROM remi_time_entry te WHERE te.wbs_task_id = w.id AND te.deleted = 0 AND te.status = 'APPROVED') = 0
                """;
            List<Map<String, Object>> diffs = jdbcTemplate.queryForList(sql);
            for (Map<String, Object> diff : diffs) {
