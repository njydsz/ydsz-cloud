 ...
 * 调度节点心跳上报组件。
 *
 * <p>每个 cronjob 实例启动时注册到 {@code remi_job_node} 表，
 ...
 * </ol>
 *
 * <p>仅在 {@code remi.cronjob.leader.enabled=true} 时启用，避免 Leaderless 模式下产生无用记录。
 *
 * <p>P1-1: 仅在 {@code remi.cronjob.node-discovery.type=db} 时注册。
...
public class JobNodeHeartbeat {
     ...
     */
    @Scheduled(fixedDelayString = "${remi.cronjob.executor.heartbeat-interval-seconds:10}s")
