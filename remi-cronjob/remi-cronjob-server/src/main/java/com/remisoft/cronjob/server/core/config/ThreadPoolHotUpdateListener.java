package com.remisoft.cronjob.server.core.config;
...
public class ThreadPoolHotUpdateListener {
     ...
     * Nacos 配置变更监听。
     *
     * <p>监听 {@code remi-cronjob.yml}（或对应的 dataId），当检测到 executor 相关配置变更时，
     * 解析新配置并应用到运行中的线程池。
     *
     * @param configInfo Nacos 下发的配置内容（YAML 或 JSON）
     */
    @NacosConfigListener(dataId = "${remi.cronjob.config-data-id:remi-cronjob.yml}", timeout = 5000)
     ...
     * 兼容两层嵌套：顶层 remi.cronjob.executor 或直接 executor。
     *
