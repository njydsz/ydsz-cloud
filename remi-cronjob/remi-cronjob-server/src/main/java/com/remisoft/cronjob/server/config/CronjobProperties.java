 ...
 * 定时任务调度服务配置属性。
 *
 * <p>支持在 application.yml / Nacos 中通过 {@code remi.cronjob.*} 前缀进行动态覆盖。
 *
 * <h3>关键配置项</h3>
 * <ul>
...
@Configuration
@Validated
@ConfigurationProperties(prefix = "remi.cronjob")
public class CronjobProperties {
    ...
    public static class Leader {
        ...
        /** 角色（多套调度集群隔离时使用） */
        private String role = "remi-job-scheduler";
     ...
     * </ul>
     *
     * <p>未配置租户配额记录时（{@code remi_tenant_quota} 表无对应行），默认不限制（unlimited）。
     */
    @Data
    public static class Quota {
        ...
        /** 默认任务数上限（当租户未在 remi_tenant_quota 表配置时使用，null=unlimited） */
     ...
     * <h3>工作流程</h3>
     * <ol>
     *   <li>Leader 计算分片分配方案（{@link com.remisoft.cronjob.server.core.sharding.ShardingStrategy}）</li>
     ...
     * 告警扫描配置（P3-2 周期性告警扫描）。
     *
     * <p>控制 {@link com.remisoft.cronjob.server.core.alert.AlertScanner} 的扫描间隔。
     * 仅 Leader 节点启用周期性扫描，统计 FAIL_RATE / DURATION_P95 等需要聚合计算的告警类型。
     *
     * <p>对应配置前缀 {@code remi.cronjob.alert.scan-interval-ms}（与 {@link AlertProperties} 共享前缀，
     ...
     *   <li>{@code db}：基于 remi_job_node 心跳表（向后兼容，需配合 JobNodeHeartbeat + JobNodeReaper）</li>
     * </ul>
     */
    @Data
     ...
     * P2-2: 日志归档清理配置。
     *
     * <p>控制 {@link com.remisoft.cronjob.server.core.cleaner.LogCleaner} 的清理行为：
     * <ul>
     *   <li>{@link #retentionDays} 日志保留天数（超过此天数的日志将被清理，默认 30 天）</li>
     *   <li>{@link #batchSize} 单批删除条数（避免大事务锁表，默认 1000 条/批）</li>
     * </ul>
     *
     * <p>清理范围：remi_job_log / remi_job_log_content /
     * remi_job_alert_log / remi_job_task，每天凌晨 3 点由 Leader 节点执行。
     */
    @Data
     ...
     * P3-3.3: 任务制品（Artifact）存储配置。
     *
     * <p>控制 {@link com.remisoft.cronjob.server.service.impl.JobArtifactService} 的制品文件
