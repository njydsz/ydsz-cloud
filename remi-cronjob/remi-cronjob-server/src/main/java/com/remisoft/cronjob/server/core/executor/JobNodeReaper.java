 ...
 *
 * <p>定时清理 {@code remi_job_node} 表中的僵尸节点，并对掉线节点上的 RUNNING 任务执行故障转移：
 * <ol>
 ...
 * <h3>执行条件</h3>
 * <ul>
 *   <li>仅当 {@code remi.cronjob.leader.enabled=true} 时启用</li>
 *   <li>仅 Leader 节点执行，避免多实例重复清理</li>
 *   <li>P1-1: 仅在 {@code remi.cronjob.node-discovery.type=db} 时注册
...
@ConditionalOnProperty(name = "remi.cronjob.node-discovery.type", havingValue = "db")
public class JobNodeReaper {
     ...
     * 定时清理僵尸节点（默认每 5 分钟一次）。
     */
    @Scheduled(fixedDelayString = "${remi.cronjob.node-reaper.interval-ms:300000}")
    public void reap() {
     ...
     * <ul>
     *   <li>非分片任务（shardIndex=null）：{@code remi:job:lock:{jobKey}}</li>
     *   <li>分片任务（shardIndex>=0）：{@code remi:job:lock:{jobKey}:shard:{shardIndex}}</li>
     * </ul>
