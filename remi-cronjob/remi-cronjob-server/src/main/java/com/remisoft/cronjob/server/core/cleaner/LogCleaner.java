package com.remisoft.cronjob.server.core.cleaner;
...
import com.remisoft.cronjob.infra.mapper.job.JobAlertLogMapper;
import com.remisoft.cronjob.infra.mapper.job.JobHistoryMapper;
import com.remisoft.cronjob.infra.mapper.job.JobTaskMapper;
import com.remisoft.cronjob.infra.mapper.log.JobLogContentMapper;
 ...
 * 日志归档清理器（P2-2）。
 *
 * <p>仅当 {@code remi.cronjob.leader.enabled=true} 且当前节点是 Leader 时启用。
 * 每天凌晨 3 点定时清理超过保留天数的日志记录，释放磁盘空间。
 *
 * <h3>清理范围</h3>
 * <ul>
 *   <li>remi_job_log：任务执行日志（含 is_slow 慢任务标记）</li>
 *   <li>remi_job_log_content：任务日志内容（在线日志白屏化）</li>
 *   <li>remi_job_alert_log：告警日志</li>
 *   <li>remi_job_task：MapReduce 子任务记录</li>
 *   <li>remi_job_history：任务配置历史版本</li>
 * </ul>
 *
 * <h3>设计要点</h3>
 * <ul>
...
public class LogCleaner {
     ...
     * 可通过配置 {@code remi.cronjob.log-retention.cron} 覆盖。
     */
    @Scheduled(cron = "${remi.cronjob.log-retention.cron:0 0 3 * * ?}")
    public void clean() {
        if (!cronjobProperties.getLeader().isEnabled()) {
            return;
        }
        ...
        long totalCleaned = 0;
        // 每张表独立清理，单表异常不影响其他表
        totalCleaned += cleanTable("remi_job_log", before, batchSize, jobLogMapper::cleanExpiredLogs);
        totalCleaned += cleanTable("remi_job_log_content", before, batchSize, jobLogContentMapper::cleanExpiredLogs);
        totalCleaned += cleanTable("remi_job_alert_log", before, batchSize, jobAlertLogMapper::cleanExpiredLogs);
        totalCleaned += cleanTable("remi_job_task", before, batchSize, jobTaskMapper::cleanExpiredLogs);
        totalCleaned += cleanTable("remi_job_history", before, batchSize, jobHistoryMapper::cleanExpiredLogs);
