package com.remisoft.cronjob.server.service.alert;
 ...
 * 告警规则 Service
 *
 * <p>为定时任务提供"任务执行异常自动告警"能力。支持多种触发条件(连续失败 N 次、
 * 成功率低于阈值、平均耗时超阈值等)和多种通知通道(站内通知/短信/邮件/企业微信/钉钉)。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li><b>CRUD</b>：{@link #createRule} / {@link #updateRule} / {@link #deleteRule} / {@link #getRuleById}</li>
 *   <li><b>查询</b>：{@link #listRules} / {@link #queryAlertLogs}</li>
 *   <li><b>启停</b>：{@link #toggleRule} — 启用/禁用告警规则</li>
 * </ul>
 *
 * <p><b>告警规则：</b>每条规则绑定一个或多个任务 ID,配置触发条件(规则表达式)、
 * 通知通道(支持组合)、接收人(角色/用户/部门)。规则修改不影响历史告警日志。
 *
 * <p><b>告警日志：</b>触发后写入 {@code remi_job_alert_log},包含触发时间、条件命中值、通知发送结果。
 *
