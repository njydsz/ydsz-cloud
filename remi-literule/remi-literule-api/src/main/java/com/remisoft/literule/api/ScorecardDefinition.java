 ...
 * <ul>
 *   <li>{@code DESCENDING}（默认）：分数越低风险越高，redThreshold &lt; yellowThreshold</li>
 ...
 *
 * <p><b>评级映射（grades）</b>：可选，按分数区间映射自定义评级（如 A/B/C/D），
 * 若配置则覆盖 redThreshold/yellowThreshold 的三级映射。
 *
 * <p>持久化于 {@code remi_rule_scorecard}（见 V048），由 {@code ScorecardConfigProvider} SPI 加载，
 * 通过 {@link com.remisoft.literule.server.impl.ScorecardRule#from(ScorecardDefinition, com.remisoft.literule.server.expr.ExpressionEvaluator)}
 * 转换为可执行规则。
 *
 * <p>JSON 示例（复杂评分卡）：
 * <pre>
...
public class ScorecardDefinition implements Serializable {
    ...
    @Builder.Default
    private ScoreDirection scoreDirection = ScoreDirection.DESCENDING;
