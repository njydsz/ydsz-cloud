 ...
 * 脚本规则定义（DTO）
 *
 * <p>基于 LiteExpr 的动态脚本规则，适用于表达式无法覆盖的复杂场景
 * （多步骤条件判断、循环检查、复杂对象操作等）。
 *
 * <p>脚本约定：
 * <ul>
 *   <li>通过 {@code facts} 变量访问事实数据（{@code Map<String, Object>}）</li>
 *   <li>返回 boolean：true=触发，false=不触发</li>
 *   <li>可设置 {@code severity} / {@code title} / {@code description} 变量自定义结果</li>
 * </ul>
 *
 * <p>沙箱模式（默认启用）禁止 System.exit / Runtime.exec / 反射 / 文件 I/O / 网络访问等危险 API。
 *
 * <p>持久化于 {@code remi_rule_script}（见 V048，script 字段为 TEXT），
 * 由 {@code ScriptConfigProvider} SPI 加载，
 * 通过 {@link com.remisoft.literule.server.impl.ScriptRule#from(ScriptDefinition)}
 * 转换为可执行规则。
 *
 * @author remi-team
 * @since 1.0.0
