 ...
 * {@link com.remisoft.literule.server.core.DefaultRuleEngine} 在评估前会比较
 * {@code rule.getTenantId()} 与 {@code context.getTenantId()}，仅当两者匹配时才评估该规则。
 * 默认 "1"（单租户部署），向后兼容。
 *
 * <p>1.6.0 起新增 {@code environment} 字段，用于运行时多环境隔离（P1-5）：
 * 与 tenantId 维度正交，支持 dev/staging/prod 环境隔离。
 * 规则的 environment 为 {@link RuleEnvironment#DEFAULT "default"} 时匹配任何上下文环境（向后兼容）；
 * 非 "default" 时必须与 {@link #getEnvironment()} 完全匹配。
 * 默认 "default"，向后兼容。
 *
 * @author remi-team
 * @since 1.0.0
...
public final class RuleContext implements Serializable {
     ...
     * {@code rule.getEnvironment()} 与 {@code environment} 匹配过滤规则。
     ...
     * environment 默认 {@link RuleEnvironment#DEFAULT "default"}（向后兼容）。
     *
     * @param facts    事实数据
     * @param scenario 业务场景
     * @param source   触发来源
     ...
     * 获取环境标识
     *
     * <p>引擎评估时按 {@code rule.getEnvironment()} 与本字段匹配过滤：
     * 规则 environment 为 {@link RuleEnvironment#DEFAULT "default"} 时匹配任何上下文环境；
