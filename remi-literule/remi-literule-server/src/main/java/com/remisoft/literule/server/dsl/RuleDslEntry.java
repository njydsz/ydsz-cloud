 ...
 * <ul>
 *   <li>{@code expression}（默认）- 表达式规则，配合 condition / severity / title / description</li>
 *   <li>{@code scorecard} - 评分卡规则，配合 base_score / factors / grades / direction 等</li>
 *   <li>{@code decision_table} - 决策表规则，配合 condition_columns / action_columns / rows</li>
 *   <li>{@code decision_tree} - 决策树规则，配合 tree_nodes</li>
 *   <li>{@code script} - 脚本规则，配合 script_language / script_body</li>
 *   <li>{@code static_rule} - 静态规则（无条件，始终触发）</li>
 * </ul>
 *
...
public class RuleDslEntry implements Serializable {
     ...
     * 规则类型
     *
     * <p>可选值：expression / scorecard / decision_table / decision_tree / script / static_rule
     * 默认 expression。
     */
    @Builder.Default
