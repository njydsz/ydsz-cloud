...
@Data
@Validated
@ConfigurationProperties(prefix = "remi.literule")
public class LiteRuleProperties {
     ...
     * {@link com.remisoft.literule.server.spi.FileRuleSource} Bean。
     ...
     * 多级缓存配置（P1-1）
     *
     * <p>启用后自动装饰 {@link com.remisoft.literule.server.spi.RuleConfigProvider} 为
     * {@link com.remisoft.literule.server.cache.CachingRuleConfigProvider}，
     ...
     * {@code remi.literule.default-tenant-id} 配置覆盖。
     *
     * @since 1.0.0
     */
     ...
     * 规则+模型融合配置（P3-1）
     *
     * <p>启用后，规则引擎在评估前会调用所有已注册的 {@link com.remisoft.literule.domain.model.ModelInputProvider}
     * 获取模型输出，注入到 {@link com.remisoft.literule.api.RuleContext} 的 facts 中，
     ...
     * {@link com.remisoft.literule.server.spi.FactProvider}
     * 从外部数据源（DB、Redis、HTTP API 等）动态采集事实数据，
     * 合并到 {@link com.remisoft.literule.api.RuleContext} 的 facts 中。
     *
     ...
     * 断点调试配置（P0-2）
     *
     * <p>控制 {@link com.remisoft.literule.server.core.DefaultBreakpointHook} 的 SUSPEND 超时时间。
     ...
     * 文件规则源配置（P2-3）
     *
     * <p>控制 {@link com.remisoft.literule.server.spi.FileRuleSource} 的加载行为。
     *
     * @since 1.0.0
     */
    @Data
     ...
     * 规则+模型融合配置（P3-1）
     *
     * <p>控制 {@link com.remisoft.literule.domain.model.ModelInputRegistry} 与
     * {@link com.remisoft.literule.domain.model.MockModelInputProvider} 的行为。
    ...
    public static class ModelConfig {
         ...
         * 单个模型调用超时（毫秒）
         *
         * <p>每个 {@link com.remisoft.literule.domain.model.ModelInputProvider} 调用最多等待该时长，
         ...
         * false：模型调用失败时抛出 {@link com.remisoft.literule.domain.model.ModelInvocationException}，
         ...
         * 是否启用 Mock 模型提供者
         *
         * <p>true：自动注册 {@link com.remisoft.literule.domain.model.MockModelInputProvider}，
         * 返回配置的模拟模型输出，便于开发/测试；
         * false（默认）：不注册 Mock，需业务方提供真实 {@link com.remisoft.literule.domain.model.ModelInputProvider} 实现。
         */
     ...
     * 动态事实采集配置（P0-2）
     *
     * <p>控制 {@link com.remisoft.literule.server.spi.FactProviderRegistry} 的行为。
     * 默认关闭（{@code enabled=false}），需显式启用以保证向后兼容。
     *
     * @since 1.0.0
     */
    ...
    public static class FactConfig {
         ...
         * 单个 provider 调用超时（毫秒）
         *
         * <p>每个 {@link com.remisoft.literule.server.spi.FactProvider} 调用最多等待该时长，
         * 超时则取消并返回空 Map（或抛异常，取决于 {@link #fallbackOnError}）。
         ...
         * {@link com.remisoft.literule.server.spi.FactCollectionException}，
    ...
    public static class DebugConfig {
         ...
         * 是否启用断点调试
         *
         * <p>true（默认）：自动装配 {@link com.remisoft.literule.server.core.DefaultBreakpointHook}；
         * false：不装配，断点调试功能不可用。
         */
        private boolean enabled = true;
     ...
     * {@link com.remisoft.literule.server.core.RuleLifecycleService} 将生成退役建议：
     * <ul>
    ...
    public static class LifecycleConfig {
         ...
         * 是否启用退役检测
         *
         * <p>true（默认）：自动装配 {@link com.remisoft.literule.server.core.RuleLifecycleService}；
         * false：不装配，退役检测功能不可用。
         */
        private boolean enabled = true;
