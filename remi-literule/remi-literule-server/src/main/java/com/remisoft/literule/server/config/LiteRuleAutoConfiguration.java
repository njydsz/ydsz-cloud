package com.remisoft.literule.server.config;
...
import com.remisoft.cronjob.api.client.CronjobServiceClient;
import com.remisoft.literule.api.RuleEngine;
import com.remisoft.literule.domain.model.MockModelInputProvider;
import com.remisoft.literule.domain.model.ModelInputProvider;
import com.remisoft.literule.domain.model.ModelInputRegistry;
import com.remisoft.literule.server.approval.ApprovalPermissionChecker;
import com.remisoft.literule.server.approval.ApprovalRecordRepository;
import com.remisoft.literule.server.approval.RuleApprovalService;
import com.remisoft.literule.server.approval.RuleApprovalWorkflowBridge;
import com.remisoft.literule.server.audit.RuleAuditLogService;
import com.remisoft.literule.server.benchmark.RuleStressTestService;
import com.remisoft.literule.server.cache.CachingRuleConfigProvider;
import com.remisoft.literule.server.cep.CEPEngine;
import com.remisoft.literule.server.core.AsyncTraceRecorder;
import com.remisoft.literule.server.core.BreakpointHook;
import com.remisoft.literule.server.core.DefaultBreakpointHook;
import com.remisoft.literule.server.core.DefaultRuleEngine;
import com.remisoft.literule.server.core.EvaluationResultCache;
import com.remisoft.literule.server.core.InMemoryRuleMetrics;
import com.remisoft.literule.server.core.MicrometerRuleMetrics;
import com.remisoft.literule.server.core.ParallelRuleEvaluator;
import com.remisoft.literule.server.core.RuleCanaryRouter;
import com.remisoft.literule.server.core.RuleCircuitBreaker;
import com.remisoft.literule.server.core.RuleDocumentationService;
import com.remisoft.literule.server.core.RuleEffectivenessService;
import com.remisoft.literule.server.core.RuleLifecycleService;
import com.remisoft.literule.server.core.RuleMetrics;
import com.remisoft.literule.server.core.RuleTimeoutExecutor;
import com.remisoft.literule.server.expr.EmptyVariableRegistry;
import com.remisoft.literule.api.expr.ExpressionEvaluator;
import com.remisoft.literule.server.expr.ExpressionValidationService;
import com.remisoft.literule.server.expr.VariableRegistry;
import com.remisoft.literule.server.expr.liteexpr.LiteExprEvaluator;
import com.remisoft.literule.server.health.LiteRuleHealthIndicator;
import com.remisoft.literule.server.replay.ExecutionReplayService;
import com.remisoft.literule.server.orchestrator.RuleChain;
...
import com.remisoft.literule.server.security.RulePermissionChecker;
import com.remisoft.literule.server.spi.CronjobTriggerActionHandler;
import com.remisoft.literule.server.spi.DecisionTableConfigProvider;
import com.remisoft.literule.server.spi.DecisionTreeConfigProvider;
import com.remisoft.literule.server.spi.DefaultAlertActionHandler;
import com.remisoft.literule.server.spi.FactProvider;
import com.remisoft.literule.server.spi.FactProviderRegistry;
import com.remisoft.literule.server.spi.DbRuleSource;
import com.remisoft.literule.server.spi.FileRuleSource;
import com.remisoft.literule.server.spi.RuleSource;
import com.remisoft.literule.server.spi.RuleSourceManager;
import com.remisoft.literule.server.spi.RuleActionDispatcher;
import com.remisoft.literule.server.spi.RuleActionHandler;
...
import com.remisoft.literule.server.spi.ScorecardConfigProvider;
import com.remisoft.literule.server.spi.ScriptConfigProvider;
import com.remisoft.literule.server.spi.TraceRecorder;
import com.remisoft.literule.server.spi.WorkflowTriggerActionHandler;
 ...
 *
 * <p>通过 {@code remi.literule.*} 配置规则文件路径、组件扫描包、监控启用等。
 *
 * @author remi-team
 * @since 1.0.0
 */
...
@EnableScheduling
@ConditionalOnProperty(prefix = "remi.literule", name = "enabled", havingValue = "true", matchIfMissing = true)
public class LiteRuleAutoConfiguration {
     ...
     *   <li>当 circuitBreakerMinEvaluations > 0 时启用 {@link RuleCircuitBreaker}（P2-14: openStateMs 由 {@code remi.literule.circuit-breaker-open-state-ms} 配置）</li>
     ...
     * {@code remi.literule.cache.enabled=true}（默认 true）时，
     ...
     *   <li>{@code remi.literule.cache.l2-enabled=true}（默认 true）</li>
     * </ul>
     * 任一不满足则仅启用 L1。
     *
    ...
    @ConditionalOnBean(RuleConfigProvider.class)
    @ConditionalOnProperty(prefix = "remi.literule.cache", name = "enabled", havingValue = "true", matchIfMissing = true)
    @Primary
     ...
     *
     * <p>可通过 {@code remi.literule.cep.enabled=false} 关闭。
     *
     * @return CEPEngine 实例
     * @since 1.0.0
     */
    ...
    @ConditionalOnProperty(
            prefix = "remi.literule.cep", name = "enabled", havingValue = "true", matchIfMissing = true)
     ...
     *
     * <p>可通过 {@code remi.literule.debug.enabled=false} 关闭。
     *
    ...
    @ConditionalOnMissingBean
    @ConditionalOnProperty(
            prefix = "remi.literule.debug", name = "enabled", havingValue = "true", matchIfMissing = true)
    ...
    @ConditionalOnProperty(prefix = "remi.literule", name = "health-enabled",
     ...
     * 数据库规则数据源 Bean（P1-11）
     *
     * <p>当 {@code RuleConfigProvider} 存在且 {@code remi.literule.rule-source.type=db}（默认）时
    ...
    @ConditionalOnBean(RuleConfigProvider.class)
    @ConditionalOnProperty(prefix = "remi.literule.rule-source", name = "type",
     ...
     * 通过 {@code remi.literule.annotation-scan-base-packages} 指定扫描基包（逗号分隔）。
     *
     ...
     * 文件规则数据源 Bean（P2-3）
     *
     * <p>当 {@code remi.literule.file-source.enabled=true} 时自动装配
    ...
    @ConditionalOnMissingBean
    @ConditionalOnProperty(
            prefix = "remi.literule.file-source", name = "enabled", havingValue = "true")
     ...
     * 模型输入注册表 Bean（P3-1）
     *
     * <p>当 {@code remi.literule.model.enabled=true} 时自动装配，聚合所有
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "remi.literule.model", name = "enabled", havingValue = "true")
     ...
     *
     * <p>当 {@code remi.literule.model.mock-enabled=true} 时自动装配，返回配置的
     * 模拟模型输出，便于开发/测试环境验证规则+模型融合能力，无需依赖真实模型服务。
     *
     * <p>输出可通过 {@code remi.literule.model.mock-outputs} 配置自定义：
     * <pre>
     * remi:
     *   literule:
     *     model:
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "remi.literule.model", name = "mock-enabled", havingValue = "true")
     ...
     *
     * <p>当 {@code remi.literule.fact.enabled=true} 时自动装配，聚合所有
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "remi.literule.fact", name = "enabled", havingValue = "true")
     ...
     * 规则动作分发器 Bean（P1-1）
     *
     * <p>当 {@code remi.literule.action.enabled=true}（默认 true）时自动装配，
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "remi.literule.action", name = "enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     * 默认告警动作处理器 Bean（P1-1）
     *
     * <p>当 {@code remi.literule.action.default-alert-enabled=true}（默认 true）时自动装配，
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "remi.literule.action", name = "default-alert-enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     *
     * <p>当 classpath 中存在 {@code CronjobServiceClient}（由 remi-cronjob-api 提供）且
     * {@code remi.literule.action.cronjob-trigger-enabled=true}（默认 true）时自动装配。
    ...
    @ConditionalOnProperty(prefix = "remi.literule.action", name = "cronjob-trigger-enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     *
     * <p>当 classpath 中存在 {@code WorkflowServiceClient}（由 remi-workflow-api 提供）且
     * {@code remi.literule.action.workflow-trigger-enabled=true}（默认 true）时自动装配。
     * 规则触发后自动启动关联的工作流流程实例。
     *
    ...
    @ConditionalOnProperty(prefix = "remi.literule.action", name = "workflow-trigger-enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     *
     * <p>默认 7 天滑动窗口，可通过 {@code remi.literule.effectiveness.window-days} 配置。
     *
    ...
    @ConditionalOnMissingBean
    @ConditionalOnProperty(
            prefix = "remi.literule.effectiveness", name = "enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     * 规则分组并行评估器（P2-3）
     *
     * <p>当 {@code remi.literule.performance.parallel-enabled=true} 时装配，
     * 将候选规则按互斥组分组并行评估。
     *
     * @param properties 配置属性
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "remi.literule.performance", name = "parallel-enabled", havingValue = "true")
     ...
     * 生成 {@link com.remisoft.literule.api.RetirementSuggestion} 建议列表。
     *
     * <p>可通过 {@code remi.literule.lifecycle.enabled=false} 关闭。
     *
     * @param ruleEngine       规则引擎
     * @param configProvider   规则配置提供者
    ...
    @ConditionalOnBean(RuleConfigProvider.class)
    @ConditionalOnProperty(
            prefix = "remi.literule.lifecycle", name = "enabled",
            havingValue = "true", matchIfMissing = true)
     ...
     *
     * <p>可通过 {@code remi.literule.lifecycle.enabled=false} 关闭（复用生命周期开关）。
     *
     * @param configProvider   规则配置提供者
     * @param ruleEngine       规则引擎
     ...
     * <p>当存在 {@link RuleAdminService} 时自动装配，
     * 记录规则全生命周期操作的审计日志。
     * 默认使用内存存储，可通过 {@link com.remisoft.literule.server.audit.RuleAuditLogService.AuditLogStore}
     * SPI 提供持久化实现。
     *
     ...
     *
     * <p>可通过 {@code remi.literule.sdk.enabled=false} 关闭。
     *
     * @param ruleEngine 规则引擎
     * @param evaluator  表达式求值器
     * @param properties 配置属性
    ...
    @ConditionalOnProperty(prefix = "remi.literule.sdk", name = "enabled",
            havingValue = "true", matchIfMissing = true)
