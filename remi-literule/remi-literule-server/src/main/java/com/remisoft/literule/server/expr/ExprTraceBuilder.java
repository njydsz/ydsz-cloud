...
public class ExprTraceBuilder {
    ...
    /**
     * 追踪节点（简化版，映射到 {@link com.remisoft.literule.server.expr.ExpressionTraceNode}）
     */
     ...
     *
     * <p>仅将变量名与求值结果作为叶节点追加到当前追踪列表，不清除已有节点，
     * 因此多个变量节点会依次累积，供后续逻辑/二元等父节点收纳为子树。
     * 非线程安全：同一 {@link ExprTraceBuilder} 不应跨线程并发使用。
     *
     * @param name  变量名
     ...
     *
     * <p>将之前累积的子节点整体作为本节点的 {@code children}，
     * 然后清空当前列表、仅保留本逻辑节点（模式：累积子节点 → 汇总为单节点）。
     * {@code shortCircuited=true} 表示从该项起短路，不再继续求值。
     *
