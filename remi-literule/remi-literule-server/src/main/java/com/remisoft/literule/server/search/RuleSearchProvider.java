package com.remisoft.literule.server.search;
...
import com.remisoft.literule.infra.mapper.RuleDefinitionMapper;
...
public class RuleSearchProvider implements SearchProvider<RuleDefinitionDO> {
    ...
    public List<SearchField> getSearchableFields() {
        return List.of(
                SearchField.builder()
                        .name("title").label("规则名称").type(FieldType.TEXT)
                        .weight(3.0f).searchable(true).highlightable(true).sortable(true)
                        .build(),
                SearchField.builder()
                        .name("subtitle").label("分类").type(FieldType.KEYWORD)
                        .weight(2.0f).searchable(true).aggregatable(true)
                        .build(),
                SearchField.builder()
                        .name("content").label("规则编码").type(FieldType.KEYWORD)
                        .weight(1.5f).searchable(true).highlightable(true)
                        .build(),
                SearchField.builder()
