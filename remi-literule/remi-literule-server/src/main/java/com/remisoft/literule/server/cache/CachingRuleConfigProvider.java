package com.remisoft.literule.server.cache;
...
import com.remisoft.literule.server.config.LiteRuleProperties;
 ...
 * 多级缓存规则配置提供者（P1-1）
 *
 * <p>装饰器模式实现的 remi-common-cache (L1) + Redis (L2) 两级缓存，
 ...
 * </ul>
 *
 * <p>并发安全：remi-common-cache 的 {@code cache.get(key, mapper)} 保证同一 key 仅一个线程执行加载，
 * 其余线程阻塞等待结果，天然防止缓存击穿。
 *
 * @since 1.0.0
 * @author remi-team
...
public class CachingRuleConfigProvider implements RuleConfigProvider {
     ...
     * 中发布的事件仍能正常触发。
     *
     * <p>由 {@link com.remisoft.literule.server.config.RuleHotReloader} 同源事件触发，
