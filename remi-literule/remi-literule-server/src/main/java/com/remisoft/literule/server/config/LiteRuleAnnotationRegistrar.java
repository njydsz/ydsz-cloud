...
import com.remisoft.literule.domain.annotation.LiteRule;
 ...
 * 将其注册到 {@link RuleEngine}；同时按 {@code remi.literule.annotation-scan-base-packages}
 * 配置的基包扫描 {@link RuleDefinition} Bean（纯声明式表达式规则）并按需包装为
 * {@link ExpressionRule} 后注册。
 *
 * <p>实现为 {@link SmartInitializingSingleton}，在所有非懒加载单例实例化之后、
 * 容器完全启动之前执行一次，确保扫描到的 Bean 全部就绪。
 *
 * <h3>使用示例</h3>
 * <pre>{@code
 * @LiteRule
...
public class LiteRuleAnnotationRegistrar implements SmartInitializingSingleton {
    ...
    public void afterSingletonsInstantiated() {
        ...
        log.info("[LiteRule-Annotation] 声明式规则注册完成: @LiteRule={}, @RuleDefinition={}",
                liteRuleCount, defMetaCount);
    }
        ...
        for (Object bean : beans.values()) {
            ...
            try {
                ...
                log.error("[LiteRule-Annotation] 注册规则失败: rule={} err={}",
                        ((Rule) bean).getCode(), e.getMessage(), e);
            }
        }
        return count;
    }
            ...
            for (RuleDefinition bean : defBeans.values()) {
                ...
                try {
                    ...
                    log.error("[LiteRule-Annotation] 注册 RuleDefinition 失败: def={} err={}",
                            ((RuleDefinition) bean).getCode(), e.getMessage(), e);
                }
            }
        }
        return count;
