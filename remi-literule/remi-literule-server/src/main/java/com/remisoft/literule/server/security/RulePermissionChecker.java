package com.remisoft.literule.server.security;
 ...
 * <p><b>权限编码格式</b>：
 * <ul>
 *   <li>{@code execution:rule:save} - 无路径段，表示全目录权限（向后兼容）</li>
 *   <li>{@code execution:rule:save:finance} - 仅 finance 一级目录</li>
 *   <li>{@code execution:rule:save:finance/*} - finance 下所有一级子目录</li>
 *   <li>{@code execution:rule:save:finance/**} - finance 下全部子目录（多级递归）</li>
 *   <li>{@code execution:rule:save:finance/credit} - 精确到 finance/credit 路径</li>
 * </ul>
 *
 * <p><b>路径匹配规则</b>：
 * <ul>
 *   <li>{@code *} 匹配单级目录（不含 {@code /}）</li>
 *   <li>{@code **} 匹配多级目录（含 {@code /}，可跨多层）</li>
...
public class RulePermissionChecker {
     ...
     * 构造权限校验器
     *
     * @param configProvider 规则配置提供者，用于按 ruleCode 查询 categoryPath；
     *                       为 null 时 hasPermissionForRule 无法解析规则路径
     */
     ...
     * <ul>
     *   <li>无 categoryPath 段（如 {@code execution:rule:save}）：全目录权限，返回 true</li>
     *   <li>含 categoryPath 段：按通配符匹配规则所属的 categoryPath</li>
     * </ul>
     *
     ...
     * 校验对特定规则的权限
     *
     * <p>自动从 {@link RuleConfigProvider} 查询规则的 categoryPath，再调用
     ...
     */
    public List<String> filterUnauthorized(String permission, Collection<String> ruleCodes, String operator) {
     ...
     */
    public Set<String> collectMatchingPatterns(Collection<String> permissions, String namespace, String action) {
     ...
     * <p>支持 Ant 风格通配符：
     * <ul>
     *   <li>{@code *} 匹配单级目录（不含 /）</li>
     *   <li>{@code **} 匹配多级目录（含 /）</li>
     *   <li>无通配符时按前缀匹配（pattern 是 path 的前缀，或 path 是 pattern 的前缀）</li>
     * </ul>
     *
     ...
     * <ul>
     *   <li>{@code **} 段：贪婪匹配，尝试剩余 path 段的全部可能位置</li>
     *   <li>{@code *} 段：匹配单段（非空）</li>
     *   <li>普通段：精确匹配</li>
     * </ul>
     */
     ...
     * 前缀匹配（无通配符场景）
     *
     * <p>规则：pattern 匹配 path 当且仅当 path 等于 pattern，或 path 是 pattern 的子目录。
     * 例如：
     * <ul>
     *   <li>pattern="finance", path="finance/credit" -> true（path 是 pattern 的子目录）</li>
     *   <li>pattern="finance/credit", path="finance/credit" -> true（完全相等）</li>
     *   <li>pattern="finance/credit", path="finance" -> false（path 是 pattern 的父目录，无权限）</li>
     *   <li>pattern="finance", path="" -> false（规则无分类，类别特定权限不匹配）</li>
     * </ul>
     */
