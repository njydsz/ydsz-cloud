package com.remisoft.literule.server.version;
 ...
 * 规则版本 Diff 服务
 *
 * <p>对两个 {@link RuleDefinition} 进行字段级结构化对比，产出 {@link RuleVersionDiff}。
 * 支持的字段对比维度：
 * <ul>
 *   <li>基本信息：code, name, category, categoryPath, description, owner, scope</li>
 *   <li>表达式：conditionExpression, severityExpression, titleTemplate, descriptionTemplate</li>
 *   <li>执行配置：priority, enabled, mutexGroup, defaultSeverity</li>
 *   <li>灰度配置：canaryRatio, canaryConditionExpression, canarySeverityExpression</li>
 *   <li>生命周期：status, effectiveFrom, effectiveTo, environment</li>
 * </ul>
 *
...
public class RuleVersionDiffService {
    ...
    public RuleVersionDiff diff(RuleDefinition oldDef, RuleDefinition newDef) {
        ...
        compareField(entries, "categoryPath", "分类路径", oldDef.getCategoryPath(), newDef.getCategoryPath());
        ...
        // 表达式
        compareField(entries, "conditionExpression", "条件表达式", oldDef.getConditionExpression(), newDef.getConditionExpression());
        compareField(entries, "severityExpression", "严重度表达式", oldDef.getSeverityExpression(), newDef.getSeverityExpression());
        compareField(entries, "titleTemplate", "标题模板", oldDef.getTitleTemplate(), newDef.getTitleTemplate());
        compareField(entries, "descriptionTemplate", "描述模板", oldDef.getDescriptionTemplate(), newDef.getDescriptionTemplate());
        ...
        compareSeverity(entries, "defaultSeverity", "默认严重度", oldDef.getDefaultSeverity(), newDef.getDefaultSeverity());

        // 灰度配置
        compareField(entries, "canaryRatio", "灰度比例", oldDef.getCanaryRatio(), newDef.getCanaryRatio());
        compareField(entries, "canaryConditionExpression", "灰度条件表达式", oldDef.getCanaryConditionExpression(), newDef.getCanaryConditionExpression());
        compareField(entries, "canarySeverityExpression", "灰度严重度表达式", oldDef.getCanarySeverityExpression(), newDef.getCanarySeverityExpression());

        // 生命周期
        compareField(entries, "status", "状态", oldDef.getStatus(), newDef.getStatus());
        compareField(entries, "effectiveFrom", "生效时间", oldDef.getEffectiveFrom(), newDef.getEffectiveFrom());
     ...
     * 对比单个字段（字符串类型）
     */
    private void compareField(List<RuleVersionDiff.DiffEntry> entries, String field, String label, Object oldVal, Object newVal) {
     ...
     * 对比严重度
     */
    private void compareSeverity(List<RuleVersionDiff.DiffEntry> entries, String field, String label,
    ...
    private String buildSummary(RuleDefinition oldDef, RuleDefinition newDef, List<RuleVersionDiff.DiffEntry> entries) {
        int modified = (int) entries.stream().filter(e -> e.getType() == RuleVersionDiff.DiffType.MODIFIED).count();
        int added = (int) entries.stream().filter(e -> e.getType() == RuleVersionDiff.DiffType.ADDED).count();
        int removed = (int) entries.stream().filter(e -> e.getType() == RuleVersionDiff.DiffType.REMOVED).count();
        return String.format("v%d → v%d: %d 项修改, %d 项新增, %d 项删除",
