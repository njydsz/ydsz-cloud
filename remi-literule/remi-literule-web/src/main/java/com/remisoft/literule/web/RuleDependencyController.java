...
import com.remisoft.literule.api.dto.RuleDependencyAddDTO;
...
import com.remisoft.literule.server.spi.RuleDependencyProvider;
 ...
 * 规则依赖 Controller
 *
 * <p>业务背景：复杂业务场景下规则之间存在依赖关系（如规则 A 触发后才能执行规则 B），
 * 通过显式声明依赖关系，规则引擎可在禁用规则时自动级联禁用下游规则，避免误判。
 *
 * <p>核心能力：
 * <ul>
 *   <li>添加/删除规则依赖</li>
 *   <li>查询正向依赖（依赖了哪些规则）与反向依赖（被哪些规则依赖）</li>
 *   <li>级联禁用影响分析</li>
 * </ul>
 *
