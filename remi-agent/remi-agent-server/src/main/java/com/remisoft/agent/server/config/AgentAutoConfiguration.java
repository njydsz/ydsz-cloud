package com.remisoft.agent.server.config;
...
import com.remisoft.agent.domain.json.AgentJsonModule;
...
import com.remisoft.agent.infra.guardrail.PiiMaskingGuardrail;
import com.remisoft.agent.infra.guardrail.PromptInjectionGuardrail;
import com.remisoft.agent.infra.llm.LlmClientRouter;
import com.remisoft.agent.infra.llm.OpenAiCompatibleClient;
import com.remisoft.agent.infra.memory.RedisConversationMemory;
import com.remisoft.agent.infra.rag.InMemoryVectorStore;
import com.remisoft.agent.infra.rag.HybridRetriever;
import com.remisoft.agent.infra.rag.OpenAiEmbeddingClient;
import com.remisoft.agent.infra.rag.PgVectorStore;
import com.remisoft.agent.infra.rag.SimpleTextChunker;
import com.remisoft.agent.infra.tool.DefaultToolRegistry;
import com.remisoft.agent.infra.tool.ToolAnnotationScanner;
import com.remisoft.agent.infra.trace.InMemoryTraceRecorder;
import com.remisoft.agent.server.agent.AgentFactory;
import com.remisoft.agent.server.agent.DagOrchestrationExecutor;
import com.remisoft.agent.server.analytics.CostAnalysisService;
import com.remisoft.agent.server.chat.AgentRequestGuard;
import com.remisoft.agent.server.chat.GuardrailService;
...
import com.remisoft.agent.server.health.AgentHealthIndicator;
import org.springframework.beans.factory.ObjectProvider;
/**
 * Agent 模块自动配置。
 *
 * <p>承担 remi-agent 微服务的核心 Bean 注册职责，包括 LLM 客户端、对话记忆、工具注册、
 *
...
@ConditionalOnProperty(prefix = "remi.agent", name = "enabled", havingValue = "true", matchIfMissing = true)
@Slf4j
public class AgentAutoConfiguration {
     ...
     *
     * <p>由 {@code JsonAutoConfiguration} 自动发现并纳入 {@code JsonModuleRegistrar}，
     * 使 {@code ChatRequest} / {@code ChatMessage} / {@code ToolCall} / {@code ToolDefinition}
     * / {@code TokenUsage} 在全局 {@code toJson} / {@code toObject} 路径中统一产出 OpenAI 契约形状，
     * 替代此前在 {@link OpenAiCompatibleClient} 中手工拼装 JSON 的做法，避免契约多处维护而漂移。
     *
     ...
     *
     * <p>先以 {@code remi.agent.llm} 下的默认配置注册主 Provider（它同时成为
     * {@link LlmClientRouter} 的兜底 defaultClient），再遍历 {@code providers} 注册多模型；
     * 配置中 {@code enabled=false} 的条目会被跳过，便于灰度开关某个 Provider 而无需删配置。
     * 所有 Provider 共用同一份 {@code timeoutSeconds} 超时设置。
     *
     * <p><b>降级</b>：路由器对网络超时、限流、Provider 5xx 自动切换备用 Provider；
     * 认证失败与模型不存在不降级，直接抛出，避免无效重试放大故障。
     *
     ...
     * 装配基于 Redis 的对话记忆存储。
     *
     * <p>Redis List 的容量取「记忆滑动窗口 2 倍」与「至少 50」的较大值：
     * 多留一倍余量是为了防止窗口边界处的消息被 trim 掉，导致下一轮取不满上下文。
     * 记忆按 {@code ttlHours} 自动过期，超期对话不再计入上下文，也无需额外清理任务。
     *
     ...
     *
     * <p>默认实现为内存 {@link DefaultToolRegistry}，注册表随进程存活；
     * 多实例部署时各节点各自持有一份，因此工具注册必须在<b>每个节点启动时</b>幂等完成，
     * 不能依赖运行期单点注册。
     *
     ...
     *
     * <p>默认使用内存实现（容量 1000 条、TTL 24 小时，超限按最旧淘汰），
     * 仅适合开发调试与短周期排障；<b>重启即丢失、且多实例数据不互通</b>，
     * 生产环境若需长期留存应覆盖为数据库或 APM 实现。
     *
     * @return 链路记录器；仅在容器中不存在其他 {@link TraceRecorder} 时生效
     */
    @Bean
     ...
     * 装配提示词注入防护（输入侧护栏）。
     *
     * <p>在用户输入进入 Prompt 之前拦截"忽略以上指令"一类的越权诱导，防止系统提示被覆盖。
     * 注意 {@link ConditionalOnMissingBean} 作用于 {@link InputGuardrail} 类型：
     * 一旦业务侧自定义了任意输入护栏，本默认护栏<b>不会再注册</b>，需自行保留注入防护能力。
     *
     * @return 输入护栏实例
     */
    @Bean
     ...
     * 装配 PII 脱敏护栏（输出侧护栏）。
     *
     * <p>在模型回复返回前对手机号、身份证、邮箱等个人敏感信息做掩码，满足合规要求。
     * 同样受 {@link ConditionalOnMissingBean} 类型级约束：业务侧自定义任意
     ...
     *
     * <p>{@code rag.embeddingApiKey} / {@code rag.embeddingBaseUrl} 留空时自动回落到
     * LLM 的同名配置——多数场景下向量与对话走同一家 Provider，可少配一套凭据；
     * 只有向量服务独立部署时才需要单独指定。
     *
     * <p><b>注意</b>：{@code rag.dimension} 必须与向量库中已有数据的维度一致，
     * 中途改动会导致既有向量无法检索，需要重新灌库。
     *
     ...
     * 重叠是为了避免语义在切割点被割裂导致召回缺失；重叠越大召回越稳，但索引体积与
     * Embedding 调用成本同步上升，需权衡配置。
     *
     ...
     * 探测通过（扩展已安装、表结构就绪）时才使用数据库存储；否则一律回落到
     * {@link InMemoryVectorStore}，保证 RAG 能力在环境未就绪时仍可用于开发联调。
     *
     * <p><b>降级代价</b>：内存实现的向量随进程消失且各实例数据不共享，
     * 生产环境务必确认日志中未出现降级，否则检索结果会因节点而异。
     *
     ...
     *
     * <p>Spring 按类型注入<b>全部</b> {@link InputGuardrail} / {@link OutputGuardrail} 实现，
     * 执行顺序取决于 Bean 的排序（可用 {@code @Order} 干预）；业务自定义护栏会与默认护栏
     * 一并生效，无需修改本配置。拦截结果通过 {@link AgentMetrics} 上报，便于观测误杀率。
     *
     ...
     *
     * <p>这是本配置类的<b>汇聚点</b>：模型调用、记忆、工具、RAG、护栏、链路追踪、
     * 指标与成本核算在此拼装成完整执行链路。任一上游 Bean 被业务覆盖，
     * 都会透明地反映到工厂创建出的 Agent 上。
     *
     ...
     *
     * <p>强制从容器取名为 {@code agentDagExecutor} 的虚拟线程池（由 common-thread 统一托管），
     * 而非自行 new 线程池——集中管理才能统一监控与优雅停机，也避免各模块各建一套导致线程膨胀。
     * 该 Bean 缺失时容器启动会直接失败，属于<b>强依赖</b>，不做静默降级。
     *
     ...
     *
     * <p>向量存储、链路记录、成本核算、指标四项以 {@link ObjectProvider} 惰性注入：
     * 它们属于<b>可选能力</b>，缺失时健康检查跳过对应项而非报错，
     * 保证裁剪部署（如关闭 RAG）时应用仍能通过健康探针。
     * LLM 客户端与对话记忆则为强依赖，直接注入。
     *
