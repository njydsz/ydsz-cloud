 ...
 *
 * <h3>幂等去重</h3>
 * <p>基于 Redis SETNX，key = {@code remi:agent:idem:{requestId}}，TTL 60s。
 * 同一 requestId 60 秒内只能成功调用一次。
 *
 * <h3>限流</h3>
 * <p>基于 Redis INCR + EXPIRE 实现固定窗口计数，key = {@code remi:agent:rate:{userId}}，
 * 默认 10 QPM（每分钟 10 次）。
 *
 * @author remi-team
 * @since 1.0.0
...
public class AgentRequestGuard {
    ...
    private static final String IDEM_KEY_PREFIX = "remi:agent:idem:";
    private static final String RATE_KEY_PREFIX = "remi:agent:rate:";
