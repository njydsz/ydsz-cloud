 ...
 * 摘要压缩对话记忆
 *
 * <p>包装 {@link ConversationMemory}，当消息数超过 {@link #summaryThreshold} 时，
 * 自动将较早的消息通过 LLM 压缩为摘要，只保留最近 {@link #keepRecentCount} 条原始消息。
 *
 * <h3>对标竞品</h3>
 * <ul>
 *   <li>LangChain ConversationSummaryBufferMemory</li>
 *   <li>OpenAI Assistants Thread（自动截断 + 摘要）</li>
 *   <li>Dify 对话记忆压缩</li>
 * </ul>
 *
 * <h3>执行流程</h3>
...
public class SummaryConversationMemory implements ConversationMemory {
    ...
    public SummaryConversationMemory(ConversationMemory delegate, LlmClient llmClient,
                                      String model, int summaryThreshold, int keepRecentCount) {
    ...
    private void tryCompress(String conversationId) {
        try {
            ...
            if (summary != null && !summary.isBlank()) {
                ...
                log.info("[Memory-Summary] 对话压缩完成: convId={}, compressed={}, kept={}",
                        conversationId, oldMessages.size(), recentMessages.size());
            }
        } catch (Exception e) {
            log.warn("[Memory-Summary] 对话压缩失败: convId={}, error={}",
     ...
     * 调用 LLM 生成摘要
     */
    private String summarizeMessages(String conversationId, List<ChatMessage> messages,
