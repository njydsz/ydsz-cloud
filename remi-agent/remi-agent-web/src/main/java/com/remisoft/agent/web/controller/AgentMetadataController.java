 ...
 * <ul>
 *   <li>{@code GET /api/v1/agent/models} - 获取可用模型 / Provider 列表</li>
 *   <li>{@code GET /api/v1/agent/tools} - 获取已注册工具列表</li>
 * </ul>
 *
 * <p>这些接口供前端 Agent 编辑器渲染「可用模型」下拉选择器和「可用工具」勾选列表，
 * 不涉及 Agent 执行逻辑。Agent 的同步 / 流式执行见 {@link AgentController}。
 *
 * <h3>核心能力</h3>
 * <ul>
 *   <li>多 LLM Provider 元数据暴露（通过 {@link LlmClientRouter} 统一抽象）</li>
 *   <li>工具注册中心元数据暴露（通过 {@link ToolRegistry} 查询）</li>
 * </ul>
 *
 * <h3>设计原则</h3>
 * <p>本 Controller 仅做参数透传与 VO 转换，所有元数据来源由
 * {@link LlmClient} 与 {@link ToolRegistry} 提供，不包含业务编排逻辑。
 *
 * @author remi-team
 * @since 1.0.0
 *
