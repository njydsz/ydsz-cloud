 ...
 * 用户-部门关联实体
 *
 * <p>对应数据库表 {@code remi_user_dept}，是连接用户与部门的多对多中间表。
 * 支持用户兼职多个部门（兼岗），通过 {@link #isPrimary} 字段标识主部门。
 *
 * <p><b>设计要点：</b>
 * <ul>
 *   <li>一个用户可有多个部门，但只能有 1 个主部门（{@code isPrimary=1}）</li>
 *   <li>主部门用于：① 默认审批人；② 默认工时归属；③ 默认数据范围</li>
 *   <li>辅部门用于：① 跨部门项目协作；② 多部门审批人展开</li>
 * </ul>
 *
 * <p><b>主部门唯一性约束：</b>应在 Service 层校验「同一用户只允许一个主部门」，
 * 通过事务保证 + 加锁实现。SQL 层可通过部分唯一索引实现：
 * <pre>{@code
 * CREATE UNIQUE INDEX uk_user_primary_dept ON remi_user_dept (user_id) WHERE is_primary = 1;
 * }</pre>
 *
 * <p><b>典型使用：</b>
 * <pre>{@code
