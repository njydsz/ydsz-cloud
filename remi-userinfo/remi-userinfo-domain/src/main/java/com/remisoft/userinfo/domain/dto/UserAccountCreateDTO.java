 ...
 * 用户创建请求 DTO。
 *
 * <p>用于 {@code POST /api/v1/user} 接口，创建系统用户账号。
 * 创建后账号默认状态为「启用」，密码经 BCrypt 加密后存储。
 *
 * <p><b>校验规则：</b>
 * <ul>
 *   <li>{@code username} — 必填，全局唯一，长度 ≤ 64</li>
 *   <li>{@code password} — 必填，长度 8-64，须符合密码策略（大小写+数字+特殊字符）</li>
 *   <li>{@code realName} — 必填，长度 ≤ 64</li>
 *   <li>{@code phone} / {@code email} — 可选，须符合格式校验</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
...
public class UserAccountCreateDTO implements Serializable {
    ...
    /** 所属部门 ID（关联 {@code remi_department.id}，支持审批人展开） */
    private String deptId;

    /** 直属上级用户 ID（关联 {@code remi_user_account.id}，支持 leader: 审批人展开） */
