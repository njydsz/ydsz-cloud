...
| **用户字段** | 用户自定义字段扩展（`remi_user_field`） |
| **国际化** | 语言 CRUD（`remi_language`） |
...
remi-userinfo/
├── pom.xml
├── remi-userinfo-api/                 # API 层：Feign Client + Fallback
│   └── src/main/java/com/remisoft/userinfo/api/
│       ├── client/                    # UserServiceClient / OrgQueryClient
│       └── fallback/                  # Feign 降级实现
├── remi-userinfo-domain/              # 领域层：Entity + DTO + VO
│   └── src/main/java/com/remisoft/userinfo/domain/
...
├── remi-userinfo-infra/               # 基础设施层：Mapper + Repository
│   └── src/main/java/com/remisoft/userinfo/infra/mapper/
├── remi-userinfo-server/              # 应用层：Service + Config + Health
│   └── src/main/java/com/remisoft/userinfo/server/
│       ├── config/                    # UserInfoProperties + UserInfoConfiguration
│       ├── service/                   # Auth/RBAC/Org/Menu/OAuth2 Service
│       └── health/                    # UserInfoHealthIndicator
└── remi-userinfo-web/                 # Web 层：Controller + Bootstrap
    └── src/main/java/com/remisoft/userinfo/web/
...
| 表名 | 说明 |
|---|---|
| `remi_user_account` | 用户账号（含登录失败计数/锁定时间/最后登录信息） |
| `remi_role` | 角色定义 |
| `remi_menu` | 菜单/按钮/API 权限定义 |
| `remi_user_role` | 用户-角色关联 |
| `remi_role_permission` | 角色-权限关联 |
| `remi_department` | 部门（树形结构） |
| `remi_company` | 公司 |
| `remi_company_dept` | 公司-部门关联 |
| `remi_post` | 岗位 |
| `remi_user_dept` | 用户-部门关联 |
| `remi_user_post` | 用户-岗位关联 |
| `remi_user_field` | 用户自定义字段 |
| `remi_language` | 语言 |
| `remi_login_audit` | 登录审计（物理表在 remi-system） |
| `remi_user_session` | 用户会话（Redis 存储） |
| `remi_user_2fa` | 双因子认证密钥 |
...
| 配置项 | 默认值 | 说明 |
|---|---|---|
| `remi.userinfo.health-enabled` | `true` | 是否启用健康检查 |
| `remi.auth.token.enabled` | `true` | 是否启用 JWT Token 服务 |
| `remi.auth.token.secret-key` | （必填） | JWT 签名密钥（至少 32 字符） |
| `remi.auth.token.access-token-expire-seconds` | `7200` | Access Token 有效期（秒） |
| `remi.auth.token.refresh-token-expire-seconds` | `604800` | Refresh Token 有效期（秒） |
| `remi.auth.ldap.enabled` | `false` | 是否启用 LDAP/ADFS 域认证 |
| `remi.auth.ldap.host` | — | LDAP 服务器地址 |
| `remi.auth.ldap.port` | `389` | LDAP 端口 |
| `remi.auth.ldap.domain` | — | LDAP 域后缀（如 `@remisoft`） |

## 常见问题

### Q1：登录失败 "账号或密码错误" 但密码正确

1. 检查 `remi_user_account` 中 `status = 'ENABLED'` 且 `deleted = 0`
...
3. `remi.auth.token.secret-key` 被修改过

### Q3：LDAP 认证失败

1. 检查 `remi.auth.ldap.enabled=true`
2. 检查 LDAP 服务器连通性
3. 用户名需去掉域后缀（系统自动拼接 `domain`）

### Q4：菜单树不显示

1. 检查 `remi_menu` 表是否有数据且 `status = 'ENABLED'`
2. 检查用户角色是否关联了对应菜单权限
3. 菜单树按 `parent_id` 递归构建，根节点 `parent_id = 0`

---

> 本模块是 REMI 的**身份认证与组织架构中心**，所有业务模块通过 Feign 调用获取用户/部门信息。
