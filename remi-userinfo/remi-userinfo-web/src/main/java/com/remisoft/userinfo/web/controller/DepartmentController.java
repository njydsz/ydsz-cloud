 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.userinfo.server.service.DepartmentService 部门业务逻辑
 */
@RestController
...
public class DepartmentController {
    ...
    @Idempotent(key = "remi:userinfo:DepartmentController:create:lock", ttlSeconds = 5)
    @PostMapping
    ...
    @Audit(module = "部门管理", type = AuditType.OPERATION, action = AuditAction.UPDATE,
            content = "'更新部门: ' + #dto.id")
    @Idempotent(key = "remi:userinfo:DepartmentController:update:lock", ttlSeconds = 5)
    @PutMapping
    ...
    @Idempotent(key = "remi:userinfo:DepartmentController:remove:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
