...
import com.remisoft.userinfo.domain.dto.ResetPasswordDTO;
import com.remisoft.userinfo.domain.dto.UserAccountCreateDTO;
import com.remisoft.userinfo.domain.dto.UserAccountPageQueryDTO;
import com.remisoft.userinfo.domain.dto.UserAccountUpdateDTO;
 ...
 * 是用户中心服务（remi-userinfo）最核心的 Controller，被各业务模块通过 Feign
 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.userinfo.server.service.UserAccountService 用户业务逻辑
...
public class UserAccountController {
    ...
    @Idempotent(key = "remi:userinfo:UserAccountController:create:lock", ttlSeconds = 5)
    ...
    @Audit(module = "用户管理", type = AuditType.OPERATION, action = AuditAction.UPDATE,
            content = "'更新用户: ' + #dto.id")
    @Idempotent(key = "remi:userinfo:UserAccountController:update:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:userinfo:UserAccountController:remove:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
    ...
    @Idempotent(key = "remi:userinfo:UserAccountController:changePassword:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:userinfo:UserAccountController:resetPassword:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:userinfo:UserAccountController:assignRoles:lock", ttlSeconds = 5)
