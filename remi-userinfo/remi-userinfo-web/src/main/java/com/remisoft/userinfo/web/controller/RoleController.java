 ...
 * </ul>
 *
 * <p><b>权限分配设计：</b>角色 → 菜单（{@code remi_menu}）通过 {@code remi_role_menu} 中间表多对多关联。
 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.userinfo.server.service.RoleService 角色业务逻辑
 * @see com.remisoft.userinfo.web.controller.MenuController 菜单 Controller（权限分配的目标对象）
 * @see com.remisoft.userinfo.web.controller.UserAccountController 用户 Controller（角色授予目标）
 */
@RestController
...
public class RoleController {
    ...
    @Idempotent(key = "remi:userinfo:RoleController:create:lock", ttlSeconds = 5)
    ...
    @Audit(module = "角色管理", type = AuditType.OPERATION, action = AuditAction.UPDATE,
            content = "'更新角色: ' + #dto.id")
    @Idempotent(key = "remi:userinfo:RoleController:update:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:userinfo:RoleController:remove:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
    ...
    @Idempotent(key = "remi:userinfo:RoleController:assignPermissions:lock", ttlSeconds = 5)
