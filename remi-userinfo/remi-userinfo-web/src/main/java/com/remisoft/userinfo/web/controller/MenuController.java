...
import com.remisoft.userinfo.domain.vo.MenuTreeVO;
import com.remisoft.userinfo.domain.vo.MenuVO;
import com.remisoft.userinfo.server.service.MenuService;

import io.swagger.v3.oas.annotations.Operation;
import io.swagger.v3.oas.annotations.tags.Tag;
import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;
import com.remisoft.userinfo.domain.dto.post.MenuPostDTO;
import com.remisoft.userinfo.domain.dto.put.MenuPutDTO;

/**
 * 菜单/权限 Controller
 *
 * <p>提供菜单的完整管理能力（CRUD）、菜单树查询、当前用户菜单查询。
 * 菜单（{@code remi_menu}）是 RBAC 模型中最细粒度的「权限点」，既可以表示前端路由节点，
 ...
 *   <li>菜单树查询（{@code /tree}）：返回整棵菜单树，前端管理系统渲染</li>
 *   <li>当前用户菜单查询（{@code /current}）：根据当前用户角色，返回可见菜单（树形）</li>
 *   <li>菜单启用/禁用（{@code /enable} / {@code /disable}）</li>
 * </ul>
 *
 * <p><b>菜单 vs 权限码：</b>
 * <ul>
 *   <li>本 Controller 管理的「菜单」含前端路由信息和权限码 {@code permCode}</li>
 *   <li>角色-权限分配通过 {@link RoleController#assignPermissions} 绑定菜单到角色</li>
 *   <li>后端接口鉴权通过 {@code @AuthApiPermission(apiCodes=...)} 引用同一份 {@code permCode}</li>
 * </ul>
 *
 * <p><b>安全特性：</b>
 * <ul>
 ...
 *   <li>菜单删除会校验是否被角色引用（避免悬挂引用）</li>
 *   <li>菜单变更会触发权限缓存失效（通过 {@code PermissionCacheInvalidator}）</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.userinfo.server.service.MenuService 菜单业务逻辑
 * @see com.remisoft.userinfo.domain.entity.Menu 菜单实体（含 permCode 字段）
 * @see com.remisoft.userinfo.web.controller.RoleController 角色 Controller（关联分配）
 */
@RestController
...
public class MenuController {
    ...
    @Idempotent(key = "remi:userinfo:MenuController:create:lock", ttlSeconds = 5)
    ...
    @Audit(module = "菜单管理", type = AuditType.OPERATION, action = AuditAction.UPDATE,
            content = "'更新菜单: ' + #dto.id")
    @Idempotent(key = "remi:userinfo:MenuController:update:lock", ttlSeconds = 5)
     ...
     * <p>删除前置校验：
     * <ul>
     *   <li>有<b>子菜单</b>的菜单<b>禁止删除</b></li>
     *   <li>有<b>角色引用</b>的菜单<b>禁止删除</b>（避免悬挂引用）</li>
     * </ul>
    ...
    @Idempotent(key = "remi:userinfo:MenuController:remove:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
