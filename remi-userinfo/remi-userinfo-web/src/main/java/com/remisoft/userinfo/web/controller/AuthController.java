...
import com.remisoft.userinfo.server.auth.AuthService;
 ...
 *   <li>密码错误次数超限自动锁定账号（{@code remi.auth.login-fail-threshold}）</li>
 * </ul>
 *
 * <p><b>Token 设计：</b>
 * <ul>
 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.userinfo.server.auth.AuthService 认证业务逻辑
 * @see com.remisoft.userinfo.web.controller.OAuth2Controller OAuth2.0 授权端点
 */
@RestController
...
public class AuthController {
     ...
     * {@code remi.auth.lock-duration-minutes} 分钟（默认 30 分钟）。
     * <p>成功登录会重置失败计数；失败累加计数到 {@code remi.auth.login-fail-threshold}（默认 5 次）触发锁定。
     *
    ...
    @Idempotent(key = "remi:userinfo:AuthController:login:lock", ttlSeconds = 5)
    @PostMapping("/login")
     ...
     * <p>启用分布式锁（{@code remi:userinfo:AuthController:refresh:lock}）防止并发重放。
     * <p>限流 50 QPS；5 秒幂等保护。
     *
    ...
    @Idempotent(key = "remi:userinfo:AuthController:refresh:lock", ttlSeconds = 5)
    @PostMapping("/refresh")
