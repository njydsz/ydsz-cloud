 ...
 * @see com.remisoft.userinfo.web.controller.UserAccountController 用户 Controller（兼任岗位维护）
 */
@RestController
...
public class PostController {
    ...
    @Idempotent(key = "remi:userinfo:PostController:create:lock", ttlSeconds = 5)
    ...
    @Audit(module = "岗位管理", type = AuditType.OPERATION, action = AuditAction.UPDATE,
            content = "'更新岗位: ' + #dto.id")
    @Idempotent(key = "remi:userinfo:PostController:update:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:userinfo:PostController:remove:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
