...
import com.remisoft.userinfo.domain.query.LanguagePageQuery;
import com.remisoft.userinfo.domain.vo.LanguageVO;
import com.remisoft.userinfo.server.service.LanguageService;
 ...
 *   <li>后端消息文案（{@code LocaleContextHolder} 匹配 {@code remi_i18n_message} 表）</li>
 ...
 * @see com.remisoft.userinfo.domain.entity.Language 语言实体
 */
@RestController
...
public class LanguageController {
    ...
    @Idempotent(key = "remi:userinfo:LanguageController:create:lock", ttlSeconds = 5)
    ...
    @Audit(module = "语言管理", type = AuditType.OPERATION, action = AuditAction.UPDATE,
            content = "'更新语言: ' + #dto.id")
    @Idempotent(key = "remi:userinfo:LanguageController:update:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:userinfo:LanguageController:remove:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
