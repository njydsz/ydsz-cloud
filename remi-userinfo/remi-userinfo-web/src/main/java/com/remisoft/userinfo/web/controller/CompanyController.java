 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.userinfo.server.service.CompanyService 公司业务逻辑
...
public class CompanyController {
    ...
    @Idempotent(key = "remi:userinfo:CompanyController:create:lock", ttlSeconds = 5)
    @PostMapping
    ...
    @Audit(module = "公司管理", type = AuditType.OPERATION, action = AuditAction.UPDATE,
            content = "'更新公司: ' + #dto.id")
    @Idempotent(key = "remi:userinfo:CompanyController:update:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:userinfo:CompanyController:remove:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
