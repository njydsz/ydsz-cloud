package com.remisoft.userinfo.server.metrics;
 ...
 *
 * <p>P0-2 架构优化：继承 {@link AbstractModuleMetrics}，统一指标前缀 {@code remi_userinfo_}，
 ...
 * <ul>
 *   <li>{@code remi_userinfo_logins_total{result=success|fail}} — 登录成功/失败计数</li>
 *   <li>{@code remi_userinfo_auth_duration_ms} — 认证耗时分布（P50/P90/P99）</li>
 *   <li>{@code remi_userinfo_online_sessions} — 在线会话数（Gauge）</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
...
public class UserInfoMetrics extends AbstractModuleMetrics {
    ...
    public UserInfoMetrics(MeterRegistry meterRegistry) {
        super(meterRegistry, "remi_userinfo_");
     ...
     * 记录一次登录成功。
     *
     * <p>累加 {@code remi_userinfo_logins_total{result=success}} 计数器，并将在线会话
     * Gauge {@code remi_userinfo_online_sessions} +1。应在登录链路「鉴权通过且会话已建立」
     ...
     *
     * <p>仅累加 {@code remi_userinfo_logins_total{result=fail}} 计数器；<b>不</b>改变在线
    ...
    /**
     * 记录一次登出，将在在线会话 Gauge {@code remi_userinfo_online_sessions} -1。
     *
     ...
     * 关闭，从而记录 {@code remi_userinfo_auth_duration_ms} 的 P50/P90/P99 分布。每次调用
    ...
    /**
     * 结束一次认证耗时采样并记录到 {@code remi_userinfo_auth_duration_ms}。
     *
