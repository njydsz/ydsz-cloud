 ...
 * 岗位 Service 接口
 *
 * <p>封装岗位的完整业务逻辑：CRUD、跨服务名称富化。
 * 岗位是「职责维度」，描述用户做什么事（如 PM、DEV、QA），区别于角色（权限维度）。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li>岗位 CRUD</li>
 *   <li>岗位全量列表查询（按 {@code sortOrder} 升序）</li>
 *   <li>跨服务名称富化（{@code batchNamesByIds}，供 NameAssembler 调用）</li>
 * </ul>
 *
 * <p><b>典型使用：</b>
 * <ul>
 *   <li>前端岗位管理页面通过 {@code /api/v1/post/list} 加载岗位列表</li>
 *   <li>用户列表通过 {@code NameAssembler} 自动富化 {@code postName} 字段</li>
 *   <li>审批人展开：{@code position:PM} 触发时按 {@code postCode} 匹配</li>
 * </ul>
 *
 ...
 * @see com.remisoft.userinfo.web.controller.PostController 岗位 Controller
 */
public interface PostService {
     ...
     *
     * <p>实现：单条 SQL {@code SELECT id, post_name FROM remi_post WHERE id IN (...)}，
