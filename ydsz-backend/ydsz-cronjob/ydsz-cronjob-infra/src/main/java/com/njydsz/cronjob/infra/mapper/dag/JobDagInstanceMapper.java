...
public interface JobDagInstanceMapper extends BaseMapper<JobDagInstance> {
     ...
     */
    @Update("UPDATE ydsz_job_dag_instance SET status = #{toStatus}, updated_at = CURRENT_TIMESTAMP "
     ...
     */
    @Update("UPDATE ydsz_job_dag_instance SET status = 'RUNNING', started_at = #{startedAt}, "
     ...
     */
    @Update("UPDATE ydsz_job_dag_instance SET status = #{finalStatus}, finished_at = #{finishedAt}, "
     ...
     */
    @Update("UPDATE ydsz_job_dag_instance SET context_json = #{contextJson}, "
            + "       updated_at = CURRENT_TIMESTAMP "
     ...
     */
    @Update("UPDATE ydsz_job_dag_instance "
     ...
     */
    @Update("UPDATE ydsz_job_dag_instance SET status = 'PAUSED', updated_at = CURRENT_TIMESTAMP "
            + "WHERE id = #{instanceId} AND status = 'RUNNING' AND deleted = 0")
     ...
     */
    @Update("UPDATE ydsz_job_dag_instance SET status = 'RUNNING', updated_at = CURRENT_TIMESTAMP "
     ...
     * @return 受影响行数
     */
    @Update("UPDATE ydsz_job_dag_instance SET status = 'CANCELED', finished_at = #{finishedAt}, "
