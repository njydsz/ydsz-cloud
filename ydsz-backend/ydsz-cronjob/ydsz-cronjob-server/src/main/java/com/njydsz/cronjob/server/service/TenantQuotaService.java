 ...
 * 默认禁用（{@code ydsz.cronjob.quota.enabled=false}），启用后在任务创建/派发时自动校验。
 *
 * <h3>配额优先级</h3>
 * <ol>
 *   <li>DB 记录（{@code ydsz_tenant_quota} 表对应租户的配置）</li>
 *   <li>全局默认（{@code ydsz.cronjob.quota.default-max-*} 配置）</li>
...
public interface TenantQuotaService {
     ...
     *
     * <p>当 {@code ydsz.cronjob.quota.enabled=false} 时直接返回（不检查）。
     ...
     *
     * <p>通过 Redis 实时计数器 {@code ydsz:quota:concurrent:{tenantId}} 获取当前并发数，
     ...
     *
     * <p>通过 Redis 日计数器 {@code ydsz:quota:daily:{tenantId}:{yyyyMMdd}} 获取当日执行数，
     ...
     * <ul>
     *   <li>{@code ydsz:quota:concurrent:{tenantId}}（并发计数，需在 {@link #recordExecutionEnd} 中 DECR）</li>
     *   <li>{@code ydsz:quota:daily:{tenantId}:{yyyyMMdd}}（日执行计数，不释放）</li>
     * </ul>
     *
     ...
     * 记录任务执行结束（P7-3）。
     *
     * <p>DECR {@code ydsz:quota:concurrent:{tenantId}} 计数器，保证不会为负。
     * 日执行计数器不释放（跨天自动过期）。
