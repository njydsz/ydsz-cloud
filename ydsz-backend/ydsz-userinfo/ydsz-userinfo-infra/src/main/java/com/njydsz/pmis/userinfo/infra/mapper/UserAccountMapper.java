...
public interface UserAccountMapper extends BaseMapper<UserAccountDO> {
     ...
     * @return 用户 ID 列表
     */
    @Select("SELECT id FROM ydsz_user_account WHERE dept_id = #{deptId} AND status = 'ENABLED' AND deleted = 0")
     ...
     * @return 用户 ID 列表
     */
    @Select("SELECT id FROM ydsz_user_account WHERE position_code = #{positionCode} AND status = 'ENABLED' AND deleted = 0")
     ...
     */
    @Select("SELECT leader_id FROM ydsz_user_account WHERE id = #{userId} AND deleted = 0")
     ...
     */
    @Select("SELECT dept_id FROM ydsz_user_account WHERE id = #{userId} AND deleted = 0")
