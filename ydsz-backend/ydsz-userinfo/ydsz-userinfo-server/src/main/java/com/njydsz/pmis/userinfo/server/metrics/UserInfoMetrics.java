 ...
 * </ul>
 *
 * <p>所有指标前缀 {@code ydsz_user_}，便于在 Grafana 看板中筛选。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class UserInfoMetrics extends AbstractModuleMetrics {
    ...
    public UserInfoMetrics(MeterRegistry meterRegistry, JdbcTemplate jdbcTemplate) {
        super(meterRegistry, "ydsz_user_");
        this.jdbcTemplate = jdbcTemplate;
    }

    @PostConstruct
    public void init() {
        Gauge.builder("ydsz_user_online_count", onlineUsers, AtomicLong::doubleValue)
                .description("在线用户数（活跃会话去重）")
                .register(registry);

        Gauge.builder("ydsz_user_active_sessions", activeSessions, AtomicLong::doubleValue)
                .description("活跃会话数")
                .register(registry);

        Gauge.builder("ydsz_user_locked_accounts", lockedAccounts, AtomicLong::doubleValue)
                .description("锁定账号数")
                .register(registry);

        Gauge.builder("ydsz_user_disabled_accounts", disabledAccounts, AtomicLong::doubleValue)
                .description("停用账号数")
                .register(registry);

        Gauge.builder("ydsz_user_total", totalUsers, AtomicLong::doubleValue)
                .description("总用户数")
                .register(registry);

        log.info("[UserInfoMetrics] 指标注册完成: ydsz_user_online_count, ydsz_user_active_sessions, ydsz_user_locked_accounts, ydsz_user_disabled_accounts, ydsz_user_total");
    }
        ...
        try {
            ...
            activeSessions.set(
                    jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM ydsz_user_session WHERE status = 'ACTIVE' AND expire_at > NOW()",
                            Long.class));
            // 在线用户数（去重）
            onlineUsers.set(
                    jdbcTemplate.queryForObject(
                            "SELECT COUNT(DISTINCT user_id) FROM ydsz_user_session WHERE status = 'ACTIVE' AND expire_at > NOW()",
                            Long.class));
            // 锁定账号数
            lockedAccounts.set(
                    jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM ydsz_user_account WHERE locked_until IS NOT NULL AND locked_until > NOW()",
                            Long.class));
            // 停用账号数
            disabledAccounts.set(
                    jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM ydsz_user_account WHERE status = 'DISABLED'",
                            Long.class));
            // 总用户数
            totalUsers.set(
                    jdbcTemplate.queryForObject(
                            "SELECT COUNT(*) FROM ydsz_user_account WHERE deleted = 0",
                            Long.class));
        } catch (Exception e) {
