 ...
 * 公司-部门关联实体
 *
 * <p>对应数据库表 {@code ydsz_company_dept}，是连接公司与部门的多对多中间表。
 * 一个部门可被多个公司共享（如「研发中心」归属于集团总部和子公司），
 * 一个公司可包含多个部门（含跨公司调岗场景）。
 *
 * <p><b>设计要点：</b>
 * <ul>
 *   <li>采用「关系实体」模式（带审计字段），便于追溯归属关系变更</li>
 *   <li>由 {@code CompanyDeptController} 维护（批量绑定/解绑）</li>
 *   <li>主归属逻辑由 {@link Department#getLeaderId()} 维护（非本中间表）</li>
 * </ul>
 *
 * <p><b>与 {@link UserDept} 的区别：</b>
 * <ul>
 *   <li>{@code CompanyDept}：组织结构维度（公司 → 部门）</li>
 *   <li>{@code UserDept}：人员归属维度（用户 → 部门）</li>
 *   <li>两者正交，可独立维护</li>
 * </ul>
 *
 * <p><b>典型使用：</b>
 ...
 * }</pre>
 *
 * <p><b>索引设计：</b>普通索引 {@code idx_company_id}（{@code company_id}）、
