 ...
 * XSS JSON 自动配置。
 *
 * <p>当 {@code ydsz.safe.xss.enabled=true} 时，通过 {@link SafeJsonModule}（YdszJsonModule SPI）
 * 自动将 {@link XssStringDeserializer} 注册到全局 YdszJson 引擎，
 * 使 JSON 反序列化时自动对 String 字段进行 XSS 清洗。
 *
 * <p>注册方式由原来的手动 {@code YdszJson.register()} 改为 Spring Bean 自动发现，
...
public class XssAutoConfiguration {
    ...
    /**
     * 注册 Safe JSON 模块（YdszJsonModule SPI）。
     *
     * <p>此 Bean 会被 {@code JsonAutoConfiguration} 中的
     * {@code JsonModuleRegistrar} 自动发现并注册到 YdszJson 引擎。
     *
     * @return Safe JSON 模块实例
     */
    @Bean
    public YdszJsonModule safeJsonModule() {
        log.debug("[XssAutoConfiguration] Registering SafeJsonModule via YdszJsonModule SPI");
