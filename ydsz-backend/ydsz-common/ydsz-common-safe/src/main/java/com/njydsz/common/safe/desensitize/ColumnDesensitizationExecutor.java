 ...
 * 列脱敏执行器。
 *
 * <p>线程安全的单例执行器，负责根据 {@link ColumnDesensitizationContext.DesensitizationRuleConfig}
 * 对字符串值执行脱敏处理。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li>缓存编译后的正则表达式，避免重复编译开销</li>
 *   <li>支持内置规则（{@link ColumnDesensitizationRule}）与自定义规则（CUSTOM）</li>
 *   <li>对 null/空值、null 规则等边界场景进行兜底处理</li>
 * </ul>
 *
 * <p><b>典型用法：</b>
 * <pre>{@code
...
public final class ColumnDesensitizationExecutor {
     ...
     * <p>处理规则：
     * <ul>
     *   <li>config 为 null 或 value 为 null/空 → 原样返回</li>
     *   <li>规则为 null（仅标记需脱敏） → 返回 {@link #DEFAULT_MASK}</li>
     *   <li>规则为 CUSTOM 且自定义正则为空 → 返回 {@link #DEFAULT_MASK}</li>
     *   <li>其余情况按规则正则替换；正则编译/替换异常时返回 {@link #DEFAULT_MASK}</li>
     * </ul>
     *
     * @param value  待脱敏的原始值
     * @param config 脱敏规则配置
