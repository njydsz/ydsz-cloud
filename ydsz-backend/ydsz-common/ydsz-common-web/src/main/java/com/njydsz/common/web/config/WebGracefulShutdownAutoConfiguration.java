 ...
 * <ul>
 *   <li>监听 {@link ContextClosedEvent}，记录停机开始时间点；</li>
 *   <li>监听 {@link WebServerInitializedEvent}，记录启动完成时间点；</li>
 *   <li>监听 {@link ApplicationFailedEvent}，记录应用启动失败事件；</li>
 *   <li>支持通过 {@code ydsz.web.shutdown.log-enabled=false} 关闭停机日志。</li>
 * </ul>
 *
 ...
 * </pre>
 *
 * <p><b>原理：</b>启用 {@code server.shutdown=graceful} 后，Spring Boot 的 Web 服务器
 * （Tomcat / Jetty / Undertow）会拒绝新请求并等待在飞请求（in-flight requests）完成。
 * 本配置的 {@link ShutdownEventListener} 会在停机开始时输出日志，便于运维观测。
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
@ConditionalOnProperty(prefix = "ydsz.web.shutdown", name = "log-enabled", havingValue = "true", matchIfMissing = true)
public class WebGracefulShutdownAutoConfiguration {

    private static final Logger log = LoggerFactory.getLogger(WebGracefulShutdownAutoConfiguration.class);

    /**
     * 注册优雅停机事件监听器 Bean。
     *
     * <p>监听 Spring 容器的关闭事件、WebServer 启动完成事件、应用启动失败事件，
     * 输出可观测性日志。
     *
     ...
     * 优雅停机事件监听器
     *
     * <p>统一处理 WebServer 启动完成、Context 关闭、应用启动失败等事件。
     * 通过实现 {@link ApplicationListener} 而非 {@code @EventListener} 注解，
     * 确保事件在 Bean 初始化阶段也能被捕获。
     */
    public static class ShutdownEventListener implements ApplicationListener<ApplicationEvent> {
        ...
        private void handleContextClosed() {
            log.info("[Shutdown] 应用上下文开始关闭，等待在飞请求完成"
