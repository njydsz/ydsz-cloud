 ...
 * 模块健康检查抽象基类。
 *
 * <p>提供统一的模板方法模式和辅助方法，消除各模块 HealthIndicator 中的重复代码。
 * 子类只需覆写 {@link #doHealthCheck(Health.Builder)} 添加模块特有探针。
 *
 * <p>使用示例：
 * <pre>{@code
 * @Component
...
public abstract class AbstractModuleHealthIndicator implements HealthIndicator {
    ...
    protected void checkOptionalStatus(
            Health.Builder builder, String componentName, Object component, Supplier<String> statusSupplier) {
        if (component == null) {
            builder.withDetail(componentName, "NOT_CONFIGURED");
            return;
        }
