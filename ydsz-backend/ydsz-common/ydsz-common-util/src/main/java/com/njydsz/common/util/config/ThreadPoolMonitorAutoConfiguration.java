...
public class ThreadPoolMonitorAutoConfiguration {
    ...
    public static class ThreadPoolMonitor {
         ...
         *
         * <p>注册以下 Gauge 指标（前缀 ydsz.threadpool）：
         * <ul>
        ...
        public void registerWithMeterRegistry(MeterRegistry registry) {
            ...
            for (Map.Entry<String, ThreadPoolExecutor> entry : registeredPools.entrySet()) {
                ...
                Gauge.builder("ydsz.threadpool.pool.size", executor, ThreadPoolExecutor::getPoolSize)
                        .tags(tags).register(registry);
                Gauge.builder("ydsz.threadpool.pool.active", executor, ThreadPoolExecutor::getActiveCount)
                        .tags(tags).register(registry);
                Gauge.builder("ydsz.threadpool.pool.completed", executor, ThreadPoolExecutor::getCompletedTaskCount)
                        .tags(tags).register(registry);
                Gauge.builder("ydsz.threadpool.pool.queue.size", executor, e -> e.getQueue().size())
                        .tags(tags).register(registry);
                Gauge.builder("ydsz.threadpool.pool.queue.capacity", executor, e -> e.getQueue().remainingCapacity())
                        .tags(tags).register(registry);
                Gauge.builder("ydsz.threadpool.pool.largest.size", executor, ThreadPoolExecutor::getLargestPoolSize)
                        .tags(tags).register(registry);
            }
