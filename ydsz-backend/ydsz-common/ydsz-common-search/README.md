...
PG 引擎（默认）还需引入 `ydsz-common-jdbc` 与 `spring-jdbc`；搜索分析/质量追踪需 `ydsz-common-redis`；ES/Solr/OpenSearch 引擎按需引入对应客户端。
...
| `ydsz.search.primary` | `pg` | 主引擎名称（pg / memory / es / redis / solr / opensearch） |
| `ydsz.search.fallbacks` | `[memory]` | 降级引擎链（按顺序尝试） |
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `ydsz.search.index.sync-mode` | `event` | 索引同步模式 |
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `ydsz.search.degrade.enabled` | `true` | 是否启用降级 |
| `ydsz.search.degrade.fallback-to-like` | `true` | 是否降级到 LIKE 查询 |
| `ydsz.search.degrade.probe-interval` | `30` | 探测间隔秒（5~300） |
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `ydsz.search.synonym.enabled` | `false` | 是否启用同义词扩展 |
| `ydsz.search.synonym.file` | `classpath:synonyms.txt` | 同义词词典路径（每行一组，逗号分隔） |
| `ydsz.search.pinyin.enabled` | `false` | 是否启用拼音转换 |
| `ydsz.search.pinyin.file` | `classpath:pinyin.txt` | 拼音词典路径（每行 汉字=拼音） |
...
| `ydsz.search.es.host` | `localhost` | ES 主机 |
| `ydsz.search.es.port` | `9200` | ES 端口 |
| `ydsz.search.es.scheme` | `http` | ES 协议 |
| `ydsz.search.es.index-name` | `ydsz_search` | ES 索引名 |
| `ydsz.search.redis.index-name` | `ydsz_search_idx` | RediSearch 索引名 |
| `ydsz.search.redis.key-prefix` | `search:doc:` | Redis Hash key 前缀 |
| `ydsz.search.solr.base-url` | `http://localhost:8983/solr` | Solr 基础 URL |
| `ydsz.search.solr.core` | `ydsz_search` | Solr core 名称 |
| `ydsz.search.opensearch.host` | `localhost` | OpenSearch 主机 |
| `ydsz.search.opensearch.port` | `9200` | OpenSearch 端口 |
| `ydsz.search.opensearch.scheme` | `http` | OpenSearch 协议 |
| `ydsz.search.opensearch.index-name` | `ydsz_search` | OpenSearch 索引名 |
...
2. **降级链顺序**：主引擎不可用或搜索异常时，按 `ydsz.search.fallbacks` 配置顺序尝试降级引擎，降级结果会标记 `degraded=true`；`memory` 引擎始终可用作兜底。
