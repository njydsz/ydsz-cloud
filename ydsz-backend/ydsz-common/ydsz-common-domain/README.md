...
> YDSZ DDD 领域模型基类库（L3 基础服务层）— 查询模型、状态机契约、领域事件、树形结构
...
> BaseDTO、TokenConstants/FilterIgnoreConstant（与 common-core 重复）、DAG/Job 框架（归属 ydsz-cronjob）、
> EventStore 事件溯源查询方法（Outbox 为 append-only）。
> 数据库实体的持久化基类请使用 `ydsz-common-jdbc` 的 `MpBaseEntity`（见下文「实体持久化」）。
...
| `EventStore` | 领域事件追加存储接口（append-only）。默认实现由 `ydsz-common-event` 的 OutboxEventStore 提供。**注意**：本接口仅承诺写入能力，不承诺事件回放（Outbox 为 forward-only）；如需事件溯源请扩展本接口 |
...
> **重要**：数据库实体的持久化基类在 `ydsz-common-jdbc` 模块，请继承 `MpBaseEntity`（含 `@TableId` 雪花算法、审计字段自动填充、乐观锁 revision、软删除 deleted、租户 tenant_id），**不要**继承本模块的历史基类（`BaseEntity/BaseLong/BaseString` 已在 v1.4.0 移除）。
...
> SpEL 评估器及其缓存配置（`ydsz.domain.spel.*`）已随 DAG 引擎迁移至 `ydsz-cronjob` 模块。
...
| `EventStore` | 领域事件追加存储契约（append-only） | `ydsz-common-event` 的 OutboxEventStore（默认） |
...
7. **DAG/Job 归属**：DAG 工作流引擎（SpELConditionEvaluator、DagInstanceStatus、DagNodeStatus）与 Job 分布式处理框架（JobHandler、MapReduceProcessor 等）已迁移至 `ydsz-cronjob` 模块，请勿在本模块引用。
...
- **v1.0.0**（2026-08-02）：对标 `ydsz-common-jdbc` 标准格式重构 README，聚焦当前实际能力
