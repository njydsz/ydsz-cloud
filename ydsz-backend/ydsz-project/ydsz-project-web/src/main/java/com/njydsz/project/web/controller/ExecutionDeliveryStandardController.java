...
public class ExecutionDeliveryStandardController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ExecutionDeliveryStandardDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ExecutionDeliveryStandardDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ExecutionDeliveryStandardDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ExecutionDeliveryStandardDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ExecutionDeliveryStandard")
    public BaseResponse<Boolean> update(@RequestBody ExecutionDeliveryStandardDO e) { return BaseResponse.success(service.updateById(e)); }
