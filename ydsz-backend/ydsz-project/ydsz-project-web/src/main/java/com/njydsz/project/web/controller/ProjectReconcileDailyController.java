...
public class ProjectReconcileDailyController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ProjectReconcileDailyDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ProjectReconcileDailyDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ProjectReconcileDailyDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ProjectReconcileDailyDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ProjectReconcileDaily")
    public BaseResponse<Boolean> update(@RequestBody ProjectReconcileDailyDO e) { return BaseResponse.success(service.updateById(e)); }
