...
public class RateInternalController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<RateInternalDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<RateInternalDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<RateInternalDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody RateInternalDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update RateInternal")
    public BaseResponse<Boolean> update(@RequestBody RateInternalDO e) { return BaseResponse.success(service.updateById(e)); }
