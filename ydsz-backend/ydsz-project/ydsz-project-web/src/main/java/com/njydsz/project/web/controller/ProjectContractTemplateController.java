...
public class ProjectContractTemplateController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ProjectContractTemplateDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ProjectContractTemplateDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ProjectContractTemplateDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ProjectContractTemplateDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ProjectContractTemplate")
    public BaseResponse<Boolean> update(@RequestBody ProjectContractTemplateDO e) { return BaseResponse.success(service.updateById(e)); }
