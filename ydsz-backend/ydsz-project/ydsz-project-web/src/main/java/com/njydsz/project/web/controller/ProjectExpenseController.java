...
public class ProjectExpenseController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ProjectExpenseDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ProjectExpenseDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ProjectExpenseDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ProjectExpenseDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ProjectExpense")
    public BaseResponse<Boolean> update(@RequestBody ProjectExpenseDO e) { return BaseResponse.success(service.updateById(e)); }
