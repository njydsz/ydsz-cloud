 ...
 * 布局配置以 JSON 存储在 ydsz_dashboard_layout 表中，
...
public class DashboardLayoutController {
    ...
    public BaseResponse<Void> save(@PathVariable String layoutKey, @RequestBody Map<String, Object> body) {
        ...
        int updated = jdbcTemplate.update(
                "UPDATE ydsz_dashboard_layout SET layout_config = ?::jsonb, updated_at = ? " +
        ...
        if (updated == 0) {
            jdbcTemplate.update(
                    "INSERT INTO ydsz_dashboard_layout (user_id, layout_key, layout_config, created_by, created_at, updated_at, tenant_id) " +
                            "VALUES (?, ?, ?::jsonb, ?, ?, ?, 1)",
    ...
    public BaseResponse<Map<String, Object>> load(@PathVariable String layoutKey) {
        ...
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT layout_config FROM ydsz_dashboard_layout WHERE user_id = ? AND layout_key = ?",
                userId, layoutKey
        );
    ...
    public BaseResponse<Void> reset(@PathVariable String layoutKey) {
        String userId = AuthContext.getUserId();
        jdbcTemplate.update(
                "DELETE FROM ydsz_dashboard_layout WHERE user_id = ? AND layout_key = ?",
                userId, layoutKey
        );
