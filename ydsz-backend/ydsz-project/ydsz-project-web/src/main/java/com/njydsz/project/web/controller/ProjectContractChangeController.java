...
public class ProjectContractChangeController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ProjectContractChangeDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ProjectContractChangeDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ProjectContractChangeDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ProjectContractChangeDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ProjectContractChange")
    public BaseResponse<Boolean> update(@RequestBody ProjectContractChangeDO e) { return BaseResponse.success(service.updateById(e)); }
