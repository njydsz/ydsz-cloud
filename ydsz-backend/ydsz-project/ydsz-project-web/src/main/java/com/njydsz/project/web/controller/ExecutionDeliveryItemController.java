...
public class ExecutionDeliveryItemController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ExecutionDeliveryItemDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ExecutionDeliveryItemDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ExecutionDeliveryItemDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ExecutionDeliveryItemDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ExecutionDeliveryItem")
    public BaseResponse<Boolean> update(@RequestBody ExecutionDeliveryItemDO e) { return BaseResponse.success(service.updateById(e)); }
