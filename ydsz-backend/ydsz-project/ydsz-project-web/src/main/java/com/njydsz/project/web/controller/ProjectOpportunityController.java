...
public class ProjectOpportunityController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ProjectOpportunityDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ProjectOpportunityDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ProjectOpportunityDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ProjectOpportunityDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ProjectOpportunity")
    public BaseResponse<Boolean> update(@RequestBody ProjectOpportunityDO e) { return BaseResponse.success(service.updateById(e)); }
