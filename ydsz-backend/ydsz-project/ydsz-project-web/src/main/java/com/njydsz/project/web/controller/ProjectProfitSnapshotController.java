...
public class ProjectProfitSnapshotController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ProjectProfitSnapshotDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ProjectProfitSnapshotDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ProjectProfitSnapshotDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ProjectProfitSnapshotDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ProjectProfitSnapshot")
    public BaseResponse<Boolean> update(@RequestBody ProjectProfitSnapshotDO e) { return BaseResponse.success(service.updateById(e)); }
