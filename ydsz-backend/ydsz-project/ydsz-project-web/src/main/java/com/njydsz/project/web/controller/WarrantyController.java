...
public class WarrantyController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<WarrantyDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<WarrantyDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<WarrantyDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody WarrantyDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update Warranty")
    public BaseResponse<Boolean> update(@RequestBody WarrantyDO e) { return BaseResponse.success(service.updateById(e)); }
