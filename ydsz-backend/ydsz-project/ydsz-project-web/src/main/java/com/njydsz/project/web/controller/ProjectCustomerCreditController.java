...
public class ProjectCustomerCreditController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ProjectCustomerCreditDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ProjectCustomerCreditDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ProjectCustomerCreditDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ProjectCustomerCreditDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ProjectCustomerCredit")
    public BaseResponse<Boolean> update(@RequestBody ProjectCustomerCreditDO e) { return BaseResponse.success(service.updateById(e)); }
