...
public class CostAllocationController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<CostAllocationDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<CostAllocationDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<CostAllocationDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody CostAllocationDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update CostAllocation")
    public BaseResponse<Boolean> update(@RequestBody CostAllocationDO e) { return BaseResponse.success(service.updateById(e)); }
