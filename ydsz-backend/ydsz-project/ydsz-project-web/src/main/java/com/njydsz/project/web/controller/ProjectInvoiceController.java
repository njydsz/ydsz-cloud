...
public class ProjectInvoiceController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<ProjectInvoiceDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<ProjectInvoiceDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<ProjectInvoiceDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody ProjectInvoiceDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update ProjectInvoice")
    public BaseResponse<Boolean> update(@RequestBody ProjectInvoiceDO e) { return BaseResponse.success(service.updateById(e)); }
