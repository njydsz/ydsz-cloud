...
public class BillableUtilizationSnapshotController {
    ...
    @GetMapping("/{id}")
    public BaseResponse<BillableUtilizationSnapshotDO> getById(@PathVariable String id) { return BaseResponse.success(service.getById(id)); }

    @GetMapping("/page")
    public PageResponse<BillableUtilizationSnapshotDO> page(@RequestParam(defaultValue="1") int p, @RequestParam(defaultValue="10") int s) {
        IPage<BillableUtilizationSnapshotDO> r = service.page(p, s);
    ...
    public BaseResponse<Boolean> save(@RequestBody BillableUtilizationSnapshotDO e) { return BaseResponse.success(service.save(e)); }

    @PutMapping
    @Audit(action=AuditAction.UPDATE, module="PROJECT", description="Update BillableUtilizationSnapshot")
    public BaseResponse<Boolean> update(@RequestBody BillableUtilizationSnapshotDO e) { return BaseResponse.success(service.updateById(e)); }
