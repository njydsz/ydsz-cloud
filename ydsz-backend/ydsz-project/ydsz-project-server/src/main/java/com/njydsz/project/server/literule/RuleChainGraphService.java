...
import com.njydsz.literule.infra.mapper.RuleChainGraphMapper;
 ...
 * 规则链画布 Service（P0-1）
 *
 * <p>提供画布的 CRUD 与持久化能力，画布内容以 JSON 形式存储到 ydsz_rule_chain_graph 表。
 *
...
public class RuleChainGraphService implements RuleChainGraphProvider {
    ...
    public RuleChainGraph save(String ruleCode, RuleChainGraph graph, String operator) {
        ...
        if (existing == null) {
            ...
            DO.setContentJson(YdszJson.toJson(graph));
        ...
        existing.setContentJson(YdszJson.toJson(graph));
    ...
    private RuleChainGraph toGraph(RuleChainGraphDO DO) {
        try {
            return YdszJson.parseMap(DO.getContentJson(), RuleChainGraph.class);
        } catch (Exception e) {
