...
import com.njydsz.project.domain.dto.WarrantyCreateDTO;
import com.njydsz.project.domain.dto.WarrantyTerminateDTO;
import com.njydsz.project.domain.entity.WarrantyDO;
import com.njydsz.project.domain.enums.WarrantyStatus;
import com.njydsz.project.infra.mapper.WarrantyMapper;
import com.njydsz.project.server.engine.AfterSalesCodeGen;
...
public class WarrantyServiceImpl implements WarrantyService {
    ...
    public String create(WarrantyCreateDTO dto) {
        ...
        if (w.getDurationMonths() <= 0 || w.getDurationMonths() > 120) {
            throw new SysException(BaseResultCode.BAD_REQUEST, "error.execution.msg_75b5c555");
        }
        ...
        if (w.getNoticeDays() < 0 || w.getNoticeDays() > 180) {
            throw new SysException(BaseResultCode.BAD_REQUEST, "error.execution.msg_f4127654");
        }
    ...
    public void terminate(WarrantyTerminateDTO dto) {
        ...
        if (w == null) throw new SysException(BaseResultCode.NOT_FOUND, "error.execution.msg_6457af8b");
        WarrantyStatus st = WarrantyStatus.fromCode(w.getStatus());
        if (st == null || st.isTerminal()) {
            throw new SysException(BaseResultCode.BAD_REQUEST, "error.execution.msg_b9835ff3", w.getStatus());
        }
        if (!st.canTransitTo(WarrantyStatus.TERMINATED)) {
            throw new SysException(BaseResultCode.BAD_REQUEST, "error.execution.msg_5b3f83db", st.getDesc());
        }
