 ...
 * <p>覆盖指标：
 * <ul>
 *   <li>ydsz_evm_red_projects_count — EVM 红色项目数（>3 告警）</li>
 *   <li>ydsz_bench_total_cost — Bench 闲置成本合计（>50万 告警）</li>
 *   <li>ydsz_billable_utilization_avg — 可计费利用率均值（<60% 告警）</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.4.0
...
public class YdszBusinessMetricsJob {
    ...
    @PostConstruct
    public void init() {
        Gauge.builder("ydsz_evm_red_projects_count", evmRedProjectsCount, AtomicLong::doubleValue)
                .description("EVM 红色项目数（SPI/CPI < 0.9）")
                .register(meterRegistry);
        Gauge.builder("ydsz_bench_total_cost", benchTotalCost, AtomicLong::doubleValue)
                .description("Bench 闲置成本合计（元）")
                .register(meterRegistry);
        Gauge.builder("ydsz_billable_utilization_avg", billableUtilizationAvg, AtomicLong::doubleValue)
                .description("可计费利用率均值（百分比）")
                .register(meterRegistry);
        Gauge.builder("ydsz_contract_monthly_amount", contractMonthlyAmount, AtomicLong::doubleValue)
                .description("当月新增合同金额（元）")
                .register(meterRegistry);
        Gauge.builder("ydsz_invoice_monthly_amount", invoiceMonthlyAmount, AtomicLong::doubleValue)
                .description("当月开票金额（元）")
                .register(meterRegistry);
        Gauge.builder("ydsz_payment_monthly_amount", paymentMonthlyAmount, AtomicLong::doubleValue)
                .description("当月回款金额（元）")
                .register(meterRegistry);
        Gauge.builder("ydsz_collection_rate", collectionRate, AtomicLong::doubleValue)
                .description("回款率（百分比）")
                .register(meterRegistry);
        Gauge.builder("ydsz_pending_approval_count", pendingApprovalCount, AtomicLong::doubleValue)
                .description("待审批工单数")
                .register(meterRegistry);
        log.info("[YdszBusinessMetrics] 已注册 8 个业务指标 Gauge");
    }
    ...
    public void refresh() {
        try {
            ...
            log.warn("[YdszBusinessMetrics] 刷新指标失败: {}", e.getMessage());
        }
    }
    ...
    private void refreshEvmRedCount() {
        try {
            Long count = jdbcTemplate.queryForObject(
                    "SELECT COUNT(DISTINCT initiation_id) FROM ydsz_evm_measure " +
                            "WHERE deleted = 0 AND (spi < 0.9 OR cpi < 0.9) " +
                            "AND measure_date = (SELECT MAX(measure_date) FROM ydsz_evm_measure WHERE deleted = 0)",
                    Long.class);
            evmRedProjectsCount.set(count != null ? count : 0);
        } catch (Exception e) {
            log.debug("[YdszBusinessMetrics] EVM 红色项目数查询失败: {}", e.getMessage());
        }
    }
    ...
    private void refreshBenchCost() {
        try {
            Long cost = jdbcTemplate.queryForObject(
                    "SELECT COALESCE(SUM(monthly_cost), 0) FROM ydsz_resource_bench " +
                            "WHERE deleted = 0 AND status = 'BENCH' AND tenant_id = 1",
                    Long.class);
            benchTotalCost.set(cost != null ? cost : 0);
        } catch (Exception e) {
            log.debug("[YdszBusinessMetrics] Bench 闲置成本查询失败: {}", e.getMessage());
        }
    }
    ...
    private void refreshUtilization() {
        try {
            ...
            Long avg = jdbcTemplate.queryForObject(
                    "SELECT COALESCE(AVG(utilization_rate), 0)::BIGINT FROM ydsz_billable_utilization_snapshot " +
            ...
            if (avg == null || avg == 0) {
                avg = jdbcTemplate.queryForObject(
                        "SELECT CASE WHEN SUM(total_hours) = 0 THEN 0 " +
                                ...
                                "FROM ydsz_billable_utilization_snapshot " +
                                "WHERE deleted = 0 AND snapshot_date >= CURRENT_DATE - INTERVAL '30 days'",
                        Long.class);
            }
            billableUtilizationAvg.set(avg != null ? avg : 0);
        } catch (Exception e) {
            log.debug("[YdszBusinessMetrics] 可计费利用率查询失败: {}", e.getMessage());
        }
    }
    ...
    private void refreshContractAmount() {
        try {
            Long amount = jdbcTemplate.queryForObject(
                    "SELECT COALESCE(SUM(contract_amount), 0)::BIGINT FROM ydsz_contract " +
                            "WHERE deleted = 0 AND sign_date >= date_trunc('month', CURRENT_DATE)",
                    Long.class);
            contractMonthlyAmount.set(amount != null ? amount : 0);
        } catch (Exception e) {
            log.debug("[YdszBusinessMetrics] 当月合同金额查询失败: {}", e.getMessage());
        }
    }
    ...
    private void refreshInvoiceAmount() {
        try {
            Long amount = jdbcTemplate.queryForObject(
                    "SELECT COALESCE(SUM(amount), 0)::BIGINT FROM ydsz_invoice " +
            ...
            log.debug("[YdszBusinessMetrics] 当月开票金额查询失败: {}", e.getMessage());
        }
    }
        ...
        try {
            Long rate = jdbcTemplate.queryForObject(
                    "SELECT CASE WHEN SUM(contract_amount) = 0 THEN 0 " +
                            ...
                            "FROM ydsz_contract c LEFT JOIN LATERAL " +
                            "(SELECT COALESCE(SUM(amount), 0) AS paid_amount FROM ydsz_payment " +
            ...
            log.debug("[YdszBusinessMetrics] 回款率查询失败: {}", e.getMessage());
        }
    }
    ...
    private void refreshPendingApproval() {
        try {
            Long count = jdbcTemplate.queryForObject(
                    "SELECT COUNT(*) FROM ydsz_flow_instance_task " +
                            "WHERE deleted = 0 AND status = 'PENDING'",
                    Long.class);
            pendingApprovalCount.set(count != null ? count : 0);
        } catch (Exception e) {
            log.debug("[YdszBusinessMetrics] 待审批工单数查询失败: {}", e.getMessage());
        }
    }
}
