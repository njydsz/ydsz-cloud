...
public interface ContractMapper extends BaseMapper<ContractDO> {
     ...
     * @return 合同总金额合计
     */
    @Select("SELECT COALESCE(SUM(total_amount), 0) FROM ydsz_project_contract WHERE deleted = 0")
     ...
     * @return 合同金额合计
     */
    @Select("SELECT COALESCE(SUM(total_amount), 0) FROM ydsz_project_contract " +
    ...
    @Select("SELECT customer_id AS customerId, customer_name AS customerName, " +
            ...
            "FROM ydsz_project_contract WHERE deleted = 0 " +
