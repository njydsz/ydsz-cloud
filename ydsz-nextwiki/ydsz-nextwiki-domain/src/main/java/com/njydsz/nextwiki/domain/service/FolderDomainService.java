 ...
 * 目录树领域服务
 *
 * <p>封装目录树操作的核心业务逻辑：创建目录、移动、重命名、递归删除、路径计算。 继承 {@code common-domain} 的 {@code TreeNode} 抽象，但持久化到数据库。
 *
 * <p><b>路径规则：</b>
 *
 * <ul>
...
public class FolderDomainService {
  ...
  public FileNode createFolder(String parentId, String name, String userId) {
    ...
    boolean nameExists =
        children.stream().anyMatch(c -> c.getName().equalsIgnoreCase(name) && c.isFolder());
    if (nameExists) {
  ...
  public FileNode move(String nodeId, String targetParentId, String userId) {
    ...
    log.info(
        "[FolderDomainService] 移动: nodeId={}, oldPath={}, newPath={}", nodeId, oldPath, newPath);
    return node;
  }
  ...
  public FileNode rename(String nodeId, String newName, String userId) {
    ...
    log.info(
        "[FolderDomainService] 重命名: nodeId={}, oldName={}, newName={}", nodeId, oldName, newName);
    return node;
  }
  ...
  private void updateChildrenPaths(
      String parentId, String oldPathPrefix, String newPathPrefix, int newLevel, String userId) {
    ...
    int affected =
        fileNodeRepository.batchUpdatePathPrefix(
