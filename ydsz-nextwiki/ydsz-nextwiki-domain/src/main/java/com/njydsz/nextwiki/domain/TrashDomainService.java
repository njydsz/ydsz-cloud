 ...
 * NextWiki 回收站领域服务。
 *
 * <p>负责回收站条目的纯领域逻辑：状态校验、状态迁移、事件构建。
 * 不直接依赖 Repository，所有数据访问由 server 层编排。
 *
 * <p><b>DDD 合规：</b>本服务不依赖 Spring {@code ApplicationEventPublisher}，
...
public class TrashDomainService {
   ...
   *
   * <p>server 层负责将返回的 {@link TrashItemDTO} 通过 repository 持久化。
   *
   * @param node 待删除的文件节点
   * @param userId 操作人 ID
  ...
  /**
   * 从回收站恢复：校验状态合法性、执行状态迁移、构建恢复事件。
   *
   * <p><b>DDD 合规：</b>本方法返回 {@link FileOperatedEvent} 而非直接发布事件，
   * 由 server 层通过 {@code ApplicationEventPublisher} 发布，保持领域层对 Spring 的零依赖。
   *
   * @param trashItem 待恢复的回收站条目
   * @param node 对应的文件节点（可能为 {@code null}，表示原节点已不存在）
   * @param userId 操作人 ID
   * @return 恢复事件（由 server 层发布）
   * @throws BusinessException 状态不允许恢复时抛出 {@link NextwikiExceptionCode#TRASH_INVALID_STATUS}
   */
  public FileOperatedEvent restore(TrashItemDTO trashItem, FileNodeVO node, String userId) {
   ...
   * 永久删除：执行状态迁移。
   *
   * <p>server 层负责通过 repository 查询 {@link TrashItemDTO} 并传入本方法，
   * 再通过 repository 持久化状态变更与物理删除文件节点。
   *
   ...
   *
   * <p>server 层负责通过 repository 查询过期条目并传入本方法，
   * 再通过 repository 持久化状态变更。
   *
