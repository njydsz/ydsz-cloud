 ...
 * 网盘文件节点实体
 * <p>
 * 统一表示文件和目录（通过 {@link #nodeType} 区分），构成目录树的核心节点。
 * 与底层存储层（{@code IFileStorage}）的对象一一对应，但额外持久化了
 * 目录结构、版本号、标签、分享状态等业务元数据。
 *
 * <p><b>核心设计：</b>
 * <ul>
 *   <li>使用 {@code parentId} + {@code path} 实现目录树（闭包路径模式）</li>
 *   <li>{@code storageKey} 对应底层存储的对象键（objectName）</li>
 *   <li>{@code version} 记录当前版本号，支持版本回溯</li>
 *   <li>{@code deletedTime} 支持回收站功能（逻辑删除 + 过期清理）</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class FileNode extends MpBaseEntity<String> implements Serializable {
     ...
     *
     * <p>由 {@code ContentExtractionApplicationService} 解析后设置，
     * 供 {@code WikiSearchProvider#toIndexDocument} 填充到统一搜索索引的 content 字段。
     * 此字段不映射到数据库，仅作内存传递用途。
     */
