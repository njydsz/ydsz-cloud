 ...
 * 本服务为纯领域逻辑组件，不执行任何数据访问或事件发布；数据访问与副作用由应用层负责。
 *
 * <p><b>路径规则：</b>
 *
 * <ul>
...
public class FolderDomainService {
   ...
   * 构建目录节点（纯领域逻辑，不执行持久化）。
   *
   * <p>由应用层传入父目录与同级子节点列表，本方法完成同名校验、路径计算与实体构建。
   * 返回的 {@link FileNodeVO} 实例需由应用层持久化。
   *
   * @param parent 父目录节点（已由应用层解析）
   ...
   * 执行移动操作（纯领域逻辑，不执行持久化）。
   *
   * <p>将 {@code node} 移动到 {@code targetParent} 下，计算新路径、层级与排序号。
   * 若节点为目录且目标为其自身或其子树，则抛出循环移动异常。
   *
   * @param node 待移动的节点（已由应用层加载）
   * @param targetParent 目标父目录节点（已由应用层加载）
   ...
   *
   * <p>更新节点名称与路径。若节点为目录，路径变更需由应用层同步到子节点。
   *
   * @param node 待重命名的节点（已由应用层加载）
   ...
   * 计算目录统计信息（纯领域逻辑）。
   *
   * @param folder 目录节点（已由应用层加载）
   * @param descendants 目录的全部后代节点（不含目录自身；已由应用层加载）
   * @return 统计结果
   */
