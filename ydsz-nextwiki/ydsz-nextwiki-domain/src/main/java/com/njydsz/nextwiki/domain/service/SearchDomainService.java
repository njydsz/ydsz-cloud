 ...
 * 搜索领域服务
 *
 * <p>维护 {@code ydsz_wiki_search_index} 表，提供基于数据库的文件名/路径/标签搜索能力。
 *
 * <p><b>职责定位（双索引架构）：</b>
 *
 * <ul>
 *   <li>{@code ydsz_wiki_search_index} 表 — 搜索引擎不可用时的 <b>DB 降级存储</b>，仅在统一搜索（{@code
 *       ydsz-common-search}）不可用时提供 LIKE 兜底
 *   <li>{@code ydsz_wiki_search_index} 表（{@code WikiSearchProvider} 维护）— 统一搜索引擎的主索引，支持全文检索、高亮、聚合、权重排序
 * </ul>
 *
 * <p><b>搜索优先级：</b>
 *
 ...
 * </ol>
 *
 * <p><b>分层原则：</b> 本类仅包含纯领域逻辑（搜索评分、高亮构建、索引组装）。
...
public class SearchDomainService {
   ...
   *
   * <p>对已分页查询的搜索索引列表进行评分、高亮构建和结果组装。
   * 分页查询由 server 层通过 {@code SearchIndexRepository} 完成，
   * 本类仅负责领域逻辑处理。
   *
   ...
   *
   * <p>根据文件节点和标签数据，构建 {@link SearchIndexDTO} DTO。
   * DTO 持久化由 server 层通过 {@code SearchIndexRepository} 完成。
   *
