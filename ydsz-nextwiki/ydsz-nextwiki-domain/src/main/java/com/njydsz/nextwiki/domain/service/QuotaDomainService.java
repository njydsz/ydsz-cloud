 ...
 * <p><b>缓存策略：</b>
 * <ul>
 *   <li>读：先从 Redis 缓存读取（key = {@code nextwiki:quota:scopeType:scopeId}），
 *       未命中则查 DB 并回写缓存（TTL 30 分钟）</li>
 *   <li>写：DB 原子更新成功后立即失效缓存，强制下次读取从 DB 获取最新值</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class QuotaDomainService {
    ...
    public StorageQuota setQuota(String scopeType, String scopeId, Long limit,
