...
public interface FileNodeRepository {
  ...
  /** 根据 ID 查询文件节点 */
  FileNodeDO findById(String id);

  /** 查询子节点列表 */
  List<FileNodeDO> findChildren(String parentId);
   ...
   * @return 分页结果
   */
  PageResponse<List<FileNodeDO>> findPageChildren(
      String parentId, String nodeType, String sortBy, String sortDir, int page, int pageSize);

  /** 根据路径前缀查询（用于递归操作） */
  List<FileNodeDO> findByPathPrefix(String pathPrefix);
  ...
  /** 保存文件节点 */
  FileNodeDO save(FileNodeDO node);
   ...
   * @since 1.0.0
   */
  int saveBatch(List<FileNodeDO> nodes);
  ...
  /** 批量查询 */
  List<FileNodeDO> findByIds(List<String> ids);

  /** 更新存储用量（移动/删除时更新目录统计） */
  void updateSize(String id, Long sizeDelta);

  /** 按文件名搜索（LIKE） */
  List<FileNodeDO> searchByName(String keyword, String createdBy);
  ...
  /** 查询用户大文件 Top-N */
  List<FileNodeDO> findTopLargeFilesByUser(String userId, int limit);

  /** 按后缀统计文件数量和大小 */
  List<FileTypeStat> statsBySuffixAndUser(String userId);

  /** 查询用户根目录 */
  FileNodeDO findOrCreateRoot(String userId);

  /** 按文件哈希查询（用于秒传去重） */
  FileNodeDO findByFileHash(String fileHash);

  /** 按 createdBy + parentId 查询同名文件（防重复上传） */
  List<FileNodeDO> findByNameAndParent(String name, String parentId, String createdBy);
   ...
   */
  List<FileNodeDO> findAllDescendantsByPath(String folderPath);
   ...
   * @since 1.0.0
   */
  List<FileNodeDO> findDescendantsByPage(String folderPath, int offset, int limit);
   ...
   */
  List<FileNodeDO> findAllDescendants(String folderId);
   ...
   * @return 冷数据候选列表
   */
  List<FileNodeDO> findColdCandidates(LocalDateTime threshold, String excludeSuffixes, int limit);
