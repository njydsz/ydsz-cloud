...
-- ============================================================
-- 背景：ydsz_share_access_log 为高写入表，长期累积拖慢查询。
-- 策略：按月份将 90 天前的访问日志归档到 ydsz_share_access_log_archive，
...
COMMENT ON TABLE ydsz_share_access_log_archive IS '分享访问日志归档表（按月分区，防止主表无限膨胀）';

-- 2. 归档函数：将 created_at 早于 retention_days 的日志搬移到归档表
CREATE OR REPLACE FUNCTION ydsz_archive_share_access_logs(retention_days INT)
RETURNS INT AS $$
DECLARE
    archived_count INT := 0;
BEGIN
    INSERT INTO ydsz_share_access_log_archive
    ...
    DELETE FROM ydsz_share_access_log
    WHERE created_at < NOW() - (retention_days || ' days')::INTERVAL
      AND deleted = 0;
...
COMMENT ON FUNCTION ydsz_archive_share_access_logs(INT) IS '分享访问日志归档：迁移 retention_days 天前的记录到归档表并从主表删除';
...
BEGIN
    FOR i IN -12..3 LOOP
        ...
        EXECUTE format(
            'CREATE TABLE IF NOT EXISTS ydsz_share_access_log_archive_%s PARTITION OF ydsz_share_access_log_archive '
...
CREATE INDEX IF NOT EXISTS idx_archive_share_created
    ON ydsz_share_access_log_archive (share_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_archive_created
    ON ydsz_share_access_log_archive (created_at DESC);

-- 5. 使用说明（由定时任务按周调用，例如每周日凌晨 4:00）：
--    SELECT ydsz_archive_share_access_logs(90);
