...
public class CdnApplicationService {
  ...
  public void prefetchUrls(List<String> urls) {
    ...
    log.info(
        "{\"cdn\":\"prefetch\",\"provider\":\"{}\",\"count\":{},\"urls\":{}}",
        properties.getCdn().getProvider(),
        urls.size(),
        urls);
  }
  ...
  public void refreshUrls(List<String> urls) {
    ...
    log.info(
        "{\"cdn\":\"refresh\",\"provider\":\"{}\",\"count\":{},\"urls\":{}}",
        properties.getCdn().getProvider(),
        urls.size(),
        urls);
  }
  ...
  public void refreshDirectory(String directoryPath) {
    ...
    log.info(
        "{\"cdn\":\"refreshDir\",\"provider\":\"{}\",\"directory\":\"{}\"}",
        properties.getCdn().getProvider(),
        directoryPath);
  }
  ...
  public void purgeCache(String storageKey) {
    ...
    log.info(
        "{\"cdn\":\"purge\",\"provider\":\"{}\",\"storageKey\":\"{}\",\"cdnUrl\":\"{}\"}",
        properties.getCdn().getProvider(),
        storageKey,
        cdnUrl);
  }
   ...
   * 生成 CDN 访问 URL。
   *
   * <p>当 CDN 启用且配置了域名时，将 storageKey 映射为 {@code https://域名/storageKey}； 否则返回 {@code
   * null}（由调用方回退到对象存储源站 URL）。
   *
   ...
   *
   * <p>采用 SHA-256（替代不安全的 MD5，P2-8 修复）对 {@code storageKey + 过期时间 + secretKey} 签名， 校验方按相同规则重建签名并比对
   * {@code expires} 与 {@code sign}，防止 URL 被篡改或过期后滥用。
   *
   * @param storageKey 存储对象键
