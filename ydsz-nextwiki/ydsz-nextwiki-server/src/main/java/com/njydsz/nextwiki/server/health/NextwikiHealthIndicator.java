 ...
 *
 * <p>健康检查与指标采集职责分离：所有业务指标（upload/download/delete/...） 由 {@code NextwikiMetrics} 通过 {@link
 * com.njydsz.common.sentry.adapter.SentryMetricsAdapter} 体系承载， 本指标器不持有指标转发门面，违反云顶编码规范"业务模块优先使用
...
public class NextwikiHealthIndicator extends AbstractModuleHealthIndicator {
   ...
   *
   * <p><b>判定约定：</b>文件存储未配置时<b>不</b>将实例判为 DOWN，仅追加 {@code warning} 明细。原因是知识库的浏览、检索、权限等只读能力不依赖对象存储，
   * 若因此摘除实例会放大故障面；真正致命的是数据库不可用，该项由 {@code checkTableProbe} 决定最终健康态。
   *
   * <p>数据库探针使用固定的哨兵用户 {@code "health-check"} 执行计数查询， 该用户预期无数据，返回 0 属正常，只要不抛异常即视为连接健康。
   *
  ...
  protected void doHealthCheck(Health.Builder builder) {
    ...
    // 数据库探针
    checkTableProbe(
        builder, "databaseConnected", () -> fileNodeRepository.countByUser("health-check"));
