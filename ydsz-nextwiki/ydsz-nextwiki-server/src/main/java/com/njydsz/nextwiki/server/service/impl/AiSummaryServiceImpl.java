 ...
 * AI 智能摘要服务实现（预留）。
 *
 * <p>当前为桩实现，依赖 {@link NextwikiProperties#isLlmEnabled()} 开关控制是否启用。
 * 后续对接真实 LLM 服务时替换 {@link #generateSummary} 内部逻辑。
 *
 * <p>后续实现方向：
 * <ul>
 *   <li>读取文件内容（文本提取）</li>
 *   <li>调用 LLM API 生成摘要</li>
 *   <li>缓存摘要结果（Redis，TTL 24h）</li>
 *   <li>异步执行 + 进度通知</li>
 * </ul>
 *
 * @author ydsz-team
...
public class AiSummaryServiceImpl implements AiSummaryService {
    ...
    public SummaryResult generateSummary(String fileNodeId, String summaryType, Integer maxLength) {
        ...
        String placeholderSummary = String.format(
        ...
        result.setSummary(placeholderSummary.substring(0, Math.min(placeholderSummary.length(), actualLength)));
        result.setSummaryType(type);
        result.setWordCount(Math.min(placeholderSummary.length(), actualLength));
        result.setGeneratedAt(LocalDateTime.now());

        log.info("[AiSummaryService] 生成摘要(预留): fileNodeId={}, type={}, length={}",
            fileNodeId, type, result.getWordCount());
        return result;
    }
