 ...
 * <p><b>归档策略：</b>
 * <ul>
 *   <li>冷数据判定：文件 {@code coldDaysThreshold} 天未访问（updated_at 早于阈值）</li>
 *   <li>排除规则：临时文件（tmp/cache）不归档</li>
 *   <li>分批处理：每批 {@code batchSize} 个文件，避免一次性处理过多</li>
 *   <li>存储类型：迁移至 GLACIER/DEEP_ARCHIVE 等低成本存储</li>
 * </ul>
 *
 * <p>后续实现方向：
 * <ul>
 *   <li>对接云厂商归档存储 API（S3 Glacier / 阿里归档 / 华为归档）</li>
 *   <li>解冻请求队列：用户访问归档文件时触发异步解冻</li>
 *   <li>存储成本统计：按月计算冷热存储成本对比</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class ColdDataArchivalService {
    ...
    public int scanAndArchive() {
        ...
        int coldDays = config.getColdDaysThreshold() > 0
            ? config.getColdDaysThreshold() : DEFAULT_COLD_DAYS;
        ...
        List<FileNode> coldCandidates = fileNodeRepository.findColdCandidates(
            threshold, config.getExcludeExtensions(), batchSize);
        ...
        for (FileNode file : coldCandidates) {
            try {
                ...
                log.warn("[ColdDataArchival] 归档失败: fileNodeId={}, name={}, error={}",
                    file.getId(), file.getName(), e.getMessage());
            }
        }

        log.info("[ColdDataArchival] 归档完成: archived={}, total={}",
            archivedCount, coldCandidates.size());
    ...
    private void archiveFile(FileNode file, String storageClass) {
        ...
        log.info("[ColdDataArchival] 归档文件: fileNodeId={}, name={}, -> {}",
            file.getId(), file.getName(), storageClass);
