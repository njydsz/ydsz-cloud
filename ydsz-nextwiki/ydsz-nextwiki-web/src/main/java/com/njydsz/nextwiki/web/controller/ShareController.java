 ...
 *
 * <ul>
 *   <li>{@code POST /shares} - 创建分享链接（支持密码/提取码/过期/访问次数限制）
 ...
 *
 * <ul>
 *   <li>分享链接：生成全局唯一 shareCode（短链形式），对外可匿名访问
 *   <li>密码保护：可选 password 字段，访问时需输入明文密码
 *   <li>提取码：4-6 位数字提取码（区别于密码，作为辅助验证）
 *   <li>过期控制：expireTime 控制分享链接的有效期
 *   <li>访问次数：maxAccessCount 控制最大访问次数，到达上限后链接失效
 * </ul>
 *
 * <h3>安全与稳定性</h3>
 *
 ...
 *   <li>公开验证接口加 {@link RateLimit} 限流（50 QPS）防密码爆破
 *   <li>所有接口均加 {@link AuthApiPermission} 权限码校验（NEXTWIKI_SHARE_*）
 * </ul>
 *
 * <h3>接口路径</h3>
 *
...
public class ShareController {
   ...
   * 创建文件分享链接。
   *
   * <p>基于文件节点 ID 创建一条分享记录，并返回 shareCode（用于生成可访问的 URL）。 可选配置密码 / 提取码 / 过期时间 / 最大访问次数。
   *
   ...
   * 验证分享链接的访问权限。
   *
   * <p>对外公开接口（无需登录），传入 shareCode + 提取码 + 密码进行三重校验。 验证通过后返回 {@link ShareLink}，前端基于此访问分享内容。访问次数会自动
   * +1。
   *
