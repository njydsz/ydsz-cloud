 ...
 *
 * <ul>
 *   <li>{@code POST /storage/presigned-upload} - 生成上传预签名 URL</li>
 *   <li>{@code POST /storage/presigned-download} - 生成下载预签名 URL</li>
 * </ul>
 *
 * <p>使用场景：
 * <ul>
 *   <li>大文件直传：减轻服务端带宽压力</li>
 *   <li>客户端上传：移动端/Web 端直接上传到对象存储</li>
 *   <li>临时分享：生成临时下载链接</li>
 * </ul>
 *
 * @author ydsz-team
...
public class PresignedUrlController {
    ...
    public BaseResponse<PresignedUrlResponse> generateUploadUrl(
        ...
        String url = fileStorage.generatePresignedUploadUrl(
            null,
            request.getObjectKey(),
            java.time.Duration.ofSeconds(
                request.getExpireSeconds() != null
    ...
    public BaseResponse<PresignedUrlResponse> generateDownloadUrl(
        ...
        String url = fileStorage.generatePresignedUrl(
            request.getObjectKey(),
            request.getExpireSeconds() != null
                ? request.getExpireSeconds()
                : 3600);
