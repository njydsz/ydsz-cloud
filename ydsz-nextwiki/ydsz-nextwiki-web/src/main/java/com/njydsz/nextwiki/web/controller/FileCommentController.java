 ...
 *
 * <ul>
 *   <li>{@code GET /comments/file/{fileNodeId}} - 查询文件的所有评论（含回复）
 *   <li>{@code POST /comments} - 添加评论或回复（支持 {@code parentCommentId} 嵌套回复）
 *   <li>{@code DELETE /comments/{id}} - 删除评论
 *   <li>{@code POST /comments/{id}/resolve} - 标记评论已解决
 * </ul>
 *
 * <h3>核心能力</h3>
 *
 * <ul>
 *   <li>嵌套回复：通过 {@code parentCommentId} 实现评论的树形回复结构
 *   <li>位置批注：{@code position} 字段支持文档内位置定位（行号/偏移量），用于精确批注
 *   <li>解决状态：评论可标记为 resolved（已解决），便于审阅流程闭环
 *   <li>编辑追踪：{@code edited} / {@code revision} 字段记录编辑历史
 * </ul>
 *
 * <h3>安全与稳定性</h3>
 *
 ...
 *   <li>所有接口加 {@link AuthApiPermission} 权限码校验（NEXTWIKI_FILE_*）
 *   <li>评论软删除（{@code deleted=1}），保留审计追溯
 * </ul>
 *
 * <h3>接口路径</h3>
 *
 ...
 *
 * <p>TODO: 待接入 {@code ydsz_nw_file_comment} 表 + FileCommentMapper 后启用完整评论能力
 *
 * @author ydsz-team
 * @since 1.0.0
 */
...
public class FileCommentController {
   ...
   *
   * <ul>
   *   <li>顶级评论：不传 {@code parentCommentId}
   ...
   *
   * <p>软删除而非物理删除，便于审计追溯和上下文保留。 子评论（回复）不会被级联删除，但会随父评论在列表中隐藏。
   *
   ...
   * 标记评论为已解决。
   *
   * <p>由评论作者或文件所有者触发；标记后评论会从"未解决"列表中移除。 解决操作记录 resolvedBy / resolvedAt 字段。
   *
   * @param commentId 评论 ID
   * @param userId 操作人 ID
   * @return 统一响应结果
