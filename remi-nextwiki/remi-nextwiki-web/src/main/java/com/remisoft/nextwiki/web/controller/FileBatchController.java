...
import com.remisoft.nextwiki.server.service.FileApplicationService;
 ...
 *
 * <h3>业务背景</h3>
 * <p>网盘文件（NextWiki）模块在日常使用中存在大量"一次操作多个节点"或"对历史版本进行管理"的场景，
 * 例如批量清理、批量归档、版本回滚等。将这些能力从核心单文件 CRUD 中剥离，便于按需扩展批量策略
 * 与版本审计逻辑，同时降低单 Controller 的代码体积与心智负担。
 *
 * <h3>核心能力</h3>
 * <ul>
 *   <li>批量操作：批量删除 / 批量移动，统一返回 {@link FileApplicationService.BatchResult}（含成功/失败明细）</li>
 *   <li>版本管理：版本历史查询（按版本号倒序）、回滚到指定版本（保留历史，新增回滚记录）</li>
 *   <li>个性化：切换文件星标状态，便于"我的星标"视图聚合展示</li>
 * </ul>
 *
 * <h3>安全与稳定性</h3>
 * <ul>
...
public class FileBatchController {
    ...
    @Idempotent(key = "remi:nextwiki:FileController:batchDelete:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:nextwiki:FileController:batchMove:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:nextwiki:FileController:rollbackVersion:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:nextwiki:FileController:toggleStar:lock", ttlSeconds = 5)
