...
import com.remisoft.nextwiki.domain.entity.TrashItem;
import com.remisoft.nextwiki.server.service.TrashApplicationService;
 ...
 * <ul>
 *   <li>{@code GET /trash/list} - 查询当前用户的回收站列表</li>
 *   <li>{@code POST /trash/{id}/restore} - 恢复单个回收站项目</li>
 *   <li>{@code POST /trash/batch-restore} - 批量恢复多个项目</li>
 *   <li>{@code DELETE /trash/{id}} - 永久删除单个项目（不可恢复）</li>
 *   <li>{@code DELETE /trash/empty} - 清空整个回收站（不可恢复）</li>
 * </ul>
 *
 * <h3>核心能力</h3>
 * <ul>
 *   <li>软删除：{@code FileController#delete} 不会立即清理物理文件，而是移入回收站</li>
 *   <li>保留期：默认 30 天，超期由定时任务自动永久删除（{@code NextwikiScheduledJobs}）</li>
 *   <li>恢复语义：恢复后节点回到删除前的父目录（parentId 已记录在 TrashItem 中）</li>
 *   <li>级联恢复：恢复文件夹会递归恢复其下所有文件</li>
 * </ul>
 *
 * <h3>安全与稳定性</h3>
 * <ul>
 *   <li>所有写操作均加 {@link Idempotent} 防重（5s TTL）</li>
 *   <li>所有写操作均加 {@link Audit} 异步落库审计日志（操作人 + 项目 ID 留痕）</li>
 *   <li>永久删除（purge/empty）为不可逆操作，需 NEXTWIKI_TRASH_PURGE 权限码</li>
 *   <li>所有接口均加 {@link AuthApiPermission} 权限码校验</li>
 *   <li>仅本人可恢复/删除自己的回收站项目（service 层按 userId 过滤）</li>
 * </ul>
 *
 * <h3>接口路径</h3>
 * <pre>
 ...
 *                                            ↓
 *                                   remi-nextwiki-server.TrashApplicationService
 *                                            ↓
 *                                   remi-nextwiki-server.NextwikiScheduledJobs (定时清理)
 *                                            ↓
 *                                   remi-nextwiki-infra.TrashItemMapper
 *                                            ↓
 *                                   remi_trash_item
 * </pre>
 *
...
public class TrashController {
     ...
     * 从回收站恢复单个项目。
     *
     * <p>恢复后节点回到删除前的父目录；若原父目录已不存在则恢复到根目录。
     * 文件夹下文件会被级联恢复。
     *
    ...
    @Idempotent(key = "remi:nextwiki:TrashController:restore:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:nextwiki:TrashController:batchRestore:lock", ttlSeconds = 5)
     ...
     * 永久删除回收站中的单个项目（不可恢复）。
     *
     * <p>会级联删除：节点记录 + 历史版本 + 物理文件 + 关联 ACL/Tag/Comment 等。
     * 高危操作，需 NEXTWIKI_TRASH_PURGE 权限。
     *
     * @param trashItemId 回收站项目 ID
     * @param userId      当前用户 ID
     * @return 统一响应结果
    ...
    @Idempotent(key = "remi:nextwiki:TrashController:purge:lock", ttlSeconds = 5)
    ...
    @Idempotent(key = "remi:nextwiki:TrashController:emptyTrash:lock", ttlSeconds = 5)
    @DeleteMapping("/empty")
