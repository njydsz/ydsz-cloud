...
import com.remisoft.nextwiki.server.service.TagApplicationService;
 ...
 * <ul>
 *   <li>{@code POST /tags} - 创建标签（自定义名称 + 颜色）</li>
 *   <li>{@code GET /tags} - 查询所有可用标签</li>
 *   <li>{@code POST /tags/bind} - 为文件绑定标签（支持批量）</li>
 *   <li>{@code GET /tags/file/{fileNodeId}} - 查询指定文件的全部标签</li>
 *   <li>{@code GET /tags/recommend/{fileNodeId}} - 基于文件内容推荐标签（AI 能力）</li>
 * </ul>
 *
 * <h3>核心能力</h3>
 * <ul>
 *   <li>手动标签：用户自定义名称 + 颜色（如"合同-红色"、"财务-蓝色"）</li>
 *   <li>AI 推荐：基于文件标题/内容（关键词提取）智能推荐标签</li>
 *   <li>批量绑定：单次请求可为文件绑定多个标签</li>
 *   <li>标签筛选：前端"按标签筛选文件"功能依赖此接口</li>
 * </ul>
 *
 * <h3>安全与稳定性</h3>
 * <ul>
 ...
 *   <li>所有接口均加 {@link AuthApiPermission} 权限码校验（NEXTWIKI_TAG_*）</li>
 *   <li>标签命名全局唯一，由 service 层去重校验</li>
 * </ul>
 *
 * <h3>接口路径</h3>
 * <pre>
 ...
 *                                            ↓
 *                                   remi-nextwiki-server.TagApplicationService
 *                                            ↓
 *                                   remi-nextwiki-infra.TagMapper
 *                                            ↓
 *                                   remi_tag / remi_file_tag
 * </pre>
 *
 * @author remi-team
 * @since 1.0.0
...
@io.swagger.v3.oas.annotations.tags.Tag(name = "标签管理", description = "标签创建、绑定、智能推荐") // FQN-OK: name conflict with Tag entity
public class TagController {
    ...
    @Idempotent(key = "remi:nextwiki:TagController:createTag:lock", ttlSeconds = 5)
    @PostMapping
    ...
    @Idempotent(key = "remi:nextwiki:TagController:bindTag:lock", ttlSeconds = 5)
    @PostMapping("/bind")
     ...
     * 基于文件内容智能推荐标签。
     *
     * <p>通常基于文件标题/正文/AI Summary 做关键词提取，匹配现有标签库，
     * 返回 TopN 推荐结果。建议前端在用户上传后自动调用此接口预填标签。
     *
     * @param fileNodeId 文件节点 ID
