...
import com.remisoft.nextwiki.server.service.FilePermissionService;
 ...
 * <ul>
 *   <li>{@code POST /files/{nodeId}/lock} - 锁定文件（Check-out，独占编辑权）</li>
 *   <li>{@code POST /files/{nodeId}/unlock} - 解锁文件（Check-in，释放编辑权）</li>
 * </ul>
 *
 * <h3>核心能力</h3>
 * <ul>
 *   <li>悲观锁：锁定后仅锁定者可编辑，其他用户上传/重命名/移动等写操作被拒绝</li>
 *   <li>状态隔离：P0-R3 修复后，锁定状态使用 {@code status='locked'} 字段记录，
 *       不再覆盖共享状态（{@code shareStatus}）</li>
 *   <li>权限校验：P2-R2 修复后增加 {@link FilePermissionService#checkWrite} 权限检查</li>
 *   <li>再锁定：锁定者可重新锁定自己的文件（刷新 {@code updatedAt}）</li>
 * </ul>
 *
 * <h3>典型使用流程</h3>
 * <pre>
 ...
 *   <li>所有接口加 {@link AuthApiPermission} 权限码校验（NEXTWIKI_FILE_UPLOAD）</li>
 *   <li>仅文件所有者和已授权用户可执行锁定（由 {@link FilePermissionService} 校验）</li>
 * </ul>
 *
 * <h3>接口路径</h3>
 * <pre>
 ...
 *                                            ↓
 *                                   remi-nextwiki-domain.FileNodeRepository
 *                                   remi-nextwiki-server.FilePermissionService
 *                                            ↓
...
public class FileLockController {
     ...
     * 锁定文件（Check-out）。
     *
     * <p>将文件状态置为 {@code locked}，并记录锁定人（{@code updatedBy}）。
     * 已锁定且非本人锁定的文件再次锁定会被拒绝。
     *
    ...
    @Idempotent(key = "remi:nextwiki:FileLockController:filelock:lock", ttlSeconds = 5)
     ...
     * 仅本人锁定的文件可解锁（{@link FilePermissionService#checkWrite} 校验）。
     *
     * @param nodeId 文件节点 ID
     * @param userId 当前用户 ID
     * @return 统一响应结果
    ...
    @Idempotent(key = "remi:nextwiki:FileLockController:unlock:lock", ttlSeconds = 5)
