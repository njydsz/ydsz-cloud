...
本模块是 REMI 的**一体化网盘知识库平台**，融合文件存储、文档解析、全文搜索、在线预览、版本控制、分享协作。
...
remi-nextwiki/
├── pom.xml
├── remi-nextwiki-api/                 # API 层：Feign Client + DTO
├── remi-nextwiki-domain/              # 领域层：Entity + Repository 接口 + 领域服务
│   └── src/main/java/com/remisoft/nextwiki/domain/
...
├── remi-nextwiki-infra/               # 基础设施层：仓储实现 + Mapper
├── remi-nextwiki-server/              # 应用层：应用服务 + 配置 + 健康检查 + 监听器
└── remi-nextwiki-web/                 # Web 层：Controller + 启动类
    └── src/main/java/com/remisoft/nextwiki/web/
...
> 本模块是 REMI 的**文件存储与知识协作中心**，复用 common-file（存储抽象）/ common-search（全文检索）/ common-lock（分布式锁）公共能力。
