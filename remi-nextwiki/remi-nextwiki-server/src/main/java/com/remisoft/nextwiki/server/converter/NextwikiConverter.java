package com.remisoft.nextwiki.server.converter;
 ...
 * 当前仅提供 {@link FileNode} → {@link FileNodeVO} 的转换；
 * 后续如需新增其他实体转换，可在此类中扩展对应方法。
 *
 * <p><b>字段映射说明：</b>
 * <ul>
 *   <li>{@code thumbnailKey}（存储键）→ {@code thumbnailUrl}：此处直接透传存储键，
 *       由前端或 CDN 网关拼接完整可访问 URL。</li>
 *   <li>{@code children} / {@code tags}：默认填充空集合（{@link Collections#emptyList()}），
 *       避免下游空指针；调用方如需注入实际数据，可通过 {@link FileNodeVO#setChildren(List)} /
 *       {@link FileNodeVO#setTags(List)} 覆盖。</li>
 *   <li>审计字段（createdBy / createdAt / updatedAt）：从 {@code MpBaseEntity} 继承字段透传。</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
