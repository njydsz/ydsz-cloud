...
#
#  说明:
#    P3-1 安全版: 仅替换 DO 文件中 @TableId 注解下的 String id -> String id
...
foreach ($f in $entityFiles) {
    ...
    $content = $content -replace "@TableId\(type = IdType\.AUTO\)\s*(\r?\n\s*)private String id;", "@TableId(type = IdType.ASSIGN_ID)`$1private String id;"
    $content = $content -replace "@TableId\s*(\r?\n\s*)private String id;", "@TableId`$1private String id;"
    $content = $content -replace "private String id;", "private String id;"
