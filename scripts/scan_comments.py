...
# Find Java files with minimal class-level Javadoc
results = []
for java_file in base.rglob('*.java'):
    ...
    # Extract class/interface/enum Javadoc
    # Find the Javadoc right before class/interface/enum declaration
    lines = content.split('\n')
    ...
    for i, line in enumerate(lines):
        ...
        if stripped.startswith('/**'):
            ...
            while j < len(lines) and '*/' not in lines[j]:
                j += 1
            javadoc_lines = j - i + 1
            # Check if this Javadoc is for a class/interface/enum
            k = j + 1
            while k < len(lines) and (lines[k].strip() == '' or lines[k].strip().startswith('@') or lines[k].strip().startswith('//')):
                k += 1
            if k < len(lines):
                next_line = lines[k].strip()
                if any(next_line.startswith(kw) for kw in ['public class', 'public interface', 'public enum', 'public abstract class', 'public final class', 'public sealed', 'class ', 'interface ', 'enum ']):
