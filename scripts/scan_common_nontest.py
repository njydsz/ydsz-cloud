#!/usr/bin/env python3
"""Scan common module non-test Java files for comment-to-code ratio."""
import os
...
for root, _, fnames in os.walk(base):
    for fn in fnames:
        ...
        # Skip test files
        if '/test/' in fpath.replace(os.sep, '/'):
            continue
...
print(f"Non-test files with ratio < 12%: {len(results)}")
print(f"{'Comments':>8} {'Code':>6} {'Ratio':>6}  File")
print('-' * 100)
for rel, cl, code, ratio in results[:30]:
