...
COMMENT ON COLUMN ydsz_com_outbox.schema_version IS '事件 schema 版本（向前兼容）';
COMMENT ON COLUMN ydsz_com_outbox.compressed IS 'payload 是否 GZIP 压缩存储';
COMMENT ON COLUMN ydsz_com_outbox.tenant_id IS '租户 ID（多租户隔离）';
COMMENT ON COLUMN ydsz_com_outbox.trace_id IS '链路追踪 ID';
COMMENT ON COLUMN ydsz_com_outbox.idempotency_key IS '幂等去重 ID';
...
CREATE INDEX IF NOT EXISTS idx_ydsz_com_outbox_dedup ON ydsz_com_outbox (idempotency_key, status);
CREATE INDEX IF NOT EXISTS idx_ydsz_com_outbox_aggregate ON ydsz_com_outbox (aggregate_type, aggregate_id, created_at DESC);

-- ============================================================================
-- Outbox 归档表（ydsz-common-event）
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_ydsz_com_outbox_archive_aggregate ON ydsz_com_outbox_archive (aggregate_id, created_at);
CREATE INDEX IF NOT EXISTS idx_ydsz_com_outbox_archive_event_type ON ydsz_com_outbox_archive (event_type, created_at);
CREATE INDEX IF NOT EXISTS idx_ydsz_com_outbox_archive_created_at ON ydsz_com_outbox_archive (created_at);
CREATE INDEX IF NOT EXISTS idx_ydsz_com_outbox_archive_archived_at ON ydsz_com_outbox_archive (archived_at);

COMMENT ON TABLE ydsz_com_outbox_archive IS 'Outbox 事件归档表（已投递或已丢弃的消息）';
...
-- ============================================================================
-- 搜索全文索引表（按租户 HASH 分区，2026-09-21 合并自 V26.09.21__search_index_hash_partition）
-- ============================================================================
-- 目的：将全文索引表按 tenant_id 做 HASH 分区，实现大租户数据物理隔离、索引清理秒级完成
-- 注意：线上切换请采用"双写 → 历史迁移 → 切读 → 下线旧表"灰度策略
...
-- 创建 8 个 HASH 分区
CREATE TABLE IF NOT EXISTS ydzs_search_idx_part_0 PARTITION OF ydsz_com_search_index_partitioned
    FOR VALUES WITH (MODULUS 8, REMAINDER 0);
CREATE TABLE IF NOT EXISTS ydzs_search_idx_part_1 PARTITION OF ydsz_com_search_index_partitioned
    FOR VALUES WITH (MODULUS 8, REMAINDER 1);
CREATE TABLE IF NOT EXISTS ydzs_search_idx_part_2 PARTITION OF ydsz_com_search_index_partitioned
    FOR VALUES WITH (MODULUS 8, REMAINDER 2);
CREATE TABLE IF NOT EXISTS ydzs_search_idx_part_3 PARTITION OF ydsz_com_search_index_partitioned
    FOR VALUES WITH (MODULUS 8, REMAINDER 3);
CREATE TABLE IF NOT EXISTS ydzs_search_idx_part_4 PARTITION OF ydsz_com_search_index_partitioned
    FOR VALUES WITH (MODULUS 8, REMAINDER 4);
CREATE TABLE IF NOT EXISTS ydzs_search_idx_part_5 PARTITION OF ydsz_com_search_index_partitioned
    FOR VALUES WITH (MODULUS 8, REMAINDER 5);
CREATE TABLE IF NOT EXISTS ydzs_search_idx_part_6 PARTITION OF ydsz_com_search_index_partitioned
    FOR VALUES WITH (MODULUS 8, REMAINDER 6);
CREATE TABLE IF NOT EXISTS ydzs_search_idx_part_7 PARTITION OF ydsz_com_search_index_partitioned
    FOR VALUES WITH (MODULUS 8, REMAINDER 7);

COMMENT ON TABLE ydsz_com_search_index_partitioned IS '搜索全文索引分区表（按 tenant_id HASH 分区，原 ydzs_wiki_search_index 升级）';
