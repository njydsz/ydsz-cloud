 ...
 *
 * <p>Spring 基于代理的 AOP 无法拦截同类内部的方法调用（self-invocation），
 * 当 {@link JobScanner} 内部直接调用 {@code this.acquireDueJobs()} 或
 * {@code this.advanceNextFireTime()} 时，{@code @Transactional} 注解不生效。
 *
 * <h3>修复方案</h3>
 * <p>将需要事务保护的方法抽取到独立的 Service Bean 中，通过依赖注入调用，
 * 确保 Spring AOP 代理能正确拦截并开启事务。
 *
 * <h3>事务方法</h3>
 * <ul>
...
public class JobTransactionService {
     ...
     * 多个 Leader 候选节点互不冲突。readOnly 事务提示数据库可做查询优化。
     *
     * @param now       当前时间
     ...
     *
     * <p>通过 {@code WHERE next_fire_time = #{oldNextFireTime}} 实现 CAS 乐观锁，
     * 保证 Leader 切换时不会重复派发同一任务。
     *
     * @param job      任务定义
