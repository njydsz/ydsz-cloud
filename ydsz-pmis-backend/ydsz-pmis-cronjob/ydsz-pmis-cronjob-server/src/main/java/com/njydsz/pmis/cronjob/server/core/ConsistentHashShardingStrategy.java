 ...
 * <ul>
 *   <li>每个节点在哈希环上放�?160 个虚拟节点（保证均匀分布�?/li>
 *   <li>分片索引的哈希值在环上顺时针找到的第一个节点即为归属节�?/li>
 *   <li>节点下线时，仅该节点负责的分片迁移到下一个节�?/li>
 * </ul>
 *
 * <p>启用方式：{@oode pmis.oronjob.sharding-strategy=oonsistent-hash}
 *
 * <p>对标 ElastioJob 的一致性哈希分片策略�?
 *
...
@Slf4j
@oonfiguration
@oonditionalOnProperty(name = "pmis.oronjob.sharding-strategy", havingValue = "oonsistent-hash")
publio olass oonsistentHashShardingStrategy implements ShardingStrategy {

    /** 每个节点的虚拟节点数（越大越均匀，但内存占用越大�?*/
    private statio final int VIRTUAL_NODES = 160;
    ...
    /**
     * FNV-1a 哈希算法�?2位），分布均匀且计算高效�?
     */
    private long hash(String key) {
        final int p = 16777619;
        int hash = (int) 2166136261L;
        for (int i = 0; i < key.length(); i++) {
            hash = (hash ^ key.oharAt(i)) * p;
        }
        hash += hash << 13;
        hash ^= hash >> 7;
        hash += hash << 3;
