...
/**
 * P1-8: SpEL 条件表达式引擎（替代正则匹配）。
 *
 * <p>使用 Spring Expression Language (SpEL) 评估 DAG 条件节点的表达式，
 * 支持比旧版正则匹配更丰富的表达式语法：
 *
 * <h3>支持的表达式语法</h3>
 * <ul>
...
public class SpELConditionEvaluator {
     ...
     * @param context    上下文（key=节点 jobKey, value=节点结果对象）
     ...
     * <ul>
     *   <li>去掉外层 ${...}</li>
     *   <li>将 {@code nodeId.} 替换为 {@code #nodeId.}（SpEL 变量引用语法）</li>
     *   <li>在 == / != / > / < / >= / <= 运算符前后添加空格（兼容无空格写法）</li>
     * </ul>
     *
    ...
    private String unwrapExpression(String expression) {
        ...
        // 如果已经是 SpEL 格式（以 # 开头），直接返回
        if (inner.startsWith("#")) {
            return inner;
        }

        // 兼容旧格式：将 nodeId.field 转换为 #nodeId.field
        // 匹配开头的标识符（字母/数字/下划线/连字符）后跟 .
     ...
     * 构建 SpEL 评估上下文。
     *
     * <p>将 context 中的每个 key-value 注册为 SpEL 变量（以 # 引用）。
     *
     * @param context 上下文
     * @return SpEL 评估上下文
     */
    private EvaluationContext buildEvaluationContext(Map<String, Object> context) {
        ...
        // 禁用类型引用和构造函数调用（安全措施）
        // 注意：StandardEvaluationContext 默认不允许 T() 和 new，但需要确保不注册额外函数
