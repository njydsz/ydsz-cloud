...
import com.njydsz.pmis.cronjob.core.dag.DagEdge;
...
import com.njydsz.pmis.cronjob.dto.dag.JobDagSaveDTO;
...
import com.njydsz.pmis.cronjob.mapper.dag.JobDagVersionMapper;
import com.njydsz.pmis.cronjob.mapper.job.JobMapper;
import com.njydsz.pmis.cronjob.service.dag.JobDagService;
...
public class JobDagServiceImpl implements JobDagService {
     ...
     * @throws BizException 当 JSON 格式无效、节点缺失或存在环依赖时抛出
     */
    ...
    private Map<String, List<String>> buildAdjacencyListFromDagDefinition(DagDefinition definition) {
        ...
        // 确保所有节点都在邻接表中（即使没有出边）
        for (com.njydsz.pmis.cronjob.core.dag.DagNode node : definition.nodes()) {
     ...
     * @throws BizException 当 triggerType=CRON 且 cronExpression 为空时抛出
     */
