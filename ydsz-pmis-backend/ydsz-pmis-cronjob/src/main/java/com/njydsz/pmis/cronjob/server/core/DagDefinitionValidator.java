...
public class DagDefinitionValidator {
    ...
    /**
     * 校验 DAG 定义，不通过时抛出 {@link BizException}。
     *
    ...
    public void validate(DagDefinition definition) {
        if (definition == null) {
            throw new BizException(BizErrorCode.BAD_REQUEST, "error.cronjob.msg_dag_definition_empty");
        }
    ...
    private void validateNodes(DagDefinition definition) {
        if (definition.nodes() == null || definition.nodes().isEmpty()) {
            throw new BizException(BizErrorCode.BAD_REQUEST, "error.cronjob.msg_dag_no_nodes");
        }
        Set<String> seen = new HashSet<>();
        for (DagNode node : definition.nodes()) {
            if (node.jobKey() == null || node.jobKey().isBlank()) {
                throw new BizException(BizErrorCode.BAD_REQUEST, "error.cronjob.msg_dag_node_key_missing");
            }
            if (!seen.add(node.jobKey())) {
                throw new BizException(BizErrorCode.BAD_REQUEST, "error.cronjob.msg_dag_node_key_duplicate");
            }
        }
    }
    ...
    private void validateEdges(DagDefinition definition) {
        ...
        for (DagEdge edge : definition.edges()) {
            if (edge.from() == null || edge.to() == null) {
                throw new BizException(BizErrorCode.BAD_REQUEST, "error.cronjob.msg_dag_edge_invalid");
            }
    ...
    private void validateRootNodes(DagDefinition definition) {
        ...
        if (roots.isEmpty()) {
            throw new BizException(BizErrorCode.BAD_REQUEST, "error.cronjob.msg_dag_no_root");
        }
    }
