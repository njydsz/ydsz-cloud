 ...
 *
 * <p>定义告警触发条件、级别、通知通道与去重策略。规则可绑定到具体任务
 * （{@link #jobId} 非空），也可作为全局规则（{@link #jobId} 为 NULL）应用于所有任务。
 *
 * <h3>告警类型</h3>
 * <ul>
 *   <li>{@code FAIL}：任务执行失败即告警</li>
 *   <li>{@code TIMEOUT}：任务执行超时即告警</li>
 *   <li>{@code SLOW}：任务执行慢（耗时 &gt;= threshold 毫秒）</li>
 *   <li>{@code FAIL_RATE}：时间窗口内失败率 &gt;= threshold（百分比 0-100）</li>
 *   <li>{@code DURATION_P95}：时间窗口内 P95 耗时 &gt;= threshold（毫秒）</li>
 * </ul>
 *
 * <h3>去重策略</h3>
 * <ul>
 *   <li>{@link #cooldownMinutes}：冷却窗口，同一规则在冷却期内不重复告警</li>
...
public class JobAlertRuleDO extends BaseDO {
    ...
    /** 规则来源: MANUAL 手动创建(默认) / SLA 由SLA规则自动生成(P2-2-merge 合并自 pmis_job_sla) */
