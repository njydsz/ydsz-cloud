...
public class SatisfactionServiceImpl implements SatisfactionService {
    ...
    public String submit(SatisfactionCreateDTO dto) {
        ...
        if (s.getScore() == null) {
            throw new BizException(StandardResultCode.BAD_REQUEST, "error.execution.msg_a3a869d4");
        }
        SatisfactionLevel level = SatisfactionLevel.fromScore(s.getScore());
        if (level == null) {
            throw new BizException(StandardResultCode.BAD_REQUEST, "error.execution.msg_37c4fe7e");
        }
    ...
    public void markFollowUp(String id, String note) {
        if (id == null) throw new BizException(StandardResultCode.BAD_REQUEST, "error.execution.msg_35ec26fe");
        SatisfactionDO s = satisfactionMapper.selectById(id);
        if (s == null) throw new BizException(StandardResultCode.NOT_FOUND, "error.execution.msg_4b213f7c");
        s.setFollowUp(true);
    ...
    private void validate(SatisfactionCreateDTO dto) {
        ...
        if (dto.getProfessionalism() != null && (dto.getProfessionalism() < 1 || dto.getProfessionalism() > 5)) {
            throw new BizException(StandardResultCode.BAD_REQUEST, "error.execution.msg_ef96f33e");
        }
        if (dto.getTimeliness() != null && (dto.getTimeliness() < 1 || dto.getTimeliness() > 5)) {
            throw new BizException(StandardResultCode.BAD_REQUEST, "error.execution.msg_86a9a3df");
        }
        if (dto.getQuality() != null && (dto.getQuality() < 1 || dto.getQuality() > 5)) {
            throw new BizException(StandardResultCode.BAD_REQUEST, "error.execution.msg_6568138a");
        }
        if (dto.getAttitude() != null && (dto.getAttitude() < 1 || dto.getAttitude() > 5)) {
            throw new BizException(StandardResultCode.BAD_REQUEST, "error.execution.msg_2803de1f");
        }
    }
}
