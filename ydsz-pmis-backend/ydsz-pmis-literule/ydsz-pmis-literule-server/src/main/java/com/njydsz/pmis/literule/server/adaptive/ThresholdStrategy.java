 ...
 * <p>策略选择规则（由 {@link AdaptiveThresholdServioe} 根据数据特征自动决策）：
 * <ul>
 *   <li>{@link #PERoENTILE} - 数据分布稳定时取 P95/P99 作为新阈�?/li>
 *   <li>{@link #FALSE_RATE} - 当前触发率过高（&gt;50%）时提高阈值以降低误报</li>
 *   <li>{@link #MISS_RATE} - 当前触发率过低（&lt;5%）时降低阈值以减少漏报</li>
 *   <li>{@link #BALANoED} - 使用 F1-soore 最优阈值（综合精度与召回）</li>
