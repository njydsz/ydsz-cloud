package com.njydsz.userinfo.server.service.impl.user;
...
import com.njydsz.common.redis.service.BloomFilterService;
import com.njydsz.common.util.id.TracerUtils;
import com.njydsz.common.util.security.PwdUtils;
import com.njydsz.userinfo.domain.dto.auth.LoginRequest;
import com.njydsz.userinfo.domain.dto.auth.LoginResult;
...
import com.njydsz.userinfo.infra.mapper.user.User2FAMapper;
import com.njydsz.userinfo.infra.mapper.user.UserAccountMapper;
import com.njydsz.userinfo.infra.mapper.user.UserRoleMapper;
import com.njydsz.userinfo.server.constant.CacheConstants;
import com.njydsz.userinfo.server.security.AccountLockInfo;
import com.njydsz.userinfo.server.security.DataScopeHelper;
import com.njydsz.userinfo.server.security.LoginAuditEvent;
import com.njydsz.userinfo.server.security.LoginStatus;
import com.njydsz.userinfo.server.security.PasswordPolicy;
import com.njydsz.userinfo.server.security.TotpUtil;
import com.njydsz.userinfo.server.service.auth.SessionService;
import com.njydsz.userinfo.server.service.impl.auth.JwtSimpleBuilder;
import com.njydsz.userinfo.server.service.org.DepartmentService;
...
public class UserAccountServiceImpl implements UserAccountService {
    ...
    public String create(UserAccountDO user, String rawPassword) {
        ...
        user.setPassword(PwdUtils.hashPasswordBCrypt(rawPassword));
        user.setSalt("");
    ...
    public void update(UserAccountDO user) {
        if (user.getId() == null) {
            throw new SysException(BaseResultCode.BAD_REQUEST, "error.user.msg_668e9add");
        }
    ...
    public void delete(String userId) {
        ...
        if (isSuperAdmin) {
            throw new SysException(BaseResultCode.BAD_REQUEST, "error.user.msg_5b101e42");
        }
    ...
    public void resetPassword(String userId, String newPassword) {
        ...
        // BCrypt 哈希存储在 password 字段，salt 字段留空（BCrypt 自带盐）
        u.setPassword(PwdUtils.hashPasswordBCrypt(newPassword));
        u.setSalt("");
    ...
    public LoginResult login(LoginRequest request) {
        if (request == null || !StringUtils.hasText(request.getUsername())) {
            throw new SysException(BaseResultCode.BAD_REQUEST, "error.user.msg_0b62b5ce");
        }
        ...
        if (lockPolicy.isLocked(u.getLockedUntil())) {
            ...
            throw new SysException(BaseResultCode.ACCOUNT_LOCKED,
        ...
        boolean oldHashWasBcrypt = PwdUtils.isBCryptFormat(u.getPassword());
        boolean passwordOk = oldHashWasBcrypt
                ? PwdUtils.verifyPasswordBCrypt(request.getPassword(), u.getPassword())
                : PwdUtils.verifyPasswordWithSha256Salt(request.getPassword(), u.getPassword(), u.getSalt());
        if (!passwordOk) {
        ...
        if (!oldHashWasBcrypt) {
            try {
                upgradePasswordHash(u.getId(), PwdUtils.hashPasswordBCrypt(request.getPassword()));
            } catch (Exception ex) {
        ...
        if (mfaRequired && twofa != null) {
            ...
            if (!mfaPassed) {
                ...
                throw new SysException(BaseResultCode.MFA_INVALID);
            }
        }
    ...
    public void changePassword(String userId, String oldPassword, String newPassword) {
        ...
        boolean oldPasswordOk = oldHashWasBcrypt
                ? PwdUtils.verifyPasswordBCrypt(oldPassword, u.getPassword())
                : PwdUtils.verifyPasswordWithSha256Salt(oldPassword, u.getPassword(), u.getSalt());
        if (!oldPasswordOk) {
    ...
    public void upgradePasswordHash(String userId, String bcryptHash) {
        if (userId == null || !PwdUtils.isBCryptFormat(bcryptHash)) {
        ...
        if (PwdUtils.isBCryptFormat(u.getPassword())) {
            return;
        }
