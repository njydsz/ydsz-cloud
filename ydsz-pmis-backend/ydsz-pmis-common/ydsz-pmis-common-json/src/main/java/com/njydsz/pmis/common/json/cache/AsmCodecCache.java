 ...
 *   <li>生成后的类缓存在 ConcurrentHashMap 中</li>
...
public final class AsmCodecCache {
     ...
     * 获取或创建 ASM 序列化器
     *
     * <p>优化：使用单次 ConcurrentHashMap.get() 替代 containsKey() + get() 双次查找</p>
     */
     ...
     * 内部通过泛型辅助方法捕获通配符类型，避免调用方 unchecked cast</p>
     *
     * @param obj 要序列化的对象
     * @param writer JSON 写入器
     * @return 是否成功使用 ASM 序列化器完成序列化
     */
     ...
     *
     * <p>调用方通过此方法间接调用 serializer.serialize()，
     * 将 unchecked cast 隔离在此方法内部，避免在各调用点重复出现</p>
     */
    ...
    /**
     * 获取或创建 ASM 反序列化器（非类型参数版本，接受 Class<?>）
     *
     * @param beanType Bean 类型
