...
public final class YdszSerializationProvider {
    ...
    /** 小 JSON 的 StringBuilder 初始容量（适合简单 Bean） */
    private static final int SMALL_SB_CAPACITY = 1024;

    /** 中 JSON 的 StringBuilder 初始容量（适合一般 Bean） */
    private static final int MEDIUM_SB_CAPACITY = 4096;

    /** 大 JSON 的 StringBuilder 初始容量（适合大集合/复杂嵌套） */
    ...
    public static String serialize(Object obj) {
        ...
        // 快速路径1：Bean 类型直接使用 ASM 序列化器，跳过 StringBuilder 中转
        ...
        // 快速路径2：Collection 类型直接使用 JSONWriter，跳过 StringBuilder 中转
    ...
    private static boolean tryFastSerialize(Object obj, StringBuilder sb) {
        ...
        // 使用 FastJSON2 JSONWriter 进行快速序列化（复用 ThreadLocal 池）
        JSONWriter writer = FAST_WRITER_POOL.get();
        writer.reset();
    ...
    /**
     * 获取或创建 BeanSerializerInfo（FastJSON2 架构优化）
     */
