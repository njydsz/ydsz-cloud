...
/**
 * 高性能反射缓存（替代 ASM 字节码生成）
 * 
 * <p>预计算 MethodHandle/Constructor，消除运行时反射查找开销</p>
 * 
 * <p><b>性能优势：</b></p>
 * <ul>
...
public final class FastReflectCache {
    ...
    public static <T> BeanCache<T> getOrCreate(Class<T> beanType) {
        BeanCache<?> cached = CACHE.get(beanType);
        if (cached != null) {
            return castBeanCache(cached);
        }
        BeanCache<T> newCache = new BeanCache<>(beanType);
        CACHE.put(beanType, newCache);
        return newCache;
    }

    private static <T> BeanCache<T> castBeanCache(BeanCache<?> cache) {
        return (BeanCache<T>) cache;
    }
