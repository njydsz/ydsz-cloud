 ...
 * <p>核心特性：
 * <ul>
 *   <li>写穿透：put 操作同步写入后端存储，保证数据一致性</li>
 *   <li>删除传播：remove 操作同步从后端存储删除</li>
 *   <li>批量写入：putAll 批量写入后端存储，减少网络开销</li>
 *   <li>异常传播：后端存储写入失败时抛出异常，避免数据不一致</li>
 * </ul>
 *
 * <p>工作原理：
 * <ol>
 *   <li>put 操作：先写入后端存储，成功后更新缓存</li>
 *   <li>remove 操作：先从缓存获取值，然后同步删除后端存储和缓存</li>
 *   <li>clear 操作：遍历所有缓存项，逐个从后端存储删除</li>
 * </ol>
 *
 * <p>适用场景：
 * <ul>
 *   <li>数据库缓存：写穿透保证缓存与数据库一致性</li>
 *   <li>配置中心：配置更新需要同步持久化</li>
 *   <li>用户会话：会话数据需要持久化存储</li>
 * </ul>
 *
 * <p>使用示例：
 * <pre>{@code
