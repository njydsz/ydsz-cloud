 ...
 *
 * <p>参考 Caffeine 的 AsyncCache 设计，提供完全异步的缓存操作能力。
 * 与 {@link Cache} 的同步操作不同，AsyncCache 的所有操作都是非阻塞的，
 * 适合在响应式编程和异步 IO 场景中使用。
 *
 * <p>核心特性：
 * <ul>
 *   <li>异步查询：getIfPresent 返回 CompletableFuture</li>
 *   <li>异步加载：get 方法支持异步加载器，自动防击穿</li>
 *   <li>异步批量：getAll 支持批量异步加载</li>
 *   <li>异步写入：put 返回 CompletableFuture</li>
 * </ul>
 *
 * @param <K> 键类型
 * @param <V> 值类型
...
public interface AsyncCache<K, V> {
     ...
     */
    CompletableFuture<Map<K, V>> getAll(Collection<K> keys, AsyncFunction<Collection<K>, Map<K, V>> loader);
