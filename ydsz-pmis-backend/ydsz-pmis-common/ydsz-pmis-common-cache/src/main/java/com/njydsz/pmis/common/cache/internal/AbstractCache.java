 ...
 * <ul>
 *   <li>命中/未命中计数与命中率统计</li>
 *   <li>删除监听器管理与通知</li>
 *   <li>带加载器的获取（{@code get(key, loader)}）</li>
 *   <li>异步获取（{@code getAsync(key, loader)}）</li>
 * </ul>
 *
 * <p>子类只需实现 {@link #getIfPresent(Object)}、{@link #put(Object, Object)}、
 * {@link #remove(Object)}、{@link #clear()}、{@link #estimatedSize()}、
 * {@link #containsKey(Object)}、{@link #keySet()}、{@link #values()} 等核心方法。
 *
 * <p>线程安全说明：本类使用 {@link LongAdder} 和 {@link CopyOnWriteArrayList}，
 * 统计和监听器操作均为线程安全。子类需自行保证核心方法的线程安全。
 *
 * @param <K> 键类型
 * @param <V> 值类型
 * @author Marvin Lee
...
public abstract class AbstractCache<K, V> implements Cache<K, V> {
     ...
     * 删除监听器列表
     */
    protected final List<RemovalListener<? super K, ? super V>> listeners = new CopyOnWriteArrayList<>();
