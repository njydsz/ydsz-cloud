 ...
 * <p>核心特性：
 * <ul>
 *   <li>流畅 API：链式调用，语义清晰</li>
 *   <li>灵活配置：支持多种缓存类型、淘汰策略、统计开关</li>
 *   <li>类型安全：泛型约束，编译期类型检查</li>
 *   <li>默认 TINYLFU：命中率最优的默认缓存类型</li>
 * </ul>
 *
 * <p>使用示例：
 * <pre>{@code
...
public final class CacheBuilder<K, V> {
     ...
     * @return this
     */
    public CacheBuilder<K, V> maximumWeight(long maximumWeight, Weigher<? super K, ? super V> weigher) {
     ...
     * 设置自定义过期策略
     *
     * <p>允许为每个缓存条目动态计算过期时间，而非使用全局固定的过期策略。
     * 如果设置了此策略，将覆盖 expireAfterWrite 和 expireAfterAccess 的配置。
     *
     ...
     * 设置异步任务执行器
     *
     * <p>用于异步加载缓存值（getAsync 方法）和自动刷新任务。
     * 如果未设置，将使用默认线程池。
     *
     ...
     * 设置并发级别
     *
     * <p>估算并发修改的线程数，用于优化锁段数量。
     * 默认值为 32，如果设置为 1，则不使用分段锁。
     *
     ...
     * 使用弱引用键存储缓存键
     *
     * <p>键如果没有被其他地方引用，可以被 GC 回收。
     * 注意：这会覆盖 type 配置，将缓存类型设置为 WEAK_KEY。
     *
     * @return this
     */
     ...
     * 使用弱引用值存储缓存值
     *
     * <p>值如果没有被其他地方引用，可以被 GC 回收。
     * 注意：这会覆盖 type 配置，将缓存类型设置为 WEAK_VALUE。
     *
     * @return this
     */
     ...
     * 使用软引用值存储缓存值
     *
     * <p>值在 JVM 内存不足时可以被 GC 回收。
     * 注意：这会覆盖 type 配置，将缓存类型设置为 SOFT_VALUE。
     *
     * @return this
     */
    ...
    private void validate() {
        ...
        if (expireAfterWriteDuration == 0) {
            throw new IllegalArgumentException("expireAfterWrite duration must be greater than 0, or -1 for no expiration");
        }
        if (expireAfterAccessDuration == 0) {
            throw new IllegalArgumentException("expireAfterAccess duration must be greater than 0, or -1 for no expiration");
        }
    }
    ...
    public LoadingCache<K, V> buildLoadingCache() {
        ...
        TimeUnit effectiveRefreshUnit = effectiveRefreshDuration > 0
    ...
    private Cache<K, V> createBaseCache() {
        ...
        if (!supportsExpire) {
            if (expireAfterWriteDuration > 0) {
                log.warn("缓存类型 {} 不支持 expireAfterWrite 配置，该参数将被忽略。" +
                        "如需过期策略，请使用 CacheType.TTL 或 CacheType.ENHANCED_LOADING", type);
            }
            if (expireAfterAccessDuration > 0) {
                log.warn("缓存类型 {} 不支持 expireAfterAccess 配置，该参数将被忽略。" +
                        "如需过期策略，请使用 CacheType.TTL 或 CacheType.ENHANCED_LOADING", type);
            }
            if (refreshAfterWriteDuration > 0) {
                log.warn("缓存类型 {} 不支持 refreshAfterWrite 配置，该参数将被忽略。" +
                        "如需自动刷新策略，请使用 CacheType.TTL 或 CacheType.ENHANCED_LOADING", type);
            }
        }
        ...
        switch (type) {
            ...
            case TTL:
                if (expireAfterWriteDuration > 0) {
                    return TTLCache.create(expireAfterWriteDuration, expireAfterWriteUnit, TTLMode.WRITE,
                            recordStats, true, 60,
                            refreshAfterWriteDuration > 0 ? refreshAfterWriteUnit.toMillis(refreshAfterWriteDuration) : 0,
                            loader, taskExecutor != null ? taskExecutor : listenerExecutor);
                } else if (expireAfterAccessDuration > 0) {
                    return TTLCache.create(expireAfterAccessDuration, expireAfterAccessUnit, TTLMode.ACCESS,
                            recordStats, true, 60,
                            refreshAfterWriteDuration > 0 ? refreshAfterWriteUnit.toMillis(refreshAfterWriteDuration) : 0,
                            loader, taskExecutor != null ? taskExecutor : listenerExecutor);
                } else {
            ...
            case ENHANCED_LOADING:
                ...
                TimeUnit elcRefreshUnit = elcRefreshDuration > 0
                        ? (refreshAfterWriteUnit != null ? refreshAfterWriteUnit : TimeUnit.MILLISECONDS)
                        : TimeUnit.NANOSECONDS;
                return EnhancedLoadingCache.create(
