 ...
 *
 * <p>使用 Resilience4j CircuitBreaker 包装缓存操作，当后端缓存（如 Redis）不可用时，
 * 自动降级为直接返回 null（让调用方回退到 L1 本地缓存或数据源加载）。
 *
 * <p>熔断策略：
 * <ul>
 *   <li>滑动窗口大小：100 次请求</li>
 *   <li>失败率阈值：50%</li>
 *   <li>熔断打开后等待时间：10 秒</li>
 *   <li>半开状态允许请求数：10</li>
 * </ul>
 *
 * @param <K> 键类型
...
public class Resilience4jCacheDecorator<K, V> implements Cache<K, V> {
    ...
    public CompletableFuture<V> getAsync(K key, AsyncFunction<K, V> loader) {
        return CircuitBreaker.decorateCompletionStage(circuitBreaker,
                () -> delegate.getAsync(key, loader)).get().toCompletableFuture();
    }
    ...
    public long estimatedSize() {
        return CircuitBreaker.decorateSupplier(circuitBreaker, delegate::estimatedSize)
                .get();
    }

    @Override
    public boolean containsKey(K key) {
        return CircuitBreaker.decorateSupplier(circuitBreaker, () -> delegate.containsKey(key))
                .get();
    }
    ...
    public V computeIfAbsent(K key, Function<K, V> mappingFunction) {
        return CircuitBreaker.decorateSupplier(circuitBreaker,
                () -> delegate.computeIfAbsent(key, mappingFunction)).get();
    }

    @Override
    public V compute(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction) {
        return CircuitBreaker.decorateSupplier(circuitBreaker,
                () -> delegate.compute(key, remappingFunction)).get();
    }
