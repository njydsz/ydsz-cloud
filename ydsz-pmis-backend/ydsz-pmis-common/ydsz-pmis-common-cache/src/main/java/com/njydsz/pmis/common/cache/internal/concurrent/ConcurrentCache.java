 ...
 * 并发安全缓存实现。
 *
 * <p>基于 {@link ConcurrentHashMap} 实现线程安全的缓存，
 * 适用于高并发读写场景。提供原子性的 {@code get(key, loader)} 和
 * {@code getAsync(key, loader)} 操作，避免缓存击穿。
 *
 * <p>与 {@link LRUCache} 不同，本类不提供淘汰策略，缓存容量无上限。
 * 如需容量限制，请使用 {@link LRUCache} 配合 {@link StripedConcurrentCache} 分段锁。
 *
 * @param <K> 键类型
 * @param <V> 值类型
 * @author Marvin Lee
