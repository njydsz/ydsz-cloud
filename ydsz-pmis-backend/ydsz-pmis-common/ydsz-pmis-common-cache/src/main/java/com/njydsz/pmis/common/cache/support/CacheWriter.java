 ...
 * <p>核心功能：
 * <ul>
 *   <li>写穿透（Write-Through）：数据同时写入缓存和后端存储，保证一致性</li>
 *   <li>写回（Write-Behind）：数据先写入缓存，异步批量写入后端存储，提升性能</li>
 *   <li>批量写入：支持批量写入操作，减少数据库压力</li>
 * </ul>
 *
 * <p>写穿透模式：
 * <ol>
 *   <li>应用调用 put(key, value)</li>
 *   <li>缓存写入器同步写入后端存储</li>
 *   <li>写入成功后更新缓存</li>
 * </ol>
 *
 ...
 *   <li>应用调用 put(key, value)</li>
 *   <li>仅更新缓存，标记为脏数据</li>
 *   <li>后台线程异步批量写入后端存储</li>
 * </ol>
 *
 * <p>适用场景：
 * <ul>
 *   <li>数据库缓存：写穿透保证数据一致性</li>
 *   <li>配置中心：写回模式减少数据库写入压力</li>
 *   <li>会话管理：异步写回提升响应速度</li>
 * </ul>
 *
 * <p>使用示例：
 * <pre>{@code
