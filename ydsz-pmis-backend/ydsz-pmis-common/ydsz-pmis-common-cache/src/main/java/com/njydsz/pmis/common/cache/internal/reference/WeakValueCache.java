 ...
 * <p>核心特性：
 * <ul>
 *   <li>弱引用值：当值不再被其他引用时，可被 GC 回收</li>
 *   <li>键保留：键对象正常保留，不影响查找</li>
 *   <li>自动清理：GC 时自动清理被回收的值</li>
 *   <li>线程安全：使用 ConcurrentHashMap 保证并发安全</li>
 * </ul>
 *
 * <p>工作原理：
 * <ol>
 *   <li>使用 WeakReference 封装值对象</li>
 *   <li>ReferenceQueue 用于跟踪被 GC 回收的值</li>
 *   <li>每次缓存操作前先清理已回收的条目</li>
 *   <li>当值对象只有缓存引用时，GC 会回收并回调</li>
 * </ol>
 *
 * <p>适用场景：
 * <ul>
 *   <li>缓存大对象：避免大对象占用过多内存</li>
 *   <li>计算结果缓存：临时存储计算结果</li>
 *   <li>资源池：管理稀缺资源</li>
 * </ul>
 *
 * <p>注意事项：
 * <ul>
 *   <li>值可能被 GC 回收，返回 null</li>
 *   <li>不适合需要可靠返回值的场景</li>
 *   <li>清理操作有轻微性能开销</li>
 * </ul>
 *
