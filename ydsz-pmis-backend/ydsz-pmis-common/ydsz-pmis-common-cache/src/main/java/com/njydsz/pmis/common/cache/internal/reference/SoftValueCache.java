 ...
 * <p>核心特性：
 * <ul>
 *   <li>软引用值：当内存不足时，GC 会回收软引用对象</li>
 *   <li>内存敏感：JVM 内存不足时自动释放缓存</li>
 *   <li>优先保留：相比弱引用，软引用会被更晚回收</li>
 *   <li>线程安全：使用 ConcurrentHashMap 保证并发安全</li>
 * </ul>
 *
 * <p>工作原理：
 * <ol>
 *   <li>使用 SoftReference 封装值对象</li>
 *   <li>ReferenceQueue 用于跟踪被 GC 回收的值</li>
 *   <li>每次缓存操作前先清理已回收的条目</li>
 *   <li>当 JVM 内存不足时，GC 会根据 LRU 策略回收软引用</li>
 * </ol>
 *
 * <p>适用场景：
 * <ul>
 *   <li>内存敏感缓存：担心 OOM 的场景</li>
 *   <li>图片缓存：Android 开发常用</li>
 *   <li>文档缓存：大对象缓存</li>
 *   <li>页面缓存：Web 页面缓存</li>
 * </ul>
 *
 ...
 * <ul>
 *   <li>软引用：内存不足时回收，适合缓存</li>
 *   <li>弱引用：GC 发现即回收，适合元数据缓存</li>
 * </ul>
 *
 * @param <K> 键类型
 * @param <V> 值类型
