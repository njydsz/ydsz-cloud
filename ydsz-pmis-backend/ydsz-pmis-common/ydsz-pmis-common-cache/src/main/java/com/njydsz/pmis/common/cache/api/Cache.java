 ...
 * <ul>
 *   <li>查询操作：get、getIfPresent、containsKey</li>
 *   <li>写入操作：put、putIfAbsent、compute、computeIfAbsent</li>
 *   <li>删除操作：remove、invalidate、invalidateAll、clear</li>
 *   <li>批量操作：getAll、putAll、removeAll</li>
 *   <li>统计信息：getStats、getHitRate、estimatedSize</li>
 *   <li>视图操作：asMap、keySet、values</li>
 *   <li>维护操作：cleanUp</li>
 * </ul>
 *
 * <p>防穿透/击穿/雪崩防护请使用 {@link CacheProtectionGuard}。
 * 空值占位管理请使用 {@link NullValueGuard}。
 *
 * @param <K> 键类型
 * @param <V> 值类型
 * @author Marvin Lee
...
public interface Cache<K, V> {
     ...
     * 获取缓存策略查询接口
     *
     * <p>允许在运行时查询和调整缓存策略（淘汰、过期等）。
     * 不是所有缓存类型都支持策略查询，不支持时返回 Optional.empty()。
     *
    ...
    default CachePolicy policy() {
        ...
        return new CachePolicy() {
            @Override
            public Optional<EvictionPolicy> eviction() { return Optional.empty(); }
            @Override
            public Optional<ExpirationPolicy> expiration() { return Optional.empty(); }
        };
    }
