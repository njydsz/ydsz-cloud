 ...
 * <p>核心特性：
 * <ul>
 *   <li>权重淘汰：根据元素权重进行淘汰，权重高的元素可能被淘汰</li>
 *   <li>可配置权重：支持自定义权重计算器，灵活控制权重策略</li>
 *   <li>容量保护：总权重不超过上限，防止内存溢出</li>
 ...
 * <ol>
 *   <li>每个缓存项关联一个权重值</li>
 *   <li>put 操作时检查总权重是否超限</li>
 *   <li>超限时按插入顺序淘汰旧元素直到权重满足</li>
 *   <li>权重计算器由用户自定义（Weigher 接口）</li>
 * </ol>
 *
 * <p>适用场景：
 * <ul>
 *   <li>内存敏感场景：不同大小对象需要不同的缓存配额</li>
 *   <li>资源受限环境：如嵌入式系统、移动设备</li>
 *   <li>多租户场景：按租户分配缓存配额</li>
 * </ul>
 *
 * <p>权重计算示例：
 * <pre>{@code
