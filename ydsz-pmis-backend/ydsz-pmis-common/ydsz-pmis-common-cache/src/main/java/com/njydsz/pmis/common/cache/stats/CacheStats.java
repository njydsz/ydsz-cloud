package com.njydsz.pmis.common.cache.stats;

/**
 * 缓存统计信息类
 *
 * <p>提供缓存运行时的各项统计指标，包括命中率、加载次数、加载耗时等，
 * 用于监控缓存性能和调优。
 *
 * <p>主要指标：
 * <ul>
 ...
 *   <li>loadCount：加载器调用总次数</li>
 *   <li>loadSuccessCount：加载成功次数</li>
 *   <li>loadExceptionCount：加载异常次数</li>
 *   <li>totalLoadTimeNanos：总加载时间（纳秒）</li>
 * </ul>
 *
 * <p>计算指标：
 * <ul>
 *   <li>hitRate：命中率 = 命中次数 / (命中次数 + 未命中次数)</li>
 *   <li>averageLoadPenalty：平均加载耗时 = 总加载时间 / 加载成功次数</li>
 * </ul>
 *
 * @author Marvin Lee
...
public class CacheStats {
    ...
    public CacheStats(long hitCount, long missCount, long evictionCount, long loadCount,
                      long loadSuccessCount, long loadExceptionCount, long totalLoadTimeNanos) {
    ...
    public CacheStats(long hitCount, long missCount, long loadCount,
                      long loadSuccessCount, long loadExceptionCount, long totalLoadTimeNanos) {
        this(hitCount, missCount, 0, loadCount, loadSuccessCount, loadExceptionCount, totalLoadTimeNanos);
    }
    ...
    public String toString() {
        return String.format(
            ...
            hitCount, missCount, getHitRate() * 100, loadCount,
            loadSuccessCount, loadExceptionCount, getAverageLoadPenalty() / 1_000_000.0
        );
    }
}
