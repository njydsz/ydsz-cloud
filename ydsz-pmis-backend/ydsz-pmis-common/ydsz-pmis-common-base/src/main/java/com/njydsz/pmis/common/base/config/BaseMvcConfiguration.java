 ...
 * 以及注册自己的拦截器和过滤器 Bean。
 *
 * <p>JSON 序列化统一使用 Jackson（大厂标准），通过 {@link MappingJackson2HttpMessageConverter} 处理。
 * ObjectMapper 优先使用 Spring 容器中注入的实例，若不存在则使用 JsonUtils 的全局实例。
 *
 * @author Marvin Lee
...
public abstract class BaseMvcConfiguration implements WebMvcConfigurer {
     ...
     * 注册 HTTP 消息转换器
     *
     * <p>使用 {@link MappingJackson2HttpMessageConverter} 统一 JSON 序列化行为。
     * ObjectMapper 优先从 Spring 容器获取，业务方可通过自定义 Bean 接管序列化配置。
     *
    ...
    public HttpMessageConverters httpMessageConverters(ObjectProvider<ObjectMapper> objectMapperProvider) {
        // 优先使用 Spring 容器中注入的 ObjectMapper，若不存在则使用 JsonUtils 的全局实例
