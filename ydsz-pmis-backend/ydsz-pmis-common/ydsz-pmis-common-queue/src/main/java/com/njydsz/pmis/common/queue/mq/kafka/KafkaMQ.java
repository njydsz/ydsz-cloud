...
public class KafkaMQ extends AbstractMessageQueue {
    ...
    public KafkaMQ(KafkaQueueProperties properties, ExecutorService consumerExecutor) {
        super("Kafka");
        if (properties == null) {
            throw new BizException("Kafka 配置不能为空");
        }
    ...
    public IMessagePublisher createPublisher(String channel) {
        ...
        if (channel == null || channel.isEmpty()) {
            throw new BizException("主题名称不能为空");
        }
    ...
    private void validateConnection() {
        try {
            ...
            throw new BizException("Kafka 连接失败，请检查配置：" + e.getMessage());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new BizException("Kafka 连接验证被中断");
        }
    }
}
