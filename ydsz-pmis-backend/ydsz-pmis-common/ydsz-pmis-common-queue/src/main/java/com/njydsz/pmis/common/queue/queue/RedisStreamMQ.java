 ...
 *
 * <p><b>连接复用：</b>通过 {@link RedisService} 复用 pmis-common-redis 的连接。
 *
...
public class RedisStreamMQ implements IMessageQueue {
    ...
    /**
     * 基于 RedisService 构造（复用 pmis-common-redis 连接，推荐）
     *
    ...
    public RedisStreamMQ(RedisService redisService, QueueProperties config, ExecutorService consumerExecutor) {
        ...
        log.info("[RedisStreamMQ] 初始化成功（复用 pmis-common-redis 连接），消费者组: {}",
    ...
    public IMessagePublisher createPublisher(String channel) {
        ...
        if (channel == null || channel.isEmpty()) {
            throw new BizException("通道名称不能为空");
        }
    ...
    public void close() {
        ...
        synchronized (this) {
            ...
            closed = true;
            // RedisTemplate 由 pmis-common-redis 管理，无需关闭
