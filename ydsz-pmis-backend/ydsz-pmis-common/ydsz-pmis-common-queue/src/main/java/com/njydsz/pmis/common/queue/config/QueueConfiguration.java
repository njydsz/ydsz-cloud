 ...
 * 当配置项 {@code pmis.queue.enabled=true} 时自动启用。
 *
 * <p><b>Redis 连接复用：</b>
 * 优先使用 pmis-common-redis 的 {@link RedisService} 复用 Redis 连接，
...
@EnableScheduling
@ConditionalOnProperty(prefix = "pmis.queue", name = "enabled", havingValue = "true", matchIfMissing = true)
public class QueueConfiguration {
    ...
    public void init() {
        ...
        if (redisService != null) {
            log.info("[Queue] 消息队列配置初始化，Redis 连接：复用 pmis-common-redis");
        } else {
            log.info("[Queue] 消息队列配置初始化，Redis 连接：自建 JedisPool（{}:{}）",
                    queueProperties.resolvedHost(),
                    queueProperties.resolvedPort());
            log.info("[Queue] 提示：推荐引入 pmis-common-redis 模块以复用 Redis 连接，" +
     ...
     * 优先注入 pmis-common-redis 的 {@link RedisService} 复用连接，
    ...
    public IMessageQueueProvider messageQueueProvider(ExecutorService consumerExecutor) {
        RedisService redisService = redisServiceProvider.getIfAvailable();
        if (redisService != null) {
            log.info("[Queue] 创建消息队列提供者（复用 pmis-common-redis 连接）");
    ...
    public DeadLetterQueueService deadLetterQueueService(IMessageQueueProvider messageQueueProvider) {
        RedisService redisService = redisServiceProvider.getIfAvailable();
        if (redisService != null) {
            log.info("[Queue] 创建死信队列服务（复用 pmis-common-redis 连接）");
    ...
    @ConditionalOnProperty(prefix = "pmis.queue.trace", name = "enabled", havingValue = "true")
