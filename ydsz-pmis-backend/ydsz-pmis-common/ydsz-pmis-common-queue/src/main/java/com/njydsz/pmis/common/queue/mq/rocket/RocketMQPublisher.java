 ...
 * <p><b>技术特点：</b>
 * <ul>
 *   <li>同步发送：带重试机制，失败时抛出 BizException</li>
...
public class RocketMQPublisher implements IMessagePublisher {
    ...
    public void publish(String message) {
        ...
        try {
            ...
            throw new BizException("RocketMQ 消息发布失败：" + e.getMessage(), e);
        }
    }
    ...
    public void publish(QueueMessage message) {
        ...
        throw new BizException("RocketMQ 消息发布失败，已重试 " + maxRetryTimes + " 次：" + errorMsg, lastException);
    }
            ...
            producer.send(msg, new SendCallback() {
                ...
                public void onSuccess(SendResult result) {
                    try {
                        ...
                        future.completeExceptionally(new BizException("RocketMQ 异步消息发送状态异常：" + e.getMessage(), e));
                    }
                }
                ...
                public void onException(Throwable e) {
                    ...
                    future.completeExceptionally(new BizException("RocketMQ 异步消息发送失败：" + e.getMessage(), e));
                }
            });
        } catch (Exception e) {
            log.error("[RocketMQ] 异步消息发送初始化失败，topic={}, traceId={}",
                    topic, message.getTraceId(), e);
            future.completeExceptionally(new BizException("RocketMQ 异步消息发送初始化失败：" + e.getMessage(), e));
        }
        return future;
    }
    ...
    public void publishDelayed(QueueMessage message, long delayMillis) {
        ...
        if (delayMillis <= 0) {
            throw new BizException("延迟时间必须大于 0");
        }
        checkNotClosed();
        ...
        log.error("[RocketMQ] 延迟消息发布失败，已重试 {} 次，topic={}, traceId={}",
                maxRetryTimes, topic, message.getTraceId(), lastException);
        String errorMsg = lastException != null ? lastException.getMessage() : "未知错误";
        throw new BizException("RocketMQ 延迟消息发布失败，已重试 " + maxRetryTimes + " 次：" + errorMsg, lastException);
    }
    ...
    private DefaultMQProducer createProducer(RocketMQProperties properties) {
        try {
            ...
            throw new BizException("RocketMQ 生产者创建失败：" + e.getMessage(), e);
        }
    }
    ...
    private void checkNotClosed() {
        if (closed) {
            throw new BizException("发布者已关闭，无法继续操作");
        }
    }
}
