 ...
 * 消息队列配置类
 *
 * <p>支持通过 Nacos 进行动态配置，配置前缀为 {@code pmis.queue}。
 * 支持通过 {@link MessageQueueFactory} 创建消息队列实例，复用 pmis-common-redis 连接。
 *
 * <p><b>连接复用：</b>
 * <p>当提供 {@link com.njydsz.pmis.common.redis.service.RedisService} 时，Redis 队列
 * 优先复用 pmis-common-redis 的连接，避免重复创建 JedisPool。
 *
...
@Slf4j
@Data
@ConfigurationProperties(prefix = "pmis.queue")
public class QueueProperties {
    ...
    /**
     * 构建消息队列工厂实例（复用 pmis-common-redis 连接，推荐）
     *
    ...
    public IMessageQueueProvider buildFactory(com.njydsz.pmis.common.redis.service.RedisService redisService,
        ...
        log.info("构建消息队列工厂（复用 pmis-common-redis 连接）");
