package com.remisoft.comm.notify.config;

import com.remisoft.comm.notify.channel.NotifyChannelStrategy;
import com.remisoft.comm.notify.core.NotifyService;
import com.remisoft.comm.notify.core.NotifyServiceImpl;
import com.remisoft.comm.notify.core.PersistentNotifyRetryQueue;
import com.remisoft.comm.notify.ratelimit.NotifyRateLimiterManager;
import com.remisoft.comm.notify.template.TemplateEngine;
...
import com.remisoft.comm.notify.core.AsyncNotifyService;
import com.remisoft.comm.notify.core.NotifyRetryQueue;
 ...
 * 统一消息通知自动配置类
 *
 * <p>注意：邮件发送 Bean 由 {@code remi-comm-email} 模块的 {@code EmailConfiguration} 负责创建，
...
@ConditionalOnProperty(prefix = "pmis.notify", name = "enabled", havingValue = "true", matchIfMissing = true)
public class NotifyConfiguration {
	 ...
	 *
	 * <p>当 {@code pmis.notify.rate-limit.enabled=true}（默认）时生效，
	 ...
	 *
	 * <p>当 {@code pmis.notify.retryQueue.persistent=true} 时，使用 Redis 持久化队列
	 * （支持多实例共享、重启不丢失），Redis 不可用时降级为内存队列。
	 * 当 {@code persistent=false} 时，直接使用内存队列。
	 * 队列容量、批量大小等参数可通过 {@code pmis.notify.retryQueue} 配置。
	 *
	 * @param properties         通知配置属性
