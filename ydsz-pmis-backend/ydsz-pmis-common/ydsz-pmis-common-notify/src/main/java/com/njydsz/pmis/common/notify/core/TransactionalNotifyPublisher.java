 ...
 * 事务安全通知发布器（P0-1）
 *
 * <p>确保通知发送与业务操作的原子性：业务事务提交后才触发通知发送，
 * 避免业务回滚但通知已发出导致的数据不一致问题。
 *
 * <p><b>工作机制：</b>
 * <ol>
 *   <li>业务代码调用 {@link #publishAfterCommit} 发布通知事件</li>
 *   <li>事件被暂存，不立即发送</li>
 *   <li>当外层事务成功提交后，{@link TransactionalEventListener} 回调触发</li>
 *   <li>回调中调用 {@link AsyncNotifyService} 异步发送通知</li>
 * </ol>
 *
...
public class TransactionalNotifyPublisher {
     ...
     */
    public TransactionalNotifyPublisher(ApplicationEventPublisher eventPublisher,
     ...
     * @param content  内容
     */
    public void publishAfterCommit(NotifyChannel channel, String receiver, String title, String content) {
     ...
     */
    public void publishAfterCommit(NotifyChannel channel, String receiver,
                                   String templateCode, Object templateParams) {
        NotifyRequest request = NotifyRequest.of(channel, receiver, templateCode, null)
    ...
    public void publishAfterCommit(NotifyRequest request) {
        ...
        log.debug("[TransactionalNotifyPublisher] 发布事务后通知事件: channel={}, receiver={}",
     ...
     * 事务提交后事件监听
     *
     * <p>仅当外层事务成功提交后才触发通知发送。
     * 使用 {@link TransactionPhase#AFTER_COMMIT} 确保事务一致性。
     *
     * @param event 通知事件
     */
    ...
    public void onAfterCommit(NotifyAfterCommitEvent event) {
        ...
        log.info("[TransactionalNotifyPublisher] 事务已提交，开始发送通知: channel={}, receiver={}",
                request.getChannel().getName(), request.getReceiver());
