 ...
 *
 * <p><b>设计目标：</b>
 * 支持扁平化实体设计，实体无需继承多层基类，只需在字段/类上标注对应注解即可具备能力。
 *
 * <p><b>使用示例：</b>
 * <pre>{@code
 * // 检查是否启用租户隔离
...
public final class EntityCapabilities {
     ...
     * 检查实体是否启用租户隔离能力
     *
     * <p>判断依据：实体中存在被 {@link TenantId} 注解标记的字段。
     *
     ...
     * 检查实体是否具备审计字段能力
     *
     * <p>判断依据：实体中同时存在被 {@link CreatedBy}、{@link CreateTime}、
     ...
     * 检查实体是否启用乐观锁能力
     *
     * <p>判断依据：实体中存在被 {@link Version} 或 MyBatis-Plus
     * {@code com.baomidou.mybatisplus.annotation.Version} 注解标记的字段。
     * 优先检查自定义注解，若未找到再检查 MyBatis-Plus 注解。
     *
     * @param entityClass 实体类
