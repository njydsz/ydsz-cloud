...
public class CosStorage extends AbstractFileStorage {
    ...
    protected boolean doFolderExists(String bucketName, String folderName) {
        try {
            ...
            com.qcloud.cos.model.ObjectMetadata metadata = cosClient.getObjectMetadata(bucketName, key);
    ...
    protected String buildPrivateUrl(String bucketName, String objectName) {
        try {
            ...
            log.error("[COS] generate private url failed, bucket={}, object={}, message={}",
                    bucketName, objectName, e.getMessage());
            return "";
        }
    }
    ...
    protected void doUploadPart(String bucketName, String chunkObjectName,
        ...
        try {
            ...
            log.info("[COS] chunk uploaded, bucket={}, chunk={}, part={}",
    ...
    protected void doAbortMultipartUpload(String bucketName, String objectName, String uploadId) {
        try {
            ...
            log.warn("[COS] abort multipart upload failed, bucket={}, object={}, uploadId={}, message={}",
    ...
    protected ObjectMetadata doGetMetadata(String bucketName, String objectName) {
        try {
            com.qcloud.cos.model.ObjectMetadata cosMetadata = cosClient.getObjectMetadata(bucketName, objectName);
            ...
            metadata.setLastModified(cosMetadata.getLastModified() != null
                    ? LocalDateTime.ofInstant(cosMetadata.getLastModified().toInstant(), ZoneId.systemDefault())
                    : null);
            metadata.setIsDirectory(false);
            return metadata;
        } catch (Exception e) {
            log.error("[COS] doGetMetadata failed, bucket={}, object={}, message={}",
                    bucketName, objectName, e.getMessage());
            return null;
        }
    }
