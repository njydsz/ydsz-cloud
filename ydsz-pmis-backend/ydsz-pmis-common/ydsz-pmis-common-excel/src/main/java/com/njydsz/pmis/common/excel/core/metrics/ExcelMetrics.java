package com.njydsz.pmis.common.excel.core.metrics;
 ...
 * Excel 模块可观测性指标
 *
 * <p>基于 Micrometer 的指标采集，覆盖 Excel 读写全流程关键路径。</p>
 *
 * <h3>指标清单</h3>
 * <ul>
 *   <li>{@code excel.write.duration} — 写入耗时（Timer, P50/P90/P99）</li>
 *   <li>{@code excel.read.duration} — 读取耗时（Timer, P50/P90/P99）</li>
 *   <li>{@code excel.rows.written} — 写入行数（Counter）</li>
 *   <li>{@code excel.rows.read} — 读取行数（Counter）</li>
 *   <li>{@code excel.write.failures} — 写入失败次数（Counter）</li>
 *   <li>{@code excel.read.failures} — 读取失败次数（Counter）</li>
 *   <li>{@code excel.cache.hits} — 缓存命中次数（Counter）</li>
 *   <li>{@code excel.cache.misses} — 缓存未命中次数（Counter）</li>
 * </ul>
 *
 * <p>当 MeterRegistry 不可用时（micrometer 未引入），所有方法为空操作，
 * 不影响业务逻辑。</p>
 *
...
public class ExcelMetrics {
    ...
    public static void recordWrite(Duration duration, int rows, String engine, boolean success) {
        ...
        Timer.builder("excel.write.duration")
                ...
                .tags(Tags.of(
                        Tag.of(TAG_ENGINE, engine),
                        Tag.of(TAG_RESULT, success ? "success" : "failure")))
                .register(registry)
                .record(duration);

        if (success) {
            registry.counter("excel.rows.written",
                    Tags.of(Tag.of(TAG_ENGINE, engine))).increment(rows);
        } else {
            registry.counter("excel.write.failures",
                    Tags.of(Tag.of(TAG_ENGINE, engine))).increment();
        }
    }
    ...
    public static void recordRead(Duration duration, int rows, String engine, boolean success) {
        ...
        if (success) {
            registry.counter("excel.rows.read",
                    Tags.of(Tag.of(TAG_ENGINE, engine))).increment(rows);
        } else {
            registry.counter("excel.read.failures",
                    Tags.of(Tag.of(TAG_ENGINE, engine))).increment();
        }
    }
