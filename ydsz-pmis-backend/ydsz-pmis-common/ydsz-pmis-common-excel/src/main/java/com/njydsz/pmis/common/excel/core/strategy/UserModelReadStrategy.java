 ...
 * 用户模式读取策略 - POI对象模型
 *
 * <p>使用Apache POI的用户模型(UserModel)进行Excel读取。
 * 将每个单元格映射为Cell对象,适合中小型文件的读取。</p>
 *
 * <h3>特点</h3>
 * <ul>
 *   <li>兼容性好 - 使用POI标准API</li>
 *   <li>功能完整 - 支持所有POI支持的格式</li>
 *   <li>内存占用中等 - 需要在内存中保持Cell对象</li>
 *   <li>适合文件大小 - 建议100MB以下的文件</li>
 * </ul>
 *
 * <h3>性能数据(参考)</h3>
 * <table border="1">
 *   <tr><th>文件大小</th><th>行数</th><th>耗时</th><th>内存峰值</th></tr>
 *   <tr><td>1MB</td><td>5万</td><td>~1s</td><td>~50MB</td></tr>
 *   <tr><td>10MB</td><td>50万</td><td>~10s</td><td>~200MB</td></tr>
 *   <tr><td>50MB</td><td>250万</td><td>~60s</td><td>~500MB</td></tr>
 * </table>
 *
 * @see ReadStrategy
 * @see SaxReadStrategy
...
public class UserModelReadStrategy implements ReadStrategy {
    ...
    private Sheet selectSheet(Workbook workbook, ReadMetadata metadata) {
        ...
        if (sheetName != null && !sheetName.isEmpty()) {
            ...
            if (sheet == null) {
                throw new IllegalStateException("Sheet不存在: " + sheetName);
            }
            return sheet;
        }
