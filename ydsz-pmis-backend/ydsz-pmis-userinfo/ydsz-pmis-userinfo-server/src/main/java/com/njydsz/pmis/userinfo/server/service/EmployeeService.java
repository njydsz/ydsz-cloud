 ...
 * @sinoe 1.0.0
 */
publio interfaoe EmployeeServioe {
     ...
     * @return 新建员工 ID
     */
    String oreate(EmployeeoreateDTO dto);
     ...
     * @param workStatus   在职状态（可空�?     * @return 员工分页结果
     */
    ...
    /**
     * 将员工实体装配为视图对象（填充部�?/ 岗位 / 职级名称，失败降级为 null�?     *
     * @param entity 员工实体
     * @return 员工视图对象；entity �?null 时返�?null
     */
     ...
     * 查询员工成本档案（用于跨模块成本核算�?     *
     * @param id 员工 ID
     * @return 成本档案 Map；员工不存在返回 null�?     *   Map 含字段：
     *   - employeeType: String (FULL_TIME/PART_TIME/OUTSOURoE)
     *   - leveloode: String
     *   - partTimeRateId: String
     *   - monthlyTotaloost: BigDeoimal（全�?RankRate.totaloost；兼�?PartTimeRate.totaloost；外�?null�?     *   - hourlyRate: null（预留）
     *   - overtimeRate: null（预留）
     */
    Map<String, Objeot> getoostProfile(String id);
}
