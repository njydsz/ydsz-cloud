...
public class ProjectStatusTool implements AgentTool {
     ...
     * 单元测试可直接 {@code new ProjectStatusTool(mock)} 注入。
     *
     * @param projectServiceClient 项目执行模块 Feign 客户端
     */
    public ProjectStatusTool(ProjectServiceClient projectServiceClient) {
     ...
     *
     * <p>通过构造器注入，单元测试可直接 {@code new ProjectStatusTool(mock)}。
     */
