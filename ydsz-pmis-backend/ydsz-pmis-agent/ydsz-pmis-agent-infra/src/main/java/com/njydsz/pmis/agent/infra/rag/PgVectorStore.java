 ...
 * 依赖表 {@code pmis_agent_document_chunk}（含 vector 类型列）。
 *
 * <p><b>DDL：</b>
 * <pre>
 * CREATE TABLE pmis_agent_document_chunk (
 ...
 * );
 * CREATE INDEX idx_chunk_embedding ON pmis_agent_document_chunk USING ivfflat (embedding vector_cosine_ops);
 * CREATE INDEX idx_chunk_doc ON pmis_agent_document_chunk(document_id);
 * </pre>
 *
 * @author ydsz-pmis-team
 * @since 1.2.0
 */
public class PgVectorStore implements com.njydsz.pmis.agent.domain.rag.VectorStore {

    private static final Logger log = LoggerFactory.getLogger(PgVectorStore.class);

    private final JdbcTemplate jdbcTemplate;
    private final com.njydsz.pmis.agent.domain.rag.EmbeddingClient embeddingClient;
    private final int dimension;

    public PgVectorStore(JdbcTemplate jdbcTemplate,
                         com.njydsz.pmis.agent.domain.rag.EmbeddingClient embeddingClient) {
    ...
    public void store(TextChunk chunk) {
        String sql = """
                INSERT INTO pmis_agent_document_chunk
        ...
        String metadataJson = JSON.toJSONString(chunk.getMetadata());
    ...
    public List<TextChunk> searchByVector(List<Float> embedding, int topK, double minScore) {
        ...
        String sql = """
                ...
                FROM pmis_agent_document_chunk
    ...
    public void deleteByDocument(String documentId) {
        jdbcTemplate.update(
                "DELETE FROM pmis_agent_document_chunk WHERE document_id = ?",
                documentId);
    ...
    public long count() {
        try {
            Long count = jdbcTemplate.queryForObject(
                    "SELECT COUNT(*) FROM pmis_agent_document_chunk", Long.class);
    ...
    public boolean isAvailable() {
        try {
            jdbcTemplate.queryForObject("SELECT 1 FROM pmis_agent_document_chunk LIMIT 1", Integer.class);
            return true;
        } catch (Exception e) {
            return false;
        }
