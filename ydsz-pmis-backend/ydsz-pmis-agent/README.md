...
```
ydsz-pmis-agent/
├── ydsz-pmis-agent-api/          # Feign 客户端 + DTO
├── ydsz-pmis-agent-domain/       # 领域模型（Agent/Conversation/Tool/Memory/Prompt）
├── ydsz-pmis-agent-infra/        # LLM Provider + Redis 记忆 + 向量存储
├── ydsz-pmis-agent-server/       # 应用服务（ChatService + Config + Health）
└── ydsz-pmis-agent-web/          # REST API + 启动入口
```
