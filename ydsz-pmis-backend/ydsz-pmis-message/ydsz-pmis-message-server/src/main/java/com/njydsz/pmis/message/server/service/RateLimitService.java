...
public interface RateLimitService {
    ...
    /**
     * P2-5: 多维度发送限流检查�?     *
     * <p>�?receiver / templateCode / tenant 三个维度分别做令牌桶限流�?     * 任一维度超限即返�?false。各维度开关与 permits �?{@code MessageProperties.rateLimit} 配置�?     * 维度间为 AND 关系：所有启用的维度都通过才允许发送�?     *
     * <p>调用方应在限流失败时记录 {@code messageMetrics.recordSend(channel, "RATE_LIMITED", 0)}
     * 并抛�?{@code SysException(RATE_LIMIT)}�?     *
     * @param channel      通道（用于日志，不参与限�?key�?     * @param receiver     接收人（可为空，空则跳过 receiver 维度�?     * @param templateCode 模板编码（可为空，空则跳�?template 维度�?     * @param tenantId     租户 ID（可为空，空则跳�?tenant 维度�?     * @return true 表示所有启用的维度都未超限，允许发�?     */
    boolean checkSendLimit(String channel, String receiver, String templateCode, String tenantId);

    /**
     * P0-5: 优先级感知的多维度限流检查�?     *
     * <p>�?{@link #checkSendLimit} 基础上增加优先级感知�?     * <ul>
     *   <li>URGENT：跳�?template �?tenant 维度限流，仅保留 receiver 维度</li>
     *   <li>HIGH：限流阈值提�?2 �?/li>
     *   <li>NORMAL：正常限�?/li>
     *   <li>LOW：限流阈值减�?/li>
     * </ul>
     *
     * @param channel      通道
     * @param receiver     接收�?     * @param templateCode 模板编码
