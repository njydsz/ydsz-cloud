...
/**
 * 消息通道 SPI 接口�? *
 * <p>不同通道（SMS/EMAIL/PUSH/INAPP/WEBHOOK/DINGTALK/WECOM/FEISHU）实现此接口�? * �?{@link ChannelRouter} 统一收集、路由与分发。通道类型字符串需�? * {@link com.njydsz.pmis.message.domain.enums.MessageChannelEnum} 枚举名保持一致（大写）�? *
...
public interface MessageChannel {
    ...
    /**
     * 通道类型，大写，�?{@link com.njydsz.pmis.message.domain.enums.MessageChannelEnum} 一致�?     *
     ...
     * 发送消息�?     *
     * @param request 消息请求
     * @return 发送结果（含供应商侧追�?ID），失败时返�?{@code fail}
     */
     ...
     * @param logDO 消息日志实体（含 providerTraceId 用于查询�?     * @return 回执结果；空表示渠道不支持或暂无回执
     */
