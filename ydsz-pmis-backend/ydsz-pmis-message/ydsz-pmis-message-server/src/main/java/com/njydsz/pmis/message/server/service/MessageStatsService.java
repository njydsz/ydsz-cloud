...
public interface MessageStatsService {
    ...
    /**
     * P2-2: 消息转化漏斗分析�?     *
     * <p>漏斗四阶段：sent(已发�? �?delivered(已送达) �?read(已读) �?clicked(已点�?�?     * 各阶段为累积计数（clicked 隐含 read 隐含 delivered）�?     * 支持按通道和模板编码过滤，用于精细化分析特定渠�?模板的转化效果�?     *
     * @param start       起始时间（含），null 则默认最�?24h
     * @param end         结束时间（含），null 则当前时�?     * @param channel     通道过滤（可选，null/�?则不过滤�?     * @param templateCode 模板编码过滤（可选，null/�?则不过滤�?     * @return 漏斗统计（含各阶段数量与转化率）
     */
    FunnelStatsVO getFunnel(LocalDateTime start, LocalDateTime end, String channel, String templateCode);

    /**
     * P2-4: 成本看板统计�?     *
     * <p>按通道维度统计发送成本：单条成本 × 成功发送数 = 通道总成本�?     * 通道单价�?{@code pmis.message.cost.unit-prices} 配置�?     *
     * @param start 起始时间（含），null 则默认最�?24h
