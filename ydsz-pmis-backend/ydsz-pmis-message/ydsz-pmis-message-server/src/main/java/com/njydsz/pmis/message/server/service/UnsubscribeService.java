...
/**
 * 退订中心服务（P1-5）�? *
 * <p>提供基于 HMAC 签名 token 的一键退订能力，以及管理后台的退订记录查询与恢复订阅�? * �?{@link SubscriptionService} 协作：本接口负责 token 解析与编排，
...
public interface UnsubscribeService {
    ...
    /**
     * 通过 token 一键退订�?     *
     * <p>token 校验通过后调�?{@link SubscriptionService#unsubscribe} 执行退订�?     * 幂等：重复调用不会报错，仅更新退订时间�?     *
     ...
     * 分页查询已退订记录（管理后台）�?     *
     * @param query 查询参数
     * @return 分页结果，仅包含 status=UNSUBSCRIBED 的记�?     */
    PageResponse<MsgSubscriptionDO> pageUnsubscribed(UnsubscribeQueryDTO query);

    /**
     * 恢复订阅（管理后�?/ 用户自助）�?     *
     * <p>将指�?(userId, topicCode, channel) 的订阅状态从 UNSUBSCRIBED 改回 SUBSCRIBED�?     * 并清空退订时间。若记录不存在则�?SUBSCRIBED 新建�?     *
     * @param userId    用户 ID
     * @param topicCode 主题编码
     * @param channel   通道
     */
