 ...
 * <p>多实例部署下，{@link RealtimePushService} 不再直接调用 {@code SimpMessagingTemplate}�? * 而是将推送指令封装为本对象发布到 Redis Channel，所有实例订阅后各自推送到本地 WebSocket session�? * 从而实现跨节点广播�? *
 * <p>推送类型：
 * <ul>
 *   <li>{@code USER}：推送到指定用户的个人频�?{@code /topic/user/{userId}/notifications}</li>
