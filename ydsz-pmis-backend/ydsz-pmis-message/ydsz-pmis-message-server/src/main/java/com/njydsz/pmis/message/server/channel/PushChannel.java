 ...
 * <p>实现 {@link MessageChannel} SPI，内部根�?{@code pmis.message.push.provider}
 * 配置选择实际 {@link PushProvider}（getui / mock），无匹配时降级�?mock�? *
...
public class PushChannel implements MessageChannel {
    ...
    public MessageResult send(MessageRequest request) {
        if (request.getReceiver() == null || request.getReceiver().isBlank()) {
            return MessageResult.fail(CHANNEL_TYPE, "推送目标不能为�?);
        }
    ...
    public List<MessageResult> batchSend(List<MessageRequest> requests) {
        ...
        log.info("[PushChannel] 批量推�? provider={} count={} success={}",
