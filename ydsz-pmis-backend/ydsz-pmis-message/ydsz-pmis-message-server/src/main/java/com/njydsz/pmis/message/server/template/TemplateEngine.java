 ...
 *   <li>条件渲染：{@code {{#if var}}A{{else}}B{{/if}}}，支�?truthy 判定�?else 分支</li>
 *   <li>循环渲染：{@code {{#each list}}...{{this}}...{{this.prop}}...{{@index}}...{{/each}}}</li>
 *   <li>必填参数校验：{@link #render(String, Map, Set)} 缺失时抛 {@code SysException}</li>
 * </ul>
 *
 * <p>多渠道差异化�?{@code TemplateService.loadByCodeAndChannel} 在模板加载层实现�? * 引擎仅负责按给定模板内容渲染�? *
...
public interface TemplateEngine {
    ...
    /**
     * P0-3: 渲染模板并校验必填参数�?     *
     * <p>在渲染前校验 {@code requiredKeys} 中的 key 是否存在�?{@code params} 且非 null / 非空白字符串�?     * 任一缺失�?{@link com.njydsz.pmis.common.exception.custom.SysException}（错误码 MISSING_PARAMETER）�?     *
     * @param template     模板内容
