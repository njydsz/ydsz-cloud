 ...
 * <p>将推送指令发布到 Redis Channel {@code pmis:ws:cluster:push}，所有应用实�? * 通过 {@link WebSocketClusterSubscriber} 订阅�?Channel，收到消息后推送到本地
...
public class WebSocketClusterPublisher {
     ...
     * @param message 集群推送消�?     * @return true 表示发布成功
     */
    public boolean publish(WebSocketClusterMessage message) {
        ...
        try {
            ...
            log.debug("[WS-Cluster] 发布集群推�? type={} userId={} topic={}",
                    message.getPushType(), message.getUserId(), message.getTopic());
            return true;
        } catch (Exception e) {
            log.warn("[WS-Cluster] 发布失败,降级本地推�? err={}", e.getMessage());
            return false;
        }
    }
}
