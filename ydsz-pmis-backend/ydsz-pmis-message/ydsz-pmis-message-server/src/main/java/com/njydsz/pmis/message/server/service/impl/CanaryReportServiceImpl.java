...
/**
 * 灰度 A/B 报表服务实现（P1-6）�? *
 * <p>基于 {@code pmis_msg_log} �?selectCount 聚合,分别统计�? * <ul>
 *   <li>实验组（treatment）：{@code canary_key = canaryKey},即命中灰度并切换实验模板/通道的消�?/li>
 *   <li>对照组（control）：{@code template_code = canaryKey AND canary = 0 AND canary_key IS NULL},
 *       即未命中灰度、使用基线模板的消息</li>
 * </ul>
 * 每组统计 total/success/failed/retry/dead/delivered/read/clicked �?successRate/deliveryRate/readRate�? * 时间范围通过 created_at 区间过滤,null 时默认最�?7 天（灰度实验通常以天/周为单位观察）�? *
