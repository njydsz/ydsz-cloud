...
/**
 * RocketMQ 消息生产者封装�? *
 * <p>统一封装 {@link RocketMQTemplate} 同步/异步发�?自动生成雪花 messageId 保证消费端幂等�? * 条件装配:仅当 classpath 存在 RocketMQTemplate �?{@code rocketmq.producer.group} 配置时生效�? *
...
public class RocketMQMessageProducer {
    ...
    public String syncSend(MessageRequest req) {
        ...
        if (result == null || result.getSendStatus() != SendStatus.SEND_OK) {
            ...
            throw new RuntimeException("RocketMQ syncSend 状态异�? " + status);
        }
    ...
    /**
     * P2-3: 发送事务消息（半消息）�?     *
     * <p>发送半消息�?RocketMQ 会回�?{@link com.njydsz.pmis.message.server.producer.MessageTransactionListener}
     * 执行本地事务（校验模�?通道�?根据结果 COMMIT / ROLLBACK�?     * 适用于业务侧需要确保通知请求仅在本地事务成功后才投递的场景�?     *
     * @param req 消息请求
     * @return RocketMQ 半消�?ID（后�?commit/rollback �?TransactionListener 决定�?     */
