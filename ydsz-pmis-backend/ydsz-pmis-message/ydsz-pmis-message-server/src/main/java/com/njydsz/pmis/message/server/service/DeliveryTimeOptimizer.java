...
/**
 * P1-1: 智能推送时间优化器�? *
 * <p>基于用户历史活跃数据（消息已�?点击行为），学习每个用户的最佳推送时间窗口�? * 对于非紧急消息，推荐在用户最活跃的时段推送，提升消息打开率和用户体验�? *
...
public interface DeliveryTimeOptimizer {
    ...
    /**
     * 记录用户活跃行为�?     *
     * <p>当用户查�?点击消息时调用，更新用户活跃度画像�?     * Redis Bitmap �?hour-of-week 存储，每周一轮转�?     *
     * @param userId 用户 ID
     * @param channel 通道
     */
    ...
    /**
     * 获取用户最佳推送时间�?     *
     * <p>基于用户历史活跃数据，推荐未来最近的最佳推送时间窗口�?     * 如果用户无历史活跃数据，返回 null（由调用方使用默认时间）�?     *
     * @param userId  用户 ID
     * @param channel 通道
