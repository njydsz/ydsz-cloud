...
public class EmailBounceHandler {
     ...
     * 处理邮件退信回调�?     *
     * @param logId       消息日志 ID
     * @param bounceType  退信类�? HARD(硬退�?邮箱不存�? / SOFT(软退�?临时失败)
     * @param reason      退信原�?     * @param recipient   退信收件人
     */
    public void handleBounce(String logId, String bounceType, String reason, String recipient) {
        ...
        log.info("[EmailBounce] 退信处�? logId={} type={} recipient={} reason={}",
                logId, bounceType, recipient, reason);

        // 硬退信时记录无效邮箱（后续可用于用户通道绑定状态更新）
        if ("HARD".equalsIgnoreCase(bounceType)) {
            log.warn("[EmailBounce] 硬退�?建议标记邮箱无效: recipient={}", recipient);
        }
    }
}
