...
public interface MessageService {
    ...
    /**
     * 批量发送消息（同步循环,限制 100 �?批）�?     * 每条请求�?bizId 会统一设置�?batchId,便于后续进度查询�?     *
     * @param requests 消息请求列表
     * @param batchId  批次 ID（业务侧生成�?     * @return 批量发送结果（含成�?失败/跳过计数�?     */
     ...
     * <p>发送半消息�?�?{@link com.njydsz.pmis.message.server.producer.MessageTransactionListener}
     * 执行本地事务校验（通道/模板有效性）,COMMIT 后消费端异步处理�?     * 适用于业务侧需要确保通知请求仅在本地校验通过后才投递的场景�?     *
     * @param request 消息发送请�?     * @return 发送结果（success=true 表示半消息已提交,实际发送由消费端异步完成）
     */
