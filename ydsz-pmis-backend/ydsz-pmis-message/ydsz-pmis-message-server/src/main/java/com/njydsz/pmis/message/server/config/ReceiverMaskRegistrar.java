...
/**
 * 接收�?receiver)智能脱敏注册器�? *
 * <p>消息日志�?receiver 字段可能是手机号 / 邮箱 / 用户 ID / openId 等多种形态，
 * 单一固定策略无法覆盖。本类在启动时向 {@link SensitiveUtil} 注册名为
 * {@code "default"} �?CUSTOM 脱敏函数（{@link SensitiveSerializer} �? * {@code @Sensitive(CUSTOM)} 字段固定调用 "default" handler），按值特征自动选择策略�? * <ul>
 ...
 * </ul>
 *
 * <p><b>副作用警�?/b>：本注册器覆盖全局 "default" CUSTOM handler�? * 影响所�?{@code @Sensitive(CUSTOM)} 字段。当前项目其它模块均使用固定策略
 * （PHONE/EMAIL/ID_CARD 等），未使用 CUSTOM，故无冲突。若未来其它模块需�? * 不同�?CUSTOM 行为，应重构 {@link SensitiveSerializer} 支持 handlerName 参数�? *
...
public class ReceiverMaskRegistrar {
    ...
    /**
     * 启动时注�?receiver 脱敏 handler�?     */
    @PostConstruct
    public void register() {
        SensitiveUtil.register("default", ReceiverMaskRegistrar::maskReceiver);
        log.info("[ReceiverMaskRegistrar] receiver 脱敏 handler 已注�?);
    }
