 ...
 * </ol>
 *
 * <p>超时�?{@code pmis.webhook.connect-timeout / read-timeout}。发送失败被捕获并转为失败结果�? *
...
public class WebhookChannel implements MessageChannel {
    ...
    public MessageResult send(MessageRequest request) {
        ...
        if (!StringUtils.hasText(webhookUrl)) {
            log.warn("[WEBHOOK] 未配�?Webhook URL，跳过发�? receiver={}", request.getReceiver());
            return MessageResult.fail(CHANNEL_TYPE, "Webhook URL 未配�?);
        }
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                log.info("[WEBHOOK] 发送成�? url={} status={}", webhookUrl, statusCode);
    ...
    /**
     * 解析 Webhook URL，优先级：params.webhookUrl &gt; receiver(http 开�? &gt; 默认配置�?     *
     * @param request 消息请求
