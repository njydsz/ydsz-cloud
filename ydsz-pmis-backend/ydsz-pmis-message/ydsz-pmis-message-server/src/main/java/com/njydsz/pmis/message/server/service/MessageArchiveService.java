 ...
 * <ul>
 *   <li>全文搜索：按 content/receiver/templateCode 等字段模糊查�?/li>
...
public interface MessageArchiveService {
    ...
    /**
     * 全文搜索消息日志�?     *
     * @param keyword    搜索关键词（匹配 content/receiver/templateCode�?     * @param channel    通道过滤（null=不限�?     * @param status     状态过滤（null=不限�?     * @param bizType    业务类型过滤（null=不限�?     * @param startTime  开始时间（null=不限�?     * @param endTime    结束时间（null=不限�?     * @param tenantId   租户 ID
     * @param pageNum    页码�? 开始）
     * @param pageSize   每页条数
     * @return 分页结果
     */
