package com.njydsz.pmis.message.server.metrics;
 ...
 * <p>指标命名规范：
 * <ul>
 *   <li>{@code pmis.message.send.duration} — 发送耗时（Timer）</li>
 *   <li>{@code pmis.message.consume.delay} — 消费延迟（Timer）</li>
 *   <li>{@code pmis.message.send.total} — 发送总数（Counter）</li>
 *   <li>{@code pmis.message.send.success} — 发送成功数（Counter）</li>
 *   <li>{@code pmis.message.send.failure} — 发送失败数（Counter）</li>
 *   <li>{@code pmis.message.exception} — 异常计数（Counter）</li>
 * </ul>
 *
...
public class MessageServiceMetrics {
    ...
    public void recordSendDuration(String channel, Duration duration) {
        Timer timer = timerCache.computeIfAbsent(
                "send:" + channel,
                k -> Timer.builder("pmis.message.send.duration")
    ...
    public void recordConsumeDelay(String channel, long delayMillis) {
        Timer timer = timerCache.computeIfAbsent(
                "consume:" + channel,
                k -> Timer.builder("pmis.message.consume.delay")
                        .tags("channel", channel)
                        .description("消息消费延迟")
                        .publishPercentiles(0.5, 0.9, 0.99)
                        .register(meterRegistry)
    ...
    public void recordSendFailure(String channel, String templateCode, String tenantId, String errorType) {
        incrementCounter("pmis.message.send.total",
                ...
                "template", templateCode != null ? templateCode : "none",
                "tenant", tenantId != null ? tenantId : "default");
        incrementCounter("pmis.message.send.failure",
    ...
    public void recordException(String channel, String exceptionType) {
        incrementCounter("pmis.message.exception",
                "channel", channel,
