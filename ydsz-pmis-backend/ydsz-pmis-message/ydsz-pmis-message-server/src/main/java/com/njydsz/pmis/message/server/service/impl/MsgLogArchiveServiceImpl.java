 ...
 * <p>幂等性：
 * <ul>
 *   <li>DETACH 使用 CONCURRENTLY（PG 14+），不阻塞读�?/li>
...
public class MsgLogArchiveServiceImpl implements MsgLogArchiveService {
     ...
     * <p>cron: �?�?�?�?�?周。{@code 0 30 2 1 * *} = 每月 1 �?02:30:00�?     */
    ...
    public void scheduledArchive() {
        ...
        try {
            ...
            if (created != null) {
                log.info("[MsgLogArchive] 预创建分�? {}", created);
            }
        } catch (Exception e) {
            log.error("[MsgLogArchive] 预创建分区失�? {}", e.getMessage(), e);
        }
    }
    ...
    public List<String> archive(int year, int month) {
        ...
        for (int i = 0; i <= 24; i++) {
            ...
            if (!partitionExists(partitionName)) {
                log.debug("[MsgLogArchive] 分区不存�?跳过: {}", partitionName);
                continue;
            }
            ...
            try {
                // DETACH: PG 14+ 支持 CONCURRENTLY,不阻塞父表读�?                jdbcTemplate.execute(String.format(
                ...
                log.warn("[MsgLogArchive] DETACH 失败,跳过该分�? {} - {}", partitionName, e.getMessage());
                continue;
            }
    ...
    public String ensurePartition(int year, int month) {
        ...
        if (partitionExists(partitionName)) {
            log.debug("[MsgLogArchive] 分区已存�?跳过: {}", partitionName);
            return null;
        }
        YearMonth start = ym;
    ...
    private boolean supportsDetachConcurrently() {
        try {
            ...
            log.warn("[MsgLogArchive] 获取 PG 版本失败,默认使用�?CONCURRENTLY DETACH: {}", e.getMessage());
            return false;
        }
    }

    /**
     * 生成分区�? {@code pmis_msg_log_y2026m05}�?     *
