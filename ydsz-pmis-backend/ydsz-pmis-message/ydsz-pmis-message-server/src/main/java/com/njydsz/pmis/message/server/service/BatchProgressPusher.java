...
/**
 * 批量发送实时进度推送器（P1-10）�? *
 * <p>批量发送过程中，按进度阈值（�?10% 或每 100 条）通过 WebSocket 推送实时进�?
 * 发起人可在前端看到批量发送的实时状态�? *
...
public class BatchProgressPusher {
    ...
    public void pushProgress(MsgBatchDO batch, String senderId) {
        ...
        // 计算进度百分�?        int total = batch.getTotal() == null ? 0 : batch.getTotal();
        ...
        try {
            ...
            log.debug("[BatchProgress] 推送进�? batchId={} progress={}%",
                    batch.getBatchId(), Math.round(progress));
        } catch (Exception e) {
            log.warn("[BatchProgress] 推送失�? batchId={} err={}", batch.getBatchId(), e.getMessage());
        }
    }
