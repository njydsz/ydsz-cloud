 ...
 * <p>异步批量发送流程：
 * <ol>
 *   <li>{@link #submitBatch} 创建 PENDING 批次记录，返�?batchId</li>
 *   <li>{@link #executeBatch} 异步处理：逐条调用 {@link MessageService#send}�? *       实时更新 success/failed/skipped 计数</li>
 *   <li>处理完成后更新状态为 COMPLETED / FAILED</li>
 * </ol>
 *
 * <p>支持 receiverList 模式（统一模板+接收人列表展开）和 requests 模式（每条独立请求）�? * 单批最�?10000 条，超出拒绝。异步处理通过 Spring {@code @Async} 线程池执行�? *
...
public class BatchServiceImpl implements BatchService {
    ...
    public MsgBatchDO submitBatch(BatchSendRequestDTO dto) {
        if (dto == null) {
            throw new SysException(StandardResultCode.BAD_REQUEST, "批量发送参数不能为�?);
        }
        // 构建请求列表
        List<MessageRequest> requests = buildRequests(dto);
        if (requests.isEmpty()) {
            throw new SysException(StandardResultCode.BAD_REQUEST, "接收人列表为�?);
        }
        if (requests.size() > MAX_BATCH_SIZE) {
            throw new SysException(StandardResultCode.BAD_REQUEST,
                    "单批最�?" + MAX_BATCH_SIZE + " 条，当前 " + requests.size() + " �?);
        }
        // 创建批次记录
        ...
        log.info("[Batch] 批次已创�? batchId={} total={} channel={}", batchId, requests.size(), dto.getChannel());
    ...
    public BatchProgressVO getProgress(String batchId) {
        ...
        MsgBatchDO batch = msgBatchMapper.selectOne(new LambdaQueryWrapper<MsgBatchDO>()
                ...
                .last("LIMIT 1"));
        if (batch == null) {
            throw new SysException(StandardResultCode.NOT_FOUND, "批次不存�? " + batchId);
        }
    ...
    public void executeBatch(String batchId) {
        ...
        log.warn("[Batch] executeBatch(batchId) 暂不支持�?DB 恢复请求列表，请使用 executeBatchAsync(batchId, requests)");
    }
    ...
    private void doExecuteBatch(String batchId, List<MessageRequest> requests) {
        ...
        for (int i = 0; i < requests.size(); i++) {
            ...
            try {
                ...
                log.warn("[Batch] 单条发送失�? batchId={} idx={} err={}", batchId, i, e.getMessage());
                failed++;
            }
            // �?100 条更新一次进�?            if ((i + 1) % 100 == 0 || i == requests.size() - 1) {
