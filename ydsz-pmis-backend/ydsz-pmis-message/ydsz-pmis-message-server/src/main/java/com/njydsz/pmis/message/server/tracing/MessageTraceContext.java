 ...
 * <p>实现 {@link AutoCloseable}，配�?try-with-resources 在重�?/ 死信 / 回执等异步或回调环节
 * 进入时将 traceId 写入 MDC，退出时自动恢复 / 清除，确保日志始终携�?traceId�? *
 * <p>典型用法�? * <pre>{@code
 * try (MessageTraceContext ctx = MessageTraceContext.enter(logDO.getTraceId())) {
 *     // 此作用域�?MDC.traceId 已设置，所有日志自动携�? *     channelRouter.dispatch(logDO);
 * }
 * // 退出后 MDC 自动恢复/清除
 * }</pre>
 *
 * <p>traceId �?null / 空白时自动生成新 traceId（{@link TraceIdUtil#getOrCreate()}），
 * 保证下游日志可追溯�? *
 * <p>注意：本类仅管理 MDC 中的 traceId，不干预 Brave / Micrometer Tracing �?span 上下文�? * previousTraceId 读取�?{@link MDC#get} 而非 {@link TraceIdUtil#get()}，避�?Brave
...
public final class MessageTraceContext implements AutoCloseable {
    ...
    /** 进入�?MDC 中的 traceId（用于退出时恢复，null 表示原来无） */
    ...
    /**
     * 进入追踪上下文：�?traceId 写入 MDC�?     *
     * @param traceId 待设置的 traceId；为 null / 空白时自动生�?     * @return 上下文实例（try-with-resources 自动清理�?     */
    public static MessageTraceContext enter(String traceId) {
        // 仅读�?MDC（不�?Brave fallback），避免恢复时把 Brave traceId 当作 previous
