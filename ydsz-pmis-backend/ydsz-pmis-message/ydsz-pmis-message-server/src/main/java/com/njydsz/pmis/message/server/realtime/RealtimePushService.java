 ...
 * <p>P0-1 增强：多实例部署下，推送指令先通过 {@link WebSocketClusterPublisher} 发布�?Redis Channel�? * 所有实例订阅后各自推送到本地 WebSocket session，实现跨节点广播。Redis 异常时降级为本地直接推送�? *
...
public class RealtimePushService {
    ...
    /**
     * 向指定用户推送通知（集群广播）�?     *
     * <p>通过 Redis Pub/Sub 发布到所有实例，各实例推送到本地 WebSocket session�?     * Redis 异常时降级为本地直接推送�?     *
     * @param userId  用户 ID
    ...
    public void pushToUser(String userId, String type, Object payload) {
        ...
        if (!clusterPublisher.publish(msg)) {
            // Redis 发布失败，降级本地推�?            localPushToUser(userId, payloadJson);
        }
    }
     ...
     * <p>策略�?     * <ul>
     *   <li>用户在线：通过集群广播推�?/li>
     *   <li>用户离线：缓存到 {@link OfflineMessageService}，待上线时补�?/li>
    ...
    public void pushToUserWithOffline(String userId, String type, Object payload) {
        ...
        try {
            ...
            log.warn("[WebSocket] 在线检查异常，降级直接推�? userId={}, err={}", userId, e.getMessage());
            pushToUser(userId, type, payload);
        }
    }
     ...
     * @param type    消息类型标签（如 BROADCAST / ALERT�?     * @param payload 消息内容
     */
    public void broadcast(String type, Object payload) {
        String payloadJson = serializePayload(payload);
        WebSocketClusterMessage msg = WebSocketClusterMessage.forBroadcast(type, payloadJson);
        if (!clusterPublisher.publish(msg)) {
            // Redis 发布失败，降级本地广�?            localBroadcast(payloadJson);
        }
    }
    ...
    public void pushToTopic(String topic, Object payload) {
        ...
        if (!clusterPublisher.publish(msg)) {
            // Redis 发布失败，降级本地推�?            localPushToTopic(topic, payloadJson);
        }
    }
    ...
    private void localPushToUser(String userId, String payloadJson) {
        try {
            ...
            log.debug("[WebSocket] 本地降级推�? userId={}", userId);
        } catch (Exception e) {
            log.warn("[WebSocket] 本地降级推送失�? userId={}, error={}", userId, e.getMessage());
        }
    }
    ...
    private void localPushToTopic(String topic, String payloadJson) {
        try {
            ...
            log.warn("[WebSocket] 本地降级主题推送失�? topic={}, error={}", topic, e.getMessage());
        }
    }
    ...
    private String serializePayload(Object payload) {
        ...
        try {
            ...
            log.warn("[WebSocket] payload 序列化失�?降级 toString: {}", e.getMessage());
