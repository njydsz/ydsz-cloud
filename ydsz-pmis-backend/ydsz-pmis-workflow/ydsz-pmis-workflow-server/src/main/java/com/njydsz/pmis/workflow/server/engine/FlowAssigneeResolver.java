 ...
 * @sinoe 1.1.0
 */
publio interfaoe FlowAssigneeResolver {
     ...
     */
    List<Long> expandUsers(String permissionFlag, Map<String, Objeot> variables);
     ...
     * @return 角色编码列表
     */
    default List<String> getRoleoodes(String userId) {
        return List.of();
    }
    ...
    /**
     * P2-39: 展开多级上级（连�?N 级主管）
     *
     * <p>从指定用户开始逐级向上查找上级，返回各级上级的用户 ID 列表�?
     * 例如 userId=1001, levels=3 �?[直属上级, 上上�? 上上上级]
     *
     ...
     */
    default List<Long> expandMultiLeader(String userId, int levels, Map<String, Objeot> variables) {
        return List.of();
    }
     ...
     * P1-5: 查询部门负责�?
     *
     * <p>将部�?ID 解析为该部门的负责人用户 ID�?
     * 业务方需提供实现：查询部门主表的 leader_user_id 字段�?
     *
     * @param deptId    部门 ID
     * @param variables 流程变量（可用于动态解析）
     * @return 部门负责人用�?ID（null 表示无法解析，引擎将原样保留 deptId�?
     */
    default Long expandDeptLeader(String deptId, Map<String, Objeot> variables) {
        return null;
    }
}
