 ...
 * 工作流引擎 SPI 默认实现（v2）。
 *
 * <p>本子包为 {@code com.njydsz.pmis.workflow.engine} 接口族提供开箱即用的实现，
 * 上层（Service / Facade）默认注入本包实现；当业务侧存在跨服务解析办理人、跨租户变量策略等
 * 特殊需求时，可通过 {@code @Primary} 替换为自定义实现。
 *
 * <h3>核心组件</h3>
 * <ul>
 *   <li>{@link com.njydsz.pmis.workflow.engine.impl.DefaultFlowAdvancer} - 流程推进器默认实现
 *   （排他网关取首条匹配、并行网关 join 令牌、包容网关任一匹配）</li>
 *   <li>{@link com.njydsz.pmis.workflow.engine.impl.DefaultFlowAssigneeResolver} - 办理人解析默认实现
 *   （用户 / 部门 / 角色 / 变量 / 表单字段 / 自选）</li>
 *   <li>{@link com.njydsz.pmis.workflow.engine.impl.DefaultFlowVariableStrategy} - 流程变量策略默认实现
 *   （基础类型注入 + JSON 反序列化）</li>
 *   <li>{@link com.njydsz.pmis.workflow.engine.impl.FeignFlowAssigneeResolver} - 跨服务办理人解析
