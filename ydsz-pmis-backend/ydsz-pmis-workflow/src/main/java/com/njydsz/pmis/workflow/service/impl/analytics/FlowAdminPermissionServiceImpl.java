...
import com.njydsz.pmis.workflow.entity.analytics.FlowAdminRoleDO;
import com.njydsz.pmis.workflow.mapper.analytics.FlowAdminRoleMapper;
import com.njydsz.pmis.workflow.service.analytics.FlowAdminPermissionService;
...
public class FlowAdminPermissionServiceImpl implements FlowAdminPermissionService {
    ...
    public void grantRole(String userId, String roleCode, String tenantId) {
        if (!StringUtils.hasText(userId) || !StringUtils.hasText(roleCode)) {
            throw new BizException(BizErrorCode.BAD_REQUEST, "用户 ID 和角色编码不能为空");
        }
        // 检查是否已存在
