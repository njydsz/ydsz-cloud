 ...
 * </ul>
 *
 * <p>P2-1: 通过 {@link ObjectProvider}&lt;{@link RedisStringOps}&gt; 可选注入 Redis 二级缓存服务。
 * 仅当启用 {@code ydsz.feign.name-assembler.redis-cache-enabled=true} 且 classpath 中存在
 * {@link RedisStringOps} 时，Redis L2 缓存才会启用。
 *
...
public class UserInfoNameAssemblerAutoConfiguration {
     ...
     * 业务模块已引入 ydsz-userinfo-api 且开启了 FeignClient 扫描（{@link OrgQueryClient} 已注册），
     ...
     *
     * <p>P2-1: 通过 {@link ObjectProvider}&lt;{@link RedisStringOps}&gt; 可选注入 Redis 服务，
     * 实现 L1 (Caffeine) + L2 (Redis) 多级缓存。
     *
