 ...
 *
 * <ul>
 *   <li><b>Feign 调用</b>：通过 {@link OrgQueryClient} 的 5 个 batch-names 端点 （user / dept / role / post /
 *       company）批量解析 ID → 名称。
 *   <li><b>多级缓存</b>：P2-1 新增 L1（Caffeine 本地缓存，5 分钟 TTL）+ L2（Redis 分布式缓存，10 分钟 TTL，可配置关闭）， {@link
 *       #resolveName} 走缓存；{@link #batchResolveNames} 不走缓存（避免污染）。
 *   <li><b>try-catch 降级</b>：所有 Feign 调用包裹 try-catch，失败时返回空 Map / null， 不抛异常阻断业务主流程。
 *   <li><b>兜底策略</b>：{@link #enrich} / {@link #enrichOne} 在 Feign 失败或 ID 未命中时， 用 ID 字符串本身顶替 name
 *       字段（避免前端空白），可通过 {@code ydsz.feign.name-assembler.fallback-to-id=false} 关闭。
 *   <li><b>N+1 防护</b>：{@link #enrich} 内部自动收集所有 ID 后一次批量调用。
 * </ul>
 *
 * <p><b>线程安全</b>：所有状态字段使用并发容器，可被多线程共享。
 *
 * <p><b>注册方式</b>：通过 {@link UserInfoNameAssemblerAutoConfiguration} 注册， 启用条件为 classpath 中存在 {@link
