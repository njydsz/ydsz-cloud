...
/**
 * Testcontainers 集成测试基类 — 用户模块（云顶编码规范 §14.3 集成测试）。
 *
 * <p>提供：
 *
 * <ul>
...
@ActiveProfiles("test")
public abstract class UserInfoTestInitializer {

  /** PostgreSQL 16 容器 — 整个 JVM 生命周期内只启动一次，所有 IT 用例共享 */
  @Container
  protected static final PostgreSQLContainer<?> POSTGRES =
      new PostgreSQLContainer<>(DockerImageName.parse("postgres:16"))
          .withDatabaseName("ydsz_test_userinfo")
          .withUsername("test")
          .withPassword("test")
          .withReuse(true);
  ...
  static void configureDataSource(DynamicPropertyRegistry registry) {
    ...
    registry.add("ydsz.jdbc.mapper-scan-packages", () -> "com.njydsz.userinfo.infra.mapper");
