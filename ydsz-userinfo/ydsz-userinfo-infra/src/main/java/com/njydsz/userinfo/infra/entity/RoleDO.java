 ...
 * UserRoleDO} 和 {@link RolePermissionDO} 维护多对多关系。
 *
 * <p><b>RBAC 链路示意：</b>
 *
 * <pre>
 *   UserAccountDO ─(N)── UserRoleDO ──(N)── RoleDO ─(N)── RolePermissionDO ─(N)── MenuDO
 *       用户              中间表          角色              中间表             权限点
 * </pre>
 *
 * <p><b>核心字段：</b>
 ...
 *
 * <p><b>索引设计：</b>唯一索引 {@code uk_role_code}（{@code role_code}）， 普通索引 {@code idx_tenant_id}（{@code
 * tenant_id}）。
 *
 * @author ydsz-team
 * @since 1.0.0
 * @see UserRoleDO 用户-角色中间表
 * @see RolePermissionDO 角色-权限中间表
...
public class RoleDO extends MpBaseEntity<String> {
   ...
   *
   * <ul>
   *   <li>{@code ALL}：全部数据（无限制）
   *   <li>{@code DEPT_AND_CHILD}：本部门及子部门数据
   *   <li>{@code DEPT}：仅本部门数据
   *   <li>{@code SELF}：仅本人数据
   *   <li>{@code CUSTOM}：自定义部门（需配合 {@code role_custom_dept} 中间表）
   * </ul>
   */
