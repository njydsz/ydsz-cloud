 ...
 *
 * <p><b>自定义 SQL 说明：</b>本 Mapper 仅保留两处<b>必须原子化</b>的登录安全操作
 * （失败计数自增、成功态重置），以消除 {@code read-modify-write} 并发竞态；
 * 其余查询通过 Service 层使用 MyBatis-Plus 的 {@code LambdaQueryWrapper} 构造，避免 XML 维护成本。
 *
 * <p><b>主要索引：</b>
 * <ul>
...
public interface UserAccountMapper extends BaseMapper<UserAccount> {
     ...
     *
     * <p>通过单条 SQL 完成「自增 + 条件锁定」，避免业务层先读后写的并发竞态
     * （并发失败时计数丢失、锁定阈值无法达成的安全漏洞）。
     *
     * @param id           用户 ID
    ...
    @Update("""
            ...
            """)
    int increaseLoginFailCount(@Param("id") String id,
