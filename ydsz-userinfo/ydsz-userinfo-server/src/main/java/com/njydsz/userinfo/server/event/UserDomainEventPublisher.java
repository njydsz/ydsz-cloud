 ...
 *
 * <ul>
 *   <li>USER_CREATED — 用户创建时
 *   <li>USER_UPDATED — 用户更新时
 *   <li>USER_DELETED — 用户删除时
 *   <li>ROLE_CHANGED — 角色分配/变更时
 *   <li>DEPARTMENT_CHANGED — 部门变更时
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
...
public class UserDomainEventPublisher {
  ...
  public void publishUserCreated(UserAccount user) {
    ...
    publish(
        ...
        UserDomainEvent.of(
            DomainEventTypes.USER_CREATED,
            user.getId(),
            Map.of("username", user.getUsername(), "realName", user.getRealName())));
  }
  ...
  public void publishRoleChanged(String userId, int roleCount) {
    publish(
        ...
        UserDomainEvent.of(
            DomainEventTypes.ROLE_CHANGED,
            userId,
            Map.of("action", "ROLE_CHANGED", "roleCount", roleCount)));
  }
  ...
  public void publishDepartmentChanged(Department dept, String action) {
    ...
    publish(
        DomainEventTypes.ORG_STRUCTURE_CHANGED,
        dept.getId(),
        "DEPARTMENT",
        new UserDomainEvent(
            DomainEventTypes.ORG_STRUCTURE_CHANGED,
  ...
  private void publish(
      String eventType, String aggregateId, String aggregateType, DomainEvent event) {
