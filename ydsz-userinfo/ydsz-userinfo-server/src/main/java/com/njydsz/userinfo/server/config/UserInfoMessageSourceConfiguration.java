 ...
 * i18n/userinfo-messages} 合并加载， 确保 {@code ExceptionCodeScanner} 启动校验通过且异常 {@code getMessage()} 返回
 * 正确的国际化文案（而非裸 key）。
 *
 * <p><b>基名加载顺序：</b>
 *
 ...
 * </ol>
 *
 * <p>{@code @ConditionalOnMissingBean} 保证与 ydsz-common 的自动装配互不冲突： 若上下文中已存在其他 {@link MessageSource}
 * Bean，本配置不生效。
 *
 * @author ydsz-team
 * @since 26.09.01
...
public class UserInfoMessageSourceConfiguration {
  ...
  /** MessageSource 缓存刷新间隔（秒）。 */
  private static final int CACHE_SECONDS = 5;

  /**
   * 创建扩展基名的国际化消息源。
   *
   * <p>将 common 模块基名与 userinfo 模块基名合并， 使 {@code ExceptionCodeScanner} 能校验全量异常码 key， 同时保证 {@code
   * AbstractYdszException.getMessage()} 返回正确的用户可读文案。
   *
  ...
  public MessageSource messageSource() {
    ...
    log.info("[UserInfoMessageSource] 用户中心 i18n 基名已注册，basenames={}", Arrays.toString(allBasenames));
    return source;
  }
}
