 ...
 * 用户账号 Service 接口
 *
 * <p>封装用户账号的完整业务逻辑：CRUD、密码管理、角色分配、审批人展开查询、跨服务富化。 是用户中心服务（ydsz-userinfo）最核心的 Service，被各业务模块通过 {@code
 * UserAccountClient}（Feign）远程调用获取用户基础信息。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 ...
 *   <li>角色分配与查询（{@code assignRoles} / {@code getUserRoleIds}）
 *   <li>审批人展开查询（按角色编码 / 岗位编码 / 直属上级查询用户 ID 列表）
 ...
 *
 * <ul>
 *   <li>本接口中所有 {@code listXxxByXxxId} 方法可被工作流服务（ydsz-workflow）调用， 实现「角色/部门/岗位/直属上级」等审批人展开语义
 *   <li>{@code batchUserNames} 供 NameAssembler 富化业务 VO 的 createdBy/updatedBy/assignee 等字段
 * </ul>
 *
...
public interface UserAccountService {
   ...
   *
   * <p>支持字段：{@code username} 模糊、{@code phone} 精确、{@code email} 精确、 {@code deptId} 精确、{@code status}
   * 精确。
   *
   * @param query 查询条件
