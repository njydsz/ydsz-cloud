package com.njydsz.userinfo.server.service;

import java.util.List;

import com.njydsz.userinfo.domain.dto.UserDeptDTO;
import com.njydsz.userinfo.domain.vo.UserDeptVO;

/**
 * 用户-部门 Service 接口
 *
 * <p>封装用户-部门关联的完整业务逻辑：CRUD、主部门唯一性管理。 一个用户可属于多个部门（兼岗），但只能有 1 个主部门。
 *
 * <p><b>核心职责：</b>
 *
 * <ul>
 *   <li>用户-部门关联 CRUD
 *   <li>主部门唯一性管理（同一用户仅 1 个主部门，事务保证）
 * </ul>
 *
 * <p><b>典型使用：</b>
 *
 * <ul>
 *   <li>用户管理页面维护「所属部门」字段（支持多选 + 标记主部门）
 *   <li>审批人展开：{@code dept:xxx} 触发时匹配用户-部门关联表所有部门
 *   <li>数据权限：{@code Role.dataScope} 配合部门树实现隔离
 * </ul>
 *
 * <p><b>事务：</b>所有写操作（{@code save/updateById/removeById}）开启 {@code @Transactional(rollbackFor =
 * Exception.class)}，确保任一异常触发完整回滚。
 *
 * @author ydsz-team
 * @since 26.09.01
 * @see com.njydsz.userinfo.domain.vo.UserDeptVO 用户-部门关联 VO
 */
public interface UserDeptService {

  /**
   * 根据 ID 查询用户-部门关联。
   *
   * @param id 主键 ID
   * @return 用户-部门关联 VO
   */
  UserDeptVO getById(String id);

  /**
   * 查询全部用户-部门关联。
   *
   * <p>注意：新 Repository API 不直接支持无参全量查询，此方法返回空列表。
   * 请使用 {@code UserDeptRepository.findByUserId(userId)} 等具体查询方法。
   *
   * @return 关联列表（当前实现返回空列表）
   */
  List<UserDeptVO> list();

  /**
   * 创建用户-部门关联。
   *
   * <p>校验：① 用户与部门必须存在；② 设为主部门时取消该用户其它主部门（事务内）。
   *
   * @param dto 用户-部门关联 DTO
   * @return 新关联主键 ID
   */
  String save(UserDeptDTO dto);

  /**
   * 更新用户-部门关联。
   *
   * <p>设为主部门时取消该用户其它主部门（事务内）。
   *
   * @param dto 用户-部门关联 DTO（含 ID）
   * @return true=成功
   */
  boolean updateById(UserDeptDTO dto);

  /**
   * 删除用户-部门关联。
   *
   * @param id 主键 ID
   * @return true=成功
   */
  boolean removeById(String id);
}
