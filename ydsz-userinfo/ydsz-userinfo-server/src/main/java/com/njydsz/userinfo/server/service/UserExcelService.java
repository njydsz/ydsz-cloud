 ...
 * <p><b>导入流程：</b>
 * <ol>
 *   <li>读取 Excel 文件解析为 {@link UserImportDTO} 列表</li>
 *   <li>逐行校验（必填字段、用户名唯一、部门编码存在等）</li>
 *   <li>逐行创建用户（失败记入 failDetails，不中断后续行）</li>
 *   <li>返回导入结果（总数、成功数、失败数、失败明细）</li>
 * </ol>
 *
 * <p><b>导出流程：</b>
 * <ol>
 *   <li>查询用户列表（可按条件过滤）</li>
 *   <li>转换为导出 DTO</li>
 *   <li>使用 common-excel 生成 Excel 字节数组</li>
 * </ol>
 *
 * @author ydsz-team
 * @since 1.0.0
