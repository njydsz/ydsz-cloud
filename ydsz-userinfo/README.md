...
│       ├── query/                     # 分页查询对象（10 个）
│       ├── vo/                        # 视图对象（30+ 个，含 TreeVO / SecurityDashboardVO）
│       ├── enums/                     # 领域枚举（EnableStatusEnum / BanType / DeviceType / UserLifecycleStatusEnum 等）
│       ├── event/                     # 领域事件（UserDomainEvent / UserDomainEventType）
│       ├── alert/                     # 安全告警（SecurityAlert + SecurityAlertRepository）
│       ├── auth/                      # UserIdentityProvider
│       ├── config/                    # MfaSecretEncryptor
...
│       ├── config/                    # AesMfaSecretEncryptor / PlainMfaSecretEncryptor / SocialAuthProperties
│       └── social/                    # 社交登录提供者
...
└── ydsz-userinfo-web/                 # Web 层：Controller + Filter + Bootstrap
    └── src/main/java/com/njydsz/userinfo/web/
        ...
        └── controller/                # 29 个 Controller
            ...
            ├── LdapSyncController.java          # /api/v1/ldap-sync
            ├── MenuController.java              # /api/v1/menu
            ├── OAuth2ApplicationController.java # /api/v1/oauth2/application
            ...
            ├── SamlController.java              # /api/v1/saml
            ├── SamlIdpConfigController.java     # /api/v1/saml/idp
            ├── ScimController.java              # /scim/v2（SCIM 2.0 用户供给）
            ├── SecurityAlertController.java     # /api/v1/security-alert
            ├── SecurityDashboardController.java # /api/v1/security-dashboard
            ├── SocialAccountController.java     # /api/v1/social-account
            ├── SocialClientConfigController.java # /api/v1/social-client
            ├── SsoMetricsController.java        # /api/v1/sso/metrics
            ├── TokenExchangeController.java     # /api/v1/token-exchange
            ...
            ├── AdminSessionController.java      # /api/v1/admin/session（会话管理 + 账号封禁）
            ├── device/
            │   └── DeviceSessionController.java # /api/v1/admin/device（设备会话管理）
            ├── selfservice/
...
| `/api/v1/oauth2/application` | OAuth2 应用注册管理 |
| `/.well-known/openid-configuration` | OIDC Discovery 元数据文档 |
| `/.well-known/jwks.json` | JWKS 公钥集合 |
| `/api/v1/saml` | SAML 2.0 Service Provider 端点 |
| `/api/v1/saml/idp` | SAML IdP 配置管理 |
| `/api/v1/cas` | CAS 协议端点（单点登录/登出） |
| `/scim/v2` | SCIM 2.0 用户供给（RFC 7643/7644） |
| `/api/v1/webauthn` | WebAuthn/Passkey 无密码认证 |
| `/api/v1/social-account` | 社交账号关联管理 |
| `/api/v1/social-client` | 社交登录客户端配置 |
| `/api/v1/auth-policy` | 认证策略管理 |
| `/api/v1/security-alert` | 安全告警管理 |
| `/api/v1/security-dashboard` | 安全仪表盘 |
| `/api/v1/admin/session` | 会话管理 + 账号封禁 |
| `/api/v1/admin/device` | 设备会话管理 |
| `/api/v1/sso/metrics` | SSO 指标（CAS/OAuth2/SAML/OIDC/社交登录/WebAuthn） |
| `/api/v1/token-exchange` | Token 交换 |
| `/api/v1/ldap-sync` | LDAP 同步管理 |
...
| **跨域 SSO** | CrossDomainSsoFilter + CrossDomainTokenService |
