 ...
 * 用户-岗位关联实体
 *
 * <p>对应数据库表 {@code ydsz_user_post}，是连接用户与岗位的多对多中间表。
 * 一个用户可同时担任多个岗位（PM + SA），一个岗位可被多个用户承担。
 *
 * <p><b>设计要点：</b>
 * <ul>
 *   <li>采用「关系实体」模式（带审计字段），便于追溯岗位分配历史</li>
 *   <li>主岗位逻辑由 {@link UserAccount#getPositionCode()} 维护（非本中间表），
 *       避免主岗位/兼岗位混淆</li>
 *   <li>由 {@code UserPostController.assignPosts} 接口维护（批量分配）</li>
 * </ul>
 *
 * <p><b>与 {@link UserAccount#positionCode} 的关系：</b>
 * <ul>
 *   <li>{@code UserAccount.positionCode}：单值字段，记录用户的「主岗位」（{@code PM}）</li>
 *   <li>{@code UserPost} 中间表：多值关系，记录用户的「所有岗位」（{@code PM} + {@code SA}）</li>
 * </ul>
