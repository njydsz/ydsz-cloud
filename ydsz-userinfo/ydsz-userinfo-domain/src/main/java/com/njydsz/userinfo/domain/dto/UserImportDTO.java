 ...
 * <p><b>导入字段说明：</b>
 * <ul>
 *   <li>{@code username}：登录用户名（必填，全局唯一）</li>
 *   <li>{@code realName}：真实姓名（必填）</li>
 *   <li>{@code password}：初始密码（必填，需符合密码策略）</li>
 *   <li>{@code phone}：手机号（可选）</li>
 *   <li>{@code email}：邮箱（可选）</li>
 *   <li>{@code deptCode}：部门编码（可选，按编码关联部门）</li>
 *   <li>{@code positionCode}：岗位编码（可选，如 PM/DEV/QA）</li>
 *   <li>{@code leaderUsername}：上级用户名（可选，按用户名关联上级）</li>
 * </ul>
 *
 * @author ydsz-team
 * @since 1.0.0
