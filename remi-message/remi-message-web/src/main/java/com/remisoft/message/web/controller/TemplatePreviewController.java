...
import com.remisoft.message.domain.enums.MessageResultCode;
 ...
 * <ol>
 *   <li>从 {@code remi_msg_template} 加载模板（含 subject / content / variableDefs）</li>
 *   <li>按 {@code variableDefs}（JSON 格式：{@code {"name": "code", "type": "string", "required": true, "defaultValue": "000000"}}）解析变量定义</li>
 *   <li>调用 {@link com.remisoft.message.server.template.TemplateVariableValidator#validateAndFill} 校验必填项 + 填充默认值</li>
 *   <li>用 {@link com.remisoft.message.server.template.TemplateEngine#render} 替换 {@code ${var}} 占位符</li>
 *   <li>返回渲染后的 content / subject</li>
 * </ol>
 *
 * <p><b>多租户隔离：</b>按 {@link com.remisoft.common.security.TenantContext#getTenantId()} 当前租户加载模板，
 * 跨租户模板不可见。
 *
 * <p><b>安全特性：</b>
 * <ul>
 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.message.server.template.TemplateEngine 模板渲染引擎
 * @see com.remisoft.message.server.template.TemplateVariableValidator 变量校验器
 * @see com.remisoft.message.server.service.template.TemplateService 模板服务
 */
...
public class TemplatePreviewController {
    ...
    @Idempotent(key = "remi:message:TemplatePreviewController:previewRaw:lock", ttlSeconds = 5)
    @PostMapping("/raw")
