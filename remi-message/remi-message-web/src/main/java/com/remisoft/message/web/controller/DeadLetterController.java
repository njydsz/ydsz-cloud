 ...
 * 死信指超过最大重试次数（默认 {@code remi.message.max-retry-count}，通常 5 次）仍发送失败的消息，
 * 由 {@code RetryScheduler} 调度器在每次重试失败后递增 {@code retryCount}，
 * 超过阈值后将 {@code remi_msg_log.status} 置为 {@code DEAD}，进入死信状态。
 *
 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.message.server.service.core.MessageLogService 消息日志服务
 * @see com.remisoft.message.domain.entity.core.MsgLog 发送日志实体
...
public class DeadLetterController {
    ...
    @Idempotent(key = "remi:message:DeadLetterController:resend:lock", ttlSeconds = 5)
