...
import com.remisoft.message.domain.dto.config.UserChannelBindingDTO;
 ...
 * @author remi-team
 * @since 1.0.0
 * @see com.remisoft.message.server.service.config.UserChannelBindingService 用户通道绑定服务
 * @see com.remisoft.message.domain.entity.config.MsgUserChannel 绑定实体
 */
...
public class UserChannelBindingController {
    ...
    @Idempotent(key = "remi:message:UserChannelBindingController:upsert:lock", ttlSeconds = 5)
    @PostMapping
    ...
    @Idempotent(key = "remi:message:UserChannelBindingController:delete:lock", ttlSeconds = 5)
    @DeleteMapping("/{id}")
