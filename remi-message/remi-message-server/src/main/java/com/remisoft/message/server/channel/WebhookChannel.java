 ...
 *   <li>系统配置 {@code remi.webhook.default-url}（兜底默认地址）</li>
 * </ol>
 *
 * <p>超时取 {@code remi.webhook.connect-timeout / read-timeout}。发送失败被捕获并转为失败结果。
 *
 * @author remi-team
 * @since 1.0.0
 */
...
public class WebhookChannel implements MessageChannel {
    ...
    /**
     * 注入配置后按 {@code remi.webhook.connect-timeout / read-timeout} 构建 RestClient。
     */
    @PostConstruct
    public void init() {
    ...
    public MessageResult send(MessageRequest request) {
        ...
        payload.put("title", request.getSubject() == null ? "REMI 通知" : request.getSubject());
