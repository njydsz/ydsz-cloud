 ...
 * 用户消息偏好 Service
 *
 * <p>管理用户对各通道(站内/短信/邮件/推送/IM)的"接收/不接收"偏好,
 * 在消息发送前由 {@code MessageService} 调用 {@link #getByUser} 判断是否允许该通道发送。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li><b>CRUD</b>：{@link #upsert} / {@link #delete}</li>
 *   <li><b>查询</b>：{@link #getByUser} / {@link #listByUser}</li>
 * </ul>
 *
 * <p><b>三元组唯一约束：</b>{@code (userId, channel, bizType)} 唯一。
 * <ul>
 *   <li>{@code channel} — 通道(IN_APP/SMS/EMAIL/PUSH/IM)</li>
 *   <li>{@code bizType} — 业务类型(用于按业务线粒度配置,如 WORKFLOW/PROJECT/CONTRACT 等)</li>
 * </ul>
 *
