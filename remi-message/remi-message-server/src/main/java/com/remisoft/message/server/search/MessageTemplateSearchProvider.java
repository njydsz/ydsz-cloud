package com.remisoft.message.server.search;
...
public class MessageTemplateSearchProvider implements SearchProvider<MsgTemplate> {
    ...
    public List<SearchField> getSearchableFields() {
        return List.of(
                SearchField.builder()
                        .name("title").label("模板主题").type(FieldType.TEXT)
                        .weight(3.0f).searchable(true).highlightable(true).sortable(true)
                        .build(),
                SearchField.builder()
                        .name("subtitle").label("模板编码").type(FieldType.KEYWORD)
                        .weight(2.0f).searchable(true).highlightable(true)
                        .build(),
                SearchField.builder()
                        .name("content").label("模板内容").type(FieldType.TEXT)
