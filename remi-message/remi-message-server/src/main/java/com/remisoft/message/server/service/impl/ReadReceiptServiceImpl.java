 ...
 * <p>短信短链：
 * <ul>
 *   <li>生成：Redis 存储映射 {@code remi:shortlink:{code} → originalUrl}，TTL 7 天</li>
...
public class ReadReceiptServiceImpl implements ReadReceiptService {
    ...
    /** 短链映射 Redis key 前缀 */
    private static final String SHORTLINK_PREFIX = "remi:shortlink:";
    /** 已读状态 Redis key 前缀 */
    private static final String READ_STATUS_PREFIX = "remi:read:";
    /** 短链消息 ID 映射前缀 */
    private static final String SHORTLINK_MSG_PREFIX = "remi:shortlink:msg:";
    /** 短链 TTL（7 天） */
     ...
     *
     * <p>短码由雪花算法生成，Redis 中以 {@code remi:shortlink:{code} → originalUrl}、
     * {@code remi:shortlink:msg:{code} → msgId} 存储，TTL 7 天。写入失败时降级返回原始 URL，
     * 保证短信发送主流程不因短链异常中断。
     *
     ...
     * 处理邮件追踪像素回调，标记邮件已读。
     *
     * <p>在 Redis 写入 {@code remi:read:email:{msgId}=1}（TTL 30 天）；msgId 为空直接忽略，
     * 写入异常仅告警不影响主流程。
     *
     * @param msgId 消息 ID
     */
