...
import com.remisoft.message.domain.dto.receipt.ReceiptResult;
import com.remisoft.message.domain.entity.core.MsgLog;
import com.remisoft.message.domain.enums.core.MessageStatusEnum;
import com.remisoft.message.domain.enums.receipt.ReceiptStatusEnum;
...
@EnableScheduling
@ConditionalOnProperty(prefix = "remi.message", name = "receipt-pull-enabled", havingValue = "true", matchIfMissing = true)
public class ReceiptPuller {
     ...
     * 定时扫描回执缺失的消息。
     *
     * <p>默认 120s 扫描一次，通过 {@code remi.message.receipt-pull-scan-interval-ms} 配置。
     * 分布式锁通过 {@link DistributedScheduled} 注解自动管理，TTL 60s，获取失败直接跳过。
     */
    @Scheduled(fixedDelayString = "${remi.message.receipt-pull-scan-interval-ms:120000}")
