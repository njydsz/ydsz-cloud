package com.remisoft.message.server.service.batch;
 ...
 * 消息批次 Service
 *
 * <p>管理异步批量发送的批次生命周期：批次创建、进度跟踪、后台异步执行。
 * 适用于"一次业务操作要发送数百/数千条消息"的场景(如全员公告),通过异步化
 * 避免同步阻塞,前端可轮询 {@link #getProgress} 获取处理进度。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li><b>创建批次</b>：{@link #submitBatch} — 立即返回批次 ID,后台异步处理</li>
 *   <li><b>进度查询</b>：{@link #getProgress} — 返回 total/success/failed/pending 计数</li>
 *   <li><b>异步执行</b>：{@link #executeBatch} — 由后台调度器/线程池调用</li>
 * </ul>
 *
