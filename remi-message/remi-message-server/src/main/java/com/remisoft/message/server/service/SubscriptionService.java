package com.remisoft.message.server.service.config;

import java.util.List;

import com.remisoft.message.domain.dto.config.SubscriptionUpsertDTO;
import com.remisoft.message.domain.entity.config.MsgSubscription;

/**
 * 订阅关系 Service
 *
 * <p>封装用户对「主题+通道」维度的订阅/退订语义，是消息中心默认订阅模型的核心入口。
 * 业务调用方通过本服务判断某用户对某主题某通道是否处于"已退订"状态,若已退订则跳过发送。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li><b>CRUD</b>：{@link #upsert} — 新增或更新订阅关系</li>
 *   <li><b>查询</b>：{@link #listByUser} / {@link #listByTopic} — 按用户/主题查询订阅列表</li>
 *   <li><b>判断</b>：{@link #isSubscribed} / {@link #isBlocked} — 用于发送前置检查</li>
 *   <li><b>退订</b>：{@link #unsubscribe} — 用户自助退订,无记录时按 UNSUBSCRIBED 新建(P1-5)</li>
 * </ul>
 *
 * <p><b>默认订阅语义：</b>系统中"未配置"≠"未订阅"。
 * <ul>
 *   <li>无记录或 {@code SUBSCRIBED} → 视为已订阅（允许发送）</li>
 *   <li>仅当存在 {@code UNSUBSCRIBED} 记录时视为退订（拦截发送）</li>
 * </ul>
 *
