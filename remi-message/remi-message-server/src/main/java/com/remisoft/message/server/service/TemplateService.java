package com.remisoft.message.server.service.template;
 ...
 * 消息模板 Service 接口
 *
 * <p>提供消息模板（{@code remi_msg_template}）的完整管理能力：CRUD、版本管理、
 * 模板审核、模板预览、变量渲染。模板是消息发送的"内容模板"，通过 {@code ${var}}
 * 占位符实现动态内容替换。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li><b>CRUD</b>：{@link #create} / {@link #update} / {@link #delete} / {@link #getById} / {@link #page}</li>
 *   <li><b>加载渲染</b>：{@link #loadByCodeAndChannel} — 发送时按 (code, channel, locale) 三元组加载</li>
 *   <li><b>审核流</b>：{@link #audit} — 模板上线前需通过审核（避免乱发）</li>
 *   <li><b>预览</b>：{@link #preview} — 渲染模板但不发送，供开发调试</li>
 * </ul>
 *
 * <p><b>多语言回退：</b>{@link #loadByCodeAndChannel} 在 locale 为空时回退到默认语言（{@code zh-CN}），
 * 保证总有可用模板。
 *
 * <p><b>事务：</b>所有写操作（{@code create/update/delete/audit}）开启
 * {@code @Transactional(rollbackFor = Exception.class)}。
 *
 * @author remi-team
 * @since 1.0.0
 *
 * @see com.remisoft.message.domain.entity.template.MsgTemplate 模板实体
 * @see com.remisoft.message.server.service.TemplateVersionService 模板版本 Service
 */
public interface TemplateService {
     ...
     * 删除模板（逻辑删除）
     *
     * <p>已被业务引用的模板<b>禁止删除</b>（避免历史消息无模板可查）。
     * 由 Service 层在删除前校验。
     *
     * @param id 模板 ID
     */
     ...
     *
     * <p>消息发送时的核心反查接口，按 (templateCode, channel, locale) 三元组定位模板内容。
     * 多语言回退：locale=en-US 未命中时回退到 locale=zh-CN。
     *
     ...
     * 审核模板
     *
     * <p>模板上线前必须通过审核（{@code approve=true}）。审核通过后模板状态变为 {@code APPROVED}，
     * 可被 {@link #loadByCodeAndChannel} 加载。
     *
     * @param id  模板 ID
