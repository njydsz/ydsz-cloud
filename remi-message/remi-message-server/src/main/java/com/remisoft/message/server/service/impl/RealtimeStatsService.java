 ...
 * <ul>
 *   <li>每分钟维度：{@code remi:stats:realtime:{yyyyMMddHHmm}} → Hash(channel, count)</li>
 *   <li>延迟分位数：{@code remi:stats:latency:{channel}} → Sorted Set(score=costMs, member=msgId)</li>
 *   <li>渠道错误计数：{@code remi:stats:errors:{channel}:{yyyyMMdd}} → INCR</li>
 * </ul>
 *
...
public class RealtimeStatsService {
    ...
    public void recordSend(String channel, String status, long costMs) {
        try {
            String minuteKey = "remi:stats:realtime:" + LocalDateTime.now().format(MINUTE_FMT);
            // 按状态+通道计数
            ...
            if ("SUCCESS".equals(status) && costMs > 0) {
                String latencyKey = "remi:stats:latency:" + channel;
            ...
            if (!"SUCCESS".equals(status)) {
                String errorKey = "remi:stats:errors:" + channel + ":" + LocalDateTime.now().format(DAY_FMT);
    ...
    public Map<String, Long> getDailyErrorCounts() {
        ...
        for (String channel : new String[]{"SMS", "EMAIL", "PUSH", "INAPP", "DINGTALK", "WECOM", "WECOM_APP", "FEISHU", "WEBHOOK"}) {
            String key = "remi:stats:errors:" + channel + ":" + daySuffix;
