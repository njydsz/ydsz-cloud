...
import com.remisoft.message.infra.mapper.receipt.MsgReceiptMapper;
 ...
 * 回执服务实现。
 *
 * <p>管理消息送达/已读/点击回执 ({@code remi_msg_receipt})，包括 IM 渠道 Webhook 接收、
 *
...
public class ReceiptServiceImpl implements ReceiptService {
     ...
     * 处理渠道回执回调（送达/已读/点击）。
     *
     * <p>将回执落库 {@code remi_msg_receipt} 并联动更新消息日志回执状态；进入追踪上下文（外部回调无原始 traceId 时自动生成）。
     * 日志不存在仅告警不抛异常，保证回执可靠落库。dto 或 logId 为空抛 BAD_REQUEST。
     *
     * @param dto 回执回调数据（含 logId/回执类型/渠道信息等）
     * @throws com.remisoft.common.exception.custom.SysException dto 或关联 logId 为空时
     */
    @Override
