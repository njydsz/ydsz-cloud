...
@EnableScheduling
@ConditionalOnProperty(prefix = "remi.message", name = "retry-enabled", havingValue = "true", matchIfMissing = true)
public class RetryScanner {
     ...
     * 定时扫描重试队列。
     *
     * <p>默认 30s 扫描一次,通过 {@code remi.message.retry-scan-interval-ms} 配置。
     * 分布式锁通过 {@link DistributedScheduled} 注解自动管理,TTL 60s,获取失败直接跳过。
     */
    @Scheduled(fixedDelayString = "${remi.message.retry-scan-interval-ms:30000}")
