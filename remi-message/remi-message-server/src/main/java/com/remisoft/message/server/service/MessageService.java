 ...
 *
 * <p>提供全渠道（站内/短信/邮件/企业 IM/推送）消息的发送、日志查询、事务消息等核心能力。
 * 是消息中心（remi-message）的<b>统一入口</b>，所有消息发送都通过本 Service 进入。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li><b>同步发送</b>：{@link #send}（跨模块标准 DTO）/ {@link #sendDirect}（本模块 DTO）</li>
 *   <li><b>批量发送</b>：{@link #batchSend} — 同步循环，单批 ≤ 100 条</li>
 *   <li><b>事务消息</b>：{@link #sendTransactionally} — RocketMQ 半消息 + 本地事务校验</li>
 *   <li><b>异步发送</b>：{@link #sendAsync} — 落库 PENDING → MQ → 消费者回调</li>
 *   <li><b>日志查询</b>：{@link #pageLog} — 发送日志分页查询</li>
 * </ul>
 *
 * <p><b>通道路由：</b>由 {@code RouteRuleService} 按"用户偏好 → 路由规则 → 默认通道"三级决策
 * 选择最终通道，未匹配规则时降级到站内通知。
 *
 * <p><b>可靠性保证：</b>
 * <ul>
 *   <li>所有发送请求先落库（{@code MsgLog}）再投递，DB 是 Source of Truth</li>
 *   <li>失败时由 {@code RetryScanner} 自动重试（指数退避 + 最大次数）</li>
 *   <li>超过 {@code remi.message.receipt-timeout-minutes} 未回执标记为 TIMEOUT</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
 *
 * @see com.remisoft.message.server.service.RouteRuleService 路由规则 Service
 * @see com.remisoft.message.server.service.ReceiptService 回执 Service
 * @see com.remisoft.message.domain.entity.core.MsgLog 消息日志实体
 */
public interface MessageService {
     ...
     *
     * <p>通过 {@code MessageRequest} 跨服务传递参数，是各业务模块通过
     * {@code MessageClient}（Feign）调用的标准入口。
     *
     ...
     * 直接发送消息（走本模块 DTO）
     *
     * <p>供 remi-message 内部 Controller 使用，比 {@link #send} 多了本模块扩展字段
     * （如 {@code scheduledAt} 定时发送、{@code priority} 优先级）。
     *
     * @param dto 发送参数
     * @return 发送结果
     */
     ...
     *
     * <p>每条请求的 bizId 会统一设置为 batchId，便于后续进度查询。
     * 限制：单批最多 100 条，超出抛异常；失败条目不影响其他条目。
     *
     ...
     * 分页查询消息发送日志
     *
     * <p>支持按 {@code channel / receiver / templateCode / status / businessType} 多条件过滤。
     * 走 {@code idx_created_at} 索引，按创建时间倒序。
     *
     ...
     *
     * <p>发送半消息后，由 {@link com.remisoft.message.server.producer.MessageTransactionListener}
