package com.remisoft.message.domain.enums.core;

/**
 * 消息发送通道枚举（8 通道统一抽象）
 *
 * <p>对应 SQL {@code remi_msg_log.channel} 与 {@code remi_msg_template.channel} 的 CHECK 约束取值。
 * 每个枚举值对应一个 {@code ChannelStrategy} 实现，由 {@code ChannelRouter} 按通道类型分发。
 *
 * <p><b>通道分类：</b>
 * <ul>
 *   <li><b>传统通道</b>：SMS（短信）、EMAIL（邮件）、PUSH（App 推送）、INAPP（站内信）</li>
 *   <li><b>IM 通道</b>：DINGTALK（钉钉群机器人）、DINGTALK_WORK（钉钉工作通知）、
 *       WECOM（企业微信群机器人）、WECOM_APP（企业微信应用消息）、FEISHU（飞书）</li>
 *   <li><b>扩展通道</b>：WEBHOOK（自定义 Webhook）</li>
 * </ul>
 *
 * <p><b>通道能力差异：</b>不同通道支持的富文本格式、字符长度限制、速率限制、回执能力均不同，
 * 由 {@code ChannelStrategy} 实现类各自处理降级和适配逻辑。
 *
 * @author remi-team
 * @since 1.0.0
 */
