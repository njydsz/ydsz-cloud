 ...
 */
@Data
@ConfigurationProperties(prefix = "remi.safe.field-encryption")
public class EncryptFieldProperties {
     ...
     *
     * <p>当数据库中存储的密文无法被正确解密时（GCM 认证失败 / 密钥版本缺失 / 密文被篡改 / Base64 解码失败），
     * 按此策略处理：
     * <ul>
     *   <li>{@link DecryptFailureStrategy#THROW}（默认）— 直接抛异常，阻止读取链路继续；</li>
     *   <li>{@link DecryptFailureStrategy#RETURN_MASKED} — 返回 {@link #maskedValue}（默认 {@code ****}）；</li>
     *   <li>{@link DecryptFailureStrategy#RETURN_ORIGINAL} — 返回原值（不推荐，仅用于历史明文兼容）。</li>
     * </ul>
     *
     * <p><b>注意：</b>非密文格式的历史明文数据会自动识别并原样返回，不触发此策略。
     * 此策略仅用于「值看起来像密文但解密失败」的场景。
     */
