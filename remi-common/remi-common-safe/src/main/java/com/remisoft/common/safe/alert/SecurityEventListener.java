 ...
 * 安全事件监听器
 *
 * <p>串联安全事件处理链：当 {@link SecurityEventPublisher} 发布安全事件后，
 * 此监听器负责将事件分发给所有下游处理器：
 * <ul>
 *   <li>{@link SafeMetrics} — 采集 Micrometer 指标（Counter/Timer）</li>
 *   <li>{@link SecurityAuditLogger} — 记录结构化 JSON 审计日志（含 traceId）</li>
 * </ul>
 *
 * <p>安全事件处理链：
 * <pre>{@code
...
public class SecurityEventListener {
    ...
    public SecurityEventListener(SafeMetrics safeMetrics, SecurityAuditLogger auditLogger) {
        ...
        log.info("安全事件监听器初始化: metrics={}, audit={}",
