package com.remisoft.common.safe.aspect;
...
import com.remisoft.common.safe.annotation.Xss;
import com.remisoft.common.util.string.StringUtils;

/**
 * XSS 防护验证器
 *
 * <p>实现 Jakarta Validation 框架的 ConstraintValidator 接口，
 * 用于验证字符串参数是否包含潜在的 XSS 攻击代码。
 *
 * <p><b>检测能力：</b>
 * <ul>
 *   <li>HTML 标签检测：script、iframe、object、embed、applet 等</li>
 *   <li>危险协议检测：javascript:, vbscript:, data:, mailto: 等</li>
 *   <li>事件处理器检测：onload, onerror, onclick 等所有 on* 事件</li>
 *   <li>危险函数检测：eval, alert, prompt, confirm, expression 等</li>
 *   <li>DOM 对象检测：document, window, navigator, cookie 等</li>
 *   <li>编码绕过检测：HTML 实体编码、Unicode 编码、URL 编码等</li>
 * </ul>
 *
 * <p><b>与 XssFilter 的区别：</b>
 * <ul>
 *   <li>XssValidator：用于编程式参数校验，基于 Jakarta Validation</li>
...
public class XssValidator implements ConstraintValidator<Xss, String> {
    ...
    private static final Pattern IFRAME_PATTERN = Pattern.compile("<iframe|<frame|<object|<embed|<applet", Pattern.CASE_INSENSITIVE);
    private static final Pattern META_TAG_PATTERN = Pattern.compile("<meta|<link|<base|<area", Pattern.CASE_INSENSITIVE);
    private static final Pattern FORM_TAG_PATTERN = Pattern.compile("<form|<button|<textarea|<select|<option|<input", Pattern.CASE_INSENSITIVE);
    private static final Pattern STYLE_TAG_PATTERN = Pattern.compile("<style", Pattern.CASE_INSENSITIVE);
    ...
    private static final Pattern EXPRESSION_PATTERN = Pattern.compile(
            "expression\\s*\\(|url\\s*\\(|behavior\\s*:",
            Pattern.CASE_INSENSITIVE
    );
    private static final Pattern DOM_OBJECT_PATTERN = Pattern.compile(
            "\\b(document|window|navigator|history|location|localStorage|sessionStorage|cookie|crypto|subtle|indexedDB)\\b",
            Pattern.CASE_INSENSITIVE
    );
    private static final Pattern SVG_XML_PATTERN = Pattern.compile(
            "<svg|<xml|<math|<xlink",
            Pattern.CASE_INSENSITIVE
    );
    ...
    private static final Pattern ENCODED_SCRIPT_PATTERN = Pattern.compile(
            "(?:%[0-9a-f]{2}|\\\\x[0-9a-f]{2}|\\\\u[0-9a-f]{4})+",
            Pattern.CASE_INSENSITIVE
    );
    /** 安全加固：检测 URL 编码形式的 XSS（如 %3Cscript%3E、%3cscript%3e） */
    private static final Pattern URL_ENCODED_XSS_PATTERN = Pattern.compile(
            "%3[cC](?:script|iframe|object|embed|form|input|img|svg|body|link|meta|base|area|applet|style|xml|math)",
            Pattern.CASE_INSENSITIVE
    );
    /** 安全加固：检测十六进制 HTML 实体（如 &#x3c;、&#x3C;） */
    private static final Pattern HEX_ENTITY_XSS_PATTERN = Pattern.compile(
            "&#x0*3[cC];?",
            Pattern.CASE_INSENSITIVE
    );
    /** 安全加固：检测十进制 HTML 实体（如 &#60;、&#060;、&#0060;） */
    private static final Pattern DECIMAL_ENTITY_XSS_PATTERN = Pattern.compile(
            "&#0*60;?",
            Pattern.CASE_INSENSITIVE
    );
