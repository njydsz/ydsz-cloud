 ...
 */
@AutoConfiguration
@ConditionalOnProperty(prefix = "remi.safe.field-encryption", name = "enabled", havingValue = "true", matchIfMissing = true)
    ...
    public FieldEncryptionService fieldEncryptionService(EncryptFieldProperties properties) {
        if (properties.getKeys().isEmpty()) {
            throw new IllegalStateException(
                    "字段加密已启用但未配置密钥，请配置 remi.safe.field-encryption.keys");
        }
