package com.remisoft.common.safe.ratelimit.metrics;
 ...
 * <ul>
 *   <li>{@code remi_ratelimit_decisions_total{resource, result}} - 限流决策总数</li>
 *   <li>{@code remi_ratelimit_block_total{resource, algorithm}} - 限流阻塞数</li>
 *   <li>{@code remi_ratelimit_pass_total{resource, algorithm}} - 限流通过数</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
...
public class RateLimitMetricsCollector {
    ...
    private void registerGauges() {
        ...
        meterRegistry.gauge("remi_ratelimit_decisions_total", totalDecisions);
        meterRegistry.gauge("remi_ratelimit_block_total", totalBlocked);
        meterRegistry.gauge("remi_ratelimit_pass_total", totalPassed);
    }
    ...
    public void record(RateLimitDecision decision) {
        ...
        Tags tags = Tags.of(
                ...
                "algorithm", decision.getRule() == null || decision.getRule().getAlgorithm() == null
                        ? "unknown" : decision.getRule().getAlgorithm().name(),
                "mode", decision.getRule() == null || decision.getRule().getMode() == null
                        ? "unknown" : decision.getRule().getMode().name()
        );
        Counter.builder("remi_ratelimit_decisions")
                .tags(tags)
        ...
        if (decision.getResult() == RateLimitResult.BLOCKED) {
            ...
            Counter.builder("remi_ratelimit_blocked")
                    .tags(tags)
            ...
            Counter.builder("remi_ratelimit_passed")
                    .tags(tags)
