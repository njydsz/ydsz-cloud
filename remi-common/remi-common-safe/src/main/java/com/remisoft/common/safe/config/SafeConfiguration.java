package com.remisoft.common.safe.config;

import java.util.List;

import com.remisoft.common.safe.sensitive.SensitiveDataProperties;
...
import com.remisoft.common.safe.advice.XssRequestBodyAdvice;
import com.remisoft.common.safe.alert.SafeAlertProperties;
import com.remisoft.common.safe.alert.SecurityEventAggregator;
import com.remisoft.common.safe.alert.SecurityEventListener;
import com.remisoft.common.safe.alert.SecurityEventPublisher;
import com.remisoft.common.safe.audit.SecurityAuditLogger;
import com.remisoft.common.safe.metrics.SafeMetrics;
import com.remisoft.common.safe.config.condition.XssConverterModeCondition;

import io.micrometer.core.instrument.MeterRegistry;
import com.remisoft.common.safe.config.condition.XssFilterModeCondition;
import com.remisoft.common.safe.converter.XssJsonMessageConverter;
import com.remisoft.common.json.spring.boot.JsonAutoConfiguration;
import com.remisoft.common.safe.core.JsonBodyXssCleaner;
import com.remisoft.common.safe.csrf.CsrfTokenGenerator;
import com.remisoft.common.safe.csrf.CsrfTokenRepository;
import com.remisoft.common.safe.csrf.impl.DefaultCsrfTokenGenerator;
import com.remisoft.common.safe.csrf.impl.InMemoryCsrfTokenRepository;
import com.remisoft.common.safe.csrf.impl.RedisCsrfTokenRepository;
import com.remisoft.common.safe.crypto.NonceCache;
import com.remisoft.common.safe.filter.ApiSignatureFilter;
import com.remisoft.common.safe.filter.CsrfFilter;
import com.remisoft.common.safe.filter.IpAccessFilter;
import com.remisoft.common.safe.filter.SecurityHeaderFilter;
import com.remisoft.common.safe.filter.SqlInjectionFilter;
import com.remisoft.common.safe.filter.XssFilter;
import com.remisoft.common.safe.ip.IpAccessService;
import com.remisoft.common.safe.sensitive.SensitiveDataAdvice;
...
public class SafeConfiguration {
    ...
    @ConditionalOnProperty(prefix = "remi.safe.auto-block", name = "enabled", havingValue = "true", matchIfMissing = true)
     ...
     * {@code com.remisoft.common.safe.ratelimit.config.RateLimitAutoConfiguration}，
     * 由其统一管理。本配置类不再持有旧版限流 Bean，避免与新版自动配置产生 Bean 冲突。
     *
     * <p>启用方式：通过 {@code remi.ratelimit.enabled=true}（默认 true）开启
     * 新版限流自动配置；旧版配置 {@code remi.safe.ratelimit.enabled} 已被废弃。
     */
     ...
     * 仅在 {@code remi.safe.ip-access.enabled=true} 且 Redis 可用时注册。
     *
    ...
    @ConditionalOnBean(RedisService.class)
    @ConditionalOnProperty(prefix = "remi.safe.ip-access", name = "enabled", havingValue = "true")
     ...
     * 注册防重放 Nonce 缓存
     *
     * <p>用于 API 签名验证的 nonce 防重放存储，基于 remi-common-cache 实现 TTL 自动过期。
     ...
     * 仅在 {@code remi.safe.api-signature.enabled=true} 时注册。
     *
    ...
    @ConditionalOnProperty(prefix = "remi.safe.api-signature", name = "enabled", havingValue = "true")
    ...
    public FilterRegistrationBean<SqlInjectionFilter> sqlInjectionFilterRegistration(
            SecurityEventPublisher eventPublisher,
            @Value("${remi.safe.sql-injection.whitelist-paths:}") List<String> whitelistPaths,
            @Value("${remi.safe.sql-injection.whitelist-params:}") List<String> whitelistParams) {
