package com.remisoft.common.safe.ratelimit.provider;
...
import com.remisoft.common.safe.ratelimit.spi.RateLimitRuleListener;
 ...
 * 配置式规则提供器
 *
 * <p>从 {@link RateLimitProperties#rules} 加载静态规则，适用于简单场景。
 * 复杂场景下可由 Nacos / Apollo / DB 提供器替换。
 *
 * @author remi-team
 * @since 1.0.0
 */
...
public class ConfigRuleProvider implements RateLimitRuleProvider {
        ...
        if (properties.getRules() != null) {
            for (RateLimitRule rule : properties.getRules()) {
                try {
                    ...
                    log.warn("Invalid rate limit rule skipped: resource={}, error={}",
                            rule.getResource(), ex.getMessage());
                }
            }
        }
    ...
    public void saveRule(RateLimitRule rule) {
        ...
        ruleMap.put(rule.getResource(), rule);
        notifyListeners(rule, exists ? RateLimitRuleListener.ChangeType.UPDATED
                : RateLimitRuleListener.ChangeType.ADDED);
    }
