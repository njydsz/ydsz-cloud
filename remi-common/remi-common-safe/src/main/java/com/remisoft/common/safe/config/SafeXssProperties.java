...
public class SafeXssProperties {
     ...
     * 是否启用 XSS 过滤器
     *
     * <p>默认值为 true，即启用 XSS 防护。
     * 设置为 false 可全局关闭 XSS 过滤功能。
     */
     ...
     *
     * <p>数值越小，优先级越高。
     * 默认值为 2，建议在 Web 过滤器之后、业务过滤器之前执行。
     */
    private int order = 2;
     ...
     * <ul>
     *   <li>/error：Spring Boot 错误处理端点</li>
     ...
     * <p>可选值：
     * <ul>
     *   <li>{@code filter}：使用 Filter 层进行全局参数清洗</li>
     *   <li>{@code converter}：使用 HttpMessageConverter 在 JSON 反序列化时清洗（默认）</li>
     * </ul>
     *
     * <p>两种模式互斥启用，避免双重清洗。
     * Filter 模式适用于全局统一清洗，Converter 模式适用于仅对 JSON Body 做 XSS 过滤的场景，是推荐的默认模式。
     */
     ...
     *
     * <p>启用后，通过注册 {@code XssStringDeserializer} 到 Jackson ObjectMapper，
     * 所有 String 字段在 JSON 反序列化时自动进行 XSS 清洗。
     * 默认值为 true。
     */
     ...
     * 白名单 HTML 标签
     *
     * <p>允许通过的 HTML 标签列表，其他标签会被过滤。
     * 适用于富文本编辑器场景，允许特定的 HTML 标签。
     *
     ...
     * }</pre>
     */
    private Set<String> allowedTags = new HashSet<>(Arrays.asList(
            "a", "img", "br", "p", "div", "span", "strong", "em", "ul", "ol", "li"
    ));

    /**
     * 白名单 HTML 属性
     *
     * <p>允许通过的 HTML 属性列表，其他属性会被过滤。
     * 适用于富文本编辑器场景，允许特定的 HTML 属性。
     *
     ...
     * 是否启用 HTML 标签白名单过滤
     *
     * <p>启用后，只有 allowedTags 中的标签会被保留，其他标签会被过滤。
     * 默认值为 false，即使用默认的 HTMLFilter 规则。
     */
     ...
     * <p>可选值：
     * <ul>
     *   <li>{@code LOW}：仅检测明显的 XSS 攻击（script 标签、事件处理器）</li>
     *   <li>{@code MEDIUM}：检测常见 XSS 攻击模式（默认）</li>
     *   <li>{@code HIGH}：严格检测，包括编码绕过、Unicode 绕过等</li>
     * </ul>
     */
