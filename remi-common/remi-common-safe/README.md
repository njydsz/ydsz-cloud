...
> REMI 统一安全防护基座（L5 业务服务层）

提供 XSS 防护、SQL 注入防护、CSRF 双模式防护、敏感数据脱敏、限流（多维度 + 多算法）、验证码、AES-256-GCM 加密、API 签名验证、IP 黑白名单、安全事件自动响应、Micrometer 指标、安全审计日志等全栈 Web 安全能力，是 REMI 项目所有业务服务的统一安全基座。
...
<dependency>
    ...
    <artifactId>remi-common-safe</artifactId>
</dependency>
```
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.safe.enabled` | true | 安全模块总开关（关闭后所有子能力不可用） |

### XSS 防护（`remi.safe.xss`）
...
### SQL 注入防护（`remi.safe.sql-injection`）
...
### 安全响应头（`remi.safe.security-headers`）
...
| `cluster-key-prefix` | remi:ratelimit: | 集群限流 Redis Key 前缀 |
...
### API 签名验证（`remi.safe.api-signature`）
...
### 字段加密（`remi.safe.field-encryption`）
...
import com.remisoft.common.safe.ratelimit.enums.RateLimitDimension;
...
import com.remisoft.common.safe.encrypt.EncryptField;
import com.remisoft.common.safe.encrypt.EncryptTypeHandler;
import lombok.Data;
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/safe` | 安全模块健康检查：Redis 连通性、各安全能力注册状态（XSS / SQL 注入 / CSRF / 限流 / IP 访问 / API 签名 / 脱敏 / 验证码 / 加密 / 指标 / 审计日志） | `remi.safe.enabled=true`（默认 true）+ classpath 存在 `HealthIndicator` |
...
4. **限流降级**：Redis 不可用时，集群限流自动降级为本地限流（`fallback-on-error=PASS` 默认放行，可改为 `BLOCK` 拒绝）。`@RateLimit` AOP 通过 `remi.ratelimit.aop-enabled` 控制。
5. **字段加密密钥轮换**：`@EncryptField(keyVersion=N)` 配合 `remi.safe.field-encryption.keys` 多版本密钥映射，支持平滑轮换。解密失败时按 `failure-strategy` 处理（默认 `THROW` fail-safe）。
6. **自动封禁联动**：`SecurityEventAggregator` 监听安全事件，同一 IP 在 `window-seconds` 内触发超过 `threshold` 次事件时自动调用 `IpAccessService` 封禁 IP（需启用 `remi.safe.ip-access.enabled=true`）。
