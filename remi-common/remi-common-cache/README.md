...
<dependency>
    ...
    <artifactId>remi-common-cache</artifactId>
</dependency>
```
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.cache.type` | TINYLFU | 全局默认缓存类型（LRU / LFU / TINYLFU / WEIGHTED / CONCURRENT / STRIPED / ENHANCED_LOADING） |
| `remi.cache.cache-names` | - | 启动期预创建的缓存名称列表 |
| `remi.cache.maximum-size` | 1000 | 全局默认最大容量 |
| `remi.cache.expire-after-write` | 30 | 写入后过期时间（配合 `expire-time-unit`） |
| `remi.cache.expire-after-access` | 0 | 访问后过期时间（0 表示不启用） |
| `remi.cache.expire-time-unit` | MINUTES | 过期时间单位 |
| `remi.cache.refresh-after-write` | 0 | 写入后自动刷新间隔（0 表示不刷新） |
| `remi.cache.initial-capacity` | 64 | 初始容量 |
| `remi.cache.allow-null-values` | true | 是否允许缓存 null 值（防穿透） |
| `remi.cache.record-stats` | true | 是否启用统计 |
| `remi.cache.weak-keys` | false | 是否使用弱引用键 |
| `remi.cache.weak-values` | false | 是否使用弱引用值 |
| `remi.cache.soft-values` | false | 是否使用软引用值 |
| `remi.cache.caches.<name>.*` | - | per-cache 独立配置（覆盖全局默认，字段同上） |
| `remi.cache.health-check.enabled` | true | 缓存健康检查开关 |
| `remi.cache.warmup.enabled` | false | 启动期缓存预热开关 |
| `remi.cache.annotation.enabled` | true | 注解驱动切面开关（`@Cached` / `@CacheInvalidate` / `@CacheRefresh`） |
| `remi.cache.multilevel.rebuild-lock.enabled` | false | 分布式重建锁开关（需 RedisTemplate 在 classpath） |
...
import com.remisoft.common.cache.annotation.CacheInvalidate;
import com.remisoft.common.cache.annotation.CacheRefresh;
...
```java
import com.remisoft.common.cache.export.CacheExportImport;
