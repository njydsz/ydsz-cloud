...
/**
 * RemiCache Spring Boot 自动配置
 *
 * <p>提供 RemiCache 的 Spring Boot 自动配置， 支持通过 application.yml 配置缓存参数（全局默认 + per-cache 覆盖）。
 *
 * <p>配置示例：
 *
 * <pre>
 ...
 */
@AutoConfiguration
@ConditionalOnClass(RemiCacheManager.class)
@EnableConfigurationProperties(RemiCacheProperties.class)
public class RemiCacheAutoConfiguration {

  /**
   * 创建本地（进程内）缓存管理器，作为 Spring Cache 抽象的主实现。
   *
   * <p>将 {@link RemiCacheProperties} 中的全局默认参数（类型、容量、TTL、是否允许 null、弱引用开关等）
   * 一次性灌入 {@link RemiCacheManager}，并叠加 {@code caches} 下的 per-cache 覆盖配置。
   * 使用 {@code @ConditionalOnMissingBean}，允许用户自定义 {@code RemiCacheManager} 完全覆盖本默认实例。
   *
  ...
  public RemiCacheManager springLocalCacheManager(RemiCacheProperties props) {
  ...
  public SpringCacheHealthIndicator springCacheHealthIndicator(RemiCacheManager cacheManager) {
