 ...
 * 可信代理 IP 配置
 *
 * <p>管理反向代理（Nginx/SLB）公网出口 IP 集合，用于判断 X-Forwarded-For
 * 等转发头是否可信（防止客户端伪造真实 IP）。
 *
 ...
 *             &#64;Value("${remi.util.trusted-proxies:}") Set<String> proxies) {
...
public class TrustedProxyConfiguration {
    ...
    public TrustedProxyConfiguration(Set<String> trustedIps) {
        this.trustedIps = (trustedIps == null || trustedIps.isEmpty())
