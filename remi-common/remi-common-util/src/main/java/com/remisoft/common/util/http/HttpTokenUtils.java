 ...
 * HTTP Token 提取工具类
 *
 * <p>封装从 HTTP 请求头中提取访问令牌（Access Token）的标准化逻辑，
 * 支持多种 Token 传输头和前缀剥离。
 *
 * <h2>支持的 Token 头</h2>
 * <ol>
 *   <li>{@code X-Access-Token}（Remi 自定义头，优先级最高）</li>
 *   <li>{@code Authorization}（标准 OAuth2/JWT 头）</li>
 * </ol>
 *
 * <h2>前缀剥离</h2>
 * <p>若 Token 以 "{@code remi}" 开头（如 {@code remi eyJhbG...}），
 * 自动剥离前缀并返回仅包含 Token 部分的内容。
 *
 * @author remi-team
 * @since 2.0.0
 */
public final class HttpTokenUtils {
     ...
     * 从当前请求上下文中获取 Token。
     *
     * <p>依次尝试 {@code X-Access-Token}、{@code Authorization} 头，
     * 自动剥离 Remi 私有前缀（如果存在）。
     *
