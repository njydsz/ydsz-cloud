 ...
 * <ul>
 *   <li>基础配置 + 租户覆盖配置 → 最终生效配置</li>
 *   <li>默认配置 + 环境覆盖配置 → 运行时配置</li>
 *   <li>Nacos 远程配置 + 本地 override → 合并配置</li>
 * </ul>
 *
 * <p><b>合并规则（RFC 7396）：</b>
 * <ul>
 *   <li>patch 中的字段覆盖 target 中的同名字段</li>
 *   <li>patch 中值为 {@code null} 的字段从 target 中删除</li>
 *   <li>patch 中不存在的字段保留 target 中的原值</li>
...
public final class ConfigMergeUtils {
     ...
     * 多层配置合并（按优先级从低到高）。
     *
     * <p>例如：{@code mergeLayers(defaults, envConfig, tenantConfig)}
     * 后者覆盖前者。
     *
