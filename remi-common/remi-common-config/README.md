...
| ↑ | `spring-cloud-context` 在 classpath + `remi.config.change-monitor.enabled=true`（默认 true） | `ConfigChangeBridge` |
| ↑ | `spring-boot-health` 在 classpath + `remi.config.health.enabled=true`（默认 true） | `ConfigEncryptHealthIndicator` |
...
<dependency>
    ...
    <artifactId>remi-common-config</artifactId>
</dependency>
```
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.config.change-monitor.enabled` | true | 是否启用配置变更监听桥接 |
| `remi.config.change-monitor.snapshot-old-values` | true | 是否在变更通知前快照旧值（false 时 `oldValue` 为 null，减少内存开销） |
| `remi.config.cli.enabled` | true | 是否启用 CLI 工具 Bean |
| `remi.config.cli.algorithm` | `PBEWithHMACSHA512AndAES_256` | 加密算法（与 Jasypt 对齐） |
| `remi.config.cli.key-obtention-iterations` | 1000 | 密钥派生迭代次数 |
| `remi.config.cli.pool-size` | 4 | 加密器池大小 |
| `remi.config.health.enabled` | true | 是否启用加密健康检查 |

> 加密相关配置（`jasypt.encryptor.password` / `jasypt.encryptor.algorithm`）使用 Jasypt 原生属性，不在 `remi.config` 前缀下。
...
```java
import com.remisoft.common.config.hotreload.ConfigChangeListener;
...
```java
import com.remisoft.common.config.hotreload.ConfigChangeEvent;
import com.remisoft.common.config.hotreload.ConfigChangeListener;
import org.springframework.stereotype.Component;
...
```java
import com.remisoft.common.config.hotreload.ConfigChangeBridge;
import com.remisoft.common.config.hotreload.ConfigChangeListener;
...
```bash
# 加密（主密码作为第 3 参数）
java -cp remi-common-config.jar \
  com.remisoft.common.config.cli.ConfigCliTool \
...
```java
import com.remisoft.common.config.hotreload.ConfigMergeUtils;
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/config` | 配置加密健康检查 | `spring-boot-health` 在 classpath + `remi.config.health.enabled=true`（默认 true） |
...
4. **快照内存开销**：`remi.config.change-monitor.snapshot-old-values=true`（默认）时，`ConfigChangeBridge` 会在 `RefreshEvent` 触发时快照当前 Environment 中所有可枚举属性值，属性数量较多时存在内存开销。若不需要 `oldValue`，可设为 `false` 减少 GC 压力。
...
- **v1.0.0**（2026-08-02）：补全接入方式、配置项表、使用示例、SPI 扩展点、健康检查、注意事项章节；完善配置变更桥接工作原理、加密健康检查暴露字段、CLI 工具默认参数、JSON 合并规则等核心能力描述
