package com.remisoft.common.netty.pool;

import java.util.concurrent.TimeUnit;
import java.util.concurrent.atomic.AtomicInteger;

import com.remisoft.common.netty.transport.NativeTransportDetector;
import com.remisoft.common.netty.transport.NativeTransportDetector.TransportType;
 ...
 * </ul>
 *
 * <p>使用自定义线程命名（{@code remi-netty-boss-N} / {@code remi-netty-worker-N}），
 * 便于在线程 dump 中定位问题。
 *
...
public class NettyEventLoopPool {
    ...
    public synchronized EventLoopGroup acquireBossGroup(int threads) {
        if (bossGroup == null) {
            ...
            bossGroup = NativeTransportDetector.createEventLoopGroup(transportType, n, "remi-netty-boss");
    ...
    public synchronized EventLoopGroup acquireWorkerGroup(int threads) {
        if (workerGroup == null) {
            ...
            workerGroup = NativeTransportDetector.createEventLoopGroup(transportType, n, "remi-netty-worker");
    ...
    public EventLoopGroup createIsolatedBossGroup(int threads) {
        int n = threads <= 0 ? 1 : threads;
        EventLoopGroup group = NativeTransportDetector.createEventLoopGroup(transportType, n, "remi-netty-boss-iso");
    ...
    public EventLoopGroup createIsolatedWorkerGroup(int threads) {
        int n = threads <= 0 ? Runtime.getRuntime().availableProcessors() * 2 : threads;
        EventLoopGroup group = NativeTransportDetector.createEventLoopGroup(transportType, n, "remi-netty-worker-iso");
