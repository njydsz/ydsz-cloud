package com.remisoft.common.netty.config;
...
import com.remisoft.common.netty.event.ChannelEventDispatcher;
import com.remisoft.common.netty.event.ChannelEventListener;
import com.remisoft.common.netty.event.MessageDispatcher;
import com.remisoft.common.netty.health.NettyHealthIndicator;
import com.remisoft.common.netty.metric.NettyChannelMetrics;
import com.remisoft.common.netty.pool.NettyEventLoopPool;
import com.remisoft.common.netty.server.AbstractNettyServer;
import com.remisoft.common.netty.server.NettyServerLifecycle;
 ...
 * {@code remi.netty.enabled=true} 时自动生效。
 *
 * <p>自动注册以下 Bean：
 * <ul>
 *   <li>{@link NettyEventLoopPool} — EventLoop 线程池管理器</li>
 ...
 *   <li>{@link NettyHealthIndicator} — 健康检查（当 Actuator 在 classpath 时）</li>
 * </ul>
 *
 * <p>同时通过 {@link BeanPostProcessor} 自动将 {@link NettyChannelMetrics} 和
 * {@link NettyEventLoopPool} 注入到所有 {@link AbstractNettyServer} 和
...
@ConditionalOnProperty(prefix = "remi.netty", name = "enabled", havingValue = "true", matchIfMissing = true)
public class NettyAutoConfiguration {
     ...
     * 支持 fail-fast 模式：任一 Server 启动失败时终止应用。
     *
    ...
    public NettyServerLifecycle nettyServerLifecycle(
        ...
        log.info("[Netty] 注册 NettyServerLifecycle, servers={}, failFast={}",
                serverList.size(), properties.isFailFast());
    ...
    /**
     * BeanPostProcessor — 自动将 NettyChannelMetrics 和 NettyEventLoopPool
     * 注入到所有 AbstractNettyServer 和 AbstractNettyClient Bean。
     *
     * @param metrics       指标收集器
     * @param eventLoopPool EventLoop 池
