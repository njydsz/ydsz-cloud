...
提供 TCP Server/Client 抽象、断线重连、心跳空闲检测、SSL/TLS、LengthField 编解码、EventLoop 池管理、Channel 组管理、Epoll/KQueue 原生传输、流量整形、Micrometer 指标监控、健康检查能力，是 REMI 项目网络通信的统一基座。
...
| 类 | 说明 |
|---|---|
| `NettyProperties` | Netty 配置属性（JSR-303 校验，前缀 `remi.netty.*`） |
...
<dependency>
    ...
    <artifactId>remi-common-netty</artifactId>
</dependency>
```
...
import com.remisoft.common.netty.codec.LengthFieldCodec;
...
```java
import com.remisoft.common.netty.client.AbstractNettyClient;
import com.remisoft.common.netty.config.NettyProperties;
import com.remisoft.common.netty.codec.LengthFieldCodec;
import io.netty.channel.socket.SocketChannel;
import org.springframework.stereotype.Component;
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.netty.enabled` | true | 是否启用 Netty 模块 |
| `remi.netty.boss-threads` | 1 | Boss 线程数（接受连接） |
| `remi.netty.worker-threads` | 0 | Worker 线程数（0 = CPU 核数 × 2） |
| `remi.netty.so-backlog` | 128 | 连接队列大小 |
| `remi.netty.so-keep-alive` | true | TCP KeepAlive |
| `remi.netty.tcp-no-delay` | true | TCP_NODELAY |
| `remi.netty.connect-timeout-millis` | 5000 | 连接超时（毫秒） |
| `remi.netty.shared-event-loop` | true | 是否共享 EventLoopGroup（true=全局复用，false=独立） |
| `remi.netty.shutdown-quiet-period-seconds` | 2 | 优雅关闭静默期（秒） |
| `remi.netty.shutdown-timeout-seconds` | 15 | 优雅关闭超时（秒） |
| `remi.netty.fail-fast` | true | Server 启动失败时是否终止应用 |
| `remi.netty.native-transport` | AUTO | 原生传输模式（AUTO / ENABLED / DISABLED） |
| `remi.netty.idle.reader-idle-seconds` | 60 | 读空闲超时（秒，0=不检测） |
| `remi.netty.idle.writer-idle-seconds` | 30 | 写空闲超时（秒，0=不检测） |
| `remi.netty.idle.all-idle-seconds` | 0 | 全双工空闲超时（秒，0=不检测） |
| `remi.netty.reconnect.enabled` | true | 断线重连开关 |
| `remi.netty.reconnect.initial-delay-ms` | 1000 | 初始重连延迟（毫秒） |
| `remi.netty.reconnect.max-delay-ms` | 60000 | 最大重连延迟（毫秒） |
| `remi.netty.reconnect.max-retries` | -1 | 最大重试次数（-1=无限） |
| `remi.netty.ssl.enabled` | false | SSL/TLS 开关 |
| `remi.netty.ssl.key-store` | - | 密钥库路径 |
| `remi.netty.ssl.key-store-password` | - | 密钥库密码 |
| `remi.netty.ssl.key-store-type` | PKCS12 | 密钥库类型（PKCS12 / JKS） |
| `remi.netty.ssl.trust-store` | - | 信任库路径（双向认证） |
| `remi.netty.ssl.trust-store-password` | - | 信任库密码 |
| `remi.netty.ssl.trust-store-type` | PKCS12 | 信任库类型 |
| `remi.netty.ssl.need-client-auth` | false | 是否要求客户端认证（双向认证） |
| `remi.netty.traffic-shaping.enabled` | false | 流量整形开关 |
| `remi.netty.traffic-shaping.write-limit` | 0 | 写限速（bytes/s，0=不限） |
| `remi.netty.traffic-shaping.read-limit` | 0 | 读限速（bytes/s，0=不限） |
| `remi.netty.traffic-shaping.check-interval-ms` | 1000 | 检查间隔（毫秒） |
| `remi.netty.traffic-shaping.global` | false | 是否全局流量整形（true=限制整个 Server 总带宽） |
...
```java
import com.remisoft.common.netty.event.MessageHandler;
...
```java
import com.remisoft.common.netty.event.ChannelEventListener;
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/netty` | Netty 健康检查 | `spring-boot-health` 在类路径，`remi.netty.enabled=true` |
...
| 指标 | 类型 | 说明 |
|---|---|---|
| `remi.netty.channels.active` | Gauge | 活跃 Channel 数 |
| `remi.netty.bytes.read.total` | Gauge | 累计读取字节数 |
| `remi.netty.bytes.written.total` | Gauge | 累计写入字节数 |
| `remi.netty.connections.total` | Counter | 累计连接数 |
| `remi.netty.disconnections.total` | Counter | 累计断开数 |
| `remi.netty.messages.received` | Counter | 消息接收数 |
| `remi.netty.messages.sent` | Counter | 消息发送数 |
| `remi.netty.reconnect.attempts` | Counter | 重连尝试次数 |
| `remi.netty.reconnect.successes` | Counter | 重连成功次数 |
