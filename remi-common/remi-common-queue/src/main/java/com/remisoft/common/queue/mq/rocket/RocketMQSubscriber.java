package com.remisoft.common.queue.mq.rocket;
 ...
 * RocketMQ 消息订阅者
 *
 * <p>使用 RocketMQ Push Consumer API 实现消息消费功能。
 * 支持并发消费和顺序消费，提供消息重试和死信队列支持。
 *
 * <p><b>技术特点：</b>
 * <ul>
 *   <li>Push 模式：服务端推送消息，客户端被动接收</li>
 *   <li>并发消费：多线程并发处理消息</li>
 *   <li>顺序消费：按消息顺序依次处理</li>
 *   <li>消息重试：消费失败自动重试</li>
 *   <li>死信队列：重试超过阈值进入死信队列</li>
 * </ul>
 *
 * <p><b>消息重试机制：</b>
 * <ul>
 *   <li>消费失败后，消息会在一定时间间隔后重试</li>
 *   <li>默认重试 16 次，超过后进入死信队列</li>
 *   <li>可通过配置调整重试次数和间隔</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
...
public class RocketMQSubscriber implements IMessageSubscriber {
    ...
    private void processMessage(MessageExt msgExt, IMessageHandler handler) {
        ...
        try {
            ...
            lastError.set(null);
            log.debug("[RocketMQ] 消息处理成功，topic={}, msgId={}, traceId={}",
                    msgExt.getTopic(), msgExt.getMsgId(), message.getTraceId());
        } catch (Exception e) {
            lastError.set(e);
