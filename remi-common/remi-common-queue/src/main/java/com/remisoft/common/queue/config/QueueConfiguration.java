package com.remisoft.common.queue.config;
...
import com.remisoft.common.queue.controller.DeadLetterQueueController;
import com.remisoft.common.queue.dedup.DedupCleanupScheduler;
import com.remisoft.common.queue.dedup.MessageDeduplicator;
import com.remisoft.common.queue.health.QueueHealthIndicator;
import com.remisoft.common.queue.manager.QueueManager;
import com.remisoft.common.queue.metrics.QueueMetricsBinder;
import com.remisoft.common.queue.queue.IMessageQueueProvider;
import com.remisoft.common.queue.queue.MessageQueueFactory;
import com.remisoft.common.queue.scheduler.DeadLetterRetryScheduler;
import com.remisoft.common.queue.service.DeadLetterQueueService;
import com.remisoft.common.queue.service.impl.DeadLetterQueueServiceImpl;
import com.remisoft.common.queue.service.impl.NoOpDeadLetterQueueService;
import com.remisoft.common.queue.trace.DefaultMessageTraceRecorder;
import com.remisoft.common.queue.trace.MessageTraceAspect;
import com.remisoft.common.queue.trace.MessageTraceRecorder;
import com.remisoft.common.queue.trace.RedisMessageTraceRecorder;
 ...
 *
 * <p><b>Redis 连接复用：</b>
 * 优先使用 remi-common-redis 的 {@link RedisService} 复用 Redis 连接，
...
public class QueueConfiguration {
    ...
    public void init() {
        ...
        if (redisService != null) {
            log.info("[Queue] 消息队列配置初始化，Redis 连接：复用 remi-common-redis");
        } else {
            log.info("[Queue] 消息队列配置初始化，Redis 连接：自建 JedisPool（{}:{}）",
                    queueProperties.resolvedHost(),
                    queueProperties.resolvedPort());
            log.info("[Queue] 提示：推荐引入 remi-common-redis 模块以复用 Redis 连接，" +
     ...
     * 优先注入 remi-common-redis 的 {@link RedisService} 复用连接，
    ...
    public IMessageQueueProvider messageQueueProvider(ExecutorService consumerExecutor) {
        RedisService redisService = redisServiceProvider.getIfAvailable();
        if (redisService != null) {
            log.info("[Queue] 创建消息队列提供者（复用 remi-common-redis 连接）");
    ...
    public DeadLetterQueueService deadLetterQueueService(IMessageQueueProvider messageQueueProvider) {
        RedisService redisService = redisServiceProvider.getIfAvailable();
        if (redisService != null) {
            log.info("[Queue] 创建死信队列服务（复用 remi-common-redis 连接）");
     ...
     * 创建内存消息去重器（单实例场景）
     *
     * <p>当 remi.queue.dedup-enabled=true 且 RedisService 不可用时创建。
    ...
    @ConditionalOnProperty(prefix = "remi.queue", name = "dedup-enabled", havingValue = "true")
