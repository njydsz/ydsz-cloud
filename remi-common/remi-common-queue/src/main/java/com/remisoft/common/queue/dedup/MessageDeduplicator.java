package com.remisoft.common.queue.dedup;
 ...
 * 消息幂等性去重器
 *
 * <p>提供消息消费的去重机制，防止消息被重复处理。
 * 基于消息 traceId 进行去重判断，使用内存缓存存储已处理的消息记录。
 *
 * <p><b>注意：当前实现基于内存存储，仅适用于单实例场景。</b>
 * 分布式场景应使用 {@link RedisMessageDeduplicator} 或自行实现基于 Redis 的去重。
 *
 * <p><b>去重策略：</b></p>
 * <ul>
 *   <li>记录每个 traceId 的处理时间戳</li>
 *   <li>定期清理过期记录（默认 1 小时）</li>
 *   <li>支持自定义去重窗口大小</li>
 *   <li>使用 ConcurrentHashMap 保证线程安全，避免 ReadWriteLock 读锁升级写锁死锁风险</li>
 * </ul>
 *
 * <p><b>使用示例：</b></p>
 * <pre>{@code
 ...
 * <p><b>注意事项：</b></p>
 * <ul>
 *   <li>内存去重，应用重启后去重记录会丢失</li>
 *   <li>分布式场景建议使用 {@link RedisMessageDeduplicator} 实现全局去重</li>
...
public class MessageDeduplicator {
     ...
     * 检查并标记（原子操作）
     *
     * <p>如果消息未处理过，则标记为已处理并返回 false。
     * 如果消息已处理过，则返回 true（表示重复）。
     *
     * @param traceId 消息 traceId
    ...
    public void cleanupExpired() {
        ...
        processedRecords.entrySet().removeIf(
                entry -> now - entry.getValue() > expireWindowMs
        );
        lastCleanupTime = now;
    }
