package com.remisoft.common.queue.service.impl;
...
import com.remisoft.common.queue.metrics.MessageMetrics;
import com.remisoft.common.queue.rate.ConsumerRateLimiter;
import com.remisoft.common.queue.recovery.ConsumerThreadGuard;
import com.remisoft.common.queue.retry.RetryPolicy;
import com.remisoft.common.queue.service.IMessageHandler;
 ...
 *
 * <p>使用 Redis <b>XREADGROUP</b> 命令以消费组（Consumer Group）模式读取消息，
 * 实现了消息确认（ACK）、自动重试和死信队列（DLQ）三大核心能力。
 *
 * <h3>工作流程</h3>
 * <ol>
 *   <li>初始化时通过 <b>XGROUP CREATE</b> 创建消费组（已存在则忽略 BUSYGROUP）</li>
 *   <li>消费循环中以阻塞方式 <b>XREADGROUP</b> 读取消息，每次读取 {@code batchSize} 条</li>
 *   <li>消息处理成功后 <b>XACK</b> 确认；处理失败则按指数退避策略重试</li>
 *   <li>重试次数超过 {@code retryMax} 后，将消息转入死信队列 Stream 并 ACK 原消息</li>
 * </ol>
 *
 * <h3>顺序消息支持</h3>
 * <p>当消息携带 {@code groupKey} 和 {@code sequence} 字段时，支持同分组内的顺序消费，
 * 通过 {@link QueueMessage#setSequential(String, Long)} 标记顺序属性。
 *
 * <h3>线程安全</h3>
 * <p>通过 {@link AtomicBoolean} 控制运行状态，{@link ConsumerThreadGuard} 守护消费线程
 * 自动恢复意外退出的消费循环。{@link ConsumerRateLimiter} 实现消费端限流。
 *
 * @author remi-team
 * @since 1.0.0
...
public class RedisStreamSubscriber implements IMessageSubscriber {
    ...
    /** 复用 remi-common-redis 的 Redis 连接 */
     ...
     * 构造 Redis Stream 订阅者。
     *
     * <p>初始化时会自动调用 {@link #ensureGroup()} 创建消费组。如果消费组已存在
     * （Redis 返回 BUSYGROUP），则静默忽略，保证幂等启动。
     *
    ...
    public RedisStreamSubscriber(RedisTemplate<String, Object> redisTemplate,
        ...
        ensureGroup();
        log.info("[RedisStream] 订阅者初始化完成（复用 remi-common-redis 连接），channel={}, group={}, consumer={}",
     ...
     * {@inheritDoc}
     *
     * <p>同步拉取单条消息。以阻塞方式 XREADGROUP 读取一条消息后返回其 payload。
     * 如果消息解析失败会自动 ACK 并跳过，避免消息堆积。
     *
     ...
     * {@inheritDoc}
     *
     * <p>启动异步消费循环。通过 {@link ConsumerThreadGuard} 守护消费线程，
     * 当消费线程因异常退出时会自动重启。重复调用时仅记录警告并返回已有消费者 ID。
     *
     ...
     * {@inheritDoc}
     *
     * <p>将运行状态置为 false，消费循环在下一次迭代时退出。
     * 同时停止 {@link ConsumerThreadGuard} 线程守护。
     */
    @Override
    public void stop() {
     ...
     * <ol>
     *   <li>构建 XREADGROUP 读取参数（batchSize + blockMillis 阻塞）</li>
     *   <li>以 {@code ReadOffset.lastConsumed()} 从上次消费位置继续读取</li>
     *   <li>对每条消息先执行限流 {@link ConsumerRateLimiter#acquire()}，再交给 {@link #processEntry} 处理</li>
     * </ol>
     ...
     *
     * <p>以阻塞方式 XREADGROUP 读取一条消息，解析后返回。如果解析失败则自动 ACK
     * 并返回 null，防止毒消息无限阻塞消费组。
     *
     ...
     * <p>处理流程：
     * <ol>
     *   <li>解析消息体，解析失败则 ACK 并跳过</li>
     *   <li>注入 MDC traceId 实现全链路日志追踪</li>
     *   <li>调用业务 handler.onMessage() 处理消息</li>
     *   <li>成功则 XACK 确认 + 记录指标；失败则进入 {@link #handleFailedMessage}</li>
     * </ol>
     *
     ...
     * <p>策略：
     * <ul>
     *   <li>重试次数 ≤ {@code retryMax}：按 {@link RetryPolicy} 计算退避时间，
     *       休眠后将消息重新写入原 Stream 并 ACK 旧消息（避免重复消费）</li>
     *   <li>重试次数 > {@code retryMax}：将消息写入死信队列 Stream（{@code dlqChannel}），
     *       并 ACK 原消息，防止 PEL 无限堆积</li>
     * </ul>
     *
     ...
     *
     * <p>用于重试场景（写入原 Stream）和死信场景（写入 DLQ Stream）。
     * 写入的字段包括 payload、retryCount，供消费端解析恢复重试状态。
     *
     ...
     * <p>解析顺序：
     * <ol>
     *   <li>从 {@code payload} 字段反序列化消息体（JSON）</li>
     *   <li>恢复 {@code retryCount} 重试计数</li>
     *   <li>恢复 {@code groupKey} + {@code sequence} 顺序消息属性</li>
     * </ol>
     *
     ...
     * 确保消费组存在。
     *
     * <p>调用 <b>XGROUP CREATE</b> 创建消费组。如果组已存在，Redis 返回 BUSYGROUP 错误，
     * 此处静默忽略以保证幂等启动。其他异常则向上抛出。
     */
