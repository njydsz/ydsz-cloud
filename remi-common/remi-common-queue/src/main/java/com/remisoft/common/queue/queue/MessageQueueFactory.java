...
import com.remisoft.common.queue.mq.active.ActiveMQ;
import com.remisoft.common.queue.mq.active.ActiveMQProperties;
import com.remisoft.common.queue.mq.kafka.KafkaMQ;
import com.remisoft.common.queue.mq.kafka.KafkaQueueProperties;
import com.remisoft.common.queue.mq.rabbit.RabbitMQ;
import com.remisoft.common.queue.mq.rabbit.RabbitMQProperties;
import com.remisoft.common.queue.mq.rocket.RocketMQ;
import com.remisoft.common.queue.mq.rocket.RocketMQProperties;
 ...
 *
 * <p><b>Redis 连接复用：</b>
 * Redis 队列实例复用 remi-common-redis 的连接，由 {@link RedisService} 统一管理。
 *
 * <p><b>线程池复用：</b>
...
public class MessageQueueFactory implements IMessageQueueProvider {
    ...
    private IMessageQueue createRedisListMQ() {
        if (redisService == null) {
            throw BusinessException.builder().key("使用 Redis 队列需引入 remi-common-redis 模块").build();
        }
        log.info("[Factory] 创建 Redis List 队列（复用 remi-common-redis 连接）");
    ...
    private IMessageQueue createRedisPubSubMQ() {
        ...
        log.info("[Factory] 创建 Redis PubSub 队列（复用 remi-common-redis 连接）");
    ...
    private IMessageQueue createRedisStreamMQ() {
        ...
        log.info("[Factory] 创建 Redis Stream 队列（复用 remi-common-redis 连接）");
