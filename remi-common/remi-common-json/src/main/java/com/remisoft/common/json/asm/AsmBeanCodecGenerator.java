package com.remisoft.common.json.asm;
...
import com.remisoft.common.json.annotation.JsonAlias;
import com.remisoft.common.json.annotation.JsonFormat;
import com.remisoft.common.json.annotation.JsonIgnore;
import com.remisoft.common.json.annotation.JsonIgnoreProperties;
...
import com.remisoft.common.json.annotation.JsonRootName;
import com.remisoft.common.json.annotation.JsonUnwrapped;
...
public final class AsmBeanCodecGenerator {
    ...
    public static <T> Class<? extends AsmSerializer<T>> generateSerializer(Class<T> beanType) throws Exception {
        ...
        cw.visit(V1_8, ACC_PUBLIC | ACC_FINAL, classInternalName, null,
                 "java/lang/Object", new String[]{"com/remisoft/common/json/asm/AsmSerializer"});
        ...
        for (Field field : fields) {
            ...
            if (typeCode == 13) {
                ...
                cw.visitField(ACC_PRIVATE, cachedFieldName, 
                    "Lcom/remisoft/common/json/asm/AsmSerializer;", null, null).visitEnd();
        ...
        for (FieldCacheInfo info : cachedFields) {
            ...
            mv.visitMethodInsn(INVOKESTATIC, "com/remisoft/common/json/asm/AsmBeanCodecGenerator", 
                "safeGetSerializer", "(Ljava/lang/Class;)Lcom/remisoft/common/json/asm/AsmSerializer;", false);
            mv.visitFieldInsn(PUTFIELD, classInternalName, info.fieldName, 
                "Lcom/remisoft/common/json/asm/AsmSerializer;");
        }
        ...
        mv = cw.visitMethod(ACC_PUBLIC, "serialize", 
                           "(Ljava/lang/Object;Lcom/remisoft/common/json/writer/JSONWriter;)V", null, null);
        mv.visitCode();
        ...
        mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/writer/JSONWriter", "preAllocate", "(I)V", false);

        // char[] buf = writer.buf — 直接字段访问，消除 getBuffer() 方法调用开销
        mv.visitVarInsn(ALOAD, 2);
        mv.visitFieldInsn(GETFIELD, "com/remisoft/common/json/writer/JSONWriter", "buf", "[C");
        mv.visitVarInsn(ASTORE, 3);

        // int pos = writer.pos — 直接字段访问，消除 getPosition() 方法调用开销
        mv.visitVarInsn(ALOAD, 2);
        mv.visitFieldInsn(GETFIELD, "com/remisoft/common/json/writer/JSONWriter", "pos", "I");
        ...
        mv.visitFieldInsn(PUTFIELD, "com/remisoft/common/json/writer/JSONWriter", "pos", "I");
        ...
        mvInline.visitFieldInsn(GETFIELD, "com/remisoft/common/json/writer/JSONWriter", "buf", "[C");
        mvInline.visitVarInsn(ASTORE, 3);

        // int pos = writer.pos
        mvInline.visitVarInsn(ALOAD, 2);
        mvInline.visitFieldInsn(GETFIELD, "com/remisoft/common/json/writer/JSONWriter", "pos", "I");
        ...
        mvInline.visitFieldInsn(PUTFIELD, "com/remisoft/common/json/writer/JSONWriter", "pos", "I");
    ...
    private static void emitFieldSerializationLoop(MethodVisitor mv, Field[] fields,
            ...
            for (int i = 0; i < fields.length; i++) {
                ...
                if (typeCode == 1) {
                    ...
                    mv.visitMethodInsn(INVOKESTATIC, "com/remisoft/common/json/writer/JSONWriter",
                    ...
                    mv.visitVarInsn(ALOAD, 5);
                    mv.visitVarInsn(ILOAD, 4);
                    mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/writer/JSONWriter",
                    ...
                    mv.visitVarInsn(ILOAD, 4);
                    mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/writer/JSONWriter",
                        "writeCollectionWithSerializerToBuf", "(Ljava/util/Collection;Lcom/remisoft/common/json/asm/AsmSerializer;I)I", false);
                    mv.visitVarInsn(ISTORE, 4);
                    emitReadBufFromWriter(mv);
                    ...
                    mv.visitVarInsn(ILOAD, 4);
                    mv.visitMethodInsn(INVOKESTATIC, "com/remisoft/common/json/asm/AsmBeanCodecGenerator",
                        "serializeNestedToBuf", "(Lcom/remisoft/common/json/writer/JSONWriter;Ljava/lang/Object;I)I", false);
                    mv.visitVarInsn(ISTORE, 4);
                    emitReadBufFromWriter(mv);
                    mv.visitJumpInsn(GOTO, endNull);
                    ...
                    mv.visitVarInsn(ILOAD, 4);
                    mv.visitMethodInsn(INVOKESTATIC, "com/remisoft/common/json/asm/AsmBeanCodecGenerator",
                        "serializeWithSerializerToBuf", "(Lcom/remisoft/common/json/asm/AsmSerializer;Ljava/lang/Object;Lcom/remisoft/common/json/writer/JSONWriter;I)I", false);
                    mv.visitVarInsn(ISTORE, 4);
                    emitReadBufFromWriter(mv);
    ...
    private static void emitWriteIntDirect(MethodVisitor mv) {
        ...
        mv.visitMethodInsn(INVOKESTATIC, "com/remisoft/common/json/number/NumberUtils", "writeInt", "(I[CI)I", false);
        mv.visitInsn(IADD);
    ...
    private static void emitWriteLongDirect(MethodVisitor mv) {
        ...
        mv.visitMethodInsn(INVOKESTATIC, "com/remisoft/common/json/number/NumberUtils", "writeLong", "(J[CI)I", false);
        mv.visitInsn(IADD);
        mv.visitVarInsn(ISTORE, 4);    // pos = pos + result
    }
    ...
    public static <T> Class<? extends AsmDeserializer<T>> generateDeserializer(Class<T> beanType) throws Exception {
        ...
        cw.visit(V1_8, ACC_PUBLIC | ACC_FINAL, classInternalName, null,
                 "java/lang/Object", new String[]{"com/remisoft/common/json/asm/AsmDeserializer"});
        ...
        for (Field field : fields) {
            ...
            if (typeCode == 13) {
                ...
                cw.visitField(ACC_PRIVATE, cachedFieldName, 
                    "Lcom/remisoft/common/json/asm/AsmDeserializer;", null, null).visitEnd();
        ...
        for (FieldCacheInfo info : cachedDeserFields) {
            ...
            mv.visitLdcInsn(Type.getType(getTypeDescriptorForClass(info.cachedType)));
            mv.visitMethodInsn(INVOKESTATIC, "com/remisoft/common/json/asm/AsmBeanCodecGenerator", 
                "safeGetDeserializer", "(Ljava/lang/Class;)Lcom/remisoft/common/json/asm/AsmDeserializer;", false);
            mv.visitFieldInsn(PUTFIELD, classInternalName, info.fieldName, 
                "Lcom/remisoft/common/json/asm/AsmDeserializer;");
        }
        ...
        mv = cw.visitMethod(ACC_PUBLIC, "deserialize", 
                           "(Lcom/remisoft/common/json/reader/JSONReader;)Ljava/lang/Object;", null, null);
        mv.visitCode();
        ...
        mv.visitVarInsn(ALOAD, 1);
        mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "skipWhitespace", "()V", false);
        mv.visitVarInsn(ALOAD, 1);
        mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "readChar", "()C", false);
        mv.visitInsn(POP);
        ...
        mv.visitVarInsn(ALOAD, 1);
        mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "readFieldNameHash", "()J", false);
        ...
        for (Field field : fields) {
            ...
            if (typeCode == 1) {
                mv.visitVarInsn(ALOAD, 2);
                mv.visitVarInsn(ALOAD, 1);
                mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "readStringDirect", "()Ljava/lang/String;", false);
                ...
                if (field.getType() == Integer.class) {
                    mv.visitVarInsn(ALOAD, 2);
                    mv.visitVarInsn(ALOAD, 1);
                    mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "readInt", "()I", false);
                ...
                if (field.getType() == Long.class) {
                    mv.visitVarInsn(ALOAD, 2);
                    mv.visitVarInsn(ALOAD, 1);
                    mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "readLong", "()J", false);
                ...
                if (field.getType() == Double.class) {
                    mv.visitVarInsn(ALOAD, 2);
                    mv.visitVarInsn(ALOAD, 1);
                    mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "readDouble", "()D", false);
                ...
                if (field.getType() == Float.class) {
                    mv.visitVarInsn(ALOAD, 2);
                    mv.visitVarInsn(ALOAD, 1);
                    mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "readFloat", "()F", false);
                ...
                if (field.getType() == Boolean.class) {
                    mv.visitVarInsn(ALOAD, 2);
                    mv.visitVarInsn(ALOAD, 1);
                    mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "readBoolean", "()Z", false);
                ...
                mv.visitMethodInsn(INVOKESTATIC, "com/remisoft/common/json/asm/AsmBeanCodecGenerator", 
                    ...
                    "(Lcom/remisoft/common/json/reader/JSONReader;Ljava/lang/Class;Lcom/remisoft/common/json/asm/AsmDeserializer;)Ljava/util/List;", false);
                mv.visitMethodInsn(INVOKEVIRTUAL, beanInternalName, setterName, "(Ljava/util/List;)V", false);

            } else if (typeCode == 14) {
                mv.visitVarInsn(ALOAD, 2);
                mv.visitVarInsn(ALOAD, 1);
                mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "readObjectMap", "()Ljava/util/Map;", false);
                ...
                mv.visitMethodInsn(INVOKESTATIC, "com/remisoft/common/json/asm/AsmBeanCodecGenerator", 
                    "deserializeNestedField", "(Lcom/remisoft/common/json/reader/JSONReader;Ljava/lang/Class;)Ljava/lang/Object;", false);
                ...
                mv.visitFieldInsn(GETFIELD, classInternalName, cachedDeserFieldName, 
                    "Lcom/remisoft/common/json/asm/AsmDeserializer;");
                mv.visitVarInsn(ALOAD, 1);
                mv.visitMethodInsn(INVOKEINTERFACE, "com/remisoft/common/json/asm/AsmDeserializer",
                    "deserialize", "(Lcom/remisoft/common/json/reader/JSONReader;)Ljava/lang/Object;", true);
                mv.visitTypeInsn(CHECKCAST, nestedTypeInternalName);
                mv.visitMethodInsn(INVOKEVIRTUAL, beanInternalName, setterName, 
                    "(" + getTypeDescriptor(nestedType) + ")V", false);
        ...
        mv.visitVarInsn(ALOAD, 1);
        mv.visitMethodInsn(INVOKEVIRTUAL, "com/remisoft/common/json/reader/JSONReader", "skipValue", "()V", false);
        mv.visitJumpInsn(GOTO, loopStart);
     ...
     * 检查是否为简单类型
     *
     * <p>委托给 {@link com.remisoft.common.json.util.JsonTypeUtils} 统一实现。</p>
     */
