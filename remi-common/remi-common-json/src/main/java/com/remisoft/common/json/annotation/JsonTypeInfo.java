 ...
 *
 * // 反序列化自动识别类型
 * Animal animal = RemiJson.fromJson(json, Animal.class);
 * // 返回 Dog 实例
 * </pre>
 *
 * @author remi-team
...
public @interface JsonTypeInfo {
     ...
     *
     * <p>仅在 {@link #include()} 为 {@link As#PROPERTY} 时生效。</p>
     *
     * @return 类型标识属性名
     */
     ...
     *
     * <p>控制类型信息以何种形式写入 JSON：</p>
     * <ul>
     *   <li>{@link Id#NAME}：使用子类型逻辑名（配合 {@link JsonTypeName} / {@link JsonSubType#name()}），默认</li>
     *   <li>{@link Id#CLASS}：使用 Java 完整类名</li>
     *   <li>{@link Id#MINIMAL_CLASS}：使用相对类名（截断基类包名前缀）</li>
     *   <li>{@link Id#NONE}：不写入类型信息</li>
     * </ul>
     *
     * @return 类型标识方式
     */
     ...
     *
     * <p>控制类型信息在 JSON 中的物理结构：</p>
     * <ul>
     *   <li>{@link As#PROPERTY}：作为对象的一个属性（由 {@link #property()} 指定键名），默认</li>
     *   <li>{@link As#WRAPPER_ARRAY}：以 {@code ["子类型名",{...}]} 包装数组形式</li>
     *   <li>{@link As#WRAPPER_OBJECT}：以 {@code {"子类型名":{...}}} 包装对象形式</li>
     * </ul>
     *
     * @return 包含方式
     */
