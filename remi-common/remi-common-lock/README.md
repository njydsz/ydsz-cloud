...
> 读写锁与信号量自 v1.0.0 起实现 `DistributedLocker` 接口，可纳入 `LockStrategy` 统一管理。两者均使用 `remi-common-cache` 替代 `ThreadLocal` 存储线程持有状态，通过 TTL 与最大容量自动清理，避免线程池复用场景下的内存泄漏。
...
**Redis Key 格式：** `remi:repeat:token:{userId}:{token}`，Token 与登录用户绑定，防止被盗用。
...
**降级策略：** 当 `LockStrategy` Bean 不存在（单节点 / 测试环境未装配 remi-common-lock）时，直接执行任务不加锁，保证功能可用。

**锁 Key 前缀：** `remi:schedule:`，与业务锁隔离。
...
<dependency>
    ...
    <artifactId>remi-common-lock</artifactId>
</dependency>
```

Web 场景下若需启用 `RepeatSubmitTokenController` 提供 Token 接口，消费方需自行引入 `spring-boot-starter-web` 与 `remi-common-auth`（Token 与登录用户绑定）。
...
`DistributedLockAutoConfiguration` 在 `StringRedisTemplate` 可用且 `remi.lock.enabled=true`（默认 true）时激活，自动注册以下 Bean：
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.lock.enabled` | `true` | 模块总开关 |
| `remi.lock.fallback-enabled` | `true` | Redis 不可用时降级为本地 `ReentrantLock` |
| `remi.lock.watchdog-enabled` | `true` | 看门狗自动续期开关 |
| `remi.lock.max-renew-times` | `100` | 看门狗最大续期次数（约 30 分钟） |
| `remi.lock.scheduler-pool-size` | `2` | 续期调度线程池大小 |
| `remi.lock.namespace` | - | 锁键命名空间前缀（多应用共享 Redis 时隔离） |
| `remi.lock.default-lock-timeout-seconds` | `30` | 默认锁超时时间（秒） |
| `remi.lock.acquire-pool.core-size` | `4` | 锁获取线程池核心线程数 |
| `remi.lock.acquire-pool.max-size` | `32` | 锁获取线程池最大线程数 |
| `remi.lock.acquire-pool.queue-capacity` | `256` | 锁获取线程池队列容量 |
| `remi.lock.idempotent.default-ttl-seconds` | `5` | 幂等锁默认过期时间（秒） |
| `remi.lock.idempotent.key-prefix` | `remi:idem:` | 幂等键 Redis 前缀 |
| `remi.lock.multi-lock.max-renew-count` | `30` | 多 Key 联锁最大续期次数 |
| `remi.lock.multi-lock.renew-interval-seconds` | `10` | 多 Key 联锁续期间隔（秒） |
...
```java
import java.util.concurrent.TimeUnit;
import com.remisoft.common.lock.RedisReadWriteLock;
import com.remisoft.common.lock.strategy.LockStrategy;
...
```java
import java.util.concurrent.TimeUnit;
import com.remisoft.common.lock.RedisSemaphore;
import com.remisoft.common.lock.strategy.LockStrategy;
...
8. **版本兼容**：配置前缀统一为 `remi.lock`，历史版本曾使用 `remi.distributed-lock` 前缀已废弃，不再支持。
