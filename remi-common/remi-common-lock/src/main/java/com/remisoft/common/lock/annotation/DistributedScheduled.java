 ...
 * }</pre>
 *
 * <p><b>降级策略：</b>当 {@code LockStrategy} Bean 不存在（单节点/测试环境未装配 remi-common-lock）时，
 * 直接执行任务不做加锁，保证功能可用。
 *
 * @author remi-team
 * @since 1.1.0
...
public @interface DistributedScheduled {
    ...
    /**
     * 锁的 key（不含前缀，自动添加 {@code remi:schedule:} 前缀）
     *
     * <p>支持 SpEL 表达式，如 {@code "message:#{#param}"}。
     * 大多数定时任务无入参，直接使用静态字符串即可。
     *
     * @return 锁的 key
     */
    String lockKey();
     ...
     *
     * <p>默认 300 秒（5 分钟），覆盖大多数定时任务场景。
     * 超过此时间锁自动释放，防止节点宕机导致死锁。
     *
     * @return 锁持有时间（秒）
     */
