...
import com.remisoft.common.lock.exception.IdempotentException;
 ...
 * <h3>Redis 键命名空间</h3>
 * <ul>
 *   <li>前缀：从 {@code remi.lock.idempotent.key-prefix} 读取（默认 {@code remi:idem:}）</li>
 *   <li>可选命名空间：{@code ${remi.lock.namespace}:}（多服务共享 Redis 时隔离）</li>
...
public class IdempotentAspect {
    ...
    /** 可选命名空间（多服务共享 Redis 时隔离键），来自 remi.lock.namespace */
     ...
     * @param keyPrefix          幂等键 Redis 前缀（从 remi.lock.idempotent.key-prefix 读取）
    ...
    public IdempotentAspect(IdempotentStrategy idempotentStrategy, String keyPrefix,
        ...
        this.keyPrefix = keyPrefix != null && !keyPrefix.isEmpty() ? keyPrefix : "remi:idem:";
