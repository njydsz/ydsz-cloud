package com.remisoft.common.event.processor;
...
public class OutboxProcessor {
    ...
    public OutboxProcessor(OutboxRepository outboxRepository,
        ...
        if (meterRegistry != null) {
            this.publishSuccessCounter = Counter.builder("remi.outbox.publish.success")
                    .description("Outbox messages published successfully")
                    .register(meterRegistry);
            this.publishFailureCounter = Counter.builder("remi.outbox.publish.failure")
                    .description("Outbox messages failed to publish")
                    .register(meterRegistry);
            this.deadLetterCounter = Counter.builder("remi.outbox.dead_letter")
                    .description("Outbox messages moved to dead letter")
                    .register(meterRegistry);
            this.singlePublishTimer = Timer.builder("remi.outbox.publish.single.duration")
                    .description("Outbox single message publish duration")
                    .publishPercentiles(0.5, 0.9, 0.99)
                    .publishPercentileHistogram()
                    .register(meterRegistry);
            this.batchPublishTimer = Timer.builder("remi.outbox.publish.batch.duration")
            ...
            for (OutboxStatus status : OutboxStatus.values()) {
                Gauge.builder("remi.outbox.queue.size", () -> getCachedCount(status))
