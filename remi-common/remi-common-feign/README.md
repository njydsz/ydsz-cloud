...
> REMI OpenFeign 企业级增强框架（L5 业务服务层）

提供统一编解码、ResponseUnwrapDecoder 自动解包、Resilience4j 熔断降级、舱壁隔离、链路追踪传播、动态客户端工厂、Gzip 压缩、Micrometer 监控、跨服务名称富化等能力，是 REMI 项目所有微服务间 Feign 调用的统一基座。
...
<dependency>
    ...
    <artifactId>remi-common-feign</artifactId>
</dependency>
```
...
### per-client 超时（`remi.feign.client-timeouts`）
...
### Resilience4j 熔断器（`remi.feign.circuit-breaker`）
...
### 响应拦截器（`remi.feign.response-interceptor`）
...
### 跨服务名称富化（`remi.feign.name-assembler`）
...
```java
import com.remisoft.common.feign.fallback.DefaultFallbackFactory;
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/feign` | Feign 模块健康检查：熔断器启用状态、重试策略、连接 / 读取超时、熔断器策略名称 | `remi.feign.enabled=true`（默认 true）+ classpath 存在 `HealthIndicator` |
...
1. **自动配置加载顺序**：`FeignConfiguration` → `FeignMetricsConfiguration` → `Resilience4jFeignConfiguration`（after=FeignConfiguration）→ `NameAssemblerAutoConfiguration`。Resilience4j 熔断器需显式 `remi.feign.circuit-breaker.enabled=true` 启用。
2. **请求头透传**：默认透传 16 个请求头（X-Access-Token / X-Tenant-Id / X-User-Language / X-Company-Ids / X-Dept-Ids / X-Data-Scope 等），可通过 `remi.feign.propagation.headers` 自定义。
