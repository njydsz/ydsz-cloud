 ...
 * <p><b>注入的请求头：</b>
 * <ul>
 *   <li>{@code X-Trace-Id} - 追踪唯一标识（兼容旧系统）</li>
 *   <li>{@code X-Span-Id} - Span 唯一标识</li>
 *   <li>{@code X-Parent-Span-Id} - 父 Span 标识</li>
 *   <li>{@code traceparent} - W3C TraceContext 标准头（格式: 00-{traceId}-{spanId}-{flags}）</li>
 * </ul>
 *
 * <p><b>追踪策略优先级：</b>
 * <ol>
 *   <li>当 SkyWalking agent 在 classpath 中时，使用 SkyWalking 的 traceId/spanId</li>
 *   <li>当 W3C traceparent 存在于请求头中时，延续该 trace 链路</li>
...
public class TraceRequestInterceptor implements RequestInterceptor {
    ...
    private String buildTraceparent(String traceId, String spanId) {
        ...
        return TRACEPARENT_VERSION + "-" + normalizedTraceId + "-" + normalizedSpanId + "-" + TRACEPARENT_FLAGS_SAMPLED;
    }
