# remi-common-exception

> 统一异常处理框架（L3 基础服务层）

提供 REMI 统一异常体系、错误码注册中心、RFC 7807 ProblemDetail、国际化 i18n、全局异常处理器（MVC / WebFlux / JDBC / Validation）、异常构建器、Micrometer 指标监控、Actuator 健康检查与文档端点等开箱即用能力，是所有业务模块异常处理的统一基座。
...
<dependency>
    ...
    <artifactId>remi-common-exception</artifactId>
</dependency>
```
...
### ExceptionProperties（`remi.exception.*`）

| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.exception.metrics-enabled` | true | 是否启用异常指标统计 |
| `remi.exception.global-handler-enabled` | true | 是否启用全局异常处理器 |
| `remi.exception.response-format` | `base-response` | 响应格式（`base-response` / `problem-detail`） |
| `remi.exception.include-stack-trace` | false | 是否在响应中包含堆栈信息（dev/test profile 自动开启） |
| `remi.exception.doc-endpoint-enabled` | true | 是否启用错误码文档端点 |
| `remi.exception.problem-detail-type-base-url` | `about:blank` | ProblemDetail type URI 基础 URL（RFC 7807） |
| `remi.exception.metrics-include-code-tag` | false | 是否在指标中包含高基数 code tag（默认关闭防止 Prometheus 指标爆炸） |

### I18nProperties（`remi.i18n.*`）

| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.i18n.basename` | `classpath:i18n/messages` | 资源文件基路径（支持逗号分隔多 basename） |
| `remi.i18n.encoding` | `UTF-8` | 资源文件编码 |
| `remi.i18n.dev-cache-seconds` | 0 | 开发环境缓存刷新间隔（秒），0 表示不缓存 |
| `remi.i18n.prod-cache-seconds` | 3600 | 生产环境缓存刷新间隔（秒） |
| `remi.i18n.fallback-to-system-locale` | false | 是否回退到系统语言 |
| `remi.i18n.default-message` | `未找到对应的提示信息: {0}` | 找不到国际化消息时的默认提示 |
| `remi.i18n.supported-locales` | `[zh_CN, en_US, zh_TW]` | 支持的语言列表 |
| `remi.i18n.lang-param-name` | `lang` | 语言切换请求参数名 |
| `remi.i18n.i18n-base-names` | `i18n/messages` | 国际化资源文件基础名（备用，与 basename 合并加载） |
| `remi.i18n.validate-on-startup` | true | 是否启动时校验 i18n key 可解析（fail-fast） |
...
| `/actuator/exception-codes` | 错误码文档端点，输出全部已注册错误码及 i18n 消息 | `spring-boot-actuator` 在类路径 + `remi.exception.doc-endpoint-enabled=true` |
