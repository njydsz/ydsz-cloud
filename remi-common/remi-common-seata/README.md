...
| 类 | 说明 |
|---|---|
| `TccTransactionRecoveryScanner` | 定时扫描器，`@Scheduled(fixedDelayString="${remi.seata.recovery-scan-interval-ms:10000}")`；扫描 `TRIED` 状态超时分支，回调 `TccRecoveryHandler` 执行 Cancel；超时阈值由 `recovery-timeout-threshold-ms` 控制 |
...
| 类 | 说明 |
|---|---|
| `SeataAutoConfiguration` | Spring Boot 自动配置，`remi.seata.enabled=true`（默认）时装配；含 `SeataAtConfiguration` 内嵌配置类（Seata 在 classpath 时注册） |
| `SeataProperties` | 配置属性（`remi.seata.*`），含 `TccLogStoreType` 枚举（MEMORY / REDIS） |
...
<dependency>
    ...
    <artifactId>remi-common-seata</artifactId>
</dependency>
```
...
```java
import com.remisoft.common.seata.api.DistributedTransactionManager;
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.seata.enabled` | true | 是否启用分布式事务模块 |
| `remi.seata.default-type` | LOCAL | 默认事务类型（LOCAL / TCC / SEATA_AT / SAGA） |
| `remi.seata.seata-application-id` | `remi-app` | Seata 应用 ID |
| `remi.seata.seata-tx-service-group` | `remi-tx-group` | Seata 事务组 |
| `remi.seata.tcc-enabled` | true | 是否启用 TCC 模式 |
| `remi.seata.saga-enabled` | true | 是否启用 SAGA 模式 |
| `remi.seata.seata-at-enabled` | true | 是否启用 Seata AT 模式 |
| `remi.seata.tcc-retry-count` | 3 | TCC 补偿重试次数 |
| `remi.seata.tcc-retry-interval-ms` | 1000 | TCC 补偿重试间隔（毫秒） |
| `remi.seata.tcc-try-timeout-ms` | 60000 | TCC Try 阶段超时（毫秒，0=不限制） |
| `remi.seata.tcc-log-store` | memory | TCC 日志存储类型（memory / redis） |
| `remi.seata.tcc-log-redis-key-prefix` | `remi:tcc:log:` | Redis 日志 key 前缀（redis 模式生效） |
| `remi.seata.tcc-log-redis-retention-hours` | 24 | Redis 日志保留时长（小时，redis 模式生效） |
| `remi.seata.saga-max-retries` | 5 | SAGA 最大重试次数 |
| `remi.seata.saga-retry-interval-ms` | 2000 | SAGA 重试间隔（毫秒） |
| `remi.seata.saga-timeout-ms` | 300000 | SAGA 事务超时（毫秒，0=不限制） |
| `remi.seata.recovery-scan-interval-ms` | 10000 | 事务恢复扫描间隔（毫秒） |
| `remi.seata.recovery-timeout-threshold-ms` | 60000 | 事务超时判定阈值（毫秒） |
...
```java
import com.remisoft.common.seata.api.TccAction;
import com.remisoft.common.seata.api.TccContext;
import com.remisoft.common.seata.impl.TccTransactionManager;
...
remi:
  seata:
    ...
    seata-application-id: remi-order
    seata-tx-service-group: remi-tx-group
```
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/seata` | 分布式事务健康检查 | `spring-boot-health` 在 classpath 且 `remi.seata.enabled=true` |
...
2. **TCC 日志存储生产必选 Redis**：`InMemoryTccTransactionLogStore` 重启丢失，无法跨实例恢复；生产环境应设置 `remi.seata.tcc-log-store=redis`，所有参与方必须使用相同的 `tcc-log-redis-key-prefix`。
