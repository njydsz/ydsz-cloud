package com.remisoft.common.seata.config;
...
import com.remisoft.common.seata.impl.DefaultXidPropagator;
import com.remisoft.common.seata.impl.InMemoryTccTransactionLogStore;
import com.remisoft.common.seata.impl.LocalTransactionManager;
...
import com.remisoft.common.seata.impl.SeataTransactionManager;
import com.remisoft.common.seata.impl.TccTransactionManager;
import com.remisoft.common.seata.impl.TccTransactionRecoveryScanner;
import com.remisoft.common.seata.interceptor.FeignXidRequestInterceptor;
import com.remisoft.common.seata.interceptor.XidServletFilter;
import com.remisoft.common.seata.audit.TransactionAuditLogger;
import com.remisoft.common.seata.health.SeataHealthIndicator;
 ...
 * 当 {@code remi.seata.tcc-log-store=redis} 且类路径存在 {@code RedisTemplate} 时
...
@ConditionalOnProperty(prefix = "remi.seata", name = "enabled", havingValue = "true", matchIfMissing = true)
public class SeataAutoConfiguration {
     ...
     * TCC 事务日志存储（内存版，默认）
     *
     * <p>仅当 {@code remi.seata.tcc-log-store} 未设置为 {@code redis} 或
     * 类路径无 {@code RedisTemplate} 时注册。
     */
    @Bean
    @ConditionalOnMissingBean(TccTransactionLogStore.class)
    @ConditionalOnProperty(prefix = "remi.seata", name = "tcc-log-store",
     ...
     *
     * <p>当 {@code remi.seata.tcc-log-store=redis} 且类路径存在
     * {@code RedisTemplate} 时注册，支持跨服务事务状态共享与持久化。
     */
    @Bean
    @ConditionalOnMissingBean(TccTransactionLogStore.class)
    @ConditionalOnClass(name = "org.springframework.data.redis.core.RedisTemplate")
    @ConditionalOnProperty(prefix = "remi.seata", name = "tcc-log-store", havingValue = "redis")
    public TccTransactionLogStore redisTccTransactionLogStore(
            ObjectProvider<RedisTemplate<String, Object>> redisTemplateProvider,
            SeataProperties properties) {
        RedisTemplate<String, Object> redisTemplate = redisTemplateProvider.getIfAvailable();
        if (redisTemplate == null) {
            log.warn("remi.seata.tcc-log-store=redis but no RedisTemplate available, fallback to InMemory");
            return new InMemoryTccTransactionLogStore();
        }
     ...
     *
     * <p>可通过 {@code remi.seata.tcc-enabled=false} 关闭
     */
    @Bean
    @ConditionalOnMissingBean(TccTransactionManager.class)
    @ConditionalOnProperty(prefix = "remi.seata", name = "tcc-enabled", havingValue = "true", matchIfMissing = true)
     ...
     * 默认分布式事务管理器
     *
     * <p>根据 {@code remi.seata.default-type} 选择实现
     */
    @Bean
    ...
    public DistributedTransactionManager distributedTransactionManager(
        ...
        if (txManager == null) {
            throw new IllegalStateException(
                    ...
                    + "or set remi.seata.default-type=TCC to use TCC mode without DataSource.");
        }
     ...
     * SAGA 事务编排器（P0-3）
     *
     * <p>可通过 {@code remi.seata.saga-enabled=false} 关闭
     */
    @Bean
    @ConditionalOnMissingBean(SagaOrchestrator.class)
    @ConditionalOnProperty(prefix = "remi.seata", name = "saga-enabled", havingValue = "true", matchIfMissing = true)
    ...
    @ConditionalOnProperty(prefix = "remi.seata", name = "seata-at-enabled", havingValue = "true", matchIfMissing = true)
