 ...
 *
 * <p><b>注册方式</b>：通过 {@link com.remisoft.common.seata.config.SeataAutoConfiguration}
 * 在 {@code RedisTemplate} 可用时自动注册，可通过
 * {@code remi.seata.tcc-log-store=redis} 显式启用，{@code =memory} 回退到内存版。
 *
...
public class RedisTccTransactionLogStore implements TccTransactionLogStore {
     ...
     * @param keyPrefix     key 前缀，如 {@code "remi:tcc:log:"}
    ...
    public RedisTccTransactionLogStore(RedisTemplate<String, Object> redisTemplate,
        ...
        this.keyPrefix = (keyPrefix == null || keyPrefix.isBlank()) ? "remi:tcc:log:" : keyPrefix;
