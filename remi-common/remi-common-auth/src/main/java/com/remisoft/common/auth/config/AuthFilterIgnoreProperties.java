 ...
 * 认证过滤器忽略路径配置属性。
 *
 * <p>允许应用通过 {@code remi.auth.filter-ignore.*} 配置项，
 * 在运行时动态追加或覆盖认证过滤器忽略的 URL 和服务名称。</p>
 *
 * <p><b>默认值：</b>包含项目内置的常见静态资源 URL 和 web 模块名。
 * 当 {@link #isOverrideMode() overrideMode} 为 {@code false}（默认）,
 ...
 */
@ConfigurationProperties(prefix = "remi.auth.filter-ignore")
public class AuthFilterIgnoreProperties {

    /**
     * 默认的公共忽略 URL 模式列表。
     */
    private static final List<String> DEFAULT_COMMON_IGNORE_URLS = Arrays.asList(
            "/**/css/**",
            "/**/js/**",
            "/**/images/**",
            "/**/fonts/**",
     ...
     * 默认的安全排除 URL 列表。
     */
    private static final List<String> DEFAULT_SECURITY_EXCLUDE_URLS = Arrays.asList(
            "/login",
            "/auth/**",
            "/captcha/**"
    );
    ...
    public void setCommonIgnoreUrls(List<String> commonIgnoreUrls) {
        this.commonIgnoreUrls = commonIgnoreUrls != null
                ? commonIgnoreUrls
                : new ArrayList<>(DEFAULT_COMMON_IGNORE_URLS);
    }
    ...
    public void setSecurityExcludeUrls(List<String> securityExcludeUrls) {
        this.securityExcludeUrls = securityExcludeUrls != null
                ? securityExcludeUrls
