package com.remisoft.common.auth.annotation;
 ...
 * 菜单/按钮权限校验注解。
 *
 * <p>用于标注在 Controller 或具体接口方法上，校验用户是否拥有指定的菜单或按钮权限。
 * 支持菜单级别（MENU）和按钮级别（BUTTON）两种权限类型，支持通配符匹配。
 *
 * <p><b>校验链路：</b>
 * <ol>
 *   <li>从请求头 X-Access-Token 获取 token</li>
 *   <li>根据 token 从 Redis 获取用户信息（含 roleCode，支持多角色 CSV 格式）</li>
 *   <li>根据 roleCode 加载角色关联的菜单/按钮权限集合（来自 Redis role-menu-key）</li>
 *   <li>按注解配置的 mode（AND/OR）进行权限匹配</li>
 *   <li>超级管理员（配置在 ignoreRoles）直接放行</li>
 * </ol>
 *
 * <p><b>与 {@link AuthApiPermission} 的区别：</b>
 * <ul>
 *   <li>{@link AuthApiPermission}：校验接口级别的访问权限，权限码通常粒度较细</li>
 *   <li>{@link AuthMenuPermission}：校验菜单/按钮级别的操作权限，权限码通常与前端菜单按钮对应</li>
 *   <li>实际项目中可根据业务需求选择使用或组合使用</li>
 * </ul>
 *
 * <p><b>使用示例：</b>
 * <pre>
...
public @interface AuthMenuPermission {
     ...
     * 为空时表示不限制角色，但需要通过 {@link #permissionCodes()} 校验。
     *
     * @return 角色编码数组
     */
     ...
     * 权限码建议与前端菜单/按钮的权限标识保持一致，
     * 格式建议采用 {@code 领域:资源:操作} 的命名规范。
     *
     * @return 权限码数组
     */
     ...
     *
     * <ul>
     *   <li>{@link PermissionType#MENU}：菜单级别权限（默认）</li>
     *   <li>{@link PermissionType#BUTTON}：按钮级别权限</li>
     * </ul>
     *
     * @return 权限类型
     */
