...
REMI 认证与授权框架 — JWT Token 服务、RBAC 权限模型（菜单/按钮/API/行/列五级权限）、数据权限 `@DataScope`、多租户隔离、Token 黑名单（同步 + 响应式 + 布隆过滤器）、权限层级继承、权限预检、列权限签名、权限缓存热更新、Micrometer 指标、权限国际化、启动预热。
...
| 类 | 说明 |
|---|---|
| `RateLimiter` | 限流器（默认 60 秒内 100 次，`remi.auth.rate-limit-enabled=true` 激活） |
...
| 类 | 说明 |
|---|---|
| `AuthConfiguration` | 自动配置（`@AutoConfiguration`，`remi.auth.enabled=true` 激活，含 Redis 健康检查定时任务） |
...
<dependency>
    ...
    <artifactId>remi-common-auth</artifactId>
</dependency>
```
...
### remi.auth（`AuthProperties`）

| 配置 | 默认值 | 说明 |
|---|---|---|
| `enabled` | `true` | 是否启用 RBAC 权限校验（false 时跳过所有校验） |
| `wildcard-enabled` | `true` | 是否启用通配符权限匹配（`sys:user:*`） |
| `role-menu-key` | `remi-auth:role-menu:{}` | 角色-菜单/按钮权限 Redis Key 模板 |
| `role-api-key` | `remi-auth:role-api:{}` | 角色-API 权限 Redis Key 模板 |
| `role-row-key` | `remi-auth:role-row:{}` | 角色-行权限 Redis Key 模板 |
| `role-col-key` | `remi-auth:role-col:{}` | 角色-列权限 Redis Key 模板 |
...
### remi.auth.filter（`AuthFilterProperties`）
...
```java
import com.remisoft.common.auth.event.PermissionChangePublisher;
import com.remisoft.common.auth.event.PermissionChangedEvent;
...
```java
import com.remisoft.common.auth.precheck.PermissionCheckResult;
import com.remisoft.common.auth.precheck.PermissionPreChecker;
...
| 配置类 | 激活条件 |
|---|---|
| `AuthConfiguration` | `remi.auth.enabled=true`（默认激活，含 `@EnableScheduling`） |
...
| `JwtTokenService` | jjwt 在 classpath 且 `remi.auth.token.enabled=true` |
...
- 缓存框架：remi-common-cache（替代 Caffeine，本地降级缓存）
- Redis：remi-common-redis（权限存储 + 黑名单 + Pub/Sub）
- JSON：remi-common-json（权限树序列化）
- 脱敏：remi-common-safe（列脱敏上下文）
- JWT：jjwt（optional）
