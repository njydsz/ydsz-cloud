...
| `AppOpenApiConfiguration` | OpenAPI 配置（继承 `BaseOpenApiConfiguration`），标题=`REMI App API 文档`，描述带 HTML 样式 |
| `AppCorsProperties` | CORS 配置（继承 `BaseCorsProperties`），前缀 `remi.app.cors` |
| `AppTraceProperties` | Trace 配置（继承 `BaseTraceProperties`），前缀 `remi.app.trace` |
| `AppContentCacheProperties` | 请求体缓存配置（`remi.app.content-cache`），`@Validated` + `@Min(0)` 校验 `maxSize`，默认 2MB |
...
> **API 签名验证**和**安全响应头**由 `remi-common-safe` 模块统一提供，通过 `remi.safe.api-signature.enabled` 和 `remi.safe.security-headers.enabled` 控制启用，本模块不再重复注册。`AppMvcConfiguration` 通过 `@AutoConfigureBefore(SafeConfiguration.class)` 保证装配顺序。
...
| 类 | 说明 |
|---|---|
| `AppHealthIndicator` | App 模块健康检查指示器（`@ConditionalOnClass(HealthIndicator.class)` + `remi.app.enabled=true`），暴露 `/actuator/health/app` 端点；检测 API 签名验证配置状态（启用/密钥/容差/排除数）和指标采集状态；签名验证启用但密钥未配置时标记为 DOWN |
...
<dependency>
    ...
    <artifactId>remi-common-app</artifactId>
</dependency>
```
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.app.enabled` | true | 是否启用 app 模块 |
| `remi.app.content-cache.max-size` | `2097152`（2MB） | 请求体最大缓存字节数，`@Min(0)` 校验，超过此值的内容被截断丢弃 |
| `remi.app.cors.enabled` | true | 是否启用 CORS（继承 `BaseCorsProperties`） |
| `remi.app.cors.allow-credentials` | false | 是否允许发送 Cookie 等凭证 |
| `remi.app.cors.allowed-origin-patterns` | `[]` | 允许的跨域来源模式列表（推荐 `capacitor://localhost`、`http://localhost`） |
| `remi.app.cors.allowed-headers` | `["*"]` | 允许的 HTTP 请求头列表 |
| `remi.app.cors.allowed-methods` | `["*"]` | 允许的 HTTP 请求方法列表 |
| `remi.app.cors.exposed-headers` | `[]` | 允许暴露给客户端的响应头列表 |
| `remi.app.cors.max-age` | 3600 | 预检请求缓存时间（秒） |
| `remi.app.cors.path-pattern` | `/**` | CORS 配置生效的 URL 路径模式 |
| `remi.app.cors.order` | 0 | 过滤器注册顺序 |
| `remi.app.trace.enabled` | true | 是否启用链路追踪 |
| `remi.app.trace.response-header-enabled` | true | 是否在响应头输出请求 ID |
| `remi.app.trace.request-id-header-name` | `X-Request-Id` | 请求 ID 响应头名称 |
| `remi.app.trace.request-log-enabled` | true | 是否启用请求日志 |
| `remi.app.trace.log-level` | `INFO` | 日志级别（INFO/DEBUG） |
| `remi.app.trace.sampling-rate` | 1.0 | 日志采样率 [0.0, 1.0] |
| `remi.app.trace.slow-request-threshold` | 3000 | 慢请求阈值（ms） |
| `remi.safe.api-signature.enabled` | false | API 签名验证开关（由 safe 模块提供） |
| `remi.safe.api-signature.app-secret` | - | 签名密钥（建议 `${APP_SECRET}`） |
| `remi.safe.api-signature.timestamp-tolerance-seconds` | 300 | 时间戳容差（秒） |
| `remi.safe.api-signature.nonce-expire-seconds` | 600 | Nonce 过期时间（秒） |
| `remi.safe.api-signature.excludes` | `[]` | 签名验证排除路径（如 `/api/app/public/**`、`/api/app/login`） |
| `remi.safe.security-headers.enabled` | true | 安全响应头开关（由 safe 模块提供，默认启用） |
...
| `BaseOpenApiConfiguration` | `AppOpenApiConfiguration` | 标题=`REMI App API 文档` |
| `BaseCorsProperties` | `AppCorsProperties` | 前缀=`remi.app.cors` |
| `BaseTraceProperties` | `AppTraceProperties` | 前缀=`remi.app.trace` |
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/app` | App 模块健康指标，报告 API 签名配置状态、指标采集状态 | `spring-boot-health` 在类路径 + `remi.app.enabled=true` |
| `/actuator/health/safe` | 安全模块健康指标（由 `common-safe` 提供） | `spring-boot-health` 在类路径 + `remi.safe.enabled=true` |
...
3. **API 签名验证与安全响应头**：由 `remi-common-safe` 模块统一提供，本模块不再重复注册。`AppMvcConfiguration` 通过 `@AutoConfigureBefore(SafeConfiguration.class)` 保证装配顺序。Redis 连通性和安全能力清单由 `SafeHealthIndicator` 统一报告，`AppHealthIndicator` 仅关注 App 模块特有状态，避免重复检测。
...
- **v1.0.0**（2026-08-02）：按 remi-common-jdbc 9 章节标准重构 README；补全 Filter 链顺序表、SPI 扩展点（含继承扩展点）、Micrometer 指标列表、注意事项；统一版本号为 1.0.0
