...
> REMI 公共能力底座（L1-L6 分层公共依赖库，不独立部署）
...
```
L1 基础设施层  → remi-common-core
L2 工具模块层  → remi-common-util, remi-common-json
L3 基础服务层  → remi-common-domain, remi-common-exception
L4 基础数据层  → remi-common-jdbc, remi-common-redis, remi-common-lock,
                 remi-common-cache, remi-common-thread, remi-common-tenant
L5 业务服务层  → remi-common-auth, remi-common-safe, remi-common-feign,
                 remi-common-audit, remi-common-file, remi-common-notify,
                 remi-common-queue, remi-common-docs, remi-common-excel,
                 remi-common-netty, remi-common-socket,
                 remi-common-search, remi-common-event,
                 remi-common-config, remi-common-seata, remi-common-sentry
L6 应用层     → remi-common-base, remi-common-web, remi-common-app
```
...
| 层级 | 模块 | 职责 |
|---|---|---|
| L1 | [common-core](remi-common-core/README.md) | 统一响应/请求模型、TraceId、请求上下文、JobHandler、DAG、特性开关、重试模板、线程池监控 |
| L2 | [common-util](remi-common-util/README.md) | 99 个工具类（JSON/加密/HTTP/IP/Spring/雪花 ID/Bean 拷贝 等） |
| L2 | [common-json](remi-common-json/README.md) | 高性能 JSON 引擎（ASM 字节码、SIMD 向量化、Schema 校验、JsonPath、树模型） |
| L3 | [common-domain](remi-common-domain/README.md) | DDD 基类（BaseEntity/AggregateRoot）、领域事件、规范模式、分页、树形结构 |
...
| L5 | [common-auth](remi-common-auth/README.md) | JWT、RBAC 4 注解 + 3 切面、@DataScope 数据权限、TOTP 2FA、权限缓存热更新 |
| L5 | [common-safe](remi-common-safe/README.md) | @Sensitive 7 种脱敏、@Xss、@RateLimit、CSRF、SQL 注入防护、验证码、安全事件告警 |
| L5 | [common-feign](remi-common-feign/README.md) | OpenFeign 增强、统一编解码、ResponseUnwrapDecoder、Resilience4j 熔断、动态客户端 |
| L5 | [common-audit](remi-common-audit/README.md) | @OperationLog + @Audit、事件驱动异步落库、Disruptor 高性能批写、4 种分片策略 |
| L5 | [common-file](remi-common-file/README.md) | 7 种存储平台、分片上传、断点续传、文件去重（秒传）、文件类型安全检测 |
| L5 | [common-notify](remi-common-notify/README.md) | 5 种通知渠道（邮件/短信/企微/钉钉/飞书）、SpEL 模板引擎、重试队列、DKIM 签名 |
| L5 | [common-queue](remi-common-queue/README.md) | 5 种 MQ（Redis×3/Kafka/RocketMQ/RabbitMQ/ActiveMQ）、死信队列、消息轨迹、去重 |
| L5 | [common-docs](remi-common-docs/README.md) | 8 种格式解析、预处理 Pipeline、安全扫描、PII 检测（5 种）、文本水印、PDF 脱敏、OCR |
| L5 | [common-excel](remi-common-excel/README.md) | 高性能 Excel 读写（SAX 流式/SXSSF 大文件）、并发写入、模板填充、公式注入防护 |
| L5 | [common-netty](remi-common-netty/README.md) | Netty TCP Server/Client 抽象、断线重连、心跳检测、SSL/TLS、LengthField 编解码 |
| L5 | [common-socket](remi-common-socket/README.md) | WebSocket 实时推送、集群广播、离线消息存储、认证拦截、消息限流 |
| L5 | [common-search](remi-common-search/README.md) | 统一搜索引擎（PG 全文检索/ES）、多 Provider 架构、搜索缓存、蓝绿重建、游标分页 |
| L5 | [common-event](remi-common-event/README.md) | 事务性 Outbox 模式、可靠事件投递、Outbox 处理器、健康检查 |
| L5 | [common-config](remi-common-config/README.md) | 敏感配置加密（AES-256-GCM、SHA-256 密钥派生、ENC() 格式） |
| L5 | [common-seata](remi-common-seata/README.md) | Seata 分布式事务集成（AT/TCC/SAGA 模式） |
| L5 | [common-sentry](remi-common-sentry/README.md) | 统一系统指标监控（ELK+Logstash / Loki+Alloy 双方案、SLA、告警收敛、Grafana 仪表盘） |
| L6 | [common-base](remi-common-base/README.md) | HTTP 公共基座（CORS/时区/I18n/安全头/TraceId/请求日志/全局响应包装/OpenAPI） |
| L6 | [common-web](remi-common-web/README.md) | **PC Web 端基座**（继承 base，叠加 Spring Security/WebAuthFilter/Session） |
| L6 | [common-app](remi-common-app/README.md) | **移动端 App 基座**（继承 base，叠加 API 签名验证/AppAuthHandler） |
...
各服务的启动类通过 `@SpringBootApplication(scanBasePackages = {"com.remisoft.{service}", "com.remisoft.common"})` 扫描 common 包，激活自动配置。

> **例外**：`remi-gateway` 为 reactive 栈（WebFlux），**不依赖** `common-web`（servlet 栈），只挑选 `common-core` / `common-exception` / `common-auth` 三个细粒度子模块。

## SPI 扩展点速查表

remi-common 不使用 Dubbo `@SPI` 注解，所有扩展点通过三种 Spring 原生机制暴露：
...
| **`@ConditionalOnProperty` 切换** | 通过配置项值选择实现 | 后端切换（如 `remi.sentry.metrics.primary=micrometer` / `remi.seata.tcc-log-store=redis`） |
...
5. **配置切换**（仅 `@ConditionalOnProperty` 场景）：在 `application.yml` 中通过配置项值切换实现，如 `remi.seata.tcc-log-store=redis`。
6. **避免循环依赖**：扩展点实现不应反向依赖 remi-common 的 `@Configuration` 装配类（参见 [ArchitectureTest](remi-common-app/src/test/java/com/remisoft/common/app/ArchitectureTest.java)）。

## Nacos 共享配置

**位置**：`deploy/common/nacos/remi-common.yaml`

> ⚠️ 注意：共享配置模板**不在** `remi-common/src/main/resources/` 下（本模块是聚合 POM，无 `src` 目录）。实际位于 `deploy/common/nacos/`，通过导入脚本部署到 Nacos。

**部署**：
```bash
# Ubuntu
deploy/ubuntu/scripts/import-nacos-config.sh remi dev

# Windows
deploy\windows\scripts\import-nacos-config.bat remi dev
```
...
| `remi.jwt` / `remi.security` / `remi.kms` / `remi.sentry` | JWT、IP 白名单、KMS、Sentry |
| `jasypt.encryptor` | 配置加密 |

**单一来源原则**：所有 10 个部署单元的公共配置，**只能**在 `deploy/common/nacos/remi-common.yaml` 维护。各服务 `bootstrap.yml` 通过 `spring.cloud.nacos.config.shared-configs` 引用此 dataId，服务特有配置才写到 `remi-{service}-{env}.yaml`。

## 10 个部署单元清单

| 序号 | 服务名 | 端口 | 启动类 scanBasePackages | 依赖的 common 子模块 |
|---|---|---|---|---|
| 1 | remi-gateway | 9000 | `com.remisoft.gateway`（裸 @SpringBootApplication） | common-core / common-exception / common-auth |
| 2 | remi-userinfo | 9001 | userinfo + common | 全量（通过 common-web） |
| 3 | remi-system | 9002 | system + common | 全量（通过 common-web） |
| 4 | remi-project | 9003 | project + common + literule（含 sales/finance 合并域） | 全量（通过 common-web） |
| 5 | remi-message | 9004 | message + common | 全量（通过 common-web） |
| 6 | remi-cronjob | 9005 | cronjob + common | 全量（通过 common-web） |
| 7 | remi-workflow | 9006 | workflow + common | 全量（通过 common-web） |
| 8 | remi-agent | 9007 | agent + common | 全量（通过 common-web） |
| 9 | remi-nextwiki | 9008 | nextwiki + common | 全量（通过 common-web） |
| 10 | remi-literule | 9009 | literule + common | 全量（通过 common-web） |
...
├── remi-common-core/     # L1 基础设施层
├── remi-common-util/     # L2 工具模块层
├── remi-common-json/     # L2 工具模块层（高性能 JSON 引擎）
├── remi-common-domain/   # L3 基础服务层
├── remi-common-exception/# L3 基础服务层
├── remi-common-jdbc/     # L4 基础数据层
├── remi-common-redis/    # L4 基础数据层
├── remi-common-lock/     # L4 基础数据层
├── remi-common-cache/    # L4 基础数据层（高性能多策略本地缓存）
├── remi-common-thread/   # L4 基础数据层（共享线程池自动配置）
├── remi-common-tenant/   # L4 基础数据层（多租户隔离）
├── remi-common-auth/     # L5 业务服务层
├── remi-common-safe/     # L5 业务服务层
├── remi-common-feign/    # L5 业务服务层
├── remi-common-audit/    # L5 业务服务层
├── remi-common-file/     # L5 业务服务层
├── remi-common-notify/   # L5 业务服务层
├── remi-common-queue/    # L5 业务服务层
├── remi-common-docs/     # L5 业务服务层（文档解析/安全/PII/水印/脱敏/OCR）
├── remi-common-excel/    # L5 业务服务层（高性能 Excel 读写）
├── remi-common-netty/    # L5 业务服务层（Netty TCP 通信）
├── remi-common-socket/   # L5 业务服务层（WebSocket 实时推送）
├── remi-common-search/   # L5 业务服务层（统一搜索引擎）
├── remi-common-event/    # L5 业务服务层（事务性 Outbox）
├── remi-common-config/   # L5 业务服务层（敏感配置加密）
├── remi-common-seata/    # L5 业务服务层（Seata 分布式事务）
├── remi-common-sentry/   # L5 业务服务层（统一系统指标监控）
├── remi-common-base/     # L6 应用层（HTTP 公共基座）
├── remi-common-web/      # L6 应用层（PC Web 端基座）
└── remi-common-app/      # L6 应用层（移动端 App 基座）
```
