...
提供 8 种格式解析（PDF / Word / Excel / PPT / HTML / Markdown / TXT / CSV）、预处理 Pipeline、安全扫描（宏 / PDF JS / 嵌入对象）、PII 检测（5 种敏感信息）、文本水印、PDF 脱敏、异步解析、OCR 集成能力，是 REMI 项目文档内容处理的统一基座。
...
<dependency>
    ...
    <artifactId>remi-common-docs</artifactId>
</dependency>
```
...
```java
import com.remisoft.common.docs.service.DocumentService;
import com.remisoft.common.docs.domain.DocumentParseResult;
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.docs.enabled` | true | 是否启用文档处理模块 |
| `remi.docs.max-file-size-mb` | 50 | 文件大小上限（MB，1-500） |
| `remi.docs.parse-timeout-seconds` | 60 | 解析超时时间（秒，1-600） |
| `remi.docs.security-scan-enabled` | true | 安全扫描开关 |
| `remi.docs.pii-detection-enabled` | true | PII 检测开关 |
| `remi.docs.preprocess-enabled` | true | 预处理开关 |
| `remi.docs.watermark-enabled` | true | 水印开关 |
| `remi.docs.redact-enabled` | true | 脱敏开关 |
| `remi.docs.async-pool-size` | 4 | 异步解析线程池大小（1-64） |
| `remi.docs.async-queue-capacity` | 100 | 异步解析队列容量（1-10000） |
| `remi.docs.max-chunk-size` | 2000 | 文本分块最大 token 数（100-100000） |
| `remi.docs.chunk-overlap` | 200 | 文本分块重叠量（0-10000） |
| `remi.docs.security-max-scan-pages` | 50 | 安全扫描最大页数（0-500） |
| `remi.docs.block-on-high-risk` | false | 高风险时是否阻止解析 |
| `remi.docs.watermark-font-path` | - | 水印自定义字体路径（配置后优先使用） |
| `remi.docs.classifier-rules` | - | 文档分类规则（JSON 格式） |
...
```java
import com.remisoft.common.docs.service.AsyncDocumentParser;
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/docs` | 文档处理模块健康检查 | `spring-boot-health` 在类路径，`remi.docs.enabled=true` |
