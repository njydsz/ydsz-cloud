package com.remisoft.common.docs.security.pii;
...
public class PiiDetectorComposite {
    ...
    public PiiDetectorComposite(List<PiiDetector> detectors) {
        ...
        log.info("[PiiDetectorComposite] registered {} PII detectors: {}", this.detectors.size(),
                this.detectors.stream().map(d -> d.getSupportedType().name()).toList());
    }
