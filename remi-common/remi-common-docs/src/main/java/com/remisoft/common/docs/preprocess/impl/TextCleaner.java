...
public class TextCleaner implements DocumentPreprocessor {
     ...
     *
     * <p>按 BOM → 零宽字符 → 控制字符 → 页码 的固定次序替换。
     * <b>零宽字符必须先于其他步骤清除</b>：{@code U+200B~200D} 可被插入到敏感串中间
     * （如 {@code 138<ZWSP>0013<ZWSP>8000}），不先剥离就会让后续 PII 正则整体失配，
     * 这是一条典型的检测绕过路径。控制字符则会污染日志与下游存储。
     *
     * <p>页码行清理属于<b>启发式</b>，存在误删风险：正文中独占一行的纯数字
     * （如编号列表、数据表中孤立的一行数字）会被一并移除。
     * 权衡后仍保留该规则，因为 PDF 逐页抽取产生的页码噪声对检索质量影响更大。
     *
     * <p><b>只处理 text，不同步 sections：</b>与 {@link TextNormalizer} 不同，
     * 本处理器不修改分节内容，故清洗后全文与分节文本会出现不一致，
     ...
     * 声明在预处理流水线中的执行序号。
     *
     * <p>取 20，夹在归一化（10）与分块（30）之间：
     * 需先由归一化统一字符形态，本步的正则才有确定语义；
     * 又须早于分块，避免噪声字符被计入块长度、把页码噪声固化进各个分块。
     *
