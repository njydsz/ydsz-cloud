...
public class TextNormalizer implements DocumentPreprocessor {
     ...
     *
     * <p>作为流水线首棒（order=10）运行，为后续的清洗、PII 正则匹配提供稳定的字符基线。
     * NFKC 会把全角字母数字、连字、上下标等兼容字符折叠为标准形式，
     * 使 {@code １３８…} 这类全角数字也能被手机号正则命中——
     * 若不做这步，攻击者可用全角字符轻易绕过敏感信息检测。
     *
     * <p><b>会改变字符偏移量：</b>压缩空格与空行后，文本长度缩短，
     * 原有的下标位置全部失效。因此 PII 检测必须在本预处理<b>之后</b>执行，
     * 否则 {@code PiiFinding} 中记录的 startIndex/endIndex 会错位，导致脱敏切错位置。
     *
     * <p><b>就地修改入参：</b>直接改写传入对象的 text、totalChars 与各分节内容并返回同一引用，
     * 而非返回副本。调用方若需保留原始文本，必须自行在调用前深拷贝。
     ...
     * 声明在预处理流水线中的执行序号。
     *
     * <p>取 10 使其<b>先于</b>清洗器（20）与分块器（30）执行：
     * 字符形态必须先统一，后续基于正则的清洗才有确定的匹配基准，
     * 分块也才能在稳定的文本上计算长度与重叠。
     *
