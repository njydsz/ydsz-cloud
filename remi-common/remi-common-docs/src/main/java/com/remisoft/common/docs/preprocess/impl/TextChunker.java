package com.remisoft.common.docs.preprocess.impl;
...
import com.remisoft.common.docs.config.DocsProperties;

import com.remisoft.common.docs.domain.DocumentContent;
import com.remisoft.common.docs.domain.DocumentSection;
import com.remisoft.common.docs.preprocess.DocumentPreprocessor;
...
public class TextChunker implements DocumentPreprocessor {
     ...
     *
     * <p>短文本直通：全文长度不超过 {@code remi.docs.max-chunk-size}（默认 2000）时
     * 原样返回，不产生任何 chunk 分节。调用方不能假定处理后一定存在 chunk 类型分节。
     *
     * <p><b>追加而非替换：</b>原有分节全部保留，分块结果附加在后面。
     * 这意味着处理后 {@code sections} 中同一段文字会出现两次（原分节 + chunk），
     * 下游<b>必须按 {@code type} 字段过滤</b>，否则全文检索会产生重复命中、
     * 统计字数会翻倍。设计如此是为了让需要原始结构的消费方与需要等长块的
     * 向量化消费方共用同一份产物。
     *
     * <p>块间重叠 {@code remi.docs.chunk-overlap}（默认 200）个字符，
     * 目的是避免恰好跨块边界的语义被割裂，提升 RAG 召回率。
     * 所有块的 {@code pageNumber} 统一填 1，<b>不保留原始页码</b>，
     * 因此基于 chunk 无法回溯到具体页。
     *
     * <p>就地修改入参并返回同一引用，非线程安全。
     *
     * @param content 待分块的文档内容；为 {@code null} 或其 text 为 {@code null} 时原样返回，不抛异常
     * @return 与入参同一引用；超长时其 sections 已追加 chunk 分节，
     *         注意 text 与 totalChars <b>保持不变</b>
     */
    @Override
     ...
     * 声明在预处理流水线中的执行序号。
     *
     * <p>取 30，是三个内置处理器中<b>最后一棒</b>。分块必须在归一化（10）
     * 与清洗（20）之后：若先分块，噪声字符会占用块容量、页码噪声会被固化进块内，
     ...
     *
     * <p>分块质量的核心在于"在哪里断开"。策略是<b>逐级降级</b>：
     * 优先在双换行的段落边界断开（语义最完整），
     * 单段落自身超限时交给 {@link #splitBySentence} 按句号类标点断开，
     * 保证切口尽量落在自然停顿处而非词语中间。
     *
     * <p>每次封块后，会截取当前块末尾 {@code overlap} 个字符作为下一块的起始，
     * 使相邻块共享一段上下文。所有块输出前做 {@code strip()} 去除首尾空白。
     *
     * @param text         待分块文本，调用方已确保长度超过 maxChunkSize
     * @param maxChunkSize 单块字符上限，来自配置 {@code remi.docs.max-chunk-size}
     * @param overlap      相邻块重叠字符数，来自配置 {@code remi.docs.chunk-overlap}
     ...
     *
     * <p>使用零宽后顾断言 {@code (?<=[。．.！？!?\n])} 分割，
     * 使标点<b>保留在前一句末尾</b>而非被吞掉，避免还原文本时丢失标点。
     *
     * <p>兜底分支处理"一句话本身就超过块上限"的极端情况（如无标点的长串、
     * 代码块、Base64 数据）。此时按固定步长 {@code maxChunkSize - overlap} 硬切，
     * 步长与截取长度之差恰为 overlap，使定长切分同样保持块间重叠。
     * 步长通过 {@code Math.max(1, ...)} 兜底，防止 overlap 配置得大于等于
