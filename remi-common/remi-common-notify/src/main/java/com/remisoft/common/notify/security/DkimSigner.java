package com.remisoft.common.notify.security;
 ...
 * DKIM 邮件签名支持（P2-9）
 *
 * <p>DKIM（DomainKeys Identified Mail）通过 RSA 私钥对邮件头+正文进行签名，
 * 收件方通过 DNS 中的公钥记录验证签名，防止邮件被篡改或伪造。
 *
 * <p><b>签名流程：</b>
 * <ol>
 *   <li>选择需要签名的邮件头字段（From、To、Subject、Date 等）</li>
 *   <li>构建 DKIM-Signature 头的规范数据（domain、selector、算法等）</li>
 *   <li>对邮件头规范数据和正文规范数据进行 RSA-SHA256 签名</li>
 *   <li>将签名结果以 Base64 编码写入 DKIM-Signature 头</li>
 * </ol>
 *
 * <p><b>配置示例：</b>
 * <pre>{@code
		...
		try {
			...
			for (String headerName : SIGNED_HEADERS) {
				...
				if (values != null && values.length > 0) {
					signingData.append(headerName.toLowerCase()).append(":")
							.append(values[0].trim()).append("\r\n");
				}
			}

			// 添加 DKIM-Signature 头本身（部分字段）
			String dkimHeaderPartial = "v=1; a=rsa-sha256; c=" + CANONICALIZATION + "; d="
					+ dkim.getDomain() + "; s=" + dkim.getSelector()
					+ "; h=" + String.join(":", SIGNED_HEADERS) + ";";
			...
			log.debug("[DkimSigner] DKIM 签名生成成功: domain={}, selector={}",
					dkim.getDomain(), dkim.getSelector());
