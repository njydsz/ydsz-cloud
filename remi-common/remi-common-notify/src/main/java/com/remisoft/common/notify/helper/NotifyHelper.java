package com.remisoft.common.notify.helper;
...
public class NotifyHelper {
    ...
    public void sendInApp(String receiverId, String title, String content) {
        try {
            NotifySendResult result = notifyService.send(
                    NotifyChannel.INSITE, receiverId, title, content);
            if (!result.isSuccess()) {
                log.warn("[NotifyHelper] 站内信发送失败: receiver={}, reason={}",
                        receiverId, result.getErrorMessage());
            }
        } catch (Exception e) {
            log.warn("[NotifyHelper] 站内信发送异常: receiver={}, error={}",
    ...
    public void sendEmail(String emailAddress, String title, String content) {
        try {
            NotifySendResult result = notifyService.send(
                    NotifyChannel.EMAIL, emailAddress, title, content);
            if (!result.isSuccess()) {
                log.warn("[NotifyHelper] 邮件发送失败: receiver={}, reason={}",
                        emailAddress, result.getErrorMessage());
            }
        } catch (Exception e) {
     ...
     */
    public void sendTemplate(NotifyChannel channel, String receiver,
                             String templateCode, Object templateParams) {
        try {
            NotifySendResult result = notifyService.sendTemplate(
                    channel, receiver, templateCode, templateParams);
            if (!result.isSuccess()) {
                log.warn("[NotifyHelper] 模板通知发送失败: channel={}, receiver={}, template={}, reason={}",
                        channel, receiver, templateCode, result.getErrorMessage());
            }
        } catch (Exception e) {
            log.warn("[NotifyHelper] 模板通知发送异常: channel={}, receiver={}, template={}, error={}",
                    channel, receiver, templateCode, e.getMessage(), e);
        }
    }
     ...
     * 发送系统告警通知（站内信 + 邮件双发）。
     *
     * <p>系统级告警场景使用，确保关键信息送达。
     * 两种渠道独立发送，任一失败不影响另一渠道。
     *
     * @param title   告警标题
     * @param content 告警内容
    ...
    public void batchSendInApp(java.util.List<String> receiverIds, String title, String content) {
        ...
        try {
            NotifySendResult result = notifyService.batchSend(
                    NotifyChannel.INSITE, receiverIds, title, content);
            if (!result.isSuccess()) {
                log.warn("[NotifyHelper] 批量站内信发送失败: size={}, reason={}",
                        receiverIds.size(), result.getErrorMessage());
            }
        } catch (Exception e) {
            log.warn("[NotifyHelper] 批量站内信发送异常: size={}, error={}",
                    receiverIds.size(), e.getMessage(), e);
        }
    }
    ...
    public void send(NotifyRequest request) {
        try {
            ...
            if (!result.isSuccess()) {
                log.warn("[NotifyHelper] 通知发送失败: receiver={}, reason={}",
                        request.getReceiver(), result.getErrorMessage());
            }
        } catch (Exception e) {
            log.warn("[NotifyHelper] 通知发送异常: receiver={}, error={}",
