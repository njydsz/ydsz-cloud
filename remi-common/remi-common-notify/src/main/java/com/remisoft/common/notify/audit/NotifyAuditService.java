package com.remisoft.common.notify.audit;
...
import com.remisoft.common.notify.core.NotifyTraceContext;
import com.remisoft.common.notify.enums.NotifyChannel;

/**
 * 通知审计日志服务（P1-4）
 *
 * <p>记录每条通知发送的结构化审计日志，满足合规要求（如数据安全法、个人信息保护法）。
 * 审计日志独立于业务日志，通过专用 Logger 输出，便于日志采集和合规审计。
 *
 * <p><b>审计字段：</b>
 * <ul>
 *   <li>timestamp — 时间戳</li>
 ...
 *   <li>channel — 通知渠道</li>
 *   <li>receiver_mask — 脱敏后的接收者标识</li>
 *   <li>status — 发送状态（success/failure）</li>
 *   <li>messageId — 消息ID（成功时）</li>
 *   <li>error — 错误信息（失败时）</li>
 *   <li>durationMs — 发送耗时（毫秒）</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
...
public class NotifyAuditService {
     ...
     */
    public void audit(NotifyChannel channel, String receiver, String title,
        ...
        AUDIT_LOG.info(RemiJson.toJson(auditEntry));
    }
     ...
     * @param durationMs 耗时
     */
    public void audit(NotifyChannel channel, String receiver, NotifySendResult result, long durationMs) {
