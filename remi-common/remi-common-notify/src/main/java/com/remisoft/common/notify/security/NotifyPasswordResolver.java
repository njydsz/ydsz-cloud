 ...
 *
 * <p>当 EmailConfig.security.passwordEncrypted=true 时，自动通过 Jasypt 解密 SMTP 密码/授权码。
 * 密文格式为 {@code ENC(密文)}，解密密钥从 {@code security.jasyptKey} 或环境变量
...
public class NotifyPasswordResolver {
	...
	private String decrypt(String encryptedPassword) {
		String cipherText = encryptedPassword.substring(ENC_PREFIX.length(),
				encryptedPassword.length() - ENC_SUFFIX.length());
		try {
	...
	private PooledPBEStringEncryptor getEncryptor() {
		...
		synchronized (this) {
			...
			if (!StringUtils.hasText(key)) {
				log.warn("[NotifyPasswordResolver] Jasypt 密钥未配置（jasyptKey 或 JASYPT_ENCRYPTOR_PASSWORD），跳过解密");
				initialized = true;
				return null;
			}
