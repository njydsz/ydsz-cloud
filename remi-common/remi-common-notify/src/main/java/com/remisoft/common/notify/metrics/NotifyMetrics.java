package com.remisoft.common.notify.metrics;
 ...
 * <ul>
 *   <li>{@code notify_email_sent_total{channel,result,template}} — 发送总数（含成功/失败/模板标签）</li>
 *   <li>{@code notify_email_duration_seconds{channel,template}} — 发送耗时分布</li>
 *   <li>{@code notify_email_failed_total{channel,reason,error_type}} — 失败总数（含失败原因和错误类型标签）</li>
 *   <li>{@code notify_channel_sent_total{channel,result,template}} — 各渠道发送总数</li>
 *   <li>{@code notify_retry_total{channel,result}} — 重试总数</li>
 *   <li>{@code notify_circuit_breaker_state{channel,state}} — 熔断器状态</li>
 * </ul>
 *
...
public class NotifyMetrics {
	 ...
	 */
	public void recordEmailSend(String channel, boolean success, Duration duration, String templateCode) {
	...
	public void recordEmailFailure(String channel, String reason, String errorType) {
		...
		Counter counter = counterCache.computeIfAbsent(
				METRIC_EMAIL_FAILED + "_" + channel + "_" + reason + "_" + (errorType != null ? errorType : "unknown"),
	...
	public void recordRetry(String channel, boolean success) {
		...
		Counter counter = counterCache.computeIfAbsent(
				...
				k -> Counter.builder(METRIC_RETRY)
						.tag("channel", channel)
						.tag("result", result)
						.register(meterRegistry));
		counter.increment();
