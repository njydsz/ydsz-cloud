 ...
 *
 * <p>定时探测 SMTP 服务连通性，预热连接池，避免首次发送时的连接延迟。
 * 探活失败时记录告警日志，并可通过 {@link #isHealthy()} 查询当前健康状态。
 *
 * <p>探活策略：
 * <ul>
 *   <li>每 60 秒执行一次 SMTP NOOP 探活</li>
 *   <li>启动时立即执行一次预热连接</li>
 *   <li>连续 3 次探活失败标记为不健康</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
...
public class EmailSmtpHealthChecker {
	...
	private boolean probeSmtp(NotifyProperties.EmailConfig email) {
		...
		try {
			...
			log.debug("[EmailSmtpHealthChecker] SMTP 探测失败: host={}, port={}, error={}",
					email.getSmtpHost(), email.getSmtpPort(), e.getMessage());
			return false;
		}
	}
	...
	private void onFailure(NotifyProperties.EmailConfig email) {
		...
		if (failureStreak >= MAX_FAILURE_STREAK) {
			healthy = false;
			log.error("[EmailSmtpHealthChecker] SMTP 连续 {} 次探活失败，标记为不健康, host={}, port={}",
					failureStreak, email.getSmtpHost(), email.getSmtpPort());
		} else {
			log.warn("[EmailSmtpHealthChecker] SMTP 探活失败 ({}/{}), host={}",
					failureStreak, MAX_FAILURE_STREAK, email.getSmtpHost());
		}
	}
