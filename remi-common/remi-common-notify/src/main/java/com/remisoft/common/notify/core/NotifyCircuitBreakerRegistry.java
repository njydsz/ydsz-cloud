 ...
 * 渠道熔断器注册中心（P0-3）
 *
 * <p>管理各通知渠道的熔断器实例，提供统一的熔断器访问入口。
 * 熔断器参数可通过 {@link com.remisoft.common.notify.config.NotifyProperties} 配置。
 *
 * @author remi-team
 * @since 1.0.0
 */
public class NotifyCircuitBreakerRegistry {
    ...
    private final ConcurrentMap<NotifyChannel, NotifyCircuitBreaker> breakers = new ConcurrentHashMap<>();
    ...
    public NotifyCircuitBreakerRegistry(int failureThreshold, long recoveryTimeoutMs) {
        ...
        log.info("[NotifyCircuitBreakerRegistry] 初始化完成, failureThreshold={}, recoveryTimeoutMs={}",
                failureThreshold, recoveryTimeoutMs);
    }
    ...
    public NotifyCircuitBreaker getBreaker(NotifyChannel channel) {
        return breakers.computeIfAbsent(channel,
                ch -> new NotifyCircuitBreaker(ch, failureThreshold, recoveryTimeoutMs));
    }
