...
| `AliyunSmsProvider` | 阿里云短信实现（`remi.notify.sms.provider=aliyun` 时激活） |
...
| `NotifyTemplateAutoConfiguration` | 模板自动配置（`remi.notify.template.enabled=true`，默认启用） |
...
```java
import com.remisoft.common.notify.helper.NotifyHelper;
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.notify.enabled` | true | 是否启用通知模块 |
| `remi.notify.template.enabled` | true | 是否启用模板引擎 |
| `remi.notify.template.base-path` | `classpath:notify-templates/` | 模板文件基础路径 |
| `remi.notify.template.cache-enabled` | true | 是否启用模板缓存 |
| `remi.notify.email.enabled` | true | 是否启用邮件渠道 |
| `remi.notify.email.smtp-host` | - | SMTP 主机地址 |
| `remi.notify.email.smtp-port` | 465 | SMTP 端口 |
| `remi.notify.email.from-mail` | - | 发件人邮箱 |
| `remi.notify.email.from-name` | - | 发件人显示名称 |
| `remi.notify.email.password` | - | 邮箱密码/授权码（支持 `ENC(xxx)` Jasypt 加密） |
| `remi.notify.email.auth` | true | 是否需要认证 |
| `remi.notify.email.starttls` | false | 是否启用 STARTTLS |
| `remi.notify.email.html-mode` | true | 是否以 HTML 模式发送 |
| `remi.notify.email.encoding` | UTF-8 | 编码格式 |
| `remi.notify.email.connection-timeout` | 10000 | 连接超时（ms） |
| `remi.notify.email.timeout` | 10000 | 读取超时（ms） |
| `remi.notify.email.write-timeout` | 10000 | 写入超时（ms） |
| `remi.notify.email.max-attachment-size-mb` | 20 | 最大附件总大小（MB） |
| `remi.notify.email.ssl.enabled` | true | 是否启用 SSL |
| `remi.notify.email.ssl.protocols` | TLSv1.2 | SSL 协议版本 |
| `remi.notify.email.security.password-encrypted` | false | 密码是否已加密 |
| `remi.notify.email.security.sanitize-html` | true | 是否启用 HTML 内容 XSS 过滤 |
| `remi.notify.email.tracking.enabled` | false | 是否启用已读追踪像素 |
| `remi.notify.email.tracking.pixel-base-url` | - | 追踪像素 Base URL |
| `remi.notify.email.dkim.enabled` | false | 是否启用 DKIM 签名 |
| `remi.notify.email.dkim.domain` | - | DKIM 域名 |
| `remi.notify.email.dkim.selector` | default | DKIM 选择器 |
| `remi.notify.email.dkim.private-key` | - | DKIM Base64 RSA 私钥 |
| `remi.notify.sms.enabled` | false | 是否启用短信渠道 |
| `remi.notify.sms.provider` | aliyun | 短信服务提供商 |
| `remi.notify.sms.endpoint` | - | API 端点地址 |
| `remi.notify.sms.access-key-id` | - | AccessKey ID |
| `remi.notify.sms.access-key-secret` | - | AccessKey Secret |
| `remi.notify.sms.sign-name` | - | 短信签名 |
| `remi.notify.sms.template-code` | - | 默认短信模板编码 |
| `remi.notify.sms.templates` | - | 模板映射（key=模板编码，value=模板ID） |
| `remi.notify.sms.timeout-ms` | 10000 | 发送超时（ms） |
| `remi.notify.sms.retry-count` | 2 | 失败重试次数 |
| `remi.notify.wecom.enabled` | false | 是否启用企业微信渠道 |
| `remi.notify.wecom.corp-id` | - | 企业 ID |
| `remi.notify.wecom.corp-secret` | - | 企业密钥 |
| `remi.notify.wecom.agent-id` | - | 应用 ID |
| `remi.notify.wecom.webhook-key` | - | Webhook 密钥 |
| `remi.notify.wecom.token-refresh-interval` | 7200 | Token 刷新间隔（秒） |
| `remi.notify.dingtalk.enabled` | false | 是否启用钉钉渠道 |
| `remi.notify.dingtalk.app-key` | - | 应用 Key |
| `remi.notify.dingtalk.app-secret` | - | 应用密钥 |
| `remi.notify.dingtalk.webhook-url` | - | Webhook 地址 |
| `remi.notify.dingtalk.webhook-secret` | - | Webhook 签名密钥 |
| `remi.notify.feishu.enabled` | false | 是否启用飞书渠道 |
| `remi.notify.feishu.app-id` | - | 应用 ID |
| `remi.notify.feishu.app-secret` | - | 应用密钥 |
| `remi.notify.feishu.webhook-url` | - | Webhook 地址 |
| `remi.notify.feishu.encrypt-key` | - | 加密密钥 |
| `remi.notify.insite.enabled` | true | 是否启用站内信渠道 |
| `remi.notify.insite.storage-type` | redis | 存储类型 |
| `remi.notify.insite.max-queue-size` | 10000 | 最大队列大小 |
| `remi.notify.insite.expire-minutes` | 1440 | 过期时间（分钟） |
| `remi.notify.fallback.enabled` | false | 是否启用渠道降级 |
| `remi.notify.fallback.chains` | - | 降级链映射（key=主渠道，value=备用渠道列表） |
| `remi.notify.dedup.enabled` | false | 是否启用去重 |
| `remi.notify.dedup.window-seconds` | 300 | 去重时间窗口（秒） |
| `remi.notify.dedup.redis-key-prefix` | `notify:dedup:` | Redis Key 前缀 |
| `remi.notify.retry-queue.capacity` | 10000 | 队列容量 |
| `remi.notify.retry-queue.max-retries` | 5 | 最大重试次数（0-10） |
| `remi.notify.retry-queue.batch-size` | 100 | 批量处理大小 |
| `remi.notify.retry-queue.persistent` | false | 是否使用 Redis 持久化重试队列（开启后服务重启不丢失） |
| `remi.notify.retry-queue.redis-key-prefix` | `notify:retry:` | Redis Key 前缀 |
| `remi.notify.rate-limit.enabled` | true | 是否启用限流 |
| `remi.notify.rate-limit.default-max-requests` | 100 | 默认最大请求数（每个渠道） |
| `remi.notify.rate-limit.default-window-seconds` | 60 | 默认时间窗口（秒） |
| `remi.notify.rate-limit.channel-limits` | - | 渠道级限流配置（key=渠道枚举，value=渠道限流配置） |
...
remi:
  notify:
    ...
    email:
      ...
      smtp-port: 465
      from-mail: noreply@remi.com
      from-name: remi项目管理平台
      ...
      tracking:
        enabled: true
        pixel-base-url: https://remi.example.com/api/notify/track/open
      dkim:
        enabled: true
        domain: remi.com
        selector: remi
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/notify` | 通知模块健康检查 | `spring-boot-health` 在类路径 + `remi.notify.enabled=true` |
...
1. **重试队列持久化**：`remi.notify.retry-queue.persistent=true` 时使用 Redis ZSET 持久化，服务重启不丢失待重试消息；`persistent=false` 时退化为内存队列，重启后丢失。多实例部署必须开启持久化。
