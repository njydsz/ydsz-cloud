package com.remisoft.common.search.metrics;
...
public class SearchMetrics {
    ...
    private void initMetrics() {
        ...
        searchCounter = Counter.builder("remi.search.requests")
                .description("Total search requests")
                .register(meterRegistry);
        zeroResultCounter = Counter.builder("remi.search.zero_results")
                .description("Search requests with zero results")
                .register(meterRegistry);
        searchTimer = Timer.builder("remi.search.duration")
                .description("Search duration")
                .register(meterRegistry);
        indexOpCounter = Counter.builder("remi.search.index_ops")
                .description("Total index operations")
                .register(meterRegistry);
        indexFailedCounter = Counter.builder("remi.search.index_failed")
    ...
    public void bindGauges(Supplier<Integer> cacheSizeSupplier, Supplier<Boolean> circuitOpenSupplier) {
        ...
        // 零结果率 Gauge
        Gauge.builder("remi.search.zero_result_rate", this, SearchMetrics::getZeroResultRate)
                .description("Search zero result rate")
                .register(meterRegistry);
        // 索引失败率 Gauge
        Gauge.builder("remi.search.index_failure_rate", this, SearchMetrics::getIndexFailureRate)
                .description("Index operation failure rate")
                .register(meterRegistry);
        // 缓存大小 Gauge
        if (cacheSizeSupplier != null) {
            Gauge.builder("remi.search.cache.size", cacheSizeSupplier, Supplier::get)
                    .description("Search cache entry count")
                    .register(meterRegistry);
        }
        // 熔断器状态 Gauge (1=open, 0=closed)
        if (circuitOpenSupplier != null) {
            Gauge.builder("remi.search.circuit_breaker.open", () -> circuitOpenSupplier.get() ? 1.0 : 0.0)
