 ...
 * 搜索策略 SPI
 * <p>
 * 所有搜索引擎必须实现此接口。通过策略模式，不同引擎（PG/ES/RediSearch/Solr/OpenSearch/Memory）
 * 可以自由替换，业务模块通过 {@code SearchEngineRegistry} 获取当前策略实例。
 *
 * <p>引擎可以按需额外实现 {@link IndexStrategy} 和 {@link SuggestStrategy}，
