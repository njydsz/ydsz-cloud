...
| `DefaultStorageFactory` | 默认工厂实现，按 `remi.file.type` 创建对应存储实例并单例缓存；支持 SPI 注册自定义存储类型 |
...
| `FileTypeValidator` | 文件类型校验器，支持后缀白名单 + Magic Number 双重校验，可由 `remi.file.check-magic-number` 关闭 |
...
<dependency>
    ...
    <artifactId>remi-common-file</artifactId>
</dependency>
```
...
### `remi.file.*`（FileProperties）

| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.file.enabled` | true | 是否启用文件存储模块 |
| `remi.file.type` | `minio` | 存储类型：local / minio / oss / aliyun / s3 / aws-s3 / qiniu / cos / tencent-cos / obs / huawei-obs |
| `remi.file.endpoint` | - | 存储服务 endpoint |
| `remi.file.access-key` | - | 访问密钥（建议环境变量注入） |
| `remi.file.secret-key` | - | 秘密密钥（建议环境变量注入） |
| `remi.file.bucket` | - | 默认存储桶名称 |
| `remi.file.domain` | - | 文件访问域名（CDN 或 Nginx 代理地址） |
| `remi.file.region` | - | 区域信息（部分云存储需要） |
| `remi.file.allowed-suffixes` | `[]`（空表示允许全部） | 允许上传的文件后缀白名单 |
| `remi.file.max-file-size` | `104857600`（100MB） | 单文件最大大小（字节） |
| `remi.file.max-request-size` | `104857600`（100MB） | 单次请求最大大小（字节） |
| `remi.file.connection-timeout` | `30000`（30s） | 连接超时（毫秒） |
| `remi.file.socket-timeout` | `60000`（60s） | Socket 超时（毫秒） |
| `remi.file.max-connections` | `100` | HTTP 连接池最大连接数 |
| `remi.file.retry-count` | `3` | 存储操作失败重试次数（0-10） |
| `remi.file.temporary-signature-expiry` | `3600`（1h） | 私有 URL 过期时间（秒） |
| `remi.file.part-size` | `5242880`（5MB） | 分片大小（字节，与 S3 协议对齐） |
| `remi.file.checkpoint-dir` | 系统临时目录 | 断点续传检查点目录 |
| `remi.file.check-magic-number` | true | 是否启用 Magic Number 文件头校验 |
| `remi.file.concurrency-control.enabled` | true | 是否启用上传并发保护 |
| `remi.file.concurrency-control.strategy` | `REJECT` | 并发冲突策略（REJECT / WAIT） |

### `remi.file.upload.*`（FileUploadProperties）

| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.file.upload.chunk-md5-check` | false | 是否启用分片 MD5 校验（启用后每次分片计算 MD5，合并时校验整体 MD5） |

### `remi.file.lifecycle.*`（FileLifecycleProperties）

| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.file.lifecycle.enabled` | false | 是否启用文件生命周期清理 |
| `remi.file.lifecycle.cron` | `0 0 2 * * ?` | 定时清理 Cron 表达式（默认每天凌晨 2 点） |
| `remi.file.lifecycle.bucket` | - | 目标存储桶（为空时使用默认桶） |
| `remi.file.lifecycle.dry-run` | false | 是否仅模拟执行（true 时只打印日志不实际删除） |
| `remi.file.lifecycle.rules[].prefix` | - | 文件路径前缀 |
| `remi.file.lifecycle.rules[].max-age-days` | - | 最大保留天数 |
| `remi.file.lifecycle.rules[].action` | `delete` | 到期动作（默认 delete） |
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/file` | 存储后端健康检查，由 `FileHealthIndicator` 注册 | `spring-boot-actuator` 在 classpath 且 `remi.file.enabled=true` |
...
3. **分片大小默认 5MB**：与 S3 协议对齐，过小会增加请求数，过大降低并发度与失败恢复效率。可通过 `remi.file.part-size` 调整。
...
6. **并发保护策略**：`UploadConcurrencyGuard` 仅在 Redis 可用且 `remi.file.concurrency-control.enabled=true` 时生效；`REJECT` 策略下冲突时直接抛异常，`WAIT` 策略下阻塞等待但可能超时失败。
...
9. **分片 MD5 校验默认关闭**：启用 `remi.file.upload.chunk-md5-check=true` 会增加内存与 CPU 开销，建议仅在高一致性场景启用。启用后流式累积 MD5 仅缓存 `MessageDigest` 状态（约 128 字节），不缓存原始分片数据，避免 OOM。
