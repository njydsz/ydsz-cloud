package com.remisoft.common.file.virus;
...
public class NoOpVirusScanner implements VirusScanner {
     ...
     * 空实现：直接放行，不读取也不关闭输入流。
     *
     * <p><b>流契约：</b>本实现<b>完全不消费</b> {@code inputStream}，
     * 因此流位置保持不变，调用方后续仍可从头读取并自行负责关闭。
     * 这一点与真实扫描引擎不同——真实引擎会读完整个流，
     * 调用方必须传入可重复读取的流（如包装为 {@code BufferedInputStream} 并 mark/reset）。
     *
     ...
     *
     * <p>若此处返回 {@code false}，上层会记录"扫描引擎不可用"告警，
     * 而默认场景下这属于噪音，故统一声明为可用。
     *
     * @return 恒为 {@code true}
     */
    @Override
