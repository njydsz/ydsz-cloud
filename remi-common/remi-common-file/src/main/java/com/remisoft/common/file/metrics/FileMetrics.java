package com.remisoft.common.file.metrics;
...
/**
 * 文件存储监控指标收集器
 * <p>基于 Micrometer 实现，收集文件上传/下载/删除、文件去重命中/未命中、
 * 病毒检测、上传/下载错误等监控指标。
 *
 * <p><b>监控指标列表：</b></p>
 * <ul>
 *   <li>{@code file.upload.count} - 上传总次数</li>
 *   <li>{@code file.download.count} - 下载总次数</li>
 *   <li>{@code file.delete.count} - 删除总次数</li>
 *   <li>{@code file.dedup.hit} - 秒传命中次数</li>
 *   <li>{@code file.dedup.miss} - 秒传未命中次数</li>
 *   <li>{@code file.virus.detected} - 病毒检测命中次数</li>
 *   <li>{@code file.upload.duration} - 上传耗时</li>
 *   <li>{@code file.download.duration} - 下载耗时</li>
 *   <li>{@code file.upload.errors} - 上传错误（按错误码标签区分）</li>
 *   <li>{@code file.download.errors} - 下载错误（按错误码标签区分）</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
