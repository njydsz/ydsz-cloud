...
import com.remisoft.common.file.storage.platform.CosStorage;
import com.remisoft.common.file.storage.platform.LocalStorage;
import com.remisoft.common.file.storage.platform.MinioStorage;
import com.remisoft.common.file.storage.platform.ObsStorage;
import com.remisoft.common.file.storage.platform.OssStorage;
import com.remisoft.common.file.storage.platform.QiniuStorage;
import com.remisoft.common.file.storage.platform.S3Storage;
 ...
 * 并将共享依赖（分片上下文存储、检查点服务、并发保护器、去重服务、病毒扫描、监控指标、重试助手）
 * 注入到每个存储实例中。
 *
 * <p><b>特性：</b></p>
 * <ul>
 *   <li>单例缓存：同一存储类型只创建一次实例</li>
 *   <li>支持 SPI 扩展：通过 {@link #register} 注册自定义存储类型</li>
 *   <li>依赖注入：创建实例后自动注入所有可选依赖</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
