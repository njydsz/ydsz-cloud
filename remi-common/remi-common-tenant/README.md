...
| **强依赖** | `remi-common-core`（TTL ThreadLocal）、`remi-common-jdbc`（InnerInterceptor SPI + MpBaseEntity + JSqlParser）、`remi-common-util`（AuthInfoUtils）、`remi-common-domain` |
| **可选依赖** | `remi-common-redis`、`remi-common-feign`、`remi-common-thread`、`spring-boot-starter-web`、`spring-boot-health` |
| **启用条件** | `remi.tenant.enabled=true`（默认 false，不启用时无任何租户逻辑） |
...
- `remi.tenant.table-column-mapping` 配置映射，key=表名（小写），value=列名
...
| 组件 | 说明 |
|---|---|
| `TenantLifecycleManager` | 静态工具类，管理租户上下线状态（内存 Map，生产环境应对接 `remi_tenant` 表 + Redis 缓存） |
...
<dependency>
    ...
    <artifactId>remi-common-tenant</artifactId>
</dependency>
```
...
`TenantAutoConfiguration` 在 `remi.tenant.enabled=true` 时装配以下 Bean（按条件触发）：
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.tenant.enabled` | `false` | 是否启用多租户（不引入依赖或设为 false 时无任何租户逻辑） |
| `remi.tenant.mode` | `SINGLE` | 隔离模式：`SINGLE` / `MULTI` / `ISOLATE_DB` |
| `remi.tenant.tenant-column` | `tenant_id` | 默认租户列名（`tenant-fields` 为空时使用） |
| `remi.tenant.default-claim` | `tenantId` | 默认 JWT claim 名（`tenant-fields` 为空时使用） |
| `remi.tenant.default-header` | `X-Tenant-Id` | 默认 HTTP header 名（`tenant-fields` 为空时使用） |
| `remi.tenant.super-tenant-id` | `0` | 超级管理员租户 ID（绕过隔离） |
| `remi.tenant.system-tenant-id` | `0` | 系统租户 ID（定时任务/异步/MQ） |
| `remi.tenant.tenant-fields` | 空 | 租户字段配置列表，每项含 `column` / `claim` / `header` / `multi-value` |
| `remi.tenant.table-column-mapping` | 空 | per-table 列名覆盖映射（key=表名小写，value=列名，覆盖第一个字段） |
| `remi.tenant.ignore-tables` | 空 | 忽略租户隔离的表列表（忽略大小写） |
| `remi.tenant.anon-urls` | 空 | URL 级白名单（跳过租户隔离的请求路径，前缀匹配） |
...
```java
import com.remisoft.common.tenant.SystemTenantContextRunner;
import com.remisoft.common.tenant.TenantContext;
import com.remisoft.common.tenant.TenantContextHolder;
import com.remisoft.common.tenant.annotation.TenantColumn;
import com.remisoft.common.tenant.audit.TenantAuditLogger;
import com.remisoft.common.tenant.redis.TenantAwareRedisKey;
...
| 扩展类 | 作用 | 覆盖方式 |
|---|---|---|
| `TenantDataSourceRouter` | 根据 `TenantContextHolder.getTenantId()` 路由到 `tenant_{tenantId}` 数据源 | 子类化重写 `resolveDatasourceKey(tenantId)`，从 `remi_tenant` 表查询 `datasource_key` 字段（当前为 TODO，使用约定命名） |

### 3. 配置隔离扩展点

| 扩展类 | 作用 | 覆盖方式 |
|---|---|---|
| `TenantConfigProvider` | per-tenant 配置覆盖（feature flag / 参数 / 阈值），优先级：per-tenant 覆盖 > 全局默认 | 编程式调用 `setOverride(tenantId, key, value)` 或 YAML 配置 `remi.tenant.overrides.{tenantId}.{key}` |
...
| Bean | 激活条件 | 覆盖方式 |
|---|---|---|
| `TenantInterceptorProvider` | `remi.tenant.enabled=true`（总是） | 提供同类型 Bean |
...
4. **ISOLATE_DB 模式**：当前 `TenantDataSourceRouter.resolveDatasourceKey` 使用 `tenant_{tenantId}` 命名约定，生产环境应从 `remi_tenant` 表查询 `datasource_key` 字段（代码中已标注 TODO）。
5. **生命周期管理**：`TenantLifecycleManager` 当前使用内存 Map，多实例部署时状态不同步，生产环境应对接 `remi_tenant` 表 + Redis 缓存。
