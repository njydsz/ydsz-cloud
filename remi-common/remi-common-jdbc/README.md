...
| `MapperScanConfiguration` | Mapper 扫描配置，扫描包由 `remi.jdbc.mapper-scan-packages` 指定（默认 `com.remisoft.**.mapper`） |
...
| `DynamicDataSourceAutoConfiguration` | 动态数据源自动配置（`remi.jdbc.dynamic-datasource.enabled=true`，默认启用） |
...
| `FieldFillConfiguration` | 字段填充配置（`remi.jdbc.field-fill.*`） |

### 5. 逻辑删除

| 类 | 说明 |
|---|---|
| `LogicalDeleteConfiguration` | 逻辑删除配置（`remi.jdbc.logical-delete.*`） |
...
| 类 | 说明 |
|---|---|
| `OptimisticLockConfiguration` | 乐观锁配置（`remi.jdbc.optimistic-lock.*`） |
...
| 类 | 说明 |
|---|---|
| `DataPermissionConfiguration` | 数据权限配置（`remi.jdbc.data-permission.*`） |
...
| 类 | 说明 |
|---|---|
| `SqlFirewallProperties` | 配置属性（`remi.jdbc.sql-firewall.*`） |
...
| `DatabaseCircuitBreakerAutoConfiguration` | 自动配置（`remi.jdbc.circuit-breaker.enabled=true`） |
| `CircuitBreakerProperties` | 配置属性（`remi.jdbc.circuit-breaker.*`） |
...
| 类 | 说明 |
|---|---|
| `HikariCPConfiguration` | HikariCP 配置，优先使用 `remi.jdbc.hikari.*`，未配置时回退到 Spring Boot 默认；支持连接池预热与 Micrometer 指标注册 |
...
| 类 | 说明 |
|---|---|
| `TenantIsolationProperties` | 租户隔离配置（`remi.jdbc.tenant-isolation.*`），支持 SINGLE/MULTI/ISOLATE_DB 三种模式 |
...
<dependency>
    ...
    <artifactId>remi-common-jdbc</artifactId>
</dependency>
```
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.jdbc.enabled` | true | 是否启用 JDBC 模块 |
| `remi.jdbc.mapper-scan-packages` | `com.remisoft.**.mapper` | Mapper 扫描包路径数组 |
| `remi.jdbc.hikari.*` | - | HikariCP 连接池配置（优先于 spring.datasource.hikari.*） |
| `remi.jdbc.tenant-isolation.enabled` | true | 租户隔离开关 |
| `remi.jdbc.tenant-isolation.mode` | SINGLE | 租户隔离模式（SINGLE/MULTI/ISOLATE_DB） |
| `remi.jdbc.tenant-isolation.ignore-tables` | - | 忽略租户隔离的表 |
| `remi.jdbc.tenant-isolation.anon-urls` | - | 跳过租户隔离的 URL 白名单 |
| `remi.jdbc.data-permission.enabled` | true | 数据权限开关 |
| `remi.jdbc.pagination.max-limit` | - | 最大分页大小（安全加固） |
| `remi.jdbc.pagination.db-type` | - | 显式指定数据库类型 |
| `remi.jdbc.dynamic-datasource.enabled` | true | 动态数据源开关 |
| `remi.jdbc.read-write-splitting.enabled` | false | 读写分离开关 |
| `remi.jdbc.read-write-splitting.master-ds` | master | 主库数据源名称 |
| `remi.jdbc.read-write-splitting.slave-ds-list` | `[slave]` | 从库数据源名称列表 |
| `remi.jdbc.read-write-splitting.load-balance-strategy` | round-robin | 负载均衡策略（round-robin/random/weighted） |
| `remi.jdbc.read-write-splitting.weights` | - | 从库权重映射（weighted 策略生效） |
| `remi.jdbc.slow-sql.enabled` | false | 慢 SQL 检测开关 |
| `remi.jdbc.slow-sql.threshold-millis` | 1000 | 慢 SQL 阈值（ms） |
| `remi.jdbc.slow-sql.alert-threshold-millis` | 3000 | 慢 SQL 告警阈值（ms） |
| `remi.jdbc.sql-audit.enabled` | false | SQL 审计开关 |
| `remi.jdbc.sql-audit.audit-select` | false | 是否审计 SELECT |
| `remi.jdbc.sql-audit.audit-insert` | true | 是否审计 INSERT |
| `remi.jdbc.sql-audit.audit-update` | true | 是否审计 UPDATE |
| `remi.jdbc.sql-audit.audit-delete` | true | 是否审计 DELETE |
| `remi.jdbc.sql-audit.log-parameters` | true | 是否记录 SQL 参数 |
| `remi.jdbc.sql-audit.max-parameter-length` | 500 | 参数最大长度 |
| `remi.jdbc.sql-audit.exclude-tables` | - | 排除审计的表名列表 |
| `remi.jdbc.sql-audit.exclude-methods` | - | 排除审计的 Mapper 方法名列表 |
| `remi.jdbc.sql-firewall.enabled` | false | SQL 防火墙开关 |
| `remi.jdbc.sql-firewall.block-drop-table` | true | 拦截 DROP 操作 |
| `remi.jdbc.sql-firewall.block-truncate` | true | 拦截 TRUNCATE 操作 |
| `remi.jdbc.sql-firewall.block-delete-without-where` | true | 拦截无 WHERE 的 DELETE |
| `remi.jdbc.sql-firewall.block-update-without-where` | true | 拦截无 WHERE 的 UPDATE |
| `remi.jdbc.sql-firewall.block-multi-statement` | true | 拦截分号多语句 |
| `remi.jdbc.sql-firewall.block-permission-ops` | true | 拦截 GRANT/REVOKE |
| `remi.jdbc.sql-firewall.allow-tables` | - | DROP/TRUNCATE 表白名单 |
| `remi.jdbc.circuit-breaker.enabled` | false | 数据库熔断开关 |
| `remi.jdbc.circuit-breaker.failure-threshold` | 10 | 连续失败次数阈值 |
| `remi.jdbc.circuit-breaker.open-duration-millis` | 30000 | 熔断持续时间（ms） |
| `remi.jdbc.circuit-breaker.half-open-probe-size` | 3 | 半开探测请求数 |
...
```java
import com.remisoft.common.jdbc.permission.DataPermissionIgnore;
