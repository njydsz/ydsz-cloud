package com.remisoft.common.jdbc.monitor;
 ...
 * <p>暴露的指标：
 * <ul>
 *   <li>{@code remi.jdbc.route.total} — 路由总次数（tag: datasource, sql_type, route_reason）</li>
 *   <li>{@code remi.jdbc.route.master} — 路由到主库次数（tag: sql_type）</li>
 *   <li>{@code remi.jdbc.route.slave} — 路由到从库次数（tag: datasource, sql_type）</li>
 *   <li>{@code remi.jdbc.route.transaction_forced} — 事务强制走主库次数</li>
 *   <li>{@code remi.jdbc.sql.execution.time} — SQL 执行耗时（tag: datasource, sql_type）</li>
 * </ul>
 *
 * <p>典型 Prometheus 查询示例：
 * <pre>
 * # 读写比例
 * sum(rate(remi.jdbc.route.total{route_reason="slave"}[5m]))
 * /
 * sum(rate(remi.jdbc.route.total[5m]))
 *
 * # 事务强制走主库比例
 * sum(rate(remi.jdbc.route.transaction_forced[5m]))
 * /
 * sum(rate(remi.jdbc.route.total[5m]))
 * </pre>
 *
...
public class ReadWriteSplittingMetrics {
    ...
    private static final String PREFIX = "remi.jdbc";
