...
public abstract class BaseCorsProperties {
     ...
     * 是否允许发送 Cookie 等凭证信息
     *
     * <p>默认值为 false。设为 true 时 allowedOriginPatterns 不能包含 "*"，
     * 浏览器会拒绝这种不安全组合。
     */
     ...
     * 允许的跨域来源模式列表
     *
     * <p>支持通配符模式，例如 "https://*.example.com"。
     * 推荐明确指定允许的域名，避免使用 "*" 带来的安全风险。
     */
     ...
     *
     * <p>默认值为 ["*"]，表示允许所有方法。
     * 生产环境建议显式列出 GET/POST/PUT/DELETE 等。
     */
     ...
     *
     * <p>默认为空，即不暴露额外响应头。
     * 常见需要暴露的头：X-Request-Id, X-Total-Count, Content-Disposition 等。
     */
     ...
     * <p>检查项：
     * <ul>
     *   <li>allowCredentials=true 且 allowedOriginPatterns 包含 "*"（CSRF 风险）</li>
     *   <li>CORS 启用但 allowedOriginPatterns 为空（静默失败）</li>
     *   <li>allowedOriginPatterns 包含 "*" 且未限制方法/头（过度开放）</li>
     * </ul>
     *
