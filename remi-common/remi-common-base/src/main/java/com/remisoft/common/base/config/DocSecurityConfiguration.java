...
public class DocSecurityConfiguration {
    ...
    private void checkProductionSecurity(DocProperties props) {
        ...
        if (isProduction) {
            if (!props.isProductionEnabled()) {
                log.warn("【文档安全】生产环境检测到 remi.doc.enabled=true 但 production-enabled=false，"
     ...
     * <p>包含以下路径：
     * <ul>
     *   <li>Knife4j 文档入口（{@code remi.doc.knife4j-path}）</li>
     *   <li>OpenAPI 文档 JSON（{@code remi.doc.api-docs-path} 及其子路径）</li>
