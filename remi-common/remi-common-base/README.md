...
> REMI Web/App 公共 HTTP 基座模块（L6 应用层）
...
| 类 | 说明 |
|---|---|
| `BaseTimezoneConfiguration` | 时区配置抽象基类，`@PostConstruct` 强制将 JVM 默认时区设置为配置值（默认 `Asia/Shanghai`，UTC+8），保证全局时间一致性；通过 `remi.base.timezone` 自定义 |
...
| 类 | 说明 |
|---|---|
| `BaseAutoConfiguration` | 总自动配置（`@AutoConfiguration`），激活条件：`@ConditionalOnWebApplication` + `remi.base.enabled=true`（默认 true）；注册 `TraceFilter`、`SecurityHeadersFilter`（兜底）、`RequestContextCleanupFilter`、`BaseHealthIndicator`（需 actuator 依赖） |

### 7. OpenAPI 文档配置

| 类 | 说明 |
|---|---|
| `BaseOpenApiConfiguration` | OpenAPI 文档配置抽象基类，子类覆盖 `getTitle()` 和 `getDescription()`；默认注册所有公共请求头（X-User-Id、X-Tenant-Id、X-Access-Token 等）、JWT Bearer Token 认证方案、统一联系信息；激活条件 `remi.doc.enabled=true` |
| `OpenApiAutoConfiguration` | OpenAPI 自动配置，`@ConditionalOnClass(SpringDocConfiguration)` + `remi.doc.enabled=true`；支持单分组（默认匹配 `/**`）和多分组模式（按 `packages` 或 `paths` 列表扫描）；当业务模块通过继承 `BaseOpenApiConfiguration` 提供自定义 `OpenAPI` Bean 时，本 Bean 自动退出 |
| `Knife4jAutoConfiguration` | Knife4j 增强 UI 自动配置，`@ConditionalOnClass(Knife4jOpenApiCustomizer)` + `remi.doc.enabled=true`；提供离线文档导出、全局参数配置、增强搜索分组等能力 |
| `DocAutoConfiguration` | 文档模块入口配置，激活条件 `remi.doc.enabled=true`（默认 false）；`@Import` 激活 `OpenApiAutoConfiguration`、`Knife4jAutoConfiguration`、`DefaultDocExporter`、`MarkdownDocExporter` |
| `DocProperties` | 文档配置属性（`remi.doc`），包含 `enabled`、`productionEnabled`、`basicAuth`、`apiDocsPath`、`knife4jPath`、`docVersion`、`info`（OpenApiInfo）、`groups`（GroupConfig 列表）、`export`（ExportConfig） |
| `DocSecurityConfiguration` | 文档安全配置，生产环境通过 `remi.doc.production-enabled=true` 启用 Basic 认证保护；构造阶段检测 Profile 包含 `prod`/`production` 时输出安全告警；`DocBasicAuthFilter` 使用 `MessageDigest.isEqual` 恒定时间比较防止时序攻击 |
...
| `ConfigRegistryEndpoint` | Actuator 端点（`@Endpoint(id="config-registry")`），暴露 `GET /actuator/config-registry` 查看所有 `remi.*` 配置，`GET /actuator/config-registry/{prefix}` 查看指定前缀下的配置 |
...
<dependency>
    ...
    <artifactId>remi-common-base</artifactId>
</dependency>
```
...
@ConfigurationProperties(prefix = "remi.custom.cors")
public class CustomCorsProperties extends BaseCorsProperties {
}

@ConfigurationProperties(prefix = "remi.custom.trace")
...
| 配置 | 默认值 | 说明 |
|---|---|---|
| `remi.base.enabled` | true | 是否启用 base 模块 |
| `remi.base.timezone` | `Asia/Shanghai` | JVM 默认时区 |
| `remi.base.security-headers.enabled` | true | 是否启用安全响应头 |
| `remi.base.security-headers.xss-protection` | `1; mode=block` | XSS 防护头部 |
| `remi.base.security-headers.content-type-options` | `nosniff` | 内容类型选项头部 |
| `remi.base.security-headers.frame-options` | `DENY` | 帧选项头部（DENY/SAMEORIGIN） |
| `remi.base.security-headers.hsts` | `max-age=31536000; includeSubDomains` | 严格传输安全头部 |
| `remi.base.security-headers.csp` | `default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'` | 内容安全策略头部 |
| `remi.base.security-headers.referrer-policy` | `strict-origin-when-cross-origin` | 引用策略头部 |
| `remi.base.security-headers.excludes` | `[]` | 排除路径列表（Ant 风格） |
| `remi.base.trace.enabled` | true | 是否启用链路追踪（子模块前缀为 `remi.web.trace` / `remi.app.trace`） |
| `remi.base.trace.response-header-enabled` | true | 是否在响应头输出请求 ID |
| `remi.base.trace.request-id-header-name` | `X-Request-Id` | 请求 ID 响应头名称 |
| `remi.base.trace.request-log-enabled` | true | 是否启用请求日志 |
| `remi.base.trace.request-log-format` | `detailed` | 请求日志格式（simple/detailed） |
| `remi.base.trace.log-request-params` | true | 是否记录请求参数 |
| `remi.base.trace.log-request-body` | false | 是否记录请求体 |
| `remi.base.trace.log-response-body` | false | 是否记录响应体 |
| `remi.base.trace.log-level` | `INFO` | 日志级别（INFO/DEBUG） |
| `remi.base.trace.sampling-rate` | 1.0 | 日志采样率 [0.0, 1.0]，超范围自动修正 |
| `remi.base.trace.slow-request-threshold` | 3000 | 慢请求阈值（ms） |
| `remi.doc.enabled` | false | 是否启用文档功能（默认关闭） |
| `remi.doc.production-enabled` | false | 生产环境是否允许访问文档 |
| `remi.doc.api-docs-path` | `/v3/api-docs` | OpenAPI 文档 JSON 路径 |
| `remi.doc.knife4j-path` | `/doc.html` | Knife4j 文档访问路径 |
| `remi.doc.doc-version` | 1.0.0 | 文档版本号 |
| `remi.doc.info.title` | `REMI API 文档` | 文档标题 |
| `remi.doc.info.description` | `REMI 公共框架 API 文档` | 文档描述 |
| `remi.doc.info.version` | 1.0.0 | API 版本 |
| `remi.doc.groups` | `[]` | 分组配置列表（空时单分组模式） |
| `remi.doc.export.enabled` | true | 是否启用文档导出 |
| `remi.doc.export.format` | `json` | 默认导出格式（json/yaml/html/markdown） |
| `remi.doc.export.output-dir` | `./api-docs` | 导出目录 |
| `remi.doc.basic-auth.enabled` | true | 是否启用 Basic 认证 |
| `remi.doc.basic-auth.username` | - | API 文档访问用户名（建议 `${DOC_USERNAME}`） |
| `remi.doc.basic-auth.password` | - | API 文档访问密码（建议 `${DOC_PASSWORD}` 或 Jasypt 加密） |
...
```java
import com.remisoft.common.base.exporter.AbstractDocExporter;
...
| `BaseCorsProperties` | CORS 配置前缀 | `common-web`（`remi.web.cors`）、`common-app`（`remi.app.cors`） |
| `BaseTraceProperties` | Trace 配置前缀 | `common-web`（`remi.web.trace`）、`common-app`（`remi.app.trace`） |
...
| `BaseOpenApiConfiguration` | OpenAPI 文档标题/描述 | `common-web`、`common-app`（标题=`REMI App API 文档`） |
...
| `AbstractModuleMetrics` | 模块指标前缀 | 各业务模块（如 `FlowMetrics` 前缀=`remi_flow_`） |
...
| 端点 | 说明 | 触发条件 |
|---|---|---|
| `/actuator/health/base` | base 模块健康指标，报告时区、安全响应头、文档功能状态 | `spring-boot-health` 在类路径 + `remi.base.enabled=true` |
| `/actuator/config-registry` | 配置注册端点，查看所有 `remi.*` 配置项 | `spring-boot-actuator` 在类路径 + 端点暴露配置 |
| `/actuator/config-registry/{prefix}` | 查看指定前缀下的配置项 | 同上 |

`BaseHealthIndicator` 暴露信息：

- `timezone` — 当前 JVM 时区
- `timezone.expected` — 期望时区（`remi.base.timezone`）
...
4. **文档功能默认关闭**：出于安全考虑，`remi.doc.enabled` 默认为 `false`。生产环境建议保持关闭，或配合 `remi.doc.production-enabled=true` + `remi.doc.basic-auth.enabled=true` 进行认证保护。
...
6. **`ConfigRegistryEndpoint` 安全**：此端点暴露所有 `remi.*` 配置信息，生产环境应通过 `management.endpoint.config-registry.exposure` 控制访问权限，建议仅限内网访问。
...
9. **`AbstractModuleMetrics` 前缀管理**：各业务模块的 Metrics 类继承本基类，通过构造器传入模块前缀（如 `remi_flow_`、`remi_msg_`），自动拼接到所有指标名称前，避免各模块硬编码重复字符串。

## 变更记录

- **v1.0.0**（2026-08-02）：按 remi-common-jdbc 9 章节标准重构 README；补全横切点执行顺序表、SPI 扩展点（含抽象基类扩展点）、健康检查端点、注意事项；统一版本号为 1.0.0
