...
/**
 * REMI Span 构建器（业务友好的 OTel API 封装）
 *
 * <p>对 OTel API 的链式 Builder 进行业务向封装，提供：
 * <ul>
 *   <li>统一注入租户 / 用户 / 业务单号 / 灰度标签等 REMI 自定义属性</li>
 ...
 * <pre>{@code
 * // 业务 Span
 * RemiSpan span = RemiSpan.builder(tracer, "order.create")
 ...
 */
@Slf4j
public final class RemiSpan {

    private final Span span;
    private final long startNanos;

    private RemiSpan(Span span) {
        this.span = span;
     ...
     * 设置属性
     */
    public RemiSpan setAttribute(String key, String value) {
        if (value != null) {
     ...
     */
    public RemiSpan setAttribute(String key, long value) {
        span.setAttribute(key, value);
        return this;
    }
     ...
     * @return 当前 Span 包装（链式调用）
     */
    public RemiSpan setAttribute(String key, boolean value) {
        span.setAttribute(key, value);
        return this;
    }
     ...
     * @return 当前 Span 包装（链式调用）
     */
    public RemiSpan setAttribute(AttributeKey<Long> key, long value) {
        span.setAttribute(key, value);
        return this;
    }
     ...
     * @return 当前 Span 包装（链式调用）
     */
    public RemiSpan setAttribute(AttributeKey<String> key, String value) {
        if (value != null) {
            span.setAttribute(key, value);
        }
        return this;
     ...
     * 批量设置属性
     */
    public RemiSpan setAttributes(Map<String, Object> attrs) {
     ...
     * 记录异常
     */
    public RemiSpan recordException(Throwable t) {
        if (t != null) {
     ...
     * 标记为错误并设置状态
     */
    public RemiSpan error(String errorCode, String message) {
        if (errorCode != null) {
    ...
    public void end() {
        try {
            ...
            log.debug("[RemiSpan] 结束 Span 失败: {}", e.getMessage());
        }
    }
    ...
    public static class Builder {
         ...
         *
         * <p>不调用时沿用当前线程 {@link Context} 中的 Span；异步场景下线程上下文已丢失，
         * 必须在提交任务前捕获父 Span 并在此传入，否则会断链生成孤立 Trace。
         *
         ...
         * 追加自定义字符串属性。
         *
         * <p>属性基数（cardinality）过高会显著增加后端存储与查询开销，
         * 不要把 UUID、时间戳等无限取值写入 key 或 value。
         *
         ...
         * 启动 Span
         */
        public RemiSpan start() {
            return new RemiSpan(builder.startSpan());
        }
    }
    ...
    public static <T> T run(Tracer tracer, String name, Supplier<T> action) {
        RemiSpan span = builder(tracer, name).start();
        try (Scope ignored = span.scope()) {
            T result = action.get();
            span.ok();
            return result;
