 ...
 * REMI Span 属性增强器
 *
 * <p>Span 结束前自动注入 REMI 业务上下文属性，无需业务方手动 setAttribute。
 * 支持从多种来源读取：
 * <ul>
 ...
 */
@Slf4j
public class RemiSpanEnrichmentProcessor implements SpanProcessor {

    private final EnrichmentConfig config;

    public RemiSpanEnrichmentProcessor(EnrichmentConfig config) {
        this.config = config;
        log.info("[Sentry] RemiSpanEnrichmentProcessor 初始化，来源：{}", config.getSources());
    }
    ...
    private void applyMdc(ReadWriteSpan span) {
        ...
        if (tenantId != null && tenantId.length() < 64) {
            span.setAttribute(OtelSemConv.REMI_TENANT_ID, tenantId);
        }
        String userId = mdc.get("userId");
        if (userId != null) {
            span.setAttribute(OtelSemConv.REMI_USER_ID, userId);
        }
        String businessNo = mdc.get("businessNo");
        if (businessNo != null) {
            span.setAttribute(OtelSemConv.REMI_BUSINESS_NO, businessNo);
        }
    }
