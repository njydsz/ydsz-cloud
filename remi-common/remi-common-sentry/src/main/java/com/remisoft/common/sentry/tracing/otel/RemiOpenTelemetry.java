...
/**
 * REMI OpenTelemetry 全局访问点 + 上下文传播工具
 *
 * <p>提供：
 * <ul>
 ...
 * <p>典型用法：
 * <pre>{@code
 * Tracer tracer = RemiOpenTelemetry.tracer("remi-order");
 *
 * // 注入到下游请求头
 * RemiOpenTelemetry.inject(Map.of(), (carrier, key, value) -> carrier.put(key, value));
 *
 * // 从上游请求头提取
 * RemiOpenTelemetry.extract(headers, Map::get);
 * }</pre>
 *
 * @author remi-team
 * @since 1.0.0
 */
@Slf4j
public final class RemiOpenTelemetry {

    /** 默认 Tracer 名称 */
    public static final String DEFAULT_INSTRUMENTATION_NAME = "remi";
    ...
    private RemiOpenTelemetry() {
        throw new UnsupportedOperationException("RemiOpenTelemetry is a utility class");
    }
    ...
    public static <C> void inject(C carrier, TextMapSetter<C> setter) {
        try {
            ...
            log.debug("[RemiOpenTelemetry] inject 失败: {}", e.getMessage());
        }
    }
    ...
    public static <C> Context extract(C carrier, TextMapGetter<C> getter) {
        try {
            ...
            log.debug("[RemiOpenTelemetry] extract 失败: {}", e.getMessage());
