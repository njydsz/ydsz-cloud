package com.remisoft.common.sentry.alerting;
 ...
 * 告警通知处理器
 *
 * <p>将 {@link AlertEvent} 转换为通知消息，通过 {@link NotifyService} 发送到对应渠道。
 * 根据告警级别路由：
 * <ul>
 *   <li>P0 - 钉钉（立即通知）</li>
 *   <li>P1 - 钉钉（5 分钟聚合由 AlertConverger 控制）</li>
 *   <li>P2 - 邮件</li>
 *   <li>P3 - 仅记录，不通知</li>
 * </ul>
 *
...
public class NotifyAlertHandler implements DefaultAlertPublisher.AlertHandler {
    ...
    public void handle(AlertEvent event) {
        ...
        try {
            ...
            log.warn("[Sentry] 告警通知发送失败: severity={}, name={}, err={}",
                    event.getSeverity(), event.getName(), e.getMessage());
        }
    }
    ...
    private String buildTitle(AlertEvent event) {
        return String.format("[REMI-P%s] %s",
    ...
    private String buildContent(AlertEvent event) {
        ...
        sb.append("| **摘要** | ").append(event.getSummary() != null ? event.getSummary() : "").append(" |\n");
        sb.append("| **详情** | ").append(event.getDescription() != null ? event.getDescription() : "").append(" |\n");
        sb.append("| **分类** | ").append(event.getCategory() != null ? event.getCategory() : "").append(" |\n");
