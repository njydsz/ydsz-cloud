package com.remisoft.common.sentry.metrics;
 ...
 * <p>采集指标：
 * <ul>
 *   <li>remi.system.cpu.usage - CPU 使用率（0.0~1.0）</li>
 *   <li>remi.system.cpu.load_avg - CPU 平均负载</li>
 *   <li>remi.system.memory.heap.used - 堆内存已用（bytes）</li>
 *   <li>remi.system.memory.heap.max - 堆内存最大（bytes）</li>
 *   <li>remi.system.memory.non_heap.used - 非堆内存已用（bytes）</li>
 *   <li>remi.system.disk.free - 磁盘可用空间（bytes）</li>
 *   <li>remi.system.disk.total - 磁盘总空间（bytes）</li>
 *   <li>remi.system.process.uptime - 进程运行时长（秒）</li>
 *   <li>remi.system.process.thread_count - 线程数</li>
 *   <li>remi.system.process.gc.count - GC 总次数</li>
 *   <li>remi.system.process.gc.time - GC 总耗时（毫秒）</li>
 * </ul>
 *
 * @author remi-team
 * @since 1.0.0
...
public class SystemMetricsCollector {
    ...
    private void collectCpuMetrics() {
        if (osMxBean instanceof com.sun.management.OperatingSystemMXBean sunOs) { // FQN-OK: name conflict with java.lang.management.OperatingSystemMXBean
            ...
            if (cpuUsage >= 0) {
                metricsCollector.setGauge("remi.system.cpu.usage",
                        "CPU 使用率", null, cpuUsage);
            }
            double systemCpuLoad = sunOs.getSystemCpuLoad();
            if (systemCpuLoad >= 0) {
                metricsCollector.setGauge("remi.system.cpu.system_usage",
                        "系统 CPU 使用率", null, systemCpuLoad);
            }
        }
        metricsCollector.setGauge("remi.system.cpu.load_avg",
                "CPU 平均负载", null, osMxBean.getSystemLoadAverage());
        metricsCollector.setGauge("remi.system.cpu.available_processors",
                "可用 CPU 核数", null, osMxBean.getAvailableProcessors());
    }

    private void collectMemoryMetrics() {
        MemoryUsage heapUsage = memoryMxBean.getHeapMemoryUsage();
        metricsCollector.setGauge("remi.system.memory.heap.used",
                "堆内存已用", null, heapUsage.getUsed());
        metricsCollector.setGauge("remi.system.memory.heap.max",
                "堆内存最大", null, heapUsage.getMax());
        metricsCollector.setGauge("remi.system.memory.heap.committed",
                "堆内存已分配", null, heapUsage.getCommitted());

        MemoryUsage nonHeapUsage = memoryMxBean.getNonHeapMemoryUsage();
        metricsCollector.setGauge("remi.system.memory.non_heap.used",
                "非堆内存已用", null, nonHeapUsage.getUsed());
        metricsCollector.setGauge("remi.system.memory.non_heap.committed",
    ...
    private void collectDiskMetrics() {
        ...
        if (roots != null) {
            for (int i = 0; i < roots.length; i++) {
                File root = roots[i];
                metricsCollector.setGauge("remi.system.disk.free",
                        "磁盘可用空间", Map.of("mount", root.getAbsolutePath()),
                        root.getFreeSpace());
                metricsCollector.setGauge("remi.system.disk.total",
                        "磁盘总空间", Map.of("mount", root.getAbsolutePath()),
                        root.getTotalSpace());
                metricsCollector.setGauge("remi.system.disk.usable",
    ...
    private void collectProcessMetrics() {
        metricsCollector.setGauge("remi.system.process.uptime",
                "进程运行时长（秒）", null, runtimeMxBean.getUptime() / 1000.0);
        metricsCollector.setGauge("remi.system.process.thread_count",
                "线程数", null, threadMxBean.getThreadCount());
        metricsCollector.setGauge("remi.system.process.daemon_thread_count",
                "守护线程数", null, threadMxBean.getDaemonThreadCount());
        metricsCollector.setGauge("remi.system.process.peak_thread_count",
    ...
    private void collectGcMetrics() {
        ...
        metricsCollector.setGauge("remi.system.process.gc.count",
                "GC 总次数", null, totalGcCount);
        metricsCollector.setGauge("remi.system.process.gc.time",
        ...
        if (gcDelta > 0) {
            metricsCollector.incrementCounter("remi.system.process.gc.time_delta",
