...
import com.remisoft.common.sentry.tracing.otel.ErrorEventSpanProcessor;
import com.remisoft.common.sentry.tracing.otel.OtelExporterFactory;
import com.remisoft.common.sentry.tracing.otel.OtelResources;
...
@ConditionalOnProperty(prefix = "remi.sentry.tracing.otel", name = "enabled", havingValue = "true", matchIfMissing = false)
...
public class OtelAutoConfiguration {
    ...
    /**
     * 提供全局共享的默认 {@link Tracer}，instrumentation scope 固定为 {@code remi}。
     *
    ...
    @Bean
    @ConditionalOnMissingBean
    public Tracer remiDefaultTracer(OpenTelemetry openTelemetry) {
        return openTelemetry.getTracer("remi");
    }
