 ...
 * Micrometer 指标采集器。
 *
 * <p>基于 Micrometer {@link MeterRegistry} 实现指标采集，自动暴露到 Prometheus。
 * 当 MeterRegistry 不可用时自动降级为 {@link InMemoryMetricsCollector}。
 *
 * <h3>支持的指标类型</h3>
 * <ul>
 *   <li>Counter：单调递增计数器（如请求总量、错误总量）</li>
 *   <li>Gauge：瞬时值指标（如队列大小、活跃连接数），使用 {@link AtomicReference} 动态更新</li>
 *   <li>Timer：耗时指标（如请求延迟），支持 SLO（50/100/250/500/1000/5000ms）百分位</li>
 *   <li>DistributionSummary：分布统计（如消息大小、缓存值大小），支持 SLO（1/5/10/50/100/500/1000）</li>
 * </ul>
 *
 * <h3>缓存设计</h3>
 * <p>使用 {@link ConcurrentHashMap} 缓存已注册的 Counter/Timer/DistributionSummary/Gauge，
 * 避免重复注册（Micrometer 不允许重复注册同名指标）。
 * 缓存 Key 为 {@code name + tags.toString()}，保证相同 name+tags 复用同一实例。
 *
 * <h3>降级策略</h3>
 * <p>若 MeterRegistry 为 null 或指标注册失败（如并发冲突），
 * 自动降级到内存采集器，不影响业务流程。
 *
 * @author remi-team
 * @since 1.0.0
...
public class MicrometerMetricsCollector implements MetricsCollector {
     ...
     * 递增 Counter 指标。
     *
     * <p>先检查 MeterRegistry 是否可用，不可用时降级到内存采集器。
     * 使用缓存避免重复注册同名 Counter。
     *
     * @param name        指标名称
     ...
     * 设置 Gauge 指标值。
     *
     * <p>Gauge 使用 {@link AtomicReference<Double>} 作为动态数据源，
     * 每次调用更新引用值，无需重复注册 Gauge。
     * 若首次调用则注册 Gauge 并缓存引用。
     *
     * @param name        指标名称
     * @param description 指标描述（仅首次注册时使用）
     * @param tags        指标标签（影响缓存 Key）
     ...
     * 记录 Timer 耗时指标。
     *
     * <p>自动记录 P50/P90/P95/P99 百分位，并按 SLO 区间统计分布。
     * 使用缓存避免重复注册同名 Timer。
     *
     * @param name        指标名称
     * @param description 指标描述（仅首次注册时使用）
     ...
     *
     * <p>适用于记录数值型分布数据（如消息大小、缓存值大小、队列长度）。
     * 自动按 SLO 区间统计分布。
     *
     * @param name        指标名称
     * @param description 指标描述（仅首次注册时使用）
     ...
     * 构建指标缓存 Key。
     *
     * <p>Key 格式为 {@code name + "|" + tags.toString()}，
     * 保证相同 name + tags 组合复用同一指标实例。
     *
     * @param name  指标名称
     * @param tags  指标标签
     * @return 缓存 Key 字符串
