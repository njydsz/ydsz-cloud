...
import com.remisoft.common.sentry.domain.SlaDefinition;
...
public class DefaultSlaCollector implements SlaCollector {
    ...
    public void register(SlaDefinition definition) {
        ...
        log.info("[Sentry] SLA 注册: name={}, threshold={}ms, target={}",
    ...
    public void record(String name, String stepName, long tookMillis, boolean success) {
        ...
        if (def != null) {
            ...
            metricsCollector.recordTimer("remi.sla.step.duration",
                    "SLA 步骤耗时", tags, Duration.ofMillis(tookMillis));
            metricsCollector.incrementCounter("remi.sla.step.total",
                    "SLA 步骤总执行次数", tags, 1);

            if (!success) {
                metricsCollector.incrementCounter("remi.sla.step.failed",
                        "SLA 步骤失败次数", tags, 1);
            }

            if (step != null && tookMillis > step.getTimeoutMillis()) {
                metricsCollector.incrementCounter("remi.sla.step.timeout",
                        "SLA 步骤超时次数", tags, 1);
            }
        }
    }
    ...
    public void recordTotal(String name, long tookMillis, boolean success) {
        ...
        metricsCollector.recordTimer("remi.sla.total.duration",
                "SLA 总耗时", tags, Duration.ofMillis(tookMillis));
        metricsCollector.incrementCounter("remi.sla.total.count",
                "SLA 执行总次数", tags, 1);

        if (!success) {
            metricsCollector.incrementCounter("remi.sla.total.failed",
                    "SLA 失败次数", tags, 1);
        }

        if (tookMillis > def.getThresholdMillis()) {
            metricsCollector.incrementCounter("remi.sla.violation",
                    "SLA 违反次数",
                    Map.of("sla", name),
                    1);
            log.warn("[Sentry] SLA 违反: name={}, took={}ms, threshold={}ms",
                    name, tookMillis, def.getThresholdMillis());
        }
    }
