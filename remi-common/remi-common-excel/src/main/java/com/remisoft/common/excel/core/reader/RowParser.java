package com.remisoft.common.excel.core.reader;
...
import com.remisoft.common.excel.converter.ConverterChain;
...
import com.remisoft.common.excel.support.asm.ASMFieldAccessor.FieldSetter;
import com.remisoft.common.excel.support.asm.ASMFieldAccessor.ObjectInstantiator;
 ...
 * <ul>
 *   <li>解析单行数据，映射为实体对象或Map</li>
 *   <li>高性能空行检查</li>
 *   <li>构建检查列索引数组</li>
 *   <li>单元格值类型转换（通过ConverterChain SPI）</li>
 * </ul>
 *
 * @author remi-team
...
public class RowParser {
     ...
     * <ul>
     *   <li>指定Class - 返回映射的实体对象</li>
     *   <li>未指定Class - 返回Map&lt;String, Object&gt;</li>
     * </ul>
     *
     * @param row Excel行对象
     * @param headers 表头列表
     ...
     * @return 解析后的数据对象
     */
    public Object parseRow(Row row, List<String> headers, Map<Integer, Field> fieldMap, ColumnMetadata[] columnMetadataArray) {
        ...
        try {
            ...
            if (columnMetadataArray != null) {
                ...
                for (Map.Entry<Integer, Field> entry : fieldMap.entrySet()) {
                    ...
                    FieldSetter setter = fieldSetterCache.computeIfAbsent(col, k ->
                        ReflectCache.getFieldSetter(metadata.getClazz(), field));
     ...
     * 高性能空行检查 - 仅检查有映射的列
     *
     * <p>相比原始的isEmptyRow方法，此方法只检查我们需要映射的列，
     * 避免遍历整个Excel行中的所有单元格，显著提升性能。</p>
     *
     * @param row 行对象
     ...
     * 构建检查列索引数组
     *
     * <p>提取所有需要映射的列索引，用于快速空行检查。
     * 避免检查无关的列，提升跳过空行场景下的性能。</p>
     *
     * @param columnMetadataArray 列元数据数组
     * @return 需要检查的列索引数组
     */
     ...
     *
     * <p>从Cell中提取原始值，然后委托给ConverterChain进行类型转换。
     * Cell值提取逻辑保留在RowParser中，类型转换逻辑由SPI转换器链处理。</p>
     *
     * @param cell 单元格对象
     ...
     * <ul>
     *   <li>STRING -> String</li>
     *   <li>NUMERIC(日期) -> java.util.Date</li>
     *   <li>NUMERIC(数值) -> Double</li>
     *   <li>BOOLEAN -> Boolean</li>
     *   <li>FORMULA -> 递归提取缓存结果</li>
     *   <li>ERROR -> Byte(错误码)</li>
     * </ul>
     *
     * @param cell 单元格对象
     * @return 提取的原始值
