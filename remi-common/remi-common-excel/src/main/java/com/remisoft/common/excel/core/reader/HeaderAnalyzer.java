 ...
 * <ul>
 *   <li>解析类元数据，建立列索引与字段的映射</li>
 *   <li>解析无类型映射模式下的表头</li>
 *   <li>表头名称匹配（支持自动trim）</li>
 * </ul>
 *
 * @author remi-team
...
public class HeaderAnalyzer {
     ...
     * <p>映射规则(按优先级):
     * <ol>
     *   <li>被@ExcelIgnore标记的字段会被忽略</li>
     *   <li>优先使用index指定列索引</li>
     *   <li>其次使用@ExcelProperty的value作为列名</li>
     *   <li>最后使用字段名作为列名</li>
     *   <li>列名匹配采用精确匹配策略</li>
     * </ol>
     *
     ...
     * @return 列元数据数组
     */
    public ColumnMetadata[] analyzeClassMetadata(Row headRow, List<String> headers, Map<Integer, Field> fieldMap) {
        ...
        for (Map.Entry<Integer, Field> entry : fieldMap.entrySet()) {
            ...
            columnMetadataArray[idx++] = new ColumnMetadata(col, setter, targetType, dateFormat, automaticTrim);
        }
     ...
     * 分析表头(无类型映射模式)
     *
     * <p>当未指定Class时,直接读取表头名称作为Map的key。
     * 每列的名称取自表头单元格的字符串值。</p>
     *
     * @param headRow 表头行
     * @param headers 表头名称列表(出参)
     ...
     *
     * <p>支持精确匹配和自动trim后的匹配。
     * 当ExcelConfig配置了automaticTrim时，会先trim再比较。</p>
     *
     * @param header 表头名称
