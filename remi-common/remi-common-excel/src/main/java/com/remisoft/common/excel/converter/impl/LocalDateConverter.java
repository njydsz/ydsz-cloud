...
public class LocalDateConverter implements CellValueConverter {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(LocalDateConverter.class);
    ...
    private LocalDate parseLocalDateString(String dateStr, ConvertContext context) {
        ...
        try {
            ...
            try {
                DateTimeFormatter formatter = FORMATTER_CACHE.computeIfAbsent("yyyy-MM-dd",
                    DateTimeFormatter::ofPattern);
                return LocalDate.parse(dateStr, formatter);
            } catch (Exception e2) {
                return null;
