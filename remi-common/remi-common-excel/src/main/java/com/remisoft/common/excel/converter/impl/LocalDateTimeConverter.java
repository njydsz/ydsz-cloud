package com.remisoft.common.excel.converter.impl;
 ...
 * LocalDateTime类型转换器
 *
 * <p>处理目标类型为LocalDateTime的转换。支持从String、Date、Double等原始值转换。</p>
 *
 * @author remi-team
...
public class LocalDateTimeConverter implements CellValueConverter {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(LocalDateTimeConverter.class);
    ...
    private LocalDateTime parseLocalDateTimeString(String dateStr, ConvertContext context) {
        ...
        try {
            ...
            try {
                DateTimeFormatter formatter = FORMATTER_CACHE.computeIfAbsent("yyyy-MM-dd HH:mm:ss",
                    DateTimeFormatter::ofPattern);
                return LocalDateTime.parse(dateStr, formatter);
            } catch (Exception e2) {
                return null;
