...
PMIS 提供统一的密钥管理抽象（`SecretProvider`），业务代码通过 `SecretManager` 获取密钥。
密钥来源由 `pmis.kms.provider` 决定，支持以下三种方式：

### 方式一：环境变量注入（推荐生产）

通过环境变量注入密钥，环境变量名转换规则：密钥标识 → `PMIS_SECRETS_` 前缀 + 大写 + 点号转下划线。

| 密钥标识 | 环境变量名 |
| --- | --- |
| `db.password` | `PMIS_SECRETS_DB_PASSWORD` |
| `redis.password` | `PMIS_SECRETS_REDIS_PASSWORD` |
| `jwt.secret` | `PMIS_SECRETS_JWT_SECRET` |
...
将 `pmis.kms.provider` 设为 `jasypt`，在 `pmis.kms.secrets.*` 中配置 ENC() 密文，
...
当前预留 `pmis.kms.vault` 配置结构与 `SecretProvider` 接口扩展点，
