...
# 一键安装(默认安装到 C:\ydsz,数据 C:\ydsz-data,日志 C:\ydsz-logs)
.\deploy\windows\install-ydsz-infra.ps1
```
...
```powershell
.\deploy\windows\install-ydsz-infra.ps1 -NoStart                          # 只安装不启动
.\deploy\windows\install-ydsz-infra.ps1 -Skip "es,minio"                  # 跳过指定中间件
.\deploy\windows\install-ydsz-infra.ps1 -InstallHome D:\ydsz              # 自定义安装目录
.\deploy\windows\install-ydsz-infra.ps1 -DataHome D:\ydsz-data -LogHome D:\ydsz-logs
.\deploy\windows\install-ydsz-infra.ps1 -Uninstall                        # 卸载
```
...
| Redis data | `C:\ydsz\data\redis\` |
| Nacos data | `C:\ydsz\nacos\data\` |
| MinIO data | `C:\ydsz\data\minio\` |
| RocketMQ data | `C:\ydsz\data\rocketmq\` |
| XXL-Job 日志 | `C:\ydsz\logs\xxl-job.log` |

`install-ydsz-infra.ps1` 启动时会询问 `DataHome` / `LogHome`,可自定义。

---

## 7. Windows 服务名

`install-ydsz-infra.ps1` 会注册 9 个服务(8 中间件 + rocketmq 拆为 2 个):
...
| 现象 | 排查命令 |
|---|---|
| 服务起不来 | `Get-Service ydsz-* \| Format-Table Name, Status` |
...
| Nacos 502 | `Get-Content C:\ydsz\logs\nacos.log -Tail 50` |
