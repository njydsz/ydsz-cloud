...
function Start-Svc {
    ...
    if ($state -eq 'Running') { Write-Step "$label 已在运行"; return }
    if ($state -eq 'NotInstalled') { Write-Err "$label 服务未安装（$name）"; return }
    Write-Step "启动 $label..."
    try {
...
function Stop-Svc {
    param($name, $label)
    $state = Get-SvcState $name
    if ($state -eq 'Stopped' -or $state -eq 'NotInstalled') { Write-Step "$label 已停止或未安装"; return }
    Write-Step "停止 $label..."
    try {
