...
for module_name in "${MODULES[@]}"; do
  MODULE_DIR="${COMMON_DIR}/${module_name}"

  if [[ ! -d "${MODULE_DIR}" ]]; then
    echo -e "${YELLOW}[WARN] 模块目录不存在：${MODULE_DIR}（跳过）${NC}"
    continue
  fi
  ...
  check "4. native-image.json 反射类名含 Ydsz" \
        'com\.njydsz\.pmis\.[^"]*Ydsz' \
        "*.json" \
        "${MODULE_DIR}"

  check "5. README.md / pom.xml 出现 Ydsz 品牌类名" \
        'Ydsz[A-Z]\w*' \
        "*.md" \
        "${MODULE_DIR}"
...
if [[ ${violations} -gt 0 ]]; then
  ...
  echo ""
  echo "修复指引："
  echo "  1. 公开类名:  YdszXxx → 去品牌化类名（同步修改文件名）"
