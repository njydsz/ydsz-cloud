...
CREATE INDEX idx_file_node_parent ON nw_file_node (parent_id) WHERE deleted = 0;
CREATE INDEX idx_file_node_path ON nw_file_node (path) WHERE deleted = 0;
CREATE INDEX idx_file_node_hash ON nw_file_node (file_hash) WHERE deleted = 0;
CREATE INDEX idx_file_node_created_by ON nw_file_node (created_by) WHERE deleted = 0;
CREATE INDEX idx_file_node_starred ON nw_file_node (created_by, starred) WHERE deleted = 0 AND starred = TRUE;
...
CREATE INDEX idx_file_version_node ON nw_file_version (file_node_id);
CREATE INDEX idx_file_version_active ON nw_file_version (file_node_id, is_active) WHERE deleted = 0;
...
CREATE INDEX idx_share_link_file ON nw_share_link (file_node_id) WHERE deleted = 0;
CREATE INDEX idx_share_link_user ON nw_share_link (created_by) WHERE deleted = 0 AND status = 'active';
...
CREATE INDEX idx_file_acl_node ON nw_file_acl (file_node_id) WHERE deleted = 0;
CREATE INDEX idx_file_acl_grantee ON nw_file_acl (grantee_type, grantee_id) WHERE deleted = 0;
...
CREATE INDEX idx_file_tag_node ON nw_file_tag (file_node_id) WHERE deleted = 0;
CREATE INDEX idx_file_tag_tag ON nw_file_tag (tag_id) WHERE deleted = 0;
...
CREATE UNIQUE INDEX idx_quota_scope ON nw_storage_quota (scope_type, scope_id) WHERE deleted = 0;
...
CREATE INDEX idx_trash_user ON nw_trash_item (created_by) WHERE deleted = 0 AND status = 'in_trash';
CREATE INDEX idx_trash_purge ON nw_trash_item (purge_time) WHERE deleted = 0 AND status = 'in_trash';
...
CREATE INDEX idx_search_name ON nw_search_index USING gin (to_tsvector('simple', name));
CREATE INDEX idx_search_content ON nw_search_index USING gin (to_tsvector('simple', content));
CREATE INDEX idx_search_user ON nw_search_index (created_by) WHERE deleted = 0;
