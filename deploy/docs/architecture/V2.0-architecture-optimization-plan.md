...
| 跨模块集成度 | ★★★★☆ | ★★★★★ | Feign 有效链路已覆盖 8 个 Client、13 条服务间链路；事件驱动体系仍待完善 |
...
| **注解重复/冗余** | `@RateLimit` 逐个方法标注，FlowTaskController 出现 22 次；ConfigController 存在重复 `@Idempotent` | `FlowTaskController.java:86-87,102-103,...`；`ConfigController.java:87-88` |
...
| **Feign 接口闲置** | 6 个 project Feign Client（`ProjectContractClient`、`EvmMeasureClient`、`ExecutionTimeEntryClient`、`ExecutionWbsClient`、`RateCardClient`、`FinanceClient`）+ 1 个 literule Feign Client（`LiteRuleClient`）定义但零调用 | `ConfigClient`、`AppInfoClient`、`ProjectInitiationClient` 已被 system/project/cronjob/agent 使用 |
...
| P0-1.4 | 禁止 `@RateLimit` 与 `@Idempotent` 在同方法重复标注 | ArchUnit + 自定义 PMD 规则 | 2人日 |
...
**现状**：`@RateLimit` 在 FlowTaskController 中 22 次重复标注，同一 resource 多方法重复定义 threshold；ConfigController 存在方法上两个 `@Idempotent` 注解。
...
| P0-2.3 | 消除冗余 `@RateLimit`，改为默认全局 Sentinel 规则 | 在 Nacos/Dashboard 配置中心声明规则，Controller 仅保留 `@RateLimit` 于特殊阈值方法 | 3人日 |
...
│  4. 将 @RateLimit 改为 Sentinel 规则中心   │
...
**现状**：当前有效 Feign Client 8 个（`NotificationClient`、`OrgQueryClient`、`ConfigClient`、`AppInfoClient`、`ProjectInitiationClient`、`WorkflowServiceClient`、`CronjobServiceClient`、`LiteRuleClient`），另有 6 个 project Client 与 1 个 literule Client 定义但零使用。
...
| A-003 | @RateLimit 冗余标注 | P0 | ydsz-workflow | M1 结束 |
