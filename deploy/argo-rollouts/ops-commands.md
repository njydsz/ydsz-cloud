...
#
# 适用: PMIS 14 个后端微服务 + 1 个前端 (gateway 是路由层, 不参与金丝雀)
...
kubectl argo rollouts promote pmis-execution -n pmis-prod
...
## 4. 验证当前 Rollout 状态
kubectl argo rollouts get rollout pmis-execution -n pmis-prod
...
kubectl argo rollouts get rollout pmis-execution -n pmis-prod -o yaml | grep -A 3 status
# 7.2 强制跳过分析 (应急, 不推荐)
kubectl argo rollouts set image pmis-execution execution=... --skip-analytics=true -n pmis-prod
# 7.3 完全跳过金丝雀, 直接全量 (DANGEROUS)
kubectl argo rollouts set image pmis-execution execution=... --full-promotion=true -n pmis-prod
...
## 9. 故障排查
# 9.1 Rollout 卡住不前进
kubectl argo rollouts get rollout pmis-execution -n pmis-prod -o yaml
